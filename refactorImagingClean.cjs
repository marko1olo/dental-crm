...
function refactorImaging() {
  ...
  // Now properly replace `parseImagingManifest(input)` inside the `commitImagingImport` functions
  code = code.replace(/export async function commitImagingImport\(([^]*?)const preview = await parseImagingManifest\(input\);/g, 'export async function commitImagingImport($1const preview = await parseImagingManifest(orgId, input);');
  code = code.replace(/export async function commitDicomImagingImport\(([^]*?)const preview = await parseDicomImagingManifest\(input\);/g, 'export async function commitDicomImagingImport($1const preview = await parseDicomImagingManifest(orgId, input);');
