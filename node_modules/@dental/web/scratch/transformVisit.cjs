...
const oldDictationRegex = /\s*\{\/\* Умная диктовка \*\/\}\s*<div className="smart-ai-booking"[^>]*>[\s\S]*?<\/div>\s*<\/div>\s*\{\/\* Статусные иконки/g;
if (oldDictationRegex.test(content)) {
    content = content.replace(oldDictationRegex, '\n              {/* Статусные иконки');
    console.log('Removed old dictation block');
} else {
    // try a more generic replacement
    const oldDictationBlock = /\s*\{\/\* Умная диктовка \*\/\}\s*<div className="smart-ai-booking"[^>]*>[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g;
    content = content.replace(oldDictationBlock, '');
}

// 2. Add chips under the top smart input
const topInputBlock = /(<div className="smart-ai-booking"[^>]*>[\s\S]*?<div className="smart-ai-input-wrapper"[^>]*>[\s\S]*?<\/div>)\s*(<\/div>)/;
const chipsHtml = `
              </div>
