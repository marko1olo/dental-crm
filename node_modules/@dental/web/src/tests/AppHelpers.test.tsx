...
import { assertSpeechChunkDbStores, requiredSpeechChunkDbStoreNames } from '../AppHelpers';

describe('assertSpeechChunkDbStores', () => {
  it('does not throw when all required stores are present', () => {
    const mockDb = {
      objectStoreNames: {
        contains: (storeName: string) => requiredSpeechChunkDbStoreNames.includes(storeName as any),
      }
  ...
  it('throws an error when one required store is missing', () => {
    ...
    assert.throws(
      () => assertSpeechChunkDbStores(mockDb),
      {
  ...
  it('throws an error when multiple required stores are missing', () => {
    const missingStores = [requiredSpeechChunkDbStoreNames[0], requiredSpeechChunkDbStoreNames[1]];
    const mockDb = {
      objectStoreNames: {
        contains: (storeName: string) => {
