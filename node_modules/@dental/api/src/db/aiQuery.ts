...
import type { AiRecognitionJob, CreateAiRecognitionJobInput } from "@dental/shared";
...
export async function listAiRecognitionJobsFromDb(organizationId: string): Promise<AiRecognitionJob[]> {
  const jobs = await db
    .select()
    .from(aiJobs)
...
export async function createAiRecognitionJobInDb(organizationId: string, input: CreateAiRecognitionJobInput): Promise<AiRecognitionJob> {
