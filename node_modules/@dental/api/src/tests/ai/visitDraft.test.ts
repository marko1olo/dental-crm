  ...
  test('falls back to rule-based parser when AI returns invalid JSON even inside curly braces', async () => {
    ...
    mock.method(globalThis, 'fetch', async () => {
      return {
        ok: true,
        json: async () => ({
          choices: [{ message: { content: "Here is the response: { invalid-json: no quotes }" } }]
        })
      } as any;
    });
    ...
    assert.ok(result.warnings);
    assert.ok(result.warnings.some(w => w.includes('ИИ-генерация черновика не выполнена, применен локальный разбор')));
    assert.strictEqual(result.complaint, 'пациента на острую боль в области 36 зуба');
  });
