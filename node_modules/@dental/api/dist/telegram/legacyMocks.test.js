...
describe('safeDenteTelegramPublicHttpsUrl', () => {
    test('returns valid URL string for a valid https URL', () => {
        assert.strictEqual(safeDenteTelegramPublicHttpsUrl('testField', 'https://example.com/path'), 'https://example.com/path');
    });
    test('returns null for an invalid URL string', () => {
        assert.strictEqual(safeDenteTelegramPublicHttpsUrl('testField', 'not-a-valid-url'), null);
    });
    test('returns null for a non-https URL', () => {
        assert.strictEqual(safeDenteTelegramPublicHttpsUrl('testField', 'http://example.com/path'), null);
    });
    ...
    test('returns null for URL with username/password', () => {
        assert.strictEqual(safeDenteTelegramPublicHttpsUrl('testField', 'https://user:pass@example.com'), null);
    });
});
