...
describe("buildPatientImportIntake", () => {
    ...
    test("processes unstructured text and normalizes it", () => {
        ...
        assert.ok(result.normalizedText.includes("ФИО;Телефон;Дата рождения;Комментарий"));
        assert.ok(result.normalizedText.includes("Иванов Иван Иванович;+79001234567;1990-01-01;Иванов Иван Иванович 89001234567 01.01.1990 жалоба на боль"));
    ...
    test("processes image_ocr source and appends recognition notes", () => {
        ...
        assert.ok(result.recognitionNotes[2].includes("Фото журнала должно проходить OCR/vision worker; этот endpoint принимает распознанный текст и нормализует его."));
    });
    test("processes voice_dictation source and appends recognition notes", () => {
        ...
        const result = buildPatientImportIntake(input);
        assert.strictEqual(result.recognitionNotes.length, 3);
        assert.ok(result.recognitionNotes[2].includes("Диктовка превращается в текст браузером или AI-worker, затем разбирается тем же безопасным preview."));
    });
