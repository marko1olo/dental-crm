...
import { auditEvents, clinicalRules, createClinicalRule, } from "../sampleData.js";
describe('createClinicalRule', () => {
    let originalClinicalRules;
    let originalAuditEvents;
    beforeEach(() => {
    ...
    test('fails validation when requiredServiceIds is empty for add_required_service action', () => {
        ...
        import { buildBillingSummary, treatmentPlanItems, payments, documents, serviceCatalog, serviceCatalogMap } from '../sampleData.js';
        import type { TreatmentPlanItem, Payment, GeneratedDocument, ServiceCatalogItem } from '@dental/shared';
        ...
        describe('buildBillingSummary', () => {
            ...
            function createMockPlanItem(overrides = {}) {
                return {
                    id: randomUUID(),
                        ...
                        function createMockService(overrides = {}) {
                            ...
                            it('should calculate zeros for empty state', () => {
                                ...
                                assert.deepStrictEqual(summary, {
                                    ...
                                    treatmentPlanItems, : .push(createMockPlanItem({ unitPriceRub: 2000, quantity: 1, discountRub: 0, status: 'approved' }), createMockPlanItem({ unitPriceRub: 1500, quantity: 2, discountRub: 500, status: 'in_progress' }), createMockPlanItem({ unitPriceRub: 3000, quantity: 1, discountRub: 0, status: 'cancelled' }) // Should be ignored
                                    ),
                                    const: summary = buildBillingSummary(),
                                    ...
                                    treatmentPlanItems, : .push(createMockPlanItem({ unitPriceRub: 2000, quantity: 1, discountRub: 0, status: 'cancelled' })),
                                    const: summary = buildBillingSummary(),
                                    ...
                                    treatmentPlanItems, : .push(createMockPlanItem({ unitPriceRub: 2000, quantity: 1, discountRub: 0, status: 'completed' })),
                                    const: summary = buildBillingSummary(),
                                    assert, : .strictEqual(summary.totalPlannedRub, 2000),
                                    assert, : .strictEqual(summary.openTreatmentItems, 0),
                                    it() { }
                                }(), {
                                    payments, : .push(createMockPayment({ amountRub: 1000, status: 'paid' }), createMockPayment({ amountRub: 500, status: 'planned' }), // Should be ignored
                                    ...
                                    documents, : .push(createMockDocument({ totalAmountRub: 1500, status: 'draft' }), createMockDocument({ totalAmountRub: 500, status: 'issued' }), // Should be ignored
                                    ...
                                    documents, : .push(createMockDocument({ id: doc1Id, totalAmountRub: 1000, status: 'draft' }), // unpaid
                                    ...
                                    payments, : .push(createMockPayment({ documentId: doc2Id, status: 'paid' })),
                                    const: summary = buildBillingSummary(),
                                    ...
                                    treatmentPlanItems, : .push(createMockPlanItem({ serviceId: service1.id, unitPriceRub: 2000, quantity: 1, status: 'completed' }), createMockPlanItem({ serviceId: service2.id, unitPriceRub: 3000, quantity: 1, status: 'completed' })),
                                    const: summary = buildBillingSummary(),
                                    ...
                                    treatmentPlanItems, : .push(createMockPlanItem({ serviceId: service1.id, unitPriceRub: 2500, quantity: 1, status: 'completed' })),
                                    const: summary = buildBillingSummary(),
