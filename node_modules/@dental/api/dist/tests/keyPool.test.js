...
describe('sanitizeProviderErrorMessage', () => {
    test('redacts Bearer tokens', () => {
        assert.strictEqual(sanitizeProviderErrorMessage('Invalid token: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIi...'), 'Invalid token: Bearer [redacted]');
    });
    test('redacts Token tokens', () => {
        assert.strictEqual(sanitizeProviderErrorMessage('Authorization failed. Token abcdef1234567890'), 'Authorization failed. Token [redacted]');
    });
    test('redacts Authorization headers', () => {
        assert.strictEqual(sanitizeProviderErrorMessage('Headers: Authorization: Basic QWxhZGRpbjpvcGVuIHNlc2FtZQ=='), 'Headers: Authorization: [redacted]');
        assert.strictEqual(sanitizeProviderErrorMessage('Request failed: Authorization: Bearer some-secret-token'), 'Request failed: Authorization: [redacted]');
    });
    test('redacts query parameters (api_key, token, access_token)', () => {
        assert.strictEqual(sanitizeProviderErrorMessage('URL: https://api.example.com/v1/data?api_key=secret-key-123 & other=1'), 'URL: https://api.example.com/v1/data?api_key=[redacted] & other=1');
        assert.strictEqual(sanitizeProviderErrorMessage('Error at /endpoint?token=abc-123&foo=bar'), 'Error at /endpoint?token=[redacted]&foo=bar');
        assert.strictEqual(sanitizeProviderErrorMessage('Failed fetching ?access_token=super_secret_token_val'), 'Failed fetching ?access_token=[redacted]');
    });
    test('redacts secrets, passwords in key=value formats', () => {
        assert.strictEqual(sanitizeProviderErrorMessage('Database connection failed: password=supersecretpassword123'), 'Database connection failed: password=[redacted]');
        assert.strictEqual(sanitizeProviderErrorMessage('Error: secret: my-secret-value-12345'), 'Error: secret=[redacted]');
    });
