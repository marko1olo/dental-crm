    ...
    test('happy path - builds export with valid state file', async () => {
        ...
        test('error path - file missing', async () => {
            ...
            test('error path - file missing', () => {
                ...
                assert.strictEqual(result.error, "Файл состояния еще не создан; выполните рабочее изменение и повторите проверку резервной копии." // persistenceWarningText("state_file_missing")
                );
                assert.ok(result.integrity);
            ...
            test('error path - invalid JSON', async () => {
                test('error path - invalid JSON', () => {
                    ...
                    assert.strictEqual(result.exportVersion, 1);
                    assert.strictEqual(result.payload, null);
                    assert.strictEqual(result.error, "Файл состояния не читается; используйте последнюю читаемую резервную копию и проверьте права сервера." // persistenceWarningText("state_file_unreadable")
                    );
                    assert.ok(result.integrity);
                    assert.strictEqual(result.integrity.ok, false);
