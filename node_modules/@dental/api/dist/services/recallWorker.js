...
async function processOverdueRecalls() {
    ...
    const overdue = await db
        ...
        .where(and(eq(schedulerReservations.phase, 2), // Surgical phase
    lte(schedulerReservations.recallDueAt, now), isNull(schedulerReservations.recallTriggeredAt)));
    if (overdue.length === 0)
        return;
...
function buildPatientMessage(patientName, jaw) {
    return (`Уважаемый(ая) ${patientName}! ` +
