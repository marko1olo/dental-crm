    ...
    if (model.labelOutlineProperties.length > 0) {
      ...
      for (let i = 0; i < model.currentValidInputs.length; i++) {
        ...
        if (program.isUniformUsed(uniformName)) {
          ...
          const minSpacing = Math.min(Math.abs(inputSpacing[0]), Math.abs(inputSpacing[1]), Math.abs(inputSpacing[2]));
          model._tmpTexelSize[0] = minSpacing / (inputDims[0] * Math.abs(inputSpacing[0]));
          model._tmpTexelSize[1] = minSpacing / (inputDims[1] * Math.abs(inputSpacing[1]));
          model._tmpTexelSize[2] = minSpacing / (inputDims[2] * Math.abs(inputSpacing[2]));
