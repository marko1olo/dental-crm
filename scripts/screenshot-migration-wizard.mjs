...
console.log(
	`Образец: ${dbfPath}, ${(dbf.length / 1024).toFixed(1)} КБ, 120 записей в cp866\n`,
);
...
try {
	...
	const loginField = page
		.locator(
			'input[type="email"], input[name="login"], input[placeholder*="почт" i]',
		)
		.first();
	...
	if (await onboardingDemo.isVisible().catch(() => false)) {
		...
		for (let attempt = 0; attempt < 8; attempt += 1) {
			const next = page
				.getByRole("button", {
					name: /Далее|Продолжить|Готово|Начать работу|Пропустить/i,
				})
				.first();
		...
		await page.screenshot({
			path: path.join(OUT_DIR, "01b-после-первого-запуска.png"),
		});
	}
	 ...
	 */
	await page.goto(`${WEB_URL}/#settings/imports`, {
		waitUntil: "domcontentloaded",
	});
	await page.waitForTimeout(2500);

	// Если приложение всё же осталось на другой вкладке, доводим кликом.
	const importsTab = page
		.getByRole("button", { name: /Импорт|Перенос/i })
		.first();
	...
	if (!wizardVisible) {
		await page.screenshot({
			path: path.join(OUT_DIR, "02-мастер-не-найден.png"),
			fullPage: true,
		});
	...
	check(
		"видна зона перетаскивания",
		(dropTitle ?? "").includes("Перетащите"),
		dropTitle ?? "",
	);
	...
	await page
		.locator(".mw-mapping-table")
		.waitFor({ state: "visible", timeout: 45_000 });
	await page.waitForTimeout(600);

	const encodingText = await page.locator(".mw-source-meta").textContent();
	check(
		"показана кодировка из заголовка DBF",
		(encodingText ?? "").includes("ibm866"),
		encodingText ?? "",
	);

	const mappedRows = await page
		.locator(".mw-mapping-row:not(.mw-mapping-head)")
		.count();
	check(
		"карта соответствия заполнена",
		mappedRows >= 4,
		`строк в карте: ${mappedRows}`,
	);

	const targets = await page.locator(".mw-col-target").allTextContents();
	check(
		"ФИО сопоставлено",
		targets.includes("patient.fullName"),
		targets.join(", "),
	);
	check(
		"телефон сопоставлен",
		targets.includes("patient.phone"),
		targets.join(", "),
	);
	...
	await wizard.screenshot({
		path: path.join(OUT_DIR, "03-карта-соответствия.png"),
	});
	...
	check(
		"сверка сошлась",
		(verdictText ?? "").includes("сошлась") &&
			!(verdictText ?? "").includes("НЕ"),
		verdictText ?? "",
	);
	...
	console.log(
		`       счётчики: ${counters.map((text) => text.replace(/\s+/g, " ").trim()).join(" | ")}`,
	);

	const checkTitles = await page
		.locator(".mw-check-body strong")
		.allTextContents();
	check(
		"показаны проверки сверки",
		checkTitles.length >= 4,
		`${checkTitles.length} проверок`,
	);

	const failedChecks = await page.locator(".mw-check.is-bad").count();
	check(
		"непройденных проверок нет",
		failedChecks === 0,
		`непройдено: ${failedChecks}`,
	);
	...
	check(
		"кнопка записи появилась после сухого прогона",
	...
	console.log(
		`       панель в тёмной теме: фон ${contrast?.background}, текст ${contrast?.color}`,
	);
	check(
		"панель не белая в тёмной теме",
		contrast?.background !== "rgb(255, 255, 255)",
		contrast?.background ?? "нет",
	);
	 ...
	 */
	const noise =
		/favicon|ResizeObserver|Download the React DevTools|Failed to load resource/i;
	const foreign = /SettingsImportsTab|Умный разбор/i;
	const relevantErrors = consoleErrors.filter(
		(text) => !noise.test(text) && !foreign.test(text),
	);
	const foreignErrors = consoleErrors.filter((text) => foreign.test(text));

	check(
		"нет ошибок JavaScript в мастере переноса",
	...
	await page
		.screenshot({ path: path.join(OUT_DIR, "99-ошибка.png"), fullPage: true })
		.catch(() => undefined);
} finally {
	await browser.close();
	console.log(`\nСкриншоты: ${OUT_DIR}`);
