...
async function run() {
  ...
  await page.screenshot({ path: path.join(basePath, 'Settings_Access_Dark.png'), fullPage: true });

  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'light' }]);
  await wait(1000);

  await page.screenshot({ path: path.join(basePath, 'Settings_Access_Light.png'), fullPage: true });

  // Mobile
  await page.setViewport({ width: 375, height: 812, isMobile: true });
  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'dark' }]);
  await wait(1000);
  await page.screenshot({ path: path.join(basePath, 'Settings_Access_Mobile_Dark.png'), fullPage: true });

  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'light' }]);
  await wait(1000);
  await page.screenshot({ path: path.join(basePath, 'Settings_Access_Mobile_Light.png'), fullPage: true });
