...
const { documentIssueBlockReason, renderDocumentHtml } = await import(
	pathToFileURL(rendererPath).href
);
const documentMigrationSql = readdirSync(path.resolve("apps/api/drizzle"))
	.filter((fileName) => fileName.endsWith(".sql"))
	.map((fileName) =>
		readFileSync(path.resolve("apps/api/drizzle", fileName), "utf8"),
	)
	.join("\n");
const documentFormsDocs = readFileSync(
	path.resolve("docs/12-document-generation-forms.md"),
	"utf8",
);
...
const documentCatalogRequiredFragments = {
	...
	dental_medical_card_043u: "dental medical card 043",
	orthodontic_medical_card_043_1u: "orthodontic medical card 043-1",
	daily_dentist_diary_037u: "daily dentist diary 037",
	summary_dentist_statement_039u: "summary dentist statement 039",
	...
	radiation_dose_sheet: "radiation dose sheet",
		...
		specialistVisitRecords: [
			{
				...
				informedConsentOrRefusal:
					"Информированное согласие проверено в подписанной записи DENTE.",
				clinicalToothRows: sampleClinicalToothRows,
			},
		],
...
const postVisitRecommendationsPayload = {
	postVisitRecommendations: {
		...
		nutritionInstructions: [
			"исключить горячее, острое и жесткое",
...
const treatmentPlanPayload = {
	treatmentPlan: {
		...
		prognosisAndLimits:
			"прогноз зависит от исходного состояния тканей и соблюдения рекомендаций",
...
const paidMedicalServicesContractPayload = {
	paidMedicalServicesContract: {
		...
		medicalRecommendationWarning:
			"recommendation non-compliance may affect result and timing",
		refusalAndRefundTerms:
			"refund follows provided services and documented expenses",
		warrantyAndClaimsTerms:
			"warranty follows local clinic rules and medical recommendations",
...
const completedWorksActPayload = {
	completedWorksAct: {
		...
		doctorFullName: "Smoke Doctor",
		acceptedServicesSummary:
			"Smoke accepted completed dental services from the visit treatment plan.",
		totalByActRub: 12345,
		paidRub: 12345,
...
const installmentPaymentSchedulePayload = {
	installmentPaymentSchedule: {
		...
		installments: [
			{
				label: "Smoke first installment",
				dueDate: "2026-05-25",
				amountRub: 5000,
				status: "planned",
			},
			{
				label: "Smoke final installment",
				dueDate: "2026-06-10",
				amountRub: 5000,
				status: "planned",
			},
		...
		paymentMethodNotes:
			"cash desk, payment link or bank transfer with fiscal receipt",
...
const informedConsentPayload = {
	informedConsent: {
		...
		expectedBenefit:
			"устранение боли, восстановление формы зуба и снижение риска осложнений",
		plannedAnesthesia: "проводниковая анестезия артикаином 4%",
		materialOrMedicationNotes:
			"коффердам и композитный материал по клиническим показаниям",
		...
		alternatives: [
			...
			"выбрать другой метод лечения при наличии показаний",
		],
...
const procedureSpecificConsentPayload = {
	procedureSpecificConsent: {
		...
		diagnosisOrIndication:
			"острая боль, разрушение коронковой части и риск инфекции",
		clinicalToothRows: sampleClinicalToothRows,
		plannedAnesthesia: "проводниковая анестезия артикаином 4%",
		materialsAndSystems:
			"шовный материал и гемостатическая губка по показаниям",
		patientSpecificRiskFactors: [
			"аллергии и антикоагулянты уточнены",
...
const personalDataConsentPayload = {
	personalDataProcessingConsent: {
		...
		retentionPeriod:
			"срок оказания помощи плюс обязательный срок хранения медицинской и бухгалтерской документации",
		revocationChannel:
...
const requiredFragments = new Map([
	[
		"paid_medical_services_contract",
		["DPMU-SMOKE-001", "Smoke paid dental service scope", "11.05.2023 № 736"],
	],
	[
		"completed_works_act",
		[
			"ACT-SMOKE-001",
			"FN-SMOKE-001",
			"Smoke accepted completed dental services",
		],
	],
	[
	...
	[
		"tax_deduction_certificate",
		["КНД 1151156", "ЕА-7-11/824", "Лист 001", "Номер справки"],
	],
	...
	[
		...
		["копии медицинской документации", "архив исходных снимков"],
	],
	[
		"medical_document_release_receipt",
		["Расписка о выдаче", "архив исходных снимков"],
	],
	["xray_cbct_referral", ["КЛКТ", "Беременность/ограничения"]],
	[
		"treatment_plan_acceptance",
		["Согласование плана лечения", "Отклоненные или отложенные альтернативы"],
	],
	["anesthesia_consent_log", ["Журнал введения", "Анестетик"]],
	[
		"prescription_medication_order",
		["Назначение лекарственных препаратов", "Контроль безопасности"],
	],
	[
	...
	[
		...
		["Справка о посещении", "не раскрывает диагноз"],
	],
	[
	...
	[
		...
		["Анкета пациента", "Основная жалоба", "Аллергии"],
	],
]);
...
const groupedKinds = new Set(
	documentFactoryGroups.flatMap((group) => group.kinds),
);
for (const kind of documentKindSchema.options) {
	...
	assert(
		metadata.title && metadata.label && metadata.actionLabel,
		`${kind}: incomplete metadata labels`,
	);
	assert(
		documentSourceStatusLabels[metadata.sourceStatus],
		`${kind}: missing readable source status label`,
	);
	assert(
		metadata.sourceAuthority && metadata.sourceAuthority.trim().length >= 3,
		`${kind}: missing document source authority`,
	);
	assert(
		metadata.sourceReference && metadata.sourceReference.trim().length >= 8,
		`${kind}: missing document source reference`,
	);
	assert(
		metadata.sourceNote && metadata.sourceNote.trim().length >= 24,
		`${kind}: missing practical source note`,
	);
	assert(
		/^\d{4}-\d{2}-\d{2}$/.test(metadata.sourceCheckedAt),
		`${kind}: source check date must be YYYY-MM-DD`,
	);
	assert(
		Array.isArray(metadata.sourceUrls),
		`${kind}: sourceUrls must be an array`,
	);
	for (const sourceUrl of metadata.sourceUrls) {
		assert(
			/^https:\/\/[^ ]+$/.test(sourceUrl),
			`${kind}: source URL must be absolute HTTPS`,
		);
	}
	if (
		metadata.sourceStatus === "official_form" ||
		metadata.sourceStatus === "official_workflow"
	) {
		assert(
			metadata.sourceUrls.length > 0,
	...
	if (!initialDocumentKinds.has(kind)) {
		assert(
			documentMigrationSql.includes(`'${kind}'`),
			`${kind}: missing Postgres enum migration`,
		);
	}
	if (
		...
		assert(
			documentRequiresPaidRecord(kind),
			`${kind}: tax fiscal document must require paid record`,
		);
		assert(
			documentAmountSource(kind) === "paid",
			`${kind}: tax fiscal document must use paid amount source`,
		);
	}
	if (kind === "tax_deduction_certificate") {
		assert(
			metadata.title.includes("Черновик данных"),
			`${kind}: UI must not present a data draft as the final KND certificate`,
		);
		assert(
			metadata.sourceStatus === "official_form",
			`${kind}: KND 1151156 must be marked as official-form based`,
		);
		assert(
			metadata.sourceReference.includes("КНД 1151156"),
			`${kind}: official KND reference is missing`,
		);
		assert(
			metadata.sourceReference.includes("ЕА-7-11/824"),
			`${kind}: FNS order reference is missing`,
		);
		assert(
			metadata.sourceUrls.some(
				(url) => url.includes("nalog.gov.ru") && url.includes("14112883"),
			),
			`${kind}: FNS order source URL is missing`,
		);
		assert(
			metadata.sourceUrls.some((url) => url.includes("pril1_14112883.pdf")),
			`${kind}: KND form PDF source URL is missing`,
		);
		assert(
			metadata.sourceUrls.some((url) =>
				url.includes("UT_SVOPLMEDUSL_1_278_00_05_01_02.xsd"),
			),
			`${kind}: KND 1151156 XSD 5.01 source URL is missing`,
		);
	}
	if (kind === "legacy_tax_deduction_certificate") {
		assert(
			metadata.sourceReference.includes("2021-2023"),
			`${kind}: legacy tax years must stay explicit`,
		);
		assert(
			metadata.sourceNote.includes("2021-2023"),
			`${kind}: legacy tax note must block 2024+ confusion`,
		);
	}
	if (kind === "outpatient_medical_card_025u") {
		assert(
			metadata.sourceStatus === "official_form",
			`${kind}: 025/u card must be marked as official-form based`,
		);
		assert(
			metadata.sourceReference.includes("274"),
			`${kind}: 025/u card must cite Minzdrav Order 274n`,
		);
		assert(
			metadata.sourceUrls.some((url) => url.includes("0001202505300033")),
	...
	if (kind === "informed_consent" || kind === "medical_intervention_refusal") {
		assert(
			metadata.sourceReference.includes("1051"),
			`${kind}: consent/refusal source must cite Ministry Order 1051n`,
		);
		assert(
			metadata.sourceStatus === "official_workflow",
			`${kind}: consent/refusal must be official workflow, not a fake official final form`,
		);
		assert(
			metadata.sourceUrls.some((url) => url.includes("0001202111250019")),
			`${kind}: consent/refusal official source URL is missing`,
		);
	}
	if (
		...
		assert(
			metadata.sourceReference.includes("789") ||
				metadata.sourceReference.includes("274"),
			`${kind}: medical-record source must cite current document workflow`,
		);
		assert(
			metadata.sourceUrls.some((url) => url.includes("0001202009240027")),
			`${kind}: medical-document release source URL is missing`,
		);
	}
	if (kind === "tax_deduction_application") {
		assert(
			!documentRequiresPaidRecord(kind),
			`${kind}: application should collect request data before certificate issue`,
		);
		assert(
			documentAmountSource(kind) === "none",
			`${kind}: application must not invent planned or paid amount`,
		);
	}
										...
										: kind === "minor_legal_representative_consent"
											...
											: kind === "warranty_service_memo"
												...
												: kind === "medical_intervention_refusal"
													? documentFor(kind, {
															payload: interventionRefusalPayload,
														})
	...
	assert(
		html.includes("Тестовый Пациент"),
		`${kind}: missing patient identity`,
	);
	assert(
		!html.includes("Шаблон для этого типа"),
		`${kind}: fell back to unconfigured template`,
	);
	...
	for (const fragment of fragments) {
		assert(
			...
			`${kind}: missing required fragment "${fragment}"`,
		);
	}

	if (
		planBackedFinancialKinds.has(kind) &&
		!["payment_invoice", "installment_payment_schedule"].includes(kind)
	) {
		assert(
			html.includes("Лечение кариеса с восстановлением"),
			`${kind}: missing real treatment plan service row`,
		);
		assert(
			html.includes("Профессиональная гигиена"),
			`${kind}: missing second treatment plan service row`,
		);
		assert(
			html.includes("A16.07.002"),
			`${kind}: missing catalog service code`,
		);
		assert(
			html.includes("зуб 36"),
			`${kind}: missing tooth code from treatment plan`,
		);
		assert(
			!html.includes("Стоматологические услуги по визиту/плану лечения"),
			`${kind}: rendered generic act service placeholder`,
		);
		assert(
			!html.includes("Стоматологические услуги по плану лечения"),
			`${kind}: rendered generic estimate service placeholder`,
		);
		assert(
			!html.includes("Первый платеж"),
			`${kind}: rendered blank installment placeholder`,
		);
		assert(
			!html.includes("__________ руб."),
			`${kind}: rendered blank payment amount placeholder`,
		);
	}
...
const missingClinicForConsentReason = documentIssueBlockReason(
	documentFor("informed_consent"),
	patient,
);
assert(
...
const incompleteIssueReason = documentIssueBlockReason(
	documentFor("informed_consent"),
	patient,
	renderContext,
);
...
assert(
	completeInformedConsentIssueReason === null,
	`complete informed consent payload must be issue-ready: ${completeInformedConsentIssueReason}`,
);
const procedureSpecificConsentMissingPayloadReason = documentIssueBlockReason(
	documentFor("procedure_specific_consent_packet"),
	patient,
	renderContext,
);
assert(
	procedureSpecificConsentMissingPayloadReason?.includes(
...
const minorConsentWithoutPayloadReason = documentIssueBlockReason(
	documentFor("minor_legal_representative_consent"),
	patient,
	renderContext,
);
assert(
...
const refusalWithoutPayloadReason = documentIssueBlockReason(
	documentFor("medical_intervention_refusal"),
	patient,
	renderContext,
);
assert(
...
assert(
	refusalWithPayloadReason === null,
	`complete intervention refusal payload must be issue-ready: ${refusalWithPayloadReason}`,
);
const personalDataWithoutPayloadReason = documentIssueBlockReason(
	documentFor("personal_data_processing_consent"),
	patient,
	renderContext,
);
assert(
...
assert(
	personalDataWithPayloadReason === null,
	`complete personal data consent payload must be issue-ready: ${personalDataWithPayloadReason}`,
);
const extractWithoutPayloadReason = documentIssueBlockReason(
...
assert(
	copyRequestWithPayloadReason === null,
	`complete medical record copy request payload must be issue-ready: ${copyRequestWithPayloadReason}`,
);
const postVisitWithoutPayloadReason = documentIssueBlockReason(
	documentFor("post_visit_recommendations"),
	patient,
	renderContext,
);
assert(
...
assert(
	postVisitWithPayloadReason === null,
	`complete post visit recommendations payload must be issue-ready: ${postVisitWithPayloadReason}`,
);
const treatmentPlanWithoutPayloadReason = documentIssueBlockReason(
	documentFor("treatment_plan"),
	patient,
	renderContext,
);
assert(
...
assert(
	treatmentPlanWithPayloadReason === null,
	`complete treatment plan payload must be issue-ready: ${treatmentPlanWithPayloadReason}`,
);
const treatmentPlanWithoutClinicalRowsPayload = JSON.parse(
	JSON.stringify(treatmentPlanPayload),
);
...
const treatmentPlanWithoutClinicalRowsReason = documentIssueBlockReason(
	documentFor("treatment_plan", {
		payload: treatmentPlanWithoutClinicalRowsPayload,
	}),
	patient,
	renderContext,
);
...
const paidContractWithoutPayloadReason = documentIssueBlockReason(
	documentFor("paid_medical_services_contract"),
	patient,
	renderContext,
);
...
assert(
	paidContractWithPayloadReason === null,
	`complete paid contract payload must be issue-ready: ${paidContractWithPayloadReason}`,
);
const completedWorksActWithoutPayloadReason = documentIssueBlockReason(
	documentFor("completed_works_act"),
	patient,
	renderContext,
);
assert(
...
assert(
	completedWorksActWithPayloadReason === null,
	`complete completed works act payload must be issue-ready: ${completedWorksActWithPayloadReason}`,
);
const completedWorksActWithUnknownFiscalPayload = JSON.parse(
	JSON.stringify(completedWorksActPayload),
);
completedWorksActWithUnknownFiscalPayload.completedWorksAct.fiscalReceiptNumbers =
	["FN-UNKNOWN"];
const completedWorksActWithUnknownFiscalReason = documentIssueBlockReason(
	documentFor("completed_works_act", {
		payload: completedWorksActWithUnknownFiscalPayload,
	}),
	patient,
	renderContext,
);
assert(
	completedWorksActWithUnknownFiscalReason?.includes(
		"фискальный чек без связи с оплатой",
	),
...
const treatmentCostEstimateWithoutPayloadReason = documentIssueBlockReason(
	documentFor("treatment_cost_estimate"),
	patient,
	renderContext,
);
assert(
	treatmentCostEstimateWithoutPayloadReason?.includes(
		"структурированные данные",
	),
...
const paymentInvoiceWithoutPayloadReason = documentIssueBlockReason(
	documentFor("payment_invoice"),
	patient,
	renderContext,
);
assert(
...
assert(
	paymentInvoiceWithPayloadReason === null,
	`complete payment invoice payload must be issue-ready: ${paymentInvoiceWithPayloadReason}`,
);
const installmentScheduleWithoutPayloadReason = documentIssueBlockReason(
	documentFor("installment_payment_schedule"),
	patient,
	renderContext,
);
assert(
...
assert(
	installmentScheduleWithPayloadReason === null,
	`complete installment schedule payload must be issue-ready: ${installmentScheduleWithPayloadReason}`,
);
const treatmentPlanAcceptanceWithoutPayloadReason = documentIssueBlockReason(
	documentFor("treatment_plan_acceptance"),
	patient,
	renderContext,
);
assert(
	treatmentPlanAcceptanceWithoutPayloadReason?.includes(
		"структурированные данные",
	),
...
const attendanceWithoutPayloadReason = documentIssueBlockReason(
	documentFor("visit_attendance_certificate"),
	patient,
	renderContext,
);
assert(
...
assert(
	attendanceWithPayloadReason === null,
	`complete visit attendance payload must be issue-ready: ${attendanceWithPayloadReason}`,
);
const warrantyWithoutPayloadReason = documentIssueBlockReason(
	documentFor("warranty_service_memo"),
	patient,
	renderContext,
);
assert(
...
assert(
	warrantyWithPayloadReason === null,
	`complete warranty service memo payload must be issue-ready: ${warrantyWithPayloadReason}`,
);
const paymentReceiptWithoutPayloadReason = documentIssueBlockReason(
	documentFor("payment_receipt"),
	patient,
	renderContext,
);
assert(
...
const completeIssueReason = documentIssueBlockReason(
	documentFor("payment_receipt", { payload: paymentReceiptPayload }),
	patient,
	renderContext,
);
assert(
	completeIssueReason === null,
	`complete payment receipt payload must be issue-ready: ${completeIssueReason}`,
);
const missingFiscalIssueReason = documentIssueBlockReason(
	documentFor("payment_receipt", { payload: paymentReceiptPayload }),
	patient,
	{
		...renderContext,
...
const missingFiscalDateIssueReason = documentIssueBlockReason(
	documentFor("payment_receipt", { payload: paymentReceiptPayload }),
	patient,
	{
		...renderContext,
...
const invalidFiscalDateIssueReason = documentIssueBlockReason(
	documentFor("payment_receipt", { payload: paymentReceiptPayload }),
	patient,
	{
		...renderContext,
...
const basicPaymentReceiptPayload = JSON.parse(
	JSON.stringify(paymentReceiptPayload),
);
...
const receiptTopLevelUrlOnlyHtml = renderDocumentHtml(
	documentFor("payment_receipt", { payload: paymentReceiptPayload }),
	patient,
	{
		...renderContext,
...
assert(
	receiptTopLevelUrlOnlyHtml.includes(
		"https://clinic.example.test/fiscal/top-level-only",
	),
...
const missingCorrectionFiscalIssueReason = documentIssueBlockReason(
	documentFor("payment_refund_correction_request", {
		payload: refundCorrectionPayload,
	}),
	patient,
	{
		...renderContext,
...
const unknownCorrectionReceiptPayload = JSON.parse(
	JSON.stringify(refundCorrectionPayload),
);
unknownCorrectionReceiptPayload.paymentRefundCorrection.originalFiscalReceiptNumber =
	"FN-UNKNOWN";
const unknownCorrectionReceiptIssueReason = documentIssueBlockReason(
	documentFor("payment_refund_correction_request", {
		payload: unknownCorrectionReceiptPayload,
	}),
	patient,
	renderContext,
);
assert(
	unknownCorrectionReceiptIssueReason?.includes(
		"фискальный чек без связи с оплатой",
	),
...
const missingClinicIssueReason = documentIssueBlockReason(
	...
	{
		...renderContext,
		clinicProfile: { ...clinicProfile, medicalLicenseNumber: null },
	},
);
assert(
