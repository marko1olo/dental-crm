...
const scheduleViewSource = await readFile(
...
const clinicProfileSaveBlock = sourceSlice(
	"async function saveClinicProfileFromDraft",
	"async function saveClinicProfileIfDirty",
);
const clinicModeBlock = sourceSlice(
	"async function changeClinicMode",
...
if (appSource.includes("English, fallback")) {
	fail(
		"Language selector must not expose an English UI promise before localization coverage exists.",
	);
}

if (appSource.includes('<option value="en">')) {
	fail(
		"English UI language must not be selectable until a real localization dictionary exists.",
	);
}
...
if (
	...
	fail(
		"Language selector must normalize unsupported values back to Russian instead of trusting DOM input.",
	);
}

if (appSource.includes("event.target.value as UiLanguage")) {
	fail(
		"Language selector must not cast raw DOM values as a trusted UI language.",
	);
}

if (!appSource.includes("uiLanguageOptions.map((option) =>")) {
	fail(
		"Language selector must render from the shared Russian-only language option list.",
	);
}

if (
	!appSource.includes(
		"Выбор сохраняется автоматически и остается до смены языка",
	)
) {
	fail(
		"Language settings must tell doctors that the current Russian choice is autosaved until changed.",
	);
}
...
for (const fragment of forbiddenDoctorFacingEnglishFragments) {
	if (appSource.includes(fragment))
		fail(`Doctor-facing English fragment leaked: ${fragment}`);
}
...
for (const marker of [
	...
	if (!scheduleUiSource.includes(marker))
		fail(`Schedule filter persistence UI is missing ${marker}.`);
}
...
for (const marker of [
	...
	if (!appSource.includes(marker))
		fail(`Persisted workflow select normalization is missing ${marker}.`);
}
...
if (
	!appSource.includes("loadServerUiPreferences") ||
	!appSource.includes("saveServerUiPreferences")
) {
...
for (const forbiddenCast of [
	...
	if (appSource.includes(forbiddenCast))
		fail(
			`Persisted select must normalize DOM values before saving state: ${forbiddenCast}`,
		);
}

if (saveServerPreferencesBlock.includes("telegramControlPlaneHeaders")) {
	fail(
		"Personal UI preference autosave must not depend on the clinic admin secret.",
	);
}
...
if (!sharedSource.includes("savedAt: z.string().optional()")) {
	fail(
		"Shared UI preference input schema must accept client savedAt for stale-write protection.",
	);
}

if (!saveServerPreferencesBlock.includes("response.ok")) {
	fail(
		"Server UI preference saves must check response.ok instead of silently swallowing API failures.",
	);
}
...
if (
	...
	fail(
		"Server UI preference sync must use the unlocked admin/read secret and stay local-only before unlock.",
	);
}

if (!appSource.includes("uiPreferencesSyncErrorMessage")) {
	fail(
		"UI preference server sync failures must have a reusable user-facing Russian warning.",
	);
}

if (
	appSource.includes(
		"saveServerUiPreferences(savedPreferences).catch(() => undefined)",
	)
) {
	fail(
		"Scheduled server UI preference saves must not silently swallow sync failures.",
	);
}
...
for (const marker of [
	...
	if (!appSource.includes(marker))
		fail(`UI preference retry/backoff wiring missing ${marker}.`);
}

if (!appSource.includes("{!error && uiPreferencesSyncError ? (")) {
	fail(
		"UI preference sync warning must render when the main error banner is empty.",
	);
}
...
if (
	...
	fail(
		"UI preferences must not rewrite local/server state before server hydration completes.",
	);
}
...
if (
	...
	fail(
		"Persisted protocol template selection must reset when it no longer matches the selected specialty or dashboard template set.",
	);
}

if (
	appSource.includes(
		"preferences.onboardingDismissed || loadOnboardingDismissed()",
	)
) {
	fail(
		"Onboarding dismissal must be timestamp-merged, not OR-merged from stale localStorage.",
	);
}
...
for (const rawEnumRender of [
	...
	if (appSource.includes(rawEnumRender))
		fail(
			`Onboarding renders raw enum instead of Russian label: ${rawEnumRender}`,
		);
}
...
for (const key of forbiddenClinicalKeys) {
	if (typeBlock.includes(`${key}:`))
		fail(`UiPreferences must not persist clinical/transient field ${key}.`);
}
