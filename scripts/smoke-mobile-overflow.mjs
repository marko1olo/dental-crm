...
const profileDir = path.join(
	os.tmpdir(),
	`dental-crm-edge-smoke-${process.pid}`,
);
...
async function cleanupBrowserProfile() {
	...
	try {
		...
		console.warn(
			JSON.stringify({
				warning:
					"Temporary browser profile cleanup skipped because Windows still holds a browser file lock.",
				path: profileDir,
				code: error.code,
			}),
		);
...
try {
	...
	const pageTarget =
		targets.find(
			(target) =>
				target.type === "page" && target.url?.startsWith(targetOrigin),
		) ??
		targets.find(
			(target) => target.type === "page" && target.title === "Dental CRM",
		) ??
	...
	const cdp = connectCdp(
		normalizedWebSocketUrl(pageTarget.webSocketDebuggerUrl),
	);
	await cdp.opened;
	...
	if (clickSelector) {
		...
		if (!clicked.result.value)
			throw new Error(`${clickLabel} is not clickable on the target page.`);
		for (let attempt = 0; attempt < 40; attempt += 1) {
