...
const payments = [
	...
	},
	{
		...
		amountRub: 5000,
		status: "paid",
		paidAt: "2025-12-30T10:00:00.000Z",
		payerInn: "123456789012",
	},
	{
		id: taxPayment2023Id,
		...
		amountRub: 4000,
		status: "paid",
		paidAt: "2023-12-30T10:00:00.000Z",
		payerInn: "123456789012",
	},
	{
		id: taxPaymentOtherPayerId,
		patientId,
		visitId: otherVisitId,
		amountRub: 2000,
		status: "paid",
...
const scopedTaxCertificateInput = {
	...
	payload: {
		taxPaymentSelection: { selectedPaymentIds: [taxPaymentOtherPayerId] },
	},
};
const taxRegistryInput = {
	...
	payload: {
		taxPaymentSelection: {
			selectedPaymentIds: [taxPayment2026Id, taxPaymentOtherPayerId],
		},
	},
};
...
const paidMedicalServicesContractPayload = {
	paidMedicalServicesContract: {
		...
		freeCareAvailabilityNotice:
			"free care route was explained where applicable",
		medicalRecommendationWarning:
			"recommendation non-compliance may affect result",
		refusalAndRefundTerms:
			"refund follows actually provided services and documented expenses",
...
const treatmentCostEstimatePayload = {
	treatmentCostEstimate: {
		...
		paymentMilestoneNotes:
			"оплата по этапам с выдачей кассового чека после оплаты",
...
const informedConsentPayload = {
	informedConsent: {
		...
		expectedBenefit:
			"устранение боли, восстановление формы зуба и профилактика осложнений",
		...
		explainedRisks: [
			"боль и отек после вмешательства",
			"аллергическая реакция",
			"необходимость повторного приема",
		],
		alternatives: [
			"получить второе мнение",
			"отложить лечение под наблюдением",
			"выбрать другой метод лечения",
		],
		aftercareRequirements: [
			"не есть до окончания действия анестезии",
			"соблюдать рекомендации врача",
			"обратиться при нарастающей боли или отеке",
		],
		doctorFullName: "Доктор Смоук",
...
const procedureSpecificConsentPayload = {
	procedureSpecificConsent: {
		...
		diagnosisOrIndication:
			"разрушение коронковой части и риск распространения инфекции",
		clinicalToothRows: sampleClinicalToothRows,
		plannedAnesthesia: "проводниковая анестезия артикаином 4%",
		materialsAndSystems:
			"шовный материал и гемостатическая губка по показаниям",
		...
		alternatives: [
			"эндодонтическое лечение при наличии показаний",
			"получить второе мнение",
...
const medicalRecordExtractPayload = {
	medicalRecordExtract: {
		...
		complaintAndAnamnesis:
			"Боль при накусывании в области 36 зуба, анамнез собран со слов пациента.",
		objectiveStatus: "Кариозная полость 36 зуба, перкуссия слабо болезненна.",
		diagnosis: "Кариес дентина 36 зуба.",
		clinicalToothRows: sampleClinicalToothRows,
		treatmentProvided:
			"Проведено терапевтическое лечение 36 зуба с восстановлением композитным материалом.",
		recommendations:
			"Контрольный осмотр по записи, соблюдение гигиены и обращение при боли или отеке.",
		doctorFullName: "Доктор Смоук",
		recipientFullName: "Тестовый пациент",
		recipientAuthority: "пациент лично",
...
const dentalMedicalCard043uPayload = {
	dentalMedicalCard043u: {
		...
		organization: {
			...
			licenseAuthority: "Минздрав Самарской области",
		},
		patient: {
			...
			documentSeriesNumber: "36 00 123456",
			snils: "123-456-789 00",
			medicalCardNumber: "043U-GUARD-001",
		},
		doctor: {
		...
		statusLocalis: "Кариозная полость 36 зуба, перкуссия слабо болезненна.",
		objectiveStatus: "Слизистая бледно-розовая, прикус ортогнатический.",
		diagnosisIcd10: "K02.1",
		diagnosisTooth: "36",
		diagnosisText: "Кариес дентина 36 зуба",
		clinicalToothRows: sampleClinicalToothRows,
		treatmentDescription: "Проведено терапевтическое лечение 36 зуба.",
		recommendations: "Контрольный осмотр через 6 месяцев",
		nextVisitDate: null,
		issuedAt: "2026-05-20 14:00",
	},
};
...
const postVisitRecommendationsPayload = {
	postVisitRecommendations: {
		...
		doctorFullName: "Доктор Смоук",
		allowedAfter: [
			"пить воду после окончания действия анестезии",
			"есть мягкую пищу после восстановления чувствительности",
		],
		temporaryRestrictions: [
			"не греть область удаления",
			"не полоскать активно первые сутки",
			"не курить 72 часа",
		],
		medicationAndRinsePlan: [
			"обезболивающее по схеме врача",
			"антибиотики только при отдельном назначении",
		],
		hygieneInstructions: [
			"чистить зубы аккуратно",
			"не травмировать лунку щеткой или ирригатором",
		],
		nutritionInstructions: [
			"исключить горячее и жесткое",
		...
		clinicContactInstruction:
			"связаться с клиникой по телефону или через Telegram-бот DENTE",
		telegramSummary:
			"После удаления соблюдайте ограничения и срочно свяжитесь с клиникой при кровотечении, боли, отеке, температуре или аллергии.",
...
const treatmentPlanPayload = {
	treatmentPlan: {
		...
		risksAndLimitations: [
			"изменение плана при новых данных",
...
const personalDataConsentPayload = {
	personalDataProcessingConsent: {
		...
		thirdPartyTransferRules:
			"Передача разрешена только подрядчикам и органам по законному основанию.",
		crossBorderTransferAllowed: false,
		automatedDecisionMakingAllowed: false,
		retentionPeriod:
			"обязательный срок хранения медицинской и бухгалтерской документации",
		revocationChannel:
			"письменное заявление в клинике или защищенное обращение через портал пациента",
...
assert(
	...
	"tax certificate must require paid record",
);
assert(
	documentAmountSource("tax_deduction_certificate") === "paid",
	"tax certificate must use paid amount source",
);
assert(
...
assert(
	!missingPatientIntakePayloadResult.ok &&
		missingPatientIntakePayloadResult.statusCode === 409,
	"patient intake must require structured payload",
);
...
assert(
	patientIntakePayloadResult.ok,
	"patient intake payload must pass creation guard",
);
...
assert(
	!missingPaidContractPayloadResult.ok &&
		missingPaidContractPayloadResult.statusCode === 409,
...
assert(
	paidContractPayloadResult.ok,
	"paid medical services contract payload must pass creation guard",
);
const missingPaidContractCustomerPayload = clone(
...
const missingPaidContractCustomerResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "paid_medical_services_contract",
		payload: missingPaidContractCustomerPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!missingPaidContractCustomerResult.ok &&
		missingPaidContractCustomerResult.statusCode === 409,
...
const badPaidContractAmountResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "paid_medical_services_contract",
		payload: badPaidContractAmountPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!badPaidContractAmountResult.ok &&
		badPaidContractAmountResult.statusCode === 409,
...
assert(
	!missingCompletedWorksActPayloadResult.ok &&
		missingCompletedWorksActPayloadResult.statusCode === 409,
...
assert(
	completedWorksActPayloadResult.ok,
	"completed works act payload must pass creation guard",
);
...
const badCompletedWorksActPaidResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "completed_works_act",
		payload: badCompletedWorksActPaidPayload,
	},
	{ patient, visit, paidAmountRub: 3000, plannedAmountRub: 0 },
);
assert(
	!badCompletedWorksActPaidResult.ok &&
		badCompletedWorksActPaidResult.statusCode === 409,
...
assert(
	!missingTreatmentEstimatePayloadResult.ok &&
		missingTreatmentEstimatePayloadResult.statusCode === 409,
...
assert(
	treatmentEstimatePayloadResult.ok,
	"treatment cost estimate payload must pass creation guard",
);
...
const badTreatmentEstimateLineResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "treatment_cost_estimate",
		payload: badTreatmentEstimateLinePayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!badTreatmentEstimateLineResult.ok &&
		badTreatmentEstimateLineResult.statusCode === 409,
...
const badTreatmentEstimateTotalResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "treatment_cost_estimate",
		payload: badTreatmentEstimateTotalPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!badTreatmentEstimateTotalResult.ok &&
		badTreatmentEstimateTotalResult.statusCode === 409,
...
assert(
	!missingPaymentInvoicePayloadResult.ok &&
		missingPaymentInvoicePayloadResult.statusCode === 409,
...
assert(
	paymentInvoicePayloadResult.ok,
	"payment invoice payload must pass creation guard",
);
...
const badPaymentInvoiceTotalResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "payment_invoice",
		payload: badPaymentInvoiceTotalPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!badPaymentInvoiceTotalResult.ok &&
		badPaymentInvoiceTotalResult.statusCode === 409,
...
assert(
	!missingPaymentReceiptPayloadResult.ok &&
		missingPaymentReceiptPayloadResult.statusCode === 409,
	"payment receipt must require structured payload",
);
const completePaymentReceiptInput = {
	patientId,
	visitId,
	kind: "payment_receipt",
	payload: paymentReceiptPayload,
};
const paymentReceiptSelectionError = paymentReceiptSelectionErrorForDocument(
	completePaymentReceiptInput,
	payments,
);
assert(
	paymentReceiptSelectionError === null,
	`payment receipt selected payment facts must pass: ${paymentReceiptSelectionError}`,
);
const paymentReceiptPaidAmount = paidAmountRubForDocument(
	"payment_receipt",
	completePaymentReceiptInput,
	payments,
);
assert(
	paymentReceiptPaidAmount === 3000,
	`payment receipt must use only selected payment ids, got ${paymentReceiptPaidAmount}`,
);
const paymentReceiptPayloadResult = validateDocumentCreation(
	completePaymentReceiptInput,
	{
		patient,
		visit,
...
assert(
	paymentReceiptPayloadResult.ok &&
		paymentReceiptPayloadResult.input.totalAmountRub === 3000,
...
const basicPaymentReceiptInput = {
	patientId,
	visitId,
	kind: "payment_receipt",
	payload: basicPaymentReceiptPayload,
};
...
const basicPaymentReceiptSelectionError =
	paymentReceiptSelectionErrorForDocument(
		basicPaymentReceiptInput,
		paymentsWithoutTaxIdentity,
	);
assert(
...
const basicPaymentReceiptResult = validateDocumentCreation(
	basicPaymentReceiptInput,
	{
		patient,
		visit,
...
assert(
	basicPaymentReceiptResult.ok,
	"basic payment receipt must pass without payer birth date, INN, identity document or relationship",
);
const paymentsWithoutPayerName = payments.map((payment) =>
	payment.id === taxPayment2026Id ? { ...payment, payerFullName: "" } : payment,
);
const missingPayerNameSelectionError = paymentReceiptSelectionErrorForDocument(
	basicPaymentReceiptInput,
	paymentsWithoutPayerName,
);
assert(
	missingPayerNameSelectionError,
	"payment receipt must reject selected payments without stored payer full name",
);
const paymentsWithoutTaxBirthDate = payments.map((payment) =>
	payment.id === taxPayment2026Id
		? { ...payment, payerBirthDate: null }
		: payment,
);
const missingTaxBirthDateSelectionError =
	paymentReceiptSelectionErrorForDocument(
		completePaymentReceiptInput,
...
const paymentsWithoutPayerRelationship = payments.map((payment) =>
	payment.id === taxPayment2026Id
		? { ...payment, payerRelationship: null }
		: payment,
);
const missingPayerRelationshipSelectionError =
	paymentReceiptSelectionErrorForDocument(
		completePaymentReceiptInput,
...
const paymentsWithoutPayerInnAndIdentity = payments.map((payment) =>
	payment.id === taxPayment2026Id
		? { ...payment, payerInn: null, payerIdentityDocument: null }
		: payment,
);
const missingPayerInnAndIdentitySelectionError =
	paymentReceiptSelectionErrorForDocument(
		completePaymentReceiptInput,
...
const badPaymentReceiptInput = {
	patientId,
	visitId,
	kind: "payment_receipt",
	payload: badPaymentReceiptPayload,
};
const badPaymentReceiptSelectionError = paymentReceiptSelectionErrorForDocument(
	badPaymentReceiptInput,
	payments,
);
const badPaymentReceiptResult = validateDocumentCreation(
	badPaymentReceiptInput,
	{
		patient,
		visit,
...
assert(
	!missingInstallmentSchedulePayloadResult.ok &&
		missingInstallmentSchedulePayloadResult.statusCode === 409,
...
const installmentSchedulePayloadResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "installment_payment_schedule",
		payload: installmentSchedulePayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	installmentSchedulePayloadResult.ok,
	"installment payment schedule payload must pass creation guard",
);
...
const badInstallmentScheduleBalanceResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "installment_payment_schedule",
		payload: badInstallmentScheduleBalancePayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 10000 },
);
assert(
	!badInstallmentScheduleBalanceResult.ok &&
		badInstallmentScheduleBalanceResult.statusCode === 409,
...
assert(
	!missingMinorRepresentativePayloadResult.ok &&
		missingMinorRepresentativePayloadResult.statusCode === 409,
...
const minorRepresentativePayloadResult = validateDocumentCreation(
	{
		...
		payload: minorLegalRepresentativeConsentPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	minorRepresentativePayloadResult.ok,
	"minor legal representative consent payload must pass creation guard",
);
...
assert(
	!missingWarrantyPayloadResult.ok &&
		missingWarrantyPayloadResult.statusCode === 409,
...
const warrantyPayloadResult = validateDocumentCreation(
	{
		...
		payload: warrantyServiceMemoPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	warrantyPayloadResult.ok,
	"warranty service memo payload must pass creation guard",
);
...
assert(
	!missingTaxApplicationPayloadResult.ok &&
		missingTaxApplicationPayloadResult.statusCode === 409,
...
const taxApplicationPayloadResult = validateDocumentCreation(
	{
		patientId,
		kind: "tax_deduction_application",
		payload: taxApplicationPayload,
	},
	{ patient, visit: null, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	taxApplicationPayloadResult.ok,
	"tax application payload must pass creation guard without paid records",
);
const taxApplicationOrganizationInnPayload = clone(taxApplicationPayload);
taxApplicationOrganizationInnPayload.taxDeductionApplication.taxpayerInn =
	"6312000000";
const taxApplicationOrganizationInnResult = validateDocumentCreation(
	{
		patientId,
		kind: "tax_deduction_application",
		payload: taxApplicationOrganizationInnPayload,
	},
	{ patient, visit: null, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	!taxApplicationOrganizationInnResult.ok &&
		taxApplicationOrganizationInnResult.statusCode === 409,
...
assert(
	!missingInformedConsentPayloadResult.ok &&
		missingInformedConsentPayloadResult.statusCode === 409,
	"informed consent must require structured payload",
);
const informedConsentPayloadResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "informed_consent",
		payload: informedConsentPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	informedConsentPayloadResult.ok,
	"informed consent payload must pass creation guard",
);
...
assert(
	!missingProcedureSpecificConsentPayloadResult.ok &&
		missingProcedureSpecificConsentPayloadResult.statusCode === 409,
...
const procedureSpecificConsentPayloadResult = validateDocumentCreation(
	{
		...
		payload: procedureSpecificConsentPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	procedureSpecificConsentPayloadResult.ok,
	"procedure-specific consent payload must pass creation guard",
);
...
assert(
	!missingPhotoVideoPayloadResult.ok &&
		missingPhotoVideoPayloadResult.statusCode === 409,
	"photo/video consent must require structured payload",
);
...
assert(
	photoVideoPayloadResult.ok,
	"photo/video consent payload must pass creation guard",
);
...
assert(
	!photoVideoConflictingPublicationResult.ok &&
		photoVideoConflictingPublicationResult.statusCode === 409,
...
assert(
	!missingXrayPayloadResult.ok && missingXrayPayloadResult.statusCode === 409,
	"xray/cbct referral must require structured payload",
);
...
assert(
	xrayPayloadResult.ok,
	"xray/cbct referral payload must pass creation guard",
);
...
assert(
	!missingMedicalRecordExtractPayloadResult.ok &&
		missingMedicalRecordExtractPayloadResult.statusCode === 409,
...
assert(
	medicalRecordExtractPayloadResult.ok,
	"medical record extract payload must pass creation guard",
);

const missingDental043uPayloadResult = validateDocumentCreation(
	{ patientId, visitId, kind: "dental_medical_card_043u" },
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	!missingDental043uPayloadResult.ok &&
		missingDental043uPayloadResult.statusCode === 409,
	"dental medical card 043/u must require structured payload",
);
const dental043uPayloadResult = validateDocumentCreation(
	{
		patientId,
		visitId,
...
assert(
	dental043uPayloadResult.ok,
	"dental medical card 043/u payload must pass creation guard",
);
...
assert(
	!missingMedicalRecordCopyRequestPayloadResult.ok &&
		missingMedicalRecordCopyRequestPayloadResult.statusCode === 409,
...
assert(
	medicalRecordCopyRequestPayloadResult.ok,
	"medical record copy request payload must pass creation guard",
);
...
assert(
	!missingPostVisitRecommendationsPayloadResult.ok &&
		missingPostVisitRecommendationsPayloadResult.statusCode === 409,
...
const postVisitRecommendationsPayloadResult = validateDocumentCreation(
	{
		...
		payload: postVisitRecommendationsPayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	postVisitRecommendationsPayloadResult.ok,
	"post visit recommendations payload must pass creation guard",
);
...
assert(
	!missingTreatmentPlanPayloadResult.ok &&
		missingTreatmentPlanPayloadResult.statusCode === 409,
...
assert(
	treatmentPlanPayloadResult.ok,
	"treatment plan payload must pass creation guard",
);
...
assert(
	!missingTreatmentPlanAcceptancePayloadResult.ok &&
		missingTreatmentPlanAcceptancePayloadResult.statusCode === 409,
...
const treatmentPlanAcceptancePayloadResult = validateDocumentCreation(
	{
		...
		visitId,
		kind: "treatment_plan_acceptance",
		payload: treatmentPlanAcceptancePayload,
	},
	{ patient, visit, paidAmountRub: 0, plannedAmountRub: 7000 },
);
assert(
	treatmentPlanAcceptancePayloadResult.ok,
	"treatment plan acceptance payload must pass creation guard",
);
...
assert(
	!missingAttendancePayloadResult.ok &&
		missingAttendancePayloadResult.statusCode === 409,
...
assert(
	attendancePayloadResult.ok,
	"visit attendance certificate payload must pass creation guard",
);
...
assert(
	!missingReleasePayloadResult.ok &&
		missingReleasePayloadResult.statusCode === 409,
	"medical document release must require structured payload",
);
...
assert(
	releasePayloadResult.ok,
	"medical document release payload must pass creation guard",
);
...
assert(
	!missingRefundPayloadResult.ok &&
		missingRefundPayloadResult.statusCode === 409,
	"refund/correction must require structured payload",
);
const refundPayloadResult = validateDocumentCreation(refundInput, {
	patient,
	visit,
	paidAmountRub: 3000,
	plannedAmountRub: 0,
...
assert(
	refundPayloadResult.ok && refundPayloadResult.input.totalAmountRub === 3000,
	"refund/correction payload must pass creation guard",
);
assert(
	paymentRefundCorrectionSelectionErrorForDocument(refundInput, payments) ===
		null,
	...
	paymentRefundCorrectionSelectionErrorForDocument(
		{
			...
			payload: {
				paymentRefundCorrection: {
					...refundPayload.paymentRefundCorrection,
					selectedPaymentIds: [],
				},
			},
		},
	...
	paymentRefundCorrectionSelectionErrorForDocument(
		{
			...
			payload: {
				paymentRefundCorrection: {
					...refundPayload.paymentRefundCorrection,
					selectedPaymentIds: [taxPaymentOtherPayerId],
				},
			},
		},
		payments,
...
assert(
	!refundTooLargeResult.ok && refundTooLargeResult.statusCode === 409,
	"refund/correction amount must not exceed paid facts",
);
...
assert(
	!missingPersonalDataPayloadResult.ok &&
		missingPersonalDataPayloadResult.statusCode === 409,
...
const personalDataPayloadResult = validateDocumentCreation(
	{
		...
		payload: personalDataConsentPayload,
	},
	{ patient, visit: null, paidAmountRub: 0, plannedAmountRub: 0 },
);
assert(
	personalDataPayloadResult.ok,
	"personal data consent payload must pass creation guard",
);
...
assert(
	!mismatchedPayloadSchemaResult.success,
	"createDocumentSchema must reject unrelated payload branches",
);
...
assert(
	!mismatchedPayloadGuardResult.ok &&
		mismatchedPayloadGuardResult.statusCode === 409,
	"creation guard must reject unrelated payload branches",
);

const missingTaxYear = validateDocumentCreation(
	taxCertificateWithoutYearInput,
	{
		patient,
		visit: null,
		paidAmountRub: paidAmountRubForDocument(
...
assert(
	!missingTaxYear.ok && missingTaxYear.statusCode === 409,
	"tax paid documents must require explicit payment year",
);

const taxCertificateWithoutPayerResult = validateDocumentCreation(
	taxCertificateWithoutPayerInput,
	{
		patient,
		visit: null,
		paidAmountRub: paidAmountRubForDocument(
...
assert(
	taxCertificateWithoutPayerResult.ok &&
		taxCertificateWithoutPayerResult.input.taxPayerInn === null,
...
assert(
	!legacyTaxYear.ok &&
		legacyTaxYear.statusCode === 409 &&
		legacyTaxYear.error.includes("2024"),
	"KND 1151156 documents must be blocked for pre-2024 payment years",
);
const oldFormTaxPaidAmount = paidAmountRubForDocument(
	"legacy_tax_deduction_certificate",
	oldFormTaxCertificateInput,
	payments,
);
assert(
	oldFormTaxPaidAmount === 4000,
	`legacy tax certificate must use 2021-2023 paid amount, got ${oldFormTaxPaidAmount}`,
);
...
assert(
	oldFormTaxResult.ok && oldFormTaxResult.input.totalAmountRub === 4000,
	"legacy tax certificate must normalize old-year paid amount",
);
const oldFormWrongYearResult = validateDocumentCreation(
	oldFormTaxCertificateWrongYearInput,
	{
		patient,
		visit: null,
		paidAmountRub: paidAmountRubForDocument(
...
assert(
	!missingPayment.ok && missingPayment.statusCode === 409,
	"tax certificate without payments must be blocked",
);

const taxPaidAmount = paidAmountRubForDocument(
	"tax_deduction_certificate",
	taxCertificateInput,
	payments,
);
assert(
	taxPaidAmount === 3000,
	`tax documents must aggregate only selected-year selected-payer paid records, got ${taxPaidAmount}`,
);
const spacedDocumentInnTaxInput = {
	...taxCertificateInput,
	taxPayerInn: "123 456 789 012",
};
const spacedDocumentInnSelectionError = taxPaymentSelectionErrorForDocument(
	spacedDocumentInnTaxInput,
	payments,
);
assert(
...
const spacedPaymentInnPayments = payments.map((payment) =>
	payment.id === taxPayment2026Id
		? { ...payment, payerInn: "123 456 789 012" }
		: payment,
);
const spacedPaymentInnSelectionError = taxPaymentSelectionErrorForDocument(
	taxCertificateInput,
	spacedPaymentInnPayments,
);
assert(
...
assert(
	taxResult.ok && taxResult.input.totalAmountRub === 3000,
	"tax certificate must normalize to selected-year selected-payer paid amount",
);
const scopedTaxPaidAmount = paidAmountRubForDocument(
	"tax_deduction_certificate",
	scopedTaxCertificateInput,
	payments,
);
assert(
	scopedTaxPaidAmount === 2000,
	`tax certificate payer scope must filter selected payer only, got ${scopedTaxPaidAmount}`,
);
...
const registryPaidAmount = paidAmountRubForDocument(
...
assert(
	registryResult.ok && registryResult.input.totalAmountRub === 5000,
	"tax registry must normalize to selected-year paid amount",
);
...
const contractPlanAmount = plannedAmountRubForDocument(
	"paid_medical_services_contract",
	completeContractInput,
	treatmentPlanItems,
);
assert(
	contractPlanAmount === 10000,
	`paid contract must use active visit plan amount, got ${contractPlanAmount}`,
);
...
assert(
	contractResult.ok && contractResult.input.totalAmountRub === 10000,
	"contract must normalize to visit plan amount",
);

const completeInvoiceInput = {
	...invoiceInput,
	payload: paymentInvoicePayload,
};
const invoicePlanAmount = plannedAmountRubForDocument(
	"payment_invoice",
	completeInvoiceInput,
	treatmentPlanItems,
);
assert(
	invoicePlanAmount === 10000,
	`payment invoice must use active visit plan amount, got ${invoicePlanAmount}`,
);
...
assert(
	invoiceResult.ok && invoiceResult.input.totalAmountRub === 10000,
	"payment invoice must normalize to visit plan amount",
);

const completeEstimateInput = {
	...estimateWithoutVisitInput,
	visitId,
	payload: treatmentCostEstimatePayload,
};
const estimatePlanAmount = plannedAmountRubForDocument(
	"treatment_cost_estimate",
	completeEstimateInput,
	treatmentPlanItems,
);
assert(
	estimatePlanAmount === 10000,
	`treatment estimate must use active visit plan amount, got ${estimatePlanAmount}`,
);
...
assert(
	estimateResult.ok && estimateResult.input.totalAmountRub === 10000,
	"treatment estimate must normalize to visit plan amount",
);
...
const schedulePlanAmount = plannedAmountRubForDocument(
	"installment_payment_schedule",
	completeScheduleInput,
	treatmentPlanItems,
);
assert(
	schedulePlanAmount === 10000,
	`installment schedule must use active visit plan amount, got ${schedulePlanAmount}`,
);
...
assert(
	scheduleResult.ok && scheduleResult.input.totalAmountRub === 10000,
	"installment schedule must normalize to visit plan amount",
);

const completeActInput = { ...actInput, payload: completedWorksActPayload };
const actPaidAmount = paidAmountRubForDocument(
	"completed_works_act",
	completeActInput,
	payments,
);
assert(
	actPaidAmount === 3000,
	`visit-scoped paid document must use only visit payments, got ${actPaidAmount}`,
);
const actResult = validateDocumentCreation(completeActInput, {
	patient,
	visit,
	paidAmountRub: actPaidAmount,
	plannedAmountRub: 0,
});
assert(
	actResult.ok && actResult.input.totalAmountRub === 3000,
	"act must normalize to visit paid amount",
);

const receiptWithoutVisitResult = validateDocumentCreation(
	receiptWithoutVisitInput,
	{
		patient,
		visit: null,
		paidAmountRub: paidAmountRubForDocument(
...
const estimateWithoutVisitResult = validateDocumentCreation(
	estimateWithoutVisitInput,
	{
		patient,
		visit: null,
...
assert(
	!estimateWithoutVisitResult.ok &&
		estimateWithoutVisitResult.statusCode === 409,
	"planned-amount documents without visit/plan scope must be blocked",
);

const completeApplicationInput = {
	...applicationInput,
	payload: taxApplicationPayload,
};
...
assert(
	applicationResult.ok && applicationResult.input.totalAmountRub === null,
	"tax application must not carry fake amount",
);

const completePlanAcceptanceInput = {
	...planAcceptanceInput,
	payload: treatmentPlanAcceptancePayload,
};
const planAmount = plannedAmountRubForDocument(
	"treatment_plan_acceptance",
	completePlanAcceptanceInput,
	treatmentPlanItems,
);
assert(
	planAmount === 10000,
	`planned visit document must use active visit plan items, got ${planAmount}`,
);
const planAcceptanceResult = validateDocumentCreation(
	completePlanAcceptanceInput,
	{
		patient,
		visit,
...
assert(
	planAcceptanceResult.ok &&
		planAcceptanceResult.input.totalAmountRub === 10000,
	"planned document must ignore user-supplied/global totals",
);

const missingRequiredVisitResult = validateDocumentCreation(
	missingVisitRequiredInput,
	{
		patient,
		visit: null,
		paidAmountRub: 0,
...
assert(
	!missingRequiredVisitResult.ok &&
		missingRequiredVisitResult.statusCode === 409,
	"visit-required document without visitId must be blocked",
);

const wrongVisitResult = validateDocumentCreation(
	taxCertificateWithVisitInput,
	{
		patient,
		visit: wrongVisit,
		paidAmountRub: taxPaidAmount,
...
assert(
	!wrongVisitResult.ok && wrongVisitResult.statusCode === 409,
	"wrong visit/patient link must be blocked",
);

const missingVisitResult = validateDocumentCreation(
	taxCertificateWithVisitInput,
	{
		patient,
		visit: null,
...
assert(
	!missingVisitResult.ok && missingVisitResult.statusCode === 404,
	"missing visit must be blocked when visitId is supplied",
);
...
console.log(
	JSON.stringify({
		...
		blockedPatientIntakeWithoutPayload:
			missingPatientIntakePayloadResult.statusCode,
		blockedPaidContractWithoutPayload:
			missingPaidContractPayloadResult.statusCode,
		blockedPaidContractAmountMismatch: badPaidContractAmountResult.statusCode,
		blockedCompletedWorksActWithoutPayload:
			missingCompletedWorksActPayloadResult.statusCode,
		blockedCompletedWorksActPaidMismatch:
			badCompletedWorksActPaidResult.statusCode,
		blockedTreatmentEstimateWithoutPayload:
			missingTreatmentEstimatePayloadResult.statusCode,
		blockedTreatmentEstimateLineMismatch:
			badTreatmentEstimateLineResult.statusCode,
		blockedTreatmentEstimateTotalMismatch:
			badTreatmentEstimateTotalResult.statusCode,
		blockedPaymentInvoiceWithoutPayload:
			missingPaymentInvoicePayloadResult.statusCode,
		blockedPaymentInvoiceTotalMismatch: badPaymentInvoiceTotalResult.statusCode,
		blockedInstallmentScheduleWithoutPayload:
			missingInstallmentSchedulePayloadResult.statusCode,
		blockedInstallmentScheduleBalanceMismatch:
			badInstallmentScheduleBalanceResult.statusCode,
		blockedMinorRepresentativeWithoutPayload:
			missingMinorRepresentativePayloadResult.statusCode,
		blockedWarrantyWithoutPayload: missingWarrantyPayloadResult.statusCode,
		blockedInformedConsentWithoutPayload:
			missingInformedConsentPayloadResult.statusCode,
		blockedProcedureSpecificConsentWithoutPayload:
			missingProcedureSpecificConsentPayloadResult.statusCode,
		blockedPhotoVideoWithoutPayload: missingPhotoVideoPayloadResult.statusCode,
		blockedPhotoVideoConflictingPublication:
			photoVideoConflictingPublicationResult.statusCode,
		blockedXrayWithoutPayload: missingXrayPayloadResult.statusCode,
		blockedMedicalRecordExtractWithoutPayload:
			missingMedicalRecordExtractPayloadResult.statusCode,
		blockedMedicalRecordCopyRequestWithoutPayload:
			missingMedicalRecordCopyRequestPayloadResult.statusCode,
		blockedPostVisitRecommendationsWithoutPayload:
			missingPostVisitRecommendationsPayloadResult.statusCode,
		blockedTreatmentPlanWithoutPayload:
			missingTreatmentPlanPayloadResult.statusCode,
		blockedTreatmentPlanAcceptanceWithoutPayload:
			missingTreatmentPlanAcceptancePayloadResult.statusCode,
		...
		blockedPersonalDataWithoutPayload:
			missingPersonalDataPayloadResult.statusCode,
