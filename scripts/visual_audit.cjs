...
const VIEWS = [
  { name: 'dashboard',    hash: 'dashboard',  wait: 2500 },
];
...
(async () => {
  ...
  page.on('requestfailed', request => {
    console.error(`[Browser Network] Failed: ${request.url()} - ${request.failure().errorText}`);
  });
  page.on('response', response => {
    if (!response.ok() && response.url().includes('/api/')) {
       console.error(`[Browser Network] API Error: ${response.status()} on ${response.url()}`);
    }
  });
      ...
      if (url.includes('/api/dashboard')) {
        request.respond({
          ...
          body: JSON.stringify({
            ...
            patients: [],
            clinicSettings: {
              profile: { name: "Audit Mock Clinic", id: "mock-1", organizationId: "mock-org-1", mode: "solo_doctor" },
              staff: [{ id: 'staff1', name: 'Dr. Test', active: true, role: 'doctor' }],
