 ...
 *
 * Почему список, а не маркер в самом файле: маркер требует правки файла охраны, а
 * `routes/auth.ts` в этой волне трогать нельзя (правка запланирована отдельно).
 * Запись здесь видна в выводе каждого прогона, поэтому спрятать долг ею нельзя.
 */
const KNOWN_UNCALLED_GUARD_DEBT = new Map([
	[
		"assertTenantMatch",
		{
			file: "apps/api/src/clinicalAuditService.ts",
			reason:
				"НАЙДЕН ЭТОЙ ПРОВЕРКОЙ на первом же прогоне. Единственное вхождение имени во всём дереве — само " +
				"объявление (проверено независимым поиском). Дырой сегодня не является: это чистое сравнение " +
				"двух строк, возвращающее boolean, а не барьер, — изоляцию клиник держат подписанный токен " +
				"(security/identity.ts) и фильтр organization_id в запросах, и это измеряет сквозной сценарий " +
				"tests/security/crossTenantReconProof.ts. Но форма ровно та же: инженер, проверяющий «сверяется " +
				"ли клиника ресурса», находит имя поиском и получает ложное спокойствие. Решение — удалить " +
				"поимённо; файл вне области этой волны.",
		},
	],
	[
		"requireClinicToken",
		{
			...
			reason:
				"признан МЁРТВЫМ, а не несшитым: сшивать нечего. Единственное его действие — запись " +
				"request.clinicOrganizationId, которую во всём дереве не читает никто, а проверку подписи " +
				"токена кабинета делает security/identity.ts (getRequestIdentity + requireOrganizationId) " +
				"строже: с кэшем, с меткой verified и с отказом непроверенной организации на запись. " +
				"Сшить его общим middleware НЕЛЬЗЯ — он принимает только x-dente-clinic-token и отвечал бы " +
				"401 на маршрутах отчётов, которые ходят по x-dente-staff-token. Удаление поимённо — " +
				"отдельная задача следующей волны, auth.ts правится другим владельцем.",
		},
	],
]);
