...
import { waitFor, evaluate } from "./lib/cdp.mjs";

const OUT = "C:/Users/Admin/.gemini/antigravity/brain/68ee88bd-584a-4a7f-948f-9353d73ce6fe/scratch/screenshots";
...
const tmpProfile = path.join(process.env.TEMP || "C:/tmp", "screenshot-browser-profile2");
...
try {
  ...
  const dashData = await fetch(`${apiBaseUrl}/api/dashboard`).then(r => r.json());
  console.log("Docs:", JSON.stringify(dashData.documents?.map(d => ({ id: d.id, kind: d.kind }))));

  // Create a treatment plan with AI payload via API
  const activeVisitId = dashData.activeVisit?.id;
  const activePatientId = dashData.patients?.find(p => p.id === dashData.activeVisit?.patientId)?.id;
  ...
  if (activeVisitId && activePatientId) {
    ...
    const treatmentPlanPayload = {
      ...
      payload: {
        treatmentPlan: {
          ...
          prognosisAndLimits: "Прогноз благоприятный при соблюдении рекомендаций по гигиене",
          alternatives: ["Удаление зуба с последующей имплантацией", "Наблюдение без активного лечения"],
          risksAndLimitations: ["Необходимость повторного лечения", "Изменение диагноза после диагностики"],
          plannedStages: [
            {
          ...
          patientFriendlyExplanation: "У вас кариес на зубе 36 (нижний коренной). Зуб лечим в 2 этапа: сначала диагностика, потом лечение с удалением повреждённой ткани и пломбировкой. После лечения зуб будет служить долго при хорошей гигиене.",
          patientHygieneAdvice: "Чистите зубы дважды в день по 3 минуты. Используйте межзубные ёршики — именно там начинается большинство кариесов. После лечения не нагружайте зуб твёрдой пищей 2 дня. Рекомендуем пасту с фтором 1450 ppm."
        }
      }
    };
    ...
    if (createData.id) {
      ...
      await cdp.send("Page.navigate", { url: `${apiBaseUrl}/api/documents/${docId}/treatment-plan-pdf?preview=1` });
      await sleep(4000);
      ...
      await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 1500)", returnByValue: true });
      await sleep(500);
      await shot("B02_treatment_plan_middle");

      await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 3500)", returnByValue: true });
      await sleep(500);
      await shot("B03_treatment_plan_lower");

      await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 6000)", returnByValue: true });
      await sleep(500);
      await shot("B04_treatment_plan_hygiene");

      await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, document.body.scrollHeight)", returnByValue: true });
      await sleep(500);
