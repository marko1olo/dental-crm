...
async function run() {
	...
	await check(
		"200 valid org",
		`${BASE}/api/communications/appointment-channel-inheritances`,
		VALID_ORG,
		200,
	);
	await check(
	...
	await check(
		"200 valid org",
		`${BASE}/api/schedule/external-schedule-action-logs`,
		VALID_ORG,
		200,
	);
	await check(
	...
	await check(
		"200 valid org",
		`${BASE}/api/crm/patient-duplicate-merge-queues`,
		VALID_ORG,
		200,
	);
	await check(
	...
	await check(
		"200 valid org",
		`${BASE}/api/integrations/yandex-calendar-syncs`,
		VALID_ORG,
		200,
	);
	await check(
