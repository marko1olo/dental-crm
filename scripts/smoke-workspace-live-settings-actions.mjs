...
const tempRoot = path.join(
	os.tmpdir(),
	`dental-crm-live-settings-actions-${process.pid}`,
);
...
if (!browserPath) {
	throw new Error(
		"No Chromium/Edge browser found. Set BROWSER_BIN to run the workspace live settings actions smoke test.",
	);
}
...
async function waitForRecognitionJobs(predicate, label, attempts = 80) {
	...
	throw new Error(
		`${label} did not appear in AI recognition jobs: ${JSON.stringify(current ?? null)}`,
	);
}
...
try {
	...
	const nextMode = initialDashboard.clinicSettings.workspaceProfiles.find(
		(profile) => profile.mode !== initialMode,
	)?.mode;
	if (!nextMode)
		throw new Error(
			"No alternate clinic mode found for live settings smoke action",
		);
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-clinic .clinic-config")))()`,
	...
	if (!adminUnlockResult.ok)
		throw new Error(
			`Settings admin unlock form was not filled: ${JSON.stringify(adminUnlockResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	if (!profileResult.ok)
		throw new Error(
			`Clinic profile form was not filled: ${JSON.stringify(profileResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	await waitForDashboard(
		(state) => state.clinicSettings.profile.clinicName === clinicName,
		"saved clinic profile",
	);
	await waitFor(
		cdp,
	...
	await waitForDashboard(
		(state) => state.clinicSettings.profile.mode === nextMode,
		"changed clinic mode",
	);
	await waitFor(
		cdp,
	...
	if (!staffResult.ok)
		throw new Error(
			`Staff quick-create form was not filled: ${JSON.stringify(staffResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	await waitForDashboard(
		(state) =>
			state.clinicSettings.staff.some(
				(member) => member.fullName === staffName,
			),
	...
	if (!chairResult.ok)
		throw new Error(
			`Chair quick-create form was not filled: ${JSON.stringify(chairResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	await waitForDashboard(
		(state) =>
			state.clinicSettings.chairs.some((chair) => chair.name === chairName),
		"created chair",
	);
	await waitFor(
		cdp,
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-ai .recognition-workbench")))()`,
		"AI recognition workbench",
	);
	...
	if (!recognitionFillResult.ok)
		throw new Error(
			`AI recognition workbench was not filled: ${JSON.stringify(recognitionFillResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-ai .recognition-result")))()`,
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-imports .smart-import-studio .import-workbench")))()`,
		"smart import workbench",
	);
	...
	if (!smartImportFillResult.ok)
		throw new Error(
			`Smart import workbench was not filled: ${JSON.stringify(smartImportFillResult)}`,
		);
	await waitFor(
		cdp,
		`(() => {
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-imports .smart-import-studio .import-preview")))()`,
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-sources .imaging-import-studio .import-workbench")))()`,
		"imaging import workbench",
	);
	await evaluate(
		cdp,
		`(() => {
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector("#settings-panel-sources .imaging-import-studio .import-preview")))()`,
	...
	if (
		!imagingPreviewSnapshot.hasPreview ||
		imagingPreviewSnapshot.rowCount === 0
	) {
		throw new Error(
			`Imaging import preview rows were not rendered: ${JSON.stringify(imagingPreviewSnapshot)}`,
		);
	}
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector('[data-testid="browser-picked-imaging-folder-result"]')))()`,
		"browser picked imaging files",
		160,
	);
	await waitFor(
		cdp,
	...
	const finalScreenshot = await saveScreenshot(
		cdp,
		"settings-ai-import-sources-after-actions",
	);
	cdp.close();

	const finalDashboard = await dashboard();
	const finalRecognitionJobs = await recognitionJobs();
	const createdStaff = finalDashboard.clinicSettings.staff.find(
		(member) => member.fullName === staffName,
	);
	const createdChair = finalDashboard.clinicSettings.chairs.find(
		(chair) => chair.name === chairName,
	);
	const latestRecognitionJob = finalRecognitionJobs[0];

	if (finalDashboard.clinicSettings.profile.clinicName !== clinicName)
		throw new Error("Saved clinic profile was not found in final dashboard");
	if (finalDashboard.clinicSettings.profile.mode !== nextMode)
		throw new Error("Changed clinic mode was not found in final dashboard");
	if (!createdStaff)
		throw new Error("Created staff member was not found in final dashboard");
	if (!createdChair)
		throw new Error("Created chair was not found in final dashboard");
	if (finalRecognitionJobs.length <= initialRecognitionJobCount)
		throw new Error(
			"Created AI recognition job was not found in final AI job list",
		);
	...
	console.log(
		JSON.stringify({
			...
			createdRecognitionJobId:
				latestRecognitionJob?.id ?? createdRecognitionJob?.id ?? null,
