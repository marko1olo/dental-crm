...
/*
 * НАБОР ЭКРАНА ПРИЛОЖЕНИЯ: ТОЛЬКО СМОНТИРОВАННЫЕ ПОВЕРХНОСТИ.
 *
 * ЗДЕСЬ ЧИТАЛСЯ `components/onboarding/`, И ЭТО БЫЛА МОЯ ОШИБКА. Каталог был
 * добавлен 2026-08-10 после проверки монтирования — но проверка смотрела в
 * РАБОЧЕЕ ДЕРЕВО другого агента, где App.tsx его импортировал. В коммите
 * a468d94c5 такого импорта не было НИКОГДА, и сегодня его нет: `rg` по всему
 * apps/web не находит ни одного импортёра `OnboardingGuidePanel` и
 * `FullscreenOnboardingWizard`, кроме теста на существование файлов. Каталог
 * не отслеживается гитом вовсе.
 *
 * Живая разметка онбординга — в App.tsx (коммит 6fe5532cd, две недели):
 * `aria-current` на шаге стоит в App.tsx:1505 и :2186.
 *
 * ЧЕМ ЭТО БЫЛО ОПАСНО. Пока каталог читался, требования выполнялись разметкой,
 * которую никто не рендерит: сломай онбординг в App.tsx — страж промолчал бы,
 * потому что сирота закрыла бы требование за живой экран. Ровно тот ложный
 * зелёный, о котором предупреждает комментарий в web-render-gating.
 *
 * Обнаружено обратным храповиком: после отката чужой правки счёт вырос с 2 до
 * 4, и `requireEveryInstance` отказался молчать. Счёт по составу поймал то,
 * чего не поймала бы проверка «есть хотя бы одно вхождение».
 */
/*
 * ЖИВОЙ ОНБОРДИНГ ПЕРЕЕХАЛ ЕЩЁ РАЗ — ТЕПЕРЬ В КОРЕНЬ src.
 *
 * Замер 2026-08-11: `App.tsx:70` импортирует мастер из
 * `./FullscreenOnboardingWizard` (корень `apps/web/src`), и там же лежит живая
 * разметка шагов и выбора роли. Каталог `components/onboarding/` при этом
 * существует и отслеживается гитом, но его НЕ ИМПОРТИРУЕТ НИКТО — это вторая,
 * мёртвая копия.
 *
 * Поэтому читается корневой файл, а каталог-дубль по-прежнему НЕ читается:
 * иначе требования выполнялись бы разметкой, которую никто не рендерит. Ровно
 * такую свою ошибку я откатывал в 55b759948, и повторять её нельзя.
 *
 * Если дубль однажды станет живым (появится импорт) — счёт экземпляров вырастет,
 * и обратный храповик потребует обновить число вместе с причиной.
 */
const appSource = [
	readFileSync("apps/web/src/App.tsx", "utf8"),
	readAppLogicSourceSync(),
	readFileSync("apps/web/src/FullscreenOnboardingWizard.tsx", "utf8"),
	/*
	 * AppRouter.tsx ЖИВ СЕЙЧАС, ХОТЯ КОММЕНТАРИИ В App.tsx НАЗЫВАЮТ ЕГО МЁРТВЫМ.
	 *
	 * App.tsx:120-141 пишет «AppRouter.tsx — мёртвом файле, который не
	 * импортировал никто» и даже «удалён». Проверено по факту: App.tsx:71 его
	 * ИМПОРТИРУЕТ, файл отслеживается гитом и содержит 16 признаков онбординга.
	 * Комментарий описывает прошлое состояние и сегодня врёт — доверяться надо
	 * замеру, а не описанию.
	 */
	readFileSync("apps/web/src/AppRouter.tsx", "utf8"),
