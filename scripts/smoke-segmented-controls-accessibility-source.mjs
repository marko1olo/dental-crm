...
const KNOWN_MISSING_SEGMENTED_SURFACES = new Map([
	[
		...
		"REAL — components/settings/SettingsAiTab.tsx:266 держит выбранный пресет " +
			"ТОЛЬКО классом оформления: className={`ai-target-card ${recognitionKind === " +
			"preset.kind && recognitionTarget === preset.target ? \"active\" : \"\"}`}. На " +
			"кнопке нет ни aria-pressed, ни aria-selected, ни role. Замер: связи " +
			"`recognitionKind === preset.kind` с любым атрибутом состояния нет ни в одном " +
			"файле apps/web/src. Выбранный пресет распознавания виден только глазами — " +
			"скринридер и клавиатурная навигация не сообщают, какой из них активен.",
	],
]);
...
function selectedStateBoundTo(expression) {
	return new RegExp(
		`${SELECTED_STATE_ATTRIBUTES}=\\{\\s*${selectionExpression(expression)}\\s*\\}`,
	);
}
