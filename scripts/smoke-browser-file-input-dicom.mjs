...
if (!Number.isInteger(port) || port <= 0 || port > 65_535) {
	throw new Error(
		`Invalid SMOKE_CDP_PORT: ${process.env.SMOKE_CDP_PORT ?? port}`,
	);
}
const screenshotPath =
	process.env.SMOKE_SCREENSHOT_PATH ??
	"docs/screenshots/browser-file-input-dicom-magic-mobile-current.png";
...
const profileDir = path.join(
	os.tmpdir(),
	`dental-crm-edge-file-input-smoke-${process.pid}`,
);
const fixtureDir = path.join(
	os.tmpdir(),
	`dental-crm-browser-picker-smoke-${process.pid}`,
);
...
if (!browserPath) {
	throw new Error(
		"No Chromium/Edge browser found. Set BROWSER_BIN to run the smoke test.",
	);
}
...
try {
	...
	const targets = await Promise.race([
		fetchJson(`http://127.0.0.1:${port}/json/list`, 100),
		browserExitFailure,
	]);
	const pageTarget =
	...
	if (!inputNode.nodeId) {
		throw new Error(
			`Browser local imaging file input was not found: ${JSON.stringify(routeSnapshot?.result.value ?? null)}`,
		);
	}
	...
	if (!inputState.result.value?.hasInput) {
		throw new Error(
			`Browser local imaging file input disappeared before upload: ${JSON.stringify(inputState.result.value)}`,
		);
	}
	...
	if (!result.hasResult || result.resultHidden)
		throw new Error("Browser picked imaging result did not render.");
	if (result.overflow > 1)
		throw new Error(
			`Mobile horizontal overflow detected: ${result.overflow}px.`,
		);
	if (result.leaks.length > 0)
		throw new Error(
			`Raw synthetic path/name leaked: ${result.leaks.join(", ")}`,
		);
	if (
		!String(result.storageKey).startsWith(
			"dental-crm:browser-picked-imaging-folder:last",
		)
	) {
		throw new Error(
			`Unexpected browser-picked folder storage key: ${result.storageKey}`,
		);
	}
	if (result.stored?.dicomLikeFiles !== 1)
		throw new Error("No-extension DICM fixture was not counted as DICOM.");
	if (result.stored?.archiveFiles !== 1)
		throw new Error("Archive fixture was not counted.");
	if (result.stored?.modelFiles !== 1)
		throw new Error("Dental 3D model fixture was not counted.");
	if (result.stored?.imageFiles !== 1)
		throw new Error("Image fixture was not counted.");
	if (!result.visibleText.includes("<0,1 МБ")) {
		throw new Error(
			"Small non-empty DICOM/browser folder size must not be shown as 0 МБ.",
		);
	}
	if (/\n0 МБ(?:\n|$)/.test(result.visibleText)) {
		throw new Error(
			"Small non-empty DICOM/browser folder size was shown as 0 МБ.",
		);
	}
