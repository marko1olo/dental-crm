...
const { savePersistentState, loadPersistentState } = await import(
	pathToFileURL(persistentStatePath).href
);
...
try {
	...
	assert(
		fileContent.state.staffMembers[0].id === "s1",
		"Saved state payload mismatch",
	);
	assert(
		typeof fileContent.checksum === "string",
	...
	assert(
		loadedState.staffMembers[0].name === "Dr. Smith",
		"Loaded state content mismatch",
	);
	...
	assert(
		updatedFileContent.state.staffMembers[0].name === "Dr. Smith Updated",
		"Updated state payload mismatch",
	);
	...
	try {
		const badTempRoot = mkdtempSync(
			path.join(tmpdir(), "dental-save-persist-bad-"),
		);
		...
		assert(
			!threw,
			"savePersistentState should not throw an error on file system failure",
		);
		assert(
			warnMessages.length > 0,
			"savePersistentState should log a warning on failure",
		);
		assert(
			warnMessages[0].startsWith("Dental state file save failed"),
