...
function splitNodeCommand(command) {
	...
	if (parts[0] !== "node")
		throw new Error(`Unsupported smoke command format: ${command}`);
...
console.log(
	`Running ${commands.length} smoke checks${onlyPattern ? ` matching ${onlyPattern}` : ""}.`,
);
const startedAt = Date.now();
const results = [];
...
const elapsedMs = Date.now() - startedAt;
console.log(
	`SUMMARY total=${results.length} failed=${failed.length} elapsedMs=${elapsedMs}`,
);

if (failed.length) {
	for (const result of failed) {
		console.log(
			`FAILED ${result.name} code=${result.code ?? "null"} signal=${result.signal ?? "null"}`,
		);
