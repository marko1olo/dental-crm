...
requireSnippet(
	...
	"protected preview fetches must be abortable",
);
requireSnippet(
	'if (currentView !== "imaging" || !dashboard?.imagingStudies.length) return [];',
	"preview fetch workset must stay gated to the visible imaging route",
);
requireSnippet(
	"const imagingPreviewWorkset = useMemo(() =>",
	"preview fetches must use a memoized visible workset",
);
requireSnippet(
	"chunk.map(async (study)",
	"preview fetches must not scan every dashboard image on app boot",
);
requireSnippet(
	"if (!imagingPreviewWorkset.length)",
	"preview URL map must clear when the imaging route has no visible workset",
);
requireSnippet(
	"signal: abortController.signal",
	"fetch must use the preview AbortController signal",
);
requireSnippet(
	"abortController.abort();",
	"cleanup must abort in-flight preview fetches",
);
requireSnippet(
	"revokeObjectUrlIfNeeded(blobUrl);",
	"late blob URLs created after cancellation must be revoked",
);
requireSnippet(
	"createdUrls.forEach(revokeObjectUrlIfNeeded);",
	"created preview blob URLs must be revoked on cleanup/error",
);
requireSnippet(
	"revokeObjectUrlMap(current);",
	"current preview blob URL map must be revoked before clearing state",
);
requireSnippet(
	"if (!nextUrls.has(url)) revokeObjectUrlIfNeeded(url);",
	"replaced preview blob URLs must be revoked",
);
requireSnippet(
	"headers: denteClinicalReadHeaders()",
	"protected preview fetches must keep clinical read headers",
);
...
requireSourceSnippet(
	...
	"DICOM tool-state download helper missing",
);
requireSourceSnippet(
	"function downloadDicomWorkbenchManifest()",
	"DICOM workbench download helper missing",
);
requireSourceSnippet(
	"revokeObjectUrlIfNeeded(url);",
	"DICOM download blob URLs must route through the central object URL revoke helper",
);
requireSourceSnippet(
