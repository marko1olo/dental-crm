...
if (
	!cssSource.includes("button:disabled") ||
	!cssSource.includes("cursor: not-allowed;")
) {
	throw new Error(
		"Disabled buttons must not look like a loading state; use cursor: not-allowed.",
	);
}
...
console.log(
	JSON.stringify(
		{ ok: true, checkedFiles: files.length, disabledCursorClear: true },
		null,
		2,
	),
);
