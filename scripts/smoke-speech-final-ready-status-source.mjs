...
if (finalizeStart === -1 || flushStart === -1)
	fail("Speech finalization block was not found.");
...
if (
	genericRetryIndex !== -1 &&
	finalizeBlock.indexOf("queuedCurrentRecordingCount > 0") > genericRetryIndex
) {
	fail(
...
if (waitStart === -1 || waitEnd === -1)
	fail("Speech upload wait block was not found.");
const waitBlock = appSource.slice(waitStart, waitEnd);
if (
	waitBlock.indexOf(
		"await new Promise<void>((resolve) => window.setTimeout(resolve, 150));",
	) > waitBlock.indexOf("if (!speechUploadPromisesRef.current.size) return;")
) {
	fail(
...
if (monitorStart === -1 || configureStart === -1)
	fail("Speech monitor block was not found.");
