...
process.on("unhandledRejection", (reason) => {
	if (isContextDestroyed(reason)) {
		console.log(
			"  ↻ отброшен ответ команды, потерянной при перезагрузке страницы",
		);
		return;
	}
...
async function getPageTarget(retries = 60) {
	...
	for (let attempt = 0; attempt < retries; attempt += 1) {
		try {
			...
			const page = targets.find(
				(target) =>
					target.type === "page" && String(target.url).startsWith(webBaseUrl),
			);
			if (page) return page;
		} catch {
	...
	if (lastSeen.length === 0) {
		throw new Error(
			`Отладочный порт браузера ${cdpPort} не отдал ни одной вкладки за ${retries} с. Браузер не запустился.`,
		);
	}
	throw new Error(
	...
	return new Promise((resolve, reject) => {
		...
		setTimeout(() => {
			if (pending.has(id)) {
				...
				reject(
					new Error(
						`${method}: нет ответа за ${Math.round(timeoutMs / 1000)} с`,
					),
				);
			}
		}, timeoutMs);
...
async function restoreSession() {
	...
	if (session.width && session.height)
		await setViewport(session.width, session.height);
...
async function shootViewport(fileName, theme, marker = null) {
	...
	const name = wrongSection
		? fileName.replace(/\.png$/, "_ПУСТО.png")
		: fileName;
	...
	await writeShot(
		...
		theme,
		themeState,
		wrongSection ? "НЕ ТОТ РАЗДЕЛ: " + wrongSection : "весь экран",
	);
}
...
for (const theme of THEMES) {
	console.log(`\nТема: ${theme}`);
	const applied = await withReloadRecovery(`применение темы ${theme}`, () =>
		applyTheme(theme),
	);
	console.log(
...
await withReloadRecovery("narrow/весь экран", () =>
	shootViewport("narrow_full.png", "light"),
);
...
await withReloadRecovery("finance/весь раздел", async () => {
	...
	await shootClipped(
		"finance_full.png",
		"light",
		{ x: 0, y: 0, width: 1600, height: financeHeight, scale: 0.55 },
		'.finance-split, .finance-list, [data-testid="finance-planning"]',
	);
});
...
for (const theme of THEMES) {
	await withReloadRecovery(`patients/${theme}`, async () => {
		...
		await sleep(1800);
		await shootViewport(
			`patients_${theme}_full.png`,
			theme,
			".patients-panel, .patient-list, .patients-main-grid",
		);
	});
}
...
await withReloadRecovery("communications/весь раздел", async () => {
	...
	await shootClipped(
		"communications_full.png",
		"light",
		{ x: 0, y: 0, width: 1600, height: pageHeight, scale: 0.5 },
		'[data-testid="communications-view"], .communications-summary-grid',
	);
});
