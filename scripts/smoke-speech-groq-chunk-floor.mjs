...
const { getSpeechGatewayStatus, buildSpeechRecordingStrategy } = await import(
	pathToFileURL(gatewayPath).href
);
...
if (result.providerId !== "groq_whisper")
	throw new Error(`Expected groq_whisper, got ${result.providerId}`);
if (result.minChunkMs < 10_000)
	throw new Error(`Groq min chunk was not enforced: ${result.minChunkMs}`);
if (result.recommendedChunkMs < 10_000)
	throw new Error(
		`Groq recommended chunk was not enforced: ${result.recommendedChunkMs}`,
	);
if (result.maxChunkMs < result.minChunkMs)
	throw new Error(
		`Chunk max below min: ${result.maxChunkMs} < ${result.minChunkMs}`,
	);
if (result.strategyChunkMs < 10_000)
	throw new Error(`Strategy chunk stayed too small: ${result.strategyChunkMs}`);
if (
