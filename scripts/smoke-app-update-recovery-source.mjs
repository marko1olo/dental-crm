...
function forbidIn(source, marker, label) {
	if (source.includes(marker))
		throw new Error(`${label} includes forbidden marker: ${marker}`);
}
...
[
	...
	requireIn(
		appShellSource,
		marker,
		"App shell stale chunk recovery affordance",
	),
);
...
[
	...
	requireIn(
		routeBoundarySource,
		marker,
		"Workspace route stale chunk recovery affordance",
	),
);

["onClick={() => window.location.reload()}"].forEach((marker) => {
	forbidIn(
		appShellSource,
		marker,
		"App shell must clear shell cache before manual stale refresh",
	);
	forbidIn(
		routeBoundarySource,
		marker,
		"Workspace route must clear shell cache before manual stale refresh",
	);
});
