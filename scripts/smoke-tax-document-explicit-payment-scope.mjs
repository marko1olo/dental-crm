...
try {
	...
	const firstPayment = payments.find(
		(payment) =>
	...
	assert(
		firstPayment,
		"fixture must contain a tax payment for explicit scope smoke",
	);
	...
	assert(
		missingSelectionResponse.statusCode === 409,
		`tax certificate without selected payments must be 409, got ${missingSelectionResponse.statusCode}`,
	);
	const missingSelectionError = documentErrorText(missingSelectionResponse);
	assert(
		missingSelectionError.includes("явно") ||
			missingSelectionError.includes("фискальные"),
	...
	const applicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: firstPayment.payerFullName,
				note: "first payment application",
			},
		}),
	});
	assert(
	...
	const certificateCreateResponse = await app.inject({
		...
		payload: {
			...
			totalAmountRub: 999999,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: [firstPayment.id] },
			},
		},
	});
	assert(
	...
	const certificateIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: firstPayment.payerFullName,
				note: "first payment certificate",
			},
		}),
	});
	assert(
		certificateIssueResponse.statusCode === 200,
		`tax certificate issue failed: ${certificateIssueResponse.statusCode} ${certificateIssueResponse.body}`,
	);
	const certificate = documents.find(
		(document) => document.id === certificateCreateResponse.json().id,
	);
	assert(
		certificate?.taxPaymentSnapshot?.paymentIds.includes(firstPayment.id),
		"certificate snapshot must include explicitly selected first payment",
	);
	assert(
		!certificate?.taxPaymentSnapshot?.paymentIds.includes(secondPayment.id),
		"certificate snapshot must not capture unselected second payment",
	);
	assert(
		certificate?.totalAmountRub === firstPayment.amountRub,
		"certificate amount must equal explicitly selected first payment",
	);
	...
	const registryCreateResponse = await app.inject({
		...
		payload: {
			...
			totalAmountRub: 999999,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: [secondPayment.id] },
			},
		},
	});
	assert(
			...
			payload: {
				taxDeductionApplication: {
					...
					applicantAuthorityDocument:
						"Synthetic spouse authority document for mismatch smoke",
	...
	const wrongRelationshipApplicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: firstPayment.payerFullName,
				note: "wrong relationship application",
			},
		}),
	});
	assert(
	...
	assert(
		secondApplicationCreateResponse.statusCode === 201,
		`second tax application create failed: ${secondApplicationCreateResponse.statusCode}`,
	);
	const secondApplicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: firstPayment.payerFullName,
				note: "second payment application",
			},
		}),
	});
	assert(
	...
	const registryIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: firstPayment.payerFullName,
				note: "second payment registry",
			},
		}),
	});
	assert(
		registryIssueResponse.statusCode === 200,
		`tax registry issue failed: ${registryIssueResponse.statusCode} ${registryIssueResponse.body}`,
	);
	const registry = documents.find(
		(document) => document.id === registryCreateResponse.json().id,
	);
	assert(
		registry?.taxPaymentSnapshot?.paymentIds.includes(secondPayment.id),
		"registry snapshot must include explicitly selected second payment",
	);
	assert(
		!registry?.taxPaymentSnapshot?.paymentIds.includes(firstPayment.id),
		"registry snapshot must not capture unselected first payment",
	);
	assert(
		registry?.totalAmountRub === secondPayment.amountRub,
		"registry amount must equal explicitly selected second payment",
	);
