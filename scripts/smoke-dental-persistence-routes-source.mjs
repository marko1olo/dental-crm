...
const invoiceSplitModal = readFileSync(
...
requireIn(
	invoiceSplitModal,
	"denteAdminSecretRequestHeaders({ 'Content-Type': 'application/json' })",
	"Split-payment modal writes must send tenant/session headers.",
);
requireIn(
	invoiceSplitModal,
...
forbidIn(
	thermalReceipt,
	"????",
	"Receipt simulator must not render mojibake/question-mark text.",
);
requireIn(
	thermalReceipt,
	"Кассовый чек №",
	"Receipt simulator must render readable Russian receipt copy.",
);
requireIn(
	thermalReceipt,
	"Спасибо за оплату",
	"Receipt simulator footer must be readable Russian.",
);
