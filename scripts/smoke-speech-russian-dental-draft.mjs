...
assert(
	normalized.changedPhrases.length >= 6,
	"normalization must track changed dental phrases",
);
...
assert(
	draft.quality?.detectedToothCodes.includes("36"),
	"quality must detect FDI 36",
);
assert(
	draft.quality?.signals.includes("imaging_mentioned"),
	"quality must detect imaging mention",
);
assert(
	draft.quality?.signals.includes("procedure_mentioned"),
	"quality must detect procedure mention",
);
assert(
	draft.quality?.signals.includes("plan_detected"),
	"quality must detect plan",
);
...
const scopedDraft = buildRuleBasedVisitDraftFromTranscript(
	scopedTranscript,
	"therapist",
);
const scopedNormalized = normalizeDentalSpeechTranscript(
	scopedTranscript,
	"therapist",
);

includesText(
	scopedNormalized.normalizedText,
	"ИРОПЗ",
	"scoped normalized transcript",
);
includesText(
	scopedNormalized.normalizedText,
	"фиссура",
...
assert(
	!scopedDraft.complaint.toLowerCase().includes("холод"),
	"objective cold reaction must not be copied into complaint",
);
includesText(
	scopedDraft.objectiveStatus,
	"реакция на холод",
	"scoped objective field",
);
includesText(
	scopedDraft.objectiveStatus,
	"цвет по шкале A2",
...
includesText(
	scopedDraft.treatmentPlan,
	"герметизация",
	"scoped treatment plan field",
);
...
const shorthandDraft = buildRuleBasedVisitDraftFromTranscript(
	shorthandTranscript,
	"therapist",
);
const shorthandNormalized = normalizeDentalSpeechTranscript(
	shorthandTranscript,
	"therapist",
);

includesText(
	shorthandNormalized.normalizedText,
	"status localis",
	"shorthand normalized transcript",
);
includesText(
	shorthandNormalized.normalizedText,
	"пломба",
...
includesText(
	shorthandDraft.objectiveStatus,
	"пломба",
	"shorthand objective field",
);
includesText(
	shorthandDraft.objectiveStatus,
	"холодовая проба",
...
includesText(
	shorthandDraft.diagnosis,
	"кариес дентина",
	"shorthand diagnosis field",
);
includesText(
	shorthandDraft.treatmentPlan,
	"матрица",
	"shorthand treatment plan field",
);
includesText(
	shorthandDraft.treatmentPlan,
	"клин",
...
assert(
	shorthandDraft.quality?.signals.includes("procedure_mentioned"),
	"quality must detect matrix and finishing as procedure",
);
...
const restorationDraft = buildRuleBasedVisitDraftFromTranscript(
	restorationTranscript,
	"therapist",
);
const restorationNormalized = normalizeDentalSpeechTranscript(
	restorationTranscript,
	"therapist",
);

includesText(
	restorationNormalized.normalizedText,
	"апроксимальная",
	"restoration normalized transcript",
);
includesText(
	restorationNormalized.normalizedText,
	"контактный пункт",
...
includesText(
	restorationDraft.complaint,
	"застревание пищи",
	"restoration complaint field",
);
includesText(
	restorationDraft.objectiveStatus,
	"апроксимальная",
	"restoration objective field",
);
includesText(
	restorationDraft.objectiveStatus,
	"контактный пункт",
...
includesText(
	restorationDraft.diagnosis,
	"кариес дентина",
	"restoration diagnosis field",
);
includesText(
	restorationDraft.treatmentPlan,
	"инфильтрационная",
	"restoration treatment plan field",
);
includesText(
	restorationDraft.treatmentPlan,
	"коррекция окклюзии",
...
assert(
	restorationDraft.quality?.signals.includes("procedure_mentioned"),
	"quality must detect restoration procedure",
);
...
const anatomicToothDraft = buildRuleBasedVisitDraftFromTranscript(
	anatomicToothTranscript,
	"therapist",
);
const anatomicToothNormalized = normalizeDentalSpeechTranscript(
	anatomicToothTranscript,
	"therapist",
);

includesText(
	anatomicToothNormalized.normalizedText,
	"зуб 36",
	"anatomic tooth normalized transcript",
);
includesText(
	anatomicToothNormalized.normalizedText,
	"зуб 16",
...
includesText(
	anatomicToothDraft.complaint,
	"зуб 36",
	"anatomic tooth complaint field",
);
includesText(
	anatomicToothDraft.objectiveStatus,
	"зуб 16",
	"anatomic tooth objective field",
);
includesText(
	anatomicToothDraft.objectiveStatus,
	"зуб 36",
...
includesText(
	anatomicToothDraft.diagnosis,
	"периодонтит зуб 36",
	"anatomic tooth diagnosis field",
);
includesText(
	anatomicToothDraft.treatmentPlan,
	"зуб 36",
	"anatomic tooth treatment plan field",
);
assert(
	anatomicToothDraft.quality?.detectedToothCodes.includes("36"),
	"quality must detect anatomical FDI 36",
);
assert(
	anatomicToothDraft.quality?.detectedToothCodes.includes("16"),
	"quality must detect anatomical FDI 16",
);
...
const anamnesisSurfaceDraft = buildRuleBasedVisitDraftFromTranscript(
	anamnesisSurfaceTranscript,
	"therapist",
);
const anamnesisSurfaceNormalized = normalizeDentalSpeechTranscript(
	anamnesisSurfaceTranscript,
	"therapist",
);

includesText(
	anamnesisSurfaceNormalized.normalizedText,
	"зуб 26",
	"anamnesis surface normalized transcript",
);
includesText(
	anamnesisSurfaceNormalized.normalizedText,
	"МОД",
...
includesText(
	anamnesisSurfaceDraft.complaint,
	"нет",
	"anamnesis surface complaint field",
);
assert(
	!anamnesisSurfaceDraft.complaint.toLowerCase().includes("аллерг"),
	"anamnesis must not be copied into complaint",
);
includesText(
	anamnesisSurfaceDraft.anamnesis,
	"отрицает",
	"anamnesis surface anamnesis field",
);
includesText(
	anamnesisSurfaceDraft.anamnesis,
	"не принимает",
...
includesText(
	anamnesisSurfaceDraft.objectiveStatus,
	"МОД",
	"anamnesis surface objective field",
);
includesText(
	anamnesisSurfaceDraft.objectiveStatus,
	"II класс по Блэку",
...
includesText(
	anamnesisSurfaceDraft.diagnosis,
	"кариес дентина",
	"anamnesis surface diagnosis field",
);
includesText(
	anamnesisSurfaceDraft.treatmentPlan,
	"МОД",
	"anamnesis surface treatment plan field",
);
assert(
	anamnesisSurfaceDraft.quality?.signals.includes("anamnesis_detected"),
	"quality must detect anamnesis",
);
assert(
	anamnesisSurfaceDraft.quality?.signals.includes("procedure_mentioned"),
	"quality must detect MOD restoration procedure",
);
...
const codeAndPlanDraft = buildRuleBasedVisitDraftFromTranscript(
	codeAndPlanTranscript,
	"therapist",
);
const codeAndPlanNormalized = normalizeDentalSpeechTranscript(
	codeAndPlanTranscript,
	"therapist",
);

includesText(
	codeAndPlanNormalized.normalizedText,
	"K02.1",
	"code and plan normalized transcript",
);
includesText(
	codeAndPlanNormalized.normalizedText,
	"МКБ-10",
...
includesText(
	codeAndPlanDraft.complaint,
	"зуб 14",
	"code and plan complaint field",
);
includesText(
	codeAndPlanDraft.objectiveStatus,
	"зуб 14",
	"code and plan objective field",
);
includesText(
	codeAndPlanDraft.diagnosis,
	"K02.1",
	"code and plan diagnosis field",
);
includesText(
	codeAndPlanDraft.treatmentPlan,
	"инфильтрационная",
	"code and plan treatment plan field",
);
includesText(
	codeAndPlanDraft.treatmentPlan,
	"препарирование",
...
assert(
	codeAndPlanDraft.quality?.signals.includes("plan_detected"),
	"quality must detect morphological plan sections",
);
assert(
	codeAndPlanDraft.quality?.signals.includes("procedure_mentioned"),
	"quality must detect morphological procedure sections",
);
...
const surgeonDraft = buildRuleBasedVisitDraftFromTranscript(
	surgeonTranscript,
	"surgeon",
);
includesText(surgeonDraft.complaint, "восьмерки", "surgeon complaint field");
includesText(
	surgeonDraft.objectiveStatus,
	"дистопирован",
	"surgeon objective field",
);
includesText(surgeonDraft.diagnosis, "ретенция", "surgeon diagnosis field");
includesText(
	surgeonDraft.treatmentPlan,
	"удаление зуба",
	"surgeon treatment plan field",
);
includesText(
	surgeonDraft.treatmentPlan,
	"лунки",
...
const orthopedistDraft = buildRuleBasedVisitDraftFromTranscript(
	orthopedistTranscript,
	"orthopedist",
);
includesText(
	orthopedistDraft.complaint,
	"выпадение коронки",
	"orthopedist complaint field",
);
includesText(
	orthopedistDraft.objectiveStatus,
	"культя",
	"orthopedist objective field",
);
includesText(
	orthopedistDraft.diagnosis,
	"дефект коронковой части",
	"orthopedist diagnosis field",
);
includesText(
	orthopedistDraft.treatmentPlan,
	"снятие слепков",
	"orthopedist treatment plan field",
);
includesText(
	orthopedistDraft.treatmentPlan,
	"временной коронки",
...
const orthodontistDraft = buildRuleBasedVisitDraftFromTranscript(
	orthodontistTranscript,
	"orthodontist",
);
includesText(
	orthodontistDraft.complaint,
	"неровные зубы",
	"orthodontist complaint field",
);
includesText(
	orthodontistDraft.objectiveStatus,
	"скученность зубов",
	"orthodontist objective field",
);
includesText(
	orthodontistDraft.diagnosis,
	"дистальный прикус",
	"orthodontist diagnosis field",
);
includesText(
	orthodontistDraft.treatmentPlan,
	"фиксация брекет",
	"orthodontist treatment plan field",
);
includesText(
	orthodontistDraft.treatmentPlan,
	"установка дуги",
...
const hygienistDraft = buildRuleBasedVisitDraftFromTranscript(
	hygienistTranscript,
	"hygienist",
);
includesText(
	hygienistDraft.complaint,
	"кровоточивость десен",
	"hygienist complaint field",
);
includesText(
	hygienistDraft.objectiveStatus,
	"зубной налет",
	"hygienist objective field",
);
includesText(
	hygienistDraft.diagnosis,
	"хронический гингивит",
	"hygienist diagnosis field",
);
includesText(
	hygienistDraft.treatmentPlan,
	"ультразвуковая",
	"hygienist treatment plan field",
);
includesText(
	hygienistDraft.treatmentPlan,
	"Air Flow",
