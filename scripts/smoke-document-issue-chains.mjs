...
function assertReleaseJournalHash(document, label) {
	...
	assert(
		...
		`${label} must write release journal source sha256`,
	);
	return hash;
}
...
function hasMojibakeMarker(value) {
	...
	return [...String(value ?? "")].some((char) =>
		markerCodepoints.has(char.codePointAt(0)),
	);
}
...
function assertReadableRussian(value, expected, label) {
	assert(
		value === expected,
		`${label} must be repaired to readable Russian: ${JSON.stringify(value)}`,
	);
	assert(
		!hasMojibakeMarker(value),
		`${label} must not contain mojibake markers`,
	);
}
...
try {
	...
	const contractPayload = {
		paidMedicalServicesContract: {
			...
			plannedCareReason:
				"плановое стоматологическое лечение по активному визиту",
			serviceScopeSummary:
				"оказание платных стоматологических услуг по согласованному плану лечения",
			estimatedTotalRub: 8600,
			paymentTerms: "оплата по кассовому чеку до или в день оказания услуг",
			priceChangeRules:
				"изменения объема и стоимости оформляются письменным соглашением",
			freeCareAvailabilityNotice:
				"маршрут бесплатной помощи разъяснен при наличии оснований",
			medicalRecommendationWarning:
				"несоблюдение рекомендаций врача может повлиять на результат",
			refusalAndRefundTerms:
				"возврат оформляется по фактически оказанным услугам и расходам",
			warrantyAndClaimsTerms:
				"гарантийные условия действуют по локальным правилам клиники",
	...
	assert(
		contractCreateResponse.statusCode === 201,
		`contract create failed: ${contractCreateResponse.statusCode} ${contractCreateResponse.body}`,
	);
	const contractIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: { note: "contract chain" },
		}),
	});
	assert(
		contractIssueResponse.statusCode === 200,
		`contract issue failed: ${contractIssueResponse.statusCode} ${contractIssueResponse.body}`,
	);
	...
	const actPayload = {
		completedWorksAct: {
			...
			acceptedServicesSummary:
				"Фактически оказанные и оплаченные стоматологические услуги по активному визиту.",
			totalByActRub: 3000,
			paidRub: 3000,
	...
	assert(
		wrongLinkedActResponse.statusCode === 201,
		`wrong-linked act create failed: ${wrongLinkedActResponse.statusCode}`,
	);
	...
	assert(
		wrongLinkedActIssueResponse.statusCode === 409,
		"act without exact issued contract link must be blocked",
	);
	const wrongLinkedActIssueError = documentErrorText(
		wrongLinkedActIssueResponse,
	);
	assert(
		wrongLinkedActIssueError.includes("договор"),
		`act contract block must mention contract: ${wrongLinkedActIssueError}`,
	);
	...
	assert(
		linkedActResponse.statusCode === 201,
		`linked act create failed: ${linkedActResponse.statusCode} ${linkedActResponse.body}`,
	);
	const linkedActIssueResponse = await app.inject({
		method: "POST",
		url: `/api/documents/${linkedActResponse.json().id}/issue`,
		payload: issueAttestation({
			signatureAttestation: { note: "linked act chain" },
		}),
	});
	assert(
		linkedActIssueResponse.statusCode === 200,
		`linked act issue failed: ${linkedActIssueResponse.statusCode} ${linkedActIssueResponse.body}`,
	);
	...
	const releasePayload = {
		medicalDocumentReleaseReceipt: {
			...
			deliveryProtectionNote:
				"Выдача через защищенный канал после проверки личности",
	...
	assert(
		releaseCreateResponse.statusCode === 201,
		`release receipt create failed: ${releaseCreateResponse.statusCode}`,
	);
	...
	assert(
		documentErrorText(releaseWithoutRequestResponse).includes("запрос"),
		"release receipt block must explain missing copy request",
	);
	...
	const copyRequestPayload = {
		medicalRecordCopyRequest: {
			requestedDocumentTypes: [
				...
				"Копия снимков или архив исходных снимков",
			],
			periodStart: "2026-05-01",
			periodEnd: "2026-05-20",
			requestedFormat: "pdf",
	...
	assert(
		documentErrorText(invalidCopyRequestDateIssueResponse)
			.toLowerCase()
			.includes("дат"),
	...
	assert(
		copyRequestResponse.statusCode === 201,
		`copy request create failed: ${copyRequestResponse.statusCode}`,
	);
	...
	const copyRequestIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: "Иванова Марина Сергеевна",
				note: "copy request chain",
			},
		}),
	});
	assert(
		copyRequestIssueResponse.statusCode === 200,
		`copy request issue failed: ${copyRequestIssueResponse.statusCode}`,
	);
	const issuedCopyRequest = copyRequestIssueResponse.json();
	assert(
		issuedCopyRequest.releaseJournalEntry?.entryKind === "request_registered",
		"copy request issue must write release journal entry",
	);
	assertReleaseJournalHash(issuedCopyRequest, "copy request issue");
	assert(
		typeof issuedCopyRequest.issuedSnapshotSha256 === "string" &&
			sha256Pattern.test(issuedCopyRequest.issuedSnapshotSha256),
	...
	assert(
		documentErrorText(releaseWithInvalidDateIssueResponse)
			.toLowerCase()
			.includes("дат"),
	...
	assert(
		releaseWithExtraTypeResponse.statusCode === 201,
		`broad release create failed: ${releaseWithExtraTypeResponse.statusCode}`,
	);
	...
	assert(
		releaseWithWrongFormatResponse.statusCode === 201,
		`wrong-format release create failed: ${releaseWithWrongFormatResponse.statusCode}`,
	);
		...
		payload: {
			...
			payload: {
				medicalDocumentReleaseReceipt: {
					...releasePayload.medicalDocumentReleaseReceipt,
					sourceRequestDocumentId: issuedCopyRequestId,
					recipientAuthority: "доверенное лицо без совпадающего запроса",
	...
	assert(
		releaseWithWrongAuthorityResponse.statusCode === 201,
		`wrong-authority release create failed: ${releaseWithWrongAuthorityResponse.statusCode}`,
	);
	...
	assert(
		repairedReleaseResponse.statusCode === 201,
		`repaired release create failed: ${repairedReleaseResponse.statusCode}`,
	);
	...
	assertReadableRussian(
		repairedIssuedRelease.signatureAttestation?.staffFullName,
		"Главный врач",
		"issued signature staff full name",
	);
	assertReadableRussian(
		repairedIssuedRelease.signatureAttestation?.note,
		"отметка получена",
		"issued signature note",
	);
	assertReadableRussian(
	...
	assert(
		repairedReleaseAuditResponse.statusCode === 200,
		"repaired release audit facts must load",
	);
	...
	assert(
		repairedReleaseHtmlResponse.statusCode === 200,
		"repaired release archived HTML must load",
	);
	assert(
	...
	assert(
		!hasMojibakeMarker(repairedReleaseHtmlResponse.body),
		"repaired release archived HTML must not contain mojibake markers",
	);
	...
	assert(
		linkedReleaseResponse.statusCode === 201,
		`linked release create failed: ${linkedReleaseResponse.statusCode}`,
	);
	const releaseAfterRequestResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: "Иванова Марина Сергеевна",
				note: "release receipt chain",
			},
		}),
	});
	assert(
		releaseAfterRequestResponse.statusCode === 200,
		`release receipt after request failed: ${releaseAfterRequestResponse.statusCode}`,
	);
	...
	const releaseReceiptSourceHash = assertReleaseJournalHash(
		issuedReleaseReceipt,
		"release receipt issue",
	);
	assert(
	...
	function dentalMedicalCard043uPayload(overrides = {}) {
		return {
			...
			treatmentDescription:
				"Лечение кариеса 36 зуба и композитная реставрация.",
	...
	assert(
		draftSourceExtractResponse.statusCode === 201,
		`draft-source extract create failed: ${draftSourceExtractResponse.statusCode}`,
	);
	...
	assert(
		documentErrorText(draftSourceExtractIssueResponse).includes("подпис"),
		"extract block must require signed source visits",
	);
	...
	assert(
		invalidDateExtractResponse.statusCode === 201,
		`invalid-date extract create failed: ${invalidDateExtractResponse.statusCode}`,
	);
	...
	assert(
		documentErrorText(invalidDateExtractIssueResponse)
			.toLowerCase()
			.includes("дат"),
	...
	assert(
		reversedPeriodExtractResponse.statusCode === 201,
		`reversed-period extract create failed: ${reversedPeriodExtractResponse.statusCode}`,
	);
	...
	assert(
		documentErrorText(reversedPeriodExtractIssueResponse)
			.toLowerCase()
			.includes("период"),
	...
	assert(
		outsidePeriodExtractResponse.statusCode === 201,
		`outside-period extract create failed: ${outsidePeriodExtractResponse.statusCode}`,
	);
	...
	assert(
		documentErrorText(outsidePeriodExtractIssueResponse)
			.toLowerCase()
			.includes("период"),
			...
			payload: {
				medicalRecordExtract: {
					...
					complaintAndAnamnesis:
						"Жалобы и анамнез перенесены из подписанной карты.",
	...
	assert(
		extractResponse.statusCode === 201,
		`extract create failed: ${extractResponse.statusCode}`,
	);
	...
	assert(
		extractIssueResponse.statusCode === 409,
		`extract with invalid source visits must be blocked: ${extractIssueResponse.statusCode}`,
	);
	assert(
		documentErrorText(extractIssueResponse).includes("исходных приемов"),
		"extract block must explain invalid source visits",
	);
