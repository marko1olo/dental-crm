...
function requireButtonTypes(sourceName) {
	...
	for (const tag of buttonOpenTags) {
		if (!/\stype=/.test(tag)) {
			missing.push(
				`${sourceName}: button missing explicit type attribute: ${tag.slice(0, 120)}`,
			);
		}
	}
}
...
for (const view of [
	...
	requireIn(
		...
		`daily ${view} route must have an isolated error boundary.`,
	);
}
...
requireIn(
	"app",
	'className="skip-link"',
	"desktop keyboard users need a skip link before shell navigation.",
);
requireIn(
	"app",
	'href="#workspace-content"',
	"skip link must target the workspace content region.",
);
requireIn(
	"app",
	'id="workspace-content"',
	"workspace content must be addressable by the skip link.",
);
requireIn(
	"app",
	"tabIndex={-1}",
	"workspace content must accept programmatic focus after skip navigation.",
);
requireIn(
	"app",
	'aria-label="',
	"workspace region and lazy fallbacks must expose readable names.",
);
requireIn(
	"app",
	'aria-busy="true"',
	"lazy daily-route fallbacks must expose loading state.",
);

requireIn(
	"shell",
	"export const appViews",
	"shell must own the hash-routed view registry.",
);
for (const view of [
	"schedule",
	"documents",
...
requireIn(
	"shell",
	"href={`#${view}`}",
	"sidebar links must stay keyboard-native hash anchors.",
);
requireIn(
	"shell",
	'aria-current={currentView === view ? "page" : undefined}',
	"sidebar must announce the active page.",
);
requireIn(
	"shell",
	"aria-label={`${viewLabels[view]}: ${viewHints[view]}`}",
	"sidebar links must include label plus operator hint.",
);
requireIn(
	"shell",
	"title={`${viewLabels[view]}: ${viewHints[view]}`}",
	"sidebar links must expose hover hints for desktop shell users.",
);
requireIn(
	"shell",
	"onFocus={() => onViewIntent?.(view)}",
	"keyboard focus must preload lazy daily routes.",
);
requireIn(
	"shell",
	'<details className="workspace-role-switcher"',
	"role switcher must use keyboard-native details/summary.",
);
requireIn(
	"shell",
	"aria-pressed={selectedWorkspaceRole === role}",
	"role switcher buttons must expose selected state.",
);
requireIn(
	"shell",
	'aria-label="',
	"icon-only topbar shortcuts must have accessible names.",
);
...
requireIn(
	...
	"mobile assistive navigation must respect reduced motion.",
);
requireIn(
	"css",
...
requirePattern(
	"css",
	/\.round-link\s*\{[\s\S]*?height:\s*44px;[\s\S]*?width:\s*44px;[\s\S]*?\}/,
	"patient row open controls must keep a 44px mobile touch target.",
);
...
requireIn(
	...
	"patient rows must expose a dedicated open-card control.",
);
requireIn(
	"patients",
	"aria-label={`",
	"patient row open controls must have contextual accessible names.",
);
requireIn(
	"patients",
	"aria-pressed={patientIsSelected}",
	"patient row open controls must expose selected state.",
);
...
requireIn(
	...
	"shift summary must be test-addressable.",
);
requireIn(
	"schedule",
	'aria-live="polite"',
	"schedule updates and blockers must use polite announcements.",
);
requireIn(
	"schedule",
	'className="schedule-filter-strip" aria-label=',
	"schedule filters must have a group name.",
);
requireIn(
	"schedule",
	"onKeyDown={(event: KeyboardEvent<HTMLInputElement>)",
	"schedule admin unlock must support keyboard submission.",
);
requireIn(
	"schedule",
	'event.key === "Enter"',
	"schedule admin unlock must submit on Enter.",
);
requireIn(
	"schedule",
	'id="schedule-admin-unlock-guidance"',
	"schedule admin secret guidance must be addressable.",
);
requireIn(
	"schedule",
	'aria-describedby={!adminSecretReady ? "schedule-admin-unlock-guidance" : undefined}',
	"schedule admin unlock controls must point to guidance.",
);
requireIn(
	"schedule",
	"disabled={!adminSecretReady}",
	"schedule admin unlock must block empty secret submission.",
);
requireIn(
	"schedule",
...
requireIn(
	...
	"new appointment create button must expose busy state.",
);
requireIn(
	"schedule",
	"aria-expanded={appointmentEditing}",
	"appointment edit buttons must expose expanded state.",
);
requireIn(
	"schedule",
	"aria-controls={appointmentEditorId}",
	"appointment edit buttons must point to the controlled editor.",
);
requireIn(
	"schedule",
	"aria-label={`",
	"repeated schedule row actions must have contextual accessible names.",
);
requireIn(
	"schedule",
...
requireIn(
	"documents",
	'id="documents"',
	"documents panel must be addressable.",
);
requireIn(
	"documents",
...
requireIn(
	...
	"document creation must expose busy state.",
);
requireIn(
	"documents",
	"disabled={Boolean(documentCreateSavingKind)}",
	"document creation must block duplicate submits.",
);
requireIn(
	"documents",
	"const documentActionContext =",
	"document list actions must compute contextual names.",
);
requireIn(
	"documents",
	"aria-label={`",
	"document list actions must use explicit accessible names.",
);
requireIn(
	"documents",
	"aria-busy={documentAuditLoading || undefined}",
	"document audit/passport action must expose row-local loading state.",
);
requireIn(
	"documents",
	'role="dialog"',
	"document issue and void confirmations must expose dialog semantics.",
);
requireIn(
	"documents",
	'const documentIssueMissingGuidanceId = "document-issue-missing-guidance"',
	"document issue blockers must have a stable guidance id.",
);
requireIn(
	"documents",
	'const documentVoidMissingGuidanceId = "document-void-missing-guidance"',
	"document void blockers must have a stable guidance id.",
);
requireIn(
	"documents",
	"aria-describedby={!documentIssueAttestationReady ? documentIssueMissingGuidanceId : undefined}",
	"issue confirmation button must point to blocker guidance.",
);
requireIn(
	"documents",
	"aria-describedby={!documentVoidReady ? documentVoidMissingGuidanceId : undefined}",
	"void confirmation button must point to blocker guidance.",
);

requireIn(
	"payment",
	'id="payment-capture"',
	"payment capture must be addressable from daily finance recovery actions.",
);
requireIn(
	"payment",
	'id="payment-amount-input"',
	"payment amount input must be addressable from history/empty-state jumps.",
);
requireIn(
	"payment",
	'aria-label="',
	"payment amount must have an explicit accessible label.",
);
requireIn(
	"payment",
	"aria-invalid={paymentAmountInvalid || undefined}",
	"invalid payment amount must be announced.",
);
requireIn(
	"payment",
	"aria-describedby={paymentAmountInvalid ? paymentMissingId : undefined}",
	"invalid payment amount must point to guidance.",
);
requireIn(
	"payment",
	'<details className="payment-capture-detail-section"',
	"secondary payment facts must be keyboard-native disclosures.",
);
requireIn(
	"payment",
	"<summary>",
	"payment disclosure sections must have keyboard-native summaries.",
);
requireIn(
	"payment",
	"aria-pressed={method === paymentMethod}",
	"payment method segmented controls must expose selected state.",
);
requireIn(
	"payment",
	"aria-pressed={taxDeductionCode === code}",
	"tax deduction segmented controls must expose selected state.",
);
requireIn(
	"payment",
...
requireIn(
	...
	"payment feedback and blockers must be announced politely.",
);
requireIn(
	"payment",
	"aria-busy={isSaving || undefined}",
	"payment submit must expose busy state.",
);
requireIn(
	"payment",
	"aria-describedby={!paymentReadyToSubmit ? paymentMissingId : undefined}",
	"payment submit must point to blocker guidance.",
);
requireIn(
	"payment",
	"disabled={isSaving || !paymentReadyToSubmit}",
	"payment submit must block duplicate or incomplete capture.",
);
...
requireIn(
	"communications",
	'className="communications-summary-grid" aria-label=',
	"communications summary must have a group name.",
);
requireIn(
	"communications",
	'const communicationNoteInputId = "communication-closing-note"',
	"closing note must use a stable input id.",
);
requireIn(
	"communications",
	"htmlFor={communicationNoteInputId}",
	"closing note label must target the input.",
);
requireIn(
	"communications",
	"aria-describedby={communicationNoteDescriptionId}",
	"closing note and outcomes must point to guidance.",
);
requireIn(
	"communications",
	"htmlFor={outcomeSelectId}",
	"row outcome label must target the select.",
);
requireIn(
	"communications",
	"aria-label={`${documentActionLabel}: ${task.title}`}",
	"document workflow actions must include task context.",
);
requireIn(
	"communications",
	"aria-label={`",
	"task close action must include task context.",
);
requireIn(
	"communications",
	"id={savingStatusId}",
	"row saving status must be addressable.",
);
requireIn(
	"communications",
	"aria-busy={isTaskSaving || undefined}",
	"task close button must expose busy state.",
);
requireIn(
	"communications",
	"aria-describedby={isTaskSaving ? `${completionNoteDescriptionId} ${savingStatusId}` : completionNoteDescriptionId}",
	"task close button must point to note guidance and saving status.",
);
requireIn(
	"communications",
	"disabled={communicationSaveInProgress || !selectedOutcome}",
	"task close must block duplicate or missing outcome submission.",
);
...
requireIn(
	"settings",
	'role="tabpanel"',
	"active settings content must expose tabpanel semantics.",
);
requireIn(
	"settings",
	"aria-selected={tabSelected}",
	"settings tabs must expose selected state.",
);
requireIn(
	"settings",
	"aria-controls={settingsTabPanelId(tab.id)}",
	"settings tabs must point to their panels.",
);
requireIn(
	"settings",
...
requireIn(
	...
	"settings tabs must support keyboard navigation.",
);
requireIn(
	"settings",
...
requireIn(
	"settings",
	'event.key === "Home"',
	"settings tabs must support Home key navigation.",
);
requireIn(
	"settings",
	'event.key === "End"',
	"settings tabs must support End key navigation.",
);
requireIn(
	"settings",
	"document.getElementById(nextTabButtonId)?.focus()",
	"settings keyboard navigation must move focus to the selected tab.",
);
requireIn(
	"settings",
	'id="settings-admin-unlock-guidance"',
	"settings admin secret guidance must be addressable.",
);
requireIn(
	"settings",
	'aria-describedby={!adminSecretReady ? "settings-admin-unlock-guidance" : undefined}',
	"settings admin unlock controls must point to guidance.",
);
requireIn(
	"settings",
	'event.key === "Enter"',
	"settings admin unlock must submit on Enter.",
);
requireIn(
	"settings",
	'aria-label="',
	"settings icon/quick actions must have accessible names.",
);
requireIn(
	"settings",
	'role="status" aria-live="polite"',
	"settings blockers and async feedback must be announced politely.",
);
