...
const missing = requiredSnippets.filter(
	(snippet) => !appSource.includes(snippet),
);
		...
		JSON.stringify(
			{
				...
				forbidden,
				reason:
					"Schedule autosave must retry dirty error states and reconnect without waiting for another user edit.",
			},
			null,
			2,
		),
	...
	JSON.stringify(
		{
			ok: true,
			checked:
				"Schedule autosave retries dirty error states and reconnects after browser online event.",
		},
		null,
		2,
	),
