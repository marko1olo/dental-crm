...
const recorderStopBlock = appSource.slice(
...
const manualStopBlock = appSource.slice(
	stopIndex,
	appSource.indexOf("function startImportDictation()", stopIndex),
);
requireIn(
	manualStopBlock,
...
if (uploadStart === -1 || uploadEnd === -1)
	fail("Speech upload block was not found.");
...
const quietChunkBlock = uploadBlock.slice(quietBlockStart, queuedUploadIndex);
forbidIn(
	quietChunkBlock,
	"return;",
