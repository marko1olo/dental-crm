...
const ARTIFACTS_DIR =
	"C:/Users/Admin/.gemini/antigravity/brain/e1a85de0-5463-4dad-9cfd-a687decd3eb2";
const PROOFS_DIR = "C:/Clinic_MVP/dental-crm/docs/proofs/ui_audit";
const __dirname = path.dirname(fileURLToPath(import.meta.url));

const dashboardMockRaw = await fsPromises.readFile(
	path.join(__dirname, "..", "mock-dashboard.json"),
	"utf8",
);
...
const mockVisit = {
	...
	anamnesis:
		"Боль появилась несколько дней назад, усиливается от температурных раздражителей.",
	objectiveStatus:
		"На дистально-жевательной поверхности 4.6 глубокая кариозная полость, зондирование дна резко болезненно.",
	diagnosis: "К04.0 Начальный пульпит зуба 4.6",
	treatmentPlan: "Эндодонтическое лечение каналов 4.6, временная пломба.",
	doctorSummary:
		"Проведена анестезия Sol. Ultracaini 1.7ml. Механическая и медикаментозная обработка каналов.",
	createdAt: NOW,
	updatedAt: NOW,
};
...
(async () => {
	console.log(
		"[Audit] ===== COMPLETE CROSS-DEVICE PRODUCTION-GRADE AUDIT =====",
	);
	...
	page.on("requestfailed", (request) => {
		...
		if (url.includes("/api/") || url.startsWith(APP_URL)) {
			console.error(
				`[Browser Network Failure]: ${url} failed with ${request.failure().errorText}`,
			);
			process.exit(1);
		}
	});
		...
		if (url.includes("/api/")) {
			if (
				...
				request.respond({
					status: 200,
					contentType: "application/json",
					body: dashboardMockRaw,
				});
			} else if (url.includes("/tooth-states")) {
				request.respond({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify({ success: true, states: mockToothStates }),
				});
			} else if (url.includes("/treatment-plans")) {
				request.respond({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify({ success: true, plans: mockTreatmentPlans }),
				});
			} else if (url.includes("/administrative-profile")) {
				request.respond({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify(mockPatient1.administrativeProfile),
				});
			} else if (
				url.includes("/api/patients/00000000-0000-0000-0000-000000000001")
			) {
				request.respond({
					status: 200,
					contentType: "application/json",
				...
				request.respond({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify([mockPatient1, mockPatient2]),
				});
				...
				request.respond({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify({ isAvailable: false, bridges: [] }),
				});
				...
				request.respond({
					...
					contentType: "application/json",
					body: JSON.stringify({
						readiness: { isAvailable: false, bridges: [] },
					}),
				});
				...
				request.respond({
					...
					body: JSON.stringify({
						...
						providerId: "groq",
						providerLabel: "Groq STT",
						serverUploadAllowed: true,
						localQueueRequired: false,
					}),
				});
	...
	try {
		...
		console.log(
			"[Audit] Step 2: Running screenshots for all views and viewports...",
		);
		...
		for (const view of VIEWS) {
			...
			for (const vp of VIEWPORTS) {
				for (const theme of THEMES) {
					...
					console.log(
						`  [Navigate] Setting hash to ${view.hash} (${vp.label}, ${theme})...`,
					);
					...
					if (view.name === "Patient_Portal") {
						const authContainerExists = await page.evaluate(
							() => !!document.querySelector(".portal-auth-container"),
						);
						if (authContainerExists) {
							...
							await page.evaluate(() => {
								const btn = Array.from(
									...
									(b) =>
										b.textContent.includes("Send Code") ||
										b.textContent.includes("OTP"),
								);
								if (btn) btn.click();
							});
							await sleep(1200);
					...
					if (view.name === "Odontogram") {
						...
						const clicked = await page.evaluate(() => {
							const wrappers = Array.from(
								document.querySelectorAll(".tooth-svg-wrapper"),
							);
					...
					for (const indicator of errorIndicators) {
						if (pageText.includes(indicator)) {
							console.error(
								`[FAIL] Page contains error indicator "${indicator}" on view "${view.name}" (${vp.label}, ${theme})`,
							);
							process.exit(1);
						}
					}
					...
					if (size < 15000) {
						console.error(
							`[FAIL] Screenshot ${filename} is too small (${size} bytes). Page is likely empty/blank.`,
						);
						process.exit(1);
					}
					...
					for (const prev of screenshotSizes[sizeKey]) {
						const diffPercent =
							Math.abs(prev.size - size) / Math.max(prev.size, size);
						if (diffPercent < 0.01) {
							console.error(
								`[FAIL] Duplicate layout size detected: "${filename}" is too similar to "${prev.filename}" (diff: ${(diffPercent * 100).toFixed(2)}%). Both likely show the same error page.`,
							);
							process.exit(1);
						}
					}
