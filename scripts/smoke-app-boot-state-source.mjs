...
requireIn(
	...
	"AppShell must log boot error details only outside production.",
);
forbidIn(
	appShellSource,
	"errorStack",
	"AppShell must not store or render stack traces in the production boot error UI.",
);
forbidIn(
	appShellSource,
	"<pre",
	"AppShell boot error UI must not render raw stack traces.",
);
