...
const commTelegramMigration = readFileSync(
	"apps/api/drizzle/0013_communication_telegram_runtime_tables.sql",
	"utf8",
);
const communicationWorkflowMigration = readFileSync(
	"apps/api/drizzle/0016_communication_task_workflow_code.sql",
	"utf8",
);
const taxSnapshotMigration = readFileSync(
	"apps/api/drizzle/0011_document_tax_payment_snapshot.sql",
	"utf8",
);
const paymentsMigration = readFileSync(
	"apps/api/drizzle/0012_payments_runtime_contract.sql",
	"utf8",
);
const structuredFiscalReceiptMigration = readFileSync(
	"apps/api/drizzle/0014_payment_structured_fiscal_receipt.sql",
	"utf8",
);
...
const documentVoidAttestationMigration = readFileSync(
	"apps/api/drizzle/0018_document_void_attestation.sql",
	"utf8",
);
const documentTaxXmlSnapshotMigration = readFileSync(
	"apps/api/drizzle/0019_document_tax_xml_snapshot.sql",
	"utf8",
);
const telegramVisualCardsMigration = readFileSync(
	"apps/api/drizzle/0021_telegram_visual_cards.sql",
	"utf8",
);
const telegramPostVisitCheckupDelaysMigration = readFileSync(
	"apps/api/drizzle/0022_telegram_post_visit_checkup_delays.sql",
	"utf8",
);
const telegramReviewRequestDelayMigration = readFileSync(
	"apps/api/drizzle/0023_telegram_review_request_delay.sql",
	"utf8",
);
const tenantScopeAddonsMigration = readFileSync(
	"apps/api/drizzle/0034_tenant_scope_addons.sql",
	"utf8",
);
const drizzleJournal = parseJsonFile("apps/api/drizzle/meta/_journal.json");
const documentIssueAttestationSnapshot = parseJsonFile(
	"apps/api/drizzle/meta/0017_snapshot.json",
);
const documentVoidAttestationSnapshot = parseJsonFile(
...
for (const needle of requiredSchemaExports) {
	assert(
		schemaSource.includes(needle),
		`schema.ts missing runtime DB contract: ${needle}`,
	);
}
...
for (const enumName of requiredRuntimeEnums) {
	assert(
		commTelegramMigration.includes(`CREATE TYPE "public"."${enumName}"`),
		`migration missing enum ${enumName}`,
	);
}

assert(
	taxSnapshotMigration.includes('"tax_payment_snapshot_json"'),
	"tax snapshot migration must persist issued tax payment snapshots",
);
assert(
	paymentsMigration.includes('"document_id" uuid'),
	"payments migration must add document_id for fiscal document linking",
);
assert(
	paymentsMigration.includes('"payment_method"'),
	"payments migration must convert method to payment_method enum",
);
assert(
	paymentsMigration.includes('"payment_status"'),
	"payments migration must add payment_status enum",
);
assert(
	paymentsMigration.includes('"created_at" timestamp with time zone'),
	"payments migration must add created_at",
);
assert(
	structuredFiscalReceiptMigration.includes('"fiscal_receipt" jsonb'),
	"structured fiscal receipt migration must persist FN/FD/FPD JSON",
);
assert(
...
const documentIssueSnapshotColumns =
	documentIssueAttestationSnapshot.tables?.["public.generated_documents"]
		?.columns ?? {};
assert(
...
assert(
	documentVoidAttestationMigration.includes('"void_attestation" jsonb'),
	"document void migration must persist void attestation JSON",
);
assert(
	documentVoidAttestationMigration.includes(
...
const documentVoidSnapshotColumns =
	documentVoidAttestationSnapshot.tables?.["public.generated_documents"]
		?.columns ?? {};
assert(
...
assert(
	communicationWorkflowMigration.includes('"workflow_code" text'),
	"communication workflow migration must persist stable task workflow codes",
);
assert(
	telegramVisualCardsMigration.includes('"visual_card_urls" jsonb'),
	"telegram visual cards migration must persist scenario visual card URLs",
);
assert(
	telegramPostVisitCheckupDelaysMigration.includes(
		'"post_visit_checkup_delay_hours_json" text',
	),
	"telegram checkup delay migration must persist clinic-specific post-visit timing",
);
assert(
	telegramReviewRequestDelayMigration.includes(
...
for (const needle of [
	...
	assert(
		tenantScopeAddonsMigration.includes(needle),
		`tenant scope add-ons migration missing: ${needle}`,
	);
}
...
for (const needle of requiredRuntimeMigrationNeedles) {
	assert(
		commTelegramMigration.includes(needle),
		`communication/telegram migration missing multi-clinic contract: ${needle}`,
	);
}
...
for (const tag of requiredJournalTags) {
	assert(
		journalTags.has(tag),
		`drizzle journal missing runtime migration tag: ${tag}`,
	);
}
...
for (const pattern of forbiddenSecretPatterns) {
	assert(
		!pattern.test(schemaSource),
		`schema source contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(commTelegramMigration),
		`migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(communicationWorkflowMigration),
		`communication workflow migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(structuredFiscalReceiptMigration),
		`structured fiscal receipt migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(documentIssueAttestationMigration),
		`document issue migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(documentVoidAttestationMigration),
		`document void migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(telegramVisualCardsMigration),
		`telegram visual cards migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(tenantScopeAddonsMigration),
		`tenant scope add-ons migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(telegramReviewRequestDelayMigration),
		`telegram review request delay migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(documentTaxXmlSnapshotMigration),
		`document tax XML snapshot migration contains forbidden Telegram secret shape: ${pattern}`,
	);
	assert(
		!pattern.test(telegramPostVisitCheckupDelaysMigration),
		`telegram checkup delay migration contains forbidden Telegram secret shape: ${pattern}`,
	);
}
