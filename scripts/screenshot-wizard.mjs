...
const tmpProfile = path.join(process.env.TEMP || "C:/tmp", "screenshot-wizard-profile");
...
try {
  ...
  // Wait for app
  await waitFor(cdp, `(() => document.readyState === "complete" && Boolean(document.querySelector(".app-shell")))()`, "app shell");
  await sleep(3000);
  ...
  await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 300)", returnByValue: true });
  await sleep(500);
  await shot("W02_step1_lower");

  // Click step 2 tab
  const step2Click = await evaluate(cdp, `(() => {
  ...
  await sleep(800);
  await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 200)", returnByValue: true });
  await sleep(300);
  await shot("W03_step2_stages");

  // Click step 3 tab
  const step3Click = await evaluate(cdp, `(() => {
    const btns = Array.from(document.querySelectorAll('button'));
  ...
  const step1Click = await evaluate(cdp, `(() => {
    const btns = Array.from(document.querySelectorAll('button'));
  ...
  await sleep(600);
  const detailsClick = await evaluate(cdp, `(() => {
  ...
  await sleep(500);
  await cdp.send("Runtime.evaluate", { expression: "window.scrollTo(0, 400)", returnByValue: true });
  await sleep(300);
