...
const healthFile = path.join(
	os.tmpdir(),
	`dental-speech-key-health-${process.pid}-${Date.now()}.json`,
);
...
function assertNoSecretLeak(text, label) {
	...
	assert(
		!text.includes("Authorization: Bearer"),
		`${label} leaked authorization header`,
	);
	assert(!text.includes("api_key=gsk_"), `${label} leaked query key`);
	assert(
		!text.includes("sk-abcdefghijklmnopqrstuvwxyz1234567890"),
		`${label} leaked OpenAI-style key`,
	);
}
...
try {
	...
	assert(
		candidates.length === 3,
		`Expected 3 de-duplicated Groq keys, got ${candidates.length}`,
	);
	assert(
		new Set(candidates.map((candidate) => candidate.value)).size === 3,
		"Expected raw key values to be de-duplicated",
	);
	assert(
		new Set(candidates.map((candidate) => candidate.fingerprint)).size === 3,
		"Expected unique key fingerprints",
	);

	const initialSummary = getProviderKeyPoolSummary("groq_whisper");
	assert(
		initialSummary.configuredKeyCount === 3,
		"Expected configured key count 3",
	);
	assert(
		initialSummary.availableKeyCount === 3,
		"Expected all keys available initially",
	);
	assert(
		initialSummary.coolingDownKeyCount === 0,
		"Expected no initial cooldowns",
	);
	assert(
		initialSummary.rotationEnabled === true,
		"Expected rotation to be enabled for multi-key pool",
	);
	assert(initialSummary.maxAttemptsPerProvider === 3, "Expected retry limit 3");
	assert(
		initialSummary.timeoutMs === 5000,
		"Expected provider timeout from env",
	);
	...
	assert(
		shouldTryNextProviderKey(rateLimitError),
		"Expected 429 to allow key rotation",
	);
	recordProviderKeyFailure("groq_whisper", first, rateLimitError);

	const afterRateLimit = getProviderKeyPoolSummary("groq_whisper");
	assert(
		afterRateLimit.availableKeyCount === 2,
		`Expected 2 keys after one cooldown, got ${afterRateLimit.availableKeyCount}`,
	);
	assert(
		afterRateLimit.coolingDownKeyCount === 1,
		"Expected one cooling-down key after 429",
	);
	...
	assert(
		...
		"Expected second selection to avoid tried fingerprint",
	);
	const timeoutError = new SpeechProviderRequestError(
		"Источник распознавания не ответил за 5 сек.",
		{
			retryable: true,
			timedOut: true,
		},
	...
	assert(
		shouldTryNextProviderKey(timeoutError),
		"Expected timeout to allow key rotation",
	);
	recordProviderKeyFailure("groq_whisper", second, timeoutError);

	const afterTimeout = getProviderKeyPoolSummary("groq_whisper");
	assert(
		afterTimeout.availableKeyCount === 1,
		`Expected 1 available key after two cooldowns, got ${afterTimeout.availableKeyCount}`,
	);
	assert(
		afterTimeout.coolingDownKeyCount === 2,
		"Expected two cooling-down keys",
	);
	...
	assert(
		third.fingerprint !== first.fingerprint &&
			third.fingerprint !== second.fingerprint,
		"Expected final available key",
	);
	...
	assert(
		snapshots.filter((snapshot) => !snapshot.available).length === 2,
		"Expected two unavailable cooling-down snapshots",
	);
	assert(
		snapshots.some((snapshot) => snapshot.successes === 1),
		"Expected one successful key snapshot",
	);
	assert(
		snapshots.every((snapshot) => !rawKeys.includes(snapshot.fingerprint)),
		"Fingerprints must not equal raw key values",
	);

	const persisted = await fs.readFile(healthFile, "utf8");
	assertNoSecretLeak(persisted, "persisted health file");
	assert(
		persisted.includes("groq_whisper:"),
		"Expected provider fingerprint namespace in persisted health",
	);
	assert(
		/"cooldownUntil"\s*:\s*[1-9]/.test(persisted),
		"Expected persisted cooldown timestamp",
	);
	...
	assert(
		...
		"Expected sanitized error to contain redaction marker",
	);
