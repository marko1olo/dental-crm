...
export async function waitFor(cdp, expression, label, attempts = 80) {
	...
	throw new Error(
		`${label} did not become ready: ${JSON.stringify(snapshot?.result?.value ?? null)}`,
	);
}
...
export async function evaluate(cdp, expression, label) {
	...
	if (result.exceptionDetails) {
		throw new Error(
			`${label} threw in browser: ${JSON.stringify(result.exceptionDetails)}`,
		);
	}
...
export async function setFileInputFiles(cdp, selector, files) {
	...
	console.log(
		"SMOKE TEST DEBUG: setFileInputFiles evaluation result =",
		result.result.value,
	);
	if (result.exceptionDetails) {
		throw new Error(
			`dispatch files for ${selector} threw in browser: ${JSON.stringify(result.exceptionDetails)}`,
		);
	}
}
