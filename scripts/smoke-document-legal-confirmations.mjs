...
const missing = [
	...requiredAppSnippets
		...
		.map((snippet) => `app:${snippet}`),
	...requiredSharedSnippets
		.filter((snippet) => !sharedSource.includes(snippet))
		.map((snippet) => `shared:${snippet}`),
	...requiredStyleSnippets
		.filter((snippet) => !styleSource.includes(snippet))
		.map((snippet) => `style:${snippet}`),
];
const forbidden = forbiddenAppSnippets.filter((snippet) =>
	appSource.includes(snippet),
);
if (
	/document\.status === "draft"[\s\S]{0,320}updateDocumentStatus\(document\.id, "issue"\)/.test(
		appSource,
	)
) {
	forbidden.push(
		'draft document row directly calls updateDocumentStatus(document.id, "issue")',
	);
}
	...
	...new Set(
		[...appSource.matchAll(/confirmedDocumentLiteral\((\w+)/g)]
			.filter(
				(match) =>
					!/function\s+$/.test(
						appSource.slice(Math.max(0, match.index - 16), match.index),
					),
			)
			.map((match) => match[1]),
	),
...
if (
	missing.length ||
	forbidden.length ||
	missingExplicitFalseState.length ||
	forbiddenTrueState.length
) {
	console.error(
		JSON.stringify(
			{
