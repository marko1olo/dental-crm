...
for (const [filePath, source] of Object.entries(sources)) {
	...
	if (questionHits?.length)
		garbledQuestionHits.push({ filePath, hits: questionHits.slice(0, 5) });
}
...
const missing = requiredReadableRussian.filter(
	(snippet) => !sourceBlob.includes(snippet),
);
assert(
	missing.length === 0,
	`web source lost required readable Russian snippets: ${missing.join(" | ")}`,
);
