...
} from "../apps/api/dist/telegramTransport.js";
...
async function run() {
	...
	assert(
		body.callback_query_id === "test-query-id",
		`Unexpected callback_query_id: ${body.callback_query_id}`,
	);
	...
	assert(
		body2.text === "A".repeat(200),
		`Expected text to be truncated to 200 chars. Got length ${body2.text.length}`,
	);
	...
	assert(
		resultError.errorClass === "bad_request",
		`Unexpected error class: ${resultError.errorClass}`,
	);
	assert(
		resultError.errorCode === 400,
		`Unexpected error code: ${resultError.errorCode}`,
	);
	assert(
		resultError.retryAfterSeconds === 5,
		`Unexpected retry after: ${resultError.retryAfterSeconds}`,
	);
	...
	console.log(
		JSON.stringify(
			{ ok: true, telegramBot: "transport methods checked" },
			null,
			2,
		),
	);
