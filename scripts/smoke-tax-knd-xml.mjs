...
assert(
	appSource.includes("черновой файл заархивирован"),
	"document passport UI must expose immutable FNS draft file snapshot status",
);
assert(
	appSource.includes(
...
assert(
	appSource.includes("Официальная проверка формата ФНС"),
	"Documents UI must translate XSD jargon for administrators",
);
assert(
	appSource.includes("электронная подпись"),
	"Documents UI must translate KEP jargon for administrators",
);
assert(
	appSource.includes("taxXmlSourceSnapshotSha256"),
	"document passport UI must show tax XML source snapshot hash",
);
assert(
	appSource.includes("taxXmlSnapshotSha256"),
	"document passport UI must show tax XML byte snapshot hash",
);
assert(
...
assert(
	taxXmlSource.includes('const KND_1151156_PRINT_FORM_CODE = "1151156"'),
	"tax XML must name the printed KND 1151156 form code",
);
assert(
	taxXmlSource.includes('const FNS_MEDICAL_EXPENSE_XML_KND = "1184043"'),
	"tax XML must name the electronic FNS KND 1184043 code",
);
assert(
	taxXmlSource.includes('const FNS_MEDICAL_EXPENSE_XML_VERSION = "5.01"'),
	"tax XML must name the FNS XSD version",
);
assert(
	taxXmlSource.includes("FNS_MEDICAL_EXPENSE_NOTICE_NUMBER_MAX_LENGTH = 12"),
	"tax XML must enforce official XSD НомерСвед length",
);
assert(
	taxXmlSource.includes("DENTE XML draft for issued KND"),
	"tax XML must be explicitly described as a draft export",
);
assert(
	taxXmlSource.includes("validateKnd1151156XmlDraft"),
	"tax XML must run an internal DENTE structural preflight before archiving",
);
assert(
	taxXmlSource.includes("структурную предпроверку DENTE"),
	"tax XML preflight failures must be operator-readable in Russian",
);
assert(
	taxXmlSource.includes("invalid-token"),
	"tax XML preflight must reject technical placeholder tokens",
);
assert(
	taxXmlSource.includes("не подписанный ТКС-пакет"),
	"tax XML warning must say it is not a signed TKS package",
);
assert(
	taxXmlSource.includes("СведДок"),
	"tax XML must export parseable identity document data as official СведДок",
);
assert(
	taxXmlSource.includes("КодВидДок"),
	"tax XML identity document export must include official document kind code",
);
assert(
	!taxXmlSource.includes(
		"patient.administrativeProfile?.taxpayerInn || fallbackPayment.payerInn",
	),
	"non-self tax XML must not reuse payer INN as patient INN",
);
assert(
	!taxXmlSource.includes("patient.birthDate || fallbackPayment.payerBirthDate"),
	"non-self tax XML must not reuse payer birth date as patient birth date",
);
assert(
	documentRoutesSource.includes("taxXmlSourceSnapshotForIssue"),
	"tax XML source facts must be frozen during document issue",
);
assert(
	documentRoutesSource.includes("frozenTaxXmlPatient"),
	"tax XML export must read frozen patient facts",
);
assert(
	documentRoutesSource.includes("frozenTaxXmlClinicProfile"),
	"tax XML export must read frozen clinic facts",
);
assert(
	documentRoutesSource.includes("frozenTaxXmlPayments"),
	"tax XML export must read frozen payment facts",
);
assert(
	documentRoutesSource.includes("document.taxXmlSnapshot"),
	"tax XML route must reuse the immutable first exported XML snapshot",
);
assert(
	documentRoutesSource.includes("taxXmlSourceSnapshotSha256"),
	"tax XML audit must expose the source snapshot hash",
);
assert(
	documentRoutesSource.includes("taxXmlOfficialValidationStatus"),
	"tax XML audit must expose external validation status",
);
assert(
	documentRoutesSource.includes(
		"Официальная XSD-валидация, КЭП и отправка ЭДО/ТКС",
	),
	"tax XML audit must state external XSD/KEP/EDO boundary in Russian",
);
assert(
	sampleDataSource.includes("storeTaxXmlSnapshot"),
...
try {
	...
	const taxPayment = payments.find(
		(payment) =>
			payment.patientId === activeVisit.patientId &&
			payment.payerInn === "123456789012",
	);
	...
	assert(
		draftXmlResponse.statusCode === 409,
		`draft tax XML must be blocked: ${draftXmlResponse.statusCode}`,
	);
	assert(
		documentErrorText(draftXmlResponse).includes("после выдачи"),
		"draft tax XML block must require issued certificate",
	);
	...
	assertNoMojibake(
		...
		"missing application issue response",
	);
	...
	assert(
		tenDigitInnApplicationResponse.statusCode === 400,
		`10-digit KND taxpayer INN must be rejected at create: ${tenDigitInnApplicationResponse.statusCode}`,
	);
	assert(
		String(tenDigitInnApplicationResponse.json().message).includes("12-"),
		"10-digit KND taxpayer INN block must explain 12-digit physical-person INN",
	);
	assertNoMojibake(
		tenDigitInnApplicationResponse.body,
		"10-digit payer INN create response",
	);
	...
	const identityOnlyApplicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: identityOnlyPayment.payerFullName,
				note: "identity only application",
			},
		}),
	});
	assert(
	...
	const identityOnlyCertificateResponse = await app.inject({
		...
		payload: {
			...
			totalAmountRub: 3300,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: [identityOnlyPayment.id] },
			},
		},
	});
	assert(
	...
	const identityOnlyIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: identityOnlyPayment.payerFullName,
				note: "identity only certificate",
			},
		}),
	});
	assert(
			...
			payload: {
				taxDeductionApplication: {
					...
					applicantAuthorityDocument:
						"свидетельство о браке, проверено администратором",
					requestedAt: "2026-05-20T10:30:00.000+04:00",
					duplicateWarningAccepted: true,
				},
			},
	...
	assert(
		spouseApplicationResponse.statusCode === 201,
		`spouse application create failed: ${spouseApplicationResponse.statusCode}`,
	);
	const spouseApplicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: spousePayment.payerFullName,
				note: "spouse application",
			},
		}),
	});
	assert(
		spouseApplicationIssueResponse.statusCode === 200,
		`spouse application issue failed: ${spouseApplicationIssueResponse.statusCode}`,
	);
	const spouseCertificateResponse = await app.inject({
		...
		payload: {
			...
			taxPayerInn: spousePayment.payerInn,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: [spousePayment.id] },
			},
		},
	});
	assert(
		spouseCertificateResponse.statusCode === 201,
		`spouse certificate create failed: ${spouseCertificateResponse.statusCode}`,
	);
	...
	assert(
		...
		"non-self KND certificate must require patient tax identity before issue",
	);
	assert(
		documentErrorText(spouseIssueResponse).includes("ИНН пациента"),
		"non-self KND block must explain missing patient INN",
	);
	assertNoMojibake(
		spouseIssueResponse.body,
		"non-self missing patient INN issue response",
	);
	...
	const spouseIssueReadyResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: spousePayment.payerFullName,
				note: "spouse certificate after patient tax identity",
			},
		}),
	});
	assert(
	...
	assert(
		spouseIssuedCertificate?.taxXmlSourceSnapshot?.clinicProfile.inn ===
			originalClinicFacts.inn,
	...
	try {
		...
		activePatient.administrativeProfile.identityDocument =
			"паспорт 9999 999999, выдан 09.09.2099";
		...
		assert(
			spouseXmlResponse.body.includes('ИНН="323456789012"'),
			"non-self XML must keep frozen patient INN",
		);
		assert(
			spouseXmlResponse.body.includes('ИННФЛ="631234567890"'),
			"non-self XML must keep frozen clinic INN",
		);
		assert(
			spouseXmlResponse.body.includes('ПрПациент="0"'),
			"non-self XML must mark taxpayer and patient as different people",
		);
		assert(
			spouseXmlResponse.body.includes("<Пациент"),
			"non-self XML must include frozen patient node",
		);
		assert(
			spouseXmlResponse.body.includes('НомКорр="0"'),
			"non-self XML must keep primary correction number",
		);
		assert(
			!spouseXmlResponse.body.includes("MUTATED XML"),
			"non-self XML must not use mutated live patient/clinic text",
		);
		assert(
			!spouseXmlResponse.body.includes("999999999999"),
			"non-self XML must not use mutated live INN values",
		);
		assert(
			!spouseXmlResponse.body.includes("NaN"),
			"non-self XML must not contain NaN",
		);
		assert(
			!spouseXmlResponse.body.includes("null"),
			"non-self XML must not contain null",
		);
		assertNoMojibake(
			spouseXmlResponse.body,
			"non-self tax XML after live patient/clinic mutation",
		);
	} finally {
		activePatient.fullName = originalPatientFacts.fullName;
		activePatient.birthDate = originalPatientFacts.birthDate;
		activePatient.administrativeProfile =
			originalPatientFacts.administrativeProfile;
	...
	assert(
		mismatchedApplicationResponse.statusCode === 409,
		"tax application with mismatched document/payload year must be blocked",
	);
	assert(
		documentErrorText(mismatchedApplicationResponse).includes("не совпадает"),
		"tax application mismatch must explain year mismatch",
	);
	assertNoMojibake(
		mismatchedApplicationResponse.body,
		"mismatched tax application response",
	);
	...
	assert(
		applicationCreateResponse.statusCode === 201,
		`tax application create failed: ${applicationCreateResponse.statusCode}`,
	);
	...
	const applicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: taxPayment.payerFullName,
				note: "self tax application",
			},
		}),
	});
	assert(
		applicationIssueResponse.statusCode === 200,
		`tax application issue failed: ${applicationIssueResponse.statusCode} ${applicationIssueResponse.body}`,
	);
	...
	assert(
		...
		"issued tax certificate must freeze XML payment facts",
	);
	assert(
		issuedCertificate?.taxXmlSourceSnapshot?.patient.id ===
			activeVisit.patientId,
		"issued tax certificate must freeze patient facts for XML",
	);
	assert(
		issuedCertificate?.taxXmlSourceSnapshot?.clinicProfile.inn ===
			clinicProfile.inn,
		"issued tax certificate must freeze clinic facts for XML",
	);
	assert(
		issuedCertificate?.taxXmlSourceSnapshot?.payments[0]?.id === taxPayment.id,
		"issued tax certificate must freeze XML payment rows",
	);
	assert(
		!issuedCertificate?.taxXmlSnapshot,
		"tax XML snapshot must be created only after first successful XML export",
	);
	...
	assert(
		issuedAuditFactsResponse.statusCode === 200,
		`issued tax audit facts failed: ${issuedAuditFactsResponse.statusCode}`,
	);
	const issuedAuditFacts = issuedAuditFactsResponse.json();
	assert(
		issuedAuditFacts.taxXmlOfficialValidationStatus ===
			"external_validation_required",
		"issued tax certificate audit must require external FNS XML validation",
	);
	assert(
		String(issuedAuditFacts.taxXmlOfficialValidationNote).includes(
			"Официальная XSD-валидация",
		),
	...
	assertNoMojibake(
		issuedAuditFactsResponse.body,
		"issued tax audit facts response",
	);
	...
	assert(
		missingTaxOfficeResponse.statusCode === 409,
		`tax XML without tax office code must be blocked: ${missingTaxOfficeResponse.statusCode}`,
	);
	assert(
		documentErrorText(missingTaxOfficeResponse).includes(
	...
	assertNoMojibake(
		missingTaxOfficeResponse.body,
		"missing tax office response",
	);
	...
	assert(
		...
		`tax XML export failed: ${xmlResponse.statusCode} ${xmlResponse.body}`,
	);
	assert(
		xmlResponse.headers["content-type"]?.includes("application/xml"),
		"tax XML content-type mismatch",
	);
	assert(
		xmlResponse.headers["content-disposition"]?.includes(".xml"),
		"tax XML content-disposition must expose XML filename",
	);
	...
	assert(
		xml.includes('<Документ КНД="1184043"'),
		"official XSD KND code missing",
	);
	assert(
		xml.includes("DENTE XML draft for issued KND 1151156"),
		"XML comment must tie draft export to issued printed KND certificate",
	);
	assert(
		xml.includes("Not a signed TKS package"),
		"XML comment must warn that operator validation and KEP signing are still required",
	);
	...
	assert(
		noticeNumberMatch[1].length <= 12,
		`medical expense notice number must fit official XSD maxLength=12, got ${noticeNumberMatch[1].length}`,
	);
	...
	assert(
		!xml.includes("<Пациент"),
		"self-payer XML must not include separate patient node",
	);
	assert(
		!xml.includes('СерНомДок="3600 000000"'),
		"payer identity document must be omitted when 12-digit INN is present",
	);
	...
	assert(
		...
		"XML must not contain object placeholders",
	);
	...
	assert(
		...
		"first successful tax XML export must persist immutable XML sha256",
	);
	assert(
		xmlSnapshotDocument.taxXmlSnapshot.sourceSnapshotSha256,
		"tax XML snapshot must carry the source snapshot hash",
	);
	assert(
		xmlSnapshotDocument.taxXmlSnapshot.taxOfficeCode === "6310",
		"tax XML snapshot must persist the original tax office code",
	);
	...
	assert(
		exportedAuditFactsResponse.statusCode === 200,
		`exported tax audit facts failed: ${exportedAuditFactsResponse.statusCode}`,
	);
	const exportedAuditFacts = exportedAuditFactsResponse.json();
	assert(
		exportedAuditFacts.taxXmlSnapshotSha256 ===
			xmlSnapshotDocument.taxXmlSnapshot.sha256,
		"tax audit facts must expose archived XML draft hash",
	);
	assert(
		exportedAuditFacts.taxXmlOfficialValidationStatus ===
			"external_validation_required",
	...
	assertNoMojibake(
		exportedAuditFactsResponse.body,
		"exported tax audit facts response",
	);
	...
	assert(
		xmlAfterTaxOfficeMutationResponse.body === xml,
		"tax XML re-export must return the first exported XML bytes",
	);
	assert(
		xmlAfterTaxOfficeMutationResponse.body.includes('КодНО="6310"'),
		"tax XML re-export must keep frozen tax office code",
	);
	assert(
		!xmlAfterTaxOfficeMutationResponse.body.includes('КодНО="7777"'),
		"tax XML re-export must not use mutated tax office code",
	);
	process.env.DENTE_FNS_TAX_OFFICE_CODE = "6310";
	...
	assert(
		identityOnlyXml.includes('СерНомДок="3601 000002"'),
		"identity-only XML must export payer identity document number",
	);
	assert(
		identityOnlyXml.includes('ДатаДок="02.03.2021"'),
		"identity-only XML must export payer identity document issue date",
	);
	assert(
		!identityOnlyXml.includes('ИНН="'),
		"identity-only XML must not invent taxpayer INN",
	);
	...
	assert(
		issuedXmlAfterMutationResponse.body.includes('СуммаКод1="3000.00"'),
		"issued XML must keep frozen payment amount",
	);
	assert(
		!issuedXmlAfterMutationResponse.body.includes("999999.00"),
		"issued XML must not use mutated live amount",
	);
	assert(
		issuedXmlAfterMutationResponse.body === xml,
	...
	assertNoMojibake(
		issuedXmlAfterMutationResponse.body,
		"issued tax XML after live payment mutation",
	);
