...
const profileDir = path.join(
	os.tmpdir(),
	`dental-crm-visit-live-smoke-${process.pid}`,
);
const screenshotPath =
	process.env.SMOKE_SCREENSHOT_PATH ?? "test-results/visit-live-smoke.png";
...
if (!browserPath) {
	throw new Error(
		"No Chromium/Edge browser found. Set BROWSER_BIN to run the visit live smoke test.",
	);
}
...
try {
	...
	await waitFor(
		cdp,
		`(() => Boolean(document.querySelector(".dictation-actions button")))()`,
		"dictation actions",
	);
	...
	if (!clickResult.result.value?.ok) {
		throw new Error(
			`Visit live workflow controls missing: ${JSON.stringify(clickResult.result.value)}`,
		);
	}
