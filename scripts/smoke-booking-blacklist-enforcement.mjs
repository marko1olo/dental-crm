...
} from "../apps/api/src/db/patientArchiveReasonsAndBlacklistsQuery.ts";
...
async function runBlacklistSmoke() {
	...
	assert.strictEqual(
		isBlocked,
		true,
		"Patient should be marked as booking blocked",
	);
	...
	try {
		...
		threw = true;
		assert.ok(
			err.message.includes("черный список") ||
				err.message.includes("Запись заблокирована"),
	...
	assert.strictEqual(
		threw,
		true,
		"createAppointmentInDb MUST throw an exception for blacklisted patient",
	);
	...
	const isBlockedAfter = await isPatientBookingBlocked(
		testOrgId,
		testPatientId,
	);
	assert.strictEqual(
		isBlockedAfter,
		false,
		"Patient should no longer be marked as booking blocked",
	);
