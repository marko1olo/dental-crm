...
(async () => {
	...
	try {
		...
		const clickedDemo = await page.evaluate(() => {
			...
			const demoBtn = buttons.find(
				(b) =>
					b.textContent && b.textContent.includes("Попробовать демо-режим"),
			);
			if (demoBtn) {
				demoBtn.click();
				return true;
		...
		if (clickedDemo) {
			...
			await page.evaluate(() => {
				const buttons = Array.from(document.querySelectorAll("button"));
				const proceedBtn = buttons.find(
					(b) => b.textContent && b.textContent.includes("Завершить"),
				);
