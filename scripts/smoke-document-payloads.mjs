...
const payload = documentPayloadSchema.parse({
	patientIntakeQuestionnaire: {
		chiefComplaint:
			"Боль при накусывании в области 36 зуба, плановая консультация перед лечением",
		...
		anticoagulants:
			"Антикоагулянты и препараты, влияющие на кровотечение, не принимает",
		infectiousRiskNotes: "Инфекционные риски не заявлены",
		cardioEndocrineNotes:
			"АД, сахарный диабет и сердечно-сосудистые риски требуют уточнения врачом перед вмешательством",
		emergencyContact: "+7 900 000-00-02, контактное лицо",
		additionalNotes:
			"Пациент предупрежден сообщать об изменениях до каждого визита",
	...
	paidMedicalServicesContract: {
		...
		serviceEndOrCondition:
			"until the agreed dental services are completed and accepted",
		...
		paymentTerms:
			"payment by cash register receipt before or on the service date",
		priceChangeRules:
			"any extra paid service requires written agreement before delivery",
		freeCareAvailabilityNotice:
			"patient was informed about possible free medical care routes under state guarantees where applicable",
		medicalRecommendationWarning:
			"failure to follow medical recommendations may reduce service quality, change terms or affect health",
		refusalAndRefundTerms:
			"refund follows actual provided services, documented expenses and fiscal correction rules",
		warrantyAndClaimsTerms:
			"warranty and claims follow clinic local rules, indications and patient compliance with recommendations",
	...
	completedWorksAct: {
		...
		acceptedServicesSummary:
			"Проведено лечение кариеса 36 зуба и композитная реставрация по согласованному плану.",
		totalByActRub: 12500,
		paidRub: 12500,
	...
	treatmentCostEstimate: {
		...
		excludedItems: [
			"услуги вне строк сметы",
			"дополнительная диагностика при новых показаниях",
		],
	...
	paymentInvoice: {
		...
		dueDate: "25.05.2026",
		paymentTerms:
			"оплата до визита с выдачей кассового чека после поступления денег",
		clinicBankDetails:
			"ООО DENTE Smoke Clinic, ИНН 6312000000, р/с 40702810000000000000",
		cashlessPaymentAllowed: true,
		cashDeskPaymentAllowed: true,
		qrPaymentPayload:
			"ST00012|Name=DENTE Smoke Clinic|Sum=1250000|Purpose=СЧ-2026-001",
	...
	installmentPaymentSchedule: {
		...
		latePaymentPolicy:
			"при переносе срока администратор фиксирует новый срок и уведомляет пациента",
		paymentMethodNotes:
			"оплата в кассе, по ссылке или безналично с выдачей кассового чека",
	...
	warrantyServiceMemo: {
		...
		warrantyPeriod:
			"12 месяцев при соблюдении рекомендаций и контрольных визитов",
		controlVisitSchedule:
			"контроль через 14 дней, затем профессиональная гигиена по индивидуальному графику",
		patientObligations: [
			"соблюдать гигиену",
			"приходить на контроль",
			"не перегружать реставрацию",
		],
		excludedRiskFactors: [
			"травма",
			"бруксизм",
	...
	informedConsent: {
		...
		materialOrMedicationNotes:
			"композитная реставрация, коффердам по показаниям",
		...
		alternatives: [
			...
			"выбрать другой метод восстановления",
		],
	...
	procedureSpecificConsent: {
		...
		diagnosisOrIndication:
			"разрушение коронковой части, боль и риск распространения инфекции",
		clinicalToothRows: sampleClinicalToothRows,
		plannedAnesthesia: "проводниковая анестезия артикаином 4%",
		materialsAndSystems:
			"шовный материал по показаниям, гемостатическая губка при необходимости",
	...
	prescriptionMedicationOrder: {
		...
		urgentContactReason:
			"отек, сыпь, одышка, кровотечение или нарастающая боль",
	},
	labWorkOrder: {
		clinicalToothRows: sampleClinicalToothRows,
		...
		deadline: "2026-05-30",
		technicianNotes:
			"Плотный контактный пункт, проверить окклюзию в центральной окклюзии.",
	},
	photoVideoConsent: {
	...
	xrayCbctReferral: {
		...
		clinicalQuestion:
			"оценить периапикальные ткани, корневые каналы и расстояние до нижнечелюстного канала",
		indication:
			"подготовка к хирургическому этапу и уточнение эндодонтической тактики",
		pregnancyStatus: "denied",
		safetyNotes:
			"беременность со слов пациента отрицается, использовать стандартные средства защиты",
		priority: "routine",
	...
	medicalRecordExtract: {
		...
		objectiveStatus:
			"Кариозная полость 36 зуба, перкуссия слабо болезненна, слизистая без острого отека.",
		diagnosis:
			"Кариес дентина 36 зуба, состояние после терапевтического лечения.",
		clinicalToothRows: sampleClinicalToothRows,
		treatmentProvided:
			"Проведена инфильтрационная анестезия, препарирование и восстановление 36 зуба композитным материалом.",
		recommendations:
			"Не принимать пищу до окончания действия анестезии, соблюдать гигиену, контрольный осмотр по записи.",
		doctorFullName: "Доктор Смоук",
		recipientFullName: "Тестовый пациент для payload",
		recipientAuthority: "пациент лично",
		...
		specialistVisitRecords: [
			{
				...
				anamnesis:
					"Анамнез собран со слов пациента, ранее проведена диагностика.",
				objectiveData:
					"Кариозная полость 36 зуба, слизистая без острого отека.",
				...
				orders:
					"Гигиена, контрольный осмотр, срочно обратиться при боли или отеке.",
				treatmentProvided:
					"Проведена инфильтрационная анестезия, препарирование и восстановление 36 зуба композитным материалом.",
	...
	dentalMedicalCard043u: {
		formNumber: "043/у",
		organization: {
			fullName: 'ООО "DENTE Smoke Clinic"',
			shortName: "DENTE Smoke",
			address: "г. Москва, ул. Смоук, д. 1",
			phone: "+7 (495) 000-00-00",
			ogrn: "1027700132195",
			inn: "7707083893",
			licenseNumber: "ЛО-77-01-000001",
			licenseIssueDate: "2020-01-15",
			licenseAuthority: "Департамент здравоохранения города Москвы",
		},
		patient: {
			fullName: "Пациент Смоук",
			birthDate: "1990-05-12",
			sex: "мужской",
			phone: "+7 (900) 000-00-00",
			address: "г. Москва, ул. Пациента, д. 2",
			documentSeriesNumber: "4500 123456",
			snils: "123-456-789 00",
			medicalCardNumber: "МК-043-SMOKE-001",
		},
		doctor: {
		...
		anamnesis: "Боль усиливается 3 дня, ранее лечение 36 зуба не проводилось.",
		structuredAnamnesis: null,
		statusLocalis: "36 зуб: глубокая кариозная полость, зондирование болезненно",
		objectiveStatus:
			"Слизистая бледно-розовая, прикус ортогнатический, 36 зуб с кариозной полостью",
		...
		clinicalToothRows: sampleClinicalToothRows,
		recommendations: "Гигиена, мягкая пища 24 часа, контроль при жалобах",
	...
	postVisitRecommendations: {
		...
		doctorFullName: "Доктор Смоук",
		allowedAfter: [
			"пить воду можно после окончания действия анестезии",
			"есть мягкую прохладную пищу после восстановления чувствительности",
		],
		...
		medicationAndRinsePlan: [
			"принимать обезболивающее по схеме врача",
			"антибиотики только если назначены отдельно",
			"ванночки антисептиком со вторых суток по назначению",
		],
		hygieneInstructions: [
		...
		telegramSummary:
			"После удаления соблюдайте ограничения, принимайте назначения по схеме врача и срочно свяжитесь с клиникой при кровотечении, нарастающей боли, отеке, температуре или аллергии.",
	...
	treatmentPlan: {
		...
		risksAndLimitations: [
			"изменение объема после диагностики",
			"необходимость дополнительного визита",
			"ограниченный прогноз при исходном состоянии тканей",
		],
		prognosisAndLimits:
			"прогноз благоприятный при соблюдении гигиены, рекомендаций и контрольных визитов",
	...
	treatmentPlanAcceptance: {
		...
		clinicalGoal:
			"санация 36 зуба, восстановление функции и снижение риска осложнений",
		...
		acceptedStages: [
			{
				stageName: "Терапевтический этап",
				plannedServices:
					"инфильтрационная анестезия, лечение кариеса, композитная реставрация",
				plannedTiming: "18.05.2026",
				estimatedAmountRub: 12500,
			},
			{
		...
		paymentTerms:
			"оплата по кассовому чеку до оказания услуг, рассрочка оформляется отдельным соглашением",
		...
		risksAndLimitations: [
			"изменение плана при новых клинических данных",
			"необходимость дополнительного визита",
			"ограниченный прогноз при исходном состоянии тканей",
		],
		warrantyAndControlTerms:
			"контрольные визиты обязательны, гарантийные условия зависят от соблюдения рекомендаций",
		doctorFullName: "Доктор Смоук",
	...
	medicalInterventionRefusal: {
		...
		alternativesOffered: [
			...
			"обращение в дежурную стоматологию",
		],
...
const cases = [
	...
	},
	{
		...
		payload: {
			minorLegalRepresentativeConsent: payload.minorLegalRepresentativeConsent,
		},
		fragments: [
	...
	},
	{
		...
		fragments: [
			"Гарантийная памятка",
			"композитная реставрация 36 зуба",
			"12 месяцев",
			"Что должен соблюдать пациент",
		],
		missingReason: "структурированные данные",
	},
	{
	...
	},
	{
		...
		payload: {
			prescriptionMedicationOrder: payload.prescriptionMedicationOrder,
		},
		fragments: [
			"Назначение лекарственных препаратов",
			"Клиническая привязка назначения",
			"Амоксициллин",
			"принимать после еды",
			"окклюзионная",
	...
	},
	{
		...
		fragments: [
			"Зуботехнический заказ-наряд",
			"Клиническая привязка лабораторной работы",
			"Циркониевая коронка",
			"Монолитный цирконий",
			"Внутриротовой скан STL",
			"окклюзионная",
		],
		missingReason: "структурированные данные",
	},
	...
	},
	{
		...
		payload: {
			medicalDocumentReleaseReceipt: payload.medicalDocumentReleaseReceipt,
		},
		fragments: [
			"Расписка о выдаче медицинской документации",
			"Архив исходных снимков КТ",
			"пациент лично",
	...
	},
	{
		...
		fragments: [
			...
			"Учетная форма N 043/у",
			"МК-043-SMOKE-001",
			"Пациент Смоук",
			"Боль при накусывании в области 36 зуба",
			"Кариес дентина 36 зуба",
			"Клиническая детализация по зубам",
			"окклюзионная",
			"Доктор Смоук",
	...
	},
	{
		...
		fragments: [
			"Запрос на копии медицинской документации",
			"Архив исходных снимков КТ",
			"защищенный портал",
			"исходные файлы снимков",
		],
	...
	},
	{
		kind: "post_visit_recommendations",
		payload: {
			postVisitRecommendations: postVisitRecommendationsForTopic(
				"implantation",
				{
					...
					plannedFollowUpAt:
						"контроль и снятие швов в срок, назначенный врачом",
					telegramSummary:
		...
		payload: {
			postVisitRecommendations: postVisitRecommendationsForTopic(
				"filling_restoration",
				{
					...
					plannedFollowUpAt:
						"контроль при дискомфорте, завышении пломбы или по плану врача",
					telegramSummary:
	...
	},
	{
		...
		fragments: [
			"Справка о посещении врача-стоматолога",
			"18.05.2026 10:00 - 18.05.2026 11:20",
			"не является листком нетрудоспособности",
			"диагноз",
		],
		missingReason: "структурированные данные",
	},
	...
	},
	{
		...
		payload: {
			personalDataProcessingConsent: payload.personalDataProcessingConsent,
		},
		fragments: [
...
for (const entry of cases) {
	...
	for (const fragment of entry.fragments) {
		assert(
			...
			`${entry.kind}: missing rendered payload fragment "${fragment}"`,
		);
	}
	for (const fragment of forbiddenEnglishUiFragments) {
		assert(
			!html.includes(fragment),
			`${entry.kind}: rendered English UI fragment "${fragment}"`,
		);
	}
	assert(!html.includes("undefined"), `${entry.kind}: rendered undefined`);
	const issueBlockReason = documentIssueBlockReason(
		document,
		patient,
		renderContext,
	);
	assert(
		issueBlockReason === null,
		`${entry.kind}: complete payload must be issue-ready, got ${issueBlockReason}`,
	);

	const missingPayloadDocument = documentFor(entry.kind, null);
	const reason = documentIssueBlockReason(
		missingPayloadDocument,
		patient,
		renderContext,
	);
	assert(
		reason?.includes(entry.missingReason),
		`${entry.kind}: missing payload was not blocked, got ${reason}`,
	);
}
...
assert(
	!documentPayloadSchema.safeParse(invalidInvoiceDatePayload).success,
	"payment invoice must reject impossible dates",
);
...
assert(
	!documentPayloadSchema.safeParse(invalidReceiptBirthDatePayload).success,
	"payment receipt tax birth date must reject non-official formats",
);
const invalidPaidContractDatePayload = JSON.parse(JSON.stringify(payload));
invalidPaidContractDatePayload.paidMedicalServicesContract.contractDate =
	"not-a-date";
assert(
	!documentPayloadSchema.safeParse(invalidPaidContractDatePayload).success,
	"paid medical services contract must reject non-date contract dates",
);
const invalidPaidContractServiceStartPayload = JSON.parse(
	JSON.stringify(payload),
);
invalidPaidContractServiceStartPayload.paidMedicalServicesContract.serviceStart =
	"31.02.2026";
assert(
	!documentPayloadSchema.safeParse(invalidPaidContractServiceStartPayload)
		.success,
	"paid medical services contract must reject impossible service start dates",
);
const invalidPaidContractSignedAtPayload = JSON.parse(JSON.stringify(payload));
invalidPaidContractSignedAtPayload.paidMedicalServicesContract.signedAt =
	"yesterday";
assert(
	!documentPayloadSchema.safeParse(invalidPaidContractSignedAtPayload).success,
	"paid medical services contract must reject non-date signing timestamps",
);
const invalidPaidContractTimePayload = JSON.parse(JSON.stringify(payload));
invalidPaidContractTimePayload.paidMedicalServicesContract.signedAt =
	"18.05.2026 99:99";
assert(
	!documentPayloadSchema.safeParse(invalidPaidContractTimePayload).success,
	"paid medical services contract must reject impossible signing times",
);
...
for (const [branch, field, value] of [
	...
	assert(
		!documentPayloadSchema.safeParse(invalidPayload).success,
		`${branch}.${field} must reject invalid date/time values`,
	);
}
...
const sparseLegalContext = {
	...renderContext,
	clinicProfile: sparseClinicProfile,
};
for (const [kind, branch] of [
	[
		"patient_intake_questionnaire",
		{ patientIntakeQuestionnaire: payload.patientIntakeQuestionnaire },
	],
	[
		"post_visit_recommendations",
		{ postVisitRecommendations: payload.postVisitRecommendations },
	],
	["lab_work_order", { labWorkOrder: payload.labWorkOrder }],
	[
		"warranty_service_memo",
		{ warrantyServiceMemo: payload.warrantyServiceMemo },
	],
]) {
	assert(
		documentIssueBlockReason(
			documentFor(kind, branch),
			patient,
			sparseLegalContext,
		) === null,
...
assert(
	documentIssueBlockReason(
		documentFor("payment_invoice", { paymentInvoice: payload.paymentInvoice }),
		patient,
		sparseLegalContext,
	) !== null,
...
const completedActHtml = renderDocumentHtml(
	documentFor("completed_works_act", {
		completedWorksAct: payload.completedWorksAct,
	}),
	patient,
	renderContext,
);
...
const medicalRecordExtractHtml = renderDocumentHtml(
	documentFor("medical_record_extract", {
		medicalRecordExtract: payload.medicalRecordExtract,
	}),
	patient,
	renderContext,
);
...
for (const [kind, branchName] of [
	...
	assert(
		reason?.includes("клинические строки"),
		`${kind}: missing clinical tooth rows must be blocked, got ${reason}`,
	);
}

const publicDocument = publicGeneratedDocumentSchema.parse(
	documentFor("patient_intake_questionnaire", {
		patientIntakeQuestionnaire: payload.patientIntakeQuestionnaire,
	}),
);
assert(
	!("payload" in publicDocument),
	"public document schema must redact structured medical/admin payloads",
);
assert(
	!JSON.stringify(publicDocument).includes("36"),
	"public document schema must not expose intake clinical details",
);
...
assert(
	!mismatchedCreatePayload.success,
	"createDocumentSchema must reject payload branches from another document kind",
);
assert(
...
assert(
	trimmedCreateTitlePayload.title === "Intake packet",
	"createDocumentSchema must trim custom titles",
);
...
assert(
	actualKeysEmpty.length === 0,
	"documentPayloadActualKeys must return empty array for null payload",
);

const actualKeysUndefined = documentPayloadActualKeys(undefined);
assert(
	actualKeysUndefined.length === 0,
	"documentPayloadActualKeys must return empty array for undefined payload",
);

const actualKeysFiltered = documentPayloadActualKeys({
	a: 1,
	b: undefined,
	c: "test",
});
assert(
	actualKeysFiltered.length === 2 &&
		actualKeysFiltered.includes("a") &&
		actualKeysFiltered.includes("c"),
	"documentPayloadActualKeys must ignore undefined values",
);

// documentPayloadAllowedKeys
const allowedKeysIntake = documentPayloadAllowedKeys(
	"patient_intake_questionnaire",
);
assert(
	allowedKeysIntake.length === 1 &&
		allowedKeysIntake[0] === "patientIntakeQuestionnaire",
	"documentPayloadAllowedKeys must return expected keys for known kind",
);

const allowedKeysUnknown = documentPayloadAllowedKeys("unknown_kind");
assert(
	allowedKeysUnknown.length === 0,
	"documentPayloadAllowedKeys must return empty array for unmapped/unknown kind",
);

// documentPayloadDisallowedKeys
const disallowedKeysEmpty = documentPayloadDisallowedKeys(
	"patient_intake_questionnaire",
	null,
);
assert(
	disallowedKeysEmpty.length === 0,
	"documentPayloadDisallowedKeys must return empty array for null payload",
);

const disallowedKeysAllowedOnly = documentPayloadDisallowedKeys(
	"patient_intake_questionnaire",
	{ patientIntakeQuestionnaire: { clinical: { text: "" } } },
);
assert(
	disallowedKeysAllowedOnly.length === 0,
	"documentPayloadDisallowedKeys must return empty array when only allowed keys are present",
);

const disallowedKeysMixed = documentPayloadDisallowedKeys(
	"patient_intake_questionnaire",
	{
		patientIntakeQuestionnaire: { clinical: { text: "" } },
		randomKey: 123,
...
assert(
	disallowedKeysMixed.length === 1 && disallowedKeysMixed[0] === "randomKey",
	"documentPayloadDisallowedKeys must return array of disallowed keys when unmapped keys are present",
);

console.log(
	JSON.stringify({ ok: true, checked: cases.map((entry) => entry.kind) }),
);
