...
const sampleDataUrl = pathToFileURL(
	path.resolve("apps/api/dist/sampleData.js"),
).href;
const rendererUrl = pathToFileURL(
	path.resolve("apps/api/dist/documents/renderDocument.js"),
).href;
const { buildDashboard } = await import(
	`${sampleDataUrl}?encoding-smoke=${Date.now()}`
);
const { documentIssueBlockReason, renderDocumentHtml } = await import(
	`${rendererUrl}?encoding-smoke=${Date.now()}`
);
...
const checkedStrings = collectStrings({
	dashboard,
	renderedDocuments,
	documentIssueReasons,
});
const mojibakeHits = checkedStrings.filter((value) =>
	/(?:Ã.|Â.|Ð.|Ñ.|â.|�)/.test(value),
);
const garbledQuestionHits = checkedStrings.filter((value) =>
	/\?{4,}/.test(value),
);

assert(
	checkedStrings.length > 250,
	"encoding smoke did not inspect enough UI strings",
);
assert(
	mojibakeHits.length === 0,
	`mojibake text leaked to dashboard: ${mojibakeHits.slice(0, 3).join(" | ")}`,
);
assert(
	garbledQuestionHits.length === 0,
	`replacement question-mark text leaked to dashboard: ${garbledQuestionHits.slice(0, 3).join(" | ")}`,
);
assert(
	renderedDocuments.length > 0,
	"encoding smoke did not render document HTML",
);
assert(
	renderedDocuments.some((html) => html.includes("Пациент")),
	"document HTML was not inspected as readable Russian",
);
assert(
	renderedDocuments.some((html) => html.includes('class="check-list"')),
	"document checklist HTML was not inspected",
);
assert(
	renderedDocuments.some((html) => html.includes("\u25a1")),
	"document checklist marker was not rendered as a real square marker",
);
assert(
...
assert(
	dashboard.communicationTemplates.some((template) =>
		template.title.includes("Подтверждение приема"),
	),
	"communication template title was not repaired",
);
assert(
	dashboard.protocolTemplates.some((template) =>
		template.objectiveTemplate.includes("Осмотр слизистой"),
	),
	"protocol template text was not repaired",
);
assert(
	dashboard.appointments.some((appointment) =>
		appointment.reason?.includes("Лечение 36"),
	),
	"appointment reason was not repaired",
);
assert(
	dashboard.appointmentReadiness.some((item) =>
		item.checks.some((check) => check.title === "Пациент"),
	),
...
const englishValidationHits =
	sharedSource.match(
		/"[^"]*(must be|is required|Invalid input|Telegram bot username|startsAt must|endsAt must)[^"]*"/g,
	) ?? [];
assert(
...
assert(
	...
	"API must keep bounded generic validation fallback copy.",
);
assert(
	serverSource.includes("createDenteApiApp"),
	"API server must expose an app factory for runtime error-boundary proof.",
);
assert(
	!serverSource.includes("issues: error.issues"),
	"API global validation fallback must not return raw zod issue arrays.",
);
assert(
	!serverSource.includes("localizeZodIssueMessage"),
	"API global validation fallback must not map zod issue details into public payloads.",
);
assert(
	serverSource.includes("apiTechnicalErrorPattern"),
	"API catch-all error handler must filter technical exception text.",
);
assert(
	serverSource.includes("publicApiErrorMessage"),
	"API catch-all error handler must keep only operator-readable domain messages.",
);
assert(
	!serverSource.includes("reply.send(error);"),
	"API catch-all error handler must not send raw Error objects.",
);
assert(
...
assert(
	englishApiMessageHits.length === 0,
	`english API user-facing text leaked: ${englishApiMessageHits.slice(0, 5).join(", ")}`,
);
const englishUiMessageHits = [
	'"Undo"',
	">Undo<",
	"кнопкой Undo",
	"local undo",
...
assert(
	englishUiMessageHits.length === 0,
	`english UI text leaked: ${englishUiMessageHits.slice(0, 5).join(", ")}`,
);
