...
export function declaresViolationsIn(output) {
	const counted = [...output.matchAll(/НАРУШЕНИЙ:\s*(\d+)/g)].map((match) =>
		Number(match[1]),
	);
...
function collect(directory) {
	...
	for (const entry of readdirSync(directory)) {
		...
		const isChain =
			path.basename(directory) === "chains" &&
			entry.endsWith(".ts") &&
			!entry.endsWith(".d.ts");
...
function summaryLine(output) {
	...
	const verdict = [...lines]
		.reverse()
		.find((line) => /СОШЛИСЬ|РАСХОЖДЕНИ|ШВЫ|разрыв/i.test(line));
...
if (!invokedDirectly) {
	...
	console.log(
		`Сквозных сценариев: ${scenarios.length}. Идут строго по одному: база одна на всех.\n`,
	);
	...
	const byOutcome = (name) =>
		results.filter((result) => result.outcome === name);
	const diverged = [
		...byOutcome("разошлось"),
		...byOutcome("заявил нарушения (код возврата 0)"),
	];
	...
	if (diverged.length)
		console.log(
			`разошлось: ${diverged.map((result) => result.file).join(", ")}`,
		);
	if (unavailable.length)
		console.log(
			`база недоступна: ${unavailable.map((result) => result.file).join(", ")}`,
		);
	if (timedOut.length)
		console.log(`таймаут: ${timedOut.map((result) => result.file).join(", ")}`);
