...
forbidIn(
	...
	"App.tsx must not import finance planning subcomponents directly",
);
forbidIn(
	appSource,
	"<FinancePlanningOverview",
	"App.tsx must not render finance summary directly",
);
forbidIn(
	appSource,
	"<ServiceCatalogStrip",
	"App.tsx must not render service catalog strip directly",
);

requireIn(
	financeViewSource,
	"<FinancePlanningOverview",
	"FinanceView must delegate finance summary and scenarios",
);
requireIn(
	financeViewSource,
	"<ServiceCatalogStrip",
	"FinanceView must delegate service catalog strip",
);
requireIn(
	financeViewSource,
	"billingSummary={billingSummary}",
	"FinanceView must pass patient-scoped finance totals into planning overview",
);
requireIn(
	financeViewSource,
	"onGoToVisit={onGoToVisit}",
	"FinanceView must give empty plan states a direct route to the visit",
);
requireIn(
	financeViewSource,
	"onGoToPrices={onGoToPrices}",
	"FinanceView must give empty catalog states a direct route to prices",
);
forbidIn(
	financeViewSource,
...
forbidIn(
	...
	"App.tsx must not inline finance summary cards",
);
forbidIn(
	appSource,
	'className="plan-scenario-grid"',
	"App.tsx must not inline scenario grid",
);
forbidIn(
	appSource,
	'className="service-catalog-strip"',
	"App.tsx must not inline service catalog strip",
);

requireIn(
	financeSource,
	"export function FinancePlanningOverview",
	"FinancePlanning must export summary/scenario component",
);
requireIn(
	financeSource,
...
requireIn(
	financeSource,
	'className="plan-scenarios"',
	"FinancePlanning must own plan scenario section",
);
requireIn(
	financeSource,
	'className="service-catalog-strip"',
	"FinancePlanning must own catalog strip markup",
);
requireIn(
	financeSource,
	'className="finance-empty-state"',
	"FinancePlanning must show explicit empty states",
);
requireIn(
	financeSource,
	"Вариантов плана пока нет",
	"FinancePlanning must explain missing plan scenarios",
);
requireIn(
	financeSource,
	"Каталог услуг пуст",
	"FinancePlanning must explain missing catalog data",
);
requireIn(
	financeSource,
	"onClick={onGoToVisit}",
	"Empty plan scenarios must have a direct visit action",
);
requireIn(
	financeSource,
...
requireIn(
	cssSource,
	".finance-empty-state",
	"CSS must style finance empty state",
);
requireIn(
	cssSource,
	".finance-empty-state .text-button",
	"CSS must keep finance empty-state actions aligned",
);
