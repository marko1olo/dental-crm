...
function absoluteUrl(input) {
	if (typeof input === "string")
		return new URL(input, "https://clinic.local").href;
	return input.url;
}
...
class FakeCache {
	...
	async addAll(paths) {
		for (const path of paths) {
			this.entries.set(
				new URL(path, "https://clinic.local").href,
				new Response(`cached:${path}`, { status: 200 }),
			);
		}
	}
...
let fetchHandler = async (request) =>
	new Response(`network:${pathOf(request)}`, { status: 200 });
...
assert(
	...
	"install must cache index shell fallback",
);
...
assert(
	!cacheWrites.some((path) => path.startsWith("/api/")),
	"api responses must not be cached",
);
assert(
	!cacheWrites.some((path) => path.startsWith("/imaging/")),
	"DICOM/imaging responses must not be cached",
);
assert(
	!cacheWrites.includes("/exports/model.stl"),
	"mesh/CAD/STL responses must not be cached",
);

cacheWrites.length = 0;
await dispatchFetch("/assets/workspace-A1b2C3.js");
assert(
	cacheWrites.includes("/assets/workspace-A1b2C3.js"),
...
fetchHandler = async (request) => {
	if (pathOf(request) === "/assets/workspace-stale.js")
...
assert(
	(await staleFallback.text()) === "cached-stale",
	"network-first JS assets must fall back to cached shell assets offline",
);
...
assert(
	(await navigationFallback.text()).includes("cached:/index.html"),
	"offline navigation must fall back to cached index",
);

await dispatchMessage({ type: "DENTE_SKIP_WAITING" });
assert(
	skipWaitingCalled,
	"message protocol must allow a waiting worker to activate",
);
...
assert(
	await caches.match("/index.html"),
	"stale chunk recovery must preserve index shell fallback",
);
assert(
	await caches.match("/offline.html"),
	"stale chunk recovery must preserve offline shell fallback",
);
assert(
	!(await caches.match("/assets/workspace-stale.js")),
	"stale chunk recovery must evict dynamic shell chunks",
);
assert(
