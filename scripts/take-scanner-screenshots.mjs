...
const OUT = "C:/Users/Admin/.gemini/antigravity/brain/49ca46e2-a0f7-43e5-a510-f484e6e15d21";
...
const tmpProfile = path.join(process.env.TEMP || "C:/tmp", "scanner-screenshots-profile");
...
try {
  ...
  await new Promise((resolve, reject) => { socket.onopen = resolve; socket.onerror = () => reject(new Error("CDP failed")); });
  ...
  await waitFor(cdp, `(() => !!(document.querySelector(".app-shell") || document.querySelector("input[type='email']") || document.querySelector("h1")))()`, "auth or app shell");
  await sleep(1000);
  await sleep(1000);

  // Inject token if missing and reload
  const needsAuth = await evaluate(cdp, `(() => !localStorage.getItem("dente_clinic_token"))()`);
  if (needsAuth) {
    ...
    const { configuredAuthTokenSecret } = await import("../apps/api/dist/accessGuard.js");
    const secret = configuredAuthTokenSecret();
    
    const clinicToken = signToken({ organizationId: orgId, clinicName: "Стоматология" }, secret, 86400);
    const staffToken = signToken({ userId, fullName: "Doctor", role: "doctor", organizationId: orgId }, secret, 86400);
  ...
  await waitFor(cdp, `(() => document.readyState === "complete" && Boolean(document.querySelector("h1")))()`, "scanner view");
  await sleep(3000);
