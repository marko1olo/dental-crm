...
const autoFlushBlock = sourceSlice(
...
for (const marker of [
	...
	if (!recoveryBlock.includes(marker))
		fail(
			`Speech recovery request must stay active-visit scoped before fetch: ${marker}`,
		);
}
...
for (const marker of [
	...
	if (!applyBlock.includes(marker))
		fail(`Speech apply guard missing marker: ${marker}`);
}
...
for (const marker of [
	...
	if (!source.includes(marker))
		fail(`Speech primary recording path missing marker: ${marker}`);
}
...
for (const marker of [
	...
	if (!autoFlushBlock.includes(marker))
		fail(`Speech queued-audio auto flush missing marker: ${marker}`);
}
...
for (const marker of [
	...
	if (!source.includes(marker))
		fail(`Speech queued-audio failure feedback missing marker: ${marker}`);
}
