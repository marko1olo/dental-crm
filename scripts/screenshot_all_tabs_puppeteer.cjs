...
async function run() {
	...
	for (const el of elementsNav) {
		const text = await page.evaluate(
			(e) => e.textContent || e.title || e.className,
			el,
		);
		if (
			text.toLowerCase().includes("setting") ||
			text.includes("lucide-settings")
		) {
			await el.click();
			break;
		}
	...
	await page.evaluate(() => {
		...
		const target = spans.find(
			(s) =>
				s.textContent.includes("Протоколы") ||
				s.textContent.includes("Шаблоны"),
		);
		if (target) {
			target.click();
		}
	...
	await page.screenshot({
		path: path.join(basePath, "Settings_Protocols_Dark.png"),
		fullPage: true,
	});
	...
	await page.screenshot({
		path: path.join(basePath, "Settings_Protocols_Light.png"),
		fullPage: true,
	});
	...
	await wait(1000);
	await page.screenshot({
		path: path.join(basePath, "Settings_Protocols_Mobile_Dark.png"),
		fullPage: true,
	});
	...
	await wait(1000);
	await page.screenshot({
		path: path.join(basePath, "Settings_Protocols_Mobile_Light.png"),
		fullPage: true,
	});
