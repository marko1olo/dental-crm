...
requireIn(
	appSource,
	"Выберите пациента и активный прием перед записью оплаты.",
	"Payment handler must fail visibly without a patient/visit context",
);
requireIn(
	appSource,
...
requireIn(
	recordPaymentSource,
	"rubAmountInputMissingStep(paymentAmount)",
...
requireIn(
	rubAmountInputSource,
	"!/^\\d+$/.test(compactAmount)",
	"Whole-ruble parser must reject commas, dots, signs and mixed text",
);
requireIn(
	rubAmountInputSource,
...
requireIn(
	...
	"Whole-ruble parser must expose a clear no-kopecks operator message",
);
