...
async function run() {
  ...
  await page.screenshot({ path: path.join(basePath, 'Settings_Rules_Dark.png'), fullPage: true });

  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'light' }]);
  await wait(1000);

  await page.screenshot({ path: path.join(basePath, 'Settings_Rules_Light.png'), fullPage: true });

  // Mobile
  await page.setViewport({ width: 375, height: 812, isMobile: true });
  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'dark' }]);
  await wait(1000);
  await page.screenshot({ path: path.join(basePath, 'Settings_Rules_Mobile_Dark.png'), fullPage: true });

  await page.emulateMediaFeatures([{ name: 'prefers-color-scheme', value: 'light' }]);
  await wait(1000);
  await page.screenshot({ path: path.join(basePath, 'Settings_Rules_Mobile_Light.png'), fullPage: true });
