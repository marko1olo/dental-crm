...
const { taxDeductionApplicationPayloadSchema } = await import(
...
const payments = [
	...
	},
	{
		...
		fiscalReceiptUrl:
			"https://clinic.example.test/fiscal/FN-FISCAL-2025-CRM-2026",
		payerFullName: "Fiscal Payer",
		payerInn: "123456789012",
		payerBirthDate: "1988-02-03",
		payerIdentityDocument: "passport 3600 000000",
...
certificate.payload = {
	taxPaymentSelection: { selectedPaymentIds: [payments[0].id, payments[1].id] },
};
registry.payload = {
	taxPaymentSelection: { selectedPaymentIds: [payments[0].id, payments[1].id] },
};
...
const legacyOldFormCertificate = documentFor(
	"legacy_tax_deduction_certificate",
);
legacyOldFormCertificate.taxYear = 2023;
legacyOldFormCertificate.totalAmountRub = 4000;
legacyOldFormCertificate.payload = {
	taxPaymentSelection: { selectedPaymentIds: [payments[4].id] },
};
...
assert(
	certificateHtml.includes("Fiscal Payer"),
	"certificate must use payer name from fiscal payment",
);
assert(
	certificateHtml.includes("123456789012"),
	"certificate must use payer INN from fiscal payment",
);
assert(
	certificateHtml.includes("Форма по КНД 1151156"),
	"KND certificate must render official form title",
);
assert(
	certificateHtml.includes("Лист 001"),
	"KND certificate must render page 001 control block",
);
assert(
	certificateHtml.includes("Номер справки"),
	"KND certificate must render certificate number field",
);
assert(
	certificateHtml.includes("Номер корректировки"),
	"KND certificate must render correction number field",
);
assert(
	!certificateHtml.includes("Код вида документа"),
	"KND certificate must not fill identity document code when payer INN is present",
);
assert(
	!/DENTE-KND-/.test(certificateHtml),
	"KND certificate number must not use internal DENTE slug format",
);
assert(
	certificateHtml.includes("Налогоплательщик и пациент являются одним лицом"),
	"KND certificate must render taxpayer/patient same-person flag",
);
assert(
	certificateHtml.includes("1 - да"),
	"KND certificate must mark patient-paid services as same person",
);
assert(
	certificateHtml.includes("Л041-01184-63/00000000"),
	"certificate must include clinic license data",
);
assert(
	certificateHtml.includes("FN-2026-CODE1"),
	"KND certificate must include ordinary fiscal receipt basis",
);
assert(
	certificateHtml.includes("FN-2026-CODE2"),
	"KND certificate must include expensive fiscal receipt basis",
);
assert(
	normalizedCertificateHtml.includes("1 000"),
	"certificate must split ordinary code-1 amount",
);
assert(
	normalizedCertificateHtml.includes("2 000"),
	"certificate must split expensive code-2 amount",
);
assert(
	!certificateHtml.includes("FN-FISCAL-2025-CRM-2026"),
	"certificate must use fiscal receipt year, not CRM paidAt year",
);
assert(
	documentIssueBlockReason(certificate, patient, context) === null,
	"complete tax certificate must be issue-ready",
);
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingFiscalContext,
	)?.includes("номер фискального чека"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingFiscalDateContext,
	)?.includes("дату фискального чека"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingTaxCodeContext,
	)?.includes("код медицинской услуги"),
	"tax certificate must be blocked when an included paid payment has no explicit tax deduction service code",
);
const missingTaxCodeHtml = renderDocumentHtml(
	certificate,
	patient,
	missingTaxCodeContext,
);
assert(
	missingTaxCodeHtml.includes("код не выбран"),
	"tax certificate draft must not silently default missing tax code to code 1",
);
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingPayerNameContext,
	)?.includes("ФИО плательщика"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingPayerBirthDateContext,
	)?.includes("дату рождения плательщика"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		missingPayerRelationshipContext,
	)?.includes("родство плательщика"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		invalidPayerRelationshipContext,
	)?.includes("пациент, супруг, родитель"),
	...
	payments: [
		{
			...
			payerFullName: "Spouse Fiscal Payer",
			payerInn: "223456789012",
		},
	],
});
assert(
	spousePayerHtml.includes("0 - нет"),
	"KND certificate must mark spouse payer as not the patient",
);
assert(
	spousePayerHtml.includes("Лист 002"),
	"KND certificate must render patient page when payer is not the patient",
);
assert(
	spousePayerHtml.includes(patient.birthDate),
	"KND certificate must keep patient birth date separate from payer data",
);
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		mixedTaxpayerContext,
	)?.includes("разных налогоплательщиков"),
...
assert(
	documentIssueBlockReason(
		certificate,
		patient,
		mixedIdentityDocumentContext,
	)?.includes("разных налогоплательщиков"),
	"tax certificate must be blocked when payer name and INN match but identity document differs",
);
assert(
	documentIssueBlockReason(scopedCertificate, patient, mixedTaxpayerContext) ===
		null,
	"tax certificate with payer INN scope must ignore other taxpayers in the same year",
);
const scopedRegistryHtml = renderDocumentHtml(
	scopedRegistry,
	patient,
	mixedTaxpayerContext,
);
assert(
	scopedRegistryHtml.includes("FN-2026-CODE1"),
	"scoped registry must include selected payer receipt",
);
assert(
	!scopedRegistryHtml.includes("FN-2026-CODE2"),
	"scoped registry must exclude another payer receipt",
);
assert(
	documentIssueBlockReason(legacyCertificate, patient, context)?.includes(
		"2024",
	),
	"KND 1151156 certificate must be blocked for pre-2024 payment years",
);
const legacyOldFormHtml = renderDocumentHtml(
	legacyOldFormCertificate,
	patient,
	context,
);
assert(
	legacyOldFormHtml.includes("289/БГ-3-04/256"),
	"legacy tax certificate must reference the old certificate order",
);
assert(
	legacyOldFormHtml.includes("FN-LEGACY-2023"),
	"legacy tax certificate must include old-year fiscal receipt",
);
assert(
	!legacyOldFormHtml.includes("FN-2026-CODE1"),
	"legacy tax certificate must exclude KND-year payments",
);
assert(
	documentIssueBlockReason(legacyOldFormCertificate, patient, context) === null,
	"complete legacy tax certificate must be issue-ready",
);
assert(
	documentIssueBlockReason(legacyOldFormWrongYear, patient, context)?.includes(
		"2021-2023",
	),
...
assert(
	registryHtml.includes("FN-2026-CODE1"),
	"registry must include selected-year fiscal receipt code 1",
);
assert(
	registryHtml.includes("FN-2026-CODE2"),
	"registry must include selected-year fiscal receipt code 2",
);
assert(
	registryHtml.includes("01.03.2026"),
	"registry must render fiscal receipt date first",
);
assert(
	!registryHtml.includes("02.03.2026"),
	"registry must not use CRM paidAt when fiscal receipt date exists",
);
assert(
	!registryHtml.includes("FN-OLD-YEAR"),
	"registry must exclude payments outside selected tax year",
);
assert(
	documentIssueBlockReason(registry, patient, context) === null,
	"complete tax registry must be issue-ready",
);
const issuedRegistry = {
	...
	taxPaymentSnapshot: {
		...
		fiscalReceiptKeys: [
			payments[0].fiscalReceiptNumber,
			payments[1].fiscalReceiptNumber,
		],
	...
	payments: [
		{
			...payments[0],
			amountRub: 999999,
			fiscalReceiptNumber: "MUTATED-FISCAL-AFTER-ISSUE",
		},
		{
			...payments[1],
			amountRub: 999999,
			fiscalReceiptNumber: "MUTATED-FISCAL-AFTER-ISSUE-2",
		},
	],
});
assert(
	issuedRegistryHtml.includes("FN-2026-CODE1"),
	"issued registry must keep frozen fiscal receipt code 1",
);
assert(
	issuedRegistryHtml.includes("FN-2026-CODE2"),
	"issued registry must keep frozen fiscal receipt code 2",
);
assert(
	!issuedRegistryHtml.includes("MUTATED-FISCAL-AFTER-ISSUE"),
	"issued registry must not use mutated live receipt numbers",
);
assert(
	issuedRegistryHtml.replace(/\u00a0/g, " ").includes("1 000"),
	"issued registry must keep frozen payment amount",
);
assert(
	!issuedRegistryHtml.includes("999999"),
	"issued registry must not use mutated live payment amount",
);
...
assert(
	documentIssueBlockReason(
		...
		patient,
		context,
	)?.includes("структурированные данные"),
...
assert(
	applicationHtml.includes("Fiscal Payer"),
	"tax application must render structured taxpayer name",
);
assert(
	applicationHtml.includes("123456789012"),
	"tax application must render structured taxpayer INN",
);
assert(
	applicationHtml.includes("passport 3600 000000"),
	"tax application must render structured taxpayer identity document",
);
...
assert(
	taxDeductionApplicationPayloadSchema.safeParse(validApplicationPayload)
		.success,
	"valid KND application payload must pass schema",
);
const validApplicationWithoutSelectedPaymentsPayload = {
	...validApplicationPayload,
	selectedPaymentIds: [],
};
assert(
	taxDeductionApplicationPayloadSchema.safeParse(
		validApplicationWithoutSelectedPaymentsPayload,
	).success,
...
assert(
	taxDeductionApplicationPayloadSchema.safeParse({
		...
		applicantAuthorityDocument:
			"доверенность или документ супруга, проверен администратором",
	}).success,
	"tax application for representative with authority document must pass",
);

console.log(
	JSON.stringify({ ok: true, includedReceipts: 2, excludedOldYear: true }),
);
