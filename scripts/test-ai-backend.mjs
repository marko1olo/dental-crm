...
const testTranscript =
	"пациент жалуется на острую боль в нижнем левом зубе три шесть при накусывании. объективно глубокий кариес тридцать шестого, перкуссия резко болезненна. сделаем снимок. диагноз пульпит тридцать шестого. в плане депульпирование, пломбирование каналов.";
...
async function runTest() {
	...
	try {
		...
		console.log(
			"Результат полировки речи Gemini:",
			JSON.stringify(polishResult, null, 2),
		);

		console.log("Вызов buildVisitDraftFromTranscript через Gemini...");
		const draftResult = await buildVisitDraftFromTranscript(
			testTranscript,
			"therapist",
		);
		console.log(
			"Результат генерации черновика ЭМК Gemini:",
	...
	try {
		...
		console.log(
			"Результат генерации черновика ЭМК Groq:",
			JSON.stringify(draftResult, null, 2),
		);
	} catch (err) {
