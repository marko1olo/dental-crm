...
function voidAttestation(overrides = {}) {
	return {
		...
		reasonText:
			"Smoke tax certificate correction after annual duplicate block.",
...
/*
 * ТОКЕН КАБИНЕТА ОБЯЗАТЕЛЕН, ИНАЧЕ ПРОВЕРЯЕТСЯ НЕ ПОВТОРНАЯ ВЫДАЧА СПРАВКИ, А ВХОД.
...
const smokeAuth = await createSmokeClinicAuth({
	fallbackSecret: "dente_tax_certificate_duplicate_smoke_secret",
...
try {
	...
	assert(
		taxPayment,
		"fixture must contain a tax payment for the active patient",
	);
	...
	assert(
		firstCreateResponse.statusCode === 201,
		`first tax certificate create failed: ${firstCreateResponse.statusCode}`,
	);
	...
	const applicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				...
				note: "duplicate smoke application",
			},
		}),
	});
	assert(
	...
	assert(
		firstIssueResponse.statusCode === 200,
		`first tax certificate issue failed: ${firstIssueResponse.statusCode} ${firstIssueResponse.body}`,
	);
	assert(
		firstIssueResponse.json().status === "issued",
		"first certificate must be issued",
	);
	const firstIssuedDocument = documents.find(
		(document) => document.id === firstCertificate.id,
	);
	assert(
		firstIssuedDocument?.taxPaymentSnapshot?.paymentIds.includes(taxPayment.id),
		"issued certificate must freeze covered payment ids",
	);
	...
	assert(
		secondCreateResponse.statusCode === 201,
		`second tax certificate create failed: ${secondCreateResponse.statusCode}`,
	);
	...
	assert(
		duplicateIssueResponse.statusCode === 409,
		`duplicate certificate issue must be blocked, got ${duplicateIssueResponse.statusCode}`,
	);
	assert(
	...
	assert(
		mutatedDuplicateCreateResponse.statusCode === 201,
		`mutated duplicate certificate create failed: ${mutatedDuplicateCreateResponse.statusCode}`,
	);
	...
	const newPaymentApplicationIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: newPayment.payerFullName,
				note: "new payment application",
			},
		}),
	});
	assert(
	...
	assert(
		thirdCreateResponse.statusCode === 201,
		`third tax certificate create failed: ${thirdCreateResponse.statusCode}`,
	);
	...
	const thirdDocument = documents.find(
		(document) => document.id === thirdCreateResponse.json().id,
	);
	assert(
		thirdDocument?.status !== "issued",
		"same-year same-taxpayer certificate must remain unissued",
	);
	...
	assert(
		voidFirstCertificateResponse.json().voidAttestation?.reasonCode ===
			"tax_certificate_correction",
		"voided tax certificate must keep structured correction attestation",
	);
	const voidedFirstDocument = documents.find(
		(document) => document.id === firstCertificate.id,
	);
	assert(
		voidedFirstDocument?.status === "voided",
		"first certificate must be voided before replacement issue",
	);
	assert(
	...
	const replacementIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: newPayment.payerFullName,
				note: "replacement after structured void",
			},
		}),
	});
	assert(
	...
	const replacementIssuedDocument = documents.find(
		(document) => document.id === replacementCreateResponse.json().id,
	);
	assert(
		replacementIssuedDocument?.status === "issued",
		"replacement certificate must be issued after structured void",
	);
	assert(
		replacementIssuedDocument?.taxPaymentSnapshot?.paymentIds.includes(
			newPayment.id,
		),
	...
	const spouseIssuedDocument = documents.find(
		(document) => document.id === spouseCertificateCreateResponse.json().id,
	);
	assert(
		spouseIssuedDocument?.taxPaymentSnapshot?.paymentIds.includes(
			spousePayment.id,
		),
		"spouse certificate must freeze spouse payment id",
	);
	assert(
		!spouseIssuedDocument?.taxPaymentSnapshot?.paymentIds.includes(
			taxPayment.id,
	...
	assert(
		spouseIssuedDocument?.totalAmountRub === spousePayment.amountRub,
		"spouse certificate amount must equal spouse payment",
	);
	...
	assert(
		registryCreateResponse.statusCode === 201,
		`tax registry create failed: ${registryCreateResponse.statusCode}`,
	);
	const registryIssueResponse = await app.inject({
		...
		payload: issueAttestation({
			signatureAttestation: {
				recipientFullName: taxPayment.payerFullName,
				note: "registry still allowed",
			},
		}),
	});
	assert(
		registryIssueResponse.statusCode === 200,
		`tax registry must not be blocked by issued certificate: ${registryIssueResponse.statusCode}`,
	);
