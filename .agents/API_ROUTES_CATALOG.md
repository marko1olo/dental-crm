...
В соответствии с Конституцией (**Раздел VII, п. 4**, **Мандаты 8e, 8n, 8v**) и требованиями СанПиН 3.3686-21:
* `POST /api/warehouse/:organizationId/quick-carpule-disposal` — **1-клик списание пустых карпул и отходов Класса Б врачом или администратором** (Мандат 8v: 90% пользователей — Врач и Администратор, медсестры не тыкают CRM) единолично без созыва комиссии из 3 человек. Поддерживает фоновый режим `executeSeniorNurseDisposalActInBackground`, генерирует официальный акт утилизации медотходов класса Б (`АКТ-КП-ГГГГММДД-XXX`), обновляет остаток в `inventoryItems` и регистрирует транзакцию в `inventoryTransactions`.
...
### 2. Стерилизация, журналы ПСО и автоклавы (`apps/api/src/routes/sterilization.ts`, `sanpin.ts`)
В соответствии с **Мандатом 8v** (ликвидация «медсестринской шизы», медсестры заняты у кресла и в ЦСО, не сидят за компом):
* `POST /api/registers/autofill-shift` — **Автозаполнение санитарных журналов смены нормой СанПиН в 1 клик** (форма 257/у автоклава, журнал ПСО 366/у, азопирам, генерация регламентных бланков для проверок Роспотребнадзора без интерактивной рутины).
* `POST /api/sterilization/pso/quick-norm` — Фиксация азопирамовой и фенолфталеиновой пробы в 1 клик (реакция отрицательная, норма СанПиН).
* `POST /api/sterilization/daily-tests` — Ежедневные тесты автоклава Class B (Бови-Дик, вакуум-тест, индикаторы 4-5 класса).
* `POST /api/sterilization/link` — Сервисный эндпоинт маркировки партий стерилизации. **Врач в дневнике 043/у не сканирует крафт-пакеты:** стерильность инструментального лотка гарантирована по умолчанию санитарной нормой клиники (дефолтный стерильный лоток по СанПиН 3.3686-21).
...
> Склад материалов, 1-клик списание карпул и медотходов Класса Б врачом или администратором (СанПиН 3.3686-21, Мандаты 8e, 8n, 8v), мягкий овердрафт при задержке накладных, автоматические журналы СанПиН в 1 клик (форма 257/у автоклава, ПСО 366/у, азопирам для проверок Роспотребнадзора), отходы Б/В, микроклимат и МДЛП Честный Знак. Полное искоренение «медсестринской шизы»: 90% пользователей — Врач и Администратор, медсестры не сидят за CRM, врач в 043/у не сканирует крафт-пакеты (стерильность лотка по умолчанию).
