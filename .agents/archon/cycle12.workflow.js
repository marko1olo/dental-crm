export const meta = {
  ...
  description: 'DENTE cycle 12: kopecks destroyed in the AI pricelist mode, a topbar that demotes the primary action to its own row, and eight invented prices in a mounted estimate',
  phases: [
    { title: 'Build', detail: 'the groq-mode rounding, the topbar overload, the fabricated prices and service ids' },
    { title: 'Attack', detail: 'a different agent tries to destroy each commit; a fabricated 0 for an unknown price is REVERT-grade' },
  ],
}
...
const PACKETS = [
  {
    ...
    files: 'apps/api/src/pricelist/analyzer.ts, its test files under apps/api/src/pricelist/, and packages/shared/src/migration.ts',
    brief: `
...
`,
  },
  {
    ...
    label: 'BB2 the topbar pushes «Запись» onto its own row behind three unlabelled icons',
    ...
    files: 'apps/web/src/workspaceShell.tsx, apps/web/src/styles/dente-redesign.css (the .topbar section around :604), apps/web/src/components/workspaceActions/** ONLY if the control genuinely belongs there, and the label dictionary in apps/web/src/lib/workspaceUiLabels.ts',
    brief: `
...
`,
  },
  {
    ...
    label: 'BB3 eight fabricated prices and eight fabricated service ids in a mounted estimate',
    ...
    files: 'apps/web/src/components/odontogram/TreatmentEstimator.tsx and a new test file beside it. You may read apps/web/src/components/plan/planPricing.ts and apps/api/src/db/pricelistQuery.ts but do NOT edit apps/web/src/components/settings/SettingsPricesTab.tsx (another agent has it dirty).',
    brief: `
