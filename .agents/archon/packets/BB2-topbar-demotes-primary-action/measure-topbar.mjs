...
const glyph = '<svg viewBox="0 0 24 24" width="24" height="24" aria-hidden="true"></svg>';
...
const probe = () => {
  ...
  for (const c of children.map(centre).sort((a, b) => a - b)) {
    if (!lines.length || Math.abs(c - lines[lines.length - 1]) > 18) lines.push(c);
  }
  const lineOf = (el) => {
    const c = centre(el);
    let best = 0;
  ...
  return {
    ...
    interactiveControls:
      children.filter((el) => !el.classList.contains("dnt-actions-mount")).length +
    ...
    bookIsAlone: book ? children.filter((el) => lineOf(el) === lineOf(book)).length === 1 : null,
