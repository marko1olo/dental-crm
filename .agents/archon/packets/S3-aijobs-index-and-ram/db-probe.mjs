...
try {
  if (mode === "inspect") {
    ...
    const targets = [
      ...
      { label: "physically LAST row (worst case for a Seq Scan)", order: "DESC" }
    ];
    ...
    console.log("--- EXPLAIN with index scans disabled in-session, physically LAST row ---");
    console.log(`predicate: organization_id = <org>, input_storage_path = ${row.input_storage_path}`);
    for (const line of explained.rows) console.log(line["QUERY PLAN"]);
  } else if (mode === "unique") {
    ...
    try {
      ...
      console.log("DUPLICATE (organization_id, input_storage_path) ACCEPTED — the index does NOT hold");
    } catch (error) {
      console.log(`DUPLICATE REFUSED: code=${error.code} constraint=${error.constraint}`);
    ...
    try {
      ...
      console.log("THREE ROWS WITH NULL input_storage_path IN ONE ORGANIZATION ACCEPTED — db/aiQuery.ts is unaffected");
    } catch (error) {
      console.log(`NULL-PATH ROWS REFUSED — this would break db/aiQuery.ts: ${error.message}`);
    }
    await client.query("ROLLBACK");
  } else if (mode === "ledger") {
    const indexes = await client.query(
