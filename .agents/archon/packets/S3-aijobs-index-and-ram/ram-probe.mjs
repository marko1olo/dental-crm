...
async function seed() {
	...
	try {
		...
		const filler = "протокол осмотра и лечения зуба "
			.repeat(Math.ceil(charsPerChunk / 32))
			.slice(0, charsPerChunk);
		for (let recording = 0; recording < recordingCount; recording += 1) {
			const organizationId =
				organizationIds[recording % organizationIds.length];
			const recordingId = `${PROBE_RECORDING_PREFIX}${recording}`;
			const chunks = [];
			for (
				let chunkIndex = 0;
				chunkIndex < chunksPerRecording;
				chunkIndex += 1
			) {
				chunks.push(
					buildChunk(organizationId, recordingId, chunkIndex, filler),
				);
			}
			await client.query(
...
async function cleanup() {
	...
	try {
		...
		console.log(
			`DELETED ${deleted.rowCount} ram-probe rows; ai_jobs rows now: ${total.rows[0].rows}`,
		);
	} finally {
		client.release();
		await pool.end();
...
async function measure() {
	...
	process.env.DENTAL_SPEECH_RESTORED_RECORDINGS_TOTAL = String(
		recordingCount * 4,
	);
	process.env.DENTAL_SPEECH_CACHED_RECORDINGS = String(recordingCount * 4);
	process.env.DENTAL_SPEECH_RESTORED_CHUNKS_TOTAL = String(
		recordingCount * chunksPerRecording * 4,
	);
	process.env.DENTAL_SPEECH_RESTORED_CHARS_TOTAL = String(
		recordingCount * chunksPerRecording * charsPerChunk * 4,
	);
	...
	console.log(
		`cold cache before measurement: cachedChunks=${coldState.cachedChunks} cachedChars=${coldState.cachedChars}`,
	);
	...
	console.log(
		`loadedRecordings=${state.loadedRecordings} skippedRecordings=${state.skippedRecordings}`,
	);
	console.log(
		`cachedChunks=${state.cachedChunks} cachedChars=${state.cachedChars}`,
	);
	console.log(
		`heapUsed before=${before.heapUsed} after=${after.heapUsed} delta=${heapDelta} bytes (${(heapDelta / 1024 / 1024).toFixed(2)} MB)`,
	);
	if (state.cachedChars > 0) {
		console.log(
			`bytes per cached character: ${(heapDelta / state.cachedChars).toFixed(3)}`,
		);
	}
	if (state.cachedChunks > 0) {
		console.log(
			`bytes per cached chunk: ${(heapDelta / state.cachedChunks).toFixed(1)}`,
		);
		const chunkOverhead =
			(heapDelta - state.cachedChars * 2) / state.cachedChunks;
		console.log(
			`bytes per chunk OBJECT with a two-byte-per-char string assumption: ${chunkOverhead.toFixed(1)}`,
		);
	}
	await pool.end();
}
