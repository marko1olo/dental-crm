...
function unwrap(schema, depth = 0) {
  ...
  if (t === "ZodOptional" || t === "ZodNullable" || t === "ZodDefault" || t === "ZodCatch" || t === "ZodBranded" || t === "ZodReadonly") {
    return unwrap(def.innerType, depth + 1);
  }
  if (t === "ZodEffects") return unwrap(def.schema, depth + 1);
  if (t === "ZodLazy") {
    try { return unwrap(def.getter(), depth + 1); } catch { return null; }
  }
  return schema;
}
...
function collectNumberLeaves(schema, path, out, seen, depth = 0) {
  ...
  if (t === "ZodArray") { collectNumberLeaves(inner._def.type, `${path}[]`, out, seen, depth + 1); return; }
  if (t === "ZodRecord") { collectNumberLeaves(inner._def.valueType, `${path}{}`, out, seen, depth + 1); return; }
  if (t === "ZodTuple") {
    (inner._def.items ?? []).forEach((item, i) => collectNumberLeaves(item, `${path}[${i}]`, out, seen, depth + 1));
    return;
  }
  if (t === "ZodUnion" || t === "ZodDiscriminatedUnion") {
    const opts = inner._def.options ? (Array.isArray(inner._def.options) ? inner._def.options : Array.from(inner._def.options.values())) : [];
    opts.forEach((opt, i) => collectNumberLeaves(opt, `${path}|${i}`, out, seen, depth + 1));
    return;
  }
...
function behaviour(mod) {
  ...
  for (const [name, value] of Object.entries(mod)) {
    if (!value || typeof value !== "object" || !value._def || typeof value.safeParse !== "function") continue;
...
console.log("number leaves reached: HEAD", headMap.size, "PARENT", parentMap.size);

const isMoneyName = (p) => /rub(\b|$|[.[|{&])/i.test(p) || /Rub[)\]]?$/.test(p) || /Rub\./.test(p);
...
for (const [path, head] of headMap) {
  ...
  if (!parent) { onlyHead.push(path); continue; }
...
for (const p of widened) console.log("  ", p, /rub/i.test(p) ? "" : "  <<< NOT MONEY-NAMED");
console.log("NON-MONEY-NAMED widened (mass-conversion evidence if > 0):", nonMoneyWidened.length, nonMoneyWidened);
...
const stillInt = [...headMap.entries()].filter(([p, r]) => /rub/i.test(p) && !r.kopecks).map(([p]) => p);
console.log(stillInt.length, stillInt);

console.log("\n=== MONEY-NAMED leaves at HEAD that ACCEPT 1500.505 (no kopeck precision) ===");
const loose = [...headMap.entries()].filter(([p, r]) => /rub/i.test(p) && r.thirdKopeck).map(([p]) => p);
console.log(loose.length);
for (const p of loose) console.log("  ", p);

console.log("\n=== MONEY-NAMED leaves at HEAD that ACCEPT -5000 ===");
const neg = [...headMap.entries()].filter(([p, r]) => /rub/i.test(p) && r.negative).map(([p]) => p);
...
const badCounters = [...headMap.entries()]
  .filter(([p, r]) => /(count|quantity|version|items|documents|minutes|months|year|number|tooth)/i.test(p) && !/rub/i.test(p) && r.halfCount)
  .map(([p]) => p);
...
const revertTargets = [
  "billingSummarySchema.totalPaidRub", "billingSummarySchema.totalDueRub",
  "createDocumentSchema.totalAmountRub", "patientSchema.balanceRub",
  "treatmentPlanItemSchema.unitPriceRub", "paymentReceiptPayloadSchema.totalPaidRub",
  "dentalPricelistCategorySummarySchema.averagePriceRub",
  "billingSummarySchema.openTreatmentItems", "billingSummarySchema.unpaidDocuments"
];
for (const p of revertTargets) {
  const h = headMap.get(p); const par = parentMap.get(p);
  console.log(`  ${p}: PARENT accepts 1500.50 = ${par?.kopecks}, HEAD accepts 1500.50 = ${h?.kopecks}`);
}
