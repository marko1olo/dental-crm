...
function oldExtractPrice(line) {
	const withoutServiceCodes = line.replace(
	...
	for (const match of withoutServiceCodes.matchAll(priceRegex)) {
		...
		if (priceRub !== null) {
			candidates.push({
				priceRub,
				priceMaxRub:
					priceMaxRub !== null && priceMaxRub >= priceRub ? priceMaxRub : null,
...
function oldStripPriceFromTitle(line) {
	return line
		...
		.replace(
			/(?:от\s*)?\d{1,3}(?:[\s.]\d{3})+\s*(?:-|до)?\s*\d{0,3}(?:[\s.]\d{3})?\s*(?:₽|руб\.?|р\.?)?/giu,
			" ",
		)
...
const { dentalPricelistAnalysisRequestSchema } = await import(
...
for (const item of res.items) {
	...
	const priceChanged =
		old.priceRub !== item.priceRub || old.priceMaxRub !== item.priceMaxRub;
...
for (const s of res.summary) {
	console.log(
		...
		s.averagePriceRub,
		s.minPriceRub !== null &&
			s.averagePriceRub !== null &&
			(s.averagePriceRub < s.minPriceRub || s.averagePriceRub > s.maxPriceRub)
			? "*** AVG OUT OF RANGE ***"
			: "",
	);
}
