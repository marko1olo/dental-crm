...
const notNumeric2 = cols.rows.filter((r) => r.data_type !== "numeric" || r.numeric_scale !== 2);
console.log("columns NOT numeric(x,2):", notNumeric2.length, notNumeric2.map((r) => `${r.table_name}.${r.column_name}:${r.data_type}`));
...
const tables = await client.query(`
  ...
  order by table_name
`);
console.log("\ntable existence:", tables.rows.map((r) => r.table_name));
...
for (const r of spot.rows) console.log(`   ${r.table_name}.${r.column_name} = ${r.data_type}(${r.numeric_precision},${r.numeric_scale})`);

const orgs = await client.query(`select id, name, clinic_mode from organizations order by name`);
console.log("\norganizations:", orgs.rowCount);
for (const r of orgs.rows) console.log(`   ${r.id}  ${r.name}  clinic_mode=${r.clinic_mode}`);

// Are there any REAL kopeck values in money columns today? Split by organization_id where present.
const kop = await client.query(`
...
console.log("\npayments carrying kopecks, by organization_id:", kop.rows.length ? kop.rows : kop.err);
