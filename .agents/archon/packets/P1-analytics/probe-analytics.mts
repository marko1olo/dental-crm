...
async function main() {
	...
	for (const row of paid.rows) {
		console.log(
			`  org=${row.organization_id} paid_payments=${row.n} sum_rub=${row.total}`,
		);
	}
	...
	console.log(
		`ALL PAYMENTS BY STATUS: ${anyStatus.rows.map((r) => `${r.status}=${r.n}`).join(", ") || "(table empty)"}`,
	);
	...
	console.log(
		`payments.amount_rub column type: ${colType.rows.map((r) => r.data_type).join(", ")}`,
	);
	...
	for (const org of orgs.rows) {
		...
		const res = await fetch(url, {
			headers: { "x-dente-clinic-token": token },
		});
		...
		console.log(
			`BODY: ${raw.length > 1400 ? `${raw.slice(0, 1400)}\n...[truncated]` : raw}`,
		);
		...
		if (viaClient.ok) {
			...
			console.log(
				`  -> client parse FAILED with russian message: ${viaClient.message}`,
			);
		}
	}
