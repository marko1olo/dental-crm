...
for (const kind of kinds) {
  ...
  console.log(`\nCREATE ${kind}:`, created.status, JSON.stringify(created.json)?.slice(0, 220));
  if (created.status !== 201 || !created.json?.id) continue;

  const issued = await call("POST", `/api/documents/${created.json.id}/issue`, attestation);
  console.log(`ISSUE  ${kind}:`, issued.status);
  console.log("  body:", (issued.text || "").slice(0, 400));
  if (issued.status === 200) {
    console.log("  >>> issuedByUserId in RESPONSE:", JSON.stringify(issued.json?.issuedByUserId));
