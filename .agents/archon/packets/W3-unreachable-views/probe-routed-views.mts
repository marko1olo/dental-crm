...
const apiHost =
	readEnvValue(".env.local", "API_HOST") ??
	readEnvValue(".env", "API_HOST") ??
	"127.0.0.1";
const apiPort =
	readEnvValue(".env.local", "API_PORT") ??
	readEnvValue(".env", "API_PORT") ??
	"4100";
...
console.log(
	`DB rows: inventory_items=${inventoryCount.rows[0]?.count} sterilization_logs=${sterilizationCount.rows[0]?.count} crm_leads=${leadsCount.rows[0]?.count}`,
);
/*
...
console.log(
	staffUser
		? `staff user: ${staffUser.id}`
		: "staff user: НЕТ активного сотрудника",
);
await pool.end();
...
const staffToken = staffUser
	? signToken(
			{ organizationId: organization.id, userId: staffUser.id },
			authSecret,
			600,
		)
	: null;
	...
	try {
		...
		if (Array.isArray(parsed)) {
			...
			if (first && typeof first === "object")
				shape += ` keys=[${Object.keys(first).slice(0, 6).join(",")}]`;
