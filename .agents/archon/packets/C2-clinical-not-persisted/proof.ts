...
async function main() {
	...
	console.log(
		"  titleMatchesExpectedRussian:",
		created.title === EXPECTED_TITLE,
	);
	...
	const dbRow = (
		await db.execute(
			sql`SELECT * FROM clinical_tasks WHERE id = ${taskId}::uuid`,
		)
	...
	if (dbRow) {
		...
		console.log(
			"  title bytes(base64):",
			Buffer.from(String(dbRow.title), "utf8").toString("base64"),
		);
		console.log(
			"  titleMatchesExpectedRussian:",
			String(dbRow.title) === EXPECTED_TITLE,
		);
	}

	const getRes = await fetch(
		`${base}/api/clinical/tasks?patientId=${patientId}`,
		{ headers },
	);
	const list = (await getRes.json()) as Array<Record<string, unknown>>;
	console.log(
		"GET /api/clinical/tasks?patientId=... ->",
		getRes.status,
		"count =",
		list.length,
	);
	...
	console.log(
		"SECOND GET ->",
		getRes2.status,
		"still contains created task:",
		list2.some((t) => t.id === taskId),
	);
	...
	const left = (
		await db.execute(sql`SELECT count(*)::int AS n FROM clinical_tasks`)
	).rows[0] as { n: number };
