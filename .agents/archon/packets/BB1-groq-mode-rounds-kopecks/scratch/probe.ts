 ...
 */
import { analyzePricelist, itemFromGroq } from "../../../../../apps/api/src/pricelist/analyzer.js";
...
console.log("=== 1. TITLE: PARENT regex chain vs HEAD real analyzePricelist ===");
...
for (const v of [1500.5, "1500,50", "1500.50", 18000.25, 12000.1, 0, 99999, 45.7, -30, 600] as unknown[]) {
  console.log(`  parent(${JSON.stringify(v)}) = ${JSON.stringify(asNumberOrNullParent(v))}`);
}
for (const v of [false, [], {}, "бесплатно", null] as unknown[]) {
  console.log(`  parent(${JSON.stringify(v)}) = ${JSON.stringify(asNumberOrNullParent(v))}   <-- fabricated zero?`);
}

console.log("\n=== 4. HEAD itemFromGroq on the SAME inputs (post-fix) ===");
const aiReq: DentalPricelistAnalysisRequest = { ...req, rawText: "Лечение кариеса", useServerAi: true };
for (const rec of [
  { priceRub: 1500.5 },
  { priceRub: "1500,50" },
  { priceRub: 1500.505 },
  ...
  const it = itemFromGroq({ sourceText: "Лечение кариеса", ...rec }, 0, aiReq, CATALOG);
  console.log(
...
const { parseKopecks, sumKopecks, kopecksToNumericString } = await import("@dental/shared");
const parentAvg = (p: number[]) => Math.round((p.reduce((s, x) => s + x, 0) / p.length) * 100) / 100;
const headAvg = (p: number[]) =>
  Number(kopecksToNumericString(Math.round(sumKopecks(p.map((x) => parseKopecks(x))) / p.length)));
let diffs = 0;
let checked = 0;
const shown: string[] = [];
for (let seed = 1; seed <= 200000; seed++) {
  ...
  if (a !== b) {
    diffs++;
    if (shown.length < 6) shown.push(`prices=${JSON.stringify(prices)} parent=${a} head=${b} delta=${(b - a).toFixed(4)}`);
  }
}
console.log(`  checked=${checked} disagreements=${diffs}`);
for (const s of shown) console.log(`   ${s}`);
console.log(`  headline example [300.01,300.05,300.07]: parent=${parentAvg([300.01, 300.05, 300.07])} head=${headAvg([300.01, 300.05, 300.07])}`);
