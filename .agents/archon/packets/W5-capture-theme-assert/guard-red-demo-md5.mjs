...
/*
 * ============================================================================
 * ЭТО НЕ КОНВЕЙЕР. Черновая копия scripts/ops-panels-shots.mjs с подложенным
 * дефектом — чтобы показать, что ВТОРАЯ охрана (аудит побайтовых совпадений в
 * конце прогона) тоже краснеет.
 *
 * Подложено то, что случилось в жизни (VISUAL_VERDICT.md §0, A0): один и тот же
 * кадр лёг под десятью именами разделов и тем. Здесь тёмные плиты пишутся
 * байтами светлых. Тема при этом применена ВЕРНО, поэтому проверка перед
 * снимком молчит — падение должно прийти именно от аудита md5.
 *
 * Отличия от конвейера — только четыре: OUT, cdpPort, профиль браузера и блок
 * ПОДЛОГ в writeShot.
 * ============================================================================
 */
const OUT = path.join(process.env.TEMP || "C:/tmp", "w5-guard-demo-md5-shots");
...
const res = await fetch(webBaseUrl).catch((error) => {
	throw new Error(
		`Веб-сервер на ${webBaseUrl} недоступен (${error.message}). Запустите npm run dev.`,
	);
});
...
/** ПОДЛОГ (только в этой копии): байты первой светлой плиты каждой панели. */
const sabotage = new Map();

async function writeShot(fileName, base64, theme, themeState, note) {
	// ---------------------- ПОДЛОГ (только в этой копии) ----------------------
	let payload = base64;
	const slug = fileName.replace(/^(light|dark|night)_/, "");
	if (theme === "light") sabotage.set(slug, base64);
	else if (theme === "dark" && sabotage.has(slug)) payload = sabotage.get(slug);
	// --------------------------------------------------------------------------
	...
	console.log(
		`  ✓ ${fileName} (${note}, тема «${themeState?.dataTheme ?? "?"}», палитра ${themeState?.fingerprint ?? "?"})`,
	);
	return buffer;
}
...
async function shootPanel(testId, fileName, theme) {
  ...
  `);
	if (!box || box.width < 10 || box.height < 10) {
		...
		for (const error of pageErrors.slice(-3))
...
async function shootClipped(fileName, theme, clip) {
	...
	await writeShot(
		...
		theme,
		themeState,
		`высота ${clip.height}, масштаб ${clip.scale}`,
	);
}
...
const PANELS = [
	...
	{
		view: "communications",
		testId: "message-delivery-console",
		slug: "delivery",
	},
	...
	{
		view: "patients",
		testId: "patient-duplicate-merge-queues-widget",
		slug: "duplicates",
	},
	{
		view: "patients",
...
for (const theme of THEMES) {
	...
	for (const panel of PANELS) {
		...
		const ok = await shootPanel(
			panel.testId,
			`${theme}_${panel.slug}.png`,
			theme,
		);
...
await sleep(2200);
const financeHeight = await evaluate(
	"Math.min(document.body.scrollHeight, 9000)",
);
await shootClipped("finance_full.png", "light", {
	x: 0,
	y: 0,
	width: 1600,
...
const pageHeight = await evaluate(
	"Math.min(document.body.scrollHeight, 12000)",
);
await shootClipped("communications_full.png", "light", {
	x: 0,
	y: 0,
	width: 1600,
	height: pageHeight,
...
const manifest = {
	...
	out: OUT,
	palettes: [...paletteByThemeAndViewport].map(([key, fingerprint]) => ({
		key,
		fingerprint,
	})),
...
console.log(`\nСнимки: ${OUT}`);
console.log(
	`Плит записано: ${plates.length}, уникальных md5: ${byHash.size}. Диагностических «ПУСТО»: ${misses.length}${misses.length ? " — " + misses.map((entry) => entry.file).join(", ") : ""}`,
);
...
for (const panel of PANELS) {
	const row = THEMES.map((theme) => {
		const entry = plates.find(
			(item) => item.file === `${theme}_${panel.slug}.png`,
		);
...
console.log(
	`Происхождение каждой плиты: ${path.join(OUT, "theme-audit.json")}`,
);
...
if (collisions.length > 0) {
	for (const group of collisions) {
		console.error(
			`  побайтово одинаковы: ${group.map((entry) => `${entry.file} (тема ${entry.theme}, ${entry.viewport})`).join(", ")}`,
		);
	}
	throw new Error(
