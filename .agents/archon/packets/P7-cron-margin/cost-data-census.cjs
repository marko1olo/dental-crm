...
const PROBES = [
	[
		...
		"actual material movements (unit_cost_rub, visit_id)",
	],
	[
		"inventory_transactions WHERE unit_cost_rub IS NOT NULL AND unit_cost_rub > 0",
		"  ...of those, with a real unit cost",
	],
	[
		"inventory_transactions WHERE visit_id IS NOT NULL",
		"  ...of those, attributable to a visit",
	],
	["inventory_items", "material catalogue"],
	[
		"inventory_items WHERE unit_cost_rub IS NOT NULL AND unit_cost_rub > 0",
		"  ...of those, priced",
	],
...
(async () => {
	...
	for (const [expr, label] of PROBES) {
		try {
			...
			console.log(
				String(r.rows[0].n).padStart(8),
				" ",
				expr.split(" WHERE ")[0].padEnd(28),
				label,
			);
		} catch (e) {
