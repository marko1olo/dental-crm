...
(async () => {
	...
	for (const [t, c] of COLUMN_PROBES) {
		...
		const verdict = r.rowCount
			? "EXISTS  " + r.rows[0].data_type
			: "*** MISSING ***";
	...
	for (const q of [
		...
		[
			...
			"SELECT count(*)::int n FROM appointments WHERE status='completed'",
		],
		[
			"appointments with doctor",
			"SELECT count(*)::int n FROM appointments WHERE doctor_user_id IS NOT NULL",
		],
