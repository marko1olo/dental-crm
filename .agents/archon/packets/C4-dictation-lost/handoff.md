...
4. **После вытеснения из кэша** (более 80 записей диктовки) фрагменты перестают отдаваться
   через `GET /api/speech/chunks` до следующего перезапуска, хотя лежат в `ai_jobs`. Текст не
   уничтожен, но временно не виден. Ленивую подгрузку нельзя добавить, не сделав роутовые
   функции асинхронными, то есть не тронув `routes/speech.ts` (вне делянки).
