...
for (const r of empty) {
	...
	if (r.price !== null && INVENTED.includes(r.price as number))
		bad.push("INVENTED PRICE");
	if (r.priceId && IDS.includes(r.priceId)) bad.push("INVENTED ID");
	console.log(
		`  tooth ${r.toothNumber} «${r.name}» price=${String(r.price)} priceId=${String(r.priceId)} issue=${r.issue?.kind ?? "-"} ${bad.length ? "*** " + bad.join(",") : "OK"}`,
	);
	if (estimatorItemForApi(r) !== null)
		console.log("  *** LEAK: row reached API body");
}
const tA = estimatorTotals(empty, null);
show("  totals:", tA);
console.log(
	"  saveBlock message:",
	estimatorSaveBlock(empty)?.message ?? "NULL (save allowed!)",
);
...
const filled = reconcileAutoSuggestions(
	[],
	[{ toothNumber: 11, state: "Caries" }],
	cat,
).items;
...
const offRows = reconcileAutoSuggestions(
	[],
	[{ toothNumber: 11, state: "Caries" }],
	off,
).items;
...
show("  row:", {
	price: offRows[0]?.price,
	priceId: offRows[0]?.priceId,
	issue: offRows[0]?.issue?.kind,
});
...
const amb = [
	...
	{
		id: "svc-caries2",
		title: "Лечение кариеса глубокого",
		category: "therapy",
		basePriceRub: 4100,
		active: true,
	},
];
const ambRows = reconcileAutoSuggestions(
	[],
	[{ toothNumber: 11, state: "Caries" }],
	amb,
).items;
...
show("  row:", {
	name: ambRows[0]?.name,
	price: ambRows[0]?.price,
	priceId: ambRows[0]?.priceId,
});
...
const surg = [
	{
		id: "svc-ext",
		title: "Удаление зуба хирургическое",
		category: "surgery",
		basePriceRub: 3500,
		active: true,
	},
];
const surgRows = reconcileAutoSuggestions(
	[],
	[{ toothNumber: 16, state: "Planned_Implant" }],
	surg,
).items;
...
for (const r of surgRows)
	show("  row:", { name: r.name, price: r.price, priceId: r.priceId });

/* F: float-money proof with a DIFFERENT method than their suite. */
const three = [300.01, 300.05, 300.07].map(
	(p, i) =>
		reconcileAutoSuggestions(
			[],
			[{ toothNumber: 11 + i, state: "Caries" }],
			[
				{
					id: "s" + i,
					title: "Лечение кариеса",
