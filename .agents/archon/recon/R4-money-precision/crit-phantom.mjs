...
const url = readFileSync(R+".env","utf8").split(/\r?\n/).find(l=>l.startsWith("DATABASE_URL=")).slice(13).trim();
const files = ["apps/api/src/db/schema.ts","apps/api/src/db/communicationsSchema.ts","apps/api/src/db/patientsSchema.ts"];
...
for (const f of files) {
  ...
  lines.forEach((l,i)=>{
    ...
    if (m && tbl) decls.push({ key: `${tbl}.${m[1]}`, at: `${f.split("/").pop()}:${i+1}`, mode: /mode:\s*"number"/.test(l)?"number":"string" });
  });
}
const pool = new pg.Pool({ connectionString: url, max: 1 });
const c = await pool.connect();
await c.query("SET default_transaction_read_only = on");
const live = new Map((await c.query(
...
const allLive = new Map((await c.query(
...
console.log(`declared numeric() (table-keyed) = ${decls.length}; live numeric columns = ${live.size}`);
const phantoms = decls.filter(d=>!allLive.has(d.key));
const typeMismatch = decls.filter(d=>allLive.has(d.key) && allLive.get(d.key)!=="numeric");
...
typeMismatch.forEach(d=>console.log(`  ${d.key}  @${d.at}  live=${allLive.get(d.key)}`));
...
orphans.forEach(k=>console.log(`  ${k}  numeric(${live.get(k).p},${live.get(k).s})`));
