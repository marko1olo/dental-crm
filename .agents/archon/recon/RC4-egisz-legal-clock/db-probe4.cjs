...
const Q=[
 ['generated_documents_columns',"select column_name,data_type from information_schema.columns where table_schema='public' and table_name='generated_documents' order by ordinal_position"],
 ['visits_total_and_orgs',"select organization_id,count(*) from visits group by 1"],
 ['tooth_states_by_org',"select organization_id,count(*) from tooth_states group by 1"],
];
