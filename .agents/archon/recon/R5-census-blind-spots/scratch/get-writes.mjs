	...
	const visit = (n) => {
		if (ts.isCallExpression(n) && ts.isPropertyAccessExpression(n.expression)) {
			...
			if (WRITE_METHODS.has(m) && n.arguments.length > 0) {
				...
				if (/^(schema\.)?[a-z][A-Za-z0-9]*$/.test(argText))
					hits.push(`${m}(${argText}) @${lineOf(sf, n)}`);
			}
		}
	...
	const visit = (n) => {
		...
		if (name && body) {
			...
			if (hits.length > 0) {
				...
				writerFns
					.get(name)
					.push({ file: rel(f), line: lineOf(sf, n), detail: hits.join("; ") });
			}
		}
		ts.forEachChild(n, visit);
	};
	...
	const visit = (n) => {
		if (
			...
			if (routePath.startsWith("/")) {
				...
				const viaWriters = [...calls].filter(
					(c) => writerFns.has(c) && !WRITE_METHODS.has(c),
				);
				routes.push({
					method,
					routePath,
...
const t2 = routes.filter(
	(r) => r.direct.length === 0 && r.viaWriters.length > 0,
);
console.log(`files parsed: ${files.length}`);
console.log(`GET/HEAD registrations with a string path: ${routes.length}`);
console.log(
	`named functions in apps/api/src whose own body writes the DB: ${writerFns.size}`,
);
console.log(
...
for (const r of t2) {
	...
	for (const w of r.viaWriters)
		for (const site of writerFns.get(w))
			console.log(
				`      VIA ${w}  ${site.file}:${site.line}  [${site.detail}]`,
			);
}
