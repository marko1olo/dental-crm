...
const OUT =
	process.argv[3] ?? path.join(ROOT, ".agents/archon/recon/R3-i18n-route");
...
function areaOf(r) {
	...
	if (
		...
		return "tests";
	if (/^apps\/api\/(drizzle|migrations)\//.test(r) || /\.sql$/.test(r))
		return "migrations";
...
for (const f of files) {
	...
	if (CODE_EXT.has(ext)) {
		...
		row.buckets = counts;
		row.translatableLiterals = literals.filter(
			(l) => l.bucket !== "comment",
		).length;
...
console.log(
	"FILES SCANNED (code+texty, excl node_modules/dist/.git): " + report.scanned,
);
console.log("");
console.log("== CYRILLIC-BEARING FILES AND LINES BY AREA ==");
console.log(
	pad("area", 14) + num("files", 7) + num("cyrLines", 10) + num("cyrChars", 10),
);
const areas = Object.entries(report.byArea).sort(
	(a, b) => b[1].cyrLines - a[1].cyrLines,
);
let tf = 0,
	tl = 0,
	tc = 0;
for (const [a, v] of areas) {
	console.log(
		pad(a, 14) + num(v.files, 7) + num(v.cyrLines, 10) + num(v.cyrChars, 10),
	);
	tf += v.files;
	tl += v.cyrLines;
	tc += v.cyrChars;
...
console.log("");
console.log(
	"== CODE FILES ONLY: WHERE THE CYRILLIC CHARACTERS ACTUALLY LIVE (chars) ==",
);
console.log(
	pad("area", 14) +
		num("comment", 9) +
...
console.log("");
console.log(
	"== CODE FILES ONLY: LINES BY SOLE BUCKET (mixed = line has 2+ kinds) ==",
);
console.log(
	pad("area", 14) +
		num("cmtOnly", 9) +
...
console.log("");
const ship = report.literals.filter(
	(l) =>
		["web-src", "api-src", "shared-src"].includes(l.area) &&
		l.bucket !== "comment",
);
const uniq = new Set(ship.map((l) => l.text.trim()));
console.log(
	"== TRANSLATION UNITS IN SHIPPING CODE (web-src + api-src + shared-src) ==",
);
console.log("literal occurrences : " + ship.length);
console.log("distinct strings    : " + uniq.size);
console.log(
	"interpolated tpl    : " + ship.filter((l) => l.interpolated).length,
);
console.log(
	"by bucket           : " +
		JSON.stringify(
			ship.reduce((a, l) => {
				a[l.bucket] = (a[l.bucket] || 0) + 1;
				return a;
			}, {}),
		),
);
