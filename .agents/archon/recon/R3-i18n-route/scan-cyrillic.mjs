...
const SKIP_DIRS = new Set([
  ...
  ".next", "out", ".turbo", "playwright-report", "test-results",
]);
const EXT = new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".css", ".html", ".sql", ".json"]);
...
function classify(src, ext) {
  ...
  while (i < n) {
    ...
    if (mode === "code") {
      if (!isJson && c === "/" && c2 === "/") { mode = "line"; kind[i] = COM; kind[i + 1] = COM; i += 2; continue; }
      if (!isJson && c === "/" && c2 === "*") { mode = "block"; kind[i] = COM; kind[i + 1] = COM; i += 2; continue; }
      if (isSql && c === "-" && c2 === "-") { mode = "line"; kind[i] = COM; kind[i + 1] = COM; i += 2; continue; }
      if (c === '"' || c === "'" || c === "`") { mode = "str"; quote = c; kind[i] = STR; i++; continue; }
      if (c === "{" && tmplStack.length) tmplStack[tmplStack.length - 1]++;
      if (c === "}" && tmplStack.length) {
        tmplStack[tmplStack.length - 1]--;
        if (tmplStack[tmplStack.length - 1] < 0) { tmplStack.pop(); mode = "str"; quote = "`"; kind[i] = STR; i++; continue; }
      }
      kind[i] = CODE; i++; continue;
    }
    if (mode === "line") {
      if (c === "\n") { mode = "code"; kind[i] = CODE; i++; continue; }
      kind[i] = COM; i++; continue;
    }
    if (mode === "block") {
      if (c === "*" && c2 === "/") { kind[i] = COM; kind[i + 1] = COM; mode = "code"; i += 2; continue; }
      kind[i] = COM; i++; continue;
    }
    if (mode === "str") {
      if (c === "\\") { kind[i] = STR; if (i + 1 < n) kind[i + 1] = STR; i += 2; continue; }
      if (quote === "`" && c === "$" && c2 === "{") { kind[i] = STR; kind[i + 1] = STR; tmplStack.push(0); mode = "code"; i += 2; continue; }
      if (c === quote) { kind[i] = STR; mode = "code"; i++; continue; }
      if (quote !== "`" && c === "\n") { mode = "code"; kind[i] = CODE; i++; continue; } // unterminated guard
      kind[i] = STR; i++; continue;
    }
  }
  return kind;
}
...
const stats = {
  ...
  cyrLinesMixed: 0,
  cyrCharsComment: 0, cyrCharsString: 0, cyrCharsBare: 0,
};
const perFile = [];
...
function addTop(file, key, delta) {
  ...
  if (!perTopDir.has(top)) perTopDir.set(top, { cyrLines: 0, comment: 0, string: 0, bare: 0, mixed: 0, files: 0 });
  const o = perTopDir.get(top); o[key] += delta; return o;
}
...
for (const f of files) {
  ...
  const fileRec = { file: path.relative(ROOT, f).replace(/\\/g, "/"), cyrLines: 0, comment: 0, string: 0, bare: 0, mixed: 0, charsComment: 0, charsString: 0, charsBare: 0, lines: lines.length };
  let lineNo = 1, lineHas = { c: 0, s: 0, b: 0 };
  const flush = () => {
    const tot = lineHas.c + lineHas.s + lineHas.b;
    if (tot > 0) {
      stats.cyrLines++; fileRec.cyrLines++;
      const nz = (lineHas.c > 0 ? 1 : 0) + (lineHas.s > 0 ? 1 : 0) + (lineHas.b > 0 ? 1 : 0);
      if (nz > 1) { stats.cyrLinesMixed++; fileRec.mixed++; }
      else if (lineHas.c) { stats.cyrLinesCommentOnly++; fileRec.comment++; }
      else if (lineHas.s) { stats.cyrLinesStringOnly++; fileRec.string++; }
      else { stats.cyrLinesBareOnly++; fileRec.bare++; }
    }
    lineHas = { c: 0, s: 0, b: 0 };
  };
  for (let i = 0; i < src.length; i++) {
    const ch = src[i];
    if (ch === "\n") { flush(); lineNo++; continue; }
    if (!CYR.test(ch)) continue;
    const k = kind[i];
    if (k === 2) { lineHas.c++; stats.cyrCharsComment++; fileRec.charsComment++; }
    else if (k === 1) { lineHas.s++; stats.cyrCharsString++; fileRec.charsString++; }
    else { lineHas.b++; stats.cyrCharsBare++; fileRec.charsBare++; }
  }
  flush();
  perFile.push(fileRec);
  const o = addTop(f, "cyrLines", fileRec.cyrLines);
  o.comment += fileRec.comment; o.string += fileRec.string; o.bare += fileRec.bare; o.mixed += fileRec.mixed; o.files += 1;
  ...
  if (ext === ".ts" || ext === ".tsx" || ext === ".js" || ext === ".jsx" || ext === ".mjs" || ext === ".cjs") {
    ...
    while (i2 < src.length) {
      if (kind[i2] === 1) {
        let j = i2; while (j < src.length && kind[j] === 1) j++;
        const raw = src.slice(i2, j);
        if (CYR.test(raw)) {
          const norm = raw.replace(/^["'`]|["'`]$/g, "").replace(/\s+/g, " ").trim();
          if (norm) {
            const line = src.slice(0, i2).split("\n").length;
            if (!stringLiterals.has(norm)) stringLiterals.set(norm, { count: 0, at: `${fileRec.file}:${line}`, len: norm.length, files: new Set() });
    ...
    while (i3 < src.length) {
      if (kind[i3] === 0 && CYR.test(src[i3])) {
        let a = i3; while (a > 0 && kind[a - 1] === 0 && src[a - 1] !== ">" && src[a - 1] !== "\n" && src[a - 1] !== "{" && src[a - 1] !== "}") a--;
        let b = i3; while (b < src.length && kind[b] === 0 && src[b] !== "<" && src[b] !== "\n" && src[b] !== "{" && src[b] !== "}") b++;
        const norm = src.slice(a, b).replace(/\s+/g, " ").trim();
        if (norm && CYR.test(norm)) {
          const line = src.slice(0, i3).split("\n").length;
          if (!jsxTextRuns.has(norm)) jsxTextRuns.set(norm, { count: 0, at: `${fileRec.file}:${line}`, len: norm.length, files: new Set() });
...
perFile.sort((a, b) => (b.string + b.bare + b.mixed) - (a.string + a.bare + a.mixed));

const distinctStrings = [...stringLiterals.entries()].map(([t, v]) => ({ t, count: v.count, at: v.at, len: v.len }));
const distinctJsx = [...jsxTextRuns.entries()].map(([t, v]) => ({ t, count: v.count, at: v.at, len: v.len }));
const allUnits = new Map();
for (const s of distinctStrings) allUnits.set(s.t, (allUnits.get(s.t) || 0) + s.count);
for (const s of distinctJsx) allUnits.set(s.t, (allUnits.get(s.t) || 0) + s.count);
...
const out = {
  ...
  translatable: {
    ...
    totalStringLiteralOccurrences: distinctStrings.reduce((a, b) => a + b.count, 0),
  ...
  perTopDir: [...perTopDir.entries()].map(([k, v]) => ({ dir: k, ...v })).sort((a, b) => b.cyrLines - a.cyrLines),
  top40Files: perFile.slice(0, 40),
  longestUnits: [...allUnits.entries()].sort((a, b) => b[0].length - a[0].length).slice(0, 25).map(([t, c]) => ({ len: t.length, count: c, t: t.slice(0, 160) })),
  mostRepeatedUnits: [...allUnits.entries()].sort((a, b) => b[1] - a[1]).slice(0, 40).map(([t, c]) => ({ count: c, len: t.length, t: t.slice(0, 90) })),
};

// ---- role buckets: not all Russian is translatable UI ----
function role(rel) {
  if (/\/tests?\//.test(rel) || /\.test\.[cm]?[jt]sx?$/.test(rel) || /\.spec\./.test(rel)) return "test-fixture";
  if (/sampleData|seed|fixtures?\//i.test(rel)) return "seed-demo-data";
  if (/\/documents?\//.test(rel) || /renderDocument|documentValidators|documentTemplates|docTemplates/i.test(rel)) return "legal-document-template";
  if (/\/migration\/|vendorProfiles|smartImports|LegacyMigration|SmartImport|SettingsImports/i.test(rel)) return "competitor-import-pattern";
  ...
  if (/UiLabels|UiMeta|StaticOptions|Catalog|catalog/.test(rel)) return "web-ui-dictionary";
...
for (const r of perFile) {
  const b = role(r.file);
  if (!buckets.has(b)) buckets.set(b, { bucket: b, files: 0, translatableLines: 0, commentLines: 0, charsTranslatable: 0 });
  const o = buckets.get(b);
  o.files++; o.translatableLines += r.string + r.bare + r.mixed; o.commentLines += r.comment;
...
const USER_FACING = new Set(["web-ui-surface", "web-ui-dictionary", "shared-enum-dictionary", "api-route-message"]);
...
out.userFacingUnitStats = {
  ...
  medianChars: ufArr.map((s) => s.length).sort((a, b) => a - b)[Math.floor(ufArr.length / 2)] ?? 0,
  under30chars: ufArr.filter((s) => s.length < 30).length,
  over120chars: ufArr.filter((s) => s.length > 120).length,
};
out.roleBuckets = [...buckets.values()].sort((a, b) => b.translatableLines - a.translatableLines);
fs.writeFileSync(path.join(ROOT, ".agents/archon/recon/R3-i18n-route/perfile-full.json"),
  JSON.stringify(perFile.map((r) => ({ ...r, role: role(r.file) })), null, 2), "utf8");

fs.writeFileSync(path.join(ROOT, ".agents/archon/recon/R3-i18n-route/cyrillic-census.json"), JSON.stringify(out, null, 2), "utf8");
...
console.log("wroteJson", ".agents/archon/recon/R3-i18n-route/cyrillic-census.json");
