...
for (const f of files) {
	...
	for (const nd of jsxTextNodes(src)) {
		...
		const residue = words.filter(
			(w) => !ALLOW.has(w) && !ALLOW.has(w.toUpperCase()),
		);
		if (residue.length === 0) continue;
		const key = nd.text;
		if (!leaks.has(key))
			leaks.set(key, { at: `${rel}:${nd.line}`, count: 0, residue });
		leaks.get(key).count++;
	}
}

const list = [...leaks.entries()]
	.map(([text, v]) => ({ text, ...v }))
	.sort((a, b) => b.text.length - a.text.length);

fs.writeFileSync(
	path.join(ROOT, ".agents/archon/recon/R3-i18n-route/english-leak.json"),
	JSON.stringify(
		{
			generatedAt: new Date().toISOString(),
			filesScanned: files.length,
			totalJsxTextNodes: totalNodes,
			cyrillicNodes: cyrNodes,
			latinOnlyNodes: latinNodes,
			distinctLeakCandidates: list.length,
			list,
		},
		null,
		2,
