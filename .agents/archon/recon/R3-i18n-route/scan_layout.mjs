...
function walk(d, acc) {
  ...
  for (const x of e) {
    ...
    if (x.isDirectory()) { if (!SKIP.has(x.name)) walk(p, acc); }
...
for (const f of files) {
  ...
  while ((m = re.exec(src))) {
    ...
    const fixedGrid = body.match(/grid-template-columns\s*:\s*([^;]*\d+px[^;]*)/);
    const minW = body.match(/min-width\s*:\s*(\d+)px/);
    const wrapOk = /flex-wrap\s*:\s*wrap|overflow-wrap|word-break/.test(body);
    const tags = [];
    let sev = 0;
    if (nowrap && hidden && !ellipsis) { tags.push("nowrap+hidden+NO-ellipsis=SILENT TRUNCATION"); sev = 3; }
    else if (nowrap && !wrapOk) { tags.push("nowrap (will overflow, not wrap)"); sev = Math.max(sev, 2); }
    if (fixedW && Number(fixedW[1]) >= 40 && /btn|button|chip|tab|badge|label|pill|action|nav|col|cell|head/i.test(sel)) {
      tags.push("fixed width:" + fixedW[1] + "px on a text element"); sev = Math.max(sev, 3);
    } else if (fixedW && Number(fixedW[1]) >= 60) { tags.push("fixed width:" + fixedW[1] + "px"); sev = Math.max(sev, 1); }
    if (fixedGrid) { tags.push("fixed px grid columns: " + fixedGrid[1].trim().slice(0, 70)); sev = Math.max(sev, 2); }
    if (minW && Number(minW[1]) >= 100 && nowrap) { tags.push("min-width " + minW[1] + "px + nowrap"); sev = Math.max(sev, 3); }
    if (tags.length) findings.push({ rel, line, sel: sel.slice(0, 110), tags, sev });
  }
}

findings.sort((a, b) => b.sev - a.sev || a.rel.localeCompare(b.rel) || a.line - b.line);
const bySev = findings.reduce((a, x) => { a[x.sev] = (a[x.sev] || 0) + 1; return a; }, {});
...
for (const x of findings.filter((y) => y.sev === 2).slice(0, 25)) {
  console.log("SEV2 " + x.rel + ":" + x.line + "  { " + x.sel + " }  " + x.tags.join(" | "));
}
fs.writeFileSync(path.join(ROOT, ".agents/archon/recon/R3-i18n-route/layout_risk.json"), JSON.stringify(findings, null, 1), "utf8");
