	...
	for (const st of sf.statements) {
		...
		if (clause.namedBindings && ts.isNamedImports(clause.namedBindings)) {
			for (const el of clause.namedBindings.elements)
				importName.set(el.name.text, resolved);
		}
	}
				...
				for (const prop of node.arguments[0].properties) {
					...
					if (prop.name.text === "method") {
						if (ts.isStringLiteralLike(prop.initializer))
							methods = [prop.initializer.text];
						else if (ts.isArrayLiteralExpression(prop.initializer))
							methods = prop.initializer.elements
...
function methodOf(arg) {
	...
	for (const p of arg.properties) {
		...
		if (ts.isShorthandPropertyAssignment(p) && p.name.text === "method")
			return "DYNAMIC";
		...
		if (
			...
			if (p !== null && p.startsWith("/")) {
				...
				const t = node.arguments[0]
					? node.arguments[0].getText(sf).slice(0, 90).replace(/\s+/g, " ")
					: "<none>";
				unresolved.push({
					kind: "computed",
					text: t,
...
function expand(routePath) {
	...
	if (segs.length && segs[segs.length - 1].endsWith("?"))
		out.push(segs.slice(0, -1).join("/"));
	return out;
}

const routeIndex = [];
for (const r of routes)
	for (const p of expand(r.path)) routeIndex.push({ ...r, matchPath: p });
...
for (const w of webFetches) {
	...
	const pathMatches = routeIndex.filter((r) =>
		segMatch(r.matchPath.split("/"), webSegs),
	);
	...
	results.push({
		...
		matchedRoute: methodMatches[0]
			? `${methodMatches[0].method} ${methodMatches[0].matchPath} (${methodMatches[0].file}:${methodMatches[0].line})`
			: null,
		verdict:
...
if (process.argv.includes("--json")) {
	console.log(
		JSON.stringify({ routes, webFetches: results, unresolved }, null, 2),
	);
	process.exit(0);
}
...
console.log(
	`  prefixed modules: ${[...prefixes].map(([f, p]) => `${rel(f)}=>${p}`).join(", ")}`,
);
console.log(`WEB fetch call sites with a literal /path: ${webFetches.length}`);
console.log(
	`WEB fetch call sites NOT statically resolvable: ${unresolved.length}`,
);
...
for (const r of bad.sort((a, b) => a.path.localeCompare(b.path))) {
	console.log(
		`${r.verdict}\t${r.method}\t${r.path}\t${r.file}:${r.line}\t${r.matchedMethods.join("|") || "-"}`,
	);
}
...
for (const u of unresolved)
	console.log(`${u.kind}\t${u.file}:${u.line}\t${u.text}`);
