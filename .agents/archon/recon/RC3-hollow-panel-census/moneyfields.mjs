...
const produced = new Set([
	...declaredNames(apiFiles),
	...declaredNames(sharedFiles),
]);
const webDeclared = declaredNames(webFiles);
const webAccessed = accessedNames(webFiles);

const MONEY =
	/(rub|kopeck|kopek|amount|price|total|sum|balance|cost|payout|debt|revenue|paid|discount|tariff|invoice|count|qty|quantity)/i;
...
for (const s of suspects)
	console.log(`${s.name}\t${s.sites.length}\t${s.sites.slice(0, 6).join(" ")}`);
