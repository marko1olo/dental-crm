...
function calleeNames(node) {
	...
	walk(node, (n) => {
		...
		const c =
			n.type === "CallExpression" ? n.callee : n.argument && n.argument.callee;
		if (!c) return;
...
for (const file of collectFiles(ROUTES, [])) {
	...
	walk(ast, (n) => {
		...
		const fnArgs = n.arguments.filter(
			(a) =>
				a &&
				(a.type === "ArrowFunctionExpression" ||
					a.type === "FunctionExpression"),
		);
		const objArgs = n.arguments.filter(
			...
			guards:
				Array.from(
					new Set(
						all.filter(
							(x) =>
								TENANT_VERIFIED.has(x) ||
								ADMIN_SECRET_ONLY.has(x) ||
								HAND_ROLLED.has(x) ||
								RAW_IDENTITY.has(x) ||
								OTHER_GUARD.has(x),
						),
