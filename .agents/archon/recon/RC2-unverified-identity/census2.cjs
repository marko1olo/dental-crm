...
const TENANT_VERIFIED = new Set(["requireOrganizationId","requireResolvedOrganizationId",
  "requireResolvedStaffOrAdminOrganizationId","requireClinicalReadContext",
  "requireClinicalMutationContext","requireStaffIdentity"]);
const ADMIN_SECRET_ONLY = new Set(["requireClinicalReadAccess","requireClinicalMutationAccess",
  "requireNonDoctorAccess","requireSettingsAccess","requireScheduleMutationAccess",
  "requireDicomWebSettingsAccess","requirePayoutAccess","requireTelegramControlPlaneAccess"]);
const HAND_ROLLED = new Set(["verifyToken"]);
const RAW_IDENTITY = new Set(["getRequestIdentity","resolveOrganizationId"]);
const OTHER_GUARD = new Set(["requirePermission","requireClinicToken","requireTls",
  "requirePatientOrVisit","requireWebhookAuth","verifyWebhookSignature","requireDistinguishingData",
  "assertWebhookSecret","verifyWebhookSecret"]);
const ALL_GUARDS = new Set([...TENANT_VERIFIED,...ADMIN_SECRET_ONLY,...HAND_ROLLED,...RAW_IDENTITY,...OTHER_GUARD]);
...
function isFn(n){ return n && (n.type==="ArrowFunctionExpression"||n.type==="FunctionExpression"||n.type==="FunctionDeclaration"); }

function localFunctionMap(ast) {
  const map = new Map();
  walk(ast, (n) => {
    if (n.type === "FunctionDeclaration" && n.id) map.set(n.id.name, n);
    if (n.type === "VariableDeclarator" && n.id && n.id.type === "Identifier" && isFn(n.init)) map.set(n.id.name, n.init);
  });
  return map;
}
...
function resolvedCalleeNames(node, locals, depth, seen) {
  ...
  walk(node, (n) => {
    ...
    else if (c.type === "MemberExpression" && c.property && c.property.type === "Identifier") direct.push(c.property.name);
  });
  for (const name of direct) {
    out.push(name);
    if (locals.has(name) && !seen.has(name)) {
      seen.add(name);
      out.push(...resolvedCalleeNames(locals.get(name), locals, depth + 1, seen));
    }
  }
  return out;
}
...
function isDenyAll(node) {
  ...
  let sends = 0, statuses = [];
  walk(node, (n) => {
    if (n.type !== "CallExpression") return;
    const c = n.callee;
    if (c && c.type === "MemberExpression" && c.property && (c.property.name === "status" || c.property.name === "code")) {
      const a = n.arguments[0];
      if (a && a.type === "NumericLiteral") statuses.push(a.value);
    }
    if (c && c.type === "MemberExpression" && c.property && c.property.name === "send") sends++;
  });
...
function routePathOf(arg) {
  ...
  if (arg.type === "TemplateLiteral") return arg.quasis.map((q,i)=>q.value.cooked+(arg.expressions[i]?"${...}":"")).join("");
  return null;
}
...
for (const file of collectFiles(ROUTES, [])) {
  let ast;
  try { ast = parser.parse(fs.readFileSync(file,"utf8"), { sourceType:"module", plugins:["typescript"], errorRecovery:true }); }
  ...
  walk(ast, (n) => {
    ...
    let handler = null, resolvedVia = "inline";
    const fnArgs = n.arguments.filter(isFn);
    if (fnArgs.length) handler = fnArgs[fnArgs.length-1];
    let preGuards = [];
    for (const o of n.arguments.filter((a)=>a && a.type==="ObjectExpression")) {
      for (const p of o.properties || []) {
        if (!p.key) continue;
        const key = p.key.name || p.key.value;
        if (key === "handler" && !handler) { handler = p.value; resolvedVia = "options.handler"; }
    ...
    if (!handler) {
      for (let i = n.arguments.length - 1; i >= 1; i--) {
        ...
        if (a && a.type === "Identifier" && locals.has(a.name)) { handler = locals.get(a.name); resolvedVia = "identifier:"+a.name; break; }
      }
    }
    const names = handler ? resolvedCalleeNames(handler, locals, 0, new Set()) : [];
    ...
    rows.push({
      ...
      method: c.property.name.toUpperCase(), route: rp, verdict,
      guards: Array.from(new Set(all.filter((x)=>ALL_GUARDS.has(x)))).join("+") || "-",
...
const tally = (rs)=>rs.reduce((a,r)=>{a[r.verdict]=(a[r.verdict]||0)+1;return a;},{});
console.log(JSON.stringify({
  totalRegistrations: rows.length, files: new Set(rows.map(r=>r.file)).size,
  parseFailures, byVerdict: tally(rows), mutating: tally(rows.filter(r=>MUT.has(r.method))),
