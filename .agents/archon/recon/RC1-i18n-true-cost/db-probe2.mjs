...
async function q(l, s) {
  try { const r = await c.query(s); console.log("### " + l); console.log(JSON.stringify(r.rows, null, 1)); }
  catch (e) { console.log("### " + l + " ERROR: " + String(e.message).slice(0, 200)); }
}
await q("organizations, full id + created_at + counts of dependent rows",
...
await q("clinics/organizations table columns (looking for a settings json blob)",
