...
if (!m) { console.log("DATABASE_URL not present in .env"); process.exit(1); }
...
await q("migration_runs BY organization",
...
await q("columns whose name mentions language/locale, anywhere in the schema",
  `select table_name, column_name, data_type
     from information_schema.columns
    where table_schema='public' and (column_name ilike '%lang%' or column_name ilike '%locale%')
    order by 1,2`);

await q("ui_preferences-ish tables",
