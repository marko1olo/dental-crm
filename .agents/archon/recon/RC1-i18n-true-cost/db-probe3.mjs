...
async function q(l, s) {
  ...
  catch (e) { console.log("### " + l + " ERROR: " + String(e.message).slice(0, 250)); }
}
await q("users.ui_preferences presence, BY organization",
  `select left(organization_id::text,8) as org8,
          count(*) as users,
          count(ui_preferences) as with_prefs
     from users group by 1 order by 1`);
await q("real org 4a3420d1: every user row, prefs present?, ui_language inside blob",
...
await q("what plain SELECT ... LIMIT 1 (no ORDER BY) actually returns for the real org, 5 runs",
  `select left(id::text,8) as user8 from users where organization_id::text like '4a3420d1%' limit 1`);
for (let i = 0; i < 4; i++) {
  await q("repeat LIMIT 1 (no ORDER BY) run " + (i + 2),
    `select left(id::text,8) as user8 from users where organization_id::text like '4a3420d1%' limit 1`);
}
await q("does the SAME plan hold after a seq-scan hint? (disable index scan)",
