...
function dataReason(rel) {
  ...
  if (/apps\/api\/src\/migration\//.test(r)) return "import-matcher-competitor-headers";
  if (/routes\/smartImports\.ts$/.test(r)) return "import-matcher-competitor-headers";
  ...
  if (/apps\/api\/src\/routes\/documents\//.test(r) || /apps\/api\/src\/documents\//.test(r))
...
const VISIBLE_ATTRS = new Set([
  "title", "placeholder", "alt", "label", "aria-label", "ariaLabel",
  "aria-description", "aria-placeholder", "aria-valuetext", "aria-roledescription",
  "heading", "subtitle", "caption", "description", "hint", "helpText", "help",
  "emptyText", "emptyLabel", "emptyMessage", "errorText", "errorMessage",
  "confirmLabel", "cancelLabel", "submitLabel", "actionLabel", "buttonLabel",
  "tooltip", "summary", "message", "text", "content", "legend", "badge",
  "primaryLabel", "secondaryLabel", "ctaLabel", "name", "header", "footer",
  "unit", "suffix", "prefix", "note", "warning", "detail", "details",
]);
const INVISIBLE_ATTRS = new Set([
  "className", "class", "id", "key", "href", "src", "type", "role", "style",
  "data-testid", "testId", "htmlFor", "target", "rel", "value", "name_",
  "autoComplete", "inputMode", "pattern", "accept", "method", "action",
]);
...
function walk(node, visit, parent = null, key = null) {
  ...
  for (const k of Object.keys(node)) {
    if (k === "loc" || k === "range" || k === "leadingComments" || k === "trailingComments" ||
        k === "innerComments" || k === "__parent" || k === "__key" || k === "extra") continue;
    const v = node[k];
...
function ancestors(node) {
  ...
  let p = node.__parent;
  let guard = 0;
  while (p && guard++ < 200) { out.push(p); p = p.__parent; }
  return out;
}
...
function dictInfo(node, anc) {
  ...
  if (obj && Array.isArray(obj.properties) && obj.properties.length >= 3) {
    const vals = obj.properties.filter((p) => p.type === "ObjectProperty").map((p) => p.value);
    if (vals.length >= 3 && vals.every((v) => v && (v.type === "StringLiteral" || v.type === "TemplateLiteral"))) {
      let id = null;
...
function classify(node, rel, src) {
  ...
  // JSX text
  if (node.type === "JSXText") return { bucket: "a_jsx_visible", role: "jsx-text" };

  // JSX attribute
  const attr = anc.find((a) => a.type === "JSXAttribute");
  if (attr) {
    const nm = attr.name && (attr.name.name || (attr.name.namespace ? attr.name.namespace.name + ":" + attr.name.name.name : null));
    const name = typeof nm === "string" ? nm : String(nm);
    if (VISIBLE_ATTRS.has(name)) return { bucket: "a_jsx_visible", role: "jsx-attr:" + name };
    if (INVISIBLE_ATTRS.has(name)) return { bucket: "other", role: "jsx-attr-invisible:" + name };
    return { bucket: "a_jsx_visible", role: "jsx-attr-other:" + name };
  }

  // thrown error
  for (const a of anc) {
    if (a.type === "ThrowStatement") return { bucket: "b_error_toast", role: "throw" };
    if (a.type === "NewExpression" && a.callee && /Error$/.test(calleeName(a.callee) || ""))
  ...
  for (const a of anc) {
    ...
    if (cn && TOAST_RE.test(cn)) return { bucket: "b_error_toast", role: "call:" + cn };
    if (cn && LOG_RE.test(cn) && (on === "console" || on === "logger" || on === "log" ||
        on === "req" || on === "request" || on === "fastify" || on === "app" || on === "server"))
      return { bucket: "log_only", role: (on || "?") + "." + cn };
    if (on === "console") return { bucket: "log_only", role: "console." + (cn || "?") };
    // zod validation message => user visible on form
    if (on === "z" || (cn && /^(min|max|length|regex|refine|superRefine|email|url|uuid|nonempty|int|positive|nonnegative)$/.test(cn) &&
  ...
  if (d.isDict) return { bucket: "dict_value", role: "dict:" + (d.id || "anon") + ":" + d.why, dictId: d.id };
...
const result = {
  ...
  commentStats: { files: 0, cyrillicComments: 0, cyrillicCommentLines: 0, cyrillicCommentChars: 0 },
};

const distinct = {};   // bucket -> Set
const tplSet = new Set();

function bump(obj, k, n = 1) { obj[k] = (obj[k] || 0) + n; }
...
for (const rel of files) {
  ...
  try {
    ast = parse(src, {
      ...
      plugins: [
        ...
        "jsx", "decorators-legacy", "classProperties", "topLevelAwait",
        "importAttributes", "explicitResourceManagement",
      ],
    });
  } catch (err) {
    result.parseFailures.push({ file: rel, error: String(err.message).slice(0, 200) });
    continue;
  }
  ...
  walk(ast.program, (n) => {
    if (n.type === "TSTypeAnnotation" && typeof n.start === "number" && typeof n.end === "number") {
    ...
    if (n.type === "StringLiteral" && CYR.test(n.value)) raws = [n.value];
      ...
      if (parts.length) {
        ...
        if (n.expressions.length > 0) {
          ...
          tplSet.add(rel + "::" + src.slice(n.start, Math.min(n.end, n.start + 200)));
        }
      }
    }
    if (!raws) return;
...
for (const [k, s] of Object.entries(distinct)) result.distinctByBucket[k] = s.size;
result.interpolatedTemplates.distinct = tplSet.size;

fs.writeFileSync(path.join(ROOT, ".agents/archon/recon/RC1-i18n-true-cost", OUT), JSON.stringify(result, null, 1), "utf8");

// console summary (ASCII only)
console.log("scannedFiles(with cyrillic parsed):", Object.keys(result.perFile).length, "of", result.scannedFiles);
...
for (const [k, v] of Object.entries(result.buckets).sort((a, b) => b[1] - a[1]))
  console.log(String(v).padStart(7), k, " distinct:", result.distinctByBucket[k] ?? "(n/a)");
console.log("--- per area ---");
for (const [k, v] of Object.entries(result.perArea).sort()) console.log(String(v).padStart(7), k);
console.log("--- interpolated templates ---", JSON.stringify(result.interpolatedTemplates));
