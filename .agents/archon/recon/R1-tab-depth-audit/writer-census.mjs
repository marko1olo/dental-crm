...
const schemaFiles = files(path.join(ROOT, "apps/api/src/db"), [".ts"]).concat(
	files(path.join(ROOT, "packages"), [".ts"]),
);
...
const RAW =
	/INSERT\s+INTO\s+(?:public\s*\.\s*)?["`]?([a-zA-Z_][a-zA-Z0-9_]*)["`]?/gi;
...
try {
	const txt = readFileSync(
		path.join(ROOT, ".agents/archon/recon/R1-tab-depth-audit/db-rowcounts.txt"),
		"utf8",
	);
...
for (const [ident, { sql, at }] of tables) {
	...
	rows.push({
		...
		at,
		writers: w.length,
		writerAt: w.slice(0, 2),
		readers: r.length,
		readerAt: r.slice(0, 2),
		live: live.has(sql) ? live.get(sql) : "?",
	});
}
rows.sort(
	(a, b) =>
...
console.log(
	`DECLARED TABLES: ${rows.length}   FILES SCANNED: ${allFiles.length}`,
);
console.log(`ZERO-WRITER TABLES: ${zeroW.length}`);
console.log(
	`  of which DETECTOR MISSES (live rows > 0, so a writer must exist): ${misses.length}` +
		(misses.length
			? "  -> " + misses.map((m) => `${m.sql}(${m.live})`).join(", ")
			: ""),
);
console.log(
	`ZERO-WRITER AND READ BY CODE (panel that can never fill): ${zeroW.filter((r) => r.readers > 0 && !(typeof r.live === "number" && r.live > 0)).length}`,
);
console.log(
