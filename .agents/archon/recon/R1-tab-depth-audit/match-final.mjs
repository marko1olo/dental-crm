...
const table = readFileSync(D + "route-table.txt", "utf8").split(/\r?\n/).filter(Boolean)
  .map((l) => l.split("\t"));
const norm = (p) => p.replace(/\/+$/, "").split("/").map((s) => (s.startsWith(":") || s === "*" ? "*" : s)).join("/");
...
const flat = readFileSync(D + "view-api-flat.txt", "utf8").split(/\r?\n/).filter(Boolean)
  .map((l) => l.split("\t"));
...
for (const [p, where] of [...missingGlobal].sort()) console.log(`${p}\n    ${[...where].join("\n    ")}`);
