...
function files(dir) {
	...
	for (const e of readdirSync(dir)) {
		...
		if (statSync(f).isDirectory()) {
			if (e !== "dist" && e !== "node_modules") out.push(...files(f));
			continue;
		}
...
function prefixFor(file) {
	const rel =
		"./" + path.relative(API, file).replace(/\\/g, "/").replace(/\.ts$/, "");
...
const ROUTE_RE =
	/\b(?:app|fastify|server|instance)\s*\.\s*(get|post|put|patch|delete|head|options)\s*(?:<[\s\S]{0,900}?>)?\s*\(\s*[`"']([^`"']+)[`"']/g;

const rows = [];
for (const f of files(path.join(API, "routes")).concat(
	files(path.join(API)).filter((x) => !x.includes("routes")),
)) {
	...
	while ((m = ROUTE_RE.exec(src))) {
		...
		rows.push({
			...
			path: full,
			at:
				path.relative("C:/Clinic_MVP/dental-crm", f).replace(/\\/g, "/") +
				":" +
				line,
		});
	}
