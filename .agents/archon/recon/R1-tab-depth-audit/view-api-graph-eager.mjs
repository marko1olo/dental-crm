...
function walk(rootRel) {
	...
	if (!existsSync(root))
		return { missing: true, files: [], apis: new Map(), loc: 0 };
	const seen = new Set();
	const queue = [root];
	...
	while (queue.length) {
		...
		lines.forEach((line, i) => {
			...
			for (const m of line.match(API_RE) ?? []) {
				...
				apis
					.get(key)
					.push(`${path.relative(WEB, f).replace(/\\/g, "/")}:${i + 1}`);
			}
		});
...
for (const [view, roots] of Object.entries(ROOTS)) {
	...
	out[view] = {
		...
		apiPaths: [...merged.apis.entries()]
			.map(([p, at]) => ({ p, at: at.slice(0, 3) }))
