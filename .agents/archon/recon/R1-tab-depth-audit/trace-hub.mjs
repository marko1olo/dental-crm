...
function resolveImport(fromFile, spec) {
  ...
  let base = path.resolve(path.dirname(fromFile), spec).replace(/\.jsx?$/, "");
...
function edges(f) {
  ...
  const out = [];
  for (const re of [IMPORT_RE, DYN_RE]) { re.lastIndex = 0; let m;
    while ((m = re.exec(src))) { const r = resolveImport(f, m[1]); if (r) out.push(r); } }
  return out;
}
...
while (q.length) {
  const f = q.shift();
  if (f === target) break;
  for (const n of edges(f)) if (!prev.has(n)) { prev.set(n, f); q.push(n); }
}
if (!prev.has(target)) { console.log("NO RUNTIME PATH"); process.exit(0); }
const chain = []; let c = target;
while (c) { chain.unshift(path.relative(WEB, c).replace(/\\/g, "/")); c = prev.get(c); }
