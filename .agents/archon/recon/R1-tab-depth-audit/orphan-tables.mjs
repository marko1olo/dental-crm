...
const live = new Set();
{
  ...
  let sec = "";
  for (const l of txt) {
    if (l.startsWith("--- NON-EMPTY")) { sec = "n"; continue; }
    if (l.startsWith("--- EMPTY")) { sec = "e"; continue; }
...
console.log("LIVE TABLES: " + live.size + "   DRIZZLE-DECLARED: " + declared.size);
...
console.log(inCodeNotLive.map((t) => `${t}  (writers=${declared.get(t).writers}, readers=${declared.get(t).readers})`).join("\n"));
