...
if (!process.env.DATABASE_URL) {
	console.error(
		"DATABASE_URL not present in env after loading .env — cannot probe.",
	);
	process.exit(2);
}
...
if (mode === "like") {
	...
	const r = await client.query(
     ...
     where table_schema='public' and table_name ilike $1 order by 1`,
		[pat],
	);
	console.log(
	...
	console.log(
		"NON-EMPTY: " +
			nonEmpty.length +
			"   EMPTY: " +
			(rows.length - nonEmpty.length),
	);
	console.log("\n--- NON-EMPTY (table\\trows) ---");
	for (const r of nonEmpty.sort((a, b) => b.n - a.n))
		console.log(`${r.table}\t${r.n}`);
	console.log("\n--- EMPTY (0 rows) ---");
