...
За полтора часа контрольные счёта уехали (`organizations` 2→3, `patients` 17→18, `visits` 10→15), а
`bi_analytics_snapshots` получила 6 строк, отчего «двойников с данными» стало 4 вместо 3. Счёта по
восемнадцати брошенным таблицам не двинулись ни на строку — они и есть предмет досье.
...
- «Двойник принят» доказывает подключение двойника к серверному коду, а не то, что им пользуются: **у 10
  из 14 существующих двойников** тоже ноль строк. (До NN6 здесь стояло «15 из 18» — арифметика неверна с
  самого начала: двойника нет у четырёх таблиц вообще, так что знаменатель 14, а не 18, и на момент MM4
  пустых было 11 из 14.) Пустая объявленная таблица и пустая брошенная — одинаково мёртвая функция;
  разница только в том, что первую видят аудиты.
