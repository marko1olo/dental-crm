export const meta = {
	...
	description:
		"DENTE cycle 19: a state-system log with no tenant column, safety that defaults to permissive, two auth idioms, a ratchet blind to HTTP method, an orphaned public booking form",
	phases: [
		{
			title: "Build",
			detail:
...
const PACKETS = [
	{
		...
		files:
			"apps/api/src/db/schema.ts и НОВЫЙ файл миграции в apps/api/drizzle или где лежат остальные миграции — найдите сами через fd",
...
`,
	},
	{
		...
		files:
			"ЧТЕНИЕ по всему apps/api/src/routes/**. ПРАВКА — только apps/api/src/routes/patients.ts, и только если инвентарь докажет, что сведение безопасно",
...
`,
	},
	{
		...
		label:
			"JJ4 храповик адресов не сравнивает HTTP-метод и держит пять мёртвых записей",
...
`,
	},
	{
		...
		files:
			"apps/web/src/pages/PublicBookingWidget.tsx и, если решите монтировать, точка монтирования — но её укажите в filesChanged и не трогайте workspaceShell.tsx",
