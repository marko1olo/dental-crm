...
(`schema.ts:430`). Поле `name` в `service_catalog_items` не существует. Это отдельный
дефект отдельного файла, в область этой задачи не входит и остаётся долгом.
**НЕ ПРОВЕРЕНО:** доходит ли этот код до вставки на живом мастере.
