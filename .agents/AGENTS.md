...
**8s. THE UNIVERSAL ANTI-BLOAT & BEST-OF-BREED DOCTRINE (ВСЕЛЕНСКИЙ АНТИ-БЛОАТ ДОГМАТ: ЗАПРЕТ НА КЛОНИРОВАНИЕ, ПРОЦЕДУРНЫЕ СИМУЛЯТОРЫ И ДОМЕННОЕ ЗАРАЖЕНИЕ)**:
- **Закон Единого Эталона («The Single Canonical Best-of-Breed Law»):** Категорический запрет на создание параллельных дубликатов сущностей (`*V2`, `*Advanced*`, `*Mobile*`, `*Webapp*`, перенос в соседние папки под похожими именами). Любая доработка интегрируется строго в канонический мастер-файл через опциональные пропсы/режимы. При обнаружении дублей — РОВНО ОДИН лучший компонент («The Best of Breed») впитывает функционал, а остальные немедленно уничтожаются либо схлопываются в 20–40-строчные фасады-делегаты.
- **Закон снижения трения против симуляторов («Friction-Killer Over Simulators»):** CRM — не физический тренажер и не видеоигра. Запрещено строить процедурные симуляторы прокликивания 100 точек, генерации кривых затухания, аудио-дебаггеры или видеосканеры крафт-пакетов. Обязателен 0–1-кликовый коридор нормы («Соматически здоров / Норма», «Стерилизация проведена / Тест 5 класса», пакетные списания). Честный пустой статус вместо процедурного вранья (нет интеграции — статус «Не подключено», ноль генераторов фейковых диалогов).
- **Суверенитет ограниченного контекста («Strict Bounded Context»):** Запрет на перенос академического стационарного мусора (койко-дни, трансфузиология крови Kell K1, наркотики ПКУ Трамадол/Диазепам, комиссии начмеда с 5 подписями) в частную амбулаторную стоматологию у кресла. Тест кресла: нужно ли это врачу у кресла, ассистенту или администратору по Форме 043/у и 54-ФЗ? Если нет — немедленная отбраковка.
- **Запрет на бутафорские ширмы («Zero Deceptive Scaffolds»):** Запрещено создавать скрытые контейнеры-ширмы со 100+ импортами и мертвыми слушателями событий ради обмана синтетических тестов монтирования. Либо компонент реально используется пользователем, либо он удаляется.
