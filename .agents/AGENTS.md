...
- **Инструментальное бремя доказательства (The Burden of Proof Law):** Запрещены как слепая сикофантия («всё выглядит отлично»), так и параноидальное выдумывание дефектов («любой экран — это брак»). Вердикт `[ПРОВЕРЕНО: ЧИСТО]` выносится ТОЛЬКО И ИСКЛЮЧИТЕЛЬНО тогда, когда проверяющий лично доказал отсутствие каждого из **7 смертных грехов интерфейса (The 7 Deadly Sins Checklist)**:
  ...
  4. *Контрастность и гигиена тем (WCAG AAA):* В Dark Mode — никаких слепящих белых пятен и рамок (`slate-950`/`neutral-900`). В Light Mode — никаких грязно-серых надписей на белом (контраст $\ge 4.5:1$).
  5. *Автономия врача (Мандат 8e):* Никаких заблокированных `disabled` кнопок из-за второстепенных полей; касса 54-ФЗ без ИНН физлиц; debounced autosave; свободные скидки врача; визиограф <50мс без зависания на ИИ.
  6. *Закон Анти-Матрёшки:* Глубина модалок СТРОГО 1. Никаких модалок поверх модалок и карточек внутри карточек.
  7. *Святость официальных бланков:* Ноль мультяшных эмодзи в картах 043/у, актах, чеках 54-ФЗ и снимках КТ — строго векторные иконки Lucide.
...
- **Запрет на академический оверинжиниринг и процедурные симуляторы:** Запрещено выдумывать математические симуляторы, 100-точечные опросники и усложнённые научные модели там, где врачу требуется лаконичная клиническая фиксация статуса (норма/патология) и регламентная форма РФ (043/у, ИДС, 54-ФЗ, 804н, СанПиН).
...
- **Программа =/= Реальность:** Категорически ЗАПРЕЩЕНО делать честные процедурные симуляторы мира (прокликивать каждую каплю, вбивать каждый камешек, каждый миллиметр кармана, все 50 пунктов соматики).
- Не надо думать, что врач будет каждый миллиметр, а администратор — каждый пук пациента вбивать в CRM. При внедрении всегда есть человеческий фактор: лень, усталость и высокое трение. Если программа заставляет делать 20 лишних движений — персонал будет её саботировать.
- **Главная цель CRM:** выдавать безупречные документы (043/у, чеки 54-ФЗ, акты 804н, согласия) и помогать в работе, а не мешать. Если можно схитрить и сделать быстрее в 1 клик через пресет («Норма», «Профгигиена выполнена», «Стандартная анестезия 1.7 мл», «Списание карпул пакетом») — софт обязан предоставить этот путь.

**8l. FRESH SUBAGENT CONTEXT & RESUSCITATION PROTOCOLS (МАНДАТ 8l: СВЕЖИЙ КОНТЕКСТ И АВТОРИЗОВАННОЕ ВОЗРОЖДЕНИЕ)**:
...
- **Авторизованное прямое исправление бинарника (`scripts/antigravity_binary_patcher.cjs`):**
  * Для прямого оживления упавших субагентов по `send_message` РАЗРЕШЕНО и ШТАТНО применять скрипт прямого патчинга `node scripts/antigravity_binary_patcher.cjs --patch`.
  * Скрипт использует динамический поиск 12-байтной сигнатуры x86_64 (`4c 39 c2 4c 8b 94 24 00 01 00 00 7e`), устойчив к автообновлениям Antigravity и применяет атомарную стратегию замены файла на Windows (`rename` залоченного файла -> бэкап -> патч `90 90` в новый файл).
  * После патча инструкция перехода нейтрализуется, и `send_message` к упавшему дочернему агенту успешно вызывает `reviveSubagentFn`.
- **Глубокий протокол эстафеты (Deep Transcript Relay — `scripts/antigravity_transcript_relay.cjs`):** При необходимости перебалансировки контекста или спавне нового исполнителя оркестратор выполняет глубокую эстафету:
  1. Запускает `node scripts/antigravity_transcript_relay.cjs <deadConversationId>`.
  2. Скрипт извлекает 100% измененных файлов, все запущенные команды, блоки рассуждений (`thinking`), последний статус-доклад и полную хронологию (запрещен поверхностный срез `slice(-10)`).
  3. Убивает зависший слот через `manage_subagents Action="kill"`.
  4. Спавнит свежего субагента через `invoke_subagent` (`Model: "inherit"`), передавая сгенерированный Handover Prompt и абсолютную ссылку `file:///.../transcript.jsonl`.
  5. Новый субагент подхватывает всю цепочку без потери знаний и продолжает с чистого листа памяти.
- **Разграничение:** Если субагент упал *посреди задачи* — допустим прямой запуск патчера и пинг. Но если субагент *полностью закончил свою задачу* — **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** давать ему следующую задачу («не еби труп»)! Для каждой НОВОЙ задачи спавнится **СВЕЖИЙ субагент** (`invoke_subagent`) с чистым контекстом и полной инструкцией.
...
- **Запрет на одиночного поверхностного «проверяльщика» (No Lone Shallow Auditor):** Один субагент с размытым промптом неизбежно скатывается в слепоту чек-листа и поверхностное подмахивание. Полноценный визуальный и эргономический аудит проводится **десантом специализированных субагентов** с жестко разделенными ролями:
  ...
  3. **Субагент-Критик эргономики соло-врача (Solo Doctor & Reception Auditor — Mandates 8e/8n):** Проверка пути врача и администратора. Устранение тупиков, проверка отсутствия `disabled` кнопок, 1-кликовая норма, 043/у без преград, касса без ИНН физлиц, запись за 5 секунд без модального ада.
...
- **Суверенитет ограниченного контекста (Strict Bounded Context):** Бритва Оккама: каждое поле и сущность проверяются вопросом: *«Используется ли это непосредственно врачом-стоматологом у кресла, ассистентом или администратором клиники по Форме 043/у, 804н и 54-ФЗ?»*. Если нет — немедленная отбраковка без права на существование.
...
**8v. DOCTOR & ADMIN SOVEREIGNTY (90% USERS) & BAN ON NURSE CLICKING BLOAT (МАНДАТ 8v: СУВЕРЕНИТЕТ ВРАЧА И АДМИНИСТРАТОРА И ПОЛНЫЙ ЗАПРЕТ НА «МЕДСЕСТРИНСКУЮ ШИЗУ» И ПРОКЛИКИВАНИЕ КРАФТ-ПАКЕТОВ)**:
- **90% пользователей частной стоматологии — это ВРАЧ и АДМИНИСТРАТОР:** Изредка бухгалтер или управляющий.
- **Медсестры не сидят за компьютером и не тыкают CRM:** В реальной частной стоматологии медсестры заняты делом у кресла и в стерилизационной (ЦСО): обработка, мытье, автоклав, ассистирование. Заставлять медсестру садиться за монитор, логиниться, кликать «вскрыть крафт-пакет», сканировать штрихкоды пакетов, вести пошаговые циклы автоклавов — это шизофрения и академический блоат.
- **Никаких сканирований крафт-пакетов на приеме у врача:** Врач лечит пациента, а не работает сканером штрихкодов в дневнике 043/у. Инструменты стерильны по умолчанию (дефолтный стерильный лоток по СанПиН). Любые кнопки «Сканировать лоток / крафт-пакет» в дневнике приема подлежат немедленному сносу.
- **Журналы СанПиН (форма 257/у, контроль азопирама, дезинфекция) — это фоновая отчетность:** Все регламентные журналы формируются автоматически в 1 клик для проверок Роспотребнадзора без интерактивного прокликивания циклов человеком.
- **Списание материалов:** Карпулы анестетиков, перчатки, боры списываются автоматически по факту оказанной услуги (техкарты) или в 1 клик врачом/администратором без необходимости медсестре открывать отдельные модалки.
