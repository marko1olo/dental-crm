...
**8l. FRESH SUBAGENT CONTEXT & ANTI-ZOMBIE DOCTRINE (СВЕЖИЙ КОНТЕКСТ ВМЕСТО РЕАНИМАЦИИ ТРУПОВ)**:
- **Разграничение:** Если субагент упал *посреди задачи* по квоте 429 — отправь пинг сообщением (`send_message`) для завершения. Но если субагент *полностью закончил свою задачу* — **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО** давать ему следующую задачу («не еби труп»)!
- После сотен шагов контекстное окно переполняется, модель тупеет, теряет концентрацию и галлюцинирует. Для каждой НОВОЙ задачи спавнится **СВЕЖИЙ субагент** (`invoke_subagent`) с чистым контекстом и полной инструкцией.
