# Milestone M1 Adversarial Suite Test Fix — Handoff Report

**Work Product**: Milestone M1 Adversarial Suite Fix (`apps/web/src/__tests__/m1AdversarialRemediation.test.ts`)  
**Target Project**: DENTE Dental CRM (`C:/Clinic_MVP/dental-crm`)  
**Author**: Worker M1 (`worker_m1_fix`)  
**Date**: 2026-08-18T21:31:35+04:00  

---

## 1. Observation

1. In `apps/web/src/__tests__/m1AdversarialRemediation.test.ts`, the hook `useDashboardLoaderLogic` was previously invoked directly as a plain JavaScript function within Node.js native test runner (`node --test`), resulting in `TypeError: Cannot read properties of null (reading 'useRef')` because React's dispatcher was unset.
2. In the 500 error test case, the assertion previously expected raw HTTP status `/500/`, whereas production code (`apps/web/src/lib/panelStateText.ts:141`) generates human-oriented localized text (`"сервер не смог выполнить запрос — повторите через минуту..."`).
3. Applied the `renderHookProbe` harness using `renderToStaticMarkup` from `react-dom/server` and updated all 5 test call sites (`suppresses toasts on 401`, `suppresses toasts on 403`, `shows toast on 500`, `handles network failure`, `discards stale responses`).
4. Applied the assertion fix in test 3 (`assert.match(toastMsg, /сервер не смог выполнить запрос/);`).

---

## 2. Logic Chain

1. `useDashboardLoaderLogic` employs `useRef` and `useCallback`.
2. Direct function calls in Node.js test scripts lack React render context (`ReactCurrentDispatcher.current === null`).
3. `renderToStaticMarkup(createElement(Probe))` from `react-dom/server` initializes the React Server Dispatcher during component evaluation, allowing `useRef(0)` and `useCallback` to initialize correctly with real mutable references.
4. Extracted hook callbacks remain fully callable across async promises and event loop ticks, properly preserving concurrency semantics, seq IDs, and stale response rejection logic.
5. Updating the assertion to match `/сервер не смог выполнить запрос/` aligns test expectations with real production Russian localized UI error messages.

---

## 3. Caveats

- `renderToStaticMarkup` does not invoke `useEffect` lifecycles (standard SSR behavior). Hooks using `useEffect` should either test pure decomposed logic functions or use Playwright integration tests.
- `useDashboardLoaderLogic` only uses `useRef` and `useCallback`, so 100% of its functionality is exercised by this harness.
...
### Modified Files:
- `apps/web/src/__tests__/m1AdversarialRemediation.test.ts` (427 lines, all 12 adversarial test cases passing).

### Exact Code of `apps/web/src/__tests__/m1AdversarialRemediation.test.ts`:
```ts
...
1. `npm test -w @dental/web`
   - Result: `tests 1463, pass 1463, fail 0` (Exit code 0).
2. `npm run typecheck`
   - Result: Exit code 0 across `@dental/shared`, `@dental/api`, and `@dental/web`.
3. `npm run check:encoding`
   - Result: `Кодировка в порядке: проверено 2700 файлов, замечаний нет.` (Exit code 0).
4. Direct test file execution:
   `node --import tsx --import ./testCssStub.mjs --test "src/__tests__/m1AdversarialRemediation.test.ts"` in `apps/web/`
   - Result: `tests 12, suites 4, pass 12, fail 0` (Exit code 0).
