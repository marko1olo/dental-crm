...
function findUnusedRoutes() {
  ...
  function getFiles(dir) {
    ...
    list.forEach(file => {
      ...
      } else if (filePath.endsWith('.ts') && !filePath.endsWith('.test.ts') && !filePath.includes('\\tests\\')) {
  ...
  for (const file of routeFiles) {
    ...
    try {
      ...
      const out = execSync(`rg -l "${baseName}" apps/api/src -g "!**/*.test.ts" -g "!**/tests/**"`).toString();
      // If the only result is the file itself, it's an orphan
      const lines = out.trim().split('\n').filter(l => l);
      if (lines.length === 1 && lines[0].replace(/\\/g, '/').endsWith(relPath)) {
