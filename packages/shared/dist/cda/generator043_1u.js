...
export function generateSemd043_1uXml(params) {
    ...
    const angleRows = [];
    if (angleMolarR)
        angleRows.push(`<tr><td>Смыкание первых моляров справа</td><td>${escapeXml(angleMolarR)}</td></tr>`);
    if (angleMolarL)
        angleRows.push(`<tr><td>Смыкание первых моляров слева</td><td>${escapeXml(angleMolarL)}</td></tr>`);
    if (angleCanineR)
        angleRows.push(`<tr><td>Смыкание клыков справа</td><td>${escapeXml(angleCanineR)}</td></tr>`);
    if (angleCanineL)
        angleRows.push(`<tr><td>Смыкание клыков слева</td><td>${escapeXml(angleCanineL)}</td></tr>`);
    ...
    const anthroRows = [];
    if (anthro) {
        if (anthro.facialType)
            anthroRows.push(`<tr><td>Тип лица</td><td>${escapeXml(FACIAL_TYPE_LABELS[anthro.facialType] || anthro.facialType)}</td></tr>`);
        if (anthro.profileType)
            anthroRows.push(`<tr><td>Профиль лица</td><td>${escapeXml(PROFILE_LABELS[anthro.profileType] || anthro.profileType)}</td></tr>`);
        if (anthro.facialSymmetry)
            anthroRows.push(`<tr><td>Симметрия лица</td><td>${anthro.facialSymmetry === "symmetric" ? "Симметрично" : anthro.facialSymmetry === "chin_deviation_left" ? "Смещение подбородка влево" : "Смещение подбородка вправо"}</td></tr>`);
        if (anthro.chinDeviationMm !== undefined)
            anthroRows.push(`<tr><td>Смещение подбородка</td><td>${anthro.chinDeviationMm} мм</td></tr>`);
        if (anthro.nasolabialAngleDegrees !== undefined)
            anthroRows.push(`<tr><td>Носогубный угол</td><td>${anthro.nasolabialAngleDegrees}° (норма 90-110°)</td></tr>`);
        if (anthro.mentolabialSulcus)
            anthroRows.push(`<tr><td>Подбородочно-губная борозда</td><td>${anthro.mentolabialSulcus === "normal" ? "Выражена умеренно (норма)" : anthro.mentolabialSulcus === "deep_pronounced" ? "Глубокая" : "Сглажена"}</td></tr>`);
        if (anthro.lipCompetenceAtRest)
            anthroRows.push(`<tr><td>Смыкание губ в покое</td><td>${anthro.lipCompetenceAtRest === "competent_closed" ? "Смыкаются без напряжения" : anthro.lipCompetenceAtRest === "incompetent_open" ? "Не смыкаются" : "Смыкаются с напряжением"}</td></tr>`);
        if (anthro.incisalDisplayAtSmileMm !== undefined)
            anthroRows.push(`<tr><td>Экспозиция резцов при улыбке</td><td>${anthro.incisalDisplayAtSmileMm} мм</td></tr>`);
        if (anthro.gummySmileMm !== undefined)
            anthroRows.push(`<tr><td>Десневая улыбка</td><td>${anthro.gummySmileMm > 0 ? `${anthro.gummySmileMm} мм` : "Отсутствует (норма)"}</td></tr>`);
    ...
    const cephRows = [];
    if (ceph) {
        if (ceph.skeletalClass)
            cephRows.push(`<tr><td>Скелетный класс</td><td>${escapeXml(SKELETAL_CLASS_LABELS[ceph.skeletalClass] || ceph.skeletalClass)}</td></tr>`);
        if (ceph.growthPattern)
            cephRows.push(`<tr><td>Тип роста</td><td>${ceph.growthPattern === "normodivergent" ? "Нормодивергентный" : ceph.growthPattern === "hyperdivergent_vertical" ? "Вертикальный (гипердивергентный)" : "Горизонтальный (гиподивергентный)"}</td></tr>`);
        if (ceph.snaAngle !== undefined)
            cephRows.push(`<tr><td>Угол SNA (положение в/ч)</td><td>${ceph.snaAngle}° (норма 82° ± 2°)</td></tr>`);
        if (ceph.snbAngle !== undefined)
            cephRows.push(`<tr><td>Угол SNB (положение н/ч)</td><td>${ceph.snbAngle}° (норма 80° ± 2°)</td></tr>`);
        if (ceph.anbAngle !== undefined)
            cephRows.push(`<tr><td>Угол ANB (межчелюстной сагиттальный)</td><td>${ceph.anbAngle}° (норма 2° ± 1°)</td></tr>`);
        if (ceph.witsAppraisalMm !== undefined)
            cephRows.push(`<tr><td>Число Wits</td><td>${ceph.witsAppraisalMm} мм (норма -1..0 мм)</td></tr>`);
        if (ceph.fmaAngle !== undefined)
            cephRows.push(`<tr><td>Угол FMA (Франкфурт-мандибулярный)</td><td>${ceph.fmaAngle}° (норма 25° ± 3°)</td></tr>`);
        if (ceph.snGoGnAngle !== undefined)
            cephRows.push(`<tr><td>Угол Sn-GoGn (вертикальный)</td><td>${ceph.snGoGnAngle}° (норма 32° ± 3°)</td></tr>`);
        if (ceph.upperIncisorToNaAngle !== undefined)
            cephRows.push(`<tr><td>Наклон верхних резцов (1-NA)</td><td>${ceph.upperIncisorToNaAngle}° (норма 22°)</td></tr>`);
        if (ceph.lowerIncisorToNbAngle !== undefined)
            cephRows.push(`<tr><td>Наклон нижних резцов (1-NB)</td><td>${ceph.lowerIncisorToNbAngle}° (норма 25°)</td></tr>`);
        if (ceph.interincisalAngle !== undefined)
            cephRows.push(`<tr><td>Межрезцовый угол</td><td>${ceph.interincisalAngle}° (норма 130° ± 5°)</td></tr>`);
    }
    ...
    if (ind) {
        if (ind.tonnIndexNotes)
            indexParagraphs.push(`<paragraph><strong>Индекс Тона (Tonn):</strong> ${escapeXml(ind.tonnIndexNotes)}</paragraph>`);
        if (ind.pontIndexNotes)
            indexParagraphs.push(`<paragraph><strong>Индекс Пона (Pont):</strong> ${escapeXml(ind.pontIndexNotes)}</paragraph>`);
        if (ind.boltonIndexNotes)
            indexParagraphs.push(`<paragraph><strong>Индекс Болтона (Bolton):</strong> ${escapeXml(ind.boltonIndexNotes)}</paragraph>`);
        if (ind.korkhausIndexNotes)
            indexParagraphs.push(`<paragraph><strong>Индекс Коркхауза (Korkhaus):</strong> ${escapeXml(ind.korkhausIndexNotes)}</paragraph>`);
    }
