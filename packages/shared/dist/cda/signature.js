 ...
 */
export function prepareUkepSigningPayload(xml) {
...
export function createDemonstrationGostSignature(params) {
    const now = new Date();
    const serialHex = Array.from({ length: 16 }, () => Math.floor(Math.random() * 16).toString(16))
        .join("")
        .toUpperCase();
 ...
 */
export function generateXadesXmlSignatureBlock(sig, referenceUri = "") {
