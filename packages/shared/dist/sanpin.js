...
export const SANPIN_REGULATORY_AUTHORITIES = {
    sanpin33686_21: {
        ...
        fullName: "Санитарно-эпидемиологические требования по профилактике инфекционных болезней",
        issuedBy: "Главный государственный санитарный врач РФ",
    },
    sanpin213684_21: {
        title: "СанПиН 2.1.3684-21",
        fullName: "Санитарно-эпидемиологические требования к обращению с медицинскими отходами",
        issuedBy: "Главный государственный санитарный врач РФ",
    },
    order706n: {
    ...
    order646n: {
        ...
        fullName: "Об утверждении Правил надлежащей практики хранения и перевозки лекарственных препаратов для медицинского применения",
        issuedBy: "Минздрав РФ",
    },
    guideline1904_04: {
        title: "Руководство Р 3.5.1904-04",
        fullName: "Использование ультрафиолетового бактерицидного излучения для обеззараживания воздуха в помещениях",
...
export class SanPiNRegulatoryEngine {
     ...
     */
    static evaluatePsoSampling(batchCount, testedCount, isAzopyramNegative, isPhenolphthaleinNegative) {
        ...
        if (!isAzopyramNegative) {
            return {
                ...
                rejectionReason: "Положительная азопирамовая проба (обнаружен гемоглобин / скрытая кровь). Вся партия подлежит повторной дезинфекции и ПСО.",
            };
        }
        if (!isPhenolphthaleinNegative) {
            return {
                ...
                minSampleRequired,
                samplingSatisfied: true,
                rejectionReason: "Положительная фенолфталеиновая проба (остатки щелочных компонентов моющих средств). Вся партия подлежит повторному ополаскиванию дистиллированной водой.",
            };
        }
        return {
     ...
     */
    static calculateLampLife(totalOperatingHours, maxLampHours = 8000) {
        const remainingHours = Math.max(0, maxLampHours - totalOperatingHours);
        const remainingPercent = Number(Math.max(0, Math.min(100, (remainingHours / maxLampHours) * 100)).toFixed(1));
    ...
    static evaluateTemperatureHumidity(params) {
        const { targetTempMin, targetTempMax, actualTemp, targetHumidityMin, targetHumidityMax, actualHumidity, } = params;
        const tempViolation = actualTemp < targetTempMin || actualTemp > targetTempMax;
        let humidityViolation = false;
        if (actualHumidity !== undefined &&
            actualHumidity !== null &&
        ...
        const messages = [];
        if (tempViolation) {
            messages.push(`Температура ${actualTemp}°C вне допустимого диапазона [${targetTempMin}°C .. ${targetTempMax}°C]`);
        }
        if (humidityViolation && actualHumidity !== null && actualHumidity !== undefined) {
            messages.push(`Влажность ${actualHumidity}% вне нормы [${targetHumidityMin}% .. ${targetHumidityMax}%]`);
        }
        const requiresEmergencyTransfer = tempViolation && (actualTemp > targetTempMax + 3.0 || actualTemp < targetTempMin - 2.0);
        return {
            isWithinNorm: false,
            tempViolation,
            humidityViolation,
...
export class SanPiNSterilizationEngine {
    static computeMinimumPsoSampleSize(batchCount) {
        const count = Math.max(1, Math.floor(Number(batchCount) || 1));
        return Math.max(3, Math.ceil(count * 0.01));
    }
    static evaluatePsoCleaningBatch(batchCount, testedCount, isAzopyramNegative, isPhenolphthaleinNegative) {
        return SanPiNRegulatoryEngine.evaluatePsoSampling(batchCount, testedCount, isAzopyramNegative, isPhenolphthaleinNegative);
    }
    ...
    static generateSterilizationBarcode(params) {
        ...
        const dateObj = typeof params.expiryDate === "string"
