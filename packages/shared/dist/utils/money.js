 ...
 */
export function parseKopecks(value) {
    ...
    const kopecks = Number(whole) * KOPECKS_IN_RUBLE + Number(fraction.padEnd(2, "0"));
 ...
 */
export function kopecksToNumericString(kopecks) {
...
export function kopecksToWholeRubles(kopecks) {
    assertWholeKopecks(kopecks);
    if (kopecks % KOPECKS_IN_RUBLE !== 0) {
        throw new Error(`Сумма ${kopecksToNumericString(kopecks)} руб. содержит копейки и не может быть выражена целыми рублями`);
    }
    return kopecks / KOPECKS_IN_RUBLE;
}
/** Сумма нескольких значений. Точная: складываются целые. */
export function sumKopecks(values) {
    let total = 0;
...
export function multiplyKopecks(unit, quantity) {
    assertWholeKopecks(unit);
    if (!Number.isInteger(quantity) || quantity < 0) {
        throw new Error(`Количество должно быть целым неотрицательным, получено ${quantity}`);
    }
    return unit * quantity;
}
/** Цена за единицу × дробное количество (граммы, миллилитры). Округляет до целых копеек. */
export function multiplyKopecksFractional(unit, quantity) {
    assertWholeKopecks(unit);
    if (!Number.isFinite(quantity) || quantity < 0) {
        throw new Error(`Количество должно быть конечным неотрицательным числом, получено ${quantity}`);
    }
 ...
 */
export function percentageOfKopecks(amount, basisPoints) {
    assertWholeKopecks(amount);
    if (!Number.isInteger(basisPoints) || basisPoints < 0) {
        throw new Error(`Процент должен быть целым в базисных пунктах, получено ${basisPoints}`);
    }
 ...
 */
export function splitKopecks(total, parts) {
    ...
    const split = Array.from({ length: parts }, (_, index) => sign * (base + (index < remainder ? 1 : 0)));
    return split;
}
/** Отображение для интерфейса и печатных форм: "1 500,50 ₽". */
export function formatKopecksRu(kopecks) {
    assertWholeKopecks(kopecks);
    const negative = kopecks < 0;
    const absolute = Math.abs(kopecks);
    const whole = Math.trunc(absolute / KOPECKS_IN_RUBLE);
...
function assertWholeKopecks(value) {
    if (!Number.isInteger(value)) {
        throw new Error(`Копейки должны быть целым числом, получено ${value}. Похоже, сумма прошла через плавающую точку.`);
    }
