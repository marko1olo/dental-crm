 ...
 */
export function calculateCbctVramUsage(sliceCount, width = 512, height = 512, bytesPerPixel = 2, // 16-bit DICOM
 ...
 */
export function isVramBudgetExceeded(sliceCount, width = 512, height = 512, maxVramBudgetMb = DEFAULT_MAX_CBCT_VRAM_BUDGET_MB, bytesPerPixel = 2) {
 ...
 */
export function disposeCbctSeriesTextures(gl, textures, width = 512, height = 512, bytesPerPixel = 2) {
 ...
 */
export function disposeFullWebGlPipeline(gl, resources) {
    let texturesDisposed = 0;
    ...
    const vramFreedMb = Number(((texturesDisposed * width * height * bpp) / (1024 * 1024)).toFixed(2));
    return {
        texturesDisposed,
        buffersDisposed,
        programsDisposed,
