import { calculateBoneLossAgeRatio, calculateClinicalAttachmentLevel, calculatePerioIndices, estimateBoneLossPercentFromTeeth, } from "./math.js";
/**
 * Evaluates Periodontitis Staging and Grading according to AAP/EFP 2018 World Workshop criteria.
 */
export function calculateAapEfpStagingAndGrading(teeth, summary, options) {
    ...
    const boneLoss = options?.radiographicBoneLossPercent ??
        estimateBoneLossPercentFromTeeth(teeth, currentSummary.maxCalMm);
    const blAgeRatio = calculateBoneLossAgeRatio(boneLoss, age);
    const smoking = options?.smokingStatus ?? "non_smoker";
    const diabetes = options?.diabetesStatus ?? "none";
    ...
    for (const tooth of activeTeeth) {
        ...
        for (const s of sites) {
            ...
            if (cal > maxToothCal)
                maxToothCal = cal;
            if (s.probingDepthMm > maxToothPd)
                maxToothPd = s.probingDepthMm;
    ...
    const isGeneralized = examinedTeethCount > 0
        ...
        : false;
    const extent = isGeneralized ? "generalized" : "localized";
    ...
    if ((currentSummary.maxPocketDepthMm >= 7 || teethWithSevereLoss >= 2) &&
    ...
    if (currentSummary.maxPocketDepthMm >= 6 ||
 ...
 */
export function derivePeriodontalDiagnosis(teeth, summary, options) {
