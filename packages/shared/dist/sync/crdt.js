...
export function setGlobalClockSkew(skewMs) {
    ...
    currentClockSkewMs = Math.max(-MAX_REASONABLE_SKEW_MS, Math.min(MAX_REASONABLE_SKEW_MS, skewMs));
}
export function getGlobalClockSkew() {
    return currentClockSkewMs;
}
export function calibrateClockSkew(serverIsoOrMs, clientLocalTimeMs) {
    ...
    const safeLocalTimeMs = typeof clientLocalTimeMs === "number" && Number.isFinite(clientLocalTimeMs)
        ? clientLocalTimeMs
        : Date.now();
...
export function getAdjustedNowMs(localTimeMs) {
    const isExplicit = typeof localTimeMs === "number" && Number.isFinite(localTimeMs);
    const safeLocalMs = isExplicit ? localTimeMs : Date.now();
...
export function getAdjustedNowIso(localTimeMs) {
...
export function resetGlobalClockSkew() {
    currentClockSkewMs = 0;
 ...
 */
export function mergeFieldLevelCrdt(options) {
    const { entityId, serverEntity, serverVector = {}, clientPatch, clientVector = {}, clientUpdatedAt, clientId, authorUserId, } = options;
    ...
    if (!serverEntity) {
        ...
        return {
            mergedEntity: { ...clientPatch, id: entityId },
    ...
    const mergedVector = { ...(serverVector || {}) };
    const changedFields = [];
    const conflicts = [];
    ...
    for (const [field, clientVal] of Object.entries(clientPatch)) {
        ...
        const serverFieldTime = parseIsoTimestamp(serverFieldEntry?.updatedAt);
        const clientFieldTime = parseIsoTimestamp(clientFieldEntry?.updatedAt || clientUpdatedAt);
