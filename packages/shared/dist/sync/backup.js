 ...
 */
export function derivePbkdf2Key(passphrase, saltHex, iterations = DENTE_PBKDF2_DEFAULT_ITERATIONS, keyLengthBytes = 32) {
 ...
 */
function deriveKeyStream(secret, length) {
 ...
 */
function encryptPayloadBytes(dataBytes, passphrase, saltHex, ivHex) {
...
function decryptPayloadBytes(ciphertextBytes, passphrase, saltHex, ivHex, expectedAuthTagHex) {
    const derivedKey = derivePbkdf2Key(passphrase, saltHex, 10_000, 32);
 ...
 */
function legacyXorEncryptDecrypt(dataBytes, secret) {
 ...
 */
export function createEncryptedDenteBackup(payload, options) {
    const now = new Date();
    ...
    if (encryptionAlgorithm === "AES-GCM-256") {
        ...
        const { ciphertextBytes, authTagHex: tag } = encryptPayloadBytes(plaintextBytes, passphrase, saltHex, ivHex);
 ...
 */
export function validateDenteBackupContainer(rawBackupText) {
    ...
    if (container.header.magic !== DENTE_BACKUP_MAGIC_V1 &&
    ...
    if (typeof container.header.payloadSha256 !== "string" ||
    ...
    if (!container.header.itemsCount ||
    ...
    if (typeof container.containerSignature !== "string" ||
 ...
 */
export function restoreEncryptedDenteBackup(rawBackupText, passphrase = DEFAULT_DENTE_BACKUP_PASSPHRASE) {
    ...
    const container = JSON.parse(rawBackupText);
    let encryptedBytes;
    try {
    ...
    if (isV2 && container.header.kdf?.saltHex && container.header.ivHex) {
        try {
            decryptedBytes = decryptPayloadBytes(encryptedBytes, passphrase, container.header.kdf.saltHex, container.header.ivHex, container.header.authTagHex);
        }
        catch (err) {
            throw new Error(err instanceof Error && err.message.includes("аутентификационный тег")
                ? err.message
    ...
    if (calculatedHash !== container.header.payloadSha256) {
        throw new Error(`Контрольная сумма полезной нагрузки не совпадает: ожидалось ${container.header.payloadSha256}, получено ${calculatedHash}`);
    }
    if (payload.mutations.length !== container.header.itemsCount.mutations ||
