...
import { createVectorClock, incrementVectorClock, compareVectorClocks } from "./mesh.js";
...
import { getClinicBranch, ztlSyncStageRank, } from "./multibranchTypes.js";
// Re-export all type contracts & constants
export * from "./multibranchTypes.js";
export function createPatientBranchTransferConsent(input) {
...
export function validatePatientBranchTransferConsent(consent) {
    const reasons = [];
...
export function issueDepositTransferVoucher(params) {
...
export function redeemDepositTransferVoucher(params) {
...
export function resolveDepositTransferConflict(v1, v2) {
    ...
    const clockComp = compareVectorClocks(v1.crdtClock || {}, v2.crdtClock || {});
    if (clockComp === "after") {
...
// ─────────────────────────────────────────────────────────────────────────────
export function rerouteLabOrderDestination(order, newDestinationBranchId, reason, operatorStaffName) {
...
export function mergeLabOrderSyncCrdt(local, remote) {
...
export function buildPatientClinicalSnapshot(options) {
    ...
    if (options.odontogramTeeth) {
        for (const [tNumStr, entry] of Object.entries(options.odontogramTeeth)) {
            ...
            const code = entry.statusCode || "healthy";
            if (code === "extracted_absent")
                missingCount++;
            else if (code.startsWith("caries_") || code.startsWith("pulpitis_") || code.startsWith("periodontitis_"))
                cariesCount++;
            else if (code.startsWith("filled_"))
                filledCount++;
            else if (code === "implant")
                implantsCount++;
            else if (code.startsWith("crown_"))
                crownsCount++;
            teethRecord[tNum] = {
                toothNumber: tNum,
                statusCode: code,
    ...
    const treatmentPlansAndEstimates = {
        plans,
    ...
    const labOrders = rawLabOrders.map((ord) => {
        if (ord.destinationBranchId !== targetBranchId) {
            return rerouteLabOrderDestination(ord, targetBranchId, "Автоматическое перенаправление при трансфере пациента", options.staffName);
        }
        return ord;
    });
...
export function validatePatientClinicalSnapshot(snapshot) {
    const errors = [];
...
// ─────────────────────────────────────────────────────────────────────────────
export function generateTransferVerificationQrPayload(snapshot) {
...
export function generateTransferVerificationQrSvg(snapshot, size = 180) {
...
export function generateTransferVerificationQrDataUri(snapshot, size = 180) {
    const payload = generateTransferVerificationQrPayload(snapshot);
