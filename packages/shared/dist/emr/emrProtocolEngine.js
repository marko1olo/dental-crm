 ...
 * ═══════════════════════════════════════════════════════════════════════════
 */
import { STATUTORY_EMR_PROTOCOL_CATALOG, COMPANION_ICD10_CODES, anestheticDrugLabels, statutoryAnestheticDrugLabels, blackCavityClassLabels, } from "./emrProtocolPresets.js";
export { anestheticDrugLabels, statutoryAnestheticDrugLabels };
import { getOrder804nServicesForClinicalCase, calculateOrder804nBillingEstimate, getCanalCountForTooth, } from "../toothCanalsAndBilling804n.js";
/** Проверка корректности номера зуба по FDI нотации (11-48 или 51-85) */
export function isValidFdiToothNumber(num) {
...
export function deduceBlackClassFromSurfaces(toothNumber, surfaces) {
    ...
    if (isAnterior) {
        if (surfSet.has("vestibular") && surfSet.size === 1)
            return "class_V";
        if (surfSet.has("oral") && surfSet.size === 1)
            return "class_I"; // Ямка
        if (surfSet.has("occlusal"))
            return "class_IV"; // Вовлечение режущего края
        if (surfSet.has("mesial") || surfSet.has("distal"))
            return "class_III";
        return "class_III";
    }
...
export function getClinicalProtocolTemplate(icd10Code, specialty) {
 ...
 */
export function synthesizeClinicalDiary(request) {
    ...
    const surfacesList = request.surfaces && request.surfaces.length > 0 ? request.surfaces : ["occlusal"];
    const surfaceNamesRu = surfacesList
        .map((s) => {
        if (s === "occlusal")
            return "окклюзионная";
        if (s === "vestibular")
            return "вестибулярная";
        if (s === "oral")
            return "оральная/язычная";
        if (s === "mesial")
            return "медиальная";
        if (s === "distal")
            return "дистальная";
    ...
    const anesthDrugInfo = statutoryAnestheticDrugLabels[anesthDrugKey] || statutoryAnestheticDrugLabels.septanest_1_100000;
    ...
    const anesthTechniqueName = anesthTechnique === "infiltration"
        ? "инфильтрационная"
 ...
 */
export function synthesizeDiariesFromOdontogram(teeth, doctorInfo, baseDateStr) {
    const diaries = [];
    const validTeeth = teeth.filter((t) => isValidFdiToothNumber(t.toothNumber));
    for (const tooth of validTeeth) {
        let icdCode = "K02.1";
        const status = tooth.statusCode;
 ...
 */
export function validateForm043uCompliance(input) {
    const issues = [];
    const missingBlocks = [];
    // Проверяем, передан ли дневник или полная карта 043/у
    const isExplicitCard = Boolean(input &&
    ...
    const isSingleDiary = Boolean(input &&
        !isExplicitCard &&
    ...
    const rawDiaries = fullCard?.visitDiaries ??
            ...
            : Array.isArray(input?.soapDiaries)
                ...
                : Array.isArray(input)
                    ...
                    : singleDiary
                        ? [singleDiary]
                        : []);
    const diariesToCheck = Array.isArray(rawDiaries) ? rawDiaries : [];
    ...
    for (let i = 0; i < diariesToCheck.length; i++) {
        ...
        if (icdClean.startsWith("K04")) {
            const hasEndoKeywords = protocolLower.includes("канал") ||
        ...
        const isSurgeryExtraction = icdClean === "K08.1" &&
        ...
        if (isSurgeryExtraction) {
            const hasSurgeryKeywords = protocolLower.includes("кюретаж") ||
 ...
 */
export function generateEmrAutopilotPlan(request) {
    const toothNumStr = String(request.toothNumber || "16").trim();
    const surfaces = request.surfaces && request.surfaces.length > 0 ? request.surfaces : ["occlusal"];
    const blackClass = request.cavityClass || deduceBlackCavityClassFromSurfaces(toothNumStr, surfaces);
    const template = getClinicalProtocolTemplate(request.icd10Code, request.doctorSpecialty || undefined);
...
export function synthesizeFullOdontogramAutopilot(request) {
    ...
    for (const tooth of validTeeth) {
        ...
        else if (status === "crown_metal_ceramic" ||
        ...
        const surfaces = tooth.surfaces && tooth.surfaces.length > 0 ? tooth.surfaces : ["occlusal"];
    ...
    const avgScore = autopilotItems.length > 0
...
export const ORDER_804N_PROTOCOL_DEFINITIONS = {
    "A16.07.002.001": {
        code: "A16.07.002.001",
 ...
 */
export function synthesizeProtocolFromOrder804nService(code804n, options) {
 ...
 */
export function enrichDiaryFrom804nServices(existingDiary, services804n, options) {
