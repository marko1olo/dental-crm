 ...
 */
import { z } from "zod";
import { recallPrioritySchema, recallStatusSchema, } from "../communications/recallCascade.js";
 ...
 */
export const DEFAULT_RECALL_INTERVALS_DAYS = {
    hygiene_recall: 180,
    implant_check: 90,
    ortho_adjustment: 30,
    caries_control: 180,
 ...
 */
export const RECALL_TYPE_LABELS_RU = {
 ...
 */
export function calculateNextRecallDate(baseDate, recallType, customDays) {
 ...
 */
export function filterDueRecalls(recalls, targetDate = new Date(), overdueOnly = false) {
    const targetIso = targetDate.toISOString().slice(0, 10);
    const activeStatuses = new Set(["pending", "contacted_no_answer", "snoozed"]);
 ...
 */
export function formatRecallMessage(recall, clinicName = "Стоматологическая клиника ДЕНТЕ") {
 ...
 */
export function canTransitionRecallStatus(current, target) {
    if (current === target)
        return true;
    const transitions = {
