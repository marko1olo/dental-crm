 ...
 * ============================================================================
 */
import { GENERAL_CLEANING_PRESETS, getCabinetReadinessPreset, } from "./sanpinJournalsPresets.js";
export const STATUTORY_STERILIZATION_REGIMES = [
    {
        id: "steam_134_5min",
...
export const STATUTORY_CHAMBER_5_POINTS = [
    {
        pointIndex: 1,
        code: "КТ-1",
...
export const DEFAULT_CLINIC_LEGAL = {
 ...
 */
export function calculatePsoSampleRequirements(batchCount, isCriticalSurgical = false) {
...
export function evaluatePsoTrialResult(params) {
    ...
    if (!params.isAzopyramNegative) {
        return {
            ...
            rejectionReason: "БРАК: Положительная азопирамовая проба (фиолетово-синее окрашивание — обнаружена скрытая кровь / гемоглобин). Вся партия подлежит повторной дезинфекции и предстерилизационной очистке!",
    ...
    if (!params.isPhenolphthaleinNegative) {
        return {
            ...
            minSampleRequired: minSampleCount,
            samplingSatisfied: true,
            rejectionReason: "БРАК: Положительная фенолфталеиновая проба (розово-малиновое окрашивание — остатки щелочных компонентов моющих средств). Вся партия подлежит повторному ополаскиванию дистиллированной водой!",
    ...
    if (params.isSudanNegative === false) {
        return {
            ...
            minSampleRequired: minSampleCount,
            samplingSatisfied: true,
            rejectionReason: "БРАК: Положительная проба с Суданом III (масляные/жировые загрязнения наконечников). Партия направляется на повторное обезжиривание!",
...
export function generatePsoRecordId(dateStr = new Date().toISOString().slice(0, 10), seq = Math.floor(100 + Math.random() * 900)) {
 ...
 */
export function evaluateCycleParameters(regimeId, sensors) {
    const regime = STATUTORY_STERILIZATION_REGIMES.find((r) => r.id === regimeId) || STATUTORY_STERILIZATION_REGIMES[0];
    const failureReasons = [];
    // Температура
    const isTempCompliant = sensors.actualTemperatureCelsius >= regime.tempToleranceCelsius.min &&
    ...
    if (!isTempCompliant) {
        failureReasons.push(`Температура вне нормы: ${sensors.actualTemperatureCelsius}°C (норма ${regime.tempToleranceCelsius.min}–${regime.tempToleranceCelsius.max}°C, отклонение ${tempDelta > 0 ? `+${tempDelta}` : tempDelta}°C)`);
    }
    // Давление
    ...
    if (regime.methodType === "steam_autoclave") {
        ...
        if (!isPressureCompliant) {
            failureReasons.push(`Давление пара вне нормы: ${sensors.actualPressureBar} атм/бар (норма ${regime.pressureToleranceBar.min}–${regime.pressureToleranceBar.max} атм, отклонение ${pressureDelta > 0 ? `+${pressureDelta}` : pressureDelta} атм)`);
        }
    }
    // Время экспозиции
    ...
    if (!isTimeCompliant) {
        failureReasons.push(`Недостаточная экспозиция: ${sensors.actualExposureMinutes} мин (требуется не менее ${regime.exposureTimeMinutes} мин)`);
    }
 ...
 */
export function evaluate5ChamberPoints(points) {
    if (points.length === 0) {
        return {
...
export function createDefault5ChamberPoints(indicatorTradeNameRu = "Интеграл-134 (Класс 5)", allPassed = true) {
...
export function generateForm257RecordId(date, cycleNumber, sterilizerCode) {
...
export function createForm257Record(params) {
    ...
    const failureReasons = [...cycleCompliance.failureReasons];
...
// ─────────────────────────────────────────────────────────────────────────────
export function calculateLampOperatingHours(currentOperatingHours, sessionDurationMinutes, maxHours = 8000) {
    ...
    const remainingPercent = Number(Math.max(0, Math.min(100, (remainingHours / maxHours) * 100)).toFixed(1));
...
export function calculateAirDecontaminationDuration(roomVolumeM3, productivityM3PerHour, targetEfficiencyPercent = 99) {
    const vol = Math.max(1, Number(roomVolumeM3) || 1);
    const prod = Math.max(1, Number(productivityM3PerHour) || 1);
    let k = 4.6;
    if (targetEfficiencyPercent === 95)
        k = 2.3;
    if (targetEfficiencyPercent === 99.9)
        k = 6.9;
...
export function evaluateLampFleetHealth(equipments) {
    ...
    for (const eq of equipments) {
        ...
        if (res.lampStatus === "expired_replace_now")
            expiredCount++;
        else if (res.lampStatus === "warning_replace_soon")
            warningCount++;
        else
            normalCount++;
    }
...
// ─────────────────────────────────────────────────────────────────────────────
export function calculateNextGeneralCleaningDate(lastCleaningDate, roomType = "therapeutic") {
...
export function validateCleaningScheduleCompliance(scheduledDate, actualDateTime, roomType = "therapeutic") {
...
// ─────────────────────────────────────────────────────────────────────────────
export function calculateDisinfectantSolutionMath(concentrateLiters, targetConcentrationPercent) {
...
export function calculateRequiredConcentrateForVolume(desiredSolutionVolumeLiters, targetConcentrationPercent) {
 ...
 */
export function generateForm257PrintHtml(records, clinicInfo = DEFAULT_CLINIC_LEGAL, periodLabelRu = "за текущий отчетный период") {
...
export function exportPsoJournalToCsv(records) {
    const headers = [
        "ID записи",
        "Дата и время",
...
export function exportForm257ToCsv(records) {
    const headers = [
        "ID Записи",
        "Дата",
        "Номер цикла",
...
export function exportBactericidalJournalToCsv(sessions) {
    const headers = [
        "ID",
        "Дата",
        "ID оборудования",
...
export function exportGeneralCleaningJournalToCsv(records) {
    const headers = [
        "ID",
        "План дата",
        "Факт дата",
...
export function exportDisinfectantJournalToCsv(records) {
    const headers = [
        "ID",
        "Дата и время",
        "Тип операции",
 ...
 */
export function evaluateCabinetReadiness(params) {
    ...
    if (!params.surfaceDisinfection.isCompleted) {
        ...
        missingItems.push(`Недостаточная экспозиция дезинфекции: ${params.surfaceDisinfection.exposureMinutes} мин (требуется >= ${preset.minExposureMinutes} мин для ${preset.shortLabelRu})`);
    }
    ...
    if (!params.sterileTray.isCompleted) {
        ...
        const trayMissing = [];
        if (!params.sterileTray.mirrorReady)
            trayMissing.push("зеркало");
        if (!params.sterileTray.probeReady)
            trayMissing.push("зонд");
        if (!params.sterileTray.tweezersReady)
            trayMissing.push("пинцет");
        if (!params.sterileTray.excavatorReady)
            trayMissing.push("экскаватор");
        if (!params.sterileTray.spatulaPluggerReady)
            trayMissing.push("гладилка-штопфер");
    ...
    if (preset.requiresCofferdam) {
        if (!params.isolationCofferdam.isCompleted && !params.isolationCofferdam.isNotRequiredForProfile) {
            ...
            const coffMissing = [];
            if (!params.isolationCofferdam.rubberDamSheetReady)
                coffMissing.push("латексный/силиконовый платок");
            if (!params.isolationCofferdam.clampsReady)
                coffMissing.push("клампы (2A, W8A)");
            if (!params.isolationCofferdam.forcepsReady)
                coffMissing.push("щипцы для клампов");
...
export function generateCabinetReadinessId(dateStr = new Date().toISOString().slice(0, 10), cabinetNumber = "1", seq = Math.floor(100 + Math.random() * 900)) {
    const cleanDate = dateStr.replace(/[^0-9]/g, "").slice(0, 8);
...
export function exportCabinetReadinessToCsv(records) {
    const headers = [
        "ID Записи",
        "Дата и время",
        "Кабинет",
...
export function exportTemperatureHumidityJournalToCsv(records) {
    const headers = [
        "ID",
        "Дата замера",
        "Период",
...
export function integerToRussianWords(num) {
    ...
    if (thousands % 10 === 1 && thousands % 100 !== 11)
        thousandPrefix = thousandPrefix.replace(/один$/, "одна");
    if (thousands % 10 === 2 && thousands % 100 !== 12)
        thousandPrefix = thousandPrefix.replace(/два$/, "две");
    if (rest === 0)
        return `${thousandPrefix} ${thousandWord}`;
    return `${thousandPrefix} ${thousandWord} ${integerToRussianWords(rest)}`;
}
export { integerToRussianWords as numberToRussianWords };
export function formatRussianSheetsCount(count) {
 ...
 */
export function generateSanpinConsolidatedInspectionHtml(data) {
 ...
 */
export function exportSanpinConsolidatedArchiveToCsv(data) {
    const clinic = data.clinicInfo || DEFAULT_CLINIC_LEGAL;
    const license = clinic.licenseNumber || "№ ЛО41-01137-77/00368421";
    const volume = data.volumeNumber || clinic.volumeNumber || 1;
    const periodLabel = data.periodLabelRu
