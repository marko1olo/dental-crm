 ...
 * ============================================================================
 */
import { generateSanpinDataMatrixSvg, } from "./barcodeGenerators.js";
import { getKraftMaterialDefinition, getKraftSizeDefinition, } from "./kraftPackageTypes.js";
/**
 ...
 */
export function calculatePackageExpiration(packDateInput, packageType, referenceDateInput = new Date()) {
 ...
 */
export function evaluateKraftPackageStatus(expDateInput, isBreached = false, referenceDateInput = new Date()) {
    if (isBreached) {
        return "recalled";
    }
 ...
 */
export function sanitizeForThermalPrinter(text) {
    return text
        .replace(/[\r\n\t]+/g, " ")
        .replace(/"/g, "'")
        .trim();
 ...
 */
export function generateTsplLabel(record, options = {}) {
 ...
 */
export function generateZplLabel(record, options = {}) {
    const size = options.size || "58x40";
    const copies = Math.max(1, options.copies || 1);
    const clinicName = options.clinicName || "DENTE CLINIC";
    const sanitizedTool = sanitizeForThermalPrinter(record.toolSetNameRu);
 ...
 */
export function generateThermalStickerHtml(record, options = {}) {
    const size = options.size || "58x40";
 ...
 */
export function generateA4BatchSheetHtml(records, options = {}) {
    const clinicName = options.clinicName || "Стоматологическая клиника «DENTE»";
