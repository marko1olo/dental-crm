...
function computeItemSignature(item) {
...
function generateDeterministicFiscalSign(seed) {
...
function compileShiftZReport(shiftNumber, openedAtIso, closedAtIso, receipts, docNumber, options) {
    let incomeCount = 0;
    let incomeCashKop = 0;
    let incomeCardKop = 0;
    let incomeSbpKop = 0;
...
function reconcileWithBankingRegistry(fiscalReceipts, bankRegistry) {
    if (!bankRegistry || bankRegistry.length === 0) {
        ...
        return {
            ...
            summaryText: fiscalElectronicKop === 0
 ...
 */
export function processOfflineFiscalBatch(queueItems, options = {}) {
    ...
    if (validItemsToProcess.length > 0) {
        ...
        for (const { item, signature, timestampMs } of validItemsToProcess) {
            ...
            if (timestampMs - shiftOpenedMs >= maxShiftDuration && shiftReceipts.length > 0) {
                ...
                const zReport = compileShiftZReport(currentShiftNumber, shiftOpenedAtIso, shiftClosedAtIso, shiftReceipts, currentFiscalDocNumber++, options);
                batchShifts.push({
