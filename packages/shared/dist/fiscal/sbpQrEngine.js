 ...
 */
function formatEmvTlv(tag, value) {
 ...
 */
export function generateDynamicSbpQrPayload(params) {
    const sumKopecks = rubToKopecks(params.sumRub);
    if (sumKopecks <= 0) {
        throw new Error(`Сумма динамического QR-кода СБП должна быть строго больше 0 копеек (получено: ${params.sumRub} ₽ / ${sumKopecks} коп.)`);
    }
    ...
    const emvDataWithoutCrc = formatEmvTlv("00", "01") + // Payload Format Indicator
        formatEmvTlv("01", "12") + // Dynamic QR Code (12 = dynamic, 11 = static)
        formatEmvTlv("26", formatEmvTlv("00", "ru.nspk.sbp") +
            formatEmvTlv("01", qrId) +
            formatEmvTlv("02", merchantId)) + // Merchant Account Info (NSPK SBP)
        ...
        formatEmvTlv("62", formatEmvTlv("01", orderId.slice(0, 25)) + formatEmvTlv("08", purpose.slice(0, 25))) + // Additional Data Field
        "6304"; // Checksum Tag
 ...
 */
export function calculateSbpMultiTenderSplit(draft) {
