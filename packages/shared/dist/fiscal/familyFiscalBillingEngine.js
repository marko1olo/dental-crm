...
import { calculateSbpMultiTenderSplit, } from "./sbpQrEngine.js";
export const FAMILY_RELATIONSHIP_RU = {
    self: "Лично (плательщик)",
    spouse: "Супруг / Супруга",
    parent: "Родитель (мать / отец)",
    child: "Ребенок / подопечный (до 18 / 24 лет)",
};
export const FAMILY_RELATIONSHIP_FNS_CODE = {
    self: "1",
    spouse: "2",
    parent: "3",
    child: "4",
 ...
 */
export function resolveDentalTaxDeductionCategory(serviceName, code804n) {
    const lower = serviceName.toLowerCase();
    const normalizedCode = code804n.trim().toUpperCase();
    // Minzdrav Order 804n codes for surgery & implantology
    if (normalizedCode.startsWith("A16.07.054") || // Дентальная имплантация
 ...
 */
export function compileFamilyBillingDraft(draft) {
    ...
    const membersSummary = Array.from(memberMap.values()).map((m) => ({
    ...
    const taxDeductionCertificates = membersSummary.map((m, idx) => {
