 ...
 */
const allowedHttpUrlProtocols = new Set([
    "http:",
    "https:",
]);
...
export const documentKindSourceMetadata = {
    ...
    treatment_plan: {
        ...
        sourceNote: "Услуги, этапы и суммы берутся из приёма и прайс-листа клиники, а не набираются вручную: в плане будет ровно то, что записано в карточке.",
        sourceCheckedAt: documentSourceCheckedAt,
    },
    treatment_plan_acceptance: {
        sourceStatus: "clinic_template",
    ...
    orthodontic_medical_card_043_1u: {
        sourceStatus: "official_form",
        sourceAuthority: "Минздрав России",
        sourceReference: "Приказ Минздрава России / СтАР, форма N 043-1/у (ортодонтическая карта)",
        sourceNote: "DENTE формирует ортодонтическую карту с антропометрией, цефалометрией ТРГ, расчетом индексов Тона, Пона, Болтона и планом аппаратурного лечения.",
        sourceCheckedAt: documentSourceCheckedAt,
    },
    daily_dentist_diary_037u: {
        sourceStatus: "official_form",
        sourceAuthority: "Минздрав СССР / РФ",
        sourceReference: "Приказ Минздрава СССР от 25.01.1988 N 50, форма N 037/у-88",
        sourceNote: "DENTE формирует ежедневный листок учета принятых пациентов с автоматическим подсчетом выработанных УЕТ.",
        sourceCheckedAt: documentSourceCheckedAt,
    },
    summary_dentist_statement_039u: {
        sourceStatus: "official_form",
        sourceAuthority: "Минздрав СССР / РФ",
        sourceReference: "Приказ Минздрава СССР N 50-88 / Приказ Минздрава РФ N 804н",
        sourceNote: "DENTE автоматически агрегирует объем стоматологической помощи и выработку УЕТ за отчетный период по номенклатуре Минздрава.",
        sourceCheckedAt: documentSourceCheckedAt,
    },
    medical_record_extract: {
        sourceStatus: "official_workflow",
    ...
    radiation_dose_sheet: {
        ...
        sourceReference: "СанПиН 2.6.1.1192-03, СанПиН 2.6.1.2523-09 (НРБ-99/2009)",
        sourceNote: "DENTE ведет индивидуальный радиационный паспорт пациента и рассчитывает суммарную эффективную дозу облучения с контролем безопасных порогов.",
        sourceCheckedAt: documentSourceCheckedAt,
    },
    lab_work_order: {
        sourceStatus: "clinic_template",
...
export const FDI_TOOTH_NUMBER_MESSAGE = "Недопустимый номер зуба. Система FDI: 11–18, 21–28, 31–38, 41–48 (постоянные), 51–55, 61–65, 71–75, 81–85 (молочные), 91–98 (сверхкомплектные).";
...
export const documentPayloadSchema = z
    ...
    summaryDentistStatement039u: summaryDentistStatement039uPayloadSchema.optional(),
 ...
 */
export function calculateDmsGuaranteeSplit(letter, items, options = {}) {
    const lineResults = [];
    let totalBillKop = 0;
    ...
    for (const it of items) {
        ...
        let candidateCoveredKop = Math.max(0, Math.round(lineTotalKop * (pct / 100)) - franchiseDeductionKop);
