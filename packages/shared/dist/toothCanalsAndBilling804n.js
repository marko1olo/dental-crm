 ...
 */
export const ORDER_804N_INSTRUMENTATION = {
    1: {
        code: "A16.07.030.001",
 ...
 */
export const ORDER_804N_OBTURATIONS = {
    1: {
        code: "A16.07.008.001",
 ...
 */
export const ORDER_804N_ENDODONTIC_PACKAGES = {
    1: {
        code: "A16.07.008.001",
 ...
 */
export const ORDER_804N_MEDICATION_CAOH2 = {
    code: "A16.07.091",
...
export const ORDER_804N_UNSEALING = {
    code: "A16.07.082",
 ...
 */
export function getAnatomicalRootCanalCount(fdiNumber, clinicalCanalCount) {
    if (typeof clinicalCanalCount === "number" &&
...
export function getCanalCountForTooth(fdiNumber, clinicalCanalCount) {
    const parsedFdi = typeof fdiNumber === "number"
        ? fdiNumber
 ...
 */
export function isMultiRootedTooth(fdiNumber) {
 ...
 */
export function getEndodonticOrder804nPair(canalCount) {
    const count = Math.max(1, Math.min(4, canalCount));
 ...
 */
export function getOrder804nEndoProcedureForTooth(fdiNumber, explicitCanalCount) {
    const count = (explicitCanalCount !== undefined && explicitCanalCount >= 1 && explicitCanalCount <= 4
        ? explicitCanalCount
 ...
 */
export function calculateEndodonticCompositeTreatment(fdiNumber, options) {
    const count = (options?.clinicalCanalCount !== undefined &&
...
// ─────────────────────────────────────────────────────────────────────────────
import { parseKopecks, sumKopecks, multiplyKopecks } from "./utils/money.js";
 ...
 */
export function getOrder804nServicesForClinicalCase(options) {
    const items = [];
    ...
    const canalCount = validFdi
        ...
        : (canalOverride ? Math.min(4, Math.max(1, Math.round(canalOverride))) : 1);
    const icd = (options.icd10Code || "").trim().toUpperCase();
    const isMultiSurface = Boolean(options.surfaces && options.surfaces.length >= 2);
    const createItem = (catalogItem, isMandatory = true, quantity = 1, canalsOverride) => {
    ...
    if (icd.startsWith("K04")) {
        ...
        if (options.endoVisitStage === "access_instrumentation_temporary_calcium") {
            ...
            items.push(createItem(isMultiSurface
                ? ORDER_804N_THERAPY_CATALOG.compositeFillingMultiSurfaces
                : ORDER_804N_THERAPY_CATALOG.compositeFilling1Surface, true));
        }
        else {
 ...
 */
export function calculateOrder804nBillingEstimate(options) {
