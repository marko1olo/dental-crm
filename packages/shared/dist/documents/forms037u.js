...
export function calculateDaily037uTotals(records, standardShiftQuota = 21.0) {
    let adults = 0;
    let children = 0;
    let adolescents = 0;
    let ruralCount = 0;
    ...
    for (const r of records) {
        ...
        if (isPrim)
            ...
            repeat += 1;
        if (r.isSanated || r.isSanatedInVisit)
            sanated += 1;
    ...
    const execPct = standardShiftQuota > 0 ? Number(((roundedUet / standardShiftQuota) * 100).toFixed(1)) : 100.0;
    return {
