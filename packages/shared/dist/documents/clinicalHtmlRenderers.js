...
function renderFdiToothFormulaTable(dentalFormula) {
...
export function renderForm043uHtml(payload) {
...
export function renderForm043_1uHtml(payload) {
 ...
 */
export function renderForm037uHtml(payload) {
    ...
    const rawRecords = payload.patientRecords || payload.patients || payload.records || [];
    ...
    let sumAnesthesia = 0;
    let sumUet = 0;
    const patientRows = rawRecords.map((p, idx) => {
 ...
 */
export function renderForm039uHtml(payload) {
    const clinicName = payload.organization?.fullName || payload.clinicLegalName || payload.clinic?.name || "Стоматологическая поликлиника";
    const department = payload.clinicDepartment || payload.department || "Стоматологическое отделение";
    ...
    const fillingsCaries = m.fillingsCariesTotal ?? (payload.fillingsByBlackClass ? Object.values(payload.fillingsByBlackClass).reduce((a, b) => Number(a) + Number(b), 0) : 0);
    ...
    const calendarRows = [];
    const customDays = payload.calendarDays || payload.days || payload.dailyBreakdown || [];
    for (let day = 1; day <= 31; day++) {
        const existing = customDays.find((d) => d.day === day || d.date?.endsWith(`-${String(day).padStart(2, "0")}`));
        if (existing) {
            calendarRows.push({
                day,
 ...
 */
export function renderForm003vuHtml(payload) {
    const clinicName = payload.organization?.fullName || payload.clinicLegalName || payload.clinic?.name || "Стоматологическая клиника";
    ...
    const stages = payload.treatmentStagesTimeline || payload.treatmentStages || payload.chronology || [];
    const stagesRows = stages.map((st, idx) => {
 ...
 */
export function renderRadiationDoseSheetHtml(payload) {
    const clinicName = payload.organization?.fullName || payload.clinicLegalName || payload.clinic?.name || "Стоматологическая клиника";
    const clinicAddress = payload.organization?.address || payload.clinicAddress || "";
    const licenseNumber = payload.clinicLicenseNumber || "ЛО-77-01-012345";
    const patientName = payload.patient?.fullName || payload.patientFullName || "—";
    ...
    const exposures = payload.exposureEntries || payload.exposures || payload.radiationStudies || [];
    ...
    const exposureRows = exposures.map((ex, idx) => {
...
function renderUkepDigitalSignatureBlock(ukep) {
...
export function renderForm107_1uHtml(payload) {
...
export function renderForm148_1u88Html(payload) {
    const clinicName = payload.clinicLegalName || payload.organization?.fullName || "Стоматологическая клиника";
    const clinicAddress = payload.clinicAddress || payload.organization?.address || "—";
    const clinicPhone = payload.clinicPhone || payload.organization?.phone || "—";
    const clinicOgrn = payload.clinicOgrn || payload.organization?.ogrn || "—";
...
export function renderForm148_1u04lHtml(payload) {
    const clinicName = payload.clinicLegalName || payload.organization?.fullName || "Стоматологическая клиника";
    const clinicAddress = payload.clinicAddress || payload.organization?.address || "—";
    const clinicPhone = payload.clinicPhone || payload.organization?.phone || "—";
    const clinicOgrn = payload.clinicOgrn || payload.organization?.ogrn || "—";
...
export function renderPrescriptionUniversalHtml(payload) {
...
export function renderRadiologyReferralHtml(payload) {
    const clinicName = payload.clinicLegalName || payload.organization?.fullName || "Стоматологическая клиника";
    const clinicAddress = payload.clinicAddress || payload.organization?.address || "—";
    const clinicPhone = payload.clinicPhone || payload.organization?.phone || "—";
    ...
    const targetTeethArray = targetTeeth.split(/[,;\s]+/).map((t) => Number.parseInt(t.trim(), 10));
