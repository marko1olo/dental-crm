...
export const facialMorphologicalTypeLabels = {
...
export function calculateTonnIndex(upperIncisors, lowerIncisors, isDeciduous = false) {
...
export function calculatePontIndex(sumUpperIncisorsMm, measuredPremolarWidthMm, measuredMolarWidthMm) {
    ...
    const pDesc = diffP < -0.5 ? `сужение в области премоляров на ${Math.abs(diffP)} мм` : diffP > 0.5 ? `расширение в премолярах на ${diffP} мм` : "норма в премолярах";
    const mDesc = diffM < -0.5 ? `сужение в области моляров на ${Math.abs(diffM)} мм` : diffM > 0.5 ? `расширение в молярах на ${diffM} мм` : "норма в молярах";
    return {
...
export function calculateBoltonIndex(upperInput, // массив 12 ширины или sumUpper6
lowerInput, // массив 12 ширины или sumLower6
overallUpperSum, overallLowerSum) {
    let upper6 = 0;
    let lower6 = 0;
    let upper12 = 0;
    let lower12 = 0;
