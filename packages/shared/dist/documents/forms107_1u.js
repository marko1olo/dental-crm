...
export const PRESCRIPTION_DOSAGE_FORMS_CATALOG = [
    {
        code: "tablets",
        nameRu: "таблетки",
...
export const PRESCRIPTION_ADMINISTRATION_ROUTES_CATALOG = [
    {
        code: "per_os",
...
export const DENTAL_PRESCRIPTION_DRUG_CATALOG = [
    // ── НПВС и анальгетики ──
    {
        id: "nimesulide_100",
...
export const CONTROLLED_DRUG_PRESETS = DENTAL_PRESCRIPTION_DRUG_CATALOG.filter((d) => d.category === "controlled_pku");
/** Выделенные пресеты для льготных бланков 148-1/у-04(л) */
export const PREFERENTIAL_DRUG_PRESETS = DENTAL_PRESCRIPTION_DRUG_CATALOG.filter((d) => d.category === "preferential_somatic" || d.category === "antibiotic" || d.category === "nsaid");
export const DENTAL_DRUG_DOSAGE_LIMITS = {
    nimesulide_100: {
...
export const DENTAL_DRUG_INTERACTION_RULES = [
    {
        drugA: "nsaid",
        drugB: "nsaid",
        severity: "major",
...
export function evaluatePrescriptionPharmacologicalSafety(params) {
    ...
    for (const id of drugIds) {
        ...
        if (d)
            identifiedDrugs.push(d);
    }
    for (const item of items) {
        const match = DENTAL_PRESCRIPTION_DRUG_CATALOG.find((x) => x.tradeNameRu === item.tradeName || x.latinRp === item.latinName || item.id.includes(x.id));
    ...
    for (const drug of identifiedDrugs) {
        ...
        if (limits?.pediatricMinAgeYears && age < limits.pediatricMinAgeYears) {
            ageContraindications.push(`Препарат «${drug.tradeNameRu}» (${drug.activeSubstanceRu}) противопоказан пациентам в возрасте до ${limits.pediatricMinAgeYears} лет (текущий возраст: ${age} лет).`);
        }
    }
    ...
    for (let i = 0; i < identifiedDrugs.length; i++) {
        for (let j = i + 1; j < identifiedDrugs.length; j++) {
            ...
            for (const rule of DENTAL_DRUG_INTERACTION_RULES) {
                const matchDirect = (rule.drugA === d1.id || rule.drugA === d1.category) &&
                    (rule.drugB === d2.id || rule.drugB === d2.category);
                const matchReverse = (rule.drugA === d2.id || rule.drugA === d2.category) &&
    ...
    const hasContraindications = ageContraindications.length > 0 ||
...
export function verifyPrescriptionStatutoryValidity(prescription, referenceDateIso) {
    const errors = [];
    const warnings = [];
    ...
    if (prescription.items.length === 0) {
        ...
        errors.push(`Превышено максимальное количество препаратов для формы ${form === "148-1u-88" ? "№ 148-1/у-88 (максимум 1)" : "№ 107-1/у (максимум 3)"}: выписано ${prescription.items.length}.`);
    }
...
export function generatePrescriptionPayloadFromSoap(options) {
    ...
    const seriesNum = options.customSeriesNumber ||
    ...
    if (requestedDrugIds && requestedDrugIds.length > 0) {
        selectedDrugs = DENTAL_PRESCRIPTION_DRUG_CATALOG.filter((d) => requestedDrugIds.includes(d.id));
    }
    else {
        selectedDrugs = DENTAL_PRESCRIPTION_DRUG_CATALOG.filter((d) => d.recommendedForIcd10.some((code) => icd.startsWith(code)));
    ...
    const drugItems = selectedDrugs.map((d, index) => ({
...
export function generateForm148_1u88Payload(options) {
    ...
    const drug = DENTAL_PRESCRIPTION_DRUG_CATALOG.find((d) => d.id === options.explicitDrugId) ||
