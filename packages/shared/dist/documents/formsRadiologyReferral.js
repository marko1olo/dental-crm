import { z } from "zod";
import { dentalRadiologyStudyTypeSchema, } from "./radiationDoseSheet.js";
/**
 * ═══════════════════════════════════════════════════════════════════════════
...
export const radiologyReferralGoalLabels = {
...
export function generateRadiologyReferralPayloadFromSoap(options) {
    ...
    const referralNum = options.customReferralNumber ||
        `НАПР-ЛД-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;
    // Автоопределение типа исследования и цели по МКБ-10
    let determinedStudy = options.studyType || "cbct_jaw_8x8";
    let determinedGoal = options.studyGoal || "endodontics";
