 ...
 */
export function createCarpuleQueueItem(rawBarcode, options = {}) {
 ...
 */
export function sortQueueByFefo(items) {
 ...
 */
export function groupQueueByBatch(items) {
 ...
 */
export function calculateQueueStats(items) {
    let totalCostRub = 0;
    let expiredCount = 0;
 ...
 */
export function validateQueueForDisposal(items, allowExpired = false) {
    const errors = [];
    const warnings = [];
    if (items.length === 0) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (item.isExpired) {
            if (!allowExpired) {
                errors.push(`Препарат "${item.drugInfo?.tradeName ?? item.gtin}" (серия ${item.series ?? "—"}) просрочен (${item.expirationDate}). Списание для лечения запрещено.`);
            }
            else {
                warnings.push(`Внимание: списывается просроченный препарат "${item.drugInfo?.tradeName ?? item.gtin}" (${item.expirationDate}).`);
            }
        }
        else if (item.isExpiringSoon) {
            warnings.push(`Препарат "${item.drugInfo?.tradeName ?? item.gtin}" истекает в ближайшие дни (${item.expirationDate}).`);
        }
    }
    return {
...
export function buildDisposalParamsFromQueue(options) {
    const disposalItems = options.items.map((it) => ({
        sgtin: it.sgtin,
        gtin: it.gtin,
