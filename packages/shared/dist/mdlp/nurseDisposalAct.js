...
function tripletToWords(num, isFem = false) {
 ...
 */
export function amountToRussianWords(amount) {
    if (amount === 0)
 ...
 */
export function carpulesQuantityToRussianWords(qty, unit = "карпула") {
    if (qty === 0)
        return "0 (ноль) единиц";
...
export function formatSeniorNurseDisposalActData(options) {
    const now = new Date();
    const actNumber = options.actNumber ??
    ...
    let totalCost = 0;
    const actItems = options.items.map((it, idx) => {
    ...
    return {
        ...
        basisRu: "Приказ МЗ РФ № 804н, ФЗ № 425-ФЗ, Постановление Правительства РФ № 1556 (Схема МДЛП 10560)",
 ...
 */
export function generateSeniorNurseDisposalActHtml(actData) {
    const rowsHtml = actData.items
      ...
      <tr>
        ...
        <td style="font-size: 10px;">
          ${it.patientFullName ? `Пациент: ${it.patientFullName}` : ""}${it.visitNumber ? `<br/>${it.visitNumber}` : ""}<br/>
...
function getMonthNameRu(isoDate) {
