 ...
 */
export function escapeMdlpXml(unsafe) {
    return escapeXml(unsafe);
}
/**
 * Validates parameters for MDLP Schema 10560.
 */
export function validateMdlpSchema10560Params(params) {
    const errors = [];
    ...
    if (!params.items || params.items.length === 0) {
        ...
        params.items.forEach((it, idx) => {
            if (!it.sgtin || it.sgtin.trim().length === 0) {
                ...
                errors.push(`Позиция #${idx + 1}: некорректная длина SGTIN "${it.sgtin}" (требуется >= 20 символов).`);
            }
 ...
 */
export function generateMdlpSchema10560Payload(params, options = {}) {
    ...
    const opDate = params.operationDate instanceof Date
 ...
 */
export function parseMdlpSchema10560Xml(xml) {
 ...
 */
export function safeParseMdlpSchema10560Xml(xml) {
