...
export interface FiscalShiftSummaryRecord {
	...
	// Итого выручка по смене (Наличные + Карты + СБП + Зачет аванса - Возвраты)
	readonly shiftRevenueTotalRub: number;
	readonly shiftRevenueTotalKopecks: number;

	// Наличность в кассе на конец смены
	readonly cashInDrawerEndRub?: number | undefined;
}

export interface BankReconciliationSummary {
	readonly totalCardAndSbpKktRub: number; // ККТ безнал (Карты + СБП)
	readonly totalCardAndSbpKktKopecks: number;
	readonly totalBankStatementRub: number; // Выписка банка
	readonly totalBankStatementKopecks: number;
	readonly bankAcquiringFeeRub: number; // Комиссия банка (эквайринг + СБП)
	readonly bankAcquiringFeeKopecks: number;
	readonly netBankDepositRub: number; // Поступило на р/с за вычетом комиссии
	readonly netBankDepositKopecks: number;
	readonly discrepancyRub: number; // Расхождение (ККТ безнал - Выписка банка)
 ...
 */
export function calculateFiscalPeriodStatementTotals(
	shifts: readonly FiscalShiftSummaryRecord[],
	bankStatementTotalRub?: number | undefined,
	bankAcquiringFeeRub?: number | undefined,
): {
	...
	const totals: FiscalPeriodStatementTotals = {
...
function formatRubCurrency(rub: number): string {
 ...
 */
export function generateFiscalPeriodStatementHtml(data: FiscalPeriodStatementData): string {
	const req: ClinicFiscalRequisites = {
		...DEFAULT_CLINIC_FISCAL_REQUISITES,
		...(data.clinicRequisites || {}),
	};

	const { totals, bankReconciliation } = calculateFiscalPeriodStatementTotals(
		data.shifts,
		data.bankStatementTotalRub,
		data.bankAcquiringFeeRub,
	);
 ...
 */
export function exportFiscalPeriodStatementToCsv(data: FiscalPeriodStatementData): string {
	const req: ClinicFiscalRequisites = {
		...DEFAULT_CLINIC_FISCAL_REQUISITES,
		...(data.clinicRequisites || {}),
	};
