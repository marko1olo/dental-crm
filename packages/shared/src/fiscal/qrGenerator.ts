...
function polyRest(dividend: Uint8Array, divisor: Uint8Array): Uint8Array {
...
// Version parameters (Capacity & EC Block structure for Versions 1-10)
interface VersionEcBlock {
	readonly totalDataBytes: number;
	readonly ecBytesPerBlock: number;
	readonly group1Blocks: number;
	readonly group1DataBytesPerBlock: number;
	readonly group2Blocks: number;
	readonly group2DataBytesPerBlock: number;
	readonly alignmentPatterns: readonly number[];
}

const VERSION_TABLE: Record<number, Record<QrErrorCorrectionLevel, VersionEcBlock>> = {
	1: {
...
const FORMAT_INFO: Record<QrErrorCorrectionLevel, number[]> = {
 ...
 */
function encodeData(text: string, version: number, ecLevel: QrErrorCorrectionLevel): Uint8Array {
 ...
 */
function createEcBlocks(data: Uint8Array, version: number, ecLevel: QrErrorCorrectionLevel): Uint8Array {
 ...
 */
function findSmallestVersion(text: string, ecLevel: QrErrorCorrectionLevel): number {
...
class QrMatrix {
	...
	set(r: number, c: number, val: boolean, reserved = true): void {
	...
	placeFinderPattern(row: number, col: number): void {
		for (let r = -1; r <= 7; r++) {
			for (let c = -1; c <= 7; c++) {
				const rPos = row + r;
				const cPos = col + c;
	...
	placeAlignmentPattern(row: number, col: number): void {
		for (let r = -2; r <= 2; r++) {
			for (let c = -2; c <= 2; c++) {
	...
	placeDataBits(data: Uint8Array, mask: number): void {
		let bitIdx = 0;
		let row = this.size - 1;
		let col = this.size - 1;
		let dir = -1; // upwards

		const isMasked = (r: number, c: number): boolean => {
			switch (mask) {
	...
	applyFormatInfo(ecLevel: QrErrorCorrectionLevel, mask: number): void {
		const formatBits = FORMAT_INFO[ecLevel][mask]!;
		// Top-left
		for (let i = 0; i < 15; i++) {
			const bit = ((formatBits >>> (14 - i)) & 1) === 1;
			// 0..5 -> (8, i), 6 -> (8, 7), 7 -> (8, 8), 8 -> (7, 8), 9..14 -> (14-i, 8)
			if (i <= 5) this.modules[8]![i] = bit;
			else if (i === 6) this.modules[8]![7] = bit;
 ...
 */
export function generateQrCodeDataUri(text: string, options: QrSvgOptions = {}): string {
