 ...
 */
const allowedHttpUrlProtocols: ReadonlySet<string> = new Set([
	"http:",
	"https:",
]);
...
const fnsKnd1151156OrderSourceUrl =
...
const minzdravMedicalDocumentReleaseSourceUrl =
	"https://publication.pravo.gov.ru/Document/View/0001202009240027";
const minzdravAmbulatoryFormsSourceUrl =
	"https://publication.pravo.gov.ru/document/0001202505300033";
const minzdravRadiologyRulesSourceUrl =
...
const dateLikeStringErrorMessage =
 ...
 *
 * ЗАЧЕМ ЭТО ПЕРЕЕХАЛО В ОБЩИЙ КОНТРАКТ. Набор и схема жили ТОЛЬКО на сервере, в
 * `apps/api/src/routes/odontogram.ts`, поэтому клиент физически не мог соблюдать
 * правило, которым сервер его проверяет. Следствие измерено ревизией: смета
 * начала отправлять `toothNumber`, клиент фильтровал его лишь через
 * `Number.isFinite`, и номер 19 — не существующий зуб — проходил проверку
 * клиента, после чего сервер отклонял ВЕСЬ план лечения общим сообщением. Врач
 * терял всю смету из-за одной опечатки в одной строке.
 *
 * Скопировать список на клиент было нельзя: скопированный список расходится, и
 * в этом же проекте скопированное вручную объединение разделов уже разошлось и
 * сломало сборку веба. Поэтому определение здесь одно, а обе стороны его
 * ВЫВОДЯТ. Ошибиться теперь можно только в одном месте.
 *
 * Диапазон `min(11).max(99)`, стоявший здесь раньше, пропускал 19, 20, 29, 30,
 * 39, 40, 49, 50, 56–60, 66–70, 76–80 и 86–99 — ни одно из этих чисел зубом не
 * является. Опечатка «49» вместо «48» сохранялась, попадала в план лечения со
 * стоимостью, но не показывалась в одонтограмме: врач видел строку без зуба.
 */
...
const visitDraftParserProfiles: Record<
	DentalSpecialty,
	VisitDraftParserProfile
> = {
	therapist: {
		complaintTokens: [
			"кари",
		...
		objectiveFallback:
			"Объективно уточнить зуб, полость/реставрацию, перкуссию, зондирование, реакцию на холод и данные снимка.",
		diagnosisFallback: null,
		planFallback:
			"План: уточнить диагноз, согласовать анестезию, изоляцию, лечение кариеса/эндо и контроль окклюзии.",
		reviewHints: [
	...
	orthopedist: {
		...
		objectiveFallback:
			"Объективно уточнить опоры, прикус, окклюзионные контакты, состояние пародонта и снимки опорных зубов.",
		diagnosisFallback: null,
		planFallback:
			"План: диагностика опор, фотопротокол/сканирование, варианты ортопедической конструкции, временный этап и фиксация.",
		reviewHints: [
	...
	surgeon: {
		...
		objectiveFallback:
			"Объективно уточнить отек, слизистую, подвижность, перкуссию, КТ/снимок и отношение к анатомическим структурам.",
		diagnosisFallback: null,
		planFallback:
			"План: анестезия, хирургический этап, гемостаз/швы, рекомендации, контрольный осмотр.",
		reviewHints: [
	...
	orthodontist: {
		...
		objectiveFallback:
			"Объективно уточнить прикус, классы, скученность, средние линии, ВНЧС, ОПТГ/ТРГ/сканы.",
		diagnosisFallback: null,
		planFallback:
			"План: ортодонтическая диагностика, фотопротокол, ОПТГ/ТРГ/скан, варианты брекеты/элайнеры и ретенция.",
		reviewHints: [
	...
	periodontist: {
		...
		objectiveFallback:
			"Объективно уточнить индексы налета/кровоточивости, глубину карманов, рецессии, подвижность и снимки.",
		diagnosisFallback: null,
		planFallback:
			"План: пародонтальная карта, профессиональная гигиена/скейлинг, домашняя гигиена, recall и контроль.",
		reviewHints: [
	...
	hygienist: {
		...
		objectiveFallback:
			"Объективно уточнить налет, камень, пигментацию, состояние десны, индексы гигиены и зоны риска.",
		diagnosisFallback: null,
		planFallback:
			"План: профгигиена, полировка, реминерализация по показаниям, обучение домашней гигиене и recall.",
		reviewHints: [
	...
	pediatric: {
		...
		objectiveFallback:
			"Объективно уточнить зубы молочного/постоянного прикуса, поведение ребенка, налет, кариес и слизистую.",
		diagnosisFallback: null,
		planFallback:
			"План: адаптация, лечение/профилактика по показаниям, инструкции родителю, контроль.",
		reviewHints: [
	...
	implantologist: {
		...
		objectiveFallback:
			"Объективно уточнить зону адентии, объем кости по КТ, слизистую, соседние зубы, окклюзию и риски.",
		diagnosisFallback: null,
		planFallback:
			"План: КТ-анализ, варианты имплантации/костной пластики, сроки этапов, согласие и хирургический протокол.",
		reviewHints: [
	...
	radiologist: {
		...
		objectiveFallback:
			"Описание исследования: указать тип снимка, область, качество, ключевые находки и ограничения.",
		diagnosisFallback:
			"Описание снимка требует клинической корреляции лечащим врачом.",
		planFallback:
			"План: передать описание лечащему врачу, сопоставить с жалобами и клиническим осмотром.",
		reviewHints: [
	...
	universal: {
		...
		objectiveFallback:
			"Объективно уточнить зубную формулу, слизистую, пародонтальный статус, гигиену, прикус и снимки по показаниям.",
		diagnosisFallback: null,
		planFallback:
			"План: маршрутизация по специальностям, снимки/документы по показаниям, согласование приоритетов лечения.",
		reviewHints: [
...
function dentalTermPattern(source: string): RegExp {
	return new RegExp(
		`(?<![0-9A-Za-zА-Яа-яЁё])(?:${source})(?![0-9A-Za-zА-Яа-яЁё])`,
		"gi",
	);
}
...
const spokenAnatomicToothMap: Array<[RegExp, string]> =
	spokenAnatomicToothQuadrants.flatMap(
		({ archPattern, sidePattern, quadrant }) =>
			spokenAnatomicToothPositions.flatMap(([position, positionPattern]) => {
				...
				return [
					[
						dentalTermPattern(
							String.raw`${archPattern}\s+${sidePattern}\s+(?:${positionPattern})`,
						),
						replacement,
					],
					[
						dentalTermPattern(
							String.raw`${sidePattern}\s+${archPattern}\s+(?:${positionPattern})`,
						),
						replacement,
					],
					[
						dentalTermPattern(
							String.raw`(?:${positionPattern})\s+${archPattern}\s+${sidePattern}`,
						),
						replacement,
					],
...
function applyTrackedReplacement(
	text: string,
	pattern: RegExp,
	replacement: string,
	label: string,
...
function applyTrackedPhraseReplacement(
	text: string,
	phrase: string,
	replacement: string,
	label: string,
...
function lineStartsWithSection(
	value: string,
...
// ─────────────────────────────────────────────────────────────
// PHASE 25: ORTHODONTIC CEPHALOMETRICS & CRANIOFACIAL ANALYSIS (ТРГ)
// ─────────────────────────────────────────────────────────────

export const sagittalSkeletalClassSchema = z.enum([
	"class_1_orthognathic",
	"class_2_retrognathic_mandible",
	"class_2_prognathic_maxilla",
	"class_3_prognathic_mandible",
	"class_3_retrognathic_maxilla",
]);
export type SagittalSkeletalClass = z.infer<typeof sagittalSkeletalClassSchema>;

export const verticalFacialBiotypeSchema = z.enum([
	"dolichofacial_hyperdivergent",
	"mesofacial_neutral",
	"brachyfacial_hypodivergent",
]);
export type VerticalFacialBiotype = z.infer<typeof verticalFacialBiotypeSchema>;

export const orthodonticApplianceTypeSchema = z.enum([
	"brackets_roth_022",
	"brackets_mbt_022",
	"brackets_damon_q_passive_self_ligating",
	"clear_aligners_sequential",
	"functional_twin_block",
	"functional_forsus_spring",
	"rapid_palatal_expander_marpe",
	"orthognathic_surgery_bimaxillary",
]);
export type OrthodonticApplianceType = z.infer<typeof orthodonticApplianceTypeSchema>;

export const orthodonticExtractionProtocolSchema = z.enum([
	"non_extraction_expansion_ipr",
	"extract_14_24_34_44_bimaxillary_first_premolars",
	"extract_14_24_maxillary_only",
	"extract_34_44_mandibular_only",
	"extract_18_28_38_48_third_molars",
	"asymmetric_therapeutic_extraction",
]);
export type OrthodonticExtractionProtocol = z.infer<typeof orthodonticExtractionProtocolSchema>;
...
export interface CephalometricLandmarksInput {
	S: Point2D;
	N: Point2D;
	Ba?: Point2D | undefined;
	Po: Point2D;
	Or: Point2D;
	ANS: Point2D;
	PNS: Point2D;
...
export interface CephalometricAnalysisResult {
	steiner: {
		...
		snGoGnDeg: number;
		u1NaAngleDeg: number;
		u1NaDistanceMm: number;
		l1NbAngleDeg: number;
		l1NbDistanceMm: number;
		interincisalAngleDeg: number;
		steinerUpperLipDistanceMm: number;
		steinerLowerLipDistanceMm: number;
	};
	tweed: {
		fmaDeg: number;
		impaDeg: number;
		fmiaDeg: number;
		triangleSumDeg: number;
	};
	wits: {
		witsAoBoMm: number;
	};
	classifications: {
		skeletalClass: SagittalSkeletalClass;
		facialBiotype: VerticalFacialBiotype;
		growthVector: "neutral" | "horizontal_clockwise" | "vertical_counter_clockwise";
	};
...
export class CephalometricEngine {
	/**
	 * Вычисляет угол между векторами v1->v2 и v1->v3 в градусах.
	 */
	public static calculate3PointAngle(v1: Point2D, v2: Point2D, v3: Point2D): number {
		const ax = v2.x - v1.x;
		const ay = v2.y - v1.y;
		const bx = v3.x - v1.x;
		const by = v3.y - v1.y;
		...
		const cosVal = Math.max(-1.0, Math.min(1.0, dot / (magA * magB)));
		return Number(((Math.acos(cosVal) * 180) / Math.PI).toFixed(2));
	}

	/**
	 * Вычисляет угол между двумя направленными векторами.
	 */
	public static calculate2VectorAngle(u: Point2D, v: Point2D): number {
		const dot = u.x * v.x + u.y * v.y;
		const magU = Math.hypot(u.x, u.y);
		const magV = Math.hypot(v.x, v.y);
		if (magU === 0 || magV === 0) return 0;
		const cosVal = Math.max(-1.0, Math.min(1.0, dot / (magU * magV)));
		return Number(((Math.acos(cosVal) * 180) / Math.PI).toFixed(2));
	}

	/**
	 * Вычисляет знак и величину перпендикулярного расстояния от точки P0 до прямой P1->P2.
	 */
	public static calculateSignedPerpendicularDistanceMm(
		p0: Point2D,
		p1: Point2D,
		p2: Point2D,
		...
		const numerator = dx * (p1.y - p0.y) - dy * (p1.x - p0.x);
		const distPx = numerator / length;
		return Number((distPx * scaleMmPerPixel).toFixed(2));
	}

	/**
	 * Wits appraisal: проекция точек A и B на окклюзионную плоскость.
	 */
	public static calculateWitsAppraisalMm(
		ptA: Point2D,
		ptB: Point2D,
		occAnt: Point2D,
		occPost: Point2D,
		...
		const vx = occAnt.x - occPost.x;
		const vy = occAnt.y - occPost.y;
		const mag = Math.hypot(vx, vy);
		if (mag === 0) return 0;
		const ux = vx / mag;
		const uy = vy / mag;

		const dotA = (ptA.x - occPost.x) * ux + (ptA.y - occPost.y) * uy;
		const dotB = (ptB.x - occPost.x) * ux + (ptB.y - occPost.y) * uy;

		const witsDeltaPx = dotA - dotB;
		return Number((witsDeltaPx * scaleMmPerPixel).toFixed(2));
	}

	/**
	 * Полный математический анализ ТРГ по Steiner, Tweed и Jacobson.
	 */
	public static runFullAnalysis(
		lm: CephalometricLandmarksInput,
		scaleMmPerPixel: number,
	): CephalometricAnalysisResult {
		const snaDeg = this.calculate3PointAngle(lm.N, lm.S, lm.A);
		const snbDeg = this.calculate3PointAngle(lm.N, lm.S, lm.B);
		const anbDeg = Number((snaDeg - snbDeg).toFixed(2));

		const vSN = { x: lm.N.x - lm.S.x, y: lm.N.y - lm.S.y };
		const vGoGn = { x: lm.Gn.x - lm.Go.x, y: lm.Gn.y - lm.Go.y };
		const snGoGnDeg = this.calculate2VectorAngle(vSN, vGoGn);

		const vU1 = { x: lm.U1_tip.x - lm.U1_apex.x, y: lm.U1_tip.y - lm.U1_apex.y };
		const vNA = { x: lm.A.x - lm.N.x, y: lm.A.y - lm.N.y };
		const u1NaAngleDeg = this.calculate2VectorAngle(vU1, vNA);
		const u1NaDistanceMm = this.calculateSignedPerpendicularDistanceMm(
			lm.U1_tip,
			lm.N,
			lm.A,
			scaleMmPerPixel,
		...
		const vL1 = { x: lm.L1_tip.x - lm.L1_apex.x, y: lm.L1_tip.y - lm.L1_apex.y };
		const vNB = { x: lm.B.x - lm.N.x, y: lm.B.y - lm.N.y };
		const l1NbAngleDeg = this.calculate2VectorAngle(vL1, vNB);
		const l1NbDistanceMm = this.calculateSignedPerpendicularDistanceMm(
			lm.L1_tip,
			lm.N,
			lm.B,
			scaleMmPerPixel,
		...
		const u1L1VectorAngle = this.calculate2VectorAngle(vU1, vL1);
		const interincisalAngleDeg = Number((180.0 - u1L1VectorAngle).toFixed(2));

		// Steiner S-Line
		const sLineMidNose: Point2D = {
			x: (lm.Prn.x + lm.Sn.x) / 2,
			y: (lm.Prn.y + lm.Sn.y) / 2,
		};
		const steinerUpperLipDistanceMm = this.calculateSignedPerpendicularDistanceMm(
			lm.Ls,
			lm.Pog_s,
			sLineMidNose,
			scaleMmPerPixel,
		);
		const steinerLowerLipDistanceMm = this.calculateSignedPerpendicularDistanceMm(
			lm.Li,
			lm.Pog_s,
			sLineMidNose,
			scaleMmPerPixel,
		...
		// Tweed Triangle
		const vFH = { x: lm.Or.x - lm.Po.x, y: lm.Or.y - lm.Po.y };
		const vGoMe = { x: lm.Me.x - lm.Go.x, y: lm.Me.y - lm.Go.y };
		const fmaDeg = this.calculate2VectorAngle(vFH, vGoMe);
		const impaDeg = this.calculate2VectorAngle(vL1, vGoMe);
		const fmiaDeg = Number((180.0 - (fmaDeg + impaDeg)).toFixed(2));

		// Wits Appraisal
		const witsAoBoMm = this.calculateWitsAppraisalMm(
			lm.A,
			lm.B,
			lm.Occ_ant,
			lm.Occ_post,
		...
		// Classifications
		let skeletalClass: SagittalSkeletalClass;
		if (anbDeg >= 0 && anbDeg <= 4.0 && witsAoBoMm >= -2.0 && witsAoBoMm <= 1.0) {
			skeletalClass = "class_1_orthognathic";
		} else if (anbDeg > 4.0 || witsAoBoMm > 1.0) {
			skeletalClass =
				snaDeg > 84.0
					? "class_2_prognathic_maxilla"
					: "class_2_retrognathic_mandible";
		...
		let facialBiotype: VerticalFacialBiotype;
		if (fmaDeg > 30.0 || snGoGnDeg > 36.0) {
			facialBiotype = "dolichofacial_hyperdivergent";
		} else if (fmaDeg < 20.0 || snGoGnDeg < 28.0) {
			facialBiotype = "brachyfacial_hypodivergent";
		} else {
			facialBiotype = "mesofacial_neutral";
		}
		...
		interp.push(
			`Скелетный класс: ${
				skeletalClass === "class_1_orthognathic"
					? "I класс (Нормогнатия)"
					: skeletalClass.startsWith("class_2")
						? "II класс (Дистальный прикус)"
						: "III класс (Мезиальный прикус)"
			} (ANB=${anbDeg}°, Wits=${witsAoBoMm} мм).`,
		);
		interp.push(
			`Вертикальный биотип: ${
				facialBiotype === "dolichofacial_hyperdivergent"
					? "Долихофациальный (гипердивергентный)"
					: facialBiotype === "brachyfacial_hypodivergent"
						? "Брахифациальный (гиподивергентный)"
						: "Мезофациальный (нейтральный)"
			} (FMA=${fmaDeg}°, SN-GoGn=${snGoGnDeg}°).`,
		);
		interp.push(
			`Резцы: U1-NA=${u1NaAngleDeg}° / ${u1NaDistanceMm} мм, IMPA=${impaDeg}°, L1-NB=${l1NbAngleDeg}° / ${l1NbDistanceMm} мм, Межрезцовый угол=${interincisalAngleDeg}°.`,
		);
		...
		return {
			steiner: {
				...
				l1NbAngleDeg,
				l1NbDistanceMm,
				interincisalAngleDeg,
				steinerUpperLipDistanceMm,
				steinerLowerLipDistanceMm,
			},
			tweed: {
				fmaDeg,
				impaDeg,
				fmiaDeg,
				triangleSumDeg: Number((fmaDeg + impaDeg + fmiaDeg).toFixed(2)),
			},
			wits: {
				witsAoBoMm,
			},
			...
			clinicalInterpretation: interp.join(" "),
		};
	}
}
...
// ─────────────────────────────────────────────────────────────
// PHASE 27: ENDODONTICS, CANAL CURVATURE (SCHNEIDER) & NITI FILE FATIGUE LIFECYCLE
// ─────────────────────────────────────────────────────────────

export const nitiKinematicsSchema = z.enum(["continuous_rotary", "reciprocating"]);
export type NiTiKinematics = z.infer<typeof nitiKinematicsSchema>;

export const nitiFileStatusSchema = z.enum([
	"active",
	"warning_near_fatigue",
	"locked_disposed",
	"separated_quarantine",
]);
export type NiTiFileStatus = z.infer<typeof nitiFileStatusSchema>;

export class SchneiderCurvatureCalculator {
	public static calculateAngleAndRadius(params: {
		orificePoint: Point2D;
		departurePoint: Point2D;
		apicalPoint: Point2D;
	}): {
		schneiderAngleDeg: number;
		curvatureRadiusMm: number;
		riskCategory: "straight" | "moderate" | "severe";
		hasAbruptBend: boolean;
	} {
		const { orificePoint: O, departurePoint: P, apicalPoint: A } = params;

		const vAx = P.x - O.x;
		const vAy = P.y - O.y;
		const magA = Math.hypot(vAx, vAy) || 1;

		const vBx = A.x - P.x;
		const vBy = A.y - P.y;
		const magB = Math.hypot(vBx, vBy) || 1;

		const dot = (vAx * vBx + vAy * vBy) / (magA * magB);
		const clampedDot = Math.max(-1, Math.min(1, dot));
		const angleRad = Math.acos(clampedDot);
		const schneiderAngleDeg = Number(((angleRad * 180) / Math.PI).toFixed(2));

		const arcLengthMm = magB;
		const sinHalfAngle = Math.sin((schneiderAngleDeg * Math.PI) / 360);
		const curvatureRadiusMm =
			sinHalfAngle > 0.001
				? Number((arcLengthMm / (2 * sinHalfAngle)).toFixed(2))
				: 99.0;

		let riskCategory: "straight" | "moderate" | "severe" = "straight";
		if (schneiderAngleDeg > 25) riskCategory = "severe";
		else if (schneiderAngleDeg >= 10) riskCategory = "moderate";

		const hasAbruptBend = curvatureRadiusMm < 4.0;
...
export class NiTiFileFatigueEngine {
	public static computeIncrementalFatigue(params: {
		durationSeconds: number;
		fatigueCapSeconds: number;
		schneiderAngleDeg: number;
		schneiderRadiusMm: number;
		kinematics: NiTiKinematics;
		temperatureCelsius?: number;
		isoTipSize: number;
		taper: number;
	}): {
		...
		const kappaKinematic = kinematics === "reciprocating" ? 0.4 : 1.0;
		const kappaThermal = temperatureCelsius >= 35 ? 1.25 : 1.0;
		const kappaSize = Math.pow(isoTipSize / 20, 1.5) * (taper / 0.04);

		const effectiveRadius = Math.max(1.5, schneiderRadiusMm);
		const curvatureFactor =
			Math.pow(schneiderAngleDeg / 20.0, 2) * (10.0 / effectiveRadius);

		const baseRatio = durationSeconds / Math.max(30, fatigueCapSeconds);

		const rawDeltaPhi =
			baseRatio * curvatureFactor * kappaKinematic * kappaThermal * kappaSize;
		const deltaPhi = Number(rawDeltaPhi.toFixed(4));
		const stressScore = Number(
			(curvatureFactor * kappaKinematic * kappaSize).toFixed(2),
		);

		return {
			deltaPhi,
			isOverstressed: stressScore > 3.0,
			stressScore,
		};
	}

	public static evaluateFileLifeStatus(
		currentPhi: number,
		deltaPhi: number,
	): {
		newPhi: number;
		newStatus: NiTiFileStatus;
		isLockoutRequired: boolean;
		lockoutReason: string | null;
	} {
		const newPhi = Number((currentPhi + deltaPhi).toFixed(4));

		if (newPhi >= 1.0) {
			return {
				newPhi,
				newStatus: "locked_disposed",
				isLockoutRequired: true,
				lockoutReason: `Исчерпан ресурс циклической усталости (Φ=${newPhi} >= 1.0000). Инструмент заблокирован от дальнейшего использования для предотвращения отлома.`,
			};
		}
		if (newPhi >= 0.85) {
			return {
				newPhi,
				newStatus: "warning_near_fatigue",
				isLockoutRequired: false,
				lockoutReason: `Критический уровень усталости (Φ=${newPhi} >= 0.8500). Рекомендуется утилизация после текущего зуба.`,
			};
		}
		return {
			newPhi,
