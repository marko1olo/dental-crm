 ...
 */
export function calculateEstimateTotals(items: readonly {
	readonly quantity: number;
	readonly unit_price_rub: number;
	readonly discount_rub?: number | undefined;
 ...
 */
export function computeEstimateDocumentHash(
	estimate: {
		readonly id: string;
		readonly estimate_number: string;
		readonly total_rub: number;
		readonly items: readonly { readonly id: string; readonly net_line_total_rub: number }[];
	},
	signature: {
		readonly signed_by_name: string;
		readonly signed_at_iso: string;
