...
export interface LoyaltyTierDefinition {
	...
	readonly cashbackPercent: number; // e.g. 3%, 5%, 10%
	readonly maxInvoiceCoveragePercent: number; // max % of invoice payable with points (e.g. 30%, 50%, 70%)
	readonly minLifetimeSpentKop: number; // in kopecks
 ...
 */
export function getLoyaltyTierDefinition(tierId: LoyaltyTierId): LoyaltyTierDefinition {
 ...
 */
export function calculateLoyaltyRedemption54Fz(
	...
	const distributedDiscounts = distributeDiscountProportionally(
		discountItemsInput,
		actualRedeemedPointsKop,
	);

	let eligibleIdx = 0;
	const lineItemsDiscounts: ItemDiscountSplitResult[] = input.items.map((item) => {
		const lineGross = Math.max(0, Math.round(item.priceKop * (item.quantity || 1)));
		if (item.isExcludedFromLoyalty) {
			return {
				id: item.id,
 ...
 */
export function validateLuhn16Certificate(formattedSerial: string): boolean {
