 ...
 */
export function generateCorrelationId(prefix = "cor"): string {
...
export function extractCorrelationId(
	...
	if ("get" in headers && typeof (headers as { get: unknown }).get === "function") {
		const fetchHeaders = headers as { get(name: string): string | null };
		const candidate =
			fetchHeaders.get(CORRELATION_ID_HEADER) ||
