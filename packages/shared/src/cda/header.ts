...
export function buildCdaRecordTarget(patient: PatientCdaInfo, clinicOid: string): string {
...
export function buildCdaClinicOrganization(clinic: ClinicCdaInfo): string {
...
export function buildCdaAuthor(doctor: DoctorCdaInfo, clinic: ClinicCdaInfo, effectiveTime: string): string {
...
export function buildCdaCustodian(clinic: ClinicCdaInfo): string {
	const clinicOid = clinic.oid || EGISZ_OIDS.FRMO_MO_ROOT;
	const clinicName = escapeXml(clinic.name);
...
export function buildCdaLegalAuthenticator(
	legalAuth: LegalAuthenticatorCdaInfo | undefined,
	doctor: DoctorCdaInfo,
	clinic: ClinicCdaInfo,
...
export function generateClinicalDocumentHeader(opts: CdaHeaderOptions): string {
	const {
		docTypeNsiCode,
		docTitle,
		templateOids,
