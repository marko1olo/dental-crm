...
describe("Унифицированные медицинские бланки Минздрава РФ (834н, 1094н, 1051н, 804н, ПП РФ 736)", () => {
	...
	describe("1. Форма № 043/у — Медицинская карта стоматологического больного", () => {
		...
		test("рендерит эталонный HTML Формы № 043/у с лицензией, одонтограммой и дневником Формы 043/у", () => {
			const payload = {
				organization: {
			...
			assert.ok(html.includes("4. Дневники клинического приёма (Форма 043/у)"), "Содержит официальный заголовок дневников Формы 043/у");
			assert.ok(!html.includes("4. Дневники клинического приёма (SOAP)"), "Не содержит западного акронима SOAP в официальном бланке 043/у");
		});
	});
