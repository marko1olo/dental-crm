...
describe("buildRuleBasedVisitDraftFromTranscript", () => {
	test("parses a full transcript with all sections correctly", () => {
		const transcript =
			"Жалобы: острая боль в зубе 4.5. Анамнез: боль появилась вчера. Объективно: глубокая кариозная полость. Диагноз: острый пульпит. План лечения: экстирпация пульпы.";
		...
		assert.ok(
			result.warnings.some((w) => w.includes("Распознаны зубы/сегменты: 45.")),
		);
	});
	...
	test("uses custom sourceLabel in warnings", () => {
		...
		assert.ok(
			result.warnings.some((w) =>
				w.includes(
					"My Custom Source: черновик собран по профилю специальности",
				),
			),
		);
	});
	...
	test("uses different specialty profile (surgeon)", () => {
		...
		assert.ok(
			result.warnings.some((w) => w.includes("Фокус приема: хирургия.")),
		);
