...
export function computeGtinCheckDigit(gtin13: string): number {
	if (!gtin13 || typeof gtin13 !== "string" || !/^\d{13}$/.test(gtin13)) {
		throw new Error(
			`Неверный формат входных данных для вычисления контрольной суммы GTIN: "${gtin13}". Ожидалось ровно 13 цифр.`,
		);
	}
	let sum = 0;
	for (let i = 0; i < 13; i++) {
 ...
 */
export function safeComputeGtinCheckDigit(gtin13: unknown): {
	isValid: boolean;
	checkDigit?: number;
	error?: string;
} {
 ...
 */
export function isValidGtinChecksum(gtin14: unknown): boolean {
 ...
 */
export function normalizeDataMatrixSeparators(raw: unknown): string {
...
export function parseMdlpDataMatrix(
	...
	if (!gtin) {
		...
		errors.push(
			`Неверный формат GTIN: "${gtin}". Должно быть 14 цифр.`,
		);
	} else {
	...
	if (!serialNumber) {
		errors.push(
			"Отсутствует обязательный идентификатор (21) серийного номера.",
		);
	} else if (serialNumber.length < 5 || serialNumber.length > 20) {
		warnings.push(
			`Нестандартная длина серийного номера (${serialNumber.length} симв.). Для МДЛП стандартно 13 симв.`,
		);
	}
	...
	if (!cryptoKey) {
		warnings.push(
			"Отсутствует криптоключ проверки (AI 91). Возможен сбой считывания.",
		);
	} else if (cryptoKey.length !== 4) {
		warnings.push(
			`Нестандартная длина криптоключа AI (91): ${cryptoKey.length} симв. (ожидалось 4).`,
		);
	}

	if (!cryptoSignature) {
		warnings.push(
			"Отсутствует криптохвост/подпись (AI 92). Требуется проверка источника.",
		);
	} else if (cryptoSignature.length !== 44) {
		warnings.push(
			`Нестандартная длина подписи AI (92): ${cryptoSignature.length} симв. (ожидалось 44).`,
		);
	}
	...
	if (expiryResult.isExpired) {
		warnings.push(
			`Внимание! Срок годности препарата истек: ${expiryResult.isoDate}. Запрещено к списанию.`,
		);
	} else if (expiryResult.isExpiringSoon) {
		warnings.push(
			`Предупреждение: Срок годности истекает менее чем через 90 дней (${expiryResult.isoDate}).`,
		);
	}
 ...
 */
export function formatDataMatrixForDisplay(barcode: string): string {
	const parsed = parseMdlpDataMatrix(barcode);
	if (!parsed.isValid) return barcode;
	const parts: string[] = [];
	if (parsed.gtin) parts.push(`(01)${parsed.gtin}`);
	if (parsed.serialNumber) parts.push(`(21)${parsed.serialNumber}`);
	if (parsed.expirationDateRaw) parts.push(`(17)${parsed.expirationDateRaw}`);
	if (parsed.series) parts.push(`(10)${parsed.series}`);
	if (parsed.cryptoKey) parts.push(`(91)${parsed.cryptoKey}`);
	if (parsed.cryptoSignature) parts.push(`(92)${parsed.cryptoSignature.slice(0, 8)}...`);
	return parts.join(" ");
}

export type SafeParseMdlpDataMatrixResult =
	| { success: true; data: MdlpParsedBarcode }
	| { success: false; data: MdlpParsedBarcode; error: string; errors: readonly string[] };
 ...
 */
export function safeParseMdlpDataMatrix(
	rawInput: unknown,
	referenceDate: Date = new Date(),
): SafeParseMdlpDataMatrixResult {
	try {
		...
		const fallbackData: MdlpParsedBarcode = {
			rawBarcode: typeof rawInput === "string" ? rawInput : "",
			gtin: "",
			serialNumber: "",
			cryptoKey: "",
