...
(async () => {
  ...
  const context = await browser.newContext({
    ...
    colorScheme: 'light' // We strictly test in Dark Mode as requested
  });
  ...
  await page.evaluate(() => {
    ...
    const themeBtn = btns.find(b => b.innerHTML.includes('lucide-sun') || b.innerHTML.includes('sun') || b.innerHTML.includes('Sun') || b.querySelector('svg.lucide-sun'));
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, 'odontogram_initial_layout_light.png'), fullPage: true });
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, 'odontogram_tooth46_popup_light.png'), fullPage: true });
  ...
  await page.screenshot({ path: path.join(OUTPUT_DIR, 'odontogram_tooth15_popup_light.png'), fullPage: true });
