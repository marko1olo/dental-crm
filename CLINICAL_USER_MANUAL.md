...
## 15. Детский прием и молочный прикус (Pediatric Mode)
- **Укороченные корни и сменные зубы:** При включении режима "Детский прикус" в карте одонтограммы отображается молочная зубная формула (зубы 51-55, 61-65, 71-75, 81-85). Корни молочных зубов анатомически укорочены, а постоянные зубы скрыты в челюсти до прорезывания, что снижает визуальный шум для детских стоматологов.

## 16. Групповой выбор зубов (Batch Actions & Multi-Select)
- **Ускорение работы терапевта:** Вместо прокликивания каждого зуба при тотальной санации или тотальном протезировании, врач может зажать Shift и кликнуть на несколько зубов. Вокруг зубов появится неоновое свечение. При выборе статуса в Radial Menu он применится сразу ко всей группе (например, статус "Здоров" или "Отсутствует" для нескольких зубов).

## 17. Интерактивная лента приемов (Patient Journey Timeline)
- **Glassmorphism Timeline:** В карточке пациента (ЭМК) вехи пациента отображаются в виде удобного вертикального таймлайна. Здесь собраны приемы, созданные зуботехнические заказы, медицинские алерты и финансовые транзакции. 
- На ПК лента выводится сбоку от зубной формулы, чтобы врач мог одновременно видеть текущее состояние рта и историю визитов пациента.

## 18. Smart Database Seeding (Посев данных при онбординге)
- **Мгновенный старт:** После прохождения стартового Онбординга система автоматически генерирует базовый прайс-лист и популярные шаблоны лечения (Установка имплантата, Лечение пульпита, Кариес) с уже прописанным протоколом и привязанными МКБ-10. Врачу не нужно вбивать все вручную с нуля.


## 19. Модуль стерилизации и СанПиН-контроля (Sterilization & Sanitization)
- **Стерильность лотков:** Система контролирует использование стерильных инструментов. К каждому клиническому приему (SOAP-дневнику) привязывается штрих-код стерилизационного лотка.
- **Интерфейс сканирования:** В окне SOAP-дневника встроена кнопка режима сканирования. При активации запускается симуляция штрих-код сканера. После считывания кода лотка (например, TRAY-8492-OK), он намертво сохраняется в БД в таблице дневников и связывается с логами автоклава для полной юридической прослеживаемости.

## 20. Семейные аккаунты и раздельный биллинг (Family Accounts & Shared Billing)
- **Семейные группы:** В клинике пациенты могут быть объединены в семейные группы под руководством главы семьи (например, родителя). Баланс семейного кошелька списывается атомарно для любого члена семьи.
- **Атомарное списание:** В окне оплаты инвойса при выборе метода «Семейный счет» транзакция списывает средства с общего семейного счета и закрывает счет ребенка в рамках одной транзакции PostgreSQL.

## 21. Маркетинговая воронка лидов и CRM-Канбан (CRM Leads Kanban Pipeline)
- **Канбан-доска:** Все входящие заявки и записи с сайта сперва попадают в воронку лидов (LeadsKanbanView), а не напрямую в календарь врачей, чтобы избежать замусоривания расписания.
- **Drag-and-Drop и Конвертация:** Оператор колл-центра перетаскивает карточки лидов по колонкам (Новые, В работе, Записаны, Не ответил). При переносе в «Записаны» открывается поповер назначения на прием. При выборе слота лид конвертируется в карточку Пациента и создается запись в Календаре в рамках одной транзакции.
