  ...
  if (stmt.getKind() === SyntaxKind.VariableStatement) {
    ...
    decs.forEach(dec => {
      ...
      if (stmt.getText().includes('useState')) {
        stateCount++;
      } else if (stmt.getText().includes('useCallback') || stmt.getText().includes('useMemo') || dec.getInitializer()?.getKind() === SyntaxKind.ArrowFunction) {
        functionCount++;
      }
...
console.log(`States: ${stateCount}, Effects: ${effectCount}, Functions: ${functionCount}`);
