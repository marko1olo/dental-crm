...
async function run() {
	...
	await page.screenshot({
		path: path.join(artifactDir, "screenshot_pc_light.png"),
	});

	await page.setViewportSize({ width: 390, height: 844 });
	await page.screenshot({
		path: path.join(artifactDir, "screenshot_mobile_light.png"),
	});
	...
	await page.screenshot({
		path: path.join(artifactDir, "screenshot_pc_dark.png"),
	});

	await page.setViewportSize({ width: 390, height: 844 });
	await page.screenshot({
		path: path.join(artifactDir, "screenshot_mobile_dark.png"),
	});
