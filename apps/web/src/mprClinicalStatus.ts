...
export function resolveMprClinicalPresetProjection<TProjection extends string>(
	...
	if (
		...
		availableProjections.includes(projection)
	)
		return projection;
	if (
		projection === "panoramic_reconstruction" &&
		availableProjections.includes("coronal" as TProjection)
	)
		return "coronal" as TProjection;
	if (availableProjections.includes("axial" as TProjection))
		return "axial" as TProjection;
	return availableProjections[0] ?? projection;
}

export function describeMprClinicalPresetProjectionFallback<
...
>(
	...
	const effectiveProjection = resolveMprClinicalPresetProjection(
		projection,
...
function effectiveMprClinicalPreset(
	...
	return {
		...preset,
		projection: resolveMprClinicalPresetProjection(
			preset.projection,
			input.availableProjections,
		),
	};
}

export function buildMprWorkbenchSummary(
	input: MprClinicalChecklistInput,
): string {
...
export function buildMprOperatorSummary(
	input: MprOperatorSummaryInput,
): MprOperatorSummaryCard[] {
	if (!input.canOpenMpr) {
		return [
			{
				id: "series",
				...
				detail: input.hasSeries
					? "Серия найдена, но КТ-срезы пока недоступны."
					: "Выберите готовую КЛКТ/КТ-серию.",
	...
	return [
		...
		},
		{
			...
			detail: input.protocolExact
				? "Опорный срез соответствует выбранному протоколу."
				: "Сверьте положение среза с клинической задачей.",
			tone: input.protocolExact ? "ready" : "attention",
		},
		{
			id: "sync",
...
function scoreMprClinicalPresetFit(
	input: MprClinicalPresetFitInput,
	preset: MprClinicalPresetFitTarget,
): number {
	return (
...
function describeMprClinicalPresetDelta(
	input: MprClinicalPresetFitInput,
	preset: MprClinicalPresetFitTarget,
): string[] {
	const deltas: string[] = [];
	...
	if (slabDelta !== 0)
		deltas.push(`слой ${formatSignedDelta(slabDelta, " мм")}`);
	...
	if (input.crosshair !== preset.crosshair)
		deltas.push(preset.crosshair ? "включить курсор" : "скрыть курсор");
	if (input.linkedPlanes !== preset.linkedPlanes)
		deltas.push(
			preset.linkedPlanes ? "связать плоскости" : "развязать плоскости",
		);
	return deltas;
}
...
export function findNearestMprClinicalPreset(
	...
	if (!input.canOpenMpr) {
		return {
			...
			deltas: [],
			label:
				"Выберите готовую КЛКТ/КТ-серию, чтобы увидеть ближайший протокол.",
		};
	}

	let bestPreset = presets[0];
	let bestEffectivePreset = bestPreset
		? effectiveMprClinicalPreset(input, bestPreset)
		: null;
	let bestScore = bestEffectivePreset
		? scoreMprClinicalPresetFit(input, bestEffectivePreset)
	...
	const projectionFallback =
		bestEffectivePreset.projection !== bestPreset.projection;
	return {
...
export function buildMprClinicalChecklist(
	input: MprClinicalChecklistInput,
): MprClinicalChecklistItem[] {
	return [
		{
			id: "series",
			title: "Серия",
			...
			action: input.canOpenMpr
				? "Можно настраивать плоскости."
				: "Проверьте серии и выберите готовую КЛКТ/КТ-серию.",
		...
		},
		{
			...
			detail: input.canOpenMpr
				? `${input.projectionLabel}; ${input.axisLabel}; слой ${input.slabMm} мм; ${input.sliceLabel}; ${input.windowLabel}. ${
						input.protocolExact
							? "Клинический протокол совпадает."
							: input.protocolLabel
					}`
		...
		},
		{
			...
			status: input.hasWorkbenchManifest
				? "ready"
				: input.canOpenMpr
					? "active"
					: "pending_review",
...
export function mprClinicalNextAction(items: MprClinicalChecklistItem[]) {
	return (
		items.find((item) => item.status !== "ready")?.action ??
