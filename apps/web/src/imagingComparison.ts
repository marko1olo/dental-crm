...
function normalizedImagingComparisonKey(
	value: string | null | undefined,
): string {
	return value?.trim().toLowerCase() ?? "";
}

export function imagingComparisonScore(
	...
	if (
		current.toothCode &&
		candidate.toothCode &&
		current.toothCode === candidate.toothCode
	)
		score += 6;
	if (
		normalizedImagingComparisonKey(current.region) &&
		normalizedImagingComparisonKey(current.region) ===
			normalizedImagingComparisonKey(candidate.region)
	) {
		score += 3;
	}
	...
	if (
		(current.kind === "cbct" &&
			(candidate.kind === "opg" || candidate.kind === "periapical")) ||
		(candidate.kind === "cbct" &&
			(current.kind === "opg" || current.kind === "periapical"))
	) {
		score += 2;
	}
...
export function imagingComparisonReason(
	...
	if (
		...
		return `тот же зуб ${candidate.toothCode}`;
	if (
		normalizedImagingComparisonKey(current.region) &&
		normalizedImagingComparisonKey(current.region) ===
			normalizedImagingComparisonKey(candidate.region)
	...
	if (current.kind === candidate.kind)
		return `тот же тип: ${kindLabel(candidate.kind)}`;
	if (
		(current.kind === "cbct" &&
			(candidate.kind === "opg" || candidate.kind === "periapical")) ||
		(candidate.kind === "cbct" &&
