...
function longestAnnotation(
	...
	for (const annotation of annotations) {
		...
		const length =
			polylineLengthMm(annotation.points) ?? annotation.points.length;
...
function reconstructionQualityWeight(renderPlan: DicomGpuRenderPlan | null) {
	const downsamplePenalty = renderPlan
		? clamp((renderPlan.downsampleFactor - 1) * 0.08, 0, 0.32)
		: 0;
	const hardwareWeight = renderPlan
		? clamp(renderPlan.hardwareQualityWeight, 0, 1)
		: 0.5;
	const weighted =
		qualityModeWeight(renderPlan) * 0.36 +
		gpuClassWeight(renderPlan) * 0.22 +
		interactionBudgetWeight(renderPlan) * 0.14 +
		hardwareWeight * 0.28;
...
function crossSectionStationPlan(
	curveLengthMm: number | null,
	crossSectionStepMm: number,
	canPlan: boolean,
	curvePointCount: number,
	...
	const requiredCount = Math.max(
		3,
		Math.ceil(curveLengthMm / crossSectionStepMm) + 1,
	);
	const count = Math.min(160, requiredCount);
	const rawCoverageMm =
		count >= requiredCount
			? curveLengthMm
			: Math.min(curveLengthMm, (count - 1) * crossSectionStepMm);
	const coverageMm = round2(rawCoverageMm);
	const coveragePercent = Math.min(
		100,
		Math.round((rawCoverageMm / curveLengthMm) * 100),
	);
	const middleMm = round1(
		Math.min(rawCoverageMm, Math.max(0, rawCoverageMm / 2)),
	);
...
): CtPlanningReconstructionStatus {
...
export function buildCtPlanningReconstructionPlan(input: {
	...
	const panoramicCurve = longestAnnotation(
		input.annotations,
		"panoramic_curve",
	);
	...
	const curveLengthMm = panoramicCurve
		? polylineLengthMm(panoramicCurve.points)
		: null;
	const qualityWeight = reconstructionQualityWeight(input.renderPlan);
	const curveSegmentCount = Math.max(0, curvePointCount - 1);
	const longestCurveSegmentMm = panoramicCurve
		? longestPolylineSegmentMm(panoramicCurve.points)
		: null;
	const spacingTargetMm = curveSpacingTargetMm(qualityWeight);
	const crossSectionStepMm = round1(clamp(5 - qualityWeight * 3.5, 1.5, 5));
	const slabMm = round1(
		clamp(Number.isFinite(input.slabMm) ? input.slabMm : 10, 3, 20),
	);
	const stationPlan = crossSectionStationPlan(
		curveLengthMm,
		crossSectionStepMm,
		input.canPlan,
		curvePointCount,
	...
	const curveQualityStatus: CtPlanningReconstructionStatus =
		panoramicStatus !== "ready"
			? panoramicStatus
			: longestCurveSegmentMm !== null &&
					longestCurveSegmentMm <= spacingTargetMm
				? "ready"
				: "draft";
	const crossSectionStatus: CtPlanningReconstructionStatus =
		crossSectionCount > 0 && crossSectionCoveragePercent >= 99
			? "ready"
			: input.canPlan && curvePointCount > 0
				? "draft"
				: "blocked";
	const status: CtPlanningReconstructionStatus =
		...
		curveQualityStatus === "ready"
			? "ready"
			: panoramicStatus === "draft" ||
					crossSectionStatus === "draft" ||
					curveQualityStatus === "draft"
				? "draft"
				: "blocked";
	...
	const cards: CtPlanningReconstructionCard[] = [
		{
			...
			nextAction: input.canPlan
				? "Оставить как рабочий профиль этого ПК."
				: "Сначала выбрать готовую КЛКТ/КТ-серию.",
		},
		{
			id: "opg-curve",
			title: "ОПТГ-дуга",
			...
			value:
				curveLengthMm === null
					? `${curvePointCount} точек`
					: `${curveLengthMm} мм`,
			detail:
				"Дуга хранится как кривая; тяжелые снимки остаются в просмотрщике КТ.",
			nextAction:
				panoramicStatus === "ready"
					? "Проверить положение поперечных срезов."
					: "Поставить минимум 3 точки по зубной дуге.",
		},
		{
			id: "curve-sampling",
			title: "Качество дуги",
		...
		},
		{
			...
			value:
				canalPointCount >= 3
					? `${canalPointCount} точек`
					: `${canalPointCount}/3`,
			detail:
				"Канал переносится отдельной кривой для проверки отступов и шаблона.",
			nextAction:
				canalStatus === "ready"
					? "Сверить отступы до оси импланта."
					: "Дорисовать трассу нижнечелюстного канала.",
		},
	];

	const warnings: string[] = [];
	if (longestCurveSegmentMm !== null && longestCurveSegmentMm > spacingTargetMm)
		warnings.push(
			"ОПТГ-дуга слишком редкая: добавьте контрольные точки перед панорамными срезами.",
		);
	if (!input.canPlan)
		warnings.push(
			"Для построения ОПТГ и поперечных срезов нужна готовая объемная КТ-серия.",
		);
	if (curvePointCount > 0 && curvePointCount < 3)
		warnings.push("ОПТГ-дуга начата, но минимум 3 точки еще не поставлены.");
	if (crossSectionCount >= 160)
		warnings.push(
			"Количество поперечных срезов ограничено, чтобы не перегружать слабый ПК.",
		);
	if (crossSectionRequiredCount > 160 && crossSectionCoveragePercent < 99)
		warnings.push(
			"Сетка поперечных срезов не покрывает всю дугу: сократите маршрут или увеличьте шаг в просмотрщике.",
		);
	if (qualityWeight < 0.36)
		warnings.push(
			"ПК в режиме минимальной нагрузки: показываем более редкую сетку производных срезов.",
		);
