...
export const activeScenarioRoutes: Record<
	string,
	CtPlanningExportScenarioRoute
> = {
	opg_curve: {
		owner: "doctor",
		ownerLabel: "врач",
...
function viewerPreset(
	...
	return {
		...
		slabMm,
		requiresVolume,
		restoreCommands: buildCtPlanningViewerRestoreCommands({
			projection,
			windowPreset,
			slabMm,
			requiresVolume,
...
export const activeScenarioViewerPresets: Record<
	string,
	CtPlanningExportScenarioViewerPreset
> = {
	opg_curve: viewerPreset(
...
const fallbackScenarioViewerPreset = viewerPreset(
	"axial",
	"аксиальный срез",
	"bone",
	"кость",
...
export function scenarioStatusCounts(
	artifacts: CtPlanningExportScenarioArtifact[],
): CtPlanningExportScenarioStatusCounts {
	...
	return artifacts.reduce<{
		...
		(issues, artifact) => {
			if (artifact.status === "draft" || artifact.status === "blocked") {
				issues[
					artifact.status === "draft" ? "draftArtifacts" : "blockedArtifacts"
				].push({
					id: artifact.id,
...
export function scenarioDetail(
	...
	const blockedDetail =
		counts.blocked > 0 ? `; требует действия ${counts.blocked}` : "";
...
export function scenarioAction(
	...
	if (nextArtifact)
		return nextArtifact.blocker ?? `Закрыть: ${nextArtifact.title}.`;
	...
	return missingArtifact
		? `Сначала закрыть: ${missingArtifact}.`
		: packet.nextAction;
}
...
export function buildCtPlanningExportScenarioSummary(
	...
	return {
		...
		title:
			activeScenarioLabels[packet.activeQuickActionId] ??
			packet.activeQuickActionId,
