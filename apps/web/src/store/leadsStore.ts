...
export const useLeadsStore = create<LeadsState>((set, get) => ({
	...
	fetchLeads: async () => {
		...
		try {
			...
			/*
			 * Форма ответа проверяется в рантайме, а не полагается на тип.
			 * `res.json()` возвращает `any`, поэтому присваивание в `leads: Lead[]`
			 * проверку типов проходит при ЛЮБОМ содержимом — typecheck был зелёным.
			 *
			 * Замерено в браузере 2026-08-10: при ответе не-массивом доска обращений
			 * падала целиком —
			 *   TypeError: leads.filter is not a function
			 *     LeadsKanbanView.tsx:438
			 *   [Dente] DENTE route failed: leads
			 * То есть вместо «обращений нет» оператор получал мёртвый раздел, и
			 * повторить загрузку было нечем: экран не рисовался.
			 *
			 * Приём взят у соседа, а не выдуман: так же проверяет ответ
			 * components/schedule/UrgentScheduleRequestsWidget.tsx:33-35, и его
			 * сообщение оператору тоже по-русски. Отказ уходит в общий catch ниже,
			 * который кладёт текст в `error` — доска покажет причину и кнопку
			 * «Повторить» вместо белого экрана.
			 */
			if (!Array.isArray(data)) {
				throw new Error(
					"Обращения не загружены: сервер вернул ответ неизвестного вида. Не считайте, что обращений нет — повторите, а если повторится, сообщите администратору.",
				);
			}
