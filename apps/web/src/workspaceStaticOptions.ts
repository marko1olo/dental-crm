...
export const telegramFeatureHelp: Record<DenteTelegramFeature, string> = {
	appointment_reminders:
		"Автоматические напоминания до визита без диагноза и деталей лечения.",
	appointment_confirmation:
		"Кнопки подтверждения, переноса и связи с администратором.",
	patient_linking:
		"Одноразовый код и QR для связи Telegram-чата с карточкой пациента.",
	pre_visit_intake:
		"Сбор административных данных до приема по короткому сценарию.",
	document_ready_notice:
		"Уведомление о готовности документа, сам файл остается в CRM и защищенном портале.",
	tax_document_request:
		"Статус подготовки налоговых документов без отправки PDF в чат.",
	payment_reminders:
		"Аккуратные напоминания о неоплаченных счетах и ссылках на портал.",
	post_visit_instructions:
		"Памятки после удаления, имплантации, пломбы, гигиены и других процедур.",
	recalls: "Возвратные приглашения на осмотр, гигиену и контроль.",
	review_requests:
		"Просьба оценить прием с клинической ссылкой на карту или профиль отзывов.",
	staff_daily_digest:
		"Сводка расписания и задач для сотрудников без персональных медицинских данных в тексте.",
	staff_task_alerts:
		"Служебные уведомления по очереди связи и готовности документов.",
	callback_requests:
		"Пациент может попросить звонок, задача попадает в очередь клиники.",
	voice_note_intake:
		"Голосовые обращения пока выключаются отдельно из-за риска лишних данных.",
	secure_portal_links:
		"Любые чувствительные файлы и подробности уходят только через портал.",
};

export const telegramFeatureOptions = Object.keys(
	telegramFeatureLabels,
) as DenteTelegramFeature[];
...
export type TelegramPostVisitCheckupDelayDrafts = Record<
...
export const defaultTelegramPostVisitCheckupDelayDrafts: TelegramPostVisitCheckupDelayDrafts =
	{
		extraction: String(
			defaultTelegramPostVisitCheckupDelayHoursByTopic.extraction,
		),
		implantation: String(
		...
		local_anesthesia: String(
			defaultTelegramPostVisitCheckupDelayHoursByTopic.local_anesthesia,
		),
		hygiene: String(defaultTelegramPostVisitCheckupDelayHoursByTopic.hygiene),
		prosthetics: String(
			defaultTelegramPostVisitCheckupDelayHoursByTopic.prosthetics,
		),
		orthodontics: String(
			defaultTelegramPostVisitCheckupDelayHoursByTopic.orthodontics,
		),
		periodontology: String(
			defaultTelegramPostVisitCheckupDelayHoursByTopic.periodontology,
		),
...
export const telegramPostVisitCheckupDelayFields: Array<{
...
}> = [
	{
		...
		help: "Контроль самочувствия после удаления зуба.",
	},
	{
		key: "implantation",
	...
	},
	{
		...
		label: "После пломбы",
		help: "Проверка завышения пломбы и дискомфорта.",
	},
	{
		key: "hygiene",
		label: "После гигиены",
		help: "Мягкое напоминание после профессиональной гигиены.",
	},
	{
		key: "endo",
		label: "После эндодонтии",
		help: "Контроль боли после лечения каналов.",
	},
	{
		key: "surgery",
		label: "После операции",
		help: "Хирургический контроль без диагноза в тексте.",
	},
	{
		key: "local_anesthesia",
		label: "После анестезии",
		help: "Короткий контроль после приема с анестезией.",
	},
	{
		key: "prosthetics",
		label: "После протезирования",
		help: "Проверка адаптации к конструкции.",
	},
	{
		key: "orthodontics",
		label: "После ортодонтии",
		help: "Контроль адаптации к аппарату или элайнерам.",
	},
	{
		key: "periodontology",
		label: "После пародонтологии",
		help: "Контроль десен и ухода.",
	},
	{ key: "other", label: "Другое", help: "Запасной срок для общих памяток." },
];

export const postVisitCareTopicOptions: Array<{
