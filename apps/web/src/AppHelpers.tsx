...
export function loadDocumentIssueSignatureDraft(
	...
	try {
		...
		const raw =
			...
			(organizationId
				? window.localStorage.getItem(documentIssueSignatureStorageKey)
				: null);
		...
		if (!localSavedAtFresh(savedAt, localConvenienceRetentionMs)) {
			...
			if (organizationId)
				window.localStorage.removeItem(documentIssueSignatureStorageKey);
			return fallback;
		}
		return {
			version: 1,
...
export function loadDocumentPaymentSelectionStore(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(documentPaymentSelectionStorageKey)
				: null);
		...
		if (pruned || organizationId) {
			...
			if (organizationId)
				window.localStorage.removeItem(documentPaymentSelectionStorageKey);
		}
		return { version: 1, selections };
	} catch (error) {
...
export function loadDocumentPayloadDraftStore(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(documentPayloadDraftStorageKey)
				: null);
		...
		if (pruned || organizationId) {
			...
			if (organizationId)
				window.localStorage.removeItem(documentPayloadDraftStorageKey);
		}
		return { version: 1, drafts };
	} catch {
		return emptyDocumentPayloadDraftStore();
...
export function loadLocalImagingViewerDraft(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(imagingViewerLocalKey(studyId))
				: null);
		if (!raw) return null;
		...
		if (
			...
			window.localStorage.removeItem(localKey);
			if (organizationId)
				window.localStorage.removeItem(imagingViewerLocalKey(studyId));
			return null;
		}
...
export function loadLocalDicomWorkbenchDraftFromLocalStorage(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(dicomWorkbenchLocalStorageKey)
				: null);
		if (!raw) return null;
		...
		if (!parsed) {
			window.localStorage.removeItem(localKey);
			if (organizationId)
				window.localStorage.removeItem(dicomWorkbenchLocalStorageKey);
			return null;
		}
		return parsed;
	} catch (error) {
...
export function loadLocalMprWorkbenchDraftFromLocalStorage(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(mprWorkbenchLocalKey(seriesKey))
				: null);
		if (!raw) return null;
		...
		if (
			...
			window.localStorage.removeItem(localKey);
			if (organizationId)
				window.localStorage.removeItem(mprWorkbenchLocalKey(seriesKey));
			return null;
		}
	...
	const candidates = input.folderStats
		.map((stats) => {
			...
			if (stats.databaseFiles)
				reasons.push(`${stats.databaseFiles} файлов старой базы`);
			if (stats.dumpFiles)
				reasons.push(`${stats.dumpFiles} файлов резервной копии`);
			if (stats.tableFiles)
				reasons.push(`${stats.tableFiles} табличных выгрузок`);
...
export function loadBrowserPickedImagingFolderPreview(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(browserPickedImagingFolderStorageKey)
				: null);
		if (!raw) return null;
		...
		if (!localSavedAtFresh(parsed.createdAt, localConvenienceRetentionMs)) {
			window.localStorage.removeItem(localKey);
			if (organizationId)
				window.localStorage.removeItem(browserPickedImagingFolderStorageKey);
			return null;
		}
		return parsed;
	} catch (error) {
...
export function loadLocalImagingFolderDraft(
	...
	try {
		...
		const raw =
			window.localStorage.getItem(localKey) ??
			(organizationId
				? window.localStorage.getItem(localImagingFolderStorageKey)
				: null);
		if (!raw) return null;
		...
		if (!localSavedAtFresh(parsed.savedAt, localConvenienceRetentionMs)) {
			window.localStorage.removeItem(localKey);
			if (organizationId)
				window.localStorage.removeItem(localImagingFolderStorageKey);
			return null;
		}
		return {
			...parsed,
...
export function visitNoteFormFromVisit(
	visit: Dashboard["activeVisit"] | null | undefined,
): VisitNoteForm {
	if (!visit) {
		return {
			complaint: "",
...
export function visitLocalDraftKey(
	...
	return organizationScopedLocalStorageKey(
		`dental-crm:visit-draft:${visitId ?? "no-active-visit"}`,
		organizationId,
	);
}
...
export function loadOnboardingDismissalState(
	...
	if (typeof window === "undefined") return null;
	try {
		const scopedVal = window.localStorage.getItem(
...
export function normalizeWorkingDaysDraft(
	...
	const days = Array.from(
		new Set(
			(value ?? defaultWorkingDays).filter(
...
export function newAppointmentDraftFromDashboard(
	...
	const assistant =
		savedAssistant ??
		staffArray?.find((member) => member.active && member.role === "assistant");
	...
	const patient =
		selectedPatient ??
		patientsArray?.find((candidate) => candidate.status === "active");
	return {
...
export function appointmentScheduleMissingFields(
	...
	if (
		!isOmniRoleMode &&
		(staff || []).some((s) => s.role === "assistant" && s.active) &&
	...
	try {
		const raw =
			window.localStorage.getItem(
				visitLocalDraftKey(normalizedVisitId, organizationId),
			) ??
			(organizationId
				? window.localStorage.getItem(visitLocalDraftKey(normalizedVisitId))
				: null);
		if (!raw) return null;
		...
		if (!localSavedAtFresh(parsed.savedAt, sensitiveLocalDraftRetentionMs)) {
			...
			if (organizationId)
				window.localStorage.removeItem(visitLocalDraftKey(normalizedVisitId));
			return null;
		}
...
export function loadPendingVisitSavesFromLocalStorage(
	...
	const legacyRaw = normalizedOrganizationId
		? window.localStorage.getItem(pendingVisitSaveQueueKey)
		: null;
...
export function loadPendingSpeechChunksFromLocalStorage(
	...
	try {
		...
		const scopedRaw = window.localStorage.getItem(localKey);
		const legacyRaw = normalizedOrganizationId
			? window.localStorage.getItem(pendingSpeechChunkQueueKey)
			: null;
