...
import "../sanpin/sterilizationSanpin.css";
import {
...
export function SterilizationJournalModal({
	...
	const handleAddQuickAutoclaveCycle = () => {
		...
		const currentCount = cycles.filter((c) => c.date === dateIso).length;
		const nextCycleNum = currentCount + 1;

		const defaultSterilizer = STATUTORY_STERILIZERS[0] ?? {
			id: "autoclave-melag-vacuklav-23b",
			code: "АК-01",
			brandModel: "Melag Vacuklav 23 B+ (Класс B)",
		};

		const chamberPoints = createDefaultChamberPoints();
		const sigHash = generateDigitalStampHash({
			date: dateIso,
		...
		const newCycle: Form257CycleRecord = {
			...
			time,
			cycleNumber: nextCycleNum,
			sterilizerId: defaultSterilizer.id,
			sterilizerCode: defaultSterilizer.code,
			sterilizerBrandModel: defaultSterilizer.brandModel,
			...
			indicatorTradeNameRu: "ИнтеТЕСТ 134/5 (Винар)",
			chamberPoints,
	...
	const handleAddQuickAzopyram = () => {
		...
		const newPso: PsoTestRecord = {
			id: `pso-azo-${Date.now()}`,
			date: dateIso,
			time,
			instrumentName: "Терапевтический смотровой инструментарий (зеркала, зонды, пинцеты)",
			batchItemCount: 120,
			minSampleRequired: trial.minSampleRequired,
			testedSampleCount: 5,
			isSamplingSufficient: trial.isSamplingSufficient,
			detergentBrand: "Оптимакс Про 1.5%",
			isAzopyramNegative: true,
			isPhenolphthaleinNegative: true,
			isSudanNegative: true,
			isBatchApproved: trial.isBatchApproved,
			rejectionReason: trial.rejectionReason,
			operatorFullName: nurseOperator,
			operatorPosition: "Медицинская сестра ЦСО",
			electronicSignatureHash: sigHash,
			notes: "Азопирамовая проба: отрицательно (скрытая кровь отсутствует). Допущено к стерилизации.",
			createdAt: now.toISOString(),
		};

		setPsoRecords((prev) => [newPso, ...prev]);
		showToast("Азопирамовая проба (100% норма, 0 крови) занесена в журнал Формы 366/у", "success");
	};

	// ─── 1-CLICK ADD QUICK PHENOLPHTHALEIN TEST (NEGATIVE / 0 ALKALI) ───────────
	const handleAddQuickPhenolphthalein = () => {
		const now = new Date();
		const dateIso = now.toISOString().slice(0, 10);
		const time = now.toTimeString().slice(0, 5);
		...
		const newPso: PsoTestRecord = {
			...
			date: dateIso,
			time,
			instrumentName: "Наконечники и лотки после проточной отмывки дистиллированной водой",
			batchItemCount: 120,
			minSampleRequired: trial.minSampleRequired,
			testedSampleCount: 5,
			isSamplingSufficient: trial.isSamplingSufficient,
			...
			operatorPosition: "Медицинская сестра ЦСО",
			electronicSignatureHash: sigHash,
			notes: "Фенолфталеиновая проба: отрицательно (остатки щелочных моющих средств отсутствуют, pH нейтральный).",
			createdAt: now.toISOString(),
		};

		setPsoRecords((prev) => [newPso, ...prev]);
		showToast("Фенолфталеиновая проба (100% норма, 0 щелочи) занесена в журнал Формы 366/у", "success");
	};

	// ─── 1-CLICK ADD STANDARD STERILE TRAY (THERAPY / SURGERY / ENDO) ───────────
	const handleAddStandardSterileTray = (trayType: StandardTrayType) => {
		const now = new Date();
		const parsed = createStandardSterileTrayBarcode(trayType, now, nurseOperator);
		const expiry = calculateKraftSterilityExpiration(parsed.packDateIso, (parsed.packageMaterialId as KraftPackagingType) || "kraft_self_adhesive");

		const newPackage: KraftPackageItem = {
			id: `kraft-quick-${Date.now()}`,
			barcode: parsed.rawInput,
			batchNumber: parsed.batchId,
			packageSerialNumber: parsed.serialNumber ?? 1,
			toolSetNameRu: parsed.toolSetNameRu,
			itemsIncluded: [parsed.toolSetNameRu],
			packagingType: (parsed.packageMaterialId as KraftPackagingType) || "kraft_self_adhesive",
			packagingNameRu: parsed.packageMaterialId || "Крафт-пакет",
			sterilizerCode: parsed.autoclaveId || "АК-01",
			cycleNumber: parsed.cycleNumber || 1,
			packDate: parsed.packDateIso,
			expDate: parsed.expDateIso,
			daysLifespan: expiry.daysLifespan,
			daysRemaining: expiry.daysRemaining,
			status: expiry.status,
			operatorFullName: nurseOperator,
			indicatorVerified: true,
			notes: parsed.formattedProtocolRecord043,
			createdAt: now.toISOString(),
		};

		setKraftPackages((prev) => [newPackage, ...prev]);
		setSelectedPackageForSticker(newPackage);
		showToast(`Крафт-пакет «${parsed.toolSetNameRu}» сгенерирован и готов к печати стикера`, "success");
	};
		...
		<div className="steril-journal-overlay" role="dialog" aria-modal="true">
			<div className="steril-journal-container">
						...
						<div className="steril-toolbar">
							...
							<div className="steril-toolbar-actions">
								<button
									type="button"
									className="btn-steril-primary"
									onClick={handleAddQuickAzopyram}
									title="Зафиксировать азопирамовую пробу (отрицательно, отсутствие крови)"
								>
									<Plus size={16} />
									<span>+ Азопирам (1 клик)</span>
								</button>
								<button
									type="button"
									className="btn-steril-primary"
									onClick={handleAddQuickPhenolphthalein}
									title="Зафиксировать фенолфталеиновую пробу (отрицательно, отсутствие щелочных ПАВ)"
								>
									<Plus size={16} />
									<span>+ Фенолфталеин (1 клик)</span>
								</button>
								<button
									type="button"
									className="btn-steril-secondary"
				...
				{activeTab === "kraft_packages" && (
					<div className="steril-body-content">
						{/* Fast Standard Tray 1-Click Generator Bar for Nurse & Doctors */}
						<div
							style={{
								display: "flex",
								alignItems: "center",
						...
						>
							<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
								...
								<span style={{ fontSize: "13px", fontWeight: 700, color: "var(--ink)" }}>
									1-кликовая подготовка стерильных лотков (СанПиН 3.3686-21):
								</span>
							</div>
							...
							<div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
								{STANDARD_TRAY_OPTIONS.map((tray) => (
									<button
										...
										type="button"
										className="btn-steril-secondary"
										style={{ height: "32px", fontSize: "12px", padding: "0 10px", fontWeight: 600 }}
										onClick={() => handleAddStandardSterileTray(tray.id)}
										title={tray.descriptionRu}
									>
										<Plus size={14} />
										<span>+ {tray.shortLabelRu}</span>
									</button>
								))}
							</div>
						</div>
