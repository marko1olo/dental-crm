	...
	return (
		<div className="space-y-6 animate-in fade-in duration-200">
			<div className="p-5 rounded-2xl bg-[var(--odontogram-surface,#f8fafc)] border border-[var(--odontogram-border-subtle,#e2e8f0)] space-y-3">
				<h3 className="text-sm sm:text-base font-extrabold text-[var(--odontogram-ink,#0f172a)]">
					Клиническая шкала физиологической резорбции корней молочных зубов
				</h3>
				<p className="text-[13px] text-[var(--odontogram-ink-muted,#64748b)] leading-relaxed">
					...
					return (
						...
						>
							<div className="flex items-center justify-between w-full">
								<span
									className="min-h-[28px] min-w-[44px] inline-flex items-center justify-center px-3 py-1 rounded-xl text-[13px] font-black"
									style={{ backgroundColor: def.badgeBg, color: def.badgeColor }}
								>
									{stage}%
								</span>
								<span className="text-[13px] font-bold text-[var(--odontogram-ink-muted,#64748b)]">
									{def.expectedMobilityDegree} ст.
								</span>
							</div>

							<div className="my-2 space-y-1">
								<div className="text-sm font-black text-[var(--odontogram-ink,#0f172a)] leading-snug">
									{def.nameRu}
								</div>
								<div className="text-[13px] text-[var(--odontogram-ink-muted,#64748b)] line-clamp-2">
									{def.clinicalSignRu}
								</div>
							</div>

							<div className="text-[13px] font-mono font-bold text-teal-600 dark:text-teal-400 flex items-center gap-1.5 pt-1">
			...
			<div className="p-5 rounded-2xl bg-[var(--odontogram-surface,#f8fafc)] border border-[var(--odontogram-border-subtle,#e2e8f0)] space-y-4">
				<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
					<div>
						...
						<p className="text-[13px] text-[var(--odontogram-ink-muted,#64748b)]">
							Текущий выбранный:{" "}
							<strong className="text-teal-600 dark:text-teal-400 font-mono text-sm sm:text-base">
					...
					{onUpdateToothResorption && (
						<button
							...
							className="min-h-[44px] px-5 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-500 active:bg-teal-700 text-white text-sm font-bold shadow-md shadow-teal-600/20 transition-all cursor-pointer active:scale-95 flex items-center justify-center gap-2 shrink-0"
						>
						...
						{UPPER_PRIMARY_TEETH.map((num) => {
							...
							return (
								<button
									...
									className={`min-w-[54px] min-h-[48px] px-3.5 py-2.5 rounded-xl text-sm font-mono font-black border transition-all cursor-pointer active:scale-95 flex flex-col items-center justify-center ${
										isSelected
								...
								>
									<span className="text-sm sm:text-base font-black leading-none">{num}</span>
									<span className="text-[12px] opacity-85 font-sans mt-0.5">
