				...
				<div className="mb-2 bg-neutral-950/95 backdrop-blur-lg border border-neutral-800 text-neutral-200 p-5 rounded-2xl shadow-2xl w-80 md:w-96 pointer-events-auto transition-all animate-fade-in-up max-h-[70vh] overflow-y-auto">
					...
					<div className="space-y-3.5 text-xs leading-relaxed">
						{activeTab === "nav" && (
							<>
								...
								<div className="space-y-2">
									{[
										{
											cmd: "«Перейди в расписание»",
											desc: "Открывает календарь записей",
										},
										{
											cmd: "«Открой пациентов»",
											desc: "Переходит в список пациентов клиники",
										},
										{
											cmd: "«Перейди в кассу»",
											desc: "Раздел финансов и оплат",
										},
										{
											cmd: "«Открой настройки»",
											desc: "Управление клиникой и услугами",
										},
										{
											cmd: "«Покажи маркетинг»",
											desc: "Аналитика каналов привлечения",
										},
										{
											cmd: "«Открой документы»",
											desc: "Анализатор документов и согласий",
										},
									].map((item, idx) => (
										<div
											key={idx}
											className="bg-neutral-900/50 border border-neutral-850 p-2.5 rounded-xl hover:border-neutral-705 transition-colors"
										>
						...
						{activeTab === "search" && (
							<>
								...
								<div className="space-y-2">
									{[
										{
											cmd: "«Найди пациента Смирнов»",
											desc: "Ищет карту пациента по фамилии или имени",
										},
										{
											cmd: "«Поиск Петров»",
											desc: "Быстрый глобальный поиск по базе",
										},
										{
											cmd: "«Покажи расписание на завтра»",
											desc: "Фильтрует календарь на следующий день",
										},
										{
											cmd: "«Перейди на сегодня»",
											desc: "Возвращает календарную сетку на текущую дату",
										},
										{
											cmd: "«Покажи расписание на вчера»",
											desc: "Переключает дату календаря на день назад",
										},
									].map((item, idx) => (
										<div
											key={idx}
						...
						{activeTab === "visit" && (
							<>
								<p className="text-neutral-400">
									Для заполнения карты приема диктуйте жалобы, объективный
									статус или диагнозы:
								</p>
								<div className="space-y-2">
									{[
										{
											cmd: "«Жалобы на острую боль в 45 зубе»",
											desc: "Автоматически заполнит поле жалоб пациента",
										},
										{
											cmd: "«Объективно глубокий кариес сорок шестого зуба»",
											desc: "Заполнит объективный статус",
										},
										{
											cmd: "«Диагноз хронический пульпит»",
											desc: "Установит клинический диагноз визита",
										},
										{
											cmd: "«Лечение проведено препарирование и пломбирование»",
											desc: "Добавит описание выполненных работ",
										},
										{
											cmd: "«Зуб 36 кариес дентина»",
											desc: "Отметит состояние на интерактивной зубной карте",
										},
									].map((item, idx) => (
										<div
											key={idx}
								...
								<div className="bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-xl text-amber-300 text-[11px]">
									<strong>Совет:</strong> Убедитесь, что открыта вкладка приёма
									(ЭМК), чтобы текст диктовки автоматически распределился по
									медицинским полям.
								</div>
							</>
						)}
