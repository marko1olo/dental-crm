...
import { DoctorShiftCockpitModal, type DoctorQuickCallAction } from "../doctor/DoctorShiftCockpitModal";
import "../doctor/doctorShiftCockpit.css";
			...
			>
					...
					<div className="flex items-center gap-2 text-xs text-[var(--ink-2,var(--muted,#cbd5e1))] px-2 shrink-0">
						...
						{onEmergencyVisit && (
							...
							>
								...
								{/* Piece-rate Accrual & Financial Metrics */}
				<div className="flex items-center gap-2 sm:gap-3 px-2 py-1 rounded-xl bg-[var(--paper-soft,#1e293b)] border border-[var(--line,#334155)] shrink-0">
					<div className="min-w-0">
						<div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted,#cbd5e1)] flex items-center gap-1">
							<Sparkles size={11} className="text-emerald-400" />
							<span>Заработано (сделка %):</span>
						</div>
						<div
							className="text-sm font-black text-emerald-600 dark:text-emerald-400 font-mono leading-tight mt-0.5"
							data-testid="doctor-header-earned-deal"
						>
							{formatKopecksRu(earnings.totalEarnedDealKop)}
						</div>
					</div>
					<div className="hidden sm:block w-px h-6 bg-[var(--line,#334155)]" />
					<div className="hidden sm:block text-[10px] text-[var(--muted,#cbd5e1)] space-y-0.5 shrink-0">
						<div>
							Выручка: <strong className="text-[var(--ink,#f8fafc)]">{formatKopecksRu(earnings.grossRevenueKop)}</strong>
						</div>
						<div>
							Вычет ЗТЛ/Мат: <strong className="text-amber-600 dark:text-amber-400">−{formatKopecksRu(earnings.totalLabDeductionsKop + earnings.totalMaterialDeductionsKop)}</strong>
						</div>
					</div>
				</div>
				...
				<div className="doctor-desktop-header-section shrink-0">
					...
					<button
						...
						data-testid="doctor-header-cockpit-trigger-btn" data-legacy-id="header-btn-open-cockpit"
