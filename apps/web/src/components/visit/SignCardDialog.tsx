...
export function SignCardDialog({
	...
	const [activeTab, setActiveTab] = useState<"cryptopro" | "rutoken">(
		"cryptopro",
	);
	...
	const handleSign = async () => {
		...
		try {
			...
			const { signatureBase64, provider } = await signatureService.signData(
				selectedThumbprint,
				diaryContent,
				pinCode,
				deviceId,
				...
				>
					...
					>
						<div className="sign-dialog-header">
							...
							<button className="sign-dialog-close" onClick={onClose}>
								&times;
							</button>
						</div>
						...
						<div className="sign-dialog-body">
							{fetchingCerts ? (
								...
								<div className="sign-empty-state">
									...
									<span>
										Проверьте подключение носителя или плагин браузера.
									</span>
								</div>
							) : (
								<div className="sign-form">
									...
									<div className="sign-preview-hint">
										<FileCheck2 size={14} /> Подписываемый документ: Дневник
										приема #{visitId.slice(0, 6)}
									</div>
								</div>
							)}
						</div>
