...
export function InstallmentPaymentScheduleForm({
	activeDoctor,
	documentPatient,
	money,
	...
	return (
		<article className="document-payload-card">
			<div>
				...
				<p>
					Внутренний график сроков и сумм к договору или плану лечения без
					подмены банковского кредита.
				</p>
			</div>
				...
				<div className="document-payload-collapsed-content">
					...
					<label>
						...
						<input
							...
							onChange={(event) =>
								setInstallmentScheduleBaseDocumentTitle(event.target.value)
							}
					...
					<div className="document-payload-row">
						<label>
							...
							<input
								...
								onChange={(event) =>
									setInstallmentSchedulePayerFullName(event.target.value)
								}
								placeholder={documentPatient?.fullName ?? "ФИО плательщика"}
							/>
						</label>
						<label>
							Ответственный
							<input
								...
								onChange={(event) =>
									setInstallmentScheduleResponsibleFullName(event.target.value)
								}
					...
					<div className="document-payload-row">
						<label>
							...
							<input
								...
								placeholder={String(installmentScheduleTotalRubValue() || "")}
							/>
						</label>
						<label>
							Предоплата
							<input
								...
								placeholder={String(installmentSchedulePrepaidRubValue() || "")}
							/>
						</label>
					</div>
					<label>
						...
						/>
						<span>
							Формат строки: этап | срок | сумма | запланировано / оплачено /
							просрочено / перенесено / отменено
						</span>
					</label>
					<p className="small">
						Остаток: {money(installmentScheduleRemainingRubValue())}. Платежей:{" "}
						{installmentScheduleInstallmentRows().length}.
					</p>
					<label>
						Правила просрочки
						<textarea
					...
					<label>
						...
						<textarea
							...
							onChange={(event) =>
								setInstallmentSchedulePaymentMethodNotes(event.target.value)
							}
							rows={2}
						/>
					</label>
