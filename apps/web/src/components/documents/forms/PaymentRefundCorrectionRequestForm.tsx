...
export function PaymentRefundCorrectionRequestForm({
	activePatient,
	money,
				...
				<div className="document-payload-collapsed-content">
					<label>
						...
						<select
							...
							onChange={(event) =>
								setRefundAction(
									normalizedPaymentRefundCorrectionAction(event.target.value),
								)
							}
						>
					...
					<label>
						...
						{selectedRefundCorrectionPayment ? (
							<small>
								...
								{money(selectedRefundCorrectionPayment.amountRub)} по выбранному
								исходному платежу.
							</small>
						) : null}
					</label>
					<label>
						...
						<input
							...
							onChange={(event) => setRefundAmountRub(event.target.value)}
						/>
					</label>
					<label>
						Основание
					...
					<label>
						...
						<select
							...
							onChange={(event) =>
								setRefundMethod(
									normalizedPaymentRefundCorrectionMethod(event.target.value),
								)
							}
						>
					...
					<label>
						...
						<textarea
							...
							onChange={(event) => setRefundBankDetails(event.target.value)}
							rows={2}
						/>
					</label>
					<label>
					...
					<label>
						...
						<input
							...
							onChange={(event) =>
								setRefundCorrectionFiscalReceiptNumber(event.target.value)
							}
						/>
					</label>
					<label>
