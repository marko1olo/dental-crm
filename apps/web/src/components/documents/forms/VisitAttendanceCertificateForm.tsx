...
export function VisitAttendanceCertificateForm({
	activeAppointment,
	activeDoctor,
	formatDateTime,
}: any) {
	...
	return (
		<article className="document-payload-card">
			<div>
				...
				<p>
					Фиксирует только факт и время приема без диагноза, лечения, снимков и
					стоимости.
				</p>
			</div>
				...
				<div className="document-payload-collapsed-content">
					...
					<label>
						...
						<input
							...
							onChange={(event) => setAttendancePurpose(event.target.value)}
						/>
					</label>
					<label>
						Куда предъявляется
					...
					<label>
						...
						<input
							...
							onChange={(event) => setAttendanceIssuedAt(event.target.value)}
						/>
					</label>
					<label className="document-payload-checkbox">
						<input
							...
							type="checkbox"
							onChange={(event) =>
								setAttendanceDiagnosisDisclosureExcluded(event.target.checked)
							}
						/>
					...
					<label className="document-payload-checkbox">
						<input
							...
							type="checkbox"
							onChange={(event) =>
								setAttendanceNotSickLeaveAcknowledged(event.target.checked)
							}
						/>
