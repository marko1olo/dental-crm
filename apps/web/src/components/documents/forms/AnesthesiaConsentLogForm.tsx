 ...
 */
export const AnesthesiaConsentLogForm = React.memo(
	function AnesthesiaConsentLogForm({
		inferredTreatmentArea,
	}: Pick<DocumentVisitHints, "inferredTreatmentArea">) {
		const anesthesiaAllergyRestrictionsChecked = useDocumentStore(
			(state) => state.anesthesiaAllergyRestrictionsChecked,
		);
		const setAnesthesiaAllergyRestrictionsChecked = useDocumentStore(
			(state) => state.setAnesthesiaAllergyRestrictionsChecked,
		);
		const anesthesiaAllergyStatus = useDocumentStore(
			(state) => state.anesthesiaAllergyStatus,
		);
		const setAnesthesiaAllergyStatus = useDocumentStore(
			(state) => state.setAnesthesiaAllergyStatus,
		);
		const anesthesiaAnesthetic = useDocumentStore(
			(state) => state.anesthesiaAnesthetic,
		);
		const setAnesthesiaAnesthetic = useDocumentStore(
			(state) => state.setAnesthesiaAnesthetic,
		);
		const anesthesiaConsentConfirmed = useDocumentStore(
			(state) => state.anesthesiaConsentConfirmed,
		);
		const setAnesthesiaConsentConfirmed = useDocumentStore(
			(state) => state.setAnesthesiaConsentConfirmed,
		);
		const anesthesiaDoseMl = useDocumentStore(
			(state) => state.anesthesiaDoseMl,
		);
		const setAnesthesiaDoseMl = useDocumentStore(
			(state) => state.setAnesthesiaDoseMl,
		);
		const anesthesiaDoseTime = useDocumentStore(
			(state) => state.anesthesiaDoseTime,
		);
		const setAnesthesiaDoseTime = useDocumentStore(
			(state) => state.setAnesthesiaDoseTime,
		);
		const anesthesiaMethod = useDocumentStore(
			(state) => state.anesthesiaMethod,
		);
		const setAnesthesiaMethod = useDocumentStore(
			(state) => state.setAnesthesiaMethod,
		);
		const anesthesiaReaction = useDocumentStore(
			(state) => state.anesthesiaReaction,
		);
		const setAnesthesiaReaction = useDocumentStore(
			(state) => state.setAnesthesiaReaction,
		);
		const anesthesiaRestrictionNotes = useDocumentStore(
			(state) => state.anesthesiaRestrictionNotes,
		);
		const setAnesthesiaRestrictionNotes = useDocumentStore(
			(state) => state.setAnesthesiaRestrictionNotes,
		);
		const anesthesiaRisksExplained = useDocumentStore(
			(state) => state.anesthesiaRisksExplained,
		);
		const setAnesthesiaRisksExplained = useDocumentStore(
			(state) => state.setAnesthesiaRisksExplained,
		);
		const anesthesiaVasoconstrictor = useDocumentStore(
			(state) => state.anesthesiaVasoconstrictor,
		);
		const setAnesthesiaVasoconstrictor = useDocumentStore(
			(state) => state.setAnesthesiaVasoconstrictor,
		);
		const anesthesiaZone = useDocumentStore((state) => state.anesthesiaZone);
		const setAnesthesiaZone = useDocumentStore(
			(state) => state.setAnesthesiaZone,
		);
			...
			<DocumentPayloadCard
				...
				notice={
					limits.tooLong.length > 0 ? (
						...
						>
							<strong>
								Журнал не создастся: текст в {limits.tooLong.length} поле
								длиннее, чем принимает сервер. Отказ на нажатие «Создать» при
								этом говорит про незаполненные поля, поэтому причину пишем
								здесь:
							</strong>
							<ul>
								{limits.tooLong.map((entry) => (
									<li key={entry.field}>
										{entry.label} — {entry.length} символов, принимается не
										больше {entry.limit}: сократите на{" "}
										{entry.length - entry.limit}
...
AnesthesiaConsentLogForm.displayName = "AnesthesiaConsentLogForm";
