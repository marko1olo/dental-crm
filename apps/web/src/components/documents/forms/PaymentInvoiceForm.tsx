	...
	return (
		<article className="document-payload-card">
			<div>
				...
				<p>
					Реквизиты, плательщик, срок оплаты и состав услуг. Счет не заменяет
					кассовый чек.
				</p>
			</div>
				...
				<div className="document-payload-collapsed-content">
					<div className="document-payload-row">
						...
						<label>
							...
							<input
								...
								onChange={(event) => setPaymentInvoiceDate(event.target.value)}
							/>
						</label>
					</div>
					<div className="document-payload-row">
					...
					<label>
						...
						<textarea
							...
							onChange={(event) => setPaymentInvoicePurpose(event.target.value)}
							rows={2}
						/>
					</label>
					<label>
					...
					<p className="small">
						Сумма из плана лечения: {money(paymentInvoiceTotalRubValue())}.
						Строк услуг: {plannedServiceLinesForFinancialPayload().length}.
					</p>
					...
					<label className="document-payload-checkbox">
						<input
							...
							type="checkbox"
							onChange={(event) =>
								setPaymentInvoiceServiceScopeConfirmed(event.target.checked)
							}
						/>
					...
					<label className="document-payload-checkbox">
						<input
							...
							type="checkbox"
							onChange={(event) =>
								setPaymentInvoiceFiscalNoticeConfirmed(event.target.checked)
							}
						/>
