...
export type OrthopedicWorkTypeId =
	...
	| "custom_abutment"     // Индивидуальный абатмент (Ti-Base / ZrO2) + коронка
...
export const ORTHOPEDIC_WORK_TYPES: Record<OrthopedicWorkTypeId, OrthopedicWorkTypeDefinition> = {
	...
	custom_abutment: {
		id: "custom_abutment",
		nameRu: "Индивидуальный абатмент (Ti-Base / ZrO2) + коронка",
		...
		icon: "🔩",
		defaultMaterialRu: "Титан Grade 5 ELI + ZrO2 Katana ML (Medentika / Straumann)",
...
export function checkLabDeadlineAndAlert(params: CheckLabDeadlineParams): LabDelayAlert {
	...
	if (daysToLab < 0) {
		...
		return {
			...
			fittingDateIso: fittingIso,
			appointmentId,
			alertMessageRu: `⚠️ ПРОСРОЧЕНО ЗТЛ: Заказ задерживается на ${overdueDays} дн. (план был: ${expectedRu})!`,
