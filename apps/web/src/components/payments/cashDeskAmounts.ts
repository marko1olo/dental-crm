 ...
 *
 * ЛИСТОВОЙ МОДУЛЬ. Здесь не должно появиться НИ ОДНОГО импорта. Причина не в
 * опрятности: `AppHelpers.tsx` по цепочке импортов тянет за собой таблицы
 * стилей, и модуль логики, попросивший у него `money()`, перестаёт запускаться
 * в `node:test`. Форматирование — забота вызывающего экрана: он берёт `money()`
 * сам. Отсюда наружу уходят только числа и текст для поля ввода.
 */
