/**
 * TreatmentPlan3TierComparison.tsx — Side-by-side сравнительная презентация 3 вариантов плана лечения (Эконом, Стандарт, Оптимальный).
 */
...
interface TreatmentPlan3TierComparisonProps {
...
export const TreatmentPlan3TierComparison: React.FC<TreatmentPlan3TierComparisonProps> = ({
	...
	return (
		...
		>
			{/* Top Control Bar: Installments & NDFL Settings */}
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 rounded-2xl bg-[var(--paper-soft,var(--paper,#ffffff))] border border-[var(--border,#cbd5e1)] text-xs text-[var(--ink,#0f172a)]">
				<div className="flex items-center gap-2">
					...
					<div>
						<span className="font-bold text-sm text-[var(--ink,#0f172a)]">
							3 Варианта комплексного плана
						</span>
						<p className="text-[11px] text-[var(--muted,#64748b)]">
							Сравнение по материалам, срокам, гарантиям и финансовым программам
						</p>
					</div>
				</div>
				...
				<div className="flex flex-wrap items-center gap-3">
					...
					<div className="flex items-center gap-1.5 bg-[var(--paper-strong,var(--paper,#ffffff))] px-2.5 py-1 rounded-xl border border-[var(--border,#cbd5e1)]">
						<span className="text-[11px] font-semibold text-[var(--muted,#64748b)] flex items-center gap-1">
							<Percent size={12} /> Рассрочка 0%:
						</span>
						{[3, 6, 12, 24].map((m) => (
							<button
								key={m}
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-stretch">
				{tiers.map((tier) => {
					...
					const monthlyPayment =
						tier.installments?.[installmentMonths as 3 | 6 | 12 | 24]?.monthlyPaymentRub ??
					...
					return (
						<div
							...
							className={`relative flex flex-col justify-between rounded-3xl p-5 border transition-all duration-300 cursor-pointer bg-[var(--paper-strong,var(--paper,#ffffff))] text-[var(--ink,#0f172a)] ${
								isSelected
									? `${tier.borderClass} shadow-xl scale-[1.02] z-10`
						...
						>
							{/* Recommended Pill */}
							{tier.isRecommended && (
								<div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1 px-3 py-0.5 rounded-full bg-gradient-to-r from-teal-600 to-emerald-600 text-white text-[11px] font-extrabold shadow-md uppercase tracking-wider">
									<Crown size={12} />
							...
							<div className="space-y-4">
								...
								<div className="space-y-1.5 pt-1">
									<div className="flex items-center justify-between">
										...
										<span className="text-xs font-semibold text-[var(--muted,#64748b)] flex items-center gap-1">
											<Clock size={12} /> {tier.durationWeeks} нед.
										</span>
									</div>
								...
								<div className="grid grid-cols-2 gap-2 text-xs">
									...
									<div className="p-2 rounded-xl bg-[var(--paper-soft,#f8fafc)] border border-[var(--border,#cbd5e1)] flex flex-col">
										<span className="text-[10px] text-[var(--muted,#64748b)] flex items-center gap-1">
											<Calendar size={11} /> Визиты
										</span>
										<strong className="text-[11px] text-[var(--ink,#0f172a)]">
											{tier.durationVisits} визитов
										</strong>
									</div>
								</div>

								{/* Materials & Clinical Technology Highlights */}
								<div className="space-y-2 pt-2 border-t border-[var(--border,#cbd5e1)]">
									...
									<ul className="space-y-1.5 text-xs text-[var(--muted,#64748b)]">
							...
							{/* Actions Bottom */}
							<div className="pt-5 mt-4 border-t border-[var(--border,#cbd5e1)] space-y-2">
								<button
									type="button"
								...
								>
									<PenTool size={14} />
									<span>Утвердить и подписать</span>
								</button>
							</div>
						</div>
					);
