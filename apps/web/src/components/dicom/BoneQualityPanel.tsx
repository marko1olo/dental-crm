...
export function BoneQualityPanel({
  ...
  if (huSamples.length === 0) {
    return (
      <div style={panelStyle}>
        <PanelHeader expanded={expanded} onToggle={() => setExpanded(e => !e)} toothFdi={toothFdi} />
        {expanded && (
  ...
  const protocol: DrillProtocol = generateDrillProtocol(zones, implantSystem, implantDiameterMm, implantLengthMm);
  ...
  return (
    <div style={panelStyle}>
      ...
      {expanded && (
        <>
          {/* Misch Class Badge */}
          <div style={{ padding: '10px 12px 0', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{
              background: mischColor, color: '#000', fontWeight: 800,
              fontSize: '18px', padding: '4px 14px', borderRadius: '8px',
          ...
          {protocol.warnings.length > 0 && (
            <div style={{ padding: '0 12px 8px' }}>
              {protocol.warnings.map((w, i) => (
                <div key={i} style={warnStyle}>{w}</div>
              ))}
            </div>
          )}
          ...
          <div style={{ padding: '0 12px 12px' }}>
            <div style={{ fontSize: '11px', color: '#71717a', marginBottom: '6px', fontWeight: 600 }}>
              ПРОТОКОЛ СВЕРЛЕНИЯ
              {protocol.underdrillingApplied && (
                <span style={{ marginLeft: '6px', color: '#f97316', fontSize: '10px' }}>UNDERDRILL</span>
              )}
              {protocol.corticalTapRequired && (
                <span style={{ marginLeft: '6px', color: '#ef4444', fontSize: '10px' }}>CORTICAL TAP</span>
              )}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {protocol.steps.map(step => (
                <div key={step.step} style={stepStyle}>
                  <div style={{ width: '18px', height: '18px', borderRadius: '50%', background: '#3f3f46', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: 700, color: '#e4e4e7', flexShrink: 0 }}>
                    {step.step}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '11px', fontWeight: 600, color: '#e4e4e7', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {step.drillType} — Ø{step.diameterMm}mm × {step.depthMm}mm
                    </div>
                    <div style={{ fontSize: '10px', color: '#71717a' }}>
                      {step.rpmRange} · {step.torqueNcm}{step.irrigation ? ' · 💧' : ''}
                    </div>
                    {step.note && (
                      <div style={{ fontSize: '10px', color: '#f59e0b', marginTop: '1px', whiteSpace: 'normal' }}>
                        {step.note}
                      </div>
                    )}
                  </div>
...
function PanelHeader({ expanded, onToggle, toothFdi }: { expanded: boolean; onToggle: () => void; toothFdi: number }) {
  return (
    <div
      onClick={onToggle}
      style={{ padding: '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', borderBottom: '1px solid #3f3f46', userSelect: 'none' }}
    >
      <span style={{ fontSize: '11px', fontWeight: 700, color: '#a1a1aa', letterSpacing: '0.5px' }}>
        🦴 BONE QUALITY · FDI {toothFdi}
      </span>
      <span style={{ fontSize: '10px', color: '#52525b' }}>{expanded ? '▲' : '▼'}</span>
    </div>
  );
}

function ZoneCard({ label, hu }: { label: string; hu: number }) {
  const color = hu > 1250 ? '#ef4444' : hu >= 850 ? '#22c55e' : hu >= 350 ? '#f59e0b' : '#f97316';
  return (
    <div style={{ background: '#18181b', borderRadius: '6px', padding: '6px 8px', border: '1px solid #3f3f46' }}>
      <div style={{ fontSize: '9px', color: '#71717a', marginBottom: '2px' }}>{label}</div>
      <div style={{ fontSize: '13px', fontWeight: 700, color }}>{Math.round(hu)} HU</div>
    </div>
  );
}
