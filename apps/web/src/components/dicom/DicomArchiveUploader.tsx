...
export function DicomArchiveUploader({
	...
	const [status, setStatus] = useState<string>(
		"Drag & Drop ZIP Archive or Folder here",
	);
		...
		async (e: React.DragEvent<HTMLDivElement>) => {
			...
			if (
				...
				for (let i = 0; i < allFiles.length; i++) {
					if (i % 10 === 0)
						setStatus(`Scanning files: ${i}/${allFiles.length}`);
					const f = allFiles[i];
					if (f) {
	...
	return (
		...
		>
			...
			<div className="text-neutral-400 font-medium">
				{status} (Кликните или перетащите папку)
			</div>
			{loading && (
				<div className="mt-2 w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
			)}
			<div className="text-xs text-neutral-600 mt-2">
				Works completely locally. No server upload.
			</div>
		</div>
	);
}
