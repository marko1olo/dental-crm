		...
		const renderOverlay = () => {
			...
			for (const implant of ClinicalStore.implants) {
				...
				if (tipCanvas && neckCanvas) {
					// Draw cylinder axis
					ctx.strokeStyle = isCollision
						? "#ef4444"
						: implant.color || "#0ea5e9"; // Red if collision, else blue
					ctx.lineWidth = 4;
					...
					const sleeveBottomCanvas = viewport.worldToCanvas([
						sleeveBottom.x,
						sleeveBottom.y,
						sleeveBottom.z,
					]);
					const sleeveTopCanvas = viewport.worldToCanvas([
						sleeveTop.x,
						sleeveTop.y,
						sleeveTop.z,
					]);
					...
					const {
						...
						isWarning,
						message: angMsg,
					} = getAngulationWarning(implant.direction.z);
					...
					if (isWarning && angMsg) {
						...
						window.dispatchEvent(
							new CustomEvent("ct-angulation-warn", {
			...
			for (let i = 0; i < ClinicalStore.implants.length; i++) {
				for (let j = i + 1; j < ClinicalStore.implants.length; j++) {
					...
					const dot =
						imp1.direction.x * imp2.direction.x +
					...
					if (angleDeg > 15) {
						...
						const neckCanvas1 = viewport.worldToCanvas([
							neck1.x,
							neck1.y,
							neck1.z,
						]);
						const neckCanvas2 = viewport.worldToCanvas([
							neck2.x,
							neck2.y,
							neck2.z,
						]);
						...
						if (!reportedPairs.has(pairId)) {
							...
							window.dispatchEvent(
								new CustomEvent("shadow-analyst-divergence-warn", {
									detail: {
