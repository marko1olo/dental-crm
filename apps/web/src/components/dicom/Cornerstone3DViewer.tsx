...
export function Cornerstone3DViewer({
	...
	// Tools & UI State
	const [vrPreset, setVrPreset] = useState<"CT-Bone" | "CT-Soft-Tissue">(
		"CT-Bone",
	);
	...
	useEffect(() => {
		...
		const viewportInputArray = [
			...
			{
				viewportId: viewportIds.vr,
				type: cornerstone.Enums.ViewportType.VOLUME_3D,
				element: vrRef.current as HTMLDivElement,
		...
		async function loadAndRender() {
			...
			for (let i = 0; i < imageIds.length; i += batchSize) {
				const batch = imageIds
					.slice(i, i + batchSize)
					.map((imageId) => cornerstone.imageLoader.loadAndCacheImage(imageId));
			...
			const sortedImageIds = [...imageIds].sort((a, b) => {
				...
				if (planeA?.imagePositionPatient && planeB?.imagePositionPatient) {
					return (
						planeA.imagePositionPatient[2] - planeB.imagePositionPatient[2]
					);
				}
			...
			// Tool setups
			setupMprToolGroup(renderingEngineId, [
				viewportIds.axial,
			...
			cornerstone.eventTarget.addEventListener(
				cornerstone.EVENTS.IMAGE_VOLUME_MODIFIED,
				(e: any) => {
	...
	useEffect(() => {
		...
		if (!engine) return;
		const vrViewport = engine.getViewport(
			"VR",
	...
	useEffect(() => {
		...
		return () => {
			...
			window.removeEventListener(
				"shadow-analyst-divergence-warn",
				handleDivergence,
			);
			window.removeEventListener(
	...
	// Autosave
	useEffect(() => {
		if (!volumeId || (splinePoints.length === 0 && implants.length === 0))
			return;
		...
		try {
			...
			if (annotations && annotations.length > 0) {
				...
				const rawPoints =
					...
					(annotation?.data as any)?.contour?.points ||
					[];
	...
	const setTool = (toolName: string) => {
		const toolGroup =
			cornerstoneTools.ToolGroupManager.getToolGroup("mpr-tool-group");
		if (!toolGroup) return;
	...
	const pipWidget = (
		<div
			style={{
				...
				overflow: "hidden",
				background:
					"linear-gradient(135deg, rgba(15,15,18,0.98) 0%, rgba(9,9,11,0.98) 100%)",
	...
	const content = (
		<div ref={containerRef} style={containerStyle}>
			<DicomToolbar
				...
				setPanorexThickness={setPanorexThickness}
				blendMode={blendMode}
				...
				>
					...
					<div
						style={{
							...
							overflow: "hidden",
							display:
								!maximizedViewportId || maximizedViewportId === "AXIAL"
									? "block"
									: "none",
							cursor: "crosshair",
						}}
						onDoubleClick={() =>
							setMaximizedViewportId((p) => (p === "AXIAL" ? null : "AXIAL"))
						}
					>
						<ViewportOverlays
							viewportType="AXIAL"
					...
					<div
						style={{
							...
							overflow: "hidden",
							display:
								!maximizedViewportId || maximizedViewportId === "SAGITTAL"
									? "block"
									: "none",
							cursor: "crosshair",
						}}
						onDoubleClick={() =>
							setMaximizedViewportId((p) =>
								p === "SAGITTAL" ? null : "SAGITTAL",
							)
						}
					>
						<ViewportOverlays
					...
					<div
						style={{
							...
							overflow: "hidden",
							display:
								!maximizedViewportId || maximizedViewportId === "CORONAL"
									? "block"
									: "none",
							cursor: "crosshair",
						}}
					...
					<div
						style={{
							...
							overflow: "hidden",
							display:
								!maximizedViewportId || maximizedViewportId === "VR"
									? "block"
									: "none",
							cursor: "move",
						}}
						onDoubleClick={() =>
							setMaximizedViewportId((p) => (p === "VR" ? null : "VR"))
						}
					>
						<ViewportOverlays
							viewportType="VR"
						...
						>
							<button
								style={{
									...
									padding: "4px 12px",
									background:
										vrPreset === "CT-Bone" ? "#f59e0b" : "rgba(0,0,0,0.6)",
									color: vrPreset === "CT-Bone" ? "#000" : "#a1a1aa",
									border: "1px solid",
									borderColor: vrPreset === "CT-Bone" ? "#fbbf24" : "#3f3f46",
									borderRadius: "6px",
									cursor: "pointer",
								}}
								onClick={(e) => {
							...
							<button
								style={{
									...
									padding: "4px 12px",
									background:
										vrPreset === "CT-Soft-Tissue"
											? "#f59e0b"
											: "rgba(0,0,0,0.6)",
									color: vrPreset === "CT-Soft-Tissue" ? "#000" : "#a1a1aa",
									border: "1px solid",
									borderColor:
										vrPreset === "CT-Soft-Tissue" ? "#fbbf24" : "#3f3f46",
									borderRadius: "6px",
									cursor: "pointer",
								}}
								onClick={(e) => {
									e.stopPropagation();
									setVrPreset("CT-Soft-Tissue");
								}}
							>
								Мягкие ткани
							</button>
	...
	if (!isPreview && typeof document !== "undefined") {
		if (isMinimized) {
			return (
				<>
					{createPortal(pipWidget, document.body)}
					{panorexRender}
				</>
			);
		}
