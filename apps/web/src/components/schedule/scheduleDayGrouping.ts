...
function dayRelation(dateKey: string, todayKey: string): ScheduleDayGroup["relation"] {
...
function relativeLabel(relation: ScheduleDayGroup["relation"], dateKey: string, todayKey: string): string {
...
export function groupAppointmentsByClinicDay(
  ...
  return [...byDay.keys()]
    .sort()
    .map((dateKey) => {
      const dayAppointments = [...(byDay.get(dateKey) ?? [])].sort((left, right) =>
      ...
      for (const appointment of dayAppointments) {
        ...
        const measurable = Number.isFinite(startMs) && Number.isFinite(endMs) && endMs > startMs;

        if (occupies && measurable) {
          if (occupiedUntilMs !== null) {
            const gapMinutes = Math.round((startMs - occupiedUntilMs) / MINUTE_MS);
            if (gapMinutes >= minGap) {
              const previous = occupyingSoFar[occupyingSoFar.length - 1];
              rows.push({ kind: "gap", minutes: gapMinutes, afterAppointmentId: previous?.id ?? "" });
          ...
          for (const earlier of occupyingSoFar) {
            ...
            if (!Number.isFinite(earlierStart) || !Number.isFinite(earlierEnd)) continue;
            const overlapMs = Math.min(endMs, earlierEnd) - Math.max(startMs, earlierStart);
            if (overlapMs <= 0) continue;
            const sameDoctor = Boolean(
              appointment.doctorUserId && appointment.doctorUserId === earlier.doctorUserId,
            );
            const sameChair = Boolean(appointment.chairId && appointment.chairId === earlier.chairId);
          ...
          occupiedUntilMs = occupiedUntilMs === null ? endMs : Math.max(occupiedUntilMs, endMs);
