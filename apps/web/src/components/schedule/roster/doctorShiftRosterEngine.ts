		...
		for (let i = 0; i < n; i++) {
			...
			if (s1.doctorRole === "surgeon" && !s1.assistantId) {
				conflicts.push({
					...
					severity: "warning",
					message: `Хирургическая смена ${s1.doctorName} (${s1.startTime}–${s1.endTime}) назначена без обязательного ассистента`,
					dateIso,
					shiftIds: [s1.id],
			...
			for (let j = i + 1; j < n; j++) {
				...
				if (s1.doctorId === s2.doctorId) {
					conflicts.push({
						...
						severity: "error",
						message: `Коллизия врача: ${s1.doctorName} одновременно назначен(а) на 2 смены (${s1.startTime}–${s1.endTime} в кресле ${s1.chairId} и ${s2.startTime}–${s2.endTime} в кресле ${s2.chairId})`,
						dateIso,
				...
				if (s1.assistantId && s2.assistantId && s1.assistantId === s2.assistantId) {
					...
					conflicts.push({
						...
						severity: "error",
						message: `Коллизия ассистента: ${asstName} одновременно назначен(а) к двум врачам (${s1.doctorName} и ${s2.doctorName}) в пересекающееся время (${s1.startTime}–${s1.endTime} / ${s2.startTime}–${s2.endTime})`,
						dateIso,
						shiftIds: [s1.id, s2.id],
				...
				if (s1.chairId === s2.chairId) {
					conflicts.push({
						...
						severity: "error",
						message: `Коллизия кресла: Кресло ${s1.chairId} занято одновременно двумя врачами (${s1.doctorName} и ${s2.doctorName}) в интервале ${s1.startTime}–${s1.endTime} / ${s2.startTime}–${s2.endTime}`,
						dateIso,
						shiftIds: [s1.id, s2.id],
			...
			for (const [weekKey, record] of weeksMap.entries()) {
				if (record.totalHours > weeklyLimit) {
					...
					conflicts.push({
						...
						severity: "warning",
						message: `Превышение недельного лимита ТК РФ: ${staffName} запланировано ${record.totalHours.toFixed(1)} ч за неделю ${weekKey} (норма: ${weeklyLimit} ч, переработка: +${excess.toFixed(1)} ч)`,
