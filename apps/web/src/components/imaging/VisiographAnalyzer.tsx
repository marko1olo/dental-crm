	...
	const processFile = useCallback(
		async (file: File) => {
			...
			try {
				...
				if (
					aiResult.toothStates &&
					Object.keys(aiResult.toothStates).length > 0
				) {
				...
				<div style={{ padding: "16px" }}>
							...
							{isAnalyzing ? (
								...
								>
									<UploadCloud
										size={36}
										style={{
											color: isDragOver ? "var(--teal)" : "var(--text-muted)",
										}}
									/>
									<p
										style={{
											margin: 0,
											fontWeight: 600,
											color: isDragOver ? "var(--teal)" : "var(--text)",
										}}
									>
										{isDragOver
											? "Отпустите снимок"
					...
					{error && (
						<div
							style={{
								padding: "12px 16px",
								background: "var(--error-surface, #fff0f0)",
						...
						>
							...
							{toothStatesArray.length > 0 && (
								<div>
									...
									>
										Зубы из анализа ({toothStatesArray.length} поз.) · обновлено
										в формуле
									</div>
									<div
