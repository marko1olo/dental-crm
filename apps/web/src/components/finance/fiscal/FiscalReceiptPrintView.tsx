...
export interface FiscalReceiptPrintViewProps {
	...
	readonly initialTapeWidth?: FiscalTapeWidth;
}
...
export const FiscalReceiptPrintView: React.FC<FiscalReceiptPrintViewProps> = ({
	...
	initialTapeWidth = "80mm",
}) => {
	const [tapeWidth, setTapeWidth] = useState<FiscalTapeWidth>(initialTapeWidth);
	...
	const dynamicFnsQrString = React.useMemo(() => {
		if (qrString) return qrString;
		return buildFnsFiscalReceiptQrString({
			totalRubFormatted,
			fnSerial,
			fiscalDocNumber,
			fiscalSign,
			operationType: isIncomeReturn ? "income_return" : "income",
		});
	}, [qrString, totalRubFormatted, fnSerial, fiscalDocNumber, fiscalSign, isIncomeReturn]);
	...
	const is58mm = tapeWidth === "58mm";

	return (
		<div className="space-y-4">
			{/* Thermal Width Selector Bar */}
			<div className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-100 dark:bg-slate-800/70 rounded-2xl border border-slate-200 dark:border-slate-700 max-w-sm mx-auto">
				<div className="flex items-center gap-1.5 text-xs font-bold text-slate-700 dark:text-slate-300">
					<Printer className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
					<span>Ширина ленты:</span>
				</div>
				<div className="flex items-center gap-1 bg-slate-200 dark:bg-slate-900 p-0.5 rounded-xl">
					<button
						type="button"
						...
						data-testid="tape-width-58mm-btn"
					>
						58 мм (Узкий)
					</button>
					<button
						...
						data-testid="tape-width-80mm-btn"
					>
						80 мм (Широкий)
					</button>
				</div>
			...
			<div
				className={`fiscal-receipt-view font-mono leading-tight bg-white text-slate-950 rounded-2xl border border-slate-300 shadow-md mx-auto select-text transition-all ${
					is58mm ? "w-[260px] max-w-[260px] p-3 text-[10px]" : "w-[360px] max-w-sm p-5 text-[11px]"
				}`}
				data-testid="fiscal-receipt-print-view"
			>
				{/* Clinic Header */}
				<div className="text-center space-y-1 pb-3 border-b border-dashed border-slate-400">
					<h3 className={`font-black uppercase tracking-wider ${is58mm ? "text-[11px]" : "text-xs"}`}>
						{clinicName}
					</h3>
						...
						{items.map((it, idx) => {
							...
							return (
								<div key={it.id || idx} className="space-y-0.5">
									<div className={`font-bold text-slate-900 leading-snug ${is58mm ? "text-[10px]" : "text-[11px]"}`}>
									...
									{it.markingCode && (
										<div className="flex items-center gap-1 text-[9px] text-emerald-800 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200">
											...
											<span className="truncate">[М] Честный ЗНАК (Тег 1163): {it.markingCode.slice(0, 20)}...</span>
										</div>
									)}
									...
									<div className="flex justify-between text-[9px] text-slate-500">
										<span>Без НДС (ст. 149 НК РФ / Тег 1199 = 6)</span>
				...
				<div className="py-3 space-y-1.5 border-b border-dashed border-slate-400">
					<div className="flex justify-between items-baseline font-black text-sm text-slate-950 pt-1">
						...
						<span className={`${is58mm ? "text-sm" : "text-base"} font-extrabold`}>{totalRubFormatted} ₽</span>
					</div>
				...
				<div className="py-3 border-b border-dashed border-slate-400 space-y-3">
					...
					<div className="pt-2 flex justify-between items-end text-[10px] text-slate-700">
						<div>
							...
							<div className="w-24 border-b border-slate-900 mt-3"></div>
						</div>
						<div className="text-right">
							<span className="font-semibold text-slate-900">{cashierFullName}</span>
							<div className="text-[8px] text-slate-400">(расшифровка)</div>
						</div>
					</div>
				</div>
				...
				<div className="pt-3 space-y-2 text-[9px] text-slate-600">
					...
					<div className="text-center pt-2 space-y-1.5">
						...
						<div className="space-y-0.5">
							...
							<div className="text-[8px] font-mono text-slate-600 truncate max-w-[220px] mx-auto bg-slate-100 p-1 rounded">
								{dynamicFnsQrString}
							</div>
							{ofdVerificationUrl && (
			...
			<div className="flex justify-center">
				<button
					...
					className="min-h-[44px] px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm bg-emerald-600 text-white hover:bg-emerald-700 flex items-center gap-2 cursor-pointer transition-all shadow-md active:scale-95"
				>
					<Printer className="w-4 h-4" />
					<span>Печать термочека ({tapeWidth})</span>
				</button>
			</div>
		</div>
	);
