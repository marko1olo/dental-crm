...
export const FiscalReceipt54FzModal: React.FC<FiscalReceipt54FzModalProps> = ({
	...
	const handleExecuteFiscalization = async (overrideTender?: "card" | "cash" | "sbp") => {
		...
		if (overrideTender === "card") {
			...
			setDepositAmount(0);
			setCertificateAmount(0);
			activeSplit = { ...splitInput, cardRub: totalSumRub, cashRub: 0, sbpRub: 0, depositRub: 0 };
			...
			setDepositAmount(0);
			setCertificateAmount(0);
			activeSplit = { ...splitInput, cashRub: totalSumRub, receivedCashRub: totalSumRub, cardRub: 0, sbpRub: 0, depositRub: 0 };
			...
			setDepositAmount(0);
			setCertificateAmount(0);
			activeSplit = { ...splitInput, sbpRub: totalSumRub, cardRub: 0, cashRub: 0, depositRub: 0 };
		} else if (activeTab === "payment" && !allocation.isFullyAllocated) {
			// Mandate 8e: Auto-balance remainder instead of blocking doctors/cashiers!
			if (remainingRub > 0) {
				if (cardAmount > 0) {
					const newCard = Math.max(0, cardAmount + remainingRub);
					setCardAmount(newCard);
					activeSplit = { ...splitInput, cardRub: newCard };
				} else if (cashAmount > 0) {
					const newCash = Math.max(0, cashAmount + remainingRub);
					setCashAmount(newCash);
					activeSplit = { ...splitInput, cashRub: newCash, receivedCashRub: Math.max(receivedCashRub, newCash) };
				} else if (sbpAmount > 0) {
					const newSbp = Math.max(0, sbpAmount + remainingRub);
					setSbpAmount(newSbp);
					activeSplit = { ...splitInput, sbpRub: newSbp };
				} else {
					setCardAmount(totalSumRub);
