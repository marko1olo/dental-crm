...
export function AnesthesiaQuickBar({
	...
	const handleNurseQuickDisposal = (carpulesCount = 1.0) => {
		if (disabled) return;
		showQuickToast(
			`Списана пустая карпула ${selectedDrugInfo.tradeNamesRu[0]} (${carpulesCount} шт.): отходы Класса Б, списано в 1 клик (СанПиН 3.3686-21 Класс Б, без комиссии)`,
			4000,
		);
	...
	const handleNursePacketDisposal = () => {
		if (disabled) return;
		showQuickToast(
			"Списана 1 карпула Артикаин 1:100 000 + игла 30G: списано в 1 клик (СанПиН 3.3686-21 Класс Б, без комиссии, мягкий овердрафт)",
			4000,
		);
		if (onDisposalCarpules) {
	...
	const handleNurseSeptanestDisposal = () => {
		if (disabled) return;
		showQuickToast(
			"Списана 1 карпула Септанест 1:100 000 (1.7 мл): отходы Класса Б, списано в 1 клик (СанПиН 3.3686-21 Класс Б, без комиссии)",
			4000,
		);
		if (onDisposalCarpules) {
			onDisposalCarpules(1.0, "articaine_1_100k");
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pt-1 border-t border-[var(--line)]/50">
				...
				<div className="flex items-center gap-2 flex-wrap">
					...
					<button
						...
						title="Списать пустые карпулы анестетика в 1 клик (СанПиН 3.3686-21 Класс Б, без комиссии)"
