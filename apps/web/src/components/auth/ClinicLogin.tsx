...
export function ClinicLogin({ onLoginSuccess }: ClinicLoginProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (data.clinicProfile?.organizationId)
				localStorage.setItem(
					"dente_organization_id",
					data.clinicProfile.organizationId,
				);
		...
		<div className="auth-overlay">
			...
			<div className="auth-modal animate-fade-in-up">
				...
				<form onSubmit={handleSubmit} className="auth-form">
					<div className="auth-form-group">
						<label className="auth-label">
							<Building size={12} className="auth-icon-inline" /> Логин / Email
							клиники
						</label>
						<input
							type="text"
				...
				<div className="auth-footer-hints auth-footer-hints--border">
					<p>
						Авторизация требуется для загрузки расписания и шифрования данных.
						<br />
						Демо-доступ: <code>clinic@example.com</code> /{" "}
						<code>dente2026</code>
					</p>
				</div>
			</div>
