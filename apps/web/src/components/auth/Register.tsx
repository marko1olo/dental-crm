...
export function Register({ onSuccess, onSwitchToLogin }: RegisterProps) {
  ...
  const passwordMismatch = touched.confirmPassword && confirmPassword && password !== confirmPassword;
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div className="auth-form-group">
            ...
            <div className="auth-input-wrapper">
              ...
              />
              <button type="button" className="auth-input-icon-btn" onClick={() => setShowPassword(v => !v)} tabIndex={-1}>
            ...
            {password && (
              <div className="auth-password-strength">
                {[1, 2, 3].map(i => (
                  <div key={i} className={`auth-password-strength-bar${strength.score >= i ? ' ' + strengthClass : ''}`} />
                ))}
          ...
          <div className="auth-form-group">
            <label className="auth-label"><KeyRound size={12} className="auth-icon-inline" /> Подтвердите пароль</label>
            <div className="auth-input-wrapper">
              <input
            ...
            {passwordMismatch && <p className="auth-field-error">Пароли не совпадают</p>}
          </div>

          <button type="submit" disabled={loading} className="auth-submit-btn">
            {loading ? <div className="auth-spinner"></div> : <>Зарегистрировать клинику <ArrowRight size={16} /></>}
          </button>
        </form>
