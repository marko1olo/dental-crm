...
import { UserX, Calendar, Phone, RefreshCw, AlertTriangle } from "lucide-react";
...
export const LostPatientsPanel: React.FC = () => {
  ...
  const fetchLostPatients = async () => {
    ...
    setError(null);
    try {
      const headers: Record<string, string> = auth ? auth.denteClinicalReadHeaders() : {};
      const response = await fetch("/api/analytics/lost-patients-filters", { headers });
      if (!response.ok) {
      ...
      setError(err instanceof Error ? err.message : "Не удалось загрузить список потерянных пациентов");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-bold text-base leading-tight">Потерянные пациенты ({patients.length})</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Пациенты без будущих визитов — требуют обзвона и приглашения на профгигиену
            </p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div role="alert" className="p-3 rounded-lg border text-xs bg-rose-50 text-rose-800 border-rose-200 dark:bg-rose-950/60 dark:text-rose-200 dark:border-rose-800 flex items-center gap-2">
