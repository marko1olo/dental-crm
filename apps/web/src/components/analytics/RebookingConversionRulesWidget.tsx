...
export const RebookingConversionRulesWidget: React.FC = () => {
	...
	return (
		...
		>
			{/*
				БЫЛО в заголовке: «Справедливое Распределение Конверсии Повторной
				Записи (Порог 15 Минут)» — заглавная буква в каждом слове (так в
				русском языке заголовки не пишутся) и слово «конверсия» вместо
				объяснения. В плашке рядом стояло «Врач vs Администратор KPI»:
				латинское «vs» и «KPI» в интерфейсе русской клиники.
			*/}
			<div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-200 dark:border-slate-800" title="Если пациент записался на следующий приём в течение 15 минут после визита, запись засчитывается врачу. Позже — администратору.">
				<div className="flex items-center space-x-2">
					<span className="text-xl">⚖️</span>
					<h3 className="font-semibold text-emerald-600 dark:text-emerald-400">
						Кому засчитана повторная запись
					</h3>
				</div>
						...
						>
							...
							<div className="flex items-center space-x-2">
								<span className="text-xs px-2 py-0.5 rounded border font-bold bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-950 dark:text-emerald-300 dark:border-emerald-800">
									Засчитано: {rule.creditedRoleLabel}
								</span>
							</div>
						</div>
					))}
