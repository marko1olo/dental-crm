...
/**
 * Какую долю мишени панель должна накрыть, чтобы мишень считалась
 * ЗАБЛОКИРОВАННОЙ и панель ей уступила.
 *
 * Замер живой страницы (`scratch/probe-corner-obstacles.mjs`, экран пациента)
 * объясняет, зачем этот порог нужен. Под панелью там лежит не кнопка, а
 * `<article class="clickable-card" role="button" tabindex="0">` — плитка
 * быстрого действия размером 364x113 (окно 390x844) и 399x168 (окно 840x900).
 * Панель накрывала 16% и 12% её площади соответственно, то есть плитка
 * оставалась нажимаемой почти целиком, — и всё равно панель считала её помехой и
 * уезжала вверх. А `.patient-feature-grid` — это СТЕНА таких плиток высотой
 * 368px: уступив первой, панель попадала на вторую, потом на третью, и
 * поднималась на 299px, то есть уходила почти на середину экрана, оставаясь при
 * этом на плитке. Замерено: подъём 116px -> 299px после включения досъёма.
 *
 * Половина площади — граница между «мишень скрыта» и «мишень задета». Маленькую
 * кнопку «Сохранить» панель накрывает целиком (доля 1.0) и обязана уступить;
 * большую карточку она задевает углом, и уступать нечего — нажать карточку можно
 * где угодно ещё. Вложенные в карточку настоящие кнопки при этом никуда не
 * исчезают: `document.elementsFromPoint` возвращает весь стек, и каждый элемент
 * стека проверяется отдельно по своей собственной площади.
 */
export const CORNER_OBSTACLE_BLOCK_SHARE = 0.5;
...
/**
 * Заблокирует ли панель эту мишень, если встанет в положение `footprint`.
 *
 * Ответ на вопрос «уступать ли» состоит из двух независимых частей, и раньше
 * была только первая: `isCornerObstacle` отвечает «может ли пользователь это
 * нажать», а эта функция — «отнимет ли панель у него эту возможность». Без
 * второй части панель уступала любому большому кликабельному контейнеру, задев
 * его углом.
 */
export function cornerBlocksTarget(
	footprint: CornerRect,
	target: CornerRect,
	share: number = CORNER_OBSTACLE_BLOCK_SHARE,
): boolean {
	const area =
		(target.right - target.left) * (target.bottom - target.top);
	if (area <= 0) return false;
	return cornerOverlapArea(footprint, target) / area >= share;
}
