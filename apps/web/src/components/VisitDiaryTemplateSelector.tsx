import { Baby, CheckCircle2, Clipboard, Crown, Download, HeartPulse, Layers, Loader2, Plus, ShieldCheck, Smile, Sparkles, Trash2, X } from "lucide-react";
...
export function VisitDiaryTemplateSelector({
	...
	const quickSoapButtonsEl = (
		...
		>
			...
			{/* 4 ортопедических 1-клик протокола (Мандаты 8e, 8i, 8k, 8n) */}
			<span className="text-[11px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider flex items-center gap-1 ml-1 sm:ml-2">
				<Crown className="w-3.5 h-3.5 text-amber-500" /> Ортопедия 1-Клик:
			</span>
			<button
				type="button"
		...
		showCreateForm && !isLocked ? (
			...
			>
				...
				<button
					...
					title={
						!createTitle.trim()
							? "Укажите название протокола перед сохранением"
							: isCreating
								? "Сохраняю протокол в базу клиники…"
								: "Сохранить протокол в список клиники"
					}
		...
		return (
			...
			>
				...
				<button
					...
					title={
						isLocked
							? "Дневник подписан. Установка протоколов заблокирована."
							: isSeeding
								? "Устанавливаю встроенные протоколы в базу…"
								: "Установить встроенные протоколы в базу клиники"
					}
					className="inline-flex items-center justify-center gap-2 px-3 py-2 min-h-[44px] text-sm font-medium text-[var(--on-teal,white)] bg-[var(--teal-dark)] hover:bg-[var(--teal)] rounded-xl disabled:opacity-50 transition-colors"
				>
					{isSeeding ? (
				...
				{canCreate && !showCreateForm ? (
					<button
						...
						title="Создать свой протокол приёма в списке клиники"
						...
						title={
							isLocked
								...
								: isLoading
									...
									: "Выберите клинический протокол для автозаполнения дневника"
						}
