...
describe("плашка сохранения на живом экране картотеки", () => {
	...
	test("экран монтирует плашку в оба места и не держит своей копии правила", () => {
		const source = readFileSync(join(here, "..", "..", "PatientsView.tsx"), "utf8");
		...
		assert.ok(
			!source.includes(': "Сохранено"'),
			"в разметку вернулась своя ветка «Сохранено» — правило живёт в одном месте, " +
				"patientCardSavePill.tsx, и прогоняется",
		);
		assert.ok(
			!source.includes(': "Заполнено"'),
			"в разметку вернулась безусловная ветка «Заполнено»: она утверждала, что паспорт, ИНН и " +
				"СНИЛС внесены, у пациента с пустыми реквизитами",
		);
	});
});
