...
const LOYALTY_CONFIG: Record<LoyaltyTier, { label: string; discountPct: number; color: string }> = {
...
export function PatientLoyaltyHeader({ patientId }: { patientId: string }) {
	...
	const handleSetTier = async (tier: LoyaltyTier) => {
		...
		setSaving(true);
		try {
			const res = await fetch(`/api/patients/${patientId}/administrative-profile`, {
				method: "PUT",
	...
	return (
		<div style={{ position: "relative" }}>
			<button
				...
				onMouseEnter={(e) => {
					if (!saving) e.currentTarget.style.background = `color-mix(in srgb, ${currentLoyalty.color} 15%, transparent)`;
				}}
				onMouseLeave={(e) => {
					if (!saving) e.currentTarget.style.background = `color-mix(in srgb, ${currentLoyalty.color} 10%, transparent)`;
				}}
				title="Изменить статус лояльности"
			>
				{saving ? (
					<Loader2 size={14} color={currentLoyalty.color} className="animate-spin" />
				) : (
				...
				<ChevronDown size={12} color={currentLoyalty.color} style={{ marginLeft: "2px", opacity: 0.7 }} />
			</button>
			...
			<AnimatePresence>
				{isOpen && (
					<>
						...
						>
							{(Object.entries(LOYALTY_CONFIG) as [LoyaltyTier, typeof LOYALTY_CONFIG[LoyaltyTier]][]).map(([tierKey, config]) => (
								<button
									key={tierKey}
									type="button"
									...
									style={{
										...
										padding: "8px 12px",
										borderRadius: "6px",
										background: currentTier === tierKey ? "var(--surface-100)" : "transparent",
										border: "none",
										cursor: "pointer",
										textAlign: "left",
										fontSize: "13px",
									...
									onMouseEnter={(e) => {
										if (currentTier !== tierKey) e.currentTarget.style.background = "var(--surface-50)";
									}}
									onMouseLeave={(e) => {
										if (currentTier !== tierKey) e.currentTarget.style.background = "transparent";
									}}
								>
