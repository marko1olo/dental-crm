...
export const BulkImageOperationLogsWidget: React.FC = () => {
	...
	return (
		<div
			...
			className="p-4 bg-white dark:bg-slate-900 border border-violet-500/30 rounded-xl text-slate-900 dark:text-slate-100 shadow-sm my-4"
		>
			<div className="flex items-center justify-between mb-3 border-b border-slate-200 dark:border-slate-700/60 pb-2">
				<div className="flex items-center space-x-2">
					...
					<h3 className="font-semibold text-violet-700 dark:text-violet-400">
						Пакетное связывание снимков и свойств изображений
					</h3>
				</div>
				<span className="text-xs bg-violet-100 dark:bg-violet-500/20 text-violet-700 dark:text-violet-300 px-2 py-0.5 rounded border border-violet-300 dark:border-violet-500/40 font-medium">
					Массовые операции
				</span>
			</div>

			{loading ? (
				<div className="text-slate-500 dark:text-slate-400 text-sm py-4">Загрузка журнала массовых операций...</div>
			) : logs.length === 0 ? (
				<div className="text-slate-500 dark:text-slate-400 text-sm py-3 text-center">
					Нет недавних массовых операций с снимками
				</div>
			) : (
						...
						>
							<div>
								...
								<div className="text-xs text-slate-600 dark:text-slate-300 mt-1">
									Операция: <span className="text-violet-600 dark:text-violet-300 font-semibold">{item.operationType}</span> ({item.selectedImagesCount} файлов)
								</div>
							</div>
							<div className="flex items-center space-x-2 text-xs">
								<span className="bg-violet-100 dark:bg-violet-950 text-violet-800 dark:text-violet-300 px-2.5 py-1 rounded border border-violet-300 dark:border-violet-800 font-mono">
