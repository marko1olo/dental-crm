					...
					{taskTypes.map((item) => (
						...
						>
							<div className="flex items-center justify-between">
								<span className="font-bold text-sm text-slate-900 dark:text-white" style={{ color: item.colorHex || undefined }}>
									{item.typeLabel}
								</span>
								<span className="text-xs font-mono text-indigo-700 dark:text-indigo-300 font-semibold flex items-center gap-1">
									<CheckCircle2 className="w-3 h-3" /> {item.typeCode}
								</span>
							</div>
							<div className="text-xs text-slate-600 dark:text-slate-400">
								SLA: <span className="font-semibold text-slate-900 dark:text-slate-200">{item.defaultSlaHours}ч</span> · Привязка к пациенту: {item.requiresPatientBinding ? "Да" : "Нет"}
							</div>
						</div>
					))}
				</div>
