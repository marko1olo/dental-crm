	...
	switch (mischClass) {
		case "D1":
			return {
				...
				clinicalAdvice: [
					"⚠️ Обязательно использование кортикального метчика (Bone Tap) на всю длину имплантата во избежание микропереломов и заклинивания.",
					"❄️ Обильное охлаждение стерильным физраствором (4°C) на низких оборотах (400 RPM): критический риск термического остеонекроза!",
		...
		case "D2":
			return {
				...
				clinicalAdvice: [
					...
					"🚀 Превосходный кандидат для немедленной функциональной нагрузки (Immediate Loading) при торке >= 35 N·cm.",
		...
		case "D3":
			return {
				...
				clinicalAdvice: [
					...
					"При торке < 30 N·cm предпочтителен двухэтапный протокол с установкой формирователя через 3 мес.",
				],
			};
		...
		case "D4":
			return {
				...
				clinicalAdvice: [
					"⚠️ Недопрепарирование (Under-Drilling): завершайте сверление на 1-2 шага меньше номинального диаметра имплантата.",
...
export function generateMischDrillSequence(
	...
	if (cls === "D1") {
		steps.push({
			...
			targetRpm: 25,
			maxRpm: 50,
			depthGuideRu: "Обязательное нарезание резьбы в плотной кости D1 при 25–30 N·cm",
...
export function formatMischProtocolToDiaryText(
	...
	const lines = [
		...
		"- Прогноз торка фиксации: " + analysis.estimatedInsertionTorqueNcm.expectedNcm + " N·cm (ISQ ~" + analysis.estimatedIsqScore.expectedIsq + ")",
