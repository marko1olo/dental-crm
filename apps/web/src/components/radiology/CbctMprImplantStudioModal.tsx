...
export const CbctMprImplantStudioModal: React.FC<CbctMprImplantStudioModalProps> = ({
	...
	// Slice Reslice Caching Refs (Skips re-extraction when slice coordinate & WW/WL did not change)
	const lastAxialSliceKeyRef = useRef<string>("");
	const lastCoronalSliceKeyRef = useRef<string>("");
	const lastSagittalSliceKeyRef = useRef<string>("");
	const lastAxialMetadataRef = useRef<MprSliceMetadata | null>(null);
	const lastCoronalMetadataRef = useRef<MprSliceMetadata | null>(null);
	const lastSagittalMetadataRef = useRef<MprSliceMetadata | null>(null);
	...
	const handleResetAll = useCallback(() => {
		...
		setProbeMarkers([]);
		setActiveProbe(null);
		lastAxialSliceKeyRef.current = "";
		lastCoronalSliceKeyRef.current = "";
		lastSagittalSliceKeyRef.current = "";
