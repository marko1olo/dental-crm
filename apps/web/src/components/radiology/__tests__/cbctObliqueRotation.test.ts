...
describe("CBCT Oblique MPR Rotation, Sub-Voxel Trilinear & Interactive Navigation Suite", () => {
	...
	describe("5. Oblique MPR Slice Extraction (Axial, Coronal, Sagittal)", () => {
		...
		it("extracts valid oblique coronal and sagittal slices", () => {
			...
			assert.equal(coronal.metadata.heightPx, Math.round((testVolume.dimensions.depth * testVolume.spacingMm.z) / testVolume.spacingMm.x));
			...
			assert.equal(sagittal.metadata.heightPx, Math.round((testVolume.dimensions.depth * testVolume.spacingMm.z) / testVolume.spacingMm.y));
		});
