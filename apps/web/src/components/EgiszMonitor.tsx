...
import { ShieldCheck, AlertTriangle, RefreshCcw, CheckCircle2 } from "lucide-react";
...
export const EgiszMonitor: React.FC<EgiszMonitorProps> = ({ patientId, visitId }) => {
  const [status, setStatus] = useState<"Pending" | "Sent" | "Error" | "Accepted">("Pending");
    ...
    <div className="glass-panel p-4 rounded-xl flex items-center justify-between mt-4 border border-white/10 bg-black/40 backdrop-blur-md shadow-lg">
      <div className="flex items-center gap-3">
        {status === "Accepted" ? (
          ...
          <RefreshCcw className={`text-blue-400 w-6 h-6 ${isLoading ? "animate-spin" : ""}`} />
        )}
        <div>
          <h3 className="text-sm font-semibold text-white">Интеграция с ЕГИСЗ (РЭМД)</h3>
          <p className="text-xs text-gray-300">
            {status === "Accepted" && transactionId ? (
              <span className="text-emerald-300">Успешно выгружено. Транзакция: {transactionId}</span>
            ) : status === "Error" ? (
