...
export function DoctorSnilsValidationWidget({ initialSnils = "", onValidSnils }: DoctorSnilsValidationWidgetProps) {
  ...
  async function handleValidate() {
    if (!snils.trim()) {
      setValidationResult({ ok: false, message: "Введите СНИЛС врача для проверки." });
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.ok) {
        ...
        setValidationResult({
          ok: false,
          message: data.message || "СНИЛС врача не прошёл проверку формата или контрольного числа ЕГИСЗ.",
        });
      }
    } catch (err) {
      setValidationResult({
          ...
          <div>
            ...
            {validationResult.formatted ? (
              <p className="text-[11px] opacity-90 mt-0.5">
                Форматированный СНИЛС: <code className="font-mono bg-white/50 dark:bg-black/40 px-1 rounded">{validationResult.formatted}</code>
              </p>
            ) : null}
          </div>
        </div>
