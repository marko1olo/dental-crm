 ...
 * LegacyMigrationStudio.tsx — это две с половиной тысячи строк перевода
 * советов: он показывает чек-листы готовности, «bridge kit» и操作 инструкции,
 * потому что за ним стоит маршрут, который ничего не переносит. Здесь мастер
 * работает с настоящим движком: файл заливается, колонки сопоставляются,
 * прогон идёт в фоне, в конце выдаётся акт сверки.
 *
...
interface MapResponse {
  ...
  qualityFindings: Array<{ severity: "info" | "warning" | "blocker"; message: string; affectedRows: number }>;
...
interface RunStatus {
  ...
  staging: { total: number; ready: number; loaded: number; quarantined: number };
}

interface ReconciliationResponse {
  balanced: boolean;
  checks: Array<{ code: string; title: string; expected: number; actual: number; passed: boolean; detail: string }>;
  ...
  money: { sourceTotalRub: number | null; loadedTotalRub: number | null; quarantinedTotalRub: number | null };
...
async function readResponse<T>(response: Response): Promise<{ ok: true; data: T } | { ok: false; message: string; code: string }> {
  const text = await response.text();
  let parsed: unknown = null;
  try {
    parsed = text ? JSON.parse(text) : null;
  } catch {
    return { ok: false, code: "BadResponse", message: `Сервер вернул не JSON (код ${response.status}).` };
  }
  ...
  if (!response.ok) {
    ...
    return { ok: false, code: "HttpError", message: `Запрос не выполнен (код ${response.status}).` };
  }
...
export function MigrationWizard() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setError({ code: "NetworkError", message: caught instanceof Error ? caught.message : "Файл не отправлен." });
    } finally {
      setBusy(false);
    }
  ...
  const runMapping = useCallback(async (runId: string, useLlm: boolean) => {
    ...
    try {
      ...
      setError({ code: "NetworkError", message: caught instanceof Error ? caught.message : "Сопоставление не выполнено." });
    } finally {
      setBusy(false);
    }
  }, []);
      ...
      try {
        ...
        pollTimerRef.current = window.setInterval(() => {
          void (async () => {
            ...
            if (!state) return;
            const finished = ["completed", "completed_with_quarantine", "failed", "validated", "rolled_back"].includes(
              state.run.status
            );
            if (finished) {
              if (pollTimerRef.current !== null) window.clearInterval(pollTimerRef.current);
        ...
        setError({ code: "NetworkError", message: caught instanceof Error ? caught.message : "Прогон не запущен." });
        setBusy(false);
      }
    },
    ...
    <section className="migration-wizard">
      <header className="mw-head">
        <div>
          ...
          <p className="mw-subtitle">
            Файл читается целиком, каждая строка сохраняется дословно. В боевые таблицы ничего не пишется,
            пока вы не нажмёте «Перенести в базу».
          </p>
        </div>
        <button type="button" className="mw-btn mw-btn-ghost" onClick={() => void runDiscovery()} disabled={busy}>
      ...
      <ol className="mw-steps" aria-label="Этапы переноса">
        {steps.map((item, index) => (
          ...
          >
            <span className="mw-step-dot">{index < currentIndex ? "✓" : index + 1}</span>
      ...
      {error !== null && (
        <div className="mw-alert mw-alert-bad" role="alert">
          ...
          <button type="button" className="mw-alert-close" onClick={resetError} aria-label="Закрыть">
            ×
          </button>
        </div>
      )}
      ...
      {discovery !== null && (
        <DiscoveryPanel discovery={discovery} onClose={() => setDiscovery(null)} />
      )}
      ...
      {step === "running" && <RunningPanel status={status} dryRun={lastRunWasDry} />}
    ...
    <div className="mw-panel">
      ...
      >
        ...
        <p className="mw-drop-hint">
          DBF (FoxPro, dBASE, с memo-файлами), SQLite, CSV и TSV в любой кодировке, XLSX, JSON, XML
        </p>
        <button
          type="button"
      ...
      <label className="mw-toggle">
        ...
        <span>
          ...
          <em className="mw-toggle-note">
            Модель получает только статистику колонки и маски вида «99.99.9999» — ни одного значения из
            карточек пациентов ей не передаётся.
          </em>
        </span>
      </label>

      <div className="mw-note">
        <strong>Закрытые форматы.</strong> Firebird (IDENT), MS SQL (DentalPRO), Access и 1С читать напрямую
        нельзя — это страничные форматы, привязанные к своему серверу. Нажмите «Найти базы на сервере»: движок
        опознает их и подскажет, чем открыть и что выгрузить.
      </div>
    </div>
  );
}
...
function MappingPanel(props: {
  ...
  const blockers = mapping?.qualityFindings.filter((item) => item.severity === "blocker") ?? [];
    ...
    <div className="mw-panel">
        ...
        <div className="mw-source-main">
          ...
          <span className="mw-source-meta">
            ...
            {upload.source.encodingConfidence < 0.8 && (
              <em className="mw-uncertain"> (определена неуверенно — проверьте ФИО ниже)</em>
            )}
          </span>
        </div>
      ...
      {upload.previousRunWithSameFile !== null && (
        <div className="mw-alert mw-alert-info">
          Этот файл уже загружался {new Date(upload.previousRunWithSameFile.uploadedAt).toLocaleString("ru-RU")}.
          Повторный перенос не создаст дублей: уже перенесённые записи будут обновлены.
        </div>
      )}

      {props.busy && mapping === null && <div className="mw-loading">Определяем колонки…</div>}
        ...
        <>
          <div className="mw-projection">
            ...
            <div className="mw-proj-item mw-proj-warn">
              <span className="mw-proj-value">{mapping.projectedQuarantine}</span>
              <span className="mw-proj-label">в карантин</span>
            </div>
            <div className="mw-proj-item">
              <span className="mw-proj-value">{mapping.mapping.columns.length}</span>
              <span className="mw-proj-label">колонок сопоставлено</span>
            </div>
            {mapping.llm.calls > 0 && (
              <div className="mw-proj-item">
                <span className="mw-proj-value">{mapping.llm.rejectedSuggestions}</span>
                <span className="mw-proj-label">ответов модели отклонено</span>
              </div>
            )}
          </div>

          <div className="mw-mapping-table" role="table" aria-label="Соответствие колонок">
            ...
            {mapping.mapping.columns.map((column) => (
              <div className="mw-mapping-row" role="row" key={column.sourceColumn}>
                ...
                <span role="cell">
                  <span className={`mw-badge mw-badge-${column.decidedBy}`} title={column.rationale}>
                    {DECISION_TITLES[column.decidedBy]}
                  </span>
                  <span className="mw-confidence">{Math.round(column.confidence * 100)}%</span>
                </span>
          ...
          {mapping.mapping.unmappedColumns.length > 0 && (
            <div className="mw-alert mw-alert-warn">
              Не сопоставлены: {mapping.mapping.unmappedColumns.join(", ")}. Их содержимое сохранится в исходном
              виде, но в поля карточки не запишется.
            </div>
          )}
          ...
          {mapping.qualityFindings.length > 0 && (
            <details className="mw-findings" open={blockers.length > 0}>
              <summary>
                ...
                {blockers.length > 0 && <span className="mw-findings-bad"> · {blockers.length} блокирующих</span>}
              </summary>
              <ul>
                {mapping.qualityFindings.slice(0, 20).map((finding, index) => (
                  <li key={index} className={`mw-finding mw-finding-${finding.severity}`}>
                    {finding.message}
                    {finding.affectedRows > 0 && <span className="mw-finding-rows"> — строк: {finding.affectedRows}</span>}
                  </li>
                ))}
              </ul>
            </details>
          ...
          <div className="mw-actions">
            <button type="button" className="mw-btn mw-btn-primary" onClick={props.onDryRun} disabled={props.busy}>
              Сухой прогон
            </button>
            <span className="mw-actions-note">
              Сухой прогон проверяет всё до последней строки и ничего не записывает. Запись станет доступна
              после него.
            </span>
          </div>
        </>
...
function RunningPanel(props: { status: RunStatus | null; dryRun: boolean }) {
  ...
  const phase = props.status?.run.phase ?? "Задача принята, ожидает исполнителя";

  return (
    <div className="mw-panel mw-running">
      <div className="mw-progress" role="progressbar" aria-valuenow={percent} aria-valuemin={0} aria-valuemax={100}>
      ...
      {props.status !== null && props.status.run.worker.resumeCount > 0 && (
        <div className="mw-alert mw-alert-info">
          Прогон был прерван и возобновлён ({props.status.run.worker.resumeCount}). Продолжение идёт с тех строк,
  ...
  return (
    <div className="mw-panel">
      <div className={`mw-verdict ${report.balanced ? "is-ok" : "is-bad"}`}>
        ...
        <div>
          <strong>{report.balanced ? "Сверка сошлась" : "Сверка НЕ сошлась"}</strong>
          <p>
            {report.balanced
              ? props.dryRun
      ...
      */}
      <div className="mw-counters">
        <Counter label="Строк в источнике" value={status.run.counters.sourceRows} />
        {props.dryRun ? (
          <Counter
            label="Готовы к переносу"
            value={Math.max(0, status.run.counters.sourceRows - status.run.counters.quarantinedRows)}
            tone="ok"
          />
        ) : (
          <>
            <Counter label="Создано" value={status.run.counters.loadedRows} tone="ok" />
            <Counter label="Обновлено" value={status.run.counters.updatedRows} />
            <Counter label="Дублей" value={status.run.counters.duplicateRows} />
          </>
        )}
        <Counter label="В карантине" value={status.run.counters.quarantinedRows} tone="warn" />
        {!props.dryRun && <Counter label="Пропущено" value={status.run.counters.skippedRows} />}
      </div>

      <div className="mw-checks">
        {report.checks.map((check) => (
          <div className={`mw-check ${check.passed ? "is-ok" : "is-bad"}`} key={check.code}>
      ...
      {report.quarantinePreview.length > 0 && (
        <details className="mw-quarantine" open>
          <summary>Карантин: {report.quarantinePreview.length} записей на разбор</summary>
          <ul>
            {report.quarantinePreview.slice(0, 25).map((item) => (
              <li key={item.id} className={item.blocking ? "is-blocking" : ""}>
                <span className="mw-q-reason">{REASON_TITLES[item.reason] ?? item.reason}</span>
                {item.sourceRowNumber !== null && <span className="mw-q-row">строка {item.sourceRowNumber}</span>}
                <span className="mw-q-message">{item.message}</span>
                {item.suggestedFix !== null && <span className="mw-q-fix">{item.suggestedFix}</span>}
              </li>
            ))}
          </ul>
        </details>
      ...
      <div className="mw-actions">
        {props.dryRun && report.balanced && (
          <button type="button" className="mw-btn mw-btn-danger" onClick={props.onLiveRun} disabled={props.busy}>
            Перенести в базу
          </button>
        )}
        {!props.dryRun && (
          <button type="button" className="mw-btn mw-btn-ghost" onClick={props.onRollback} disabled={props.busy}>
            Откатить перенос
          </button>
        )}
...
function Counter(props: { label: string; value: number; tone?: "ok" | "warn" }) {
  return (
...
function DiscoveryPanel(props: { discovery: DiscoveryResponse; onClose: () => void }) {
    ...
    <div className="mw-discovery">
      <header>
        ...
        <span className="mw-discovery-meta">
          просмотрено {discovery.scan.filesScanned} файлов за {(discovery.scan.elapsedMs / 1000).toFixed(1)} с
        </span>
        <button type="button" className="mw-alert-close" onClick={props.onClose} aria-label="Закрыть">
          ×
        </button>
      </header>
          ...
          <ul className="mw-discovery-list">
            {discovery.readySources.slice(0, 10).map((source) => (
              <li key={source.filePath}>
                ...
                {source.details.length > 0 && <span className="mw-d-details">{source.details.join(" ")}</span>}
              </li>
            ))}
          </ul>
        </section>
      ...
      {discovery.needsExportSources.length > 0 && (
        <section>
          <h4>Требуют выгрузки из своей программы — {discovery.summary.needsExport}</h4>
          <ul className="mw-discovery-list">
            {discovery.needsExportSources.slice(0, 10).map((source) => (
              <li key={source.filePath}>
                ...
                <span className="mw-d-path">{source.filePath}</span>
                {source.guidance !== null && <span className="mw-d-guidance">{source.guidance}</span>}
              </li>
            ))}
          </ul>
        </section>
          ...
          <ul className="mw-discovery-list">
            {discovery.imagingFolders.slice(0, 5).map((folder) => (
              <li key={folder.directory}>
                <span className="mw-d-name">{folder.fileCount} файлов DICOM</span>
