...
export const humanizeIntegrationInput = (value: string) => integrationInputLabels[value] ?? humanizeMigrationText(value);
...
export const humanizeMigrationList = (items: unknown[] | undefined, limit = items?.length ?? 0) =>
  (items ?? [])
    .slice(0, limit)
    .map(humanizeMigrationText)
    .filter(Boolean)
    .join(" · ");
export const humanizeMigrationColumns = (items: unknown[] | undefined, limit = items?.length ?? 0) =>
  (items ?? [])
    .slice(0, limit)
...
export const migrationSourceKindLabel = (sourceKind: string) => migrationLegacySourceKindLabels[sourceKind] ?? humanizeMigrationText(sourceKind);
...
export const importWarningListText = (warnings: string[], fallback: string, limit = 4) => {
  if (!warnings.length) return fallback;
  const text = warnings.slice(0, limit).map(humanizeMigrationText).filter(Boolean).join(", ");
  return text || fallback;
};
export const patientImportRowWarningText = (warnings: string[], notes: string | null | undefined) =>
...
export const imagingImportRowWarningText = (warnings: string[], filePath: string | null | undefined) =>
...
export const aiRecognitionWarningText = (warning: string) => aiRecognitionWarningLabels[warning] ?? humanizeMigrationText(warning);
export const dicomFirstFrameFileFormatLabel = (transferSyntaxUid: string | null | undefined) => {
...
export const dicomFirstFrameImageTypeLabel = (photometricInterpretation: string | null | undefined) => {
