						...
						<div className="import-workbench">
							...
							{!smartImportInputReady ? (
								<p
									...
									aria-live="polite"
								>
									Вставьте выгрузку из старой МИС, таблицу, OCR или диктовку
									перед разбором.
								</p>
							) : null}
							{!migrationHandoffReportReady ? (
								...
								>
									...
									найдите источники на ПК, выберите папку/диск или вставьте
									выгрузку.
								</p>
							) : null}
						</div>
							...
							>
								...
								{!typedBrowserMigrationDiscovery.candidates.length ? (
									...
									>
										...
										<span>
											Обычно помогает выбрать корень выше: весь диск, папку
											старой программы, папку снимков, архив выгрузки или
											сетевой экспорт.
										</span>
										<div className="migration-source-card-actions">
											<button
											...
											>
												<div>
													...
													<span>
														...
														· {migrationPrimaryOperatorStep.estimatedMinutes}{" "}
														мин ·{" "}
												...
												<div className="migration-triage-grid">
													{migrationTriageItems.map((item) => (
														...
														>
															...
															<small>
																{humanizeMigrationText(item.detail)}
															</small>
															<small>
																Что нужно:{" "}
										...
										<div className="migration-autopilot-summary">
											{typedMigrationOperatorLanes.slice(0, 5).map((lane) => (
												<article key={lane.id}>
													...
													<small>
														{humanizeMigrationText(lane.nextAction)}
													</small>
												</article>
											))}
										</div>
						...
						{typedMigrationSourceDiscovery ? (
							...
							>
								<div className="dicom-discovery-head">
									<strong>
										Найдено источников:{" "}
										{typedMigrationDiscoveryCandidates.length} · просканировано
										папок: {typedMigrationSourceDiscovery.scannedFolders}
									</strong>
									<span>
										{migrationAutopilot
									...
									<span>
										Карточки ниже уже готовы к плану переноса, проверке
										источника, предпросмотру или разбору.
									</span>
								</div>
								<div className="dicom-discovery-grid">
								...
								{!typedMigrationDiscoveryCandidates.length ? (
									...
									>
										...
										<span>
											...
											вставьте пару строк выгрузки или заполните реквизиты
											клиники для документов.
										</span>
										<div className="migration-source-card-actions">
											<button
												className="secondary-button"
						...
						{typedMigrationSourceWorkup ? (
							...
							>
								<div className="dicom-discovery-head">
									...
									<span>
										{humanizeMigrationText(
											typedMigrationSourceWorkup.nextAction,
										)}
									</span>
									<span>
										{humanizeMigrationText(
						...
						{typedMigrationSourceProbe ? (
							...
							>
								<div className="dicom-discovery-head">
									...
									<span>
										{humanizeMigrationText(
											typedMigrationSourceProbe.sourceLabel,
										)}{" "}
									...
									<span>
										{humanizeMigrationText(
											typedMigrationSourceProbe.nextAction,
										)}
									</span>
									<span>
										{humanizeMigrationText(
								...
								{typedMigrationSourceProbe.warnings
									.slice(0, 4)
									.map((warning) => (
										<small key={warning}>
											{humanizeMigrationText(warning)}
										...
										{typedSmartImportPreview.legacySources.map(
											(source, index) => (
												...
												>
													...
													<p>
														{humanizeMigrationText(source.recommendedRoute)}
													</p>
													<p>
														Нужно:{" "}
