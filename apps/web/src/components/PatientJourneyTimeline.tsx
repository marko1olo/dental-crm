    ...
    <div className="patient-journey-timeline">
      ...
      <div className="zeigarnik-progress-container">
        <div className="progress-header">
          <span className="progress-title">План лечения: Ортопедия (Фаза 2)</span>
          <span className="progress-percentage text-emerald-400">37%</span>
        </div>
        <div className="progress-bar-bg">
          <div className="progress-bar-fill" style={{ width: '37%' }}></div>
        </div>
        <p className="progress-hint">Пройдено 3 процедуры из 8. Следующий визит приблизит вас к завершению плана!</p>
      </div>
      ...
      <div className="timeline-track">
        {events.map((evt, index) => {
          ...
          return (
            <div key={evt.id} className={`timeline-item ${evt.type} ${isHighlight ? 'highlight-item' : ''}`}>
              ...
              <div className="timeline-content">
                <div className="content-header">
                  <span className="timestamp text-xs font-mono text-zinc-400">{new Date(evt.timestamp).toLocaleString()}</span>
                  {evt.status && <span className={`status-badge ${evt.status.toLowerCase().replace(' ', '-')}`}>{evt.status}</span>}
                </div>
                <h4 className={isHighlight ? 'text-lg text-white font-bold' : 'text-base text-zinc-300'}>{evt.title}</h4>
