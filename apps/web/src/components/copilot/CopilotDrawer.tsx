		...
		>
			...
			<div className="copilot-tabs">
				...
				>
					...
					<span>Задачи</span>
					{pendingCount > 0 && (
						<span className="copilot-badge-count">{pendingCount}</span>
					)}
				</button>
			</div>
			...
			<div className="copilot-feed" ref={feedRef}>
				{activeTab === "chat" ? (
					...
					>
						...
						{/* WhatsApp Human-in-the-Loop (HitL) Approval Cards */}
						...
						{/* Proactive Non-HitL Alerts in Tasks Tab (ZTL, Gap Filler, Retention, Diagnostics) */}
						{proactiveAlerts
							.filter(
								(alert) =>
						...
						{!pending &&
							...
							proactiveAlerts.filter(
								...
								>
									Нет ожидающих подтверждений. Все клинические действия
									выполнены.
								</div>
							)}
					</div>
