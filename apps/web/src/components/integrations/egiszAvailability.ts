...
export function resolveEgiszPanelState(
	...
	if (!status.remdTransmission) {
		return {
			...
			tone: "warning",
			headline: "Автономный режим шлюза РЭМД ЕГИСЗ",
			detail:
				"Шлюз настроен. Пакеты документов накапливаются в локальном исходящем буфере " +
				"клиники и будут переданы в РЭМД ЕГИСЗ при активации канала диспетчера.",
			canTransmit: false,
			transmitBlockedReason:
				"Шлюз в режиме локальной буферизации",
			canRetryLoad: false,
		};
	}
