...
export const CollaborativeChatProcessingStatesWidget: React.FC = () => {
	...
	return (
		...
		>
			<div className="flex items-center justify-between mb-3 border-b border-slate-200 dark:border-slate-700/60 pb-2">
				<div className="flex items-center space-x-2">
					...
					<h3 className="font-semibold text-indigo-700 dark:text-indigo-400">
						Командная обработка чатов (Статус «Сотрудник ответил»)
					</h3>
				</div>
			...
			{loading ? (
				<div className="text-slate-500 dark:text-slate-400 text-sm py-4">Загрузка статусов командных чатов...</div>
			) : states.length === 0 ? (
				<div className="text-slate-500 dark:text-slate-400 text-sm py-3 text-center">
					Нет нераспределённых командных диалогов
				</div>
			) : (
						...
						>
							<div>
								<div className="text-sm font-bold text-slate-900 dark:text-slate-200">
									Чат ID: <span className="font-mono text-indigo-700 dark:text-indigo-300">{item.chatId}</span>
								</div>
								<div className="text-xs text-slate-600 dark:text-slate-400 mt-1">
									Ответственный сотрудник: <span className="text-indigo-700 dark:text-indigo-300 font-semibold">{item.assignedAgentName}</span>
								</div>
							</div>
							<div className="flex items-center space-x-2 text-xs">
								{item.hasAgentReplied ? (
									...
									<span className="bg-amber-100 text-amber-800 border border-amber-300 dark:bg-amber-950 dark:text-amber-300 dark:border-amber-800 px-2.5 py-1 rounded font-mono">
										⏳ Ожидает ответа
									</span>
								)}
							</div>
