...
export type CtPlanningViewerRestoreWindowPreset = Extract<
	ImagingViewerWindowPreset,
...
export type CtPlanningViewerBridgeApplyTarget =
	| "volume"
	| "projection"
	| "window"
	| "slab";
...
export type CtPlanningViewerBridgeStatus =
	| "ready"
	| "metadata_only"
	| "blocked";
...
const windowPresetValues = new Set<CtPlanningViewerRestoreWindowPreset>([
	"bone",
	"soft_tissue",
	"implant",
	"custom",
...
function isCtPlanningViewerRestoreMode(
	value: string,
): value is CtPlanningViewerRestoreMode {
...
function isCtPlanningViewerRestoreWindowPreset(
	value: string,
): value is CtPlanningViewerRestoreWindowPreset {
...
export function buildCtPlanningViewerRestoreCommands(
	input: CtPlanningViewerRestoreInput,
...
export function serializeCtPlanningViewerRestoreCommands(
	commands: readonly CtPlanningViewerRestoreCommand[],
) {
	return commands.join("|");
}

function invalidRestoreCommandString(
	error: string,
): CtPlanningViewerRestoreParseResult {
	return {
		valid: false,
		commands: [],
		mode: null,
...
export function parseCtPlanningViewerRestoreCommandString(
	...
	if (tokens.length !== 4)
		return invalidRestoreCommandString(
			"restore command string must contain exactly 4 commands",
		);
	...
	if (!isCtPlanningViewerRestoreMode(modeToken))
		return invalidRestoreCommandString(
			"restore command mode must be load_volume or metadata_only",
		);
	if (!projectionToken.startsWith("projection:"))
		return invalidRestoreCommandString(
			"restore command projection token is missing",
		);
	if (!windowToken.startsWith("window:"))
		return invalidRestoreCommandString(
			"restore command window token is missing",
		);
	if (!slabToken.startsWith("slab:"))
		return invalidRestoreCommandString("restore command slab token is missing");

	const projection = projectionToken.slice("projection:".length);
	if (!isDicomMprProjection(projection))
		return invalidRestoreCommandString(
			"restore command projection is unsupported",
		);

	const windowPreset = windowToken.slice("window:".length);
	if (!isCtPlanningViewerRestoreWindowPreset(windowPreset))
		return invalidRestoreCommandString(
			"restore command window preset is unsupported",
		);

	const slabMm = Number(slabToken.slice("slab:".length));
	if (!Number.isFinite(slabMm) || slabMm <= 0)
		return invalidRestoreCommandString(
...
export function buildCtPlanningViewerBridgeApplyPlan(
	commandString: string,
): CtPlanningViewerBridgeApplyPlan {
	...
	const steps: CtPlanningViewerBridgeApplyStep[] = [
		{
			...
			command: parsed.mode,
			value: parsed.mode,
			required: parsed.mode === "load_volume",
		},
		{
			id: "projection",
			target: "projection",
...
export function buildCtPlanningViewerBridgeManifest(input: {
	...
	const commandString = serializeCtPlanningViewerRestoreCommands(
		input.commands,
	);
	const parsed = parseCtPlanningViewerRestoreCommandString(commandString);
