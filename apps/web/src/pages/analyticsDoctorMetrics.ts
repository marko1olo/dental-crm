...
/**
 * Денежная сумма для дашборда.
 *
 * Знак выносится вперёд, а сокращение считается по модулю: до этого
 * `formatRub(-5000)` давало «-5000 ₽», тогда как `formatRub(5000)` давало
 * «5K ₽» — одна и та же величина печаталась в двух разных форматах в
 * зависимости от знака. Для неотрицательных чисел поведение не изменилось.
 *
 * Округление до тысяч — свойство дашборда, а не денежного расчёта; точные
 * суммы к копейке живут в платежах и документах.
 */
