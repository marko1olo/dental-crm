...
export function FinancialDashboard({ metrics }: { metrics: FinancialMetrics }) {
  ...
  useEffect(() => {
    fetch('/api/billing/payouts', { headers: denteAdminSecretRequestHeaders() })
      ...
      .then(d => {
        setInvoices(Array.isArray(d.payouts) ? d.payouts.filter((i: any) => i.status !== 'paid') : []);
      })
      .catch(() => setInvoices([]));
  }, []);
  ...
  const handlePaymentSuccess = (splits: any[]) => {
    ...
    setReceiptBuffer({
      ...
      items: [{ name: 'Медицинские услуги', tooth: '', price: selectedInvoice?.totalAmountRub || 15000 }],
      splits,
      ...
      <div style={{ marginTop: '30px', marginBottom: '20px' }}>
        ...
        {invoices.length === 0 ? (
          ...
          <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
            {invoices.map(inv => (
              <div key={inv.id} style={{ background: 'var(--bg-card)', padding: '15px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <p><strong>Пациент:</strong> {inv.patientName || 'Неизвестно'}</p>
                <p><strong>Сумма:</strong> {inv.totalAmountRub} ₽</p>
                <button 
                  style={{ marginTop: '10px', padding: '8px 16px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
