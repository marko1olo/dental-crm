    ...
    <header className="topbar">
      ...
      {/*
        ...
        Отсюда два следствия, и оба проверяемы по CSS:
          — до 1140px «Настроить» и «Заблокировать» скрыты, поэтому в строке
            остаются только группа, «Прием» и «Запись»;
          — от 1141px вниз может уехать только то, что таблица стилей и так
            считает необязательным.

        ГДЕ ЭТА ГАРАНТИЯ ЗАКАНЧИВАЕТСЯ — СКАЗАНО ЧЕСТНО, ПОТОМУ ЧТО РАНЬШЕ ЗДЕСЬ
        СТОЯЛО ОБРАТНОЕ. Первая версия этого комментария утверждала, что «Запись»
        не может уехать вниз ни при какой ширине. Это неверно, и неверно по
        правилам flex: строка всегда кладёт на себя хотя бы один элемент, а
        группа помощника не сжимается (`flex: 0 0 auto`,
        `workspaceActions.css:24`). Значит остаётся ровно один случай, когда
        «Запись» окажется на второй строке: если группа ОДНА займёт всю доступную
        ширину. Он достижим на 841-1140px при длинном названии клиники, потому
        что `.topbar h1` несёт `white-space: nowrap`
        (`dente-redesign.css:386-389`) и его минимальная ширина равна полному
        названию, а не обрезанному.

        Что именно изменилось, без округления в свою пользу: до правки «Запись»
        уезжала вниз, как только на первой строке умещались любые пять элементов
        из шести, и на снимке 1440px это и произошло. Теперь для того же нужен
        край раскладки. Исправлен измеренный случай; невозможность НЕ доказана,
        остаток замерен арифметикой по CSS, а не в браузере, и записан долгом в
        `.agents/archon/packets/BB2-topbar-demotes-primary-action/handoff.md`.
      */}
      <div className="top-actions">
        ...
        */}
        ...
        {/*
          ...
          там в строке остаются только группа, «Прием» и «Запись» — то есть
          уехать вниз может лишь то, что и так необязательно. Слова
          «гарантированно однорядная» здесь стояли раньше и были преувеличением:
          точная граница разобрана в комментарии к началу строки действий.
