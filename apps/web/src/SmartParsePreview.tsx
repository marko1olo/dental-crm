...
export function SmartParsePreview({
	...
	const handleAiParse = async () => {
		...
		try {
			...
			setAiError(
				"Ошибка API (Локальный режим): Подключите ключи в .env для реального LLM-парсинга",
			);
		} finally {
			setIsAiLoading(false);
		}
	...
	const renderSchedulePreview = (data: any) => {
		if (data && data.isAiTask)
			return (
				<div className="space-y-2 text-sm">
					{data.isAiTask && (
						<div className="flex flex-col border-b border-zinc-200/30 pb-2 mb-2">
							...
							<span className="text-slate-800 dark:text-zinc-300 text-[11px] font-mono leading-tight bg-zinc-100/30 dark:bg-zinc-900/60 p-2 rounded border border-zinc-200/50 dark:border-zinc-700/50 break-words whitespace-pre-wrap max-h-[150px] overflow-y-auto">
								{data.prompt}
							</span>
						</div>
					)}
		...
		return (
			<div className="space-y-2 text-sm">
				...
				{data.patientId && (
					<div className="flex justify-between border-b border-zinc-200/30 pb-1">
			...
			<div className="space-y-2 text-sm">
				...
				{data.category && (
					<div className="flex justify-between border-b border-zinc-200/30 pb-1">
						...
						<span className="font-medium text-slate-800 dark:text-zinc-300 bg-zinc-100/50 dark:bg-zinc-800/40 px-2 py-0.5 rounded text-[10px]">
							{data.category}
						</span>
					</div>
				)}
	...
	const renderPatientPreview = (data: any) => {
		...
		if (!data || Object.keys(data).length === 0)
			return <div className="text-sm text-slate-500">Пусто...</div>;
		return (
			<div className="space-y-2 text-sm">
				{data.fullName && (
					<div className="flex justify-between border-b border-zinc-200/30 pb-1">
				...
				{data.emkUpdates &&
					Object.entries(data.emkUpdates).map(([k, v]) => {
						...
						return (
							<div
								key={k}
								className="flex flex-col border-t border-zinc-200/30 pt-1 mt-1"
							>
	...
	const renderContent = () => {
		...
		switch (type) {
			...
			default:
				return (
					<div className="text-sm text-slate-500">
						Неизвестный контекст диктовки: {type}
					</div>
				);
		}
	};
		...
		<AnimatePresence>
			{isVisible && (
				<motion.div
					...
					exit={{ opacity: 0, y: -10 }}
					className="absolute left-0 top-full mt-2 w-[400px] bg-zinc-50/40 dark:bg-zinc-950/40 backdrop-blur-md rounded-xl shadow-xl border border-zinc-200/50 dark:border-zinc-800/50 overflow-hidden z-50"
				>
					<div className="bg-zinc-100/30 dark:bg-zinc-900/40 border-b border-zinc-200/30 dark:border-zinc-800/50 p-3 flex justify-between items-center">
					...
					<div className="bg-zinc-100/30 dark:bg-zinc-900/40 p-3 border-t border-zinc-200/30 dark:border-zinc-800/50 flex gap-2">
						<button
						...
						<button
							onClick={onManual}
							disabled={isAiLoading}
							className="flex-1 bg-zinc-50/40 dark:bg-zinc-800/50 border border-zinc-200/50 dark:border-zinc-700/50 hover:bg-zinc-100/30 dark:hover:bg-zinc-700/50 disabled:opacity-50 text-slate-700 dark:text-zinc-300 font-medium text-sm py-2 px-3 rounded-lg transition-colors"
						>
							Вручную
						</button>
					</div>
