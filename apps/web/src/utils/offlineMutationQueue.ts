 ...
 *
 * Отказоустойчивая очередь локальных мутаций и черновиков в IndexedDB:
 * - Версионированная схема базы данных
 * - Точные метки времени (ISO 8601 + ms)
 * - Гарантированные UUID v4
 * - Сохранение любых изменений врача (043/у, одонтограмма, 107-1/у, документы, чеки)
 * - Прозрачный fallback на localStorage при недоступности IndexedDB
 */
...
export type MutationAction = "create" | "update" | "delete" | "sync";
...
export interface OfflineMutation<T = unknown> {
	...
	payload: T;
	timestamp: string; // ISO 8601 with milliseconds (e.g. 2026-08-22T22:45:00.123Z)
	timestampMs: number;
...
function getLocalStorageMutations(): OfflineMutation[] {
	...
	try {
		...
		logger.error("[OfflineQueue] Error reading localStorage mutations", err);
		return [];
	}
}
...
function saveLocalStorageMutations(mutations: OfflineMutation[]): void {
	...
	try {
		...
		logger.error("[OfflineQueue] Error saving localStorage mutations", err);
	}
}
...
function getLocalStorageDraft<T>(draftKey: string): OfflineDraft<T> | null {
	...
	try {
		...
		logger.error(`[OfflineQueue] Error reading localStorage draft ${draftKey}`, err);
		return null;
	}
}
...
function saveLocalStorageDraft<T>(draft: OfflineDraft<T>): void {
	...
	try {
		...
		logger.error(
			`[OfflineQueue] Error saving localStorage draft ${draft.draftKey}`,
			err,
		);
	}
}
...
function removeLocalStorageDraft(draftKey: string): void {
	...
	try {
		...
		logger.error(
			`[OfflineQueue] Error removing localStorage draft ${draftKey}`,
			err,
		);
	}
}
...
export async function enqueueOfflineMutation<T = unknown>(
	...
	try {
		...
		logger.warn(
			"[OfflineQueue] IndexedDB enqueue failed, using localStorage fallback",
			err,
		);
...
export async function getPendingOfflineMutations(filter?: {
	...
	try {
		...
		logger.warn(
			"[OfflineQueue] IndexedDB getPending failed, using localStorage fallback",
			err,
		);
		const list = getLocalStorageMutations();
		return list
...
export async function saveOfflineDraft<T = unknown>(
	...
	try {
		...
		// Also duplicate to localStorage for immediate resilience if needed
		saveLocalStorageDraft(draft);
		return draft;
	} catch (err) {
		logger.warn(
			`[OfflineQueue] IDB save draft failed for ${draftKey}, falling back to localStorage`,
			err,
		);
		saveLocalStorageDraft(draft);
		return draft;
...
export async function loadOfflineDraft<T = unknown>(
	...
	try {
		...
		logger.warn(
			`[OfflineQueue] IDB load draft failed for ${draftKey}, checking localStorage`,
			err,
		);
		return getLocalStorageDraft<T>(draftKey);
	}
...
export async function deleteOfflineDraft(draftKey: string): Promise<void> {
	...
	try {
		...
		logger.warn(`[OfflineQueue] Error deleting IDB draft ${draftKey}`, err);
	}
}
...
export async function listOfflineDrafts(filter?: {
	...
	try {
		...
		// Fallback to localStorage keys scan
		if (typeof window === "undefined" || !window.localStorage) return [];
