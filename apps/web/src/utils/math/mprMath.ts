...
export const finiteOrNull = (value: number) =>
	Number.isFinite(value) ? value : null;

export function distanceMm(
	a: DicomViewerToolStatePoint,
	b: DicomViewerToolStatePoint,
) {
...
export function resolveMprKeyboardAdjustment(input: {
	...
	switch (input.key) {
		...
		case "ArrowDown":
			return {
				kind: "slice",
				value: clampMprSliceIndex(input.sliceIndex - sliceStep, input.maxIndex),
			};
		case "ArrowUp":
			return {
				kind: "slice",
		...
		case "End":
			return {
				kind: "slice",
				value: clampMprSliceIndex(input.maxIndex, input.maxIndex),
			};
		default:
			return null;
	}
...
export function mprProjectionCompassLabels(
	...
	switch (projection) {
		case "axial":
			return {
				...
				bottom: "назад",
				left: "лево",
				summary: "аксиальная карта: перед/назад и право/лево",
			};
		case "coronal":
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "лево",
				summary: "корональная карта: верх/низ и стороны",
			};
		case "sagittal":
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "назад",
				summary: "сагиттальная карта: верх/низ и перед/назад",
			};
		case "oblique":
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "-ось",
				summary: "косая карта: стороны показывают направление угла",
			};
		case "panoramic_reconstruction":
			return {
				...
				bottom: "низ",
				left: "от дуги",
				summary: "панорамная карта: движение вдоль зубной дуги",
			};
		case "three_d_volume":
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "поворот",
				summary: "объемная карта: ориентация зависит от поворота модели",
			};
		case "mip":
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "проекция",
				summary: "карта плотности: показывает самые плотные структуры",
			};
		default:
			return {
				top: "верх",
				...
				bottom: "низ",
				left: "лево",
				summary: "карта плоскости просмотра",
			};
	}
}
...
export function clampMprSliceIndex(value: number, maxIndex: number): number {
	const safeMaxIndex = Math.max(
		0,
		Math.round(Number.isFinite(maxIndex) ? maxIndex : 0),
	);
	return clampRounded(value, 0, safeMaxIndex, 0);
}

export function formatMprAxisAngleBadge(
	axisDeg: number,
	canOpenMpr = true,
): string {
...
export function formatMprAxisDirectionLabel(input: {
	canOpenMpr: boolean;
	axisDeg: number;
}): string {
	...
	return safeAxisDeg > 0
		? `ось +${safeAxisDeg}° вправо`
		: `ось ${safeAxisDeg}° влево`;
}
...
export function formatMprSliceBadge(input: {
	canOpenMpr: boolean;
	sliceIndex: number;
	maxIndex: number;
}): string {
	...
	const safeMaxIndex = Math.max(
		0,
		Math.round(Number.isFinite(input.maxIndex) ? input.maxIndex : 0),
	);
	return `срез ${clampMprSliceIndex(input.sliceIndex, safeMaxIndex) + 1}/${safeMaxIndex + 1}`;
}

export function formatMprAxisRangeValue(input: {
	canOpenMpr: boolean;
	axisDeg: number;
}): string {
	if (!input.canOpenMpr)
		return "Ось включится после выбора готовой КЛКТ/КТ-серии.";
	return `${formatMprAxisDirectionLabel(input)}, диапазон ${mprAxisBounds.min}°...+${mprAxisBounds.max}°.`;
}

export function formatMprSlabRangeValue(input: {
	canOpenMpr: boolean;
	slabMm: number;
}): string {
	if (!input.canOpenMpr)
		return "Толщина слоя включится после выбора готовой КЛКТ/КТ-серии.";
	const safeSlabMm = clampMprSlabMm(input.slabMm);
	return `Толщина слоя ${safeSlabMm} мм, ${formatMprSlabBandLabel(safeSlabMm)}.`;
}

export function formatMprSliceRangeValue(input: {
	canOpenMpr: boolean;
	sliceIndex: number;
	maxIndex: number;
}): string {
	if (!input.canOpenMpr)
		return "Положение среза включится после выбора готовой КЛКТ/КТ-серии.";
	const safeMaxIndex = Math.max(
		0,
		Math.round(Number.isFinite(input.maxIndex) ? input.maxIndex : 0),
	);
...
export function mprSliceIndexFromFraction(
	fraction: number,
	maxIndex: number,
): number {
	const safeFraction = Number.isFinite(fraction) ? fraction : 0.5;
	return clampMprSliceIndex(
		Math.round(Math.max(0, Math.min(1, safeFraction)) * Math.max(0, maxIndex)),
		maxIndex,
	);
}
...
export function mprSliceFraction(sliceIndex: number, maxIndex: number): number {
	...
	const fraction = Math.max(
		0,
		Math.min(1, clampMprSliceIndex(sliceIndex, safeMaxIndex) / safeMaxIndex),
	);
	const snapTolerance = 0.5 / safeMaxIndex + Number.EPSILON;
	const nearestPreset = mprSlicePresetFractions.find(
		(preset) => Math.abs(fraction - preset.fraction) <= snapTolerance,
	);
...
export function buildMprAxisGuidance(input: {
	canOpenMpr: boolean;
	axisDeg: number;
	slabMm: number;
