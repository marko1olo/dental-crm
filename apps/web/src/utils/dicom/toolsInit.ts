...
export function setupMprToolGroup(renderingEngineId: string, viewportIds: string[]) {
  ...
  toolGroup.setToolActive(cornerstoneTools.ZoomTool.toolName, {
    bindings: [{ mouseButton: cornerstoneTools.Enums.MouseBindings.Primary, modifierKey: cornerstoneTools.Enums.KeyboardBindings.Shift }],
  });
...
export function setupVrToolGroup(renderingEngineId: string, vrViewportId: string) {
  const vrToolGroupId = "vr-tool-group";
  cornerstoneTools.ToolGroupManager.destroyToolGroup(vrToolGroupId);
  const vrToolGroup = cornerstoneTools.ToolGroupManager.createToolGroup(vrToolGroupId)!;
