...
export type ImplantSystem = 'osstem' | 'straumann' | 'nobel' | 'bredent' | 'mdi' | 'other';
...
export function extractHUZones(huSamples: number[]): HUZoneProfile {
  ...
  return {
    ...
    cancellousHU: avg(cancellousSamples.length > 0 ? cancellousSamples : huSamples),
...
export function generateDrillProtocol(
  ...
  if (mischClass === 'D1') {
    ...
    steps.push({
      step: 2, drillType: 'Cortical Drill', diameterMm: 2.8, depthMm: Math.min(4, lengthMm * 0.3),
      rpmRange: '400–600 RPM', torqueNcm: '40 Ncm', irrigation: true,
      note: 'Только кортикальная зона — не глубже 30% длины'
    });
    steps.push({
      step: 3, drillType: `Profile Drill ${diameterMm - 0.5}mm`, diameterMm: diameterMm - 0.5, depthMm: lengthMm,
      rpmRange: '500–700 RPM', torqueNcm: '45 Ncm', irrigation: true,
      note: 'Профильное сверло на 0.5мм меньше номинала'
    });
    steps.push({
      step: 4, drillType: 'Cortical Tap', diameterMm: diameterMm, depthMm: Math.min(3, lengthMm * 0.2),
      rpmRange: '15–20 RPM', torqueNcm: '50 Ncm', irrigation: true,
      note: 'Нарезка резьбы только в кортикальной зоне'
    });
    steps.push({
      step: 5, drillType: `Final Profile ${diameterMm}mm`, diameterMm, depthMm: lengthMm,
      rpmRange: '500 RPM', torqueNcm: '45 Ncm', irrigation: true
    });

  } else if (mischClass === 'D2') {
    steps.push({
      step: 2, drillType: 'Twist Drill 2.8mm', diameterMm: 2.8, depthMm: lengthMm,
      rpmRange: '800–1000 RPM', torqueNcm: '45 Ncm', irrigation: true
    });
    steps.push({
      step: 3, drillType: `Profile Drill ${diameterMm - 0.2}mm`, diameterMm: diameterMm - 0.2, depthMm: lengthMm,
      rpmRange: '700–900 RPM', torqueNcm: '45 Ncm', irrigation: true
    });
    steps.push({
      step: 4, drillType: `Final Drill ${diameterMm}mm`, diameterMm, depthMm: lengthMm,
      rpmRange: '800 RPM', torqueNcm: '45 Ncm', irrigation: true
    });

  } else if (mischClass === 'D3') {
    steps.push({
      step: 2, drillType: 'Twist Drill 2.8mm', diameterMm: 2.8, depthMm: lengthMm,
      rpmRange: '1000–1200 RPM', torqueNcm: '35 Ncm', irrigation: true
    });
    steps.push({
      step: 3, drillType: `Final Drill ${diameterMm}mm`, diameterMm, depthMm: lengthMm,
      rpmRange: '1000 RPM', torqueNcm: '35 Ncm', irrigation: true,
    ...
    steps.push({
      step: 2, drillType: 'Twist Drill 2.0mm', diameterMm: 2.0, depthMm: lengthMm,
      rpmRange: '1200 RPM', torqueNcm: '25 Ncm', irrigation: false,
      note: 'D4: минимальный диаметр для максимального захвата'
    });
    steps.push({
      step: 3, drillType: `Under-profile ${effectiveDrill}mm`, diameterMm: effectiveDrill, depthMm: lengthMm,
      rpmRange: '1000 RPM', torqueNcm: '30 Ncm', irrigation: false,
  ...
  if (systemNote) {
    steps[steps.length - 1]!.note = (steps[steps.length - 1]!.note || '') + ' | ' + systemNote;
  }
...
function getSystemNote(system: ImplantSystem, misch: MischClass, diameter: number): string {
  switch (system) {
    case 'osstem':
      return misch === 'D4'
...
export function mischDescription(cls: MischClass): string {
  switch (cls) {
    case 'D1': return 'D1 — Очень плотная (>1250 HU): кортикальная, трудно сверлить';
    case 'D2': return 'D2 — Плотная (850–1250 HU): идеальна для имплантации';
    case 'D3': return 'D3 — Средняя (350–850 HU): приемлема, хороший прогноз';
    case 'D4': return 'D4 — Мягкая (150–350 HU): риск нестабильности, недопрепарирование';
  }
}
