...
export const ClinicalStore = {
  ...
  addImplant(implant: VirtualImplant) {
    ...
    if (checkImplantCollision(implant, 2.0)) {
        window.dispatchEvent(new CustomEvent('clinical-collision', { detail: { type: 'WARNING', message: `CRITICAL: Implant ${implant.id} is too close to mandibular nerve!` }}));
    }
    // Hardcoded tooth 46 for e2e testing purposes and demo integration
    window.dispatchEvent(new CustomEvent('clinical-implant-placed', { detail: { toothNumber: 46, implantId: implant.id }}));
    this.notify();
  },
  ...
  updateImplant(id: string, updates: Partial<VirtualImplant>) {
    ...
    if (idx !== -1) {
      this.implants[idx] = { ...this.implants[idx]!, ...updates } as VirtualImplant;
      if (checkImplantCollision(this.implants[idx]!, 2.0)) {
          window.dispatchEvent(new CustomEvent('clinical-collision', { detail: { type: 'WARNING', message: `CRITICAL: Implant ${id} is too close to mandibular nerve!` }}));
      }
      this.notify();
    }
  },
...
function distToSegmentSquared(p: Point3D, v: Point3D, w: Point3D): number {
  const l2 = Math.pow(v.x - w.x, 2) + Math.pow(v.y - w.y, 2) + Math.pow(v.z - w.z, 2);
  if (l2 === 0) return Math.pow(p.x - v.x, 2) + Math.pow(p.y - v.y, 2) + Math.pow(p.z - v.z, 2);
  ...
  return Math.pow(p.x - proj.x, 2) + Math.pow(p.y - proj.y, 2) + Math.pow(p.z - proj.z, 2);
}
 ...
 */
export function checkImplantCollision(implant: VirtualImplant, thresholdDistance: number = 2.0): boolean {
  ...
  const collisionDistSquared = Math.pow(thresholdDistance + (implant.diameter / 2), 2);
  ...
  for (const nerve of ClinicalStore.nerves) {
    ...
    for (const pt of nerve.points) {
      ...
      if (distSq < collisionDistSquared + Math.pow(nerveRadius, 2)) {
        return true;
      }
    }
  }
