...
export const unifiedPdfGenerator = {
  ...
  async generateSurgicalReport(data: SurgicalReportData): Promise<void> {
    ...
    if (data.aiSafetyVerdict === "SAFE") {
      ...
      doc.text("AI Статус: ПРЕДУПРЕЖДЕНИЕ (Возможное коаксиальное расхождение)", 20, currentY);
    } else {
      doc.setTextColor(220, 38, 38); // Red
      doc.text("AI Статус: ОПАСНОСТЬ (Риск повреждения нижнечелюстного нерва)", 20, currentY);
    }
  ...
  async generateFinancialEstimate(data: FinancialEstimateData): Promise<void> {
    ...
    if (odontogramEl) {
      try {
        const canvas = await html2canvas(odontogramEl, { scale: 2, useCORS: true, logging: false });
        const imgData = canvas.toDataURL("image/jpeg", 0.9);
    ...
    data.phases.forEach((phase) => {
      ...
      phase.items.forEach((item) => {
        ...
        doc.text(`${item.price.toLocaleString("ru-RU")} ₽`, pageWidth - 40, currentY, { align: "right" });
        currentY += 6;
      });
      ...
      doc.text(`${phase.total.toLocaleString("ru-RU")} ₽`, pageWidth - 40, currentY, { align: "right" });
      currentY += 12;
    });
    ...
    doc.text(`${data.totalPrice.toLocaleString("ru-RU")} ₽`, pageWidth - 40, currentY, { align: "right" });
    ...
    if (data.discount > 0) {
      ...
      doc.text(`-${data.discount.toLocaleString("ru-RU")} ₽`, pageWidth - 40, currentY, { align: "right" });
    }
    ...
    doc.text(`${data.finalPrice.toLocaleString("ru-RU")} ₽`, pageWidth - 40, currentY, { align: "right" });
