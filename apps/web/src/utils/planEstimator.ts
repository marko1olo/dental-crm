...
const SYSTEM_COSTS: Record<ImplantSystem, { implantCost: number; label: string }> = {
...
const CROWN_COSTS: Record<PlanTier, { lab: number; fee: number; label: string }> = {
  standard: { lab: 8_000,  fee: 18_000, label: 'Коронка циркониевая (стандарт)' },
  premium:  { lab: 14_000, fee: 28_000, label: 'Коронка монолитная HIGH TECH (премиум)' },
};

const ABUTMENT_COSTS: Record<PlanTier, { mat: number; fee: number; label: string }> = {
  standard: { mat: 2_500,  fee: 5_000,  label: 'Абатмент титановый стандартный' },
  premium:  { mat: 6_000,  fee: 10_000, label: 'Абатмент индивидуальный CAD/CAM' },
};
...
function buildPlan(
  ...
  const totalRevenue = services.reduce((s, l) => s + l.unitPriceFee * l.quantity, 0);
  const totalMaterialCost = services.reduce((s, l) => s + l.materialCost * l.quantity, 0);
