...
const local3DTargetLabels: Record<
	DentalModelWorkbenchManifest["recommendedTarget"],
	string
> = {
...
const local3DBridgeStatusLabels: Record<
	LocalBridgeReadinessResponse["bridges"][number]["status"],
	string
> = {
...
const ctSurfaceRoles: DentalModelFileRole[] = [
	"skull_surface",
	"maxilla_surface",
	"mandible_surface",
	"ct_bone_surface",
...
function itemHasRole(
	item: DentalModelWorkbenchItem,
	roles: DentalModelFileRole[],
) {
	return (
		roles.includes(item.role) ||
		Boolean(
			item.ctSurfaceManifest && roles.includes(item.ctSurfaceManifest.role),
		)
	);
}

function itemsForRoles(
...
function localBridgeReadiness(input: LocalBridgeReadinessResponse | null) {
	const bridge =
		input?.bridges.find((item) => item.kind === "dicom_cbct") ?? null;
	return {
		bridge,
		ready: Boolean(bridge && bridge.status === "ready" && bridge.reachable),
		label: bridge
			? local3DBridgeStatusLabels[bridge.status]
			: "локальный модуль не проверен",
	};
}
...
function roleCard(input: {
	...
	const requiresLocalBridge =
		items.some(itemNeedsLocalBridge) ||
		input.manifest.recommendedTarget === "local_bridge";
	const hasBlockedItem = items.some(itemIsBlocked);
	const metadataCount = items.length;
	const formats = Array.from(
		new Set(items.map((item) => item.format.toUpperCase())),
	)
		.slice(0, 3)
		.join(", ");
	const status: CtPlanningImplantModelStatus =
		metadataCount === 0 || hasBlockedItem
			...
			: requiresLocalBridge
				...
				: items.some(
							(item) =>
								item.ctSurfaceManifest?.readiness === "metadata_only" ||
								item.loadTarget === "metadata_only",
						)
					? "draft"
					: "ready";
	const targetLabel = requiresLocalBridge
		? input.bridgeLabel
		: (local3DTargetLabels[input.manifest.recommendedTarget] ??
			input.manifest.recommendedTarget);
	const detail =
		metadataCount === 0
...
export function buildCtPlanningLocal3DReadinessPlan(input: {
	...
	const cards = [
		roleCard({
			id: "ct-surface",
			title: "КТ-поверхность",
			manifest,
			roles: ctSurfaceRoles,
	...
	const warnings = Array.from(
		new Set([
			...
			...manifest.items.flatMap(
				(item) => item.ctSurfaceManifest?.warnings ?? [],
			),
		]),
	).slice(0, 4);
	...
	return {
		...
		recommendedTargetLabel:
			local3DTargetLabels[manifest.recommendedTarget] ??
			manifest.recommendedTarget,
		cards,
		warnings,
...
function implantAxisAnnotation(annotations: DicomViewerToolStateAnnotation[]) {
	...
	for (const annotation of annotations) {
		if (annotation.type !== "implant_axis" || annotation.points.length < 2)
			continue;
...
function surgicalGuideAnnotation(
	annotations: DicomViewerToolStateAnnotation[],
) {
	let selected: DicomViewerToolStateAnnotation | null = null;
	let selectedPointCount = -1;
	for (const annotation of annotations) {
		if (annotation.type !== "surgical_guide" || annotation.points.length < 2)
			continue;
			...
			: renderPlan.gpuClass === "discrete_ok"
				...
				: renderPlan.gpuClass === "integrated_ok"
					...
					: renderPlan.gpuClass === "integrated_low"
						...
						: 0.2;
	const budget =
		renderPlan.interactionBudgetMs <= 16
			? 0.9
			: renderPlan.interactionBudgetMs <= 32
				? 0.55
				: 0.3;
	return round2(
		clamp(
			quality * 0.34 + gpu * 0.24 + budget * 0.14 + hardware * 0.28,
			0.15,
			1,
		),
	);
...
function apexLabel(
	axis: DicomViewerToolStateAnnotation | null,
...
function statusForImplant(
	hasImplant: boolean,
	hasAxis: boolean,
): CtPlanningImplantModelStatus {
...
export function buildCtPlanningImplantModelPlan(input: {
	...
	const guideRouteLengthMm = guideRoute
		? polylineLengthMm(guideRoute.points)
		: null;
	const implantLengthMm = input.implantPlan?.lengthMm ?? null;
	const implantDiameterMm = input.implantPlan?.diameterMm ?? null;
	const safetyEnvelopeDiameterMm =
		implantDiameterMm === null ? null : round1(implantDiameterMm + 4);
	const sleeveDiameterMm =
		implantDiameterMm === null ? null : round1(implantDiameterMm + 1.4);
	const sleeveLengthMm =
		implantLengthMm === null
			? null
			: round1(clamp(implantLengthMm * 0.45, 5, 9));
	...
	const cards: CtPlanningImplantModelCard[] = [
		{
			...
			value: input.implantPlan
				? `${input.implantPlan.diameterMm} x ${input.implantPlan.lengthMm} мм`
				: "нет размера",
			detail: input.implantPlan
				? `${input.implantPlan.line}, ${input.implantPlan.platform}`
				: "Типоразмер нужен до оси и шаблона.",
			nextAction: hasImplant
				? "Сверить диаметр с шириной гребня."
				: "Выбрать имплант из библиотеки.",
		},
		{
			id: "implant-axis",
			title: "Ось и апекс",
			...
			nextAction: hasAxis
				? "Проверить апекс на поперечных срезах."
				: "Поставить 2 точки оси импланта.",
		},
		{
			id: "safety-envelope",
			...
			value:
				safetyEnvelopeDiameterMm === null
					? "нет"
					: `${safetyEnvelopeDiameterMm} мм`,
			detail:
				"Диаметр импланта + 2 мм с каждой стороны для визуального контроля.",
			nextAction:
				clearance === null
					? "Добавить канал для расчета отступа."
					: clearance >= 2
						? "Отступ до канала проходит."
						: "Сместить ось или изменить размер.",
		},
		{
			id: "guide-sleeve",
			title: "Втулка шаблона",
			status:
				sleeveDiameterMm !== null && hasAxis
					? "ready"
					: sleeveDiameterMm !== null
						? "draft"
						: "blocked",
			value:
				sleeveDiameterMm === null || sleeveLengthMm === null
					? "нет"
					: `${sleeveDiameterMm} x ${sleeveLengthMm} мм`,
			detail: `${modelingLabel(modelingWeight)}: качество меняет только отображение, не размеры втулки.`,
			nextAction:
				sleeveDiameterMm !== null && hasAxis
					? "Сохранить маршрут шаблона в пакет."
					: "Нужны типоразмер и ось.",
		},
		{
			id: "guide-route",
			...
			status: guideReady
				? "ready"
				: hasGuideRoute || (hasImplant && hasAxis)
					? "draft"
					: "blocked",
			value:
				guideRouteLengthMm === null
					? `${guideRoutePointCount}/2`
					: `${guideRouteLengthMm} мм`,
			detail: "Маршрут передается как кривая; STL/CAD делает лаборатория.",
			nextAction: guideReady
				? "Передать лаборатории после сохранения пакета."
				: "Нужны имплант, ось, маршрут и отступ >= 2 мм.",
		},
	];
	const warnings: string[] = [];
	if (!hasGuideRoute) warnings.push("Маршрут шаблона не размечен.");
	if (hasGuideRoute && clearance === null)
		warnings.push("Для шаблона нужен отступ до канала.");
	if (!hasImplant) warnings.push("Имплант не выбран.");
	if (!hasAxis) warnings.push("Ось импланта не размечена.");
	if (
		axisLengthMm !== null &&
		implantLengthMm !== null &&
		axisLengthMm < implantLengthMm * 0.75
	)
		warnings.push("Размеченная ось короче выбранного импланта.");
