...
export function DocumentsView(props: DocumentsViewProps) {
	...
	const typedTaxApplicationFormOptions = taxApplicationFormOptions as Array<
		DocumentSelectOption<TaxDeductionApplicationForm>
	>;
								...
								<div className="document-payload-collapsed-content">
									...
									>
										...
										{typedEligiblePaymentReceiptPayments.length ? (
											...
											<span className="tax-payment-selection-empty">
												Нет оплаченных платежей по текущему визиту. Сначала
												сохраните оплату с фискальным чеком и данными
												плательщика.
											</span>
										)}
									</section>
								...
								<div className="document-payload-collapsed-content">
									...
									<label>
										...
										<textarea
											...
											onChange={(event) =>
												setIntakeAllergyStatus(event.target.value)
											}
											rows={2}
										/>
									</label>
									...
									<label>
										...
										<textarea
											...
											onChange={(event) =>
												setIntakeAnticoagulants(event.target.value)
											}
											rows={2}
										/>
									</label>
					...
					{selectedDocumentKind === "tax_deduction_application" ? (
						<article className="document-payload-card">
							<div>
								...
								<p>
									Заявитель, ИНН, документ, родство, год и способ выдачи без
								...
								<div className="document-payload-collapsed-content">
									...
									<div className="document-payload-row">
										<label>
											...
											>
												{typedTaxApplicationRelationshipOptions.map(
													(option) => (
														<option key={option.value} value={option.value}>
															{option.label}
														</option>
									...
									<div className="document-payload-row">
										<label>
											...
											>
												{typedTaxApplicationDeliveryChannelOptions.map(
													(option) => (
														<option key={option.value} value={option.value}>
															{option.label}
														</option>
									...
									<label>
										...
										<input
											...
											onChange={(event) =>
												setTaxApplicationContact(event.target.value)
											}
										/>
									</label>
									<label>
					...
					{selectedDocumentKind === "informed_consent" ? (
						<article className="document-payload-card">
							<div>
								...
								<p>
									Конкретное вмешательство, область, показание, риски,
								...
								<div className="document-payload-collapsed-content">
									...
									<label>
										...
										<textarea
											...
											onChange={(event) =>
												setInformedConsentRisks(event.target.value)
											}
											rows={4}
										/>
									</label>
								...
								<div className="document-payload-collapsed-content">
									<div className="document-payload-row">
										{typedPhotoVideoMaterialOptions.map((option) => (
											<label
												className="document-payload-checkbox"
												key={option.value}
											>
												<input
													...
													type="checkbox"
													onChange={() =>
														togglePhotoVideoMaterial(option.value)
													}
												/>
												{option.label}
											</label>
									...
									<label className="document-payload-checkbox">
										...
										/>
										Внешнее использование только после обезличивания, кроме
										отдельно разрешенной узнаваемой публикации
									</label>
									<label className="document-payload-checkbox">
										<input
									...
									<label>
										...
										<textarea
											...
											onChange={(event) =>
												setPhotoVideoScopeNotes(event.target.value)
											}
											rows={2}
										/>
									</label>
					...
					{selectedDocumentKind === "medical_intervention_refusal" ? (
						<article className="document-payload-card">
							<div>
								...
								<p>
									Что предложено, почему нужно, какие риски объяснены и когда
									срочно обращаться.
								</p>
							</div>
							<details className="document-manual-override">
								...
								<div className="document-payload-collapsed-content">
									<label>
										...
										<input
											...
											onChange={(event) =>
												setRefusalIntervention(event.target.value)
											}
											placeholder={
												inferredTreatmentArea
									...
									<div className="document-payload-row">
										...
										<label>
											...
											<input
												...
												onChange={(event) =>
													setRefusalConfirmedAt(event.target.value)
												}
											/>
										</label>
									</div>
