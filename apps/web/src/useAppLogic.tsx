...
export function useAppLogic(): any {
	...
	const initialDocumentIssueSignatureDraftRef =
		useRef<DocumentIssueSignatureDraft | null>(null);
	const documentIssueSignatureHydratedOrganizationIdRef = useRef<string | null>(
		null,
	);
	...
	useEffect(() => {
		...
		if (
			savedPreferences.savedAt &&
			scopedDraft.savedAt < savedPreferences.savedAt
		)
			return;
	...
	const activeCommunicationTasks = useMemo(() => {
		if (!dashboard) return [];
		return (dashboard.communicationTasks || []).filter(
			(task) => task.patientId === dashboard?.activeVisit?.patientId,
		);
	}, [dashboard]);
		...
		return activeImagingStudies
			...
			.sort(
				(left, right) =>
					right.score - left.score ||
					imagingCaptureDistanceMs(
						selectedImagingStudy.capturedAt,
						left.study.capturedAt,
					) -
						imagingCaptureDistanceMs(
							selectedImagingStudy.capturedAt,
	...
	const dicomWorkbenchSourceIsRedacted =
		dicomWorkbenchManifestHasRedactedSource(dicomViewerWorkbenchManifest);
	...
	function applyImagingViewerSessionState(
		sessionState: ImagingViewerSessionState,
	...
	async function loadImagingViewerSessionForStudy(studyId: string) {
		...
		const localDraft = loadLocalImagingViewerDraft(
			studyId,
			activeOrganizationId,
		);
		if (localDraft) {
		...
		try {
			...
			if (!response.ok)
				throw new Error(
					await responseErrorMessage(
						response,
						"Сессия просмотрщика не загружена",
					),
				);
			const payload = (await response.json()) as ImagingViewerSessionResponse;
			setImagingViewerSession(payload.session);
			const localIsNewer =
				localDraft?.clientSavedAt &&
				new Date(localDraft.clientSavedAt).getTime() >
					new Date(payload.session.updatedAt).getTime();
			if (!localIsNewer) {
				applyImagingViewerSessionState(
					payload.session.state,
					payload.session.annotations,
				);
				const localSaved = saveLocalImagingViewerDraft(
					studyId,
					{
						...
						clientSavedAt:
							payload.session.clientSavedAt ?? payload.session.updatedAt,
			...
			setImagingViewerSaveError(
				localDraft
					? "Сервер недоступен; локальный черновик просмотрщика сохранен."
					: "Сессия просмотрщика недоступна.",
			);
	...
	async function saveCurrentImagingViewerSession(clientSavedAt: string) {
		...
		if (!isOnline) {
			setImagingViewerSaveError(
				"Офлайн: локальный черновик просмотрщика сохранен до появления сети на рабочей станции.",
			);
		...
		try {
			...
			if (!response.ok)
				throw new Error(
					await responseErrorMessage(
						response,
						"Сессия просмотрщика не сохранена",
					),
				);
			const payload = (await response.json()) as ImagingViewerSessionResponse;
			setImagingViewerSession(payload.session);
			...
			if (localSaved)
				setImagingViewerLocalSavedAt(
					payload.session.clientSavedAt ?? clientSavedAt,
				);
			setImagingViewerSaveState("saved");
		} catch {
			setImagingViewerSaveError(
				"Серверное сохранение не выполнено; локальный черновик просмотра сохранен.",
			);
			setImagingViewerSaveState("queued");
		}
	}
	...
	useEffect(() => {
		...
		if (localSaved) {
			...
			setImagingViewerSaveError(
				"Браузер отклонил локальное сохранение черновика просмотрщика; держите вкладку открытой до завершения серверной записи.",
			);
			setImagingViewerSaveState("error");
		}
		if (imagingViewerSaveTimerRef.current)
			window.clearTimeout(imagingViewerSaveTimerRef.current);
	...
	const dictationQuickPhrases = useMemo(() => {
		...
		const specialtyPhrases =
			specialtyQuickPhraseLibrary[selectedSpecialty] ??
			specialtyQuickPhraseLibrary.universal;
		return [
	...
	const taxDocuments =
		(dashboard?.documents || []).filter(
			(document) => documentKindMetadata[document.kind].group === "tax",
		) ?? [];
	...
	const visitWorkflowSteps: Array<{
		...
		},
		{
			key: "emk",
			label: "ЭМК",
			detail: visitHasSavedNote
				? "запись сохранена"
				: visitNoteReadyToAccept
					? "осталось подтвердить"
					: "ждет черновик",
			state: visitHasSavedNote
				? "ready"
				: visitNoteReadyToAccept
					? "active"
					: "locked",
		},
		{
			key: "close",
			label: "Закрытие",
			detail: visitCloseChecklist?.readyToSign
				? "готово"
				: (primaryVisitWarning?.title ?? "проверка в конце"),
			state: visitCloseChecklist?.readyToSign
				? "ready"
				: visitHasSavedNote
					? "active"
					: "locked",
				...
				: primaryVisitWarning
					...
					: {
							...
							detail:
								...
								"Финальная проверка оплаты, документов и подписи приема.",
							onClick: () => scrollToVisitArea(".close-checklist"),
						};
	const speechRecoveryIssueCount =
		(speechRecordingRecovery?.recordings || []).filter(
			(recording) => recording.recoveryState !== "complete",
		).length ?? 0;
	...
	const currentSpeechQualityIssue =
		speechLastQuality && speechLastQuality.level !== "clear"
			? speechLastQuality
			: null;
	...
	const browserContinuityCritical =
		...
		(!browserContinuity.localStorageWritable ||
	...
	const browserContinuityState = !browserContinuity
		? "busy"
		: browserContinuityCritical
			? "warn"
			: "ready";
	...
	const browserContinuityChecks = [
		{
			...
			detail: lastLocalSavedAt
				? `последнее ${formatTime(lastLocalSavedAt)}`
		...
		},
		{
			...
			detail: pendingSpeechChunkCount
				? `фрагментов в очереди: ${pendingSpeechChunkCount}`
				: "аудио сохранится для отправки позже",
		},
		{
			label: "Выбор локальной КТ",
			value: browserContinuity?.directoryPickerSupported
				? "папка"
		...
		{
			label: "OPFS браузера",
			value: browserContinuity?.opfsSupported
				? "доступно"
				: browserContinuity
					? "нет"
					: "проверка",
		...
		},
		{
			...
			detail: browserContinuity?.serviceWorkerControlled
				...
				: "обновите вкладку после включения офлайн-режима",
		},
		{
			...
			detail:
				browserContinuity?.storagePersisted === true
					? "браузер не должен очищать черновики сам"
		...
		},
		{
			...
			detail:
				browserContinuity?.storageQuotaMb != null
					? `лимит ${formatMegabytes(browserContinuity.storageQuotaMb)}`
					: "оценка недоступна",
		},
		{
			label: "Синхронизация",
			...
			detail: pendingVisitSaveCount
				? `в очереди сохранений приема: ${pendingVisitSaveCount}`
				: "серверная очередь пуста",
		},
	];
	...
	const visitSafetyCards: Array<{
				...
				serverDraftSyncState === "saving"
					...
					: serverDraftSyncState === "saved" && lastServerDraftSavedAt
						...
						: serverDraftSyncState === "queued" ||
							...
							: "готов",
			detail: pendingVisitSaveCount
				? `${pendingVisitSaveCount} сохранение ожидает синхронизацию`
				: "серверный черновик включен",
			state:
				serverDraftSyncState === "saving"
					? "busy"
					: pendingVisitSaveCount ||
		...
		},
		{
			...
			value: speechRecoveryIssueCount
				? "проверить"
				: speechRecordingRecovery
					? "чисто"
					: "скоро",
			...
			state: speechRecoveryIssueCount
				? "warn"
				: speechRecordingRecovery
					? "ready"
					: "busy",
	...
	async function analyzePricelist() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(response, "Прайс не разобран"),
				);
			}
			setPricelistAnalysis(
	...
	async function ingestImportFile(file: File | undefined) {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(response, "Файл не разобран"),
				);
			}
	...
	async function previewSmartImportText(
		...
		try {
			...
			setSmartImportPreview(
				(await response.json()) as SmartImportPreviewResponse,
			);
			setSmartImportCommit(null);
		} catch (importError) {
			setError(
	...
	async function commitSmartImport() {
		...
		if (!smartImportText.trim()) {
			setError(
				"Вставьте выгрузку из старой МИС, таблицу, OCR или диктовку перед записью.",
			);
			return;
		}
		...
		if (
			smartImportPreview.patientPreview.readyRows === 0 &&
	...
	async function downloadSmartImportReport() {
		if (!smartImportText.trim()) {
			setError(
				"Вставьте выгрузку из старой МИС, таблицу, OCR или диктовку перед отчетом проверки.",
			);
			return;
		}
	...
	async function downloadSmartImportSafeHandoffReport() {
		if (!smartImportText.trim()) {
			setError(
				"Вставьте выгрузку из старой МИС, таблицу, OCR или диктовку перед отчетом переноса.",
			);
			return;
		}
	...
	function migrationAutopilotRequestPayload(
		...
		const hasClinicPayload = Object.values(clinicPayload).some(
			(value) => typeof value === "string" && value.trim(),
		);
		const knownSources = knownDiscovery?.candidates.slice(0, 18);
		const includeSmartImportText =
			options.includeSmartImportText ??
			(!knownDiscovery && Boolean(smartImportText.trim()));
		return {
			maxDepth: 5,
			maxFolders: 1600,
			...
			smartImport:
				includeSmartImportText && smartImportText.trim()
					? {
	...
	async function downloadMigrationHandoffReport() {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(response, "План миграции не создан"),
				);
			}
			const blob = await response.blob();
			const url = URL.createObjectURL(blob);
			...
			setError(
				operatorWorkflowFailureMessage("План миграции не создан", reportError),
			);
		} finally {
	...
	async function discoverMigrationSources() {
		...
		try {
			...
			const result =
				(await response.json()) as MigrationLocalSourceDiscoveryResponse;
	...
	function addMigrationDiscoveryCandidateToSmartImport(
		candidate: MigrationLocalSourceDiscoveryResponse["candidates"][number],
	) {
		setSmartImportMode("auto");
		setSmartImportText((current) =>
			[current.trim(), candidate.smartImportLine].filter(Boolean).join("\n"),
		);
		setSmartImportPreview(null);
		setSmartImportCommit(null);
	}

	function migrationCandidateCanPreview(
		candidate: MigrationLocalSourceDiscoveryResponse["candidates"][number],
	) {
		const materialCount =
	...
	async function previewMigrationAutopilotSources(
		...
		if (!sourceLines.length) {
			setError(
				"Автоплан пока не дал строк для предпросмотра. Сначала запустите поиск на ПК или выберите папку/диск старой системы.",
			);
			return;
		}
	...
	async function probeMigrationDiscoveryCandidate(
		...
		try {
			...
			setMigrationSourceProbe(
				(await response.json()) as MigrationLocalSourceProbeResponse,
			);
		} catch (probeError) {
			setError(
				operatorWorkflowFailureMessage(
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(
						response,
						"Публичный поиск клиники не выполнен",
					),
				);
			}
			setClinicPublicLookup(
				(await response.json()) as ClinicPublicLookupResponse,
			);
		} catch (lookupError) {
			setError(
				operatorWorkflowFailureMessage(
	...
	async function previewImagingImport() {
		...
		try {
			...
			setImagingImportPreview(
				(await response.json()) as ImagingImportPreviewResponse,
			);
			setImagingImportCommit(null);
	...
	function applyBrowserPickedImagingFolderPreview(
					...
					current
						? (() => {
								...
								return {
									...
									currentItem: null,
									updatedAt,
									elapsedMs: browserImagingScanElapsedFromIso(
										current.startedAt,
										updatedAt,
									),
								};
	...
	async function addBrowserMigrationFileToStats(input: {
		...
		const modifiedAt = input.file.lastModified
			? new Date(input.file.lastModified).toISOString()
			: null;
		if (
			modifiedAt &&
			(!input.stats.latestModifiedAt ||
				modifiedAt > input.stats.latestModifiedAt)
		)
			input.stats.latestModifiedAt = modifiedAt;
		if (input.fileName.toLowerCase() === "dicomdir")
			input.stats.hasDicomDir = true;
	...
	function applyBrowserMigrationDiscovery(
		discovery: MigrationLocalSourceDiscoveryResponse,
	) {
	...
	async function runBrowserMigrationSourceScan(input: {
		...
		try {
			...
			if (browserMigrationScanAbortRef.current === controller)
				browserMigrationScanAbortRef.current = null;
	...
	async function scanBrowserMigrationDirectoryHandle(
		...
		while (
			...
			publishBrowserMigrationScanProgress(
				...
				options,
				runtime,
				"проверка подпапок старой системы",
			);
			...
			try {
				...
				for await (const [entryName, handle] of current.handle.entries()) {
					...
					if (
						...
						warnings.push(
							`Браузерный список ограничил одну папку ${browserMigrationScanDirectoryEntryLimit} элементами для отзывчивости интерфейса.`,
						);
						break;
					}
					if (handle.kind === "directory") {
						if (
							scannedFolders + stack.length <
							browserMigrationScanFolderLimit
						) {
							stack.push({
								handle,
					...
					throwIfBrowserMigrationScanAborted(options.signal);
					const stats = browserMigrationStatsFor(
						statsByFolder,
						current.key,
						`${current.hint} ${entryName}`,
						current.depth,
					);
					const allowMagicRead =
						magicReads < browserMigrationScanMagicReadLimit;
					...
					publishBrowserMigrationScanProgress(
						...
						options,
						runtime,
						"проверка старых баз, выгрузок и снимков",
					);
					await maybeYieldBrowserMigrationScan(runtime, options.signal);
				}
			} catch (scanError) {
		...
		if (scannedFiles >= browserMigrationScanFileLimit)
			warnings.push(
				`Браузерный список ограничен ${browserMigrationScanFileLimit} файлами для отзывчивости интерфейса.`,
			);
		if (scannedFolders >= browserMigrationScanFolderLimit)
			warnings.push(
				`Браузерный список ограничен ${browserMigrationScanFolderLimit} папками для отзывчивости интерфейса.`,
			);
		publishBrowserMigrationScanProgress(
			progressStats,
			options,
	...
	async function scanBrowserMigrationFileList(
		fileList: FileList,
		options: BrowserMigrationScanOptions,
	): Promise<MigrationLocalSourceDiscoveryResponse> {
		const runtime = createBrowserMigrationScanRuntime(options.startedAt);
		...
		for (let fileIndex = 0; fileIndex < scanCount; fileIndex += 1) {
			...
			const stats = browserMigrationStatsFor(
				...
				folderKey,
				folderHint,
				Math.max(0, folderParts.length - 1),
			);
		...
		if (selectedFileCount > browserMigrationScanFileLimit)
			warnings.push(
				`Браузерный список ограничен ${browserMigrationScanFileLimit} файлами для отзывчивости интерфейса.`,
			);
		publishBrowserMigrationScanProgress(
	...
	async function pickBrowserMigrationSource() {
		...
		try {
			...
			setError(
				browserLocalSourceErrorMessage(
					"Браузер не открыл выбор старой базы или папки снимков",
					pickerError,
				),
			);
		} finally {
	...
	async function handleBrowserMigrationInputChange(fileList: FileList | null) {
		...
		try {
			...
			setError(
				browserLocalSourceErrorMessage(
					"Браузер не разобрал выбранные файлы старой системы",
					pickerError,
				),
			);
		} finally {
			setIsBrowserMigrationScanning(false);
			if (browserMigrationInputRef.current)
				browserMigrationInputRef.current.value = "";
		}
	}
		...
		while (
			...
			try {
				...
				for await (const [, handle] of current.entries()) {
					...
					if (
						kind === "other" &&
						magicReads < browserImagingScanMagicReadLimit
					) {
						magicReads += 1;
	...
	async function scanBrowserFileList(
		fileList: FileList,
		options: BrowserImagingScanOptions,
	): Promise<BrowserPickedImagingFolderPreview> {
		const runtime = createBrowserImagingScanRuntime(options.startedAt);
	...
	async function pickBrowserImagingFolder() {
		...
		try {
			...
			setError(
				browserLocalSourceErrorMessage(
					"Браузер не открыл выбор папки снимков",
					pickerError,
				),
			);
		} finally {
	...
	async function handleBrowserDirectoryInputChange(fileList: FileList | null) {
		...
		try {
			...
			setError(
				browserLocalSourceErrorMessage(
					"Браузер не открыл выбор файлов снимков",
					pickerError,
				),
			);
		} finally {
			setIsBrowserImagingFolderPicking(false);
			if (browserDirectoryInputRef.current)
				browserDirectoryInputRef.current.value = "";
		}
	}
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(
						response,
						"Поиск папок со снимками не выполнен",
					),
				);
			}
			const result =
				(await response.json()) as DicomLocalFolderDiscoveryResponse;
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(
						response,
						"Локальный организатор снимков не выполнен",
					),
				);
			}
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(
						response,
						"Метаданные папки снимков не прочитаны",
					),
				);
			}
			const result =
				(await response.json()) as DicomFolderSeriesPreviewResponse;
	...
	async function commitImagingImport() {
		...
		if (!imagingImportText.trim()) {
			setError(
				"Вставьте строки со снимками или выберите пример КТ/ОПТГ/ТРГ перед привязкой.",
			);
			return;
		}
		...
		try {
			...
			setError(
				operatorWorkflowFailureMessage("Снимки не записаны", importError),
			);
		} finally {
		...
		try {
			const response = await fetch("/api/clinical/rules", {
				...
				body: JSON.stringify({
					...
					requiresCompletedServiceIds:
						newRuleAction === "block_service"
							? [newRuleCompletedServiceId]
							: [],
					blockedServiceIds:
						newRuleAction === "block_service" ? [newRuleBlockedServiceId] : [],
			...
			if (!response.ok) {
				throw new Error(
					await responseErrorMessage(
						response,
						"Клиническое правило не сохранено",
					),
				);
			}
			await loadDashboard();
	...
	function clinicalToothSurfacesValue(value: string): ClinicalToothSurface[] {
		const surfaces = value
			...
			.map(
				(part) =>
					clinicalToothSurfaceAliases[normalizeClinicalToothAlias(part)],
			)
	...
	function clinicalToothStatusValue(value: string): ClinicalToothStatus {
		return (
			clinicalToothStatusAliases[normalizeClinicalToothAlias(value)] ??
			"planned"
		);
	}
	...
	async function createDocument(kind: GeneratedDocument["kind"]) {
		...
		const selectedTaxPayerInn = isTaxDocument
