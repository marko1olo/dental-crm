...
export function useAppLogic(): any {
	...
	const [clinicProfileDraft, setClinicProfileDraft] =
		useState<ClinicProfileDraft>(emptyClinicProfileDraft());
	...
	return {
		...
		/*
		 * РАСШИТО. Лента снимков активного пациента восстановлена в
		 * useImagingLogic (код из 57d904b0a~1 вместе с обоснованием про
		 * синтетический activeVisit с нулевым UUID). Здесь стоял null, и
		 * ImagingView показывал «В ленте 0» при любых данных.
		 */
		activeImagingStudies,
		activePayments,
		activeTreatmentPlanItems,
		...
		/*
		 * РАСШИТО. Значение живое: useImagingLogic.ts:252 считает
		 * `dashboard?.imagingStudies ?? []` и передаёт его в
		 * useDicomWorkbenchModule. В контекст оно не попадало, потому что
		 * здесь стоял `null`, и ImagingView получал пустоту:
		 * ImagingView.tsx:1410 показывал «снимков нет» всегда, а перебор на
		 * :1442 не давал ни одной карточки.
		 */
