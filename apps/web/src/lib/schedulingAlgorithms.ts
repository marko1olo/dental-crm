...
function getWorkingHoursForDay(
	staff: StaffMember | Chair,
	weekday: number,
): StaffWorkingDay | undefined {
	if (!staff.workingHours) return undefined;
	return staff.workingHours.find(
		(day) => day.enabled && day.weekday === weekday,
	);
}
...
export function findAvailableSlots(
	...
	const assistant = assistantId
		? dashboard.clinicSettings.staff.find((s) => s.id === assistantId)
		: null;
	...
	const sortedAppointments = [...dashboard.appointments].sort(
		(a, b) =>
			parseStrictAppointmentDateTimeMs(a.startsAt) -
			parseStrictAppointmentDateTimeMs(b.startsAt),
	);
	...
	for (let dayOffset = 0; dayOffset < 14; dayOffset++) {
		...
		const astWH = assistant
			? getWorkingHoursForDay(assistant, weekday)
			: undefined;
		...
		const startMins = [
			timeToMinutes(docWH.start || "00:00"),
			timeToMinutes(chairWH.start || "00:00"),
		];
		const endMins = [
			timeToMinutes(docWH.end || "00:00"),
			timeToMinutes(chairWH.end || "00:00"),
		];
		...
		const searchStartMin = Math.max(...startMins);
		const searchEndMin = Math.min(...endMins);
		...
		const dayAppointments = sortedAppointments.filter((app) => {
			...
			return (
				app.doctorUserId === doctorId ||
				app.chairId === chairId ||
				(assistantId && app.assistantUserId === assistantId)
			);
		});

		// Try to fit the appointment in 15-minute increments
		for (
			let time = searchStartMin;
			time <= searchEndMin - durationMinutes;
			time += 15
		) {
			if (availableSlots.length >= limit) break;
			...
			for (const app of dayAppointments) {
				...
				if (slotStartMs < appEndMs && slotEndMs > appStartMs) {
					...
					time = Math.max(
						time,
						timeToMinutes(app.endsAt.split("T")[1].substring(0, 5)) - 15,
					); // -15 because the loop adds 15
					break;
				}
			}
