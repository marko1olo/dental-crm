...
function withFixedNow<T>(iso: string, fn: () => T): T {
	...
	(globalThis as unknown as { Date: typeof Date }).Date = FixedDate as unknown as typeof Date;
	try {
		return fn();
	} finally {
...
function durationMinutes(parsed: { startsAt?: string; endsAt?: string }): number | null {
	if (!parsed.startsAt || !parsed.endsAt) return null;
	return (new RealDate(parsed.endsAt).getTime() - new RealDate(parsed.startsAt).getTime()) / 60000;
}
...
describe("smartBookingParser: даты", () => {
	it("«завтра», «сегодня», «послезавтра» считаются от текущего дня", () => {
		const parse = (text: string) => withFixedNow(NOW, () => smartBookingParser(text, dashboard));
		assert.equal(localDay(parse("Иванов кариес сегодня в 15:30").startsAt), "2026-07-27");
		assert.equal(localDay(parse("Иванов кариес завтра в 15:30").startsAt), "2026-07-28");
		assert.equal(localDay(parse("Иванов кариес послезавтра в 15:30").startsAt), "2026-07-29");
	});
	...
	it("названное число, которое уже прошло, переносится на следующий месяц", () => {
		...
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов кариес на 3 число в 15:30", dashboard));
		assert.equal(localDay(parsed.startsAt), "2026-08-03");
	});

	it("названное число этого месяца, которое ещё не прошло, остаётся в нём", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов кариес на 30 число в 15:30", dashboard));
	...
	it("название месяца не уезжает, когда сегодня 31-е", () => {
		...
		const parse = (text: string) => withFixedNow("2026-05-31T12:00:00+04:00", () => smartBookingParser(text, dashboard));
		assert.equal(localDay(parse("Иванов кариес на 5 февраля в 15:30").startsAt), "2027-02-05");
		// Врач называет июнь — приём обязан попасть в июнь, а не в июль.
		assert.equal(localDay(parse("Иванов кариес на 20 июня в 15:30").startsAt), "2026-06-20");
	});
	...
	it("время начала берётся из диктовки", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов кариес завтра в 15:30", dashboard));
...
describe("smartBookingParser: участники", () => {
	...
	it("различает пациента и врача по предлогу «к»", () => {
		const parsed = withFixedNow(NOW, () =>
			smartBookingParser("Петров на чистку завтра в 10:00 к Смирнову", dashboard),
		);
	...
	it("находит кресло по названию", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов завтра в 15:30 кресло 2", dashboard));
		assert.equal(parsed.chairId, "c2");
	});

	it("не выдумывает пациента, которого нет в списке", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Сидоров завтра в 15:30", dashboard));
...
describe("smartBookingParser: причина и длительность", () => {
	...
	it("явная длительность важнее типовой", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов кариес завтра в 12:00 на 90 минут", dashboard));
		assert.equal(durationMinutes(parsed), 90);
	});

	it("интервал «с ... до ...» задаёт и начало, и длительность", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("Иванов завтра с 14:00 до 15:30", dashboard));
		const start = new RealDate(parsed.startsAt as string);
...
describe("smartBookingParser: действия", () => {
	it("отмена помечает действие и статус", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("отмени запись Петрова", dashboard));
	...
	it("перенос помечает действие и сохраняет новое время", () => {
		const parsed = withFixedNow(NOW, () => smartBookingParser("перенеси Иванова на завтра в 16:00", dashboard));
	...
	it("остаток текста попадает в примечание, а не теряется", () => {
		const parsed = withFixedNow(NOW, () =>
			smartBookingParser("Иванов завтра в 15:30 просил позвонить заранее", dashboard),
		);
