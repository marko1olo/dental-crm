...
function runTests() {
	const testCases = [
		...
		},
		{
			...
			input:
				"Запиши Иванову на следующую среду в 15:00 на удаление зуба мудрости к Петрову",
			expectPatient: "Иванов",
			expectDoctor: "Петров",
		...
		},
		{
			...
			input:
				"Смирнов придет сегодня в 6 часов вечера на имплантацию к Удальцову",
		...
		},
		{
			...
			input:
				"Сидоров, болит 36 зуб, запиши на завтра в 12:00 на пульпит к Смирнову",
			expectPatient: "Сидоров",
			expectDoctor: "Смирнов",
			expectReason: "пульпит",
			expectTime: "12:00",
	...
	testCases.forEach((tc, _index) => {
		...
		if (
			...
			!mockDashboard.clinicSettings.staff
				.find((s) => s.id === result.doctorUserId)
				?.fullName.includes(tc.expectDoctor)
		) {
			ok = false;
			errors.push(
				`Expected doctor ${tc.expectDoctor}, got ${mockDashboard.clinicSettings.staff.find((s) => s.id === result.doctorUserId)?.fullName}`,
			);
		}
		if (
			tc.expectReason &&
			!result.reason
				?.toLowerCase()
				.includes(tc.expectReason.toLowerCase().substring(0, 4))
		) {
			ok = false;
			errors.push(`Expected reason ${tc.expectReason}, got ${result.reason}`);
		}
		...
		if (tc.expectTime) {
			...
			if (timeStr !== tc.expectTime) {
				ok = false;
				errors.push(`Expected time ${tc.expectTime}, got ${timeStr}`);
			}
		}
