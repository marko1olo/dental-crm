 ...
 */
export function isFuzzyRootMatch(
	inputWord: string,
	targetRoot: string,
): boolean {
	if (!inputWord || !targetRoot) return false;
...
function bestFuzzyNumberRoot(word: string, roots: string[]): string | null {
	...
	for (const root of roots) {
		...
		if (
			...
			(distance === bestDistance && lengthGap < bestLengthGap)
		) {
			bestRoot = root;
           ...
           полностью самодостаточна, поэтому переходим к следующему слову. */
              ...
              Забираем последнее уже выданное целое число и умножаем его. */
					...
					if (lastTwoDigits >= 11 && lastTwoDigits <= 14) {
						if (wLower.endsWith("а") || wLower.endsWith("и"))
							isValidMultiplier = false;
					} else if (lastDigit === 1) {
						if (!wLower.endsWith("а") && !wLower.endsWith("у"))
							isValidMultiplier = false;
                 ...
                 числительного, поэтому принимаем и её. */
						if (!wLower.endsWith("и") && wLower !== "тысяч")
							isValidMultiplier = false;
					} else {
						if (wLower.endsWith("а") || wLower.endsWith("и"))
							isValidMultiplier = false;
			...
			for (let j = i + 1; j < tokens.length; j++) {
             ...
             считалось продолжением числа, и накопление суммы шло дальше. */
					...
					if (!hasNext)
						hasNext =
							bestFuzzyNumberRoot(nextWord, Object.keys(numValues)) !== null;
					if (hasNext) {
						nextIsNumber = true;
					}
					break;
			...
			if (!nextIsNumber) {
				result.push(
					currentNum === 0 && matchedVal !== 0
						? token
						: currentNum.toString() + suffix,
				);
				currentNum = 0;
				inNumber = false;
			}
		...
		if (toothDigit) {
			...
			for (let j = searchStart; j <= searchEnd; j++) {
				...
				if (
					...
					isUpper = true;
				if (
					isFuzzyRootMatch(ctxWord, "нижн") ||
					isFuzzyRootMatch(ctxWord, "снизу")
				)
					isLower = true;
				if (
					isFuzzyRootMatch(ctxWord, "лев") ||
					isFuzzyRootMatch(ctxWord, "слев")
				)
					isLeft = true;
				if (
					isFuzzyRootMatch(ctxWord, "прав") ||
					isFuzzyRootMatch(ctxWord, "справ")
				)
					isRight = true;
			}
