...
describe("parsePatientDictationLocal: ФИО", () => {
	it("разбирает фамилию, имя и отчество", () => {
		assert.equal(
			parsePatientDictationLocal("Иванов Иван Иванович 89161234567 12.05.1980")
				.fullName,
			"Иванов Иван Иванович",
		);
		assert.equal(
			parsePatientDictationLocal("Иванов Иван").fullName,
			"Иванов Иван",
		);
	...
	it("приводит регистр к виду, принятому в карте", () => {
		assert.equal(
			parsePatientDictationLocal("ИВАНОВ ИВАН ИВАНОВИЧ").fullName,
			"Иванов Иван Иванович",
		);
	});
	...
	it("короткие фамилии не теряются", () => {
		assert.equal(
			parsePatientDictationLocal("Ли Ким 89161234567").fullName,
			"Ли Ким",
		);
	});
...
describe("parsePatientDictationLocal: телефон", () => {
	it("приводит запись телефона к единому виду", () => {
		...
		for (const input of variants) {
			assert.equal(
				parsePatientDictationLocal(input).phone,
				"+79161234567",
				`вход «${input}»`,
			);
		}
	...
	it("не принимает за телефон число неподходящей длины", () => {
		...
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 891612345678").phone,
			"",
		);
	});
});
...
describe("parsePatientDictationLocal: дата рождения", () => {
	it("разбирает дату цифрами и словами", () => {
		...
		assert.equal(
			parsePatientDictationLocal(
				"Сидорова Анна Сергеевна 9161234567 05.11.1992",
			).birthDate,
			"1992-11-05",
		);
		assert.equal(
			parsePatientDictationLocal("Кузнецов Олег 1 января 2000").birthDate,
			"2000-01-01",
		);
		assert.equal(
			parsePatientDictationLocal("Петров Петр 8 916 123 45 67 15 марта 1975")
				.birthDate,
			"1975-03-15",
		);
	});
	...
	it("двузначный год определяется по текущему году, а не по зашитому порогу", () => {
		...
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 12.05.30").birthDate,
			"1930-05-12",
		);
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 12.05.80").birthDate,
			"1980-05-12",
		);
		// Годы не больше текущего остаются в этом веке.
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 12.05.05").birthDate,
			"2005-05-12",
		);
	});
	...
	it("несуществующая дата не подставляется", () => {
		...
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 31.02.1990").birthDate,
			"",
		);
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 12.13.1990").birthDate,
			"",
		);
		assert.equal(
			parsePatientDictationLocal("Иванов Иван 00.05.1990").birthDate,
			"",
		);
	});
	...
	it("дата рождения в будущем не подставляется", () => {
		...
		assert.equal(
			parsePatientDictationLocal(`Иванов Иван 12.05.${nextYear}`).birthDate,
			"",
		);
	});
	...
	it("телефон и дата рождения разбираются вместе и не путаются", () => {
		const result = parsePatientDictationLocal(
			"Иванов Иван 12 мая 1980 89161234567",
		);
