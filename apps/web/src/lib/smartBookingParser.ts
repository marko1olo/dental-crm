...
const REASON_NAMES: Record<string, string> = {
	...
	кариес: "Кариес",
	удалени: "Удаление зуба",
	восьмерк: "Удаление зуба мудрости",
	гигиен: "Профгигиена",
	чистк: "Профгигиена",
	...
	синус: "Синус-лифтинг",
	швы: "Снятие швов",
	"снятие швов": "Снятие швов",
	лечени: "Лечение зуба",
	лечение: "Лечение зуба",
	терапи: "Терапия",
	отбеливани: "Отбеливание",
	пломб: "Установка пломбы",
	фторировани: "Фторирование",
	реставраци: "Реставрация зуба",
	капп: "Изготовление каппы",
	протезировани: "Протезирование",
	фиксаци: "Фиксация ортодонтического аппарата",
	ретейнер: "Установка ретейнера",
	кюретаж: "Кюретаж пародонтального кармана",
};
...
export function smartBookingParser(
	...
	const doctors = staff.filter(
		(u: StaffMember) => u.active && (u.role === "doctor" || u.role === "owner"),
	);
	const assistants = staff.filter(
		(u: StaffMember) => u.active && u.role === "assistant",
	);
	...
	for (let i = 0; i < words.length; i++) {
		...
		for (const doc of doctors) {
			...
			const matchWords = [
				...parts,
				...specs.flatMap((s: string) => s.toLowerCase().split(" ")),
			];
		...
		for (const ast of assistants) {
			...
			if (parts.some((p: string) => isFuzzyMatch(word, p))) {
				...
				if (["пациент", "запиши", "записать", "к", "ко"].includes(prev))
					score -= 100;
		...
		for (const pat of patients) {
			...
			if (parts.some((p: string) => isFuzzyMatch(word, p))) {
				...
				if (
					["к", "ко", "врачу", "доктору", "док", "стоматологу", "с"].includes(
						prev,
					) ||
					["к", "ко"].includes(prevPrev)
				)
				...
				if (foundDocs.some((d) => d.score > 100 && d.indexes.includes(i)))
					score -= 300;
	...
	if (
		bestDoc &&
		bestPat &&
		bestDoc.indexes.some((idx) => bestPat.indexes.includes(idx))
	) {
		if (bestDoc.score > bestPat.score) {
			parsed.doctorUserId = bestDoc.id;
			const otherPat = foundPats.find(
				(p) => !p.indexes.some((idx) => bestDoc.indexes.includes(idx)),
			);
			if (otherPat) parsed.patientId = otherPat.id;
		} else {
			parsed.patientId = bestPat.id;
			const otherDoc = foundDocs.find(
				(d) => !d.indexes.some((idx) => bestPat.indexes.includes(idx)),
			);
	...
	if (bestAssist && bestAssist.score >= 0) {
		...
		if (
			!parsed.patientId ||
			!bestAssist.indexes.some((idx) => bestPat?.indexes.includes(idx) || false)
		) {
		...
		wordsToRemove.forEach((p) => {
			if (p.length > 3) {
				remaining = remaining.replace(
					new RegExp(
						`(?:^|[^а-яёa-z0-9])${p.substring(0, Math.max(3, p.length - 2))}[а-я]*`,
						"ig",
					),
					" ",
				);
				...
				remaining = remaining.replace(
					new RegExp(`(?:^|[^а-яёa-z0-9])${p}(?:[^а-яёa-z0-9]|$)`, "ig"),
					" ",
				);
			}
		});
	...
	if (parsed.assistantUserId) {
		const ast = assistants.find(
			(a: StaffMember) => a.id === parsed.assistantUserId,
		);
		if (ast)
			removeWordsSafely([
				...ast.fullName.split(" "),
	...
	// Extract Actions
	const cancelRegex =
		/(?:^|[^а-яёa-z0-9])(отмен[а-я]*|удали|удалите|удалить запись|убери|сними|снимите)(?:[^а-яёa-z0-9]|$)/i;
	...
	const rescheduleRegex =
		/(?:^|[^а-яёa-z0-9])(перенес[а-я]*|перезапиши|перезаписать|перекинь|сдвинь)(?:[^а-яёa-z0-9]|$)/i;
	...
	let rangeStartHour = -1,
		rangeStartMin = -1;
	let rangeEndHour = -1,
		rangeEndMin = -1;
	const rangeRegex =
		/(?:с|от)\s+([01]?[0-9]|2[0-3])[:\-.]?([0-5][0-9])?\s+(?:до|по)\s+([01]?[0-9]|2[0-3])[:\-.]?([0-5][0-9])?(?=\s|$|[^а-яёa-z0-9])/i;
	...
	if (rangeMatch && !isCancel) {
		...
		let totalMins =
			rangeEndHour * 60 + rangeEndMin - (rangeStartHour * 60 + rangeStartMin);
	...
	if (!isCancel && !explicitDurationFound) {
		...
		const minsMatch = remaining.match(
			/(?:^|[^а-яёa-z0-9])(на\s+)?([1-9][0-9]*)\s*(минут|мин)(?:[^а-яёa-z0-9]|$)/i,
		);
		const hrsMatch = remaining.match(
			/(?:^|[^а-яёa-z0-9])(на\s+)?([1-9][0-9]?)\s*(час|часов|часа)(?:[^а-яёa-z0-9]|$)/i,
		);

		if (
			containsAnyFuzzyRoot(remaining, ["на полчаса", "на пол часа", "полчаса"])
		) {
			durationMinutes = 30;
			explicitDurationFound = true;
			remaining = remaining.replace(
				/(?:^|[^а-яёa-z0-9])(на\s+)?(полчаса|пол часа)(?:[^а-яёa-z0-9]|$)/gi,
				" ",
			);
		} else if (
			containsAnyFuzzyRoot(remaining, ["на полтора часа", "полтора часа"])
		) {
			durationMinutes = 90;
			explicitDurationFound = true;
			remaining = remaining.replace(
				/(?:^|[^а-яёa-z0-9])(на\s+)?(полтора часа)(?:[^а-яёa-z0-9]|$)/gi,
				" ",
			);
		} else if (minsMatch) {
			...
			explicitDurationFound = true;
			remaining = remaining.replace(
				/(?:^|[^а-яёa-z0-9])(на час)(?:[^а-яёa-z0-9]|$)/gi,
				" ",
			);
		}
	}
	...
	if (matchedReasonKey && !isCancel) {
		parsed.reason =
			REASON_NAMES[matchedReasonKey] ||
			matchedReasonKey.charAt(0).toUpperCase() + matchedReasonKey.slice(1);
		remaining = remaining.replace(
			new RegExp(
				`(?:^|[^а-яёa-z0-9])\\S*${matchedReasonKey}\\S*(?:[^а-яёa-z0-9]|$)`,
				"ig",
			),
			" ",
		);
	...
	for (const dw of dateWords) {
		if (containsAnyFuzzyRoot(remaining, [dw.word])) {
			...
			dateFound = true;
			remaining = remaining.replace(
				new RegExp(`(?:^|[^а-яёa-z0-9])(${dw.word})(?:[^а-яёa-z0-9]|$)`, "ig"),
				" ",
			);
			break;
		}
		...
		if (explicitDateMatch) {
			...
			for (let i = 0; i < WEEKDAYS.length; i++) {
				...
				if (containsAnyFuzzyRoot(remaining, [dayName])) {
					...
					remaining = remaining.replace(
						new RegExp(
							`(?:^|[^а-яёa-z0-9])(?:в|на|во|со)?\\s*${dayName}(?:[^а-яёa-z0-9]|$)`,
							"ig",
						),
						" ",
					);
	...
	if (rangeStartHour !== -1) {
		...
		const timeRegex =
			/(?:^|\s|\b)(?:в\s+)?([01]?[0-9]|2[0-3])[:\-.]([0-5][0-9])(?=\s|$|\b)/i;
		const timeRegexSpace =
			/(?:^|\s)(?:в|на)?\s*([0-1]?[0-9]|2[0-3])\s+([0-5][0-9])(?=\s|$)/;
		const halfRegex =
			/(?:(?:в|на|к)\s+)?(?:пол[\s-]*|в\s+половину\s+|половина\s+)([1-9]|1[0-2]|первого|второго|третьего|четвертого|пятого|шестого|седьмого|восьмого|девятого|десятого|одиннадцатого|двенадцатого)(?=\s|$|[^а-яё0-9])/i;
		const hourOnlyRegex =
			/(?:(?:в|на|к)\s+)?([01]?[0-9]|2[0-3])(?:\s*(часов|утра|вечера|вечером|дня))(?=\s|$)/i;
		...
		if (halfMatch) {
			...
			timeFound = true;
			if (
				(hourMatch[0].includes("вечера") || hourMatch[0].includes("вечером")) &&
				hours < 12
			)
				hours += 12;
			if (hourMatch[0].includes("дня") && hours < 12 && hours >= 1) hours += 12;
			if (hours >= 1 && hours <= 8 && !hourMatch[0].includes("утра"))
				hours += 12;
	...
	if (dateFound || timeFound) {
		...
		parsed.endsAt = new Date(
			targetDate.getTime() + durationMinutes * 60000,
		).toISOString();
	}
	...
	const stopWords =
		/(?:^|[^а-яёa-z0-9])(нового|новому|зовут|новый|новая|запиши|записать|перенеси|перезапиши|отмени|пац(?:иент[ауоке]?)?|пожалуйста|доктору|врачу|врач|ассистент|медсестра|кресло|прием|срочно|запись|номер|в|на|к|ко|с|до|по|от|утра|дня|вечера|вечером|часов|минут|телефон|время|жалоба|после|обеда|хирург[уа]?|терапевт[уа]?|ортодонт[уа]?|ортопед[уа]?|имплантолог[уа]?)(?:[^а-яёa-z0-9]|$)/gi;
	...
	if (!parsed.patientId && parsed.action !== "cancel") {
		...
		if (parsed.patientName) {
			const npWords = parsed.patientName
				.toLowerCase()
				.split(" ")
				.filter((w) => w.length > 2);
			npWords.forEach((w) => {
				remaining = remaining.replace(
					new RegExp(`(?:^|[^а-яёa-z0-9])${w}(?:[^а-яёa-z0-9]|$)`, "ig"),
					" ",
				);
			});
		}
		if (parsed.patientPhone) {
			...
			remaining = remaining.replace(
				new RegExp(
					`(?:^|[^0-9])(?:8|\\+7)?\\s*${last10.substring(0, 3)}[- ]?${last10.substring(3, 6)}[- ]?${last10.substring(6, 8)}[- ]?${last10.substring(8, 10)}(?:[^0-9]|$)`,
					"ig",
				),
				" ",
			);
