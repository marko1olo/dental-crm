...
export function parseScheduleDictationLocal(input: string): ParsedScheduleData {
	...
	// 0. Detect intent
	const cancelRegex =
		/(?:^|[^а-яёa-z0-9])(отмен[а-я]*|удали(?!\w)|удалите(?!\w)|удалить(?!\s+(?:зуб|восьмерк|семерк|шестерк|пятерк|четверк|тройк|двойк|единичк|корень|кист|имплант|нерв|капюшон))|убери|сними(?!\w)|снимите(?!\w))(?:[^а-яёa-z0-9]|$)/i;
	const rescheduleRegex =
		/(?:^|[^а-яёa-z0-9])(перенес[а-я]*|двин[а-я]*|сдвин[а-я]*)(?:[^а-яёa-z0-9]|$)/i;
	...
	if (cancelRegex.test(remaining)) {
		...
		remaining = remaining.replace(
			/(?:^|[^а-яёa-z0-9])(запиш[а-я]*|созда[а-я]*|добав[а-я]*)(?:[^а-яёa-z0-9]|$)/gi,
			" ",
		);
	}
	...
	// 15:00, 15.00, 15-00
	const timeRegex =
		/(?:^|\s|\b)([0-1]?[0-9]|2[0-3])[-:.]([0-5][0-9])(?=\s|$|\b)/;
	const timeRegexSpace =
		/(?:^|\s)(?:в|на)?\s*([0-1]?[0-9]|2[0-3])\s+([0-5][0-9])(?=\s|$)/;
	const hourRegex =
		/(?:^|\s)(?:в|на)\s+([0-1]?[0-9]|2[0-3])\s*(часов|часа|ч)?(?=\s|$)/;
	...
	if (containsAnyFuzzyRoot(remaining, ["послезавтра"])) {
		...
		remaining = remaining.replace(
			/(?:^|[^а-яёa-z0-9])(послезавтра)(?:[^а-яёa-z0-9]|$)/gi,
			" ",
		);
	} else if (containsAnyFuzzyRoot(remaining, ["завтра"])) {
		result.dateStr = getOffsetDate(1);
		remaining = remaining.replace(
			/(?:^|[^а-яёa-z0-9])(завтра)(?:[^а-яёa-z0-9]|$)/gi,
			" ",
		);
	} else if (containsAnyFuzzyRoot(remaining, ["сегодня", "щас", "сейчас"])) {
		result.dateStr = getOffsetDate(0);
		remaining = remaining.replace(
			/(?:^|[^а-яёa-z0-9])(сегодня|щас|сейчас)(?:[^а-яёa-z0-9]|$)/gi,
			" ",
		);
	} else {
		const explicitDateMatch = remaining.match(
			/([1-9]|[12][0-9]|3[01])\s+([а-я]{3,8})/i,
		);
		let dateFound = false;
		...
		if (!dateFound) {
			...
			for (let i = 0; i < WEEKDAYS.length; i++) {
				...
				if (containsAnyFuzzyRoot(remaining, [day])) {
					...
					remaining = remaining.replace(
						new RegExp(
							`(?:^|[^а-яёa-z0-9])(?:в|на|во)?\\s*${day}[а-я]*(?:[^а-яёa-z0-9]|$)`,
							"ig",
						),
						" ",
					);
	...
	for (const s of SERVICES) {
		if (containsAnyFuzzyRoot(remaining, [s.key])) {
			...
			remaining = remaining.replace(
				new RegExp(
					`(?:^|[^а-яёa-z0-9])\\S*${s.key}\\S*(?:[^а-яёa-z0-9]|$)`,
					"ig",
				),
				" ",
			);
