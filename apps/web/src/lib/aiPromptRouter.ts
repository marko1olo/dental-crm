	...
	Patient: {
		ExtractDetails: (
			...
			schemaFormat: object = {
				...
				passport:
					"Серия и номер паспорта, кем выдан, код подразделения, если есть",
	...
	Schedule: {
		AnalyzeNote: (
			...
			schemaFormat: object = {
				...
				service:
					"Услуга или причина обращения (например, Удаление зуба, Консультация)",
	...
	Medical: {
		StructureEmk: (
			...
			schemaFormat: object = {
				...
				objectiveStatus:
					"Объективный статус (зонд, перкуссия, пальпация, ЭОД, термопроба)",
				diagnosis:
					"Диагноз (желательно с кодом по МКБ-10, если понятно из контекста)",
...
`,
	...
	Imaging: {
		AnalyzeCTReport: (
			...
			schemaFormat: object = {
				findings: [
					"Список ключевых находок (кисты, кариес, ретенция, воспаления)",
				],
				teethAffected: ["Номера проблемных зубов"],
				implantFeasibility:
					"Оценка возможности имплантации (объем кости, близость канала), если упомянуто",
...
`,
	...
	Marketing: {
		PatientFollowUp: (
			...
			schemaFormat: object = { messageText: "текст сообщения" },
		) => `
...
Верни JSON: ${JSON.stringify(schemaFormat, null, 2)}
`,
		...
		GenerateReviewReply: (
			...
			clinicName: string,
			seoKeys: string[],
			schemaFormat: object = { replyText: "твой ответ" },
		) => `
