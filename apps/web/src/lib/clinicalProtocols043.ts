...
export function generateSoapFromOdontogramFinding(
	...
	if (stateNorm === "caries" || stateNorm.startsWith("k02")) {
		...
		const statusLocalis = isInitial
			? `Зуб ${toothTitle}: Кариозное поражение в стадии пятна на ${surfacesStr} поверхности. При осмотре: матовое меловидное пятно с потерей естественного блеска эмали. Зондирование безболезненно. Электровозбудимость пульпы в пределах физиологической нормы.`
			: isRoot
				...
				: isDeep
					? `Зуб ${toothTitle}: Кариозное поражение на ${surfacesStr} поверхности. При осмотре: глубокая кариозная полость в пределах околопульпарного дентина, выполненная размягчённым пигментированным дентином. Зондирование дна болезненно. Электровозбудимость пульпы сохранена, умеренно снижена.`
					: `Зуб ${toothTitle}: Кариозное поражение на ${surfacesStr} поверхности. При осмотре: кариозная полость средней глубины в пределах плащевого дентина. Дно и стенки плотные, пигментированные. Зондирование болезненно по эмалево-дентинной границе. Перкуссия безболезненна. Электровозбудимость пульпы в пределах нормы.`;
		...
		const treatmentDescription = isInitial
			? `Зуб ${tooth}: Профессиональная гигиена и очищение поверхности. Медикаментозная антисептическая обработка. Аппликация реминерализующего препарата / глубокое фторирование эмали фторлаком. Полировка.`
			: isRoot
				? `Местная анестезия амидного ряда (артикаин 4% 1.7 мл). Препарирование пришеечной кариозной полости зуба ${tooth}, антисептическая обработка полости (хлоргексидин 2%). Реставрация светоотверждаемым стеклоиономерным цементом (СИЦ) / компомером с моделированием анатомической формы. Шлифовка, полировка, защитный лак.\n\n${warranty.formattedWarrantyNote}`
				: isDeep
					? `Препарирование кариозной полости зуба ${tooth} на ${surfacesStr} поверхности, полная щадящая некрэктомия. Изоляция коффердамом. Антисептическая медикаментозная обработка полости 2% раствором хлоргексидина биглюконата. Лечебный препарат на основе гидроксида кальция точечно на дно, изолирующая прокладка СИЦ. Адгезивный протокол: кислотное травление 37% ортофосфорной кислотой (etching), нанесение адгезивной системы (adhesive: праймер + бонд), фотополимеризация 20 сек. Послойное моделирование (композитный светоотверждаемый материал) с восстановлением окклюзионной анатомии и контактного пункта. Окклюзионная пришлифовка по копирке, шлифовка и полировка (polishing: диски, полиры, паста) до сухого зеркального блеска.\n\n${warranty.formattedWarrantyNote}`
					: `Препарирование кариозной полости зуба ${tooth} на ${surfacesStr} поверхности, полная некрэктомия, формирование эмалевого фальца. Изоляция рабочего поля коффердамом. Медикаментозная антисептическая обработка 2% раствором хлоргексидина биглюконата. Кислотное травление эмали и дентина 37% ортофосфорной кислотой (etching: эмаль 20 сек, дентин 10 сек), тщательное смывание водой, деликатное подсушивание воздухом. Нанесение адгезивной системы (adhesive: праймер + бонд), экспозиция и втирание 20 сек, раздувание, фотополимеризация 20 сек. Послойное моделирование (композитный светоотверждаемый материал) с восстановлением анатомических бугров, фиссур и контактного пункта. Окклюзионная коррекция по копирке, шлифовка и финишная полировка (polishing: алмазные боры, диски, силиконовые головки, полировочная паста) до сухого зеркального блеска.\n\n${warranty.formattedWarrantyNote}`;
	...
	if (
		...
		return {
			...
			treatmentDescription,
			recommendations:
				"При болях — НПВС (нимесулид 100 мг / ибупрофен 400 мг) по 1 таб. после еды. Не жевать на причинную сторону 2-3 дня. Контрольный осмотр через 7-14 дней.",
		};
	}
	...
	if (
		...
		const statusLocalis = isAcute
			? `Коронка зуба ${toothTitle} изменена в цвете / глубокая кариозная полость / пломба. Полость зуба вскрыта, зондирование устьев каналов безболезненно. Вертикальная и горизонтальная перкуссия резко болезненна. Пальпация по переходной складке в области верхушки корня болезненна, слизистая гиперемирована, отечна. Реакция на температурные раздражители отсутствует. Электровозбудимость пульпы отсутствует (пульпа некротизирована). Рентгенологически: расширение периодонтальной щели в области верхушки корня.`
			: `Коронка зуба ${toothTitle} девитализирована, серый оттенок / дефект пломбы. Зондирование безболезненно. Перкуссия слабочувствительна или безболезненна. Пальпация по переходной складке безболезненна. Электровозбудимость пульпы отсутствует. Рентгенограмма: у верхушки корня определяется очаг деструкции костной ткани с четкими/нечеткими контурами (периапикальный очаг).`;

		const treatmentDescription = isAcute
			? `Инфильтрационная/проводниковая местная анестезия амидного ряда (артикаин 4% с эпинефрином 1:100 000 / 1:200 000, 1.7 мл). Создание эндодонтического доступа зуба ${tooth}, раскрытие полости, эвакуация распада из корневых каналов для создания оттока экссудата. Изоляция коффердамом. Определение рабочей длины. Инструментальная обработка каналов, обильная антисептическая ирригация (antiseptic irrigation: 0.05% хлоргексидин, теплый физраствор). Временное введение противовоспалительного препарата под герметичный временный пломбировочный материал. Назначены НПВС, щадящая диета. Повторный прием через 3–5 дней.`
			: `Инфильтрационная/проводниковая местная анестезия амидного ряда (артикаин 4% с эпинефрином 1:100 000 / 1:200 000, 1.7 мл). Раскрытие полости зуба ${tooth}, удаление старого пломбировочного материала / распломбировка и ревизия корневых каналов (canal desobturation). Прохождение каналов до физиологического апекса под контролем апекслокатора. Механическая и медикаментозная антисептическая обработка (NaOCl 3%, 2% хлоргексидин, ЭДТА 17%, УЗ-активация — antiseptic irrigation). Временная обтурация корневых каналов препаратом на основе гидроксида кальция с целью антисептического воздействия и стимуляции остеогенеза. Постановка герметичного временного пломбировочного материала. Контрольный визит через 10-14 дней для постоянного пломбирования.`;
	...
	if (
		...
		const hasExplicitPocketDepth =
			finding.pocketDepthMm !== undefined && finding.pocketDepthMm > 0;
		const icd =
			finding.icd10Override ||
			(isPeriodontitis
				? "K05.3"
		...
		const statusLocalis = isPeriodontitis
			? `Десна в области зуба ${toothTitle} отёчна, застойная гиперемия, цианотична. Обильные над- и поддесневые зубные отложения. ${hasExplicitPocketDepth ? `Глубина пародонтального кармана составляет ${finding.pocketDepthMm} мм` : "Определяются патологические пародонтальные карманы"} с серозно-гнойным экссудатом при компрессии. Патологическая подвижность I-II степени. На рентгенограмме: деструкция костной ткани межальвеолярных перегородок до 1/3–1/2 длины корня.`
			: `Маргинальный и папиллярный край десны в области зуба ${toothTitle} гиперемирован, отёчен, выраженная кровоточивость при зондировании десневой борозды. Зубодесневое прикрепление сохранено, патологические пародонтальные карманы отсутствуют. Обильный мягкий зубной налет и плотные минерализованные над- и поддесневые зубные отложения.`;

		const treatmentDescription = isPeriodontitis
			? `Аппликационная и инфильтрационная местная анестезия амидного ряда. Ультразвуковой скейлинг (ultrasonic scaling) и над/поддесневая обработка аппаратом Air-Flow (Air-Flow polishing). Закрытый кюретаж пародонтального кармана в области зуба ${tooth} кюретами Грейси. Антисептическая ирригация 0.05% раствором хлоргексидина биглюконата. Инстилляция антимикробного стоматологического геля (метронидазол + хлоргексидин). Обучение индивидуальной гигиене, подбор межзубных ершиков и монопучковых щеток. Контрольный осмотр через 10-14 дней.`
			: `Индикация зубного налета. Аппликационная анестезия десны. Ультразвуковой скейлинг с удалением массивных минерализованных над- и поддесневых зубных отложений (ultrasonic scaling). Снятие пигментированного налета водно-порошкоструйным аппаратом Air-Flow (порошок глицин/эритритол) (Air-Flow polishing). Полировка всех поверхностей зубов полировочной пастой и щеточками. Межзубные промежутки очищены флоссом и штрипсами. Глубокое фторирование эмали фторсодержащим лаком. Обучение гигиене, индивидуальный подбор средств ухода.`;
	...
	if (
		...
		const treatmentDescription = isPlannedExtraction
			? `Инфильтрационная и проводниковая местная анестезия амидного ряда (артикаин 4% с эпинефрином 1:100 000 1.7 мл) (infiltration anesthesia). Синдесмотомия — отслоение круговой связки зуба на глубину 3-5 мм распатором. Наложение анатомических щипцов / прямого или углового элеватора, люксация, элевация, аккуратная тракция зуба из альвеолы без повреждения костных стенок (elevator/forceps). Тщательный ревизионный кюретаж лунки острой кюретажной ложкой, удаление грануляций и костных отломков (socket curettage). Гемостаз: формирование устойчивого кровяного сгустка, гемостатическая коллагеновая губка с антисептиком (hemostasis). Сближение краев лунки, наложение узловых швов (биодеградируемый шовный материал). Давящий марлевый тампон на 20 минут.`
	...
	if (stateNorm === "filled") {
		...
		return {
			...
			treatmentDescription: `Зуб ${tooth}: Снятие несостоятельной пломбы, полная некрэктомия вторичного кариеса, изоляция коффердамом, медикаментозная обработка 2% хлоргексидином, адгезивный протокол (etching + primer/bond), послойное моделирование (композитный светоотверждаемый материал), шлифовка, полировка до зеркального блеска, окклюзионный контроль.`,
			recommendations:
	...
	if (stateNorm === "crown") {
		...
		return {
			...
			treatmentDescription: `Зуб ${tooth}: Местная анестезия амидного ряда (артикаин 4% 1.7 мл). Препарирование культи зуба под искусственную коронку с созданием циркулярного уступа Chamfer. Ретракция десны ретракционной нитью. Получение прецизионного двухслойного силиконового оттиска (А-силиконовый оттискной материал) и оттиска антагонистов. Изготовление и фиксация провизорной пластмассовой коронки на безэвгенольный временный цемент.`,
			recommendations:
...
export const CLINICAL_FAST_PRESETS: readonly FastClinicalPreset[] = [
	{
		...
		statusLocalis:
			"При осмотре: кариозная полость средней глубины в пределах дентина. Зондирование слабоболезненно по эмалево-дентинной границе, дно и стенки плотные, пигментированные. Перкуссия безболезненна. Холодовая проба слабоположительная, быстропроходящая. Электровозбудимость пульпы в пределах нормы.",
		treatmentDescription:
			"Инфильтрационная/проводниковая местная анестезия амидного ряда (артикаин 4% 1.7 мл). Препарирование кариозной полости, полная некрэктомия, формирование эмалевого фальца. Изоляция рабочего поля коффердамом. Медикаментозная обработка 2% раствором хлоргексидина биглюконата. Кислотное травление эмали и дентина 37% ортофосфорной кислотой (etching: эмаль 20 сек, дентин 10 сек), смывание водой, деликатное подсушивание воздухом без пересушивания. Нанесение адгезивной системы (adhesive: праймер + бонд), экспозиция 20 сек, раздувание, фотополимеризация 20 сек. Послойное моделирование (композитный светоотверждаемый материал) с восстановлением анатомической формы бугров, фиссур и контактного пункта. Окклюзионная коррекция по копирке, шлифовка и полировка (polishing: диски, полиры, полировочная паста) до сухого зеркального блеска.\n\nГарантийные обязательства:\n- Гарантийный срок на световую композитную реставрацию: 24 мес. (срок службы: 36 мес.).\n- Условия сохранения гарантии: соблюдение индивидуальной гигиены полости рта, контрольный профилактический осмотр не реже 1 раза в 6 месяцев.",
		recommendations:
	...
	},
	{
		...
		badge: "K04.0",
		description:
			"Анестезия, экстирпация пульпы, мех/мед обработка каналов, временный препарат гидроксида кальция / обтурация гуттаперчей.",
		defaultIcd10: "K04.0",
		anamnesis:
		...
		statusLocalis:
			"Глубокая кариозная полость, сообщающаяся с полостью зуба. Зондирование вскрытой точки рога пульпы резко болезненно, сопровождается кровоточивостью. Перкуссия слабочувствительна. Термопроба резко положительна с длительным последействием. Электровозбудимость пульпы снижена (соответствует воспалению коронковой пульпы). Рентгенологически: глубокий дефект твердых тканей, периодонтальная щель без деструктивных изменений.",
		treatmentDescription:
			"Инфильтрационная/проводниковая местная анестезия амидного ряда (артикаин 4% с эпинефрином 1:100 000 / 1:200 000, 1.7 мл). Препарирование, раскрытие полости зуба, создание прямого эндодонтического доступа. Изоляция коффердамом. Витальная экстирпация пульпы из корневых каналов / девитализация. Определение рабочей длины корневых каналов апекслокатором и контрольной рентгенографией. Механическая инструментальная обработка каналов NiTi ротационными файлами (canal instrumentation) с обильной ирригацией 3% гипохлоритом натрия (NaOCl) и 17% ЭДТА с ультразвуковой активацией. Высушивание стерильными бумажными штифтами. Временный препарат на основе гидроксида кальция под герметичный временный пломбировочный материал / трехмерная обтурация каналов гуттаперчей с эпоксидным силером (gutta-percha obturation) методом латеральной/вертикальной конденсации. Рентген-контроль обтурации. Восстановление коронковой части зуба (композитный светоотверждаемый материал).",
		recommendations:
			"При болях — НПВС (нимесулид 100 мг / ибупрофен 400 мг) по 1 таб. после еды. Не жевать на причинную сторону 2-3 дня. Контрольный осмотр через 7-14 дней.",
	},
	{
		...
		badge: "K04.5",
		description:
			"Ревизия, распломбировка, антисептическая обработка, временный препарат гидроксида кальция.",
		defaultIcd10: "K04.5",
		anamnesis:
			"Жалобы на чувство тяжести или ноющую боль при накусывании на зуб, изменение цвета коронки. В анамнезе зуб ранее лечен.",
		statusLocalis:
			"Зуб девитализирован, серый оттенок коронки / дефект пломбы. Полость зуба вскрыта, зондирование устьев каналов безболезненно. Перкуссия слабочувствительна. Пальпация по переходной складке безболезненна. Электровозбудимость пульпы отсутствует (пульпа некротизирована). Рентгенограмма: очаг деструкции костной ткани в периапикальной области у верхушки корня (периапикальный очаг).",
		treatmentDescription:
			"Инфильтрационная/проводниковая местная анестезия амидного ряда (артикаин 4% с эпинефрином 1:100 000 / 1:200 000, 1.7 мл). Раскрытие полости зуба, удаление старого пломбировочного материала, распломбировка и ревизия корневых каналов (canal desobturation). Прохождение каналов до апекса под контролем апекслокатора. Механическая и медикаментозная антисептическая обработка (antiseptic irrigation: NaOCl 3%, 2% хлоргексидин, ЭДТА 17%, УЗ-активация). Временная обтурация корневых каналов препаратом на основе гидроксида кальция для антисептического воздействия и стимуляции остеогенеза. Наложение герметичного временного пломбировочного материала. Контрольный визит через 10–14 дней.",
		recommendations:
			"Щадящая диета, НПВС при боли (нимесулид 100 мг), ротовые ванночки с 0.05% хлоргексидином. Явка на контрольный прием через 10-14 дней.",
	},
	{
		...
		treatmentDescription:
			"Инфильтрационная и проводниковая местная анестезия амидного ряда (артикаин 4% 1.7 мл) (infiltration anesthesia). Синдесмотомия — отслоение круговой связки зуба на глубину 3-5 мм распатором. Наложение анатомических щипцов / прямого или углового элеватора, люксация, элевация, аккуратная тракция зуба из альвеолы без повреждения костных стенок (elevator/forceps). Тщательный ревизионный кюретаж лунки острой кюретажной ложкой, удаление грануляций и костных отломков (socket curettage). Гемостаз: формирование устойчивого кровяного сгустка, гемостатическая коллагеновая губка с антисептиком (hemostasis). Сближение краев лунки, наложение узловых швов (биодеградируемый шовный материал). Давящий марлевый тампон на 20 минут.",
		recommendations:
			"1. Холод на область щеки по 15 мин первые 3-4 часа.\n2. Не полоскать рот активно (сохранять сгусток).\n3. Исключить бани, сауны и спорт на 3 дня.\n4. Щадящая диета.\n5. НПВС при боли (нимесулид 100 мг).\n6. Контрольный осмотр через 7-10 дней.",
	},
	{
		...
		badge: "Z01.2",
		description:
			"Ультразвуковой скейлинг, Air-Flow, полировка пастой, глубокое фторирование эмали.",
		defaultIcd10: "Z01.2",
		anamnesis:
	...
	},
	{
		...
		label: "Коронка из диоксида циркония на имплантате (винтовая фиксация)",
		badge: "K08.1",
		description:
			"Снятие формирователя, фиксация индивидуальной анатомической коронки из диоксида циркония с шахтой, торк 25–30 Нсм, герметизация шахты композитом.",
		defaultIcd10: "K08.1",
		anamnesis:
			"Явка на этап ортопедического лечения после успешной остеоинтеграции дентального имплантата. Жалоб нет.",
		statusLocalis:
			"Слизистая оболочка в области ранее установленного имплантата бледно-розового цвета, плотная, признаки периимплантита и мукозита отсутствуют. Формирователь десны состоятелен, десневая манжета сформирована. На рентгенограмме: костная ткань вокруг витков имплантата интактна, резорбция отсутствует.",
		treatmentDescription:
			"Антисептическая обработка полости рта (хлоргексидин 0.05%). Демонтаж формирователя десны, ирригация шахты имплантата. Примерка индивидуальной анатомической коронки из диоксида циркония (Zirconia) на титановом основании (Ti-base) с винтовой фиксацией. Проверка окклюзионных и апроксимальных контактов, краевого прилегания к платформе имплантата. Затяжка клинического фиксирующего винта динамометрическим ключом с рекомендованным усилием (25-30 Нсм). Рентген-контроль посадки на платформу. Изоляция шахты винта тефлоновой лентой, герметизация шахты светоотверждаемым композитным материалом под цвет эмали. Финишная полировка окклюзионной поверхности резиновыми головками и пастой Cleanic.",
		recommendations:
			"Контрольный осмотр через 7 дней. Контроль окклюзии и гигиены каждые 6 месяцев. Использование суперфлосса и ирригатора для межзубных промежутков в области имплантата.",
	},
	{
		id: "enamel_wear_erosion",
		label: "Клиновидный дефект / Патологическая стираемость",
		badge: "K03.1",
		description:
			"Препарирование эмалевых краев, адгезивная подготовка, пломбирование микрогибридным светоотверждаемым композитом с повышенной эластичностью.",
		defaultIcd10: "K03.1",
		anamnesis:
			"Жалобы на дефект твердых тканей у шейки зуба, гиперестезию от температурных и механических раздражителей (чистка зубов, холодная вода).",
		statusLocalis:
			"В пришеечной области вестибулярной поверхности зуба V-образный дефект твердых тканей с гладкими плотными стенками (клиновидный дефект). Зондирование дна дефекта слабоболезненно. Десна у шейки без признаков воспаления.",
		treatmentDescription:
			"Аппликационная / инфильтрационная анестезия (артикаин 4% 1.7 мл). Деликатное освежение эмалевых краев дефекта мелкозернистым бором, дезинфекция 2% хлоргексидином. Наложение ретракционной нити UltraPak #000 для защиты десневого края. Тотальное/самопротравливание (адгезив 7-го поколения / универсальный адгезив). Послойное пломбирование эластичным микрогибридным светоотверждаемым композитом (flowable/packable composite) с высоким модулем упругости. Окклюзионная проверка, удаление ретракционной нити, шлифовка и полировка поверхности силиконовыми головками до зеркального блеска. Нанесение десенситайзера / фторлака.",
		recommendations:
			"Использование мягкой зубной щетки (Soft/Ultrasoft), чистка без горизонтальных пилящих движений. Зубная паста для чувствительных зубов с гидроксиапатитом/кальцием.",
	},
];

export interface PerioPathologyPreset {
	readonly id: string;
	readonly label: string;
	readonly badge: string;
	readonly defaultIcd10: string;
...
/** Быстрые клинические пресеты патологий пародонта для формы 043/у */
export const PERIO_PATHOLOGY_PRESETS: readonly PerioPathologyPreset[] = [
	{
		id: "perio_intact",
		label: "Интактный пародонт / Клиническая норма (Z01.2)",
		badge: "Норма",
		defaultIcd10: "Z01.2",
		statusLocalis:
			"Пародонт в норме: глубина бороздки 1–2 мм, кровоточивость при зондировании отсутствует (BOP 0%), патологической подвижности нет. Десна бледно-розовая, плотная, плотно охватывает шейки зубов. Зубодесневое прикрепление сохранено на всем протяжении. Зубные отложения не определяются.",
		treatmentDescription:
			"Санация полости рта. Профилактический осмотр через 6 месяцев. Рекомендации по поддержанию индивидуальной гигиены полости рта.",
	},
	{
		id: "hygiene_airflow_ultrasound",
		label: "Профгигиена: УЗ + Air Flow (A16.07.051)",
		badge: "A16.07.051",
		defaultIcd10: "Z01.2",
		statusLocalis:
		...
		treatmentDescription:
			"Комплексная профессиональная гигиена полости рта (A16.07.051): ультразвуковое удаление зубных отложений (скейлинг), обработка аппаратом Air-Flow мелкодисперсным порошком глицина, полировка поверхностей зубов пастой Cleanic и щеточками. Антисептическая обработка 0.05% хлоргексидином. Глубокое фторирование эмали фторлаком Clinpro White Varnish. Даны индивидуальные рекомендации по гигиене и «белой диете» на 48 часов.",
	},
	{
		id: "gingivitis_catarrhal",
		label: "Хронический катаральный гингивит (K05.1)",
		badge: "K05.1",
		defaultIcd10: "K05.1",
		statusLocalis:
	...
	{
		id: "gingivitis_localized",
		label: "Гингивит локализованный (K05.1)",
		badge: "K05.1",
		defaultIcd10: "K05.1",
		statusLocalis:
			"Десна в области отдельных зубов (преимущественно фронтального отдела) гиперемирована, отечна, цианотична, кровоточит при зондировании (BOP+). Зубодесневое прикрепление сохранено, патологических пародонтальных карманов нет (глубина бороздки ≤ 2.5 мм). Мягкий зубной налет, локальные наддесневые зубные отложения.",
		treatmentDescription:
			"Профессиональная гигиена полости рта (ультразвуковое снятие отложений + Air-Flow). Антисептическая обработка (хлоргексидин 0.05%). Аппликация противовоспалительного геля (Метрогил Дента / Холисал). Индивидуальный подбор средств гигиены.",
	},
	{
		id: "gingivitis_generalized",
		label: "Гингивит генерализованный (K05.1)",
		badge: "K05.1",
		defaultIcd10: "K05.1",
		statusLocalis:
			"Диффузная гиперемия, пастозность и отек межзубных сосочков и маргинального края десны обеих челюстей. Выраженная кровоточивость при зондировании (BOP > 30%). Зубодесневое прикрепление сохранено, костные карманы отсутствуют. Обильные над- и поддесневые зубные отложения, налет курильщика.",
		treatmentDescription:
			"Комплексная профессиональная гигиена полости рта: ультразвуковой скейлинг, полировка пастами Cleanic. Местная противовоспалительная терапия. Назначение ротовых ванночек с антисептиками на 7 дней. Обучение стандартному методу чистки зубов.",
	},
	{
		id: "dental_calculus",
		label: "Зубные отложения (над- и поддесневые) (K03.6)",
		badge: "K03.6",
		defaultIcd10: "K03.6",
		statusLocalis:
			"Обнаружены массивные минерализованные зубные отложения: наддесневой зубной камень на язычных поверхностях нижних резцов и щечных поверхностях моляров, пигментированный плотный поддесневой камень. Умеренная гиперемия маргинальной десны, краевое кровотечение при зондировании.",
		treatmentDescription:
			"Профессиональная ультразвуковая чистка зубов (A16.07.051). Снятие пигментированного налета Air-Flow с порошком на основе глицина. Полировка межзубных промежутков штрипсами и полировочными головками. Реминерализующая терапия фторлаком.",
	},
	{
		id: "periodontitis_mild",
		label: "Хронический пародонтит лёгкой степени (K05.3)",
		badge: "K05.3",
		defaultIcd10: "K05.3",
		statusLocalis:
			"Десна пастозна, синюшна, выраженная кровоточивость (BOP+). Пародонтальные карманы глубиной 3.5–4 мм. Над- и поддесневые зубные отложения. Патологическая подвижность зубов: отсутствует или не более I ст. во фронтальном отделе. На рентгенограмме: деструкция кортикальной пластинки вершин межальвеолярных перегородок до 1/3 длины корня.",
		treatmentDescription:
			"Ультразвуковое снятие над- и поддесневого камня, полировка корней (SRP A16.07.051). Орошение карманов антисептиками. Пародонтальная лечебная повязка. Назначение антисептических ротовых ванночек.",
	},
	{
		id: "periodontitis_moderate",
		label: "Хронический пародонтит средней степени (K05.3)",
		badge: "K05.3",
		defaultIcd10: "K05.3",
		statusLocalis:
			"Десна синюшно-застойная, выраженная кровоточивость при пальпации и зондировании. Пародонтальные карманы глубиной 4–5.5 мм с серозно-гнойным экссудатом. Ретракция десны 1-2 мм. Поддесневой зубной камень. Патологическая подвижность I-II степени. Фуркационные дефекты I класса. На рентгенограмме: резорбция кости межальвеолярных перегородок от 1/3 до 1/2 длины корня.",
		treatmentDescription:
			"Комплексный закрытый кюретаж пародонтальных карманов (SRP A16.07.051). Медикаментозная обработка ферментами и антисептиками. Шинирование подвижных зубов стекловолоконной лентой (A16.07.019). Введение антибактериальных препаратов в карманы.",
	},
	{
		id: "periodontitis_severe",
		label: "Хронический пародонтит тяжёлой степени (K05.3)",
		badge: "K05.3",
		defaultIcd10: "K05.3",
		statusLocalis:
			"Глубокие пародонтальные карманы >6 мм с обильным гнойным отделяемым. Выраженная ретракция десны с обнажением корней. Патологическая подвижность II-III степени, веерообразное расхождение и смещение зубов. На рентгенограмме: диффузная деструкция костной ткани более 1/2 длины корней зубов.",
		treatmentDescription:
			"Неотложная антисептическая санация, эвакуация гнойного экссудата. Временное экстракоронарное шинирование. Системная антибиотикотерапия. Направление на хирургическое пародонтологическое лечение (лоскутная операция) или удаление безнадежных зубов.",
	},
	{
		id: "periodontal_abscess",
		label: "Пародонтальный абсцесс (K05.2)",
		badge: "K05.2",
		defaultIcd10: "K05.2",
		statusLocalis:
			"Острая боль при накусывании. В области десны гиперемированный, напряженный куполообразный отек с флюктуацией. Глубокий пародонтальный карман с гноетечением. Подвижность зуба II-III ст.",
		treatmentDescription:
			"Вскрытие и дренирование пародонтального абсцесса через десневой карман/разрез. Промывание антисептиками. Дренирование резиновым выпускником. Назначение анальгетиков и противовоспалительной терапии.",
	},
	{
		id: "gingival_recession",
		label: "Рецессия десны (K06.0)",
		badge: "K06.0",
		defaultIcd10: "K06.0",
		statusLocalis:
			"Апикальное смещение десневого края, обнажение корней зубов, эстетический дефект, гиперестезия твердых тканей. Зубодесневое прикрепление на обнаженных участках отсутствует.",
		treatmentDescription:
			"Реминерализирующая терапия шеек зубов, глубокое фторирование. Направление на пластику десны (закрытие рецессии соединительнотканным трансплантатом).",
	},
];
...
/**
 * Канонический 1-клик протокол анестезии нормы (Минздрав РФ / СтАР)
 * Используется клиницистом постфактум в 1 клик без таймеров и бюрократии.
 */
