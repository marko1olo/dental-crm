...
export function parseVisitDictationLocal(input: string): ParsedVisitData {
	...
	if (result.toothUpdates.length > 1) {
		...
		for (const update of result.toothUpdates)
			lastByTooth.set(update.code, update.state);
		result.toothUpdates = [...lastByTooth].map(([code, state]) => ({
			code,
			state,
		}));
	}
	...
	for (const token of tokens) {
       ...
       именно так. Добавлены и «выполн», «провед»: «проведена анестезия». */ else if (
			/^(лечени|план|сделан|сделал|выполн|провед)/.test(cleanToken)
		)
		...
		// Soft implicit triggers (only switch if no explicit section is active)
           ...
           лечения. */ else if (
					/(удал|экстракц|анестез|пломбир|коффердам|имплантац)/.test(cleanToken)
				)
					currentSection = "treatmentPlan";
			}
		}
	...
	if (result.emkUpdates.complaint) {
		...
		const clean = result.emkUpdates.complaint
			.replace(/^жал(?:обы|оба|уется|уются|уюсь)\s*(?:на)?\s*[:-]*\s*/i, "")
			.trim();
		if (clean)
