...
describe("parsePriceDictationLocal: цена", () => {
	it("разбирает цену, надиктованную словами", () => {
		assert.equal(
			parsePriceDictationLocal("лечение кариеса пять тысяч рублей терапия")
				.price,
			5000,
		);
		assert.equal(
			parsePriceDictationLocal("чистка две тысячи пятьсот рублей гигиена")
				.price,
			2500,
		);
		assert.equal(
			parsePriceDictationLocal("винир сто тысяч рублей").price,
			100000,
		);
		assert.equal(
			parsePriceDictationLocal("имплантация сорок тысяч рублей").price,
			40000,
		);
	});
	...
	it("разбирает цену в родительном падеже", () => {
		...
		assert.equal(
			parsePriceDictationLocal("имплантация сорока тысяч рублей").price,
			40000,
		);
		...
		assert.equal(
			parsePriceDictationLocal("удаление зуба тридцати тысяч").price,
			30000,
		);
	});
	...
	it("разбирает цифры с множителем словом", () => {
		...
		assert.equal(
			parsePriceDictationLocal("лечение кариеса 5 тысяч").price,
			5000,
		);
	});
	...
	it("разбирает цену цифрами", () => {
		assert.equal(
			parsePriceDictationLocal("лечение кариеса 5000 рублей").price,
			5000,
		);
		assert.equal(parsePriceDictationLocal("осмотр 500 руб").price, 500);
		assert.equal(
			parsePriceDictationLocal("осмотр стоимость 500 рублей").price,
			500,
		);
...
describe("parsePriceDictationLocal: название и категория", () => {
	it("название услуги не содержит ни цены, ни слов-подсказок", () => {
		...
		assert.equal(
			parsePriceDictationLocal("лечение кариеса 5 тысяч").serviceName,
			"Лечение кариеса",
		);
		assert.equal(
			parsePriceDictationLocal("удаление зуба тридцати тысяч").serviceName,
			"Удаление зуба",
		);
		// БЫЛО: «Осмотр стоимость» — слово-подсказка оставалось в названии.
		assert.equal(
			parsePriceDictationLocal("осмотр стоимость 500 рублей").serviceName,
			"Осмотр",
		);
		assert.equal(
			parsePriceDictationLocal("консультация за 1500").serviceName,
			"Консультация",
		);
	});
	...
	it("в названии услуги не остаётся цифр", () => {
		...
		for (const input of inputs) {
			...
			assert.ok(
				!/\d/.test(serviceName),
				`в названии «${serviceName}» осталась цифра (вход «${input}»)`,
			);
		}
	});
	...
	it("определяет категорию по ключевому слову", () => {
		...
		assert.equal(
			parsePriceDictationLocal("коронка цена двадцать пять тысяч ортопедия")
				.category,
			"Ортопедия",
		);
		assert.equal(
			parsePriceDictationLocal("имплантация сорок тысяч рублей").category,
			"Имплантация",
		);
	});
