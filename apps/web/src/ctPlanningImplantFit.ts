import type {
	CtPlanningDistanceMeasurement,
	CtPlanningGeometrySummary,
} from "./utils/math/toothGeometry";
...
function rankedDistances(values: CtPlanningDistanceMeasurement[]) {
	return values
		.filter((item) => finiteMeasurement(item.valueMm))
		.slice()
		.sort((a, b) => a.valueMm - b.valueMm);
}

function legacyDistanceMeasurements(
	values: number[],
): CtPlanningDistanceMeasurement[] {
...
function siteScopedDistances(
	values: CtPlanningDistanceMeasurement[],
	siteToothCode: string | null,
) {
	if (!siteToothCode) return values;
	return values.filter(
		(item) => !item.toothCode || sameToothCode(item.toothCode, siteToothCode),
	);
}

function foreignSiteDistanceCount(
	values: CtPlanningDistanceMeasurement[],
	siteToothCode: string | null,
) {
...
function mixedUnscopedSite(
	...
	return (
		geometrySummary.siteEvidenceToothCodes.length > 1 ||
		new Set(values.map((item) => item.toothCode).filter(Boolean)).size > 1
	);
}

function hasSiteConfirmedMeasurement(
	measurement: CtPlanningDistanceMeasurement | null,
	siteToothCode: string | null,
) {
...
function roleDistance(
	values: CtPlanningDistanceMeasurement[],
	role: CtPlanningDistanceMeasurement["role"],
) {
...
function fallbackWidth(values: CtPlanningDistanceMeasurement[]) {
	return (
		rankedDistances(values.filter((item) => item.role !== "clearance"))[0] ??
		null
	);
}

function fallbackHeight(values: CtPlanningDistanceMeasurement[]) {
	const candidates = rankedDistances(
		values.filter((item) => item.role !== "clearance"),
	);
...
function fitScore(
	...
	if (canalMarginMm !== null)
		score += canalMarginMm >= 2 ? clamp((canalMarginMm - 2) * 8, 0, 10) : -55;
...
function candidateDecisionReasons(input: {
	...
	if (input.siteEvidenceStatus === "unscoped")
		reasons.push("участок импланта не подтвержден");
	if (input.siteEvidenceStatus === "mixed")
		reasons.push("измерения смешаны между зубами");
	if (input.foreignSiteEvidenceCount > 0)
		reasons.push("линейки другого зуба исключены");
	...
	else if (input.diameterMarginMm < 0)
		reasons.push(
			`диаметр шире гребня на ${round1(Math.abs(input.diameterMarginMm))} мм`,
		);
	if (input.lengthMarginMm === null) reasons.push("нет линейки высоты");
	else if (input.lengthMarginMm < 0)
		reasons.push(
			`длина больше высоты на ${round1(Math.abs(input.lengthMarginMm))} мм`,
		);
	if (input.canalMarginMm === null) reasons.push("нет проверки ось + канал");
	else if (input.canalMarginMm < 2)
		reasons.push(`до канала ${input.canalMarginMm} мм, нужно 2 мм`);
	if (reasons.length === 0)
		reasons.push("запасы проходят; подтвердить в просмотрщике");
...
function candidateStatus(input: {
	...
	if (input.diameterMarginMm !== null && input.diameterMarginMm < 0)
		return "blocked";
	if (input.lengthMarginMm !== null && input.lengthMarginMm < 0)
		return "blocked";
	if (
...
export function buildCtPlanningImplantFitPlan(input: {
	...
	const allDistances = rankedDistances(
		input.geometrySummary.distanceMeasurements.length > 0
			...
			: legacyDistanceMeasurements(
					input.geometrySummary.distanceMeasurementsMm,
				),
	);
	const siteToothCode = input.geometrySummary.implantSiteToothCode;
	const foreignSiteEvidenceCount = foreignSiteDistanceCount(
		allDistances,
		siteToothCode,
	);
	const siteEvidenceStatus: CtPlanningImplantFitPlan["siteEvidenceStatus"] =
		siteToothCode
			? "scoped"
			: mixedUnscopedSite(allDistances, siteToothCode, input.geometrySummary)
				? "mixed"
				: "unscoped";
	const siteMatchedDistances = siteToothCode
		? rankedDistances(
				allDistances.filter((item) =>
					sameToothCode(item.toothCode, siteToothCode),
				),
			)
		: allDistances;
	const unscopedSiteDistances = siteToothCode
		? rankedDistances(allDistances.filter((item) => !item.toothCode))
		: [];
	const distances = rankedDistances(
		siteScopedDistances(allDistances, siteToothCode),
	);
	const typedWidth =
		roleDistance(siteMatchedDistances, "ridge_width") ??
		roleDistance(
			siteToothCode ? unscopedSiteDistances : distances,
			"ridge_width",
		);
	const typedHeight =
		roleDistance(siteMatchedDistances, "bone_height") ??
		roleDistance(
			siteToothCode ? unscopedSiteDistances : distances,
			"bone_height",
		);
	const widthMeasurement =
		typedWidth ??
		fallbackWidth(siteMatchedDistances) ??
		fallbackWidth(siteToothCode ? unscopedSiteDistances : distances);
	const heightMeasurement =
		typedHeight ??
		fallbackHeight(siteMatchedDistances) ??
		fallbackHeight(siteToothCode ? unscopedSiteDistances : distances);
	...
	const hasTypedHeight = hasSiteConfirmedMeasurement(
		typedHeight,
		siteToothCode,
	);
	const canalMarginMm = input.geometrySummary.minimumClearanceMm;
	const measurementRoleCount = distances.filter(
		(item) => item.role === "ridge_width" || item.role === "bone_height",
	).length;
	const candidates = input.implantLibrary
		.map((implant) => {
			const diameterMarginMm =
				ridgeWidthMm === null
					? null
					: round1(ridgeWidthMm - implant.diameterMm - 2);
			const lengthMarginMm =
				boneHeightMm === null
					? null
					: round1(boneHeightMm - implant.lengthMm - 2);
	...
	if (siteEvidenceStatus === "unscoped")
		warnings.push(
			"Участок импланта не подтвержден; скрининг остается черновиком.",
		);
	if (siteEvidenceStatus === "mixed")
		warnings.push(
			"Измерения относятся к нескольким зубам; скрининг остается черновиком.",
		);
	if (foreignSiteEvidenceCount > 0)
		warnings.push(
			"Линейки другого зуба не участвуют в ранжировании типоразмеров.",
		);
	if (!input.canPlan) warnings.push("Нужна готовая объемная КТ-серия.");
	if (ridgeWidthMm === null || !hasTypedWidth)
		warnings.push("Нет подписанной линейки ширины гребня.");
	if (boneHeightMm === null || !hasTypedHeight)
		warnings.push("Нет подписанной линейки высоты кости.");
	if (distances.length > 0 && (!hasTypedWidth || !hasTypedHeight))
		warnings.push(
			"Универсальные короткая/длинная линейки показаны только как черновик; для готовности нужны роли линейки.",
		);
	if (canalMarginMm !== null && canalMarginMm < 2)
		warnings.push(
			"Отступ до канала меньше 2 мм: типоразмер заблокирован до перестройки плана.",
		);
	const summaryLabel =
		status === "ready"
	...
	return {
		...
		widthSource:
			ridgeWidthMm === null ? "missing" : hasTypedWidth ? "typed" : "fallback",
		heightSource:
			boneHeightMm === null ? "missing" : hasTypedHeight ? "typed" : "fallback",
		summaryLabel,
