		...
		>
			...
			>
				...
				{settingsTab === "sources" && typedDicomViewerToolStateBundle ? (
					<section
						className="dicom-toolstate-result"
						aria-label="Состояние инструментов КТ-просмотрщика"
					>
						<div>
							<strong>
								Состояние просмотрщика · окон{" "}
								{typedDicomViewerToolStateBundle.viewports.length}
							</strong>
							<span>
								заметок {typedDicomViewerToolStateBundle.annotations.length} ·
								инструментов {typedDicomViewerToolStateBundle.tools.length}
							</span>
						</div>
						<button
							className="secondary-button"
							type="button"
							onClick={downloadDicomViewerToolStateBundle}
						...
						<p>
							{humanizeMigrationText(
								typedDicomViewerToolStateBundle.nextAction,
							)}
						</p>
						<small>
							{humanizeMigrationText(
								typedDicomViewerToolStateBundle.exportHints[0],
							)}
						</small>
						{typedDicomViewerToolStateBundle.warnings
							.slice(0, 3)
							.map((warning) => (
					...
					>
						<div className="import-copy">
							...
							<div>
								...
								<h2>
									Диктовка, фото журнала и снимки становятся черновиками с
									проверкой
								</h2>
								<p>
									Здесь проверяется контур обработки будущих нейронок: вход,
									уверенность, предупреждения и следующий шаг. Ничего не
						...
						>
							<div className="speech-provider-head">
								<div>
									...
									<h3>
										Голос: быстрый черновик сейчас, усиленное распознавание
										после подключения
									</h3>
								</div>
							...
							{speechGatewayStatus ? (
								<div className="speech-gateway-strip">
									...
									<span>
										отрезок записи{" "}
										{Math.round(
											speechGatewayStatus.chunkingPolicy.minChunkMs / 1000,
										)}
										-
										{Math.round(
											speechGatewayStatus.chunkingPolicy.maxChunkMs / 1000,
										)}{" "}
										сек. · фрагмент до{" "}
										{Math.round(
											speechGatewayStatus.maxChunkBytes / 1024 / 1024,
										)}{" "}
										МБ
									</span>
									<span>
									...
									<small className="speech-gateway-hint">
										...
										{speechGatewayStatus.keyPool.rotationEnabled
											...
											: ""}{" "}
										· попыток{" "}
										{speechGatewayStatus.keyPool.maxAttemptsPerProvider} · ответ
										до{" "}
										{Math.round(speechGatewayStatus.keyPool.timeoutMs / 1000)}{" "}
										сек. · словарь {speechGatewayStatus.promptPolicy.termCount}{" "}
										терминов · текст до{" "}
										{speechGatewayStatus.promptPolicy.maxChars} симв.{" "}
							...
							{speechGatewayHealthReport ? (
								...
								>
									...
									<span>
										таймаут{" "}
										{Math.round(speechGatewayHealthReport.timeoutMs / 1000)}{" "}
										сек.
									</span>
									<span>
							...
							{speechRecordingStrategy ? (
								<div className="speech-strategy-strip">
									...
									<span>
										фрагмент{" "}
										{Math.round(speechRecordingStrategy.chunkMs / 1000)} сек.
									...
									{typedSpeechRecordingRecovery.recordings
										...
										.map((recording) => (
											...
											>
												...
												<p>
													{recording.transcriptPreview || recording.nextAction}
												</p>
												<small>
												...
												{health ? (
													...
													>
														{speechProviderHealthLabels[health.healthLevel] ??
															health.healthLevel}{" "}
														· можно в приеме{" "}
														{health.safeToUseInVisit ? "да" : "нет"}
