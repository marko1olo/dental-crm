...
export function GuestLabPortal() {
  ...
  const getStatusIcon = (status: string) => {
    switch(status) {
      ...
      case 'refitting': return <RefreshCcw className="w-5 h-5 text-orange-500" />;
  ...
  const getStatusBadgeClass = (status: string) => {
    switch(status) {
      case 'in_progress': return "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800";
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 shadow-xl rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <div className="p-6 sm:p-8 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className={`px-4 py-2 rounded-full border flex items-center gap-2 font-medium ${getStatusBadgeClass(order.status)}`}>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-slate-900 dark:text-white">{order.material || "—"}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 dark:text-slate-400">Цвет (Vita)</span>
                    <span className="font-medium text-slate-900 dark:text-white">{order.colorVita || "—"}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-yellow-50/50 dark:bg-yellow-900/10 rounded-xl p-4 text-slate-700 dark:text-slate-300 border border-yellow-100 dark:border-yellow-900/30 min-h-[100px]">
                  {order.clinicalNotes ? (
                    <p className="whitespace-pre-wrap text-sm leading-relaxed">{order.clinicalNotes}</p>
                  ) : (
                    <p className="text-slate-400 italic text-sm">Врач не оставил дополнительных комментариев.</p>
                  )}
                </div>
              </div>
            </div>
