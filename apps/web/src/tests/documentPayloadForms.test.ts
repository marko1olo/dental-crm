...
const extractedForms = [
	{ name: "AnesthesiaConsentLogForm", file: "components/documents/forms/AnesthesiaConsentLogForm.tsx" },
	{ name: "InformedConsentForm", file: "components/documents/forms/InformedConsentForm.tsx" },
	{ name: "MedicalInterventionRefusalForm", file: "components/documents/forms/MedicalInterventionRefusalForm.tsx" },
	{ name: "PersonalDataProcessingConsentForm", file: "components/documents/forms/PersonalDataProcessingConsentForm.tsx" },
	{ name: "PhotoVideoConsentForm", file: "components/documents/forms/PhotoVideoConsentForm.tsx" },
	{ name: "ProcedureSpecificConsentForm", file: "components/documents/forms/ProcedureSpecificConsentForm.tsx" },
	{ name: "TaxDeductionApplicationForm", file: "components/documents/forms/TaxDeductionApplicationForm.tsx" },
] as const;
...
describe("дописывание формулировки кнопкой-подсказкой", () => {
	...
	it("пробелы и переводы строк по краям не дают «текст , формулировка»", () => {
		assert.equal(appendChipToText("  Отек десны  ", "Гнойные выделения"), "Отек десны, гнойные выделения");
		assert.equal(appendChipToText("\n\t", "Наблюдение"), "Наблюдение");
	});

	it("три нажатия подряд собирают перечисление, а не затирают предыдущее", () => {
		let value = "";
		for (const chip of ["Страх перед процедурой", "Нехватка времени", "Финансовые причины"]) {
			value = appendChipToText(value, chip);
		}
		assert.equal(value, "Страх перед процедурой, нехватка времени, финансовые причины");
	});
});
...
describe("вынесенные формы подключены, а не лежат мёртвым файлом", () => {
	for (const form of extractedForms) {
		it(`${form.name}: есть определение и есть применение в DocumentsView.tsx`, () => {
			...
			assert.match(
				documentsView,
				new RegExp(`import \\{ ${form.name} \\} from "\\./components/documents/forms/${form.name}"`),
	...
	it("общая карточка и ряд кнопок-подсказок тоже подключены", () => {
		...
		assert.equal(users.length, 6, `общую карточку используют ${users.length} форм из 6 ожидаемых`);

		const refusal = read("components/documents/forms/MedicalInterventionRefusalForm.tsx");
		assert.match(refusal, /import \{ QuickChipsRow \} from "\.\.\/QuickChipsRow"/);
		assert.equal(
			refusal.match(/<QuickChipsRow /g)?.length,
			4,
			"в форме отказа было четыре ряда кнопок-подсказок",
		);
		assert.match(refusal, /import \{ appendChipToText \} from "\.\.\/documentChipText"/);
	});
});
...
describe("формы рисуются и сохранили текст, который видел врач", () => {
	it("информированное согласие: заголовок, поля и три галочки", () => {
		...
		assert.ok(html.includes("Пациенту объяснено право отказаться до вмешательства"));
	...
	it("процедурное согласие: список блоков процедур настоящий, редактор строк зубов вызван", () => {
		const html = renderForm(
			createElement(ProcedureSpecificConsentForm, {
				...
				renderToothRowsEditor: () => createElement("div", null, "строки зубов приёма"),
			}),
		);
		...
		for (const option of procedureSpecificConsentProcedureOptions) {
			assert.ok(html.includes(option.label), `в списке блоков процедур нет «${option.label}»`);
		}
		assert.ok(html.includes("строки зубов приёма"), "редактор строк зубов не отрисован");
		assert.ok(html.includes("Персональные факторы риска пациента"));
		assert.ok(html.includes("Опрошен, значимых факторов нет"), "кнопка отрицательного ответа пропала");
	});

	it("журнал анестезии: подпись зоны берёт зону лечения приёма", () => {
		const html = renderForm(createElement(AnesthesiaConsentLogForm, { inferredTreatmentArea: "зуб 46" }));
		...
		assert.ok(html.includes("Аллергии, лекарства и ограничения проверены до введения"));
	});
	...
	it("фото и видео: каждый вид материала показан отдельной галочкой", () => {
		...
		for (const option of photoVideoMaterialOptions) {
			assert.ok(html.includes(option.label), `нет галочки материала «${option.label}»`);
		}
	...
	it("согласие на ПДн: оператор, ИНН и адрес только показываются", () => {
		const html = renderForm(createElement(PersonalDataProcessingConsentForm, { clinicProfileDraft }));
		...
		assert.equal(html.match(/readOnly=""/g)?.length, 3, "поля оператора перестали быть только для чтения");
	...
	it("заявление на налоговую справку: справочники настоящие, а не подставные", () => {
		...
		for (const option of [...taxApplicationRelationshipOptions, ...taxApplicationFormOptions, ...taxApplicationDeliveryChannelOptions]) {
			assert.ok(html.includes(option.label), `в заявлении нет варианта «${option.label}»`);
		}
		assert.ok(html.includes("Перед выдачей будет проверен дубль по тем же расходам"));
	});
	...
	it("отказ от вмешательства: четыре ряда готовых формулировок целиком", () => {
		...
		assert.equal(html.match(/quick-chip quick-chip--sm/g)?.length, 16, "кнопок-подсказок должно остаться 16");
		assert.ok(html.includes("Пациенту объяснено, когда нужна экстренная помощь"));
	});
});
...
describe("оформление карточки не потерялось вместе с inline-стилями", () => {
	 ...
	 */
	const cssGuarantees = [
		{ selector: ".document-manual-override", properties: ["background", "padding", "border-radius", "border", "margin-top"] },
		{ selector: ".document-manual-override > summary", properties: ["cursor", "font-weight", "color", "user-select"] },
		{ selector: ".document-payload-collapsed-content", properties: ["margin-top", "display", "flex-direction", "gap"] },
	];

	for (const guarantee of cssGuarantees) {
		it(`${guarantee.selector}: свойства объявлены с !important`, () => {
			const start = redesignCss.indexOf(`\n${guarantee.selector} {`);
			assert.ok(start > 0, `в dente-redesign.css нет правила ${guarantee.selector}`);
	...
	it("общая карточка рисуется без атрибута style", () => {
		const html = renderForm(createElement(AnesthesiaConsentLogForm, { inferredTreatmentArea: "зуб 46" }));
		assert.ok(!/ style=/.test(html), "в разметке карточки снова появились inline-стили");
	...
	it("у формы отказа оболочка своя: наведение и фокус-обводка на месте", () => {
		...
		assert.ok(html.includes("hover:opacity-80"), "у сводки формы отказа пропало наведение");
		assert.ok(html.includes("focus:ring-2"), "у кнопок-подсказок пропала фокус-обводка");
