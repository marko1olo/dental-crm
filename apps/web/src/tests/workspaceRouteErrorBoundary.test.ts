...
function errorWithStack(): Error {
  const error = new Error("Cannot read properties of undefined (reading 'organizationId')");
  error.stack =
...
describe("вид сбоя раздела для сотрудника клиники", () => {
  ...
  test("в разработке стек виден целиком", () => {
    ...
    assert.match(presentation.diagnostics, /^\[Error\] Cannot read properties of undefined/);
    assert.match(presentation.diagnostics, /at PatientsView \(\/assets\/PatientsView-Bq3xK1\.js:1:24177\)/);
    assert.match(presentation.diagnostics, /renderWithHooks/);
  });

  test("сообщение об ошибке не подставляется в текст для человека", () => {
    const secretish = new Error("PGlite connect ECONNREFUSED 127.0.0.1:5432 user=dente_admin");
  ...
  test("сбой загрузки чанка распознаётся и в production", () => {
    const chunkFailure = new Error("Failed to fetch dynamically imported module: /assets/FinanceView.js");
  ...
  test("время сбоя выдаётся в обоих режимах и в русском формате", () => {
    ...
    assert.match(production.reference, /^\d{2}\.\d{2}\.\d{4}, \d{2}:\d{2}:\d{2}$/);
