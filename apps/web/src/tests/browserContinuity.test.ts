...
describe("inspectBrowserContinuity", () => {
	test("adds warning when navigator.storage.estimate throws an error", async () => {
		...
		try {
			...
			assert.ok(
				status.warnings.includes("Оценка хранилища браузера недоступна"),
			);
		} finally {
...
describe("formatMegabytes", () => {
	...
	test("formats numbers > 999 with thousand separators", () => {
		...
		assert.strictEqual(
			...
			`${(1000).toLocaleString("ru-RU")} МБ`,
		);
		assert.strictEqual(
			formatMegabytes(1234567),
			`${(1234567).toLocaleString("ru-RU")} МБ`,
		);
	});
	...
	test("formats decimal numbers", () => {
		...
		assert.strictEqual(
			...
			`${(1234.56).toLocaleString("ru-RU")} МБ`,
		);
	});
});
...
describe("formatByteSize", () => {
	...
	test("formats large values using ru-RU locale formatting", () => {
		...
		assert.strictEqual(
			...
			`${(1234.5).toLocaleString("ru-RU")} МБ`,
		);
		// 99999.9 MB -> 99 999,9 МБ
		assert.strictEqual(
			formatByteSize(99999.94 * 1024 * 1024),
			`${(99999.9).toLocaleString("ru-RU")} МБ`,
		);
	});
});
