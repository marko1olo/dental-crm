...
const knownUnwiredDocumentComponents: readonly string[] = [
	// Кнопка подписания УКЭП (КриптоПро). Путь на сервере рабочий:
	// apps/api/src/routes/documents/signUkep.ts объявляет POST /api/documents/:id/sign-ukep.
	// Но её не монтирует ни один экран, поэтому подписать документ УКЭП нельзя ничем.
	// Подключить её — это НОВАЯ возможность на экране, а не перестановка кода, поэтому
	// она остаётся здесь под именем, а не исчезает из вида.
