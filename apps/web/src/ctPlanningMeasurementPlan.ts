...
function countAnnotations(
	...
	return annotations.filter(
		(annotation) =>
			types.includes(annotation.type) && hasPoints(annotation, minimumPoints),
	).length;
}
...
function densityProtocolLabel(
	...
	if (hasMixedUnits)
		return "смешанные единицы: повторить probe в одной калибровке";
	if (unitLabel !== "HU")
		return "единицы просмотра: сверить калибровку перед сверлением";
...
function countSemanticDistances(
	...
	return annotations.filter(
		(annotation) =>
			...
			annotation.semanticRole === role &&
			hasPoints(annotation, 2),
	).length;
}
...
function statusFromCount(
	count: number,
	canPlan: boolean,
): CtPlanningMeasurementPlanStatus {
...
export function buildCtPlanningMeasurementPlan(input: {
	...
	const ridgeWidthCount = countSemanticDistances(
		input.annotations,
		"ridge_width",
	);
	const boneHeightCount = countSemanticDistances(
		input.annotations,
		"bone_height",
	);
	const clearanceRoleCount = countSemanticDistances(
		input.annotations,
		"clearance",
	);
	...
	const roiAreaTotalLabel = metricTotalLabel(
		input.geometrySummary.roiAreaTotalMm2,
		"мм²",
	);
	const roiVolumeTotalLabel = metricTotalLabel(
		input.geometrySummary.roiVolumeTotalMm3,
		"мм³",
	);
	const roiVolumeSlabMm = input.geometrySummary.roiVolumeSlabMm;
	const roiDraftCount = input.geometrySummary.roiDraftCount;
	const densityProbeCount = countAnnotations(
		input.annotations,
		["bone_density_probe"],
		1,
	);
	...
	const reviewCount = input.annotations.filter(
		(annotation) => annotation.points.length > 0 && annotation.needsReview,
	).length;
	const clearance = input.geometrySummary.minimumClearanceMm;
	const clearanceStatus: CtPlanningMeasurementPlanStatus = !input.canPlan
		? "blocked"
		: clearance === null
			? "draft"
			: clearance >= 2
	...
	const cards: CtPlanningMeasurementCard[] = [
		{
			...
			detail:
				"Высота, ширина гребня и углы считаются только после постановки точек.",
			nextAction:
				linearCount + angleCount > 0
					? "Проверить плоскость и калибровку."
					: "Поставить линейку или угол.",
		},
		{
			id: "signed-ridge-bone",
			title: "Ширина и высота",
			status: !input.canPlan
				? "blocked"
				: ridgeWidthCount > 0 && boneHeightCount > 0
					? "ready"
					: "draft",
			value: `${ridgeWidthCount}/${boneHeightCount}`,
			detail:
				"Для имплант-скрининга нужны подписанные линейки ширины и высоты.",
			nextAction:
				ridgeWidthCount > 0 && boneHeightCount > 0
					? "Сверить плоскость и калибровку."
					: "Создать линейки Ширина гребня и Высота кости.",
		},
		{
			id: "roi-area",
			...
			detail:
				"Контур нужен для окна синус-лифтинга, дефекта, кисты или графта.",
			nextAction:
				areaCount > 0
					? "Проверить замыкание контура и плоскость."
					: "Обвести контур минимум тремя точками.",
		},
		{
			id: "roi-volume",
			title: "Контур объема",
			...
			nextAction:
				volumeCount > 0
					? "Проверить слой и границы."
					: "Добавить объемный контур.",
		},
		{
			id: "density",
			title: "Плотность",
			...
			value:
				densityValueCount > 0
					? densityStats.rangeLabel
					: `${densityProbeCount} точ.`,
			detail:
				densityValueCount > 0
			...
			nextAction:
				densityValueCount > 0
					? "Сверить калибровку и протокол сверления."
					: "Поставить probe и сохранить значение.",
		},
		{
			id: "canal-clearance",
			...
			detail:
				"Минимальная дистанция считается по оси импланта и кривой нижнечелюстного канала.",
		...
		},
		{
			...
			status: !input.canPlan
				? "blocked"
				: input.unsavedArtifactCount > 0
					? "draft"
					: "ready",
			value:
				input.unsavedArtifactCount > 0
					? `${input.unsavedArtifactCount} черн.`
					: "сохранен",
			detail:
				"Локальные измерения нужно сохранить в пакет просмотра до передачи.",
			nextAction:
				input.unsavedArtifactCount > 0
	...
	if (linearCount > 0 && (ridgeWidthCount === 0 || boneHeightCount === 0))
		warnings.push(
			"Линейки есть, но ширина гребня и высота кости должны быть подписаны отдельными ролями.",
		);
	if (roiDraftCount > 0)
		warnings.push("Контур: меньше 3 точек; площадь/объем не считаются.");
	if (volumeCount > 0)
		warnings.push(`Объем по контуру: слой ${roiVolumeSlabMm} мм.`);
	if (!input.canPlan) warnings.push("Нужна готовая объемная КТ-серия.");
	if (densityProbeCount > 0 && densityValueCount === 0)
		warnings.push("Точки плотности есть, но значения еще не сохранены.");
	if (densityValueCount > 0 && !densityStats.unitIsCalibratedHu)
		warnings.push(
			"Плотность сохранена в единицах просмотра; это не HU-калибровка.",
		);
	if (densityStats.hasMixedUnits)
		warnings.push(
			"Плотность смешала единицы; повторите probe в одной калибровке.",
		);
	if (reviewCount > 0)
		warnings.push("Есть измерения с предупреждением по калибровке.");
	if (input.unsavedArtifactCount > 0)
		warnings.push("Есть локальные элементы вне пакета.");
	...
	const status: CtPlanningMeasurementPlanStatus = cards.some(
