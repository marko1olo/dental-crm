...
function buildReportReleaseGate(
	packet: CtPlanningExportPacket,
): CtPlanningReportReleaseGate {
...
function pixelPolicyLabel(value: string | undefined) {
	if (value === "external_volume_only")
		return "объем открывается только во внешнем или локальном просмотре";
	if (value === "metadata_only_no_pixels")
		return "только метаданные, без данных снимков";
	return "граница просмотра не задана";
}

function bridgeSummary(
	packet: CtPlanningExportPacket,
): CtPlanningReportBridgeSummary | null {
	const scenario = packet.activeScenarioSummary;
...
function linesForFacts(facts: CtPlanningExportPacket["clinicalFacts"]) {
	return facts.map(
		(fact) =>
			`- ${fact.title}: ${fact.value} [${statusLabel(fact.tone)}]. ${fact.detail}`,
	);
}
...
function linesForLanes(lanes: CtPlanningExportPacket["lanes"]) {
	return lanes.map(
		(lane) =>
			`- ${lane.title}: ${lane.value} [${statusLabel(lane.status)}]. ${lane.detail} Next: ${lane.nextAction}`,
	);
}

export function formatCtPlanningReportText(
	sidecar: CtPlanningReportSidecar,
): string {
	...
	return [
		...
		bridge
			? `- Command summary: ${bridge.commandSummary}`
			: "- No bridge command in packet.",
		bridge?.commandString ? `- Command string: ${bridge.commandString}` : "",
		bridge
			? `- Envelope: ${bridge.envelopeVersion ?? "not set"}; launch gate: ${bridge.launchGate ?? "not set"}; apply steps: ${bridge.applyStepCount}; missing targets: ${bridge.missingTargetCount}; can launch: ${bridge.canLaunch ? "yes" : "no"}`
			: "",
		...
		"",
		"Missing items",
		...(sidecar.missingArtifacts.length > 0
			? sidecar.missingArtifacts.map((item) => `- ${item}`)
			: ["- None"]),
		"",
		"Boundary",
...
export function buildCtPlanningReport(
	packet: CtPlanningExportPacket,
	generatedAtInput: Date | string = new Date(),
): CtPlanningReport {
	const generatedAt =
		typeof generatedAtInput === "string"
			? generatedAtInput
			: generatedAtInput.toISOString();
