...
const bootErrorCopy: Record<BootAudience, BootErrorCopy> = {
  clinic: {
    ...
    otherDetail: "Интерфейс остановлен до перезагрузки, чтобы не показывать неполное рабочее место.",
    actionLabel: "Обновить рабочее место",
  },
  public: {
    lead: "Страница не открылась.",
    chunkDetail: "Часть файлов страницы не загрузилась. Обычно помогает обновление, когда вернётся связь.",
    otherDetail: "Страница остановлена до обновления, чтобы не показывать неполную форму.",
    actionLabel: "Обновить страницу",
  },
};

export function bootErrorDetail(audience: BootAudience, error: unknown): string {
...
export class BootErrorBoundary extends Component<BootErrorBoundaryProps, BootErrorBoundaryState> {
  ...
  componentDidCatch(error: unknown, errorInfo: ErrorInfo) {
     ...
     */
    console.error(`DENTE boot failed (${this.props.audience})`, error, errorInfo.componentStack);
  }
    ...
    if (this.state.failed) {
      ...
      return (
        <main className="boot-state boot-state-error" role="alert" aria-live="assertive">
          ...
          <button type="button" onClick={() => requestBootRefresh(this.props.audience)}>
            {copy.actionLabel}
          </button>
        </main>
      );
