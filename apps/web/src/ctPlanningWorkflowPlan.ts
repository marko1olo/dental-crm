...
export type CtPlanningWorkflowPhaseOwner =
	| "series"
	| "doctor"
	| "implant"
	| "admin"
...
function selectedPhaseForQuickActionId(
...
function selectedScenarioFocus(
	packet: CtPlanningExportPacket,
): CtPlanningWorkflowScenarioFocus | null {
...
export function buildCtPlanningWorkflowPlan(input: {
	...
	const opgReady =
		input.reconstructionPlan.status === "ready" && input.hasPanoramicRoute;
	const measurementReady = input.measurementPlan.status === "ready";
	const implantReady =
		input.hasImplantPlan && input.implantModelPlan.status === "ready";
	const safetyReady =
		input.validationSummary.status === "pass" && input.hasCanalRoute;
	const handoffReady =
		input.exportPacket.status === "ready" && input.unsavedArtifactCount === 0;
	...
	const phases: CtPlanningWorkflowPhase[] = [
		{
			...
			detail:
				"Объемная серия открывает ОПТГ, поперечные срезы, контуры, плотность, ось и шаблон.",
			nextAction: input.canPlan
				? "Работать с планом."
				: "Выбрать готовую КЛКТ/КТ-серию.",
		},
		{
			id: "opg-route",
			owner: "doctor",
			...
			value:
				input.reconstructionPlan.crossSectionCount > 0
					? `${input.reconstructionPlan.crossSectionCount} срезов`
					: `${input.reconstructionPlan.curvePointCount} точек`,
			detail:
				"Панорамная дуга дает маршрут ОПТГ и поперечных срезов; тяжелые снимки остаются в просмотрщике.",
			nextAction: opgReady
				? "Проверить срезы вдоль дуги."
				: "Построить панорамную дугу минимум тремя точками.",
		},
		{
			id: "measurements",
			owner: "doctor",
			...
			detail:
				"Линейка, угол, контуры, объем, плотность и отступ до канала закрываются отдельной картой.",
			nextAction: measurementReady
				? "Сверить подпись и калибровку."
				: input.measurementPlan.summaryLabel,
		},
		{
			id: "implant-model",
			owner: "implant",
			...
			value: input.implantModelPlan.sleeveDiameterMm
				? `${input.implantModelPlan.sleeveDiameterMm} мм втулка`
				: input.hasImplantPlan
					? "размер выбран"
					: "нет размера",
			detail:
				"Типоразмер, ось, апекс, защитный контур и втулка должны быть рассчитаны до шаблона.",
			nextAction: implantReady
				? "Проверить апекс и втулку."
				: "Выбрать типоразмер и поставить ось импланта.",
		},
		{
			id: "safety",
			owner: "doctor",
			...
			detail:
				"Клинические проверки смотрят серию, ОПТГ, канал, отступ, контуры, модель и шаблон.",
			nextAction: safetyReady
				? "Клинические проверки закрыты."
				: input.validationSummary.label,
		},
		{
			id: "handoff",
			...
			value:
				input.unsavedArtifactCount > 0
					...
					: `${input.exportPacket.score}%`,
			detail:
				"Пакет переносит рабочее место, разметки, имплант, маршруты и блокеры без тяжелых снимков.",
			nextAction: handoffReady
				? "Сохранить пакет плана."
				: input.exportPacket.nextAction,
		},
	];
	...
	const score = Math.round(
		(phases.reduce((sum, phase) => sum + phaseScore(phase.status), 0) /
			phases.length) *
			100,
	);
	const selectedPhaseId = selectedPhaseForQuickActionId(
	...
	return {
		...
		warnings: uniqueWarnings([
			...
			selectedScenario && selectedScenario.status !== "ready"
				? `${selectedScenario.title}: ${selectedScenario.nextAction}`
				: "",
			input.blockedTasks > 0
				? `${input.blockedTasks} задач требуют действия`
				: "",
