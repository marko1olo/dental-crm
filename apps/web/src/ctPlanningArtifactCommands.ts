...
export const ctPlanningArtifactCommands: CtPlanningArtifactCommand[] = [
	{
		...
		title: "ОПТГ-дуга",
		detail:
			"Точки зубной дуги для панорамной реконструкции и поперечных срезов.",
	...
	},
	{
		...
		title: "Линейка",
		detail:
			"Высота кости, ширина гребня или отступ до канала на выбранном срезе.",
	...
	},
	{
		...
		title: "Ширина гребня",
		detail:
			"Отдельная линейка ширины гребня для первичного скрининга диаметра импланта.",
	...
	},
	{
		...
		title: "Высота кости",
		detail:
			"Отдельная линейка высоты кости для первичного скрининга длины импланта.",
	...
	},
	{
		...
		detail:
			"Отдельная контрольная линейка от импланта до нижнечелюстного канала; не заменяет трассу канала и ось.",
	...
	},
	{
		...
		title: "Угол",
		detail:
			"Контроль наклона оси импланта, кортикальной пластинки или ортопедической оси.",
		result: "градусы",
		projection: "oblique",
		unit: "deg",
		minimumPoints: 3,
	...
	},
	{
		...
		detail:
			"Контур дефекта, окна синус-лифтинга, графта или мягкотканной зоны.",
	...
	},
	{
		...
		title: "Ось импланта",
		detail:
			"Черновик оси выбранного типоразмера для отступов, втулки и шаблона.",
		result: "ось + размер",
		projection: "oblique",
		unit: "mm",
		minimumPoints: 2,
...
function annotationRole(
	annotation: CtPlanningArtifactAnnotationRef,
): ImagingViewerAnnotationSemanticRole {
	if (annotation.semanticRole) return annotation.semanticRole;
	const text =
		`${annotation.label ?? ""} ${annotation.note ?? ""}`.toLowerCase();
	...
	return ctPlanningArtifactCommands.map((command) => {
		...
		input.annotations.forEach((annotation, index) => {
			...
			const annotationKey = annotation.id
				...
				: `${annotation.type}:local-${index}`;
			if (seenAnnotationIds.has(annotationKey)) return;
			seenAnnotationIds.add(annotationKey);
			matchingAnnotations.push(annotation);
		});
		const completedCount = matchingAnnotations.filter(
			(annotation) => annotation.pointCount >= command.minimumPoints,
		).length;
		...
		const status: CtPlanningArtifactStatus =
			completedCount > 0
				? "ready"
				: volumeBlocked || implantBlocked
					? "blocked"
					: "draft";
				...
				completedCount > 0
					...
					: draftCount > 0
						...
						: status === "blocked"
							...
							: "готово к разметке",
			actionLabel:
				completedCount > 0 || draftCount > 0 ? "Добавить еще" : "Создать",
			blocker,
		};
	});
}
