...
function clearanceCheck(
	geometry: CtPlanningGeometrySummary,
	...
	return {
		...
		detail:
			clearance >= 2
				? "Отступ до канала рассчитан."
				: "Отступ до канала меньше 2 мм.",
	};
}
...
export function buildCtPlanningValidationSummary(input: {
	...
	const checks: CtPlanningValidationCheck[] = [
		{
			...
			detail:
				input.volumeBlockedTasks > 0
					? `${input.volumeBlockedTasks} объемных задач заблокированы.`
					: "КТ-задачи можно восстановить.",
		},
		{
			id: "opg",
			title: "ОПТГ",
			...
			detail: input.hasPanoramicRoute
				? "Панорамная реконструкция доступна."
				: "Нужно построить дугу и поперечные срезы.",
		},
		{
			id: "implant",
			title: "Имплант",
			...
			detail: input.hasImplantPlan
				? "Типоразмер есть в плане."
				: "Выберите библиотечный размер перед шаблоном.",
		},
		{
			id: "axis",
			title: "Ось",
		...
		},
		{
			...
			title: "Модель",
			status:
				input.implantModelPlan.status === "ready"
					? "pass"
					: input.implantModelPlan.status === "draft"
						? "warn"
						: "fail",
			value:
				input.implantModelPlan.sleeveDiameterMm === null ||
				input.implantModelPlan.sleeveLengthMm === null
					? "нет втулки"
					: `${input.implantModelPlan.sleeveDiameterMm} x ${input.implantModelPlan.sleeveLengthMm} мм`,
			detail:
				input.implantModelPlan.status === "ready"
					? "Имплант, ось, апекс и втулка рассчитаны."
					: "Нужны типоразмер и ось импланта.",
		},
		{
			id: "roi",
			title: "Контуры",
			status:
				input.geometrySummary.areaCount > 0 ||
				input.geometrySummary.volumeCount > 0
					? "pass"
					: "warn",
			value:
				input.geometrySummary.volumeCount > 0
					...
					: input.geometrySummary.areaCount > 0
						? input.measurementPlan.roiAreaTotalLabel
						: "нет",
			detail:
				input.geometrySummary.volumeCount > 0
					? `Объем по контуру — оценка по слою ${input.measurementPlan.roiVolumeSlabMm} мм, не сегментация тканей.`
					: "Площадь или объем нужны для дефекта, пазухи, графта и дыхательных путей.",
		},
		{
			id: "measurement-map",
			title: "Измерения",
			status:
				input.measurementPlan.status === "ready"
					? "pass"
					: input.measurementPlan.status === "draft"
						? "warn"
						: "fail",
			value: `${input.measurementPlan.readyCardCount}/${input.measurementPlan.cards.length}`,
			detail:
				input.measurementPlan.status === "ready"
					? "Линейка, контуры, плотность и пакет измерений закрыты."
		...
		clearanceCheck(input.geometrySummary),
		{
			...
			value: input.implantModelPlan.guideReady
				? "готов"
				: input.implantModelPlan.hasGuideRoute
					? "маршрут"
					: "нет",
			detail: input.implantModelPlan.guideReady
				? "Маршрут, втулка, ось и отступ до канала закрыты."
				: "Шаблон требует объем, ось, имплант, втулку, канал и экспортируемый маршрут.",
		},
	];
	...
	const pendingLabel =
		pendingChecks === 1
			? "1 пункт требует действия"
			: `${pendingChecks} пунктов требуют действия`;
	const label =
		status === "pass"
			? "план собран"
			: status === "fail"
