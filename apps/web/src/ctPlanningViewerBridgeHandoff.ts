import {
	buildCtPlanningViewerBridgeAuditRecord,
	type CtPlanningViewerBridgeAuditRecord,
} from "./ctPlanningViewerBridgeAudit";
import {
...
function defaultRuntimeTruthPolicy(
	pixelPolicy: CtPlanningViewerBridgePixelPolicy,
): CtPlanningViewerBridgeRuntimeTruthPolicy {
	return {
		version: "dental-crm-ct-runtime-truth-v1",
		sourceMode: "server_or_uploaded_copy",
		executionLane:
			pixelPolicy === "metadata_only_no_pixels"
		...
		heavyDataOwner: "external_viewer_or_local_3d_module",
		summary:
			"CRM bridge envelope carries planning metadata only; heavy CT pixels and 3D geometry stay outside the browser CRM.",
	};
}
...
export function buildCtPlanningViewerBridgeHandoff(input: {
	...
	const auditRecord = buildCtPlanningViewerBridgeAuditRecord({
		payload: launchPayload,
		gate: launchGate,
	});
	const runtimeTruthPolicy =
		input.runtimeTruthPolicy ??
		defaultRuntimeTruthPolicy(launchPayload.pixelPolicy);
