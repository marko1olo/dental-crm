...
function generateCurvedData(
	...
	const d00 = direction[0] * spacing[0],
		d01 = direction[3] * spacing[1],
		d02 = direction[6] * spacing[2];
	const d10 = direction[1] * spacing[0],
		d11 = direction[4] * spacing[1],
		d12 = direction[7] * spacing[2];
	const d20 = direction[2] * spacing[0],
		d21 = direction[5] * spacing[1],
		d22 = direction[8] * spacing[2];
	...
	const i00 = (d11 * d22 - d12 * d21) * invDet,
		i01 = (d02 * d21 - d01 * d22) * invDet,
		i02 = (d01 * d12 - d02 * d11) * invDet;
	const i10 = (d12 * d20 - d10 * d22) * invDet,
		i11 = (d00 * d22 - d02 * d20) * invDet,
		i12 = (d02 * d10 - d00 * d12) * invDet;
	const i20 = (d10 * d21 - d11 * d20) * invDet,
		i21 = (d01 * d20 - d00 * d21) * invDet,
		i22 = (d00 * d11 - d01 * d10) * invDet;
	...
	let targetIndex =
		csIndex !== undefined ? csIndex : Math.floor(resampled.length / 2);
	...
	let next =
		targetIndex < resampled.length - 1 ? resampled[targetIndex + 1] : centerPt;
	if (prev === centerPt && next !== centerPt)
		prev = {
			x: centerPt.x - (next.x - centerPt.x),
			y: centerPt.y - (next.y - centerPt.y),
		};
	if (next === centerPt && prev !== centerPt)
		next = {
			x: centerPt.x + (centerPt.x - prev.x),
			y: centerPt.y + (centerPt.y - prev.y),
		};
