...
import {
	buildCtPlanningExportPacket,
	type CtPlanningExportPacket,
} from "./ctPlanningExport";
import {
	buildCtPlanningImplantModelPlan,
	type CtPlanningImplantModelPlan,
} from "./ctPlanningImplantModel";
import {
	buildCtPlanningMeasurementPlan,
	type CtPlanningMeasurementPlan,
} from "./ctPlanningMeasurementPlan";
import {
	buildCtPlanningReconstructionPlan,
	type CtPlanningReconstructionPlan,
} from "./ctPlanningReconstruction";
import {
	buildCtPlanningValidationSummary,
	type CtPlanningValidationSummary,
...
function taskCard(task: DicomViewerPlanningTask): CtPlanningTaskCard {
	const projectionLabel = task.projection
		? (projectionLabels[task.projection] ?? task.projection)
		: "любой вид";
...
function targetToolForLocalAnnotation(
	type: ImagingViewerAnnotation["type"],
): DicomViewerTargetTool {
	switch (type) {
		case "distance":
			return "LengthTool";
...
function localAnnotationToToolState(
	annotation: ImagingViewerAnnotation,
): DicomViewerToolStateAnnotation {
	const warnings = new Set<string>();
	if (annotation.points.length === 0)
		warnings.add("Черновик разметки создан, но точки еще не поставлены.");
	if (
		(annotation.type === "distance" || annotation.type === "angle") &&
		annotation.measurementValue === null
	) {
		warnings.add(
			"Значение измерения появится после калибровки и постановки точек.",
		);
	}
	return {
...
function mergeToolStateAnnotations(
	...
	const seenSourceIds = new Set(
		bundleAnnotations.map((annotation) => annotation.sourceAnnotationId),
	);
	const localToolStateAnnotations = localAnnotations
		.filter(
			(annotation) =>
				annotation.type !== "note" && !seenSourceIds.has(annotation.id),
		)
...
function unsavedPlanningAnnotationCount(
	...
	return localAnnotations.filter(
		(annotation) =>
			...
			!seenSourceIds.has(annotation.id),
	).length;
}

function hasCompletedAnnotation(
	...
	return annotations.some(
		(annotation) =>
			annotation.type === type && annotation.points.length >= minimumPoints,
	);
}
...
export function buildCtPlanningTaskSnapshot(input: {
	...
	const readyTasks = tasks.filter(
		(task) => task.status === "ready" || task.status === "active",
	).length;
	const blockedTasks = tasks.filter((task) => task.status === "blocked").length;
	const volumeBlockedTasks = tasks.filter(
		(task) => task.status === "blocked" && task.requiresVolume,
	).length;
	const mergedAnnotations = mergeToolStateAnnotations(
		input.toolStateBundle?.annotations ?? [],
		input.localAnnotations ?? [],
	);
	const effectiveImplantPlan =
		input.toolStateBundle?.implantPlan ?? input.selectedImplantPlan ?? null;
	const localImplantNotInBundle = Boolean(
		input.selectedImplantPlan &&
			(!input.toolStateBundle?.implantPlan ||
				input.toolStateBundle.implantPlan.itemId !==
					input.selectedImplantPlan.itemId),
	);
	const unsavedArtifactCount =
		unsavedPlanningAnnotationCount(
			input.toolStateBundle?.annotations ?? [],
			input.localAnnotations ?? [],
		) + (localImplantNotInBundle ? 1 : 0);
	const hasImplantPlan = Boolean(
		effectiveImplantPlan ?? input.selectedImplantId,
	);
	const hasPanoramicRoute =
		tasks.some(
			(task) =>
	...
	const hasCanalRoute =
		tasks.some(
			(task) => task.kind === "nerve_canal" && task.status !== "blocked",
	...
	const readinessScore =
		totalTasks > 0
			...
			: input.canPlan
				? Math.min(
						85,
						45 + completedArtifactCount * 6 + (hasImplantPlan ? 7 : 0),
					)
				: 15;
	const warnings = Array.from(
		new Set(tasks.flatMap((task) => task.warnings)),
	).slice(0, 4);
	...
	const reconstructionPlan = buildCtPlanningReconstructionPlan({
		...
		slabMm:
			tasks.find((task) => task.kind === "panoramic_reconstruction")?.slabMm ??
			10,
	});
	...
	const exportPacket = buildCtPlanningExportPacket({
		...
		activeQuickActionId:
			input.activeQuickActionId ??
			input.toolStateBundle?.activeQuickActionId ??
			null,
		geometrySummary,
		validationSummary,
		readinessScore,
	...
	const routeCards: CtPlanningRouteCard[] = [
		{
			...
			detail:
				"Панорамная дуга и поперечные срезы восстанавливаются вместе с рабочим местом.",
		...
		},
		{
			...
			detail:
				"Канал нерва хранится как отдельная кривая, а не как заметка в свободном тексте.",
		...
		},
		{
			...
			detail:
				"План шаблона опирается на типоразмер, ось, втулку и 3D-проекцию.",
	...
	return {
		...
		cards,
		warnings: Array.from(
			new Set([...warnings, ...geometrySummary.warnings]),
		).slice(0, 6),
	};
}
