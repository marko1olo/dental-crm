...
const [dicomFirstFramePreviewRequest, setDicomFirstFramePreviewRequest] = useState<DicomFirstFramePreviewRequestContext | null>(null);
...
function reconcileDashboardScopedUiSelections() {
  ...
  const doctorIds = new Set(dashboard.clinicSettings.staff.filter(member => member.active && (member.role === "doctor" || member.role === "owner")).map(member => member.id));
  const assistantIds = new Set(dashboard.clinicSettings.staff.filter(member => member.active && member.role === "assistant").map(member => member.id));
...
function buildOnboardingFirstAppointmentIssues(): string[] {
  const issues: string[] = [];
  const requiredClinicDraftFields: Array<[string, string]> = [["название клиники", clinicProfileDraft.clinicName], ["телефон клиники", clinicProfileDraft.phone], ["часовой пояс", clinicProfileDraft.timezone]];
  ...
  const activeAppointmentReadiness = dashboard?.activeVisit.appointmentId ? dashboard.appointmentReadiness.find(readiness => readiness.appointmentId === dashboard.activeVisit.appointmentId) : null;
  const activeAppointmentBlockingChecks = activeAppointmentReadiness?.checks.filter(check => (check.key === "team" || check.key === "schedule") && !check.ready) ?? [];
...
async function downloadPersistenceExport() {
  ...
  try {
    ...
    await loadPersistenceIntegrity({
      silent: true
    });
    setError(null);
  } catch (exportError) {
...
async function refreshBrowserContinuity(options: {
  silent?: boolean;
} = {}) {
  try {
...
async function requestBrowserStoragePersistence() {
  ...
  try {
    ...
    await refreshBrowserContinuity({
      silent: true
    });
    if (!granted) {
...
async function loadSpeechRecordingStrategy(options: {
  silent?: boolean;
} = {}) {
  try {
...
async function loadSpeechRecordingRecovery(options: {
  silent?: boolean;
} = {}) {
  try {
...
async function submitAcceptedVisitDraft(visitId: string, draftToAccept: VisitNoteDraft, doctorSummary: string | null, options: {
...
async function syncVisitDraftAutosave(clientSavedAt: string, options: {
  silent?: boolean;
} = {}) {
  if (!dashboard) return;
  ...
  try {
    const response = await fetch(`/api/visits/${dashboard.activeVisit.id}/draft/autosave`, {
      ...
      body: JSON.stringify({
        ...
        selectedSpecialty,
        transcript,
        draft: visitNoteDraftFromForm(visitNoteForm, ["Серверный снимок автосохранения. Перед принятием черновика ЭМК врач все равно проверяет текст."]),
...
async function flushPendingVisitSaves(options: {
  silent?: boolean;
} = {}) {
...
function applySpeechTranscription(result: SpeechTranscriptionResponse) {
  ...
  void loadSpeechRecordingRecovery({
    silent: true
  });
  ...
  if (text) {
    ...
    setSpeechStatusNote(result.chunk.status === "transcribed" ? `${result.chunk.providerLabel}: фрагмент ${result.chunk.chunkIndex + 1}${qualitySuffix}` : `Сохранен фрагмент ${result.chunk.chunkIndex + 1}${qualitySuffix}: ${quality.nextAction}`);
    return;
  }
  setSpeechStatusNote(`${speechQualityLabels[quality.level]}: ${quality.nextAction}`);
}

async function assembleSpeechRecording(recordingId: string, options: {
  silent?: boolean;
} = {}) {
  try {
    const params = new URLSearchParams();
    ...
    const response = await fetch(`/api/speech/recordings/${encodeURIComponent(recordingId)}/assemble${scopedQuery ? `?${scopedQuery}` : ""}`, {
      cache: "no-store",
      headers: denteClinicalReadHeaders()
    });
...
async function flushPendingSpeechChunks(options: {
  silent?: boolean;
} = {}) {
  ...
  const currentGateway = (await loadSpeechGatewayStatus({
    silent: true
  })) ?? speechGatewayStatus;
...
useEffect(() => {
  ...
  loadServerUiPreferences(preferencesAccessSecret).then(async serverPreferences => {
    if (cancelled) return;
    const localPreferences = loadUiPreferences();
    if (serverPreferences && (!localPreferences.savedAt || serverPreferences.savedAt && serverPreferences.savedAt > localPreferences.savedAt)) {
    ...
    if (!cancelled) {
      ...
      if (pendingPreferences) queueUiPreferencesServerSync(pendingPreferences, {
        delayMs: 0
      });
    }
  });
...
useEffect(() => {
  ...
  queueUiPreferencesServerSync(savedPreferences, {
    delayMs: 600
  });
  return undefined;
}, [selectedWorkspaceRole, uiLanguage, selectedSpecialty, selectedProtocolId, selectedPatientId, scheduleDoctorFilterId, scheduleAssistantFilterId, scheduleChairFilterId, scheduleDefaultDoctorUserId, scheduleDefaultAssistantUserId, scheduleDefaultChairId, scheduleStatusFilter, scheduleDateFilter, paymentMethod, taxDocumentYear, selectedDocumentKind, taxApplicationForm, taxApplicationDeliveryChannel, paymentReceiptTaxSupportRequested, documentIssueSignatureMode, documentIssueStaffFullName, documentIssueStaffRole, procedureConsentProcedureType, postVisitCareTopic, pricelistSourceKind, usePricelistAi, recognitionKind, recognitionTarget, importSourceKind, documentIngestionTarget, imagingImportSourceKind, smartImportMode, imagingKindFilter, dicomWebEndpointUrl, ohifBaseUrl, telegramBotConfigId, telegramLinkSubjectType, telegramLinkStaffId, telegramOutboxStatusFilter, telegramOutboxTemplateFilter, onboardingDismissed, onboardingDismissedAt, onboardingStep, onboardingDraftMode, uiPreferencesHydrated]);
...
const imagingPreviewWorkset = useMemo(() => {
  ...
  const activeStudies = dashboard.imagingStudies.filter(study => study.patientId === dashboard.activeVisit.patientId).sort((left, right) => right.capturedAt.localeCompare(left.capturedAt));
  const visibleStudies = imagingKindFilter === "all" ? activeStudies : activeStudies.filter(study => study.kind === imagingKindFilter);
  const selectedStudy = visibleStudies.find(study => study.id === selectedImagingStudyId) ?? visibleStudies[0] ?? null;
  const comparisonStudies = selectedStudy ? activeStudies.filter(study => study.id !== selectedStudy.id).map(study => ({
    study,
    score: imagingComparisonScore(selectedStudy, study)
  })).sort((left, right) => right.score - left.score || imagingCaptureDistanceMs(selectedStudy.capturedAt, left.study.capturedAt) - imagingCaptureDistanceMs(selectedStudy.capturedAt, right.study.capturedAt) || right.study.capturedAt.localeCompare(left.study.capturedAt)).slice(0, 4).map(item => item.study) : [];
...
useEffect(() => {
  ...
  void Promise.all(imagingPreviewWorkset.map(async (study): Promise<[string, string] | null> => {
...
useEffect(() => {
  ...
  setTelegramPostVisitCheckupDelayDrafts(normalizeTelegramPostVisitCheckupDelayDrafts(settings.postVisitCheckupDelayHoursByTopic ?? defaultTelegramPostVisitCheckupDelayHoursByTopic));
...
}, [telegramSettingsDirty, telegramModeDraft, telegramBotUsernameDraft, telegramOwnBotUsernameDraft, telegramWebhookBaseUrlDraft, telegramPatientPortalBaseUrlDraft, telegramWelcomeImageUrlDraft, telegramVisualCardUrlDrafts, telegramReviewUrlDraft, telegramMapsUrlDraft, telegramEnabledFeaturesDraft, telegramTokenTtlDraft, telegramReminderLeadTimesDraft, telegramReviewRequestDelayDraft, telegramPostVisitCheckupDelayDrafts, telegramAllowVoiceIntakeDraft, telegramStaffEscalationChannelDraft, telegramPrivacyModeDraft, telegramStatus?.settings]);
...
}, [dashboard, scheduleAssistantFilterId, scheduleChairFilterId, scheduleDefaultAssistantUserId, scheduleDefaultChairId, scheduleDefaultDoctorUserId, scheduleDoctorFilterId, selectedProtocolId, selectedPatientId, telegramLinkStaffId]);
...
useEffect(() => {
  ...
  const dirtyAppointmentIds = Array.from(appointmentScheduleDirtyIds).filter(appointmentId => appointmentScheduleSaveStates[appointmentId] !== "saving");
  if (!dirtyAppointmentIds.length) return undefined;
  const appointmentRetryingErrors = dirtyAppointmentIds.some(appointmentId => appointmentScheduleSaveStates[appointmentId] === "error");
  const saveTimer = window.setTimeout(() => {
    dirtyAppointmentIds.forEach(appointmentId => void saveAppointmentSchedule(appointmentId, {
...
}, [dashboard, staffScheduleDirtyIds, staffScheduleSaveStates, chairScheduleDirtyIds, chairScheduleSaveStates, appointmentScheduleDirtyIds, appointmentScheduleSaveStates]);
...
useEffect(() => {
  setBrowserDirectoryPickerAvailable(typeof window !== "undefined" && typeof (window as BrowserDirectoryPickerWindow).showDirectoryPicker === "function");
...
useEffect(() => {
  if (currentView === "settings" && settingsTab === "audit") {
    ...
    void refreshBrowserContinuity({
      silent: true
    });
    void loadLocalBridgeUsePlans({
      silent: true
    });
  }
...
useEffect(() => {
  if (currentView === "settings" && settingsTab === "telegram" || !onboardingDismissed && onboardingStep === "telegram") {
    void loadTelegramControlPlane({
      silent: true
    });
  }
}, [currentView, settingsTab, onboardingDismissed, onboardingStep, telegramOutboxStatusFilter, telegramOutboxTemplateFilter, telegramModeDraft, telegramBotConfigId, dashboard?.clinicSettings.profile.organizationId]);
...
useEffect(() => {
  ...
  const markOnline = () => {
    ...
    void flushPendingSpeechChunks({
      silent: true
    });
    if (lastLocalSavedAt) void syncVisitDraftAutosave(lastLocalSavedAt, {
      silent: true
    });
  };
...
useEffect(() => {
  if (!dashboard) return;
  void loadSpeechRecordingStrategy({
    silent: true
  });
  ...
  const restoreServerDraft = async () => {
    try {
      ...
      if (Number.isFinite(serverDraftAt) && serverDraftAt > Math.max(localDraftAt, activeVisitAt)) {
        ...
        setLastServerDraftSavedAt(result.serverDraft.serverSavedAt);
        setLocalDraftWasRestored(true);
        lastServerDraftSignatureRef.current = visitDraftSignature(result.serverDraft.transcript, result.serverDraft.selectedSpecialty, visitNoteFormFromDraft(result.serverDraft.draft));
      } else {
        setLastServerDraftSavedAt(result.serverDraft.serverSavedAt);
      }
    } catch {
...
useEffect(() => {
  ...
  const timeout = window.setTimeout(() => {
    void syncVisitDraftAutosave(lastLocalSavedAt, {
      silent: true
    });
  }, 1600);
  return () => window.clearTimeout(timeout);
...
const activePatient = useMemo(() => {
  ...
  return findPatient(dashboard.patients, dashboard.activeVisit.patientId) ?? dashboard.patients.find(patient => patient.status === "active") ?? null;
}, [dashboard]);

useEffect(() => {
  if (!dashboard) return;
  setSelectedPatientId((current: any) => current && dashboard.patients.some(patient => patient.id === current) ? current : activePatient?.id ?? null);
}, [activePatient?.id, dashboard?.patients.length]);

const selectedPatient = useMemo(() => {
  if (!dashboard) return null;
  return (selectedPatientId ? findPatient(dashboard.patients, selectedPatientId) : null) ?? activePatient;
...
const paymentPatientContextMessage = !documentPatient ? "Выберите пациента текущего приема перед записью оплаты." : !documentPatientMatchesActiveVisit ? `Сейчас выбран пациент ${documentPatient.fullName}, но активный прием открыт для другого пациента. Переключите активный прием перед записью оплаты.` : "";
...
useEffect(() => {
  if (!selectedPatient || !patientAdministrativeProfileDirty || patientAdministrativeProfileSaveState === "saving" || patientAdministrativeProfileValidationMessage) {
    return undefined;
  }
  const saveTimer = window.setTimeout(() => {
  ...
  return () => window.clearTimeout(saveTimer);
}, [selectedPatient?.id, patientAdministrativeProfileDirty, patientAdministrativeProfileDraft, patientAdministrativeProfileSaveState, patientAdministrativeProfileValidationMessage]);
...
const telegramLinkStaffOptions = useMemo(() => dashboard?.clinicSettings.staff.filter(member => member.active) ?? [], [dashboard]);
...
const activeDocuments = useMemo(() => {
  ...
  return dashboard.documents.filter(document => document.patientId === documentPatient.id && (!documentPatientMatchesActiveVisit || document.visitId === null || document.visitId === dashboard.activeVisit.id));
...
const activeIssuedPaidContracts = useMemo(() => {
  return activeDocuments.filter(document => document.kind === "paid_medical_services_contract" && document.status === "issued" && document.visitId !== null).sort((left, right) => (right.issuedAt ?? "").localeCompare(left.issuedAt ?? ""));
}, [activeDocuments]);
...
const issuedMedicalCopyRequestDocuments = useMemo(() => {
  return activeUsableDocuments.filter(document => document.kind === "medical_record_copy_request" && document.status === "issued").sort((left, right) => (right.issuedAt ?? "").localeCompare(left.issuedAt ?? ""));
...
useEffect(() => {
  ...
  setReleaseProtectionNote(`Выдача по запросу ${sourceDocument.title}; канал: ${medicalDocumentReleaseChannelLabels[request.requestedFormat]}. Личность получателя проверена, лишние данные третьих лиц исключаются перед передачей.`);
...
const inferredTreatmentArea = useMemo(() => {
  const toothCodes = activeTreatmentPlanItems.filter(item => item.status !== "cancelled").map(item => item.toothCode?.trim()).filter((toothCode): toothCode is string => Boolean(toothCode));
...
const activeVisitClinicalRuleEvaluations = useMemo(() => {
  ...
  return dashboard.clinicalRuleEvaluations.filter(evaluation => evaluation.patientId === dashboard.activeVisit.patientId).sort((left, right) => Number(left.resolved) - Number(right.resolved) || severityRank[left.severity] - severityRank[right.severity]);
}, [dashboard]);

const activeVisitClinicalRuleSummary = useMemo(() => clinicalRuleSummaryForUi(activeVisitClinicalRuleEvaluations, dashboard?.clinicalRuleSummary.activeRules ?? 0), [activeVisitClinicalRuleEvaluations, dashboard?.clinicalRuleSummary.activeRules]);

const patientClinicalRuleSummary = useMemo(() => clinicalRuleSummaryForUi(patientClinicalRuleEvaluations, dashboard?.clinicalRuleSummary.activeRules ?? 0), [patientClinicalRuleEvaluations, dashboard?.clinicalRuleSummary.activeRules]);
...
const patientBillingSummary = useMemo<Dashboard["billingSummary"]>(() => {
  ...
  const draftDocumentAmountRub = activeUsableDocuments.filter(document => document.status === "draft").reduce((total, document) => total + (document.totalAmountRub ?? 0), 0);
  const unpaidDocuments = activeUsableDocuments.filter(document => document.status === "draft" && (document.totalAmountRub ?? 0) > 0 && !activePayments.some(payment => payment.status === "paid" && payment.documentId === document.id)).length;
  return {
    totalPlannedRub,
    totalDiscountRub,
    totalPaidRub,
...
const taxDocumentPayerOptions = useMemo(() => {
  const optionsByKey = new Map<string, {
    key: string;
    inn: string;
    label: string;
    amountRub: number;
  ...
  for (const payment of activePayments) {
    ...
    optionsByKey.set(payerKey, {
      key: payerKey,
      inn: payerInn,
      label: payerInn ? `${payerName} · ИНН ${payerInn}${payerRelationship ? ` · ${payerRelationship}` : ""}` : `${payerName} · документ ${payerIdentity || "без ИНН"}${payerRelationship ? ` · ${payerRelationship}` : ""}`,
...
const selectedTaxDocumentPayerOption = useMemo(() => taxDocumentPayerOptions.find(option => option.key === selectedTaxDocumentPayerKey) ?? null, [selectedTaxDocumentPayerKey, taxDocumentPayerOptions]);
...
const eligibleTaxPayments = useMemo(() => {
  return activePayments.filter(payment => payment.status === "paid" && payment.amountRub > 0 && paymentTaxYearForUi(payment) === taxDocumentYear && (!selectedTaxDocumentPayerKey || taxPaymentPayerKeyForUi(payment) === selectedTaxDocumentPayerKey)).sort((left, right) => (right.fiscalReceiptIssuedAt || right.paidAt || "").localeCompare(left.fiscalReceiptIssuedAt || left.paidAt || ""));
...
const selectedEligibleTaxPayments = useMemo(() => eligibleTaxPayments.filter(payment => selectedTaxPaymentIdSet.has(payment.id)), [eligibleTaxPayments, selectedTaxPaymentIdSet]);
...
const eligiblePaymentReceiptPayments = useMemo(() => {
  return activePayments.filter(payment => payment.status === "paid" && payment.amountRub > 0 && (!dashboard?.activeVisit.id || payment.visitId === dashboard.activeVisit.id)).sort((left, right) => (right.fiscalReceiptIssuedAt || right.paidAt || "").localeCompare(left.fiscalReceiptIssuedAt || left.paidAt || ""));
...
const selectedPaymentReceiptPayments = useMemo(() => eligiblePaymentReceiptPayments.filter(payment => selectedPaymentReceiptIdSet.has(payment.id)), [eligiblePaymentReceiptPayments, selectedPaymentReceiptIdSet]);
...
const eligibleRefundCorrectionPayments = useMemo(() => {
  return activePayments.filter(payment => payment.status === "paid" && payment.amountRub > 0 && payment.fiscalReceiptNumber?.trim() && (!dashboard?.activeVisit.id || payment.visitId === dashboard.activeVisit.id)).sort((left, right) => (right.fiscalReceiptIssuedAt || right.paidAt || "").localeCompare(left.fiscalReceiptIssuedAt || left.paidAt || ""));
}, [activePayments, dashboard?.activeVisit.id]);

const selectedRefundCorrectionPayment = useMemo(() => eligibleRefundCorrectionPayments.find(payment => payment.id === refundSelectedPaymentId) ?? null, [eligibleRefundCorrectionPayments, refundSelectedPaymentId]);
...
const medicalRecordExtractDraftPersistenceKey = useMemo(() => documentPayloadDraftKey("medical_record_extract", documentLocalPersistenceOrganizationId, documentPatient?.id ?? null, medicalRecordExtractDraftVisitId), [documentLocalPersistenceOrganizationId, documentPatient?.id, medicalRecordExtractDraftVisitId]);
...
const selectedTaxApplicationPayment = useMemo(() => {
  ...
  return activePayments.find(payment => payment.status === "paid" && taxPaymentPayerKeyForUi(payment) === selectedTaxDocumentPayerKey && paymentTaxYearForUi(payment) === taxDocumentYear) ?? null;
}, [activePayments, selectedTaxDocumentPayerKey, taxDocumentYear]);
...
useEffect(() => {
  ...
  saveDocumentPaymentSelection(documentLocalPersistenceOrganizationId, taxPaymentSelectionPersistenceKey, selectedTaxPaymentIdsForCurrentDocument());
}, [documentLocalPersistenceOrganizationId, eligibleTaxPaymentIdsKey, selectedDocumentUsesTaxPaymentSelection, selectedTaxPaymentIds, taxPaymentSelectionPersistenceKey]);
...
}, [documentLocalPersistenceOrganizationId, eligiblePaymentReceiptIdsKey, selectedDocumentUsesPaymentReceiptSelection, paymentReceiptSelectionPersistenceKey]);
...
useEffect(() => {
  ...
  const eligiblePaymentReceiptIdSet = new Set(eligiblePaymentReceiptPayments.map(payment => payment.id));
  saveDocumentPaymentSelection(documentLocalPersistenceOrganizationId, paymentReceiptSelectionPersistenceKey, selectedPaymentReceiptIds.filter(paymentId => eligiblePaymentReceiptIdSet.has(paymentId)));
}, [documentLocalPersistenceOrganizationId, eligiblePaymentReceiptIdsKey, paymentReceiptSelectionPersistenceKey, selectedDocumentUsesPaymentReceiptSelection, selectedPaymentReceiptIds]);
...
useEffect(() => {
  ...
  saveOutpatient025uDocumentDraft(documentLocalPersistenceOrganizationId, outpatient025uDraftPersistenceKey, documentPatient.id, outpatient025uDraftVisitId, currentOutpatient025uDocumentDraftFields());
}, [documentPatient?.id, documentLocalPersistenceOrganizationId, outpatient025uDraftPersistenceKey, outpatient025uDraftVisitId, outpatient025uMedicalCardNumber, outpatient025uOpenedAt, outpatient025uPatientSexCode, outpatient025uCitizenship, outpatient025uRegistrationUrbanRuralCode, outpatient025uStayUrbanRuralCode, outpatient025uOmsIssuedAt, outpatient025uInsurerName, outpatient025uSocialSupportCode, outpatient025uHealthStatusDisclosureContact, outpatient025uEmploymentCode, outpatient025uDisabilityGroup, outpatient025uWorkOrStudyPlace, outpatient025uPalliativeCareNeedCode, outpatient025uBloodGroup, outpatient025uRhFactor, outpatient025uKellK1, outpatient025uOtherBloodData, outpatient025uAllergyHistory, outpatient025uFinalEpicrisis, recordExtractPeriodStart, recordExtractPeriodEnd, recordExtractSourceVisitIds, recordExtractComplaintAndAnamnesis, recordExtractObjectiveStatus, recordExtractDiagnosis, recordExtractTreatmentProvided, recordExtractRecommendations, recordExtractDoctorFullName, recordExtractPreparedFromSignedRecords, outpatient025uOfficialForm274nChecked, outpatient025uThirdPartyDataChecked, selectedDocumentKind]);
...
useEffect(() => {
  ...
  const storedDraft = loadMedicalRecordExtractDocumentDraft(documentLocalPersistenceOrganizationId, medicalRecordExtractDraftPersistenceKey);
...
useEffect(() => {
  ...
  saveMedicalRecordExtractDocumentDraft(documentLocalPersistenceOrganizationId, medicalRecordExtractDraftPersistenceKey, documentPatient.id, medicalRecordExtractDraftVisitId, currentMedicalRecordExtractDocumentDraftFields());
}, [documentPatient?.id, documentLocalPersistenceOrganizationId, medicalRecordExtractDraftPersistenceKey, medicalRecordExtractDraftVisitId, recordExtractPeriodStart, recordExtractPeriodEnd, recordExtractSourceVisitIds, recordExtractComplaintAndAnamnesis, recordExtractObjectiveStatus, recordExtractDiagnosis, recordExtractTreatmentProvided, recordExtractRecommendations, recordExtractDoctorFullName, recordExtractRecipientFullName, recordExtractRecipientAuthority, recordExtractIssuedAt, recordExtractPreparedFromSignedRecords, recordExtractThirdPartyDataChecked, selectedDocumentKind]);
...
useEffect(() => {
  ...
  setTaxApplicationTaxpayerIdentityDocument(selectedTaxApplicationPayment.payerIdentityDocument?.trim() || documentPatient?.administrativeProfile?.identityDocument?.trim() || "");
...
const activeImagingStudies = useMemo(() => {
  if (!dashboard) return [];
  return dashboard.imagingStudies.filter(study => study.patientId === dashboard.activeVisit.patientId).sort((left, right) => right.capturedAt.localeCompare(left.capturedAt));
}, [dashboard]);

const imagingKindOptions = useMemo(() => Array.from(new Set(activeImagingStudies.map(study => study.kind))), [activeImagingStudies]);

const visibleImagingStudies = useMemo(() => imagingKindFilter === "all" ? activeImagingStudies : activeImagingStudies.filter(study => study.kind === imagingKindFilter), [activeImagingStudies, imagingKindFilter]);

const latestImagingStudy = visibleImagingStudies[0] ?? null;

const selectedImagingStudy = visibleImagingStudies.find(study => study.id === selectedImagingStudyId) ?? latestImagingStudy;

const imagingComparisonCandidates = useMemo(() => {
  if (!selectedImagingStudy) return [];
  return activeImagingStudies.filter(study => study.id !== selectedImagingStudy.id).map(study => ({
    study,
  ...
  })).sort((left, right) => right.score - left.score || imagingCaptureDistanceMs(selectedImagingStudy.capturedAt, left.study.capturedAt) - imagingCaptureDistanceMs(selectedImagingStudy.capturedAt, right.study.capturedAt) || right.study.capturedAt.localeCompare(left.study.capturedAt)).slice(0, 4);
...
const imagingViewerImageStyle: CSSProperties = {
  filter: `brightness(${imagingViewerState.brightness}) contrast(${imagingViewerState.contrast}) invert(${imagingViewerState.inverted ? 1 : 0})`,
  transform: `rotate(${imagingViewerState.rotationDeg}deg) scaleX(${imagingViewerState.flipHorizontal ? -1 : 1}) scale(${imagingViewerState.zoom})`
};

const dicomFirstFrameImageStyle: CSSProperties = {
  filter: `brightness(${dicomFirstFrameViewerState.brightness}) contrast(${dicomFirstFrameViewerState.contrast}) invert(${dicomFirstFrameViewerState.inverted ? 1 : 0})`,
  transform: `rotate(${dicomFirstFrameViewerState.rotationDeg}deg) scaleX(${dicomFirstFrameViewerState.flipHorizontal ? -1 : 1}) scale(${dicomFirstFrameViewerState.zoom})`
};

const cbctWorkbenchSeries = dicomSeriesPreview?.series.find(series => series.mprReadiness.volumeCandidate) ?? dicomSeriesPreview?.series.find(series => series.recommendedViewer === "cbct_mpr") ?? null;
...
}), [ctPlanningActiveQuickActionId, ctPlanningImplantPlan, imagingViewerActiveTool, imagingViewerState, mprAxisDeg, mprCrosshairEnabled, mprLinkedPlanesEnabled, mprProjection, mprSafeSliceIndex, mprSlabMm, mprWindowPreset, selectedImagingStudy?.kind, selectedImagingViewerPlan?.mode]);

const ctPlanningAnnotationRefs = useMemo(() => imagingViewerAnnotations.map(annotation => ({
  id: annotation.id,
...
const cbctWorkbenchProjections = useMemo<MprProjection[]>(() => cbctWorkbenchSeries?.mprReadiness.projections.length ? cbctWorkbenchSeries.mprReadiness.projections : ["axial", "coronal", "sagittal"], [cbctWorkbenchSeries]);
...
const cbctWorkbenchPlanes = useMemo<CbctWorkbenchPlane[]>(() => [{
  key: "axial",
  title: "Аксиальная",
  detail: "Срез сверху-вниз"
}, {
  key: "coronal",
  title: "Корональная",
  detail: "Фронтальная плоскость"
}, {
  key: "sagittal",
  title: "Сагиттальная",
  detail: "Боковая плоскость"
}, {
...
const mprClinicalPresetButtonClass = (preset: MprClinicalPreset) => ["mpr-clinical-preset", mprNearestClinicalPreset.title === preset.title ? "nearest" : "", mprNearestClinicalPreset.exact && mprNearestClinicalPreset.title === preset.title ? "active" : ""].filter(Boolean).join(" ");
...
const createCtPlanningArtifact = (command: CtPlanningArtifactCommand) => {
  ...
  const annotation: ImagingViewerAnnotation = {
    ...
    unit: command.unit,
    note: [`Черновик КТ-разметки: ${command.detail}`, `Плоскость: ${mprProjectionLabels[projection] ?? projection}`, `Срез: ${mprSafeSliceIndex + 1}/${mprSliceMaxIndex + 1}`, `Слой: ${mprSlabMm} мм`, ctPlanningImplantPlan ? `Имплант: ${ctPlanningImplantPlan.diameterMm} x ${ctPlanningImplantPlan.lengthMm} мм` : ""].filter(Boolean).join(" · "),
...
useEffect(() => {
  ...
  mprWorkbenchSaveTimerRef.current = window.setTimeout(() => {
    void saveLocalMprWorkbenchDraft(cbctWorkbenchSeriesKey, currentMprWorkbenchState, clientSavedAt, activeOrganizationId).then(saved => {
...
async function loadImagingViewerSessionForStudy(studyId: string) {
  ...
  try {
    ...
    const localIsNewer = localDraft?.clientSavedAt && new Date(localDraft.clientSavedAt).getTime() > new Date(payload.session.updatedAt).getTime();
    if (!localIsNewer) {
      applyImagingViewerSessionState(payload.session.state, payload.session.annotations);
      const localSaved = saveLocalImagingViewerDraft(studyId, {
...
async function saveCurrentImagingViewerSession(clientSavedAt: string) {
  ...
  try {
    ...
    const payload = (await response.json()) as ImagingViewerSessionResponse;
    setImagingViewerSession(payload.session);
    const localSaved = saveLocalImagingViewerDraft(selectedImagingStudy.id, {
      state: payload.session.state,
      annotations: payload.session.annotations,
...
const visibleVisitSpecialtyFocusOptions = useMemo(() => {
  ...
  return visitSpecialtyFocusOptions.filter(option => specialtiesWithTemplates.includes(option.specialty) && visibleSpecialties.has(option.specialty));
...
const dictationQuickPhrases = useMemo(() => {
  ...
  return [{
    label: "Повод",
    text: `Повод приема: ${visitReason}.`
  }, ...specialtyPhrases];
}, [activeAppointment?.reason, selectedProtocolTemplate?.visitReason, selectedSpecialty]);

const allResourceLoads = dashboard ? [...dashboard.shiftIntelligence.doctorLoads, ...dashboard.shiftIntelligence.assistantLoads, ...dashboard.shiftIntelligence.chairLoads] : [];
...
const visitDraftBuildMissingSteps = [!activePatient ? "выберите пациента" : null, !hasVisitTranscriptText ? "добавьте текст диктовки или нажмите голосовую запись" : null].filter((step): step is string => Boolean(step));
...
const speechGatewayActiveProviderIsLocal = speechGatewayStatus?.providerId === "local_whisper" || speechGatewayStatus?.providerId === "vosk_local";

const emptyDictationVoiceActionLabel = speechRecognitionReady ? speechGatewayActiveProviderIsLocal ? "Распознать локально" : "Распознать на сервере" : "Сохранить в очередь";

const pendingSpeechFlushActionLabel = speechRecognitionReady ? "Отправить звук" : "Проверить очередь";

const pendingSpeechFlushActionTitle = speechRecognitionReady ? "Отправить сохраненные аудиофрагменты на распознавание." : "Проверить готовность распознавания. Аудио останется в локальной очереди, пока источник недоступен.";

const speechSafetyValue = pendingSpeechChunkCount ? `${pendingSpeechChunkCount} аудио` : currentSpeechQualityIssue ? speechQualityLabels[currentSpeechQualityIssue.level] : speechRecognitionReady ? speechGatewayActiveProviderIsLocal ? "локальный модуль готов" : "распознавание готово" : "очередь локально";

const speechSafetyDetail = pendingSpeechChunkCount ? "аудио сохранено и уйдет позже" : currentSpeechQualityIssue ? currentSpeechQualityIssue.nextAction : speechRecognitionReady ? speechGatewayActiveProviderIsLocal ? `${speechGatewayStatus?.providerLabel ?? "локальный модуль"}, фрагменты уходят в локальный модуль` : `${speechGatewayStatus?.providerLabel ?? "распознавание"}, звук отправляется частями` : "аудио хранится локально до готового источника";

const browserCanRequestPersistentStorage = typeof navigator !== "undefined" && Boolean(navigator.storage) && typeof navigator.storage.persist === "function";
...
const browserContinuityChecks = [{
  ...
  detail: browserContinuity?.directoryPickerSupported ? "доступ к папке только после выбора пользователем; CRM не сохраняет тяжелые данные снимков" : browserContinuity?.filePickerSupported ? "можно выбрать файлы вручную; постоянный доступ к папке не сохраняется" : "используйте серверный путь, настольный модуль или внешний просмотр"
}, {
  label: "OPFS браузера",
  value: browserContinuity?.opfsSupported ? "доступно" : browserContinuity ? "нет" : "проверка",
  detail: browserContinuity?.opfsSupported ? "синхронный файловый доступ только в worker; диагностическое хранение в CRM отключено" : "текущее восстановление КТ не требует OPFS"
}, {
  ...
  detail: browserContinuity?.browserCtOfflineStorageBoundary.mode === "metadata_only" ? "локально сохраняются план открытия, состояние и пометки; тяжелые данные снимков и 3D-моделей остаются во внешнем просмотре" : "локальное восстановление КТ не подтверждено"
}, {
  ...
  try {
    const response = await fetch("/api/settings/staff", {
      ...
      body: JSON.stringify({
        ...
        workingHours: staffWorkingHoursFromSimpleDraft(clinicProfileDraft.workdayStart, clinicProfileDraft.workdayEnd, clinicProfileDraft.workingDays)
      })
    });
    if (!response.ok) {
...
async function saveAppointmentSchedule(appointmentId: string, options: {
...
function applyProtocolTemplate(template: ProtocolTemplate) {
  ...
  setTranscript([`${template.visitReason}.`, `Жалобы: ${template.complaintPrompt}`, `Объективно: ${template.objectiveTemplate}`, `Диагнозы к проверке: ${template.diagnosisHints.join("; ")}`, `План: ${template.treatmentPlanTemplate}`, `Документы: ${template.requiredDocuments.map(kind => documentLabels[kind]).join(", ")}.`, `Снимки: ${template.suggestedImaging.map(kind => imagingKindLabels[kind]).join(", ")}.`].join("\n"));
}
...
async function polishTranscript() {
  ...
  try {
    ...
    const polishLabel = result.polishMode === "deterministic_neural" ? `ИИ-полировка ${result.modelName ?? ""}`.trim() : "локальная проверка правил";
    setSpeechStatusNote(result.changedPhrases.length ? `Текст очищен (${polishLabel}): ${result.changedPhrases.slice(0, 4).join(", ")}` : `Текст проверен (${polishLabel}): факты не добавлялись.`);
  } catch (polishError) {
...
async function buildDraft() {
  if (!dashboard || !activePatient || !hasVisitTranscriptText) {
    const missingSteps = [!dashboard ? "дождитесь загрузки приема" : null, ...visitDraftBuildMissingSteps].filter((step): step is string => Boolean(step));
...
async function acceptDraftToVisit() {
  ...
  const acceptedDraft = visitNoteDraftFromForm(visitNoteForm, draft?.warnings ?? ["Правки внесены врачом вручную. Подпись приема остается отдельным действием."]);
...
function migrationAutopilotRequestPayload(knownDiscovery: MigrationLocalSourceDiscoveryResponse | null = activeMigrationDiscoveryForAutopilot(), options: {
  includeSmartImportText?: boolean;
} = {}) {
  const clinicPayload = {
...
async function runMigrationAutopilot(knownDiscovery: MigrationLocalSourceDiscoveryResponse | null = activeMigrationDiscoveryForAutopilot(), options: {
  includeSmartImportText?: boolean;
} = {}) {
...
async function downloadMigrationHandoffReport() {
  ...
  try {
    const response = await fetch("/api/imports/smart/migration-autopilot/report.csv", {
      ...
      body: JSON.stringify(migrationAutopilotRequestPayload(knownDiscovery, {
        includeSmartImportText: Boolean(smartImportText.trim())
      }))
    });
    if (!response.ok) {
...
function migrationCandidateCanPreview(candidate: MigrationLocalSourceDiscoveryResponse["candidates"][number]) {
  const materialCount = candidate.matchedFiles + candidate.databaseFiles + candidate.dumpFiles + candidate.tableFiles + candidate.archiveFiles + candidate.dicomLikeFiles + candidate.imageFiles;
...
async function previewMigrationAutopilotSources(sourceFingerprint?: string | null) {
  ...
  const previewSources = selectedSources.length ? selectedSources.filter(source => migrationCandidateCanPreview(source.candidate)) : sources.filter(source => source.readiness.level === "ready_for_preview" || migrationCandidateCanPreview(source.candidate));
  ...
  const sourceLines = Array.from(new Set(previewSources.slice(0, 12).map(source => source.candidate.smartImportLine).filter(Boolean)));
...
function stageLocalImagingFolderRecovery(folderPath: string, metadata: Partial<Omit<LocalImagingFolderDraft, "version" | "folderPath" | "savedAt">> = {}) {
...
function rememberLocalImagingFolder(folderPath: string, metadata: Partial<Omit<LocalImagingFolderDraft, "version" | "folderPath" | "savedAt">> = {}) {
...
async function runBrowserImagingFolderScan(input: {
  ...
  try {
    ...
    setBrowserImagingScanProgress(browserImagingScanProgressFromStats({
    ...
    if (isBrowserImagingScanAbortError(scanError)) {
      setBrowserImagingScanProgress((current: any) => current ? (() => {
...
function browserMigrationStatsFor(statsByFolder: Map<string, BrowserMigrationFolderStats>, folderKey: string, folderHint: string, depth: number) {
...
async function addBrowserMigrationFileToStats(input: {
  ...
  if (kind === "database") input.stats.databaseFiles += 1;else if (kind === "dump") input.stats.dumpFiles += 1;else if (kind === "table") input.stats.tableFiles += 1;else if (kind === "archive") input.stats.archiveFiles += 1;else if (kind === "dicom") input.stats.dicomLikeFiles += 1;else if (kind === "image") input.stats.imageFiles += 1;else if (kind === "model") input.stats.modelFiles += 1;
  return kind;
}
...
async function runBrowserMigrationSourceScan(input: {
  ...
  try {
    ...
    if (isBrowserMigrationScanAbortError(scanError)) {
      setBrowserMigrationScanProgress(current => current ? (() => {
        const updatedAt = new Date().toISOString();
        return {
          ...current,
          phase: "cancelled",
...
async function scanBrowserMigrationDirectoryHandle(directoryHandle: BrowserFileSystemDirectoryHandle, options: BrowserMigrationScanOptions): Promise<MigrationLocalSourceDiscoveryResponse> {
...
async function scanBrowserDirectoryHandle(directoryHandle: BrowserFileSystemDirectoryHandle, options: BrowserImagingScanOptions): Promise<BrowserPickedImagingFolderPreview> {
  ...
  while (stack.length > 0 && stats.scannedFolders < browserImagingScanFolderLimit && stats.scannedFiles < browserImagingScanFileLimit) {
    ...
    try {
      ...
      for await (const [, handle] of current.entries()) {
        ...
        throwIfBrowserImagingScanAborted(options.signal);
        if (kind === "dicom") stats.dicomLikeFiles += 1;else if (kind === "archive") stats.archiveFiles += 1;else if (kind === "model") stats.modelFiles += 1;else if (kind === "image") stats.imageFiles += 1;
        runtime.processedUnits += 1;
...
async function organizeLocalImagingSources() {
  ...
  try {
    ...
    if (useSpecificRoot) rememberLocalImagingFolder(candidateRoot, {
      origin: "manual"
    });
...
async function scanImagingFolder() {
  ...
  rememberLocalImagingFolder(folderPath, {
    origin: "manual"
  });
  const controller = startLocalDicomOperation();
...
async function previewDicomFirstFrame(folderPath = imagingFolderPath.trim(), metadata: DicomFirstFramePreviewMetadata = {
  origin: "manual"
...
async function fetchDicomFolderWorkup(folderPath: string, sourceName: string, options: LocalDicomOperationOptions = {}): Promise<{
...
function selectPreferredDicomWorkupPlan(result: DicomFolderWorkupPlanResponse) {
  return result.plans.find(plan => plan.recommendedPath === "open_mpr") ?? result.plans.find(plan => plan.recommendedPath === "downsampled_mpr") ?? result.plans.find(plan => plan.series.mprReadiness.volumeCandidate) ?? result.plans.find(plan => plan.recommendedPath === "external_viewer") ?? result.plans[0] ?? null;
}
...
async function buildDicomFolderWorkupPlan() {
  ...
  try {
    const {
      result
    } = await fetchDicomFolderWorkup(folderPath, "dicom_folder_workup", {
      signal: controller.signal
    });
...
async function prepareDicomWorkbenchFromFolder(folderPath: string, sourceName = "dicom_local_quick_workbench", metadata: Partial<Omit<LocalImagingFolderDraft, "version" | "folderPath" | "savedAt">> = {}) {
  const cleanFolderPath = folderPath.trim();
  if (!cleanFolderPath) {
  ...
  try {
    const {
      client,
      result
    } = await fetchDicomFolderWorkup(cleanFolderPath, sourceName, {
      signal: controller.signal
    });
...
async function loadDicomWorkbenchBundles(options: {
  silent?: boolean;
  restoreLatest?: boolean;
} = {}) {
  try {
    const response = await fetch("/api/imaging/dicom/workbench-bundles?limit=6", {
      headers: denteClinicalReadHeaders()
    });
    if (!response.ok) {
...
async function saveDicomWorkbenchBundleToServer(manifest: DicomViewerWorkbenchManifestResponse | null = dicomViewerWorkbenchManifest, clientSavedAt: string | null = dicomWorkbenchLocalSavedAt, options: {
  silent?: boolean;
  signal?: AbortSignal;
} = {}) {
  ...
  try {
    ...
    setDicomWorkbenchServerBundles(bundles => [result.bundle, ...bundles.filter(bundle => bundle.id !== result.bundle.id && bundle.seriesKey !== result.bundle.seriesKey)].slice(0, 6));
...
async function reconnectDicomWorkbenchFromCurrentFolder() {
  ...
  const targetStudyUid = dicomViewerWorkbenchManifest?.toolStateBundle.seriesRef.studyInstanceUid ?? dicomWorkbenchServerBundle?.studyInstanceUid ?? latestDicomWorkbenchServerBundle?.studyInstanceUid ?? null;
  const targetSeriesUid = dicomViewerWorkbenchManifest?.toolStateBundle.seriesRef.seriesInstanceUid ?? dicomWorkbenchServerBundle?.seriesInstanceUid ?? latestDicomWorkbenchServerBundle?.seriesInstanceUid ?? null;
  const controller = startLocalDicomOperation();
  ...
  try {
    ...
    const matchedPlan = workup.plans.find(plan => (!targetStudyUid || plan.series.studyInstanceUid === targetStudyUid) && (!targetSeriesUid || plan.series.seriesInstanceUid === targetSeriesUid)) ?? workup.plans.find(plan => plan.series.mprReadiness.volumeCandidate) ?? workup.plans[0] ?? null;
    if (!matchedPlan) {
...
async function buildDicomRenderCachePlan() {
  if (!cbctWorkbenchSeries || !dicomWorkstationReadiness) {
    const missingSteps = [!cbctWorkbenchSeries ? "выберите готовую КЛКТ/КТ-серию" : null, !dicomWorkstationReadiness ? "сначала проверьте этот ПК" : null].filter((step): step is string => Boolean(step));
...
function appendVisitDictationText(value: string) {
  ...
  setTranscript((current: any) => appendSpeechTextWithoutDuplicateTail(current, cleanValue, speechGatewayStatus?.chunkingPolicy.dedupeWindowChars ?? 600));
  setDraft(null);
}
...
async function uploadSpeechBlob(blob: Blob) {
  ...
  if (blob.size > maxChunkBytes) {
    setSpeechStatusNote(`Распознавание: аудио-фрагмент ${Math.round(blob.size / 1024 / 1024)} МБ больше лимита ${Math.round(maxChunkBytes / 1024 / 1024)} МБ; запись продолжается, уменьшите длительность чанка или используйте локальный модуль.`);
    return;
  }
  ...
  if (!isOnline || !speechGatewayCanUpload(speechGatewayStatus)) {
    setSpeechStatusNote(queuedBeforeUpload ? `Фрагмент ${chunkIndex + 1} сохранен локально; распознавание отправится, когда источник будет готов.` : `Фрагмент ${chunkIndex + 1} не сохранен: локальная очередь недоступна.`);
    return;
  }
  try {
    ...
    await refreshPendingSpeechChunkState();
    setSpeechStatusNote(queued ? `Фрагмент ${chunkIndex + 1} сохранен локально и уйдет на сервер позже.` : `Фрагмент ${chunkIndex + 1} не отправлен: ${operatorReadableErrorDetailFromUnknown(speechError) ?? "повторите запись или проверьте подключение к серверу клиники"}.`);
  }
}
...
function startSpeechMonitor(stream: MediaStream, recorder: MediaRecorder, status: SpeechGatewayStatus | null) {
  ...
  try {
    ...
    setSpeechStatusNote(`${providerLabel}: умные фрагменты ${Math.round(chunkingPolicy.minChunkMs / 1000)}-${Math.round(chunkingPolicy.maxChunkMs / 1000)} сек., пауза ${chunkingPolicy.silenceMs} мс.`);
  } catch {
    stopSpeechMonitor();
    recorder.start(recommendedChunkMs);
...
function treatmentAcceptancePlannedTotalRub(): number {
  return activeTreatmentPlanItems.filter(item => item.status !== "cancelled").filter(item => !dashboard?.activeVisit.id || item.visitId === dashboard.activeVisit.id).reduce((total, item) => total + Math.max(0, item.unitPriceRub * item.quantity - item.discountRub), 0) || 0;
}
...
function clinicalToothRowsValue(): ClinicalToothRow[] {
  const fallbackArea = procedureConsentToothOrArea.trim() || treatmentPlanTeethOrAreaValue() || treatmentAcceptanceTeethOrArea.trim() || inferredTreatmentArea || "область лечения";
  const fallbackFinding = procedureConsentDiagnosisOrIndication.trim() || treatmentPlanDiagnosisSummaryValue() || treatmentAcceptanceDiagnosisSummary.trim() || recordExtractDiagnosisValue() || "клиническая находка требует уточнения врачом";
  const fallbackIndication = treatmentPlanClinicalReasonValue() || recordExtractComplaintAndAnamnesisValue() || "медицинское показание к лечению";
  const fallbackAction = dashboard?.activeVisit.treatmentPlan?.trim() || procedureConsentProcedureName.trim() || treatmentAcceptanceClinicalGoal.trim() || "согласованное стоматологическое лечение";
  return documentTextLines(clinicalToothRowsText).map((line, index) => {
    const [toothOrArea, surfaces, status, diagnosisOrFinding, indication, plannedAction, prognosis, periodontalStatus, implantOrProstheticNotes, orthodonticNotes] = line.split("|").map(part => part.trim());
    return {
...
function completedActFiscalReceiptLines(): string[] {
  ...
  return activePaidPaymentsForVisit().map(payment => payment.fiscalReceiptNumber?.trim()).filter((value): value is string => Boolean(value));
}
...
function plannedServiceLinesForFinancialPayload() {
  return activeTreatmentPlanItems.filter(item => item.status !== "cancelled").filter(item => !dashboard?.activeVisit.id || item.visitId === dashboard.activeVisit.id).map(item => {
...
function treatmentEstimateTreatmentBasisValue(): string {
  return treatmentEstimateTreatmentBasis.trim() || compactDocumentText(dashboard?.activeVisit.diagnosis, dashboard?.activeVisit.complaint, dashboard?.activeVisit.treatmentPlan) || "плановое стоматологическое лечение по результатам осмотра";
}
...
function paymentReceiptFiscalReceiptLines(): string[] {
  return selectedPaymentReceiptPayments.map(payment => payment.fiscalReceiptNumber?.trim()).filter((value): value is string => Boolean(value));
}
...
function warrantyLinkedActOrContractValue(): string {
  return warrantyLinkedActOrContract.trim() || activeUsableDocuments.find(document => document.kind === "completed_works_act" || document.kind === "paid_medical_services_contract")?.title || "акт выполненных работ или договор клиники";
}
...
function applyPostVisitCarePreset(topic: PostVisitCareTopic, options: {
  force?: boolean;
} = {}) {
  ...
  if (postVisitManualEdited && !options.force) {
    setPostVisitPresetFeedback(`Тема "${topicLabel}" выбрана. Текст не перезаписан, потому что есть ручные правки. Нажмите "Подставить памятку для темы", если нужно заменить поля.`);
    return;
  }
...
function outpatient025uLicenseValue(): string | null {
  const value = compactDocumentText(clinicProfileDraft.medicalLicenseNumber, clinicProfileDraft.medicalLicenseIssuedAt, clinicProfileDraft.medicalLicenseIssuer);
  return value || null;
}
...
function outpatient025uPayloadValue(): OutpatientMedicalCard025uPayload {
  ...
  return {
    ...
    preparedFromSignedMedicalRecords: confirmedDocumentLiteral(recordExtractPreparedFromSignedRecords, "карта 025/у собрана из подписанных медицинских записей"),
    officialForm274nChecked: confirmedDocumentLiteral(outpatient025uOfficialForm274nChecked, "структура карты 025/у сверена с приказом Минздрава N 274н"),
...
function validateDocumentPayloadForKind(kind: GeneratedDocument["kind"]): string | null {
  ...
  if (kind === "paid_medical_services_contract") {
    return requiredDocumentField(paidContractNumber, "договор, номер") ?? requiredDocumentField(paidContractDate, "договор, дата") ?? requiredDocumentField(paidContractServiceStart, "договор, начало оказания услуг") ?? requiredDocumentField(paidContractServiceEnd, "договор, окончание или условие завершения") ?? requiredDocumentField(paidContractCustomerFullNameValue(), "договор, заказчик") ?? requiredDocumentField(paidContractCareReasonValue(), "договор, основание обращения") ?? requiredDocumentField(paidContractServiceScopeValue(), "договор, состав услуг") ?? (paidContractTotalRubValue() > 0 ? null : "Укажите ориентировочную стоимость договора.") ?? requiredDocumentField(paidContractPaymentTerms, "договор, порядок оплаты") ?? requiredDocumentField(paidContractPriceChangeRules, "договор, изменение цены и объема") ?? requiredDocumentField(paidContractFreeCareNotice, "договор, уведомление о бесплатной помощи") ?? requiredDocumentField(paidContractRecommendationWarning, "договор, предупреждение о рекомендациях врача") ?? requiredDocumentField(paidContractRefundTerms, "договор, отказ и возврат") ?? requiredDocumentField(paidContractWarrantyTerms, "договор, гарантия и претензии") ?? requiredDocumentField(paidContractDoctorFullNameValue(), "договор, врач") ?? requiredDocumentField(paidContractSignedAt, "договор, дата подписания") ?? (paidContractClinicInfoConfirmed ? null : "Подтвердите, что пациент получил сведения о клинике и лицензии.") ?? (paidContractServiceListConfirmed ? null : "Подтвердите, что пациент получил перечень услуг и стоимость.") ?? (paidContractPaidBasisConfirmed ? null : "Подтвердите, что пациент понимает платную основу услуг.") ?? (paidContractWrittenChangesConfirmed ? null : "Подтвердите, что изменения договора оформляются письменно.");
  }
  if (kind === "completed_works_act") {
    return requiredDocumentField(completedActNumber, "акт, номер") ?? requiredDocumentField(completedActDate, "акт, дата") ?? requiredDocumentField(completedActContractNumber, "акт, договор") ?? (selectedCompletedActContractDocumentId ? null : "Выберите конкретный уже выданный договор для акта.") ?? requiredDocumentField(completedActServicePeriodStart, "акт, начало периода оказания") ?? requiredDocumentField(completedActServicePeriodEnd, "акт, окончание периода оказания") ?? requiredDocumentField(completedActDoctorFullNameValue(), "акт, врач-исполнитель") ?? requiredDocumentField(completedActServicesSummaryValue(), "акт, состав работ") ?? (completedActTotalRubValue() > 0 ? null : "Укажите сумму по акту.") ?? (completedActPaidRubValue() > 0 ? null : "Для акта нужна фактическая оплаченная сумма.") ?? (completedActFiscalReceiptLines().length ? null : "Добавьте номера фискальных чеков по акту.") ?? (completedActLinkedContract ? null : "Подтвердите связь акта с подписанным договором.") ?? (completedActFinalScopeConfirmed ? null : "Подтвердите финальный состав работ.") ?? (completedActFiscalReceiptsVerified ? null : "Подтвердите проверку фискальных чеков.") ?? (completedActAccepted ? null : "Подтвердите приемку работ пациентом.");
  }
  if (kind === "treatment_cost_estimate") {
    const serviceLines = plannedServiceLinesForFinancialPayload();
    return requiredDocumentField(treatmentEstimateNumber, "смета, номер") ?? requiredDocumentField(treatmentEstimateDate, "смета, дата") ?? requiredDocumentField(treatmentEstimatePatientOrPayerFullNameValue(), "смета, пациент или плательщик") ?? requiredDocumentField(treatmentEstimateTreatmentBasisValue(), "смета, основание лечения") ?? (serviceLines.length ? null : "Для сметы нужен состав услуг из плана лечения.") ?? (treatmentEstimateTotalRubValue() > 0 ? null : "Укажите итоговую сумму сметы.") ?? requiredDocumentField(treatmentEstimateValidUntil, "смета, срок действия") ?? requiredDocumentField(treatmentEstimatePriceChangeRules, "смета, правила изменения цены") ?? (documentTextLines(treatmentEstimateExcludedItems).length ? null : "Укажите, что не входит в текущую смету.") ?? requiredDocumentField(treatmentEstimatePaymentMilestoneNotes, "смета, условия оплаты") ?? requiredDocumentField(treatmentEstimateDoctorFullNameValue(), "смета, ответственный врач") ?? requiredDocumentField(treatmentEstimateSignedAt, "смета, дата ознакомления") ?? (treatmentEstimatePreliminaryConfirmed ? null : "Подтвердите предварительный характер сметы.") ?? (treatmentEstimateScopeConfirmed ? null : "Подтвердите соответствие состава услуг плану лечения.") ?? (treatmentEstimateFiscalNoticeConfirmed ? null : "Подтвердите, что смета не заменяет договор, акт и кассовый чек.") ?? (treatmentEstimateChangeRulesConfirmed ? null : "Подтвердите правило обновления сметы при изменениях.");
  }
  if (kind === "payment_invoice") {
    const serviceLines = plannedServiceLinesForFinancialPayload();
    return requiredDocumentField(paymentInvoiceNumber, "счет, номер") ?? requiredDocumentField(paymentInvoiceDate, "счет, дата") ?? requiredDocumentField(paymentInvoicePayerFullNameValue(), "счет, плательщик") ?? requiredDocumentField(paymentInvoicePurpose, "счет, назначение платежа") ?? (serviceLines.length ? null : "Для счета нужен состав услуг из плана лечения.") ?? (paymentInvoiceTotalRubValue() > 0 ? null : "Укажите сумму счета.") ?? requiredDocumentField(paymentInvoiceDueDate, "счет, срок оплаты") ?? requiredDocumentField(paymentInvoicePaymentTerms, "счет, условия оплаты") ?? requiredDocumentField(paymentInvoiceBankDetailsValue(), "счет, реквизиты клиники") ?? (paymentInvoiceCashlessAllowed || paymentInvoiceCashDeskAllowed ? null : "Выберите хотя бы один способ оплаты.") ?? (paymentInvoiceRequisitesVerified ? null : "Подтвердите проверку реквизитов клиники.") ?? (paymentInvoiceServiceScopeConfirmed ? null : "Подтвердите состав услуг счета.") ?? (paymentInvoiceFiscalNoticeConfirmed ? null : "Подтвердите предупреждение: счет не заменяет кассовый чек.");
  }
  if (kind === "payment_receipt") {
    return requiredDocumentField(paymentReceiptNumber, "квитанция, номер") ?? requiredDocumentField(paymentReceiptDate, "квитанция, дата") ?? (selectedPaymentReceiptPayments.length ? null : "Выберите оплаченные платежи для квитанции.") ?? (selectedPaymentReceiptTotalRub > 0 ? null : "Сумма выбранных платежей должна быть больше нуля.") ?? requiredDocumentField(paymentReceiptPayerFullNameValue(), "квитанция, ФИО плательщика") ?? (paymentReceiptTaxSupportRequested ? requiredDocumentField(paymentReceiptPayerBirthDateValue(), "квитанция, дата рождения плательщика") ?? requiredDocumentField(paymentReceiptPayerRelationshipValue(), "квитанция, связь плательщика с пациентом") ?? (paymentReceiptPayerInnValue().replace(/\D+/g, "").length === 12 || paymentReceiptPayerIdentityDocumentValue().trim() ? null : "Для налоговой квитанции укажите 12-значный ИНН плательщика или документ плательщика.") : null) ?? requiredDocumentField(paymentReceiptPurpose, "квитанция, назначение оплаты") ?? (paymentReceiptFiscalReceiptLines().length === selectedPaymentReceiptPayments.length ? null : "У каждого выбранного платежа должен быть номер фискального чека.") ?? (selectedPaymentReceiptPayments.every(payment => Boolean(payment.fiscalReceiptIssuedAt?.trim())) ? null : "У каждого выбранного платежа должна быть дата фискального чека.") ?? requiredDocumentField(paymentReceiptIssuedByValue(), "квитанция, кто выдал") ?? (paymentReceiptPaymentsVerified ? null : "Подтвердите сверку выбранных платежей и фискальных чеков.") ?? (paymentReceiptPayerVerified ? null : "Подтвердите проверку данных плательщика.") ?? (paymentReceiptFiscalNoticeConfirmed ? null : "Подтвердите, что квитанция не заменяет кассовый чек.");
  }
  if (kind === "installment_payment_schedule") {
    const installments = installmentScheduleInstallmentRows();
    return requiredDocumentField(installmentScheduleNumber, "график, номер") ?? requiredDocumentField(installmentScheduleDate, "график, дата") ?? requiredDocumentField(installmentScheduleBaseDocumentTitleValue(), "график, основание") ?? requiredDocumentField(installmentSchedulePayerFullNameValue(), "график, плательщик") ?? (installmentScheduleTotalRubValue() > 0 ? null : "Укажите общую сумму графика.") ?? (installmentScheduleRemainingRubValue() >= 0 ? null : "Остаток по графику не может быть отрицательным.") ?? (installments.length ? null : "Добавьте платежи графика или укажите остаток к оплате.") ?? requiredDocumentField(installmentScheduleLatePolicy, "график, правила просрочки") ?? requiredDocumentField(installmentSchedulePaymentMethodNotes, "график, способы оплаты") ?? requiredDocumentField(installmentScheduleResponsibleFullNameValue(), "график, ответственный") ?? (installmentScheduleAccepted ? null : "Подтвердите принятие графика пациентом.") ?? (installmentScheduleFiscalNoticeConfirmed ? null : "Подтвердите, что график не заменяет кассовый чек.") ?? (installmentScheduleWrittenChangesConfirmed ? null : "Подтвердите письменное оформление изменений графика.");
  }
  if (kind === "minor_legal_representative_consent") {
    return requiredDocumentField(minorRepresentativeFullNameValue(), "представитель, ФИО") ?? requiredDocumentField(minorRepresentativeRelationshipValue(), "представитель, родство или статус") ?? requiredDocumentField(minorRepresentativeIdentityDocumentValue(), "представитель, документ личности") ?? requiredDocumentField(minorRepresentativeAuthorityDocument, "представитель, основание полномочий") ?? requiredDocumentField(minorConsentPatientFullNameValue(), "несовершеннолетний, ФИО") ?? requiredDocumentField(minorConsentPatientBirthDateValue(), "несовершеннолетний, дата рождения") ?? requiredDocumentField(minorConsentInterventionScopeValue(), "согласие, вмешательство") ?? requiredDocumentField(minorConsentDiagnosisOrIndicationValue(), "согласие, диагноз или показание") ?? (documentTextLines(minorConsentRisks).length ? null : "Добавьте разъясненные риски для представителя.") ?? (documentTextLines(minorConsentAlternatives).length ? null : "Добавьте альтернативы лечения для представителя.") ?? requiredDocumentField(minorConsentDoctorFullNameValue(), "согласие, врач") ?? requiredDocumentField(minorConsentSignedAt, "согласие, дата и время") ?? (minorConsentIdentityVerified ? null : "Подтвердите проверку личности представителя.") ?? (minorConsentAuthorityVerified ? null : "Подтвердите полномочия представителя.") ?? (minorConsentExplained ? null : "Подтвердите разъяснение вмешательства, рисков и альтернатив.") ?? (minorConsentStored ? null : "Подтвердите хранение согласия в медкарте.") ?? (minorConsentAgeExplanation ? null : "Подтвердите объяснение ребенку по возрасту и состоянию.");
  }
  if (kind === "warranty_service_memo") {
    return requiredDocumentField(warrantyServiceOrWorkNameValue(), "гарантия, работа или услуга") ?? requiredDocumentField(warrantyCompletedAt, "гарантия, дата завершения") ?? requiredDocumentField(warrantyTeethOrAreaValue(), "гарантия, зубы или область") ?? requiredDocumentField(warrantyMaterialsOrSystems, "гарантия, материалы или системы") ?? requiredDocumentField(warrantyPeriod, "гарантия, срок и условия") ?? requiredDocumentField(warrantyControlVisitSchedule, "гарантия, контрольные визиты") ?? (documentTextLines(warrantyPatientObligations).length ? null : "Добавьте обязанности пациента для сохранения гарантии.") ?? (documentTextLines(warrantyExcludedRiskFactors).length ? null : "Добавьте условия, требующие отдельной оценки.") ?? (documentTextLines(warrantyUrgentContactReasons).length ? null : "Добавьте признаки для срочной связи с клиникой.") ?? requiredDocumentField(warrantyLinkedActOrContractValue(), "гарантия, связанный акт или договор") ?? requiredDocumentField(warrantyDoctorFullNameValue(), "гарантия, врач") ?? requiredDocumentField(warrantyIssuedAt, "гарантия, дата выдачи") ?? (warrantyPolicyApplied ? null : "Подтвердите применение локального гарантийного положения.") ?? (warrantyAftercareReceived ? null : "Подтвердите выдачу рекомендаций после лечения.") ?? (warrantyControlVisitsUnderstood ? null : "Подтвердите понимание контрольных визитов пациентом.");
  }
  if (kind === "patient_intake_questionnaire") {
    return requiredDocumentField(intakeChiefComplaint, "анкета, жалоба или цель визита") ?? requiredDocumentField(intakeAllergyStatus, "анкета, аллергии") ?? requiredDocumentField(intakeCurrentMedications, "анкета, постоянные препараты") ?? requiredDocumentField(intakeChronicConditions, "анкета, хронические заболевания") ?? requiredDocumentField(intakeAnticoagulants, "анкета, антикоагулянты") ?? requiredDocumentField(intakeInfectiousRiskNotes, "анкета, инфекционные риски") ?? requiredDocumentField(intakeCardioEndocrineNotes, "анкета, системные риски") ?? (intakeAccuracyConfirmed ? null : "Пациент должен подтвердить достоверность анкеты перед созданием документа.");
  }
  if (kind === "tax_deduction_application") {
    const normalizedInn = taxApplicationTaxpayerInn.replace(/[^\d]/g, "");
    return requiredDocumentField(taxApplicationTaxpayerFullName, "налоговое заявление, заявитель") ?? (taxApplicationForm === "legacy_2021_2023" && normalizedInn.length !== 10 && normalizedInn.length !== 12 ? "Для старой налоговой справки укажите 10- или 12-значный ИНН заявителя." : null) ?? (normalizedInn && normalizedInn.length !== 10 && normalizedInn.length !== 12 ? "ИНН заявителя должен содержать 10 или 12 цифр." : null) ?? (taxApplicationForm === "knd_1151156" && normalizedInn && normalizedInn.length !== 12 ? "Для КНД 1151156 ИНН физического лица должен быть 12-значным. Если ИНН нет, оставьте поле пустым и заполните документ заявителя." : null) ?? (isDateInputValue(taxApplicationTaxpayerBirthDate) ? null : "Укажите дату рождения заявителя в формате календарной даты.") ?? requiredDocumentField(taxApplicationTaxpayerIdentityDocument, "налоговое заявление, документ заявителя") ?? (taxApplicationRelationship === "self" || taxApplicationAuthorityDocument.trim() ? null : "Для заявления представителя укажите документ, подтверждающий полномочия.") ?? requiredDocumentField(taxApplicationContact, "налоговое заявление, контакт или канал выдачи") ?? (isDateTimeLocalInputValue(taxApplicationRequestedAt) ? null : "Укажите дату и время заявления через календарь.") ?? (taxApplicationDuplicateWarningAccepted ? null : "Подтвердите, что администратор проверит отсутствие повторной справки по тем же расходам.");
  }
  if (kind === "informed_consent") {
    ...
    return requiredDocumentField(informedConsentIntervention, "информированное согласие, вмешательство") ?? requiredDocumentField(effectiveArea, "информированное согласие, область или зубы") ?? requiredDocumentField(effectiveIndication, "информированное согласие, диагноз или показание") ?? requiredDocumentField(informedConsentExpectedBenefit, "информированное согласие, ожидаемая польза") ?? (documentTextLines(informedConsentRisks).length ? null : "Добавьте разъясненные риски для информированного согласия.") ?? (documentTextLines(informedConsentAlternatives).length ? null : "Добавьте альтернативы лечения для информированного согласия.") ?? (documentTextLines(informedConsentAftercare).length ? null : "Добавьте рекомендации после вмешательства для информированного согласия.") ?? requiredDocumentField(effectiveDoctor, "информированное согласие, врач") ?? requiredDocumentField(informedConsentConfirmedAt, "информированное согласие, дата подтверждения") ?? (informedConsentQuestionsAnswered ? null : "Подтвердите, что пациент получил ответы на вопросы перед согласием.") ?? (informedConsentRisksUnderstood ? null : "Подтвердите, что пациент понял риски, ограничения и прогноз.") ?? (informedConsentWithdrawUnderstood ? null : "Подтвердите, что пациенту объяснено право отказаться до вмешательства.");
  }
  if (kind === "procedure_specific_consent_packet") {
    ...
    return requiredDocumentField(procedureConsentProcedureName, "процедурное согласие, процедура") ?? requiredDocumentField(effectiveArea, "процедурное согласие, область или зубы") ?? requiredDocumentField(effectiveIndication, "процедурное согласие, показание") ?? (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? (documentTextLines(procedureConsentPatientRiskFactors).length ? null : "Добавьте персональные факторы риска пациента для процедурного согласия.") ?? (documentTextLines(procedureConsentSpecificRisks).length ? null : "Добавьте процедурные риски для процедурного согласия.") ?? (documentTextLines(procedureConsentAlternatives).length ? null : "Добавьте альтернативы лечения для процедурного согласия.") ?? (documentTextLines(procedureConsentAftercare).length ? null : "Добавьте ограничения и рекомендации после процедуры.") ?? requiredDocumentField(effectiveDoctor, "процедурное согласие, врач") ?? requiredDocumentField(procedureConsentConfirmedAt, "процедурное согласие, дата подтверждения") ?? (procedureConsentQuestionsAnswered ? null : "Подтвердите, что пациент получил ответы на вопросы по процедуре.") ?? (procedureConsentExactProcedureConfirmed ? null : "Подтвердите, что пациенту названа конкретная процедура, зона и объем.") ?? (procedureConsentRisksUnderstood ? null : "Подтвердите, что пациент понял процедурные риски и ограничения.");
  }
  if (kind === "treatment_plan") {
    return requiredDocumentField(treatmentPlanClinicalReasonValue(), "план лечения, повод обращения") ?? requiredDocumentField(treatmentPlanDiagnosisSummaryValue(), "план лечения, диагноз или клиническое основание") ?? requiredDocumentField(treatmentPlanTeethOrAreaValue(), "план лечения, зубы или область") ?? (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? (documentTextLines(treatmentPlanGoals).length ? null : "Добавьте цели лечения.") ?? (treatmentPlanStageRows().length ? null : "Добавьте этапы плана лечения.") ?? (treatmentPlanTotalRubValue() > 0 ? null : "Укажите ориентировочную стоимость плана лечения.") ?? (documentTextLines(treatmentPlanAlternatives).length ? null : "Добавьте альтернативы плана лечения.") ?? (documentTextLines(treatmentPlanRisks).length ? null : "Добавьте риски и ограничения плана лечения.") ?? requiredDocumentField(treatmentPlanPrognosis, "план лечения, прогноз и ограничения") ?? requiredDocumentField(treatmentPlanControlPlan, "план лечения, контроль") ?? requiredDocumentField(treatmentPlanDoctorFullNameValue(), "план лечения, врач") ?? requiredDocumentField(treatmentPlanPlannedAt, "план лечения, дата") ?? (treatmentPlanQuestionsAnswered ? null : "Подтвердите, что пациент получил ответы на вопросы.") ?? (treatmentPlanSeparateConsentAcknowledged ? null : "Подтвердите, что план не заменяет отдельное согласие.") ?? (treatmentPlanNewApprovalAcknowledged ? null : "Подтвердите, что изменение плана требует нового согласования.");
  }
  if (kind === "treatment_plan_acceptance") {
    return requiredDocumentField(treatmentAcceptanceClinicalGoal, "согласование плана, клиническая цель") ?? requiredDocumentField(treatmentAcceptanceDiagnosisSummary.trim() || dashboard?.activeVisit.diagnosis || dashboard?.activeVisit.complaint || "", "согласование плана, диагноз или основание") ?? requiredDocumentField(treatmentAcceptanceTeethOrArea.trim() || inferredTreatmentArea || "", "согласование плана, зубы или область") ?? (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? (treatmentAcceptanceStageRows().length ? null : "Добавьте этапы согласованного плана лечения.") ?? (treatmentAcceptanceTotalRubValue() > 0 ? null : "Укажите ориентировочную стоимость согласованного плана.") ?? requiredDocumentField(treatmentAcceptanceEstimateValidUntil, "согласование плана, срок действия сметы") ?? requiredDocumentField(treatmentAcceptancePaymentTerms, "согласование плана, условия оплаты") ?? (documentTextLines(treatmentAcceptanceRejectedAlternatives).length ? null : "Добавьте отклоненные или отложенные альтернативы.") ?? (documentTextLines(treatmentAcceptanceRisks).length ? null : "Добавьте риски и ограничения плана.") ?? requiredDocumentField(treatmentAcceptanceWarrantyTerms, "согласование плана, гарантия и контроль") ?? requiredDocumentField(treatmentAcceptanceDoctorFullName.trim() || activeDoctor?.fullName || "", "согласование плана, врач") ?? requiredDocumentField(treatmentAcceptanceAcceptedAt, "согласование плана, дата") ?? (treatmentAcceptanceQuestionsAnswered ? null : "Подтвердите, что пациент получил ответы на вопросы.") ?? (treatmentAcceptanceAlternativesUnderstood ? null : "Подтвердите, что пациент понимает альтернативы.") ?? (treatmentAcceptanceCostChangeUnderstood ? null : "Подтвердите, что пациент понимает возможность изменения стоимости.") ?? (treatmentAcceptanceRevisionAcknowledged ? null : "Подтвердите, что существенное изменение плана требует нового согласования.");
  }
  if (kind === "post_visit_recommendations") {
    return requiredDocumentField(postVisitProcedureNameValue(), "рекомендации после приема, процедура") ?? requiredDocumentField(postVisitToothOrAreaValue(), "рекомендации после приема, область") ?? requiredDocumentField(postVisitPerformedAt, "рекомендации после приема, дата приема") ?? requiredDocumentField(postVisitDoctorFullNameValue(), "рекомендации после приема, врач") ?? (documentTextLines(postVisitAllowedAfter).length ? null : "Добавьте, когда пациенту можно пить, есть и возвращаться к нагрузке.") ?? (documentTextLines(postVisitRestrictions).length ? null : "Добавьте временные ограничения после приема.") ?? (documentTextLines(postVisitMedicationAndRinsePlan).length ? null : "Добавьте назначения, полоскания или явно укажите, что назначений нет.") ?? (documentTextLines(postVisitHygieneInstructions).length ? null : "Добавьте правила гигиены после приема.") ?? (documentTextLines(postVisitNutritionInstructions).length ? null : "Добавьте рекомендации по питанию.") ?? (documentTextLines(postVisitUrgentWarningSigns).length ? null : "Добавьте тревожные признаки для срочной связи с клиникой.") ?? requiredDocumentField(postVisitClinicContactInstruction, "рекомендации после приема, контакт клиники") ?? requiredDocumentField(postVisitTelegramSummary, "рекомендации после приема, краткий текст для Telegram") ?? (postVisitPrintedCopyReceived ? null : "Подтвердите, что пациент получил рекомендации.") ?? (postVisitUrgentSignsUnderstood ? null : "Подтвердите, что пациент понимает тревожные признаки.") ?? (postVisitTelegramSafe ? null : "Подтвердите, что текст безопасен для Telegram и не содержит лишних медицинских подробностей.");
  }
  if (kind === "anesthesia_consent_log") {
    return requiredDocumentField(anesthesiaMethod, "анестезия, метод") ?? requiredDocumentField(anesthesiaAnesthetic, "анестезия, препарат") ?? requiredDocumentField(anesthesiaZone, "анестезия, зона") ?? requiredDocumentField(anesthesiaAllergyStatus, "анестезия, аллергоанамнез") ?? requiredDocumentField(anesthesiaDoseTime, "анестезия, время введения") ?? requiredDocumentField(anesthesiaDoseMl, "анестезия, доза") ?? (anesthesiaRisksExplained ? null : "Подтвердите, что пациенту объяснены риски и ограничения анестезии.") ?? (anesthesiaAllergyRestrictionsChecked ? null : "Подтвердите, что аллергии, лекарства и ограничения проверены до введения.") ?? (anesthesiaConsentConfirmed ? null : "Подтвердите согласие пациента на выбранную местную анестезию.");
  }
  if (kind === "prescription_medication_order") {
    return (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? requiredDocumentField(prescriptionMedication, "назначение, препарат") ?? requiredDocumentField(prescriptionDosage, "назначение, дозировка") ?? requiredDocumentField(prescriptionInstructions, "назначение, режим приема") ?? requiredDocumentField(prescriptionDuration, "назначение, длительность") ?? (documentTextLines(prescriptionSafetyNotes).length ? null : "Добавьте хотя бы одну памятку пациенту для назначения.") ?? requiredDocumentField(prescriptionUrgentContactReason, "назначение, когда срочно связаться");
  }
  if (kind === "lab_work_order") {
    return (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? requiredDocumentField(labWorkType, "лаборатория, вид работы") ?? requiredDocumentField(labTeethOrArea, "лаборатория, зубы или зона") ?? requiredDocumentField(labMaterial, "лаборатория, материал") ?? requiredDocumentField(labShade, "лаборатория, цвет") ?? requiredDocumentField(labSource, "лаборатория, источник данных") ?? requiredDocumentField(labDeadline, "лаборатория, срок");
  }
  if (kind === "photo_video_consent") {
    return (photoVideoMaterials.length ? null : "Отметьте хотя бы один тип фото, видео или снимков.") ?? (photoVideoClinicalRecordUseConfirmed ? null : "Подтвердите, что фото, видео и снимки вносятся в медицинскую карту пациента.") ?? (photoVideoAnonymizationConfirmed ? null : "Подтвердите, что внешнее использование возможно только после обезличивания, кроме отдельно разрешенной узнаваемой публикации.") ?? requiredDocumentField(photoVideoRevocationChannel, "фото/видео, порядок отзыва согласия") ?? (photoVideoRecognizablePublicationAllowed && !photoVideoMarketingUseAllowed && !photoVideoEducationUseAllowed ? "Публикация узнаваемых материалов возможна только вместе с отдельным разрешением на обучение или маркетинг." : null);
  }
  if (kind === "xray_cbct_referral") {
    return (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? requiredDocumentField(xrayArea, "снимок, область") ?? requiredDocumentField(xrayClinicalQuestion, "снимок, клинический вопрос") ?? requiredDocumentField(xrayIndication, "снимок, показание") ?? requiredDocumentField(xraySafetyNotes, "снимок, ограничения и защита") ?? requiredDocumentField(xrayRequestedBy.trim() || activeDoctor?.fullName || "", "снимок, назначивший врач");
  }
  if (kind === "outpatient_medical_card_025u") {
    return requiredDocumentField(clinicProfileDraft.legalName.trim() || clinicProfileDraft.clinicName.trim(), "карта 025/у, медорганизация") ?? requiredDocumentField(outpatient025uMedicalCardNumberValue(), "карта 025/у, номер медицинской карты") ?? requiredDocumentField(outpatient025uOpenedAt, "карта 025/у, дата открытия") ?? requiredDocumentField(recordExtractPeriodStart, "карта 025/у, период с") ?? requiredDocumentField(recordExtractPeriodEnd, "карта 025/у, период по") ?? (outpatient025uSourceVisitIdsValue().length ? null : "Добавьте источник подписанной медицинской записи для карты 025/у.") ?? requiredDocumentField(documentPatient?.fullName ?? "", "карта 025/у, пациент") ?? requiredDocumentField(recordExtractComplaintAndAnamnesisValue(), "карта 025/у, жалобы и анамнез") ?? requiredDocumentField(recordExtractObjectiveStatusValue(), "карта 025/у, объективный статус") ?? requiredDocumentField(recordExtractDiagnosisValue(), "карта 025/у, диагноз") ?? (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам для карты 025/у.") ?? requiredDocumentField(recordExtractTreatmentProvidedValue(), "карта 025/у, проведенное лечение") ?? requiredDocumentField(recordExtractRecommendations, "карта 025/у, назначения и рекомендации") ?? requiredDocumentField(recordExtractDoctorFullName.trim() || activeDoctor?.fullName || "", "карта 025/у, врач") ?? (recordExtractPreparedFromSignedRecords ? null : "Подтвердите, что карта 025/у собрана из подписанных медицинских записей.") ?? (outpatient025uOfficialForm274nChecked ? null : "Подтвердите сверку карты 025/у с приказом Минздрава N 274н.") ?? (outpatient025uThirdPartyDataChecked ? null : "Подтвердите, что лишние данные третьих лиц для карты 025/у исключены.");
  }
  if (kind === "medical_record_extract") {
    const sourceVisitIds = documentTextLines(recordExtractSourceVisitIds);
    return requiredDocumentField(recordExtractPeriodStart, "выписка, период с") ?? requiredDocumentField(recordExtractPeriodEnd, "выписка, период по") ?? (sourceVisitIds.length || dashboard?.activeVisit.id ? null : "Добавьте источник медицинской записи для выписки.") ?? requiredDocumentField(recordExtractComplaintAndAnamnesisValue(), "выписка, жалобы и анамнез") ?? requiredDocumentField(recordExtractObjectiveStatusValue(), "выписка, объективный статус") ?? requiredDocumentField(recordExtractDiagnosisValue(), "выписка, диагноз") ?? (clinicalToothRowsValue().length ? null : "Добавьте клинические строки по зубам или сегментам.") ?? requiredDocumentField(recordExtractTreatmentProvidedValue(), "выписка, проведенное лечение") ?? requiredDocumentField(recordExtractRecommendations, "выписка, рекомендации") ?? requiredDocumentField(recordExtractDoctorFullName.trim() || activeDoctor?.fullName || "", "выписка, врач") ?? requiredDocumentField(recordExtractRecipientFullName.trim() || documentPatient?.fullName || "", "выписка, получатель") ?? requiredDocumentField(recordExtractRecipientAuthority, "выписка, основание выдачи") ?? requiredDocumentField(recordExtractIssuedAt, "выписка, дата") ?? (recordExtractPreparedFromSignedRecords ? null : "Подтвердите, что выписка собрана из подписанных медицинских записей.") ?? (recordExtractThirdPartyDataChecked ? null : "Подтвердите, что лишние данные третьих лиц исключены.");
  }
  if (kind === "medical_record_copy_request") {
    return (documentTextLines(copyRequestDocumentTypes).length ? null : "Добавьте состав запрошенных медицинских документов.") ?? requiredDocumentField(copyRequestRecipientFullName.trim() || documentPatient?.fullName || "", "запрос копий, получатель") ?? requiredDocumentField(copyRequestRecipientIdentityDocument, "запрос копий, документ получателя") ?? requiredDocumentField(copyRequestRecipientAuthority, "запрос копий, основание полномочий") ?? requiredDocumentField(copyRequestRequestedAt, "запрос копий, дата запроса") ?? requiredDocumentField(copyRequestContactForDelivery, "запрос копий, контакт и канал выдачи") ?? (copyRequestIdentityVerified ? null : "Подтвердите проверку личности получателя.") ?? (copyRequestThirdPartyDataChecked ? null : "Подтвердите, что лишние данные третьих лиц будут исключены.");
  }
  if (kind === "visit_attendance_certificate") {
    return requiredDocumentField(attendanceStartedAtValue(), "справка о посещении, начало приема") ?? requiredDocumentField(attendanceEndedAtValue(), "справка о посещении, окончание приема") ?? requiredDocumentField(attendancePurpose, "справка о посещении, цель выдачи") ?? requiredDocumentField(attendanceIssuedAt, "справка о посещении, дата выдачи") ?? requiredDocumentField(attendanceSignedByValue(), "справка о посещении, подписант") ?? requiredDocumentField(attendanceSignedByRole, "справка о посещении, должность подписанта") ?? (attendanceDiagnosisDisclosureExcluded ? null : "Подтвердите, что диагноз и план лечения не раскрываются в справке.") ?? (attendanceNotSickLeaveAcknowledged ? null : "Подтвердите, что справка не заменяет листок нетрудоспособности.");
  }
  if (kind === "medical_document_release_receipt") {
    return requiredDocumentField(selectedReleaseSourceRequestDocumentId, "выдача документов, выданный запрос на копии") ?? requiredDocumentField(releaseRecipientFullName, "выдача документов, получатель") ?? requiredDocumentField(releaseRecipientIdentityDocument, "выдача документов, документ получателя") ?? requiredDocumentField(releaseRecipientAuthority, "выдача документов, основание полномочий") ?? (documentTextLines(releaseDocumentTypes).length ? null : "Добавьте состав выдаваемых медицинских документов.") ?? requiredDocumentField(releaseDeliveredAt, "выдача документов, дата и время") ?? requiredDocumentField(releaseProtectionNote, "выдача документов, защита передачи") ?? (releaseThirdPartyDataChecked ? null : "Подтвердите, что лишние данные третьих лиц исключены.");
  }
  if (kind === "payment_refund_correction_request") {
    const requestedAmount = normalizeRubAmountInput(refundAmountRub);
    return requiredDocumentField(refundSelectedPaymentId, "возврат/коррекция, исходный платеж") ?? (requestedAmount !== null && requestedAmount > 0 ? null : rubAmountInputMissingStep(refundAmountRub, "Укажите сумму возврата или коррекции больше нуля.", "Укажите сумму возврата или коррекции целыми рублями без копеек.")) ?? requiredDocumentField(refundReason, "возврат/коррекция, основание") ?? requiredDocumentField(refundRecipientFullName, "возврат/коррекция, получатель") ?? requiredDocumentField(refundRecipientIdentityDocument, "возврат/коррекция, документ получателя") ?? requiredDocumentField(refundOriginalFiscalReceiptNumber, "возврат/коррекция, исходный фискальный чек") ?? requiredDocumentField(refundAccountantDecision, "возврат/коррекция, решение ответственного");
  }
  if (kind === "personal_data_processing_consent") {
    ...
    return requiredDocumentField(operatorName, "ПДн, оператор клиники") ?? (operatorInn.length === 10 || operatorInn.length === 12 ? null : "ИНН оператора ПДн должен содержать 10 или 12 цифр.") ?? requiredDocumentField(clinicProfileDraft.address, "ПДн, адрес оператора") ?? (documentTextLines(personalDataPurposes).length ? null : "Добавьте цели обработки персональных данных.") ?? (documentTextLines(personalDataCategories).length ? null : "Добавьте категории персональных данных.") ?? (documentTextLines(personalDataActions).length ? null : "Добавьте действия с персональными данными.") ?? requiredDocumentField(personalDataTransferRules, "ПДн, правила передачи третьим лицам") ?? requiredDocumentField(personalDataRetentionPeriod, "ПДн, срок хранения") ?? requiredDocumentField(personalDataRevocationChannel, "ПДн, порядок отзыва") ?? requiredDocumentField(personalDataConsentGivenAt, "ПДн, дата согласия") ?? (personalDataVoluntaryConsentConfirmed ? null : "Подтвердите добровольное согласие пациента на обработку ПДн.") ?? (personalDataMedicalProcessingAcknowledged ? null : "Подтвердите, что пациент понимает обработку медицинских данных.");
  }
  if (kind === "medical_intervention_refusal") {
    return requiredDocumentField(refusalIntervention, "отказ, вмешательство") ?? requiredDocumentField(refusalClinicalIndication, "отказ, клиническое показание") ?? (documentTextLines(refusalExplainedRisks).length ? null : "Добавьте разъясненные риски отказа.") ?? (documentTextLines(refusalAlternatives).length ? null : "Добавьте предложенные альтернативы.") ?? (documentTextLines(refusalUrgentWarningSigns).length ? null : "Добавьте тревожные признаки для срочного обращения.") ?? requiredDocumentField(refusalDoctorFullName.trim() || activeDoctor?.fullName || "", "отказ, врач") ?? requiredDocumentField(refusalConfirmedAt, "отказ, дата подтверждения") ?? (refusalConsequencesUnderstood ? null : "Подтвердите, что пациент понял последствия отказа.") ?? (refusalSecondOpinionOffered ? null : "Подтвердите, что пациенту предложено второе мнение или альтернатива.") ?? (refusalEmergencyCareExplained ? null : "Подтвердите, что пациенту объяснено, когда нужна экстренная помощь.");
  }
  return null;
}
...
async function createDocument(kind: GeneratedDocument["kind"]) {
  ...
  const eligiblePaymentReceiptIdSet = new Set(eligiblePaymentReceiptPayments.map(payment => payment.id));
  const selectedPaymentReceiptIdsForDocument = requiresPaymentReceiptSelection ? selectedPaymentReceiptIds.filter(paymentId => eligiblePaymentReceiptIdSet.has(paymentId)) : [];
  ...
  const linkActiveVisit = metadata.requiresVisit || metadata.group === "payment" || metadata.group !== "tax" && metadata.amountSource !== "none";
  if (linkActiveVisit && !documentPatientMatchesActiveVisit) {
    setError(`Документ «${metadata.label}» требует активного приема пациента ${documentPatient.fullName}. Сейчас открыт прием другого пациента, поэтому система не создаст документ с чужой привязкой к приему. Откройте нужный прием или выберите документ без привязки к визиту.`);
    return;
  }
  const plannedAmount = activeTreatmentPlanItems.filter(item => item.status !== "cancelled").filter(item => !dashboard.activeVisit.id || item.visitId === dashboard.activeVisit.id).reduce((total, item) => total + Math.max(0, item.unitPriceRub * item.quantity - item.discountRub), 0) || null;
  const paidAmount = activePayments.filter(payment => payment.status === "paid").filter(payment => {
    ...
    return paymentTaxYearForUi(payment) === taxDocumentYear && (!selectedTaxDocumentPayerKey || taxPaymentPayerKeyForUi(payment) === selectedTaxDocumentPayerKey);
  }).reduce((total, payment) => total + payment.amountRub, 0) || null;
  if (amountSource === "paid" && !paidAmount) {
    setError(metadata.group === "tax" ? `Для налогового документа нужна фактическая оплата за ${taxDocumentYear} год. План лечения и оплаты других лет не подходят.` : "Для этого документа нужна фактическая оплата. План лечения или примерная сумма не подходят.");
    return;
  }
  if (kind === "payment_refund_correction_request" && documentPayload?.paymentRefundCorrection && paidAmount && documentPayload.paymentRefundCorrection.amountRub > paidAmount) {
...
async function updateDocumentStatus(documentId: string, action: "issue" | "void", payload?: unknown): Promise<boolean> {
  ...
  try {
    const headers = denteClinicalMutationHeaders(payload ? {
      "Content-Type": "application/json"
    } : {});
...
async function downloadIssuedDocumentHtml(documentId: string, options: {
...
async function recordPayment() {
  ...
  if (taxReadyPaymentRequested) {
    const missingTaxFields = [[paymentFiscalReceiptIssuedAt.trim(), "дата фискального чека"], [explicitFiscalFn, "ФН"], [explicitFiscalFd, "ФД"], [explicitFiscalFpd, "ФПД"], [paymentPayerName, "ФИО плательщика"], [explicitPayerBirthDate, "дата рождения плательщика"], [explicitPayerIdentityDocument, "документ плательщика"], [paymentPayerRelation, "родство плательщика"]].filter(([value]) => !value).map(([, label]) => label);
  ...
  const patientIsPayer = (!paymentPayerName || paymentPayerName === documentPatient.fullName) && (!paymentPayerRelation || paymentPayerRelation.toLocaleLowerCase("ru-RU") === "пациент");
  ...
  try {
    const documentForPayment = activeUsableDocuments.find(document => documentKindMetadata[document.kind].group === "payment" && document.kind !== "payment_refund_correction_request" && document.visitId === dashboard.activeVisit.id && (document.totalAmountRub ?? 0) > 0) ?? null;
    ...
    const response = await fetch("/api/billing/payments", {
      ...
      body: JSON.stringify({
        ...
        payerIdentityDocument: taxReadyPaymentRequested ? explicitPayerIdentityDocument : explicitPayerIdentityDocument || administrativePayerDocument || null,
...
function openCommunicationTaskDocumentWorkflow(task: Dashboard["communicationTasks"][number], kind: GeneratedDocument["kind"]) {
  const careTopic = (task.workflowCode ? telegramCareRequestWorkflowCareTopics[task.workflowCode] : null) ?? telegramCareRequestTaskCareTopics[task.title] ?? null;
  ...
  if (dashboard && task.patientId !== dashboard.activeVisit.patientId) {
    ...
    setError(`Открыта форма «${documentLabels[kind]}» для заявки пациента ${taskPatientName}. Перед выпуском документа переключите активный прием на этого пациента, чтобы не создать документ по текущему визиту.`);
  }
}
...
async function createTelegramLinkCode() {
  ...
  if (!subjectId) {
    setError(telegramLinkSubjectType === "patient" ? "Выберите активного пациента для Telegram-кода." : "Выберите сотрудника для Telegram-кода.");
    return;
  }
  ...
  try {
    ...
    await loadTelegramControlPlane({
      silent: true
    });
    setError(null);
  } catch (telegramError) {
...
async function sendTelegramOutboxItem(itemId: string) {
  ...
  try {
    const mutationId = typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `telegram-send-${Date.now()}`;
...
const imagingViewerSaveDetail = [`${imagingViewerAnnotations.length} разметок`, imagingViewerLocalSavedAt ? `локально ${formatTime(imagingViewerLocalSavedAt)}` : "локально ожидает", imagingViewerSession?.serverSavedAt ? `сервер ${formatTime(imagingViewerSession.serverSavedAt)}` : "сервер ожидает", imagingViewerSaveError].filter(Boolean).join(" · ");

const canRetryImagingViewerSave = imagingViewerSessionReady && Boolean(selectedImagingStudy?.id) && (imagingViewerSaveState === "queued" || imagingViewerSaveState === "error");
...
const activeWorkspaceProfile = dashboard.clinicSettings.workspaceProfiles.find(profile => profile.mode === dashboard.clinicSettings.profile.mode) ?? dashboard.clinicSettings.workspaceProfiles[0];

const activeRolePolicy = dashboard.clinicSettings.roleAccessPolicies.find(policy => policy.role === selectedWorkspaceRole) ?? dashboard.clinicSettings.roleAccessPolicies.find(policy => policy.role === "doctor") ?? dashboard.clinicSettings.roleAccessPolicies[0];

const activeQueueRole: StaffRole = selectedWorkspaceRole === "owner" ? "manager" : selectedWorkspaceRole;

const activeRoleQueue = dashboard.shiftIntelligence.roleQueues.find(queue => queue.role === activeQueueRole) ?? dashboard.shiftIntelligence.roleQueues[0];

const activeRoleWritableSections = activeRolePolicy?.canWrite ?? [];

const activeRoleRestrictedSections = activeRolePolicy?.restricted ?? [];

const roleRecommendedActions = dashboard.recommendedActions.filter(action => action.role === selectedWorkspaceRole || selectedWorkspaceRole === "owner" && action.role === "manager");
...
const roleScheduleSuggestions = dashboard.scheduleSuggestions.filter(suggestion => suggestion.ownerRole === selectedWorkspaceRole || selectedWorkspaceRole === "owner" && suggestion.ownerRole === "manager");

const selectedUiLanguageOption = uiLanguageOptions.find(option => option.value === uiLanguage) ?? defaultUiLanguageOption;

const showAdministrationTopActions = currentView === "settings" || selectedWorkspaceRole === "administrator" || selectedWorkspaceRole === "manager" || selectedWorkspaceRole === "owner";
...
const goToVisitDictation = () => {
  ...
  const openDictation = () => {
    ...
    document.querySelector<HTMLTextAreaElement>(".dictation-box textarea")?.focus({
      preventScroll: true
    });
  };
