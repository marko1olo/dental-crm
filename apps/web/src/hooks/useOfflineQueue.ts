...
export const enqueueRequest = async (
	req: Omit<OfflineRequest, "id" | "timestamp">,
) => {
...
export function useOfflineQueue() {
	const processQueue = useCallback(async () => {
		...
		try {
			...
			for (const item of queue.sort((a, b) => a.timestamp - b.timestamp)) {
				try {
					...
					if (res.ok) {
						...
						showToast(
							`Ошибка синхронизации данных (код ${res.status}). Изменения утеряны.`,
							"error",
						);
						await deleteFromQueue(item.id);
					}
					...
					logger.error("Failed to sync offline request", e);
			...
			logger.error("Error processing offline queue", e);
		}
	}, []);
