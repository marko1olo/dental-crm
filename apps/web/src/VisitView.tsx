...
export function VisitFocusBar(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function VisitNextStep(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function VisitSafetyStrip(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function SpecialtyFocusBar(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function DictationBox(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
  ...
  return (
    <div className="dictation-box">
      ...
      >
        <button
          className={
            isServerVoiceRecording ? "primary-button" : "secondary-button"
          }
          type="button"
          style={{
            padding: "12px 16px",
...
export function VisitNotePanel(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function ProtocolLibrary(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
...
export function ToothMap(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
      ...
      {toothRows.map((row, rowIndex) => (
        <div className="tooth-row" key={rowIndex === 0 ? "upper" : "lower"}>
          {row.map((code) => {
            ...
            const cycleState = () => {
              const order: Array<typeof state> = [
                "idle",
                "planned",
                "treatment",
                "watch",
              ...
              const next =
                order[(order.indexOf(state as any) + 1) % order.length];
...
export function CloseChecklist(props: VisitViewProps) {
  const {
    AlertTriangle,
    Bot,
    Check,
    ...
    >
      <div className="close-checklist-head">
        <div>
          ...
          <p>
            {primaryVisitWarning?.actionLabel ?? visitCloseChecklist.nextAction}
          </p>
        </div>
  ...
  return (
    <>
      <div className="panel visit-panel" id="visit">
        ...
        <VisitSafetyStrip {...props} />

        <SpecialtyFocusBar {...props} />

        <DictationBox {...props} />

        <VisitNotePanel {...props} />

        <ProtocolLibrary {...props} />
