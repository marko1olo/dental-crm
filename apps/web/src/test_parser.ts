...
const dashboardMock = {
	...
	patients: [{ id: "pat1", fullName: "Смирнов Алексей" }],
};
...
for (const phrase of bookingPhrases) {
	...
	console.log(
		"Parsed:",
		JSON.stringify(smartBookingParser(phrase, dashboardMock as any), null, 2),
	);
}
...
for (const phrase of visitPhrases) {
	...
	console.log(
		"Parsed:",
		JSON.stringify(parseVisitDictationLocal(phrase), null, 2),
	);
}
...
for (const phrase of patientPhrases) {
	...
	console.log(
		"Parsed:",
		JSON.stringify(parsePatientDictationLocal(phrase), null, 2),
	);
}
