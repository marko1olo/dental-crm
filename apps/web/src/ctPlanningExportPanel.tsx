...
function buildReleaseGate(
	...
	const blockedFact = clinicalFacts.find((fact) => fact?.tone === "blocked");
	const warningFact = clinicalFacts.find((fact) => fact?.tone === "warning");
...
function buildImplantFitHandoff(
	...
	if (!candidate) {
		return {
			...
			detail:
				(plan?.warnings ?? [])[0] ??
				"Нужно сверить библиотеку имплантов с линейками.",
		...
		>
			...
			<div className="ct-planning-export-lanes">
				{(packet?.lanes ?? []).map((lane) => (
					...
					>
						<span>
							{lane?.owner ? (ownerLabels[lane.owner] ?? lane.owner) : ""}
						</span>
