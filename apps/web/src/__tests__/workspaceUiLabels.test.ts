...
import { paymentTaxYearForUi, taxPaymentPayerKeyForUi, paymentFiscalReceiptLabelForUi, clinicalRuleSummaryForUi, completedActContractReferenceForUi } from '../workspaceUiLabels.js';
...
describe('workspaceUiLabels functions', () => {
  describe('paymentTaxYearForUi', () => {
    ...
    it('returns the year from fiscalReceiptIssuedAt if it starts with year', () => {
      assert.strictEqual(paymentTaxYearForUi({
        fiscalReceiptIssuedAt: '2023-10-15',
        paidAt: null
      } as unknown as Pick<Dashboard["payments"][number], "fiscalReceiptIssuedAt" | "paidAt">), 2023);
    });

    it('falls back to paidAt if fiscalReceiptIssuedAt is not present', () => {
      assert.strictEqual(paymentTaxYearForUi({ paidAt: '2022-05-20', fiscalReceiptIssuedAt: null } as unknown as Pick<Dashboard["payments"][number], "fiscalReceiptIssuedAt" | "paidAt">), 2022);
    });

    it('prefers fiscalReceiptIssuedAt over paidAt', () => {
      assert.strictEqual(paymentTaxYearForUi({ fiscalReceiptIssuedAt: '2024-01-01', paidAt: '2023-12-31' }), 2024);
    });

    it('handles ISO date strings', () => {
      assert.strictEqual(paymentTaxYearForUi({ paidAt: '2021-08-10T12:00:00Z' }), 2021);
    });

    it('handles explicit parsing fallback', () => {
      assert.strictEqual(paymentTaxYearForUi({ paidAt: 'October 13, 2014 11:13:00' }), 2014);
    });
  ...
  describe('taxPaymentPayerKeyForUi', () => {
    it('returns inn key if payerInn is present', () => {
      assert.strictEqual(taxPaymentPayerKeyForUi({ payerInn: '1234567890' }), 'inn:1234567890');
    });
  ...
  describe('paymentFiscalReceiptLabelForUi', () => {
    ...
    it('trims fiscalReceiptNumber', () => {
      assert.strictEqual(
        paymentFiscalReceiptLabelForUi({ id: '123', fiscalReceiptNumber: ' 999888 ' }),
        '999888'
      );
    });
  });
  ...
  describe('clinicalRuleSummaryForUi', () => {
    const defaultEval = {
      id: '1', ruleId: 'rule1', resolved: false, severity: 'warning', action: 'show_warning',
      missingRequiredServiceIds: [], ruleName: 'rule', entityId: 'e', entityType: 'visit'
    ...
    it('summarizes empty evaluations', () => {
      assert.deepStrictEqual(clinicalRuleSummaryForUi([], 5), {
        activeRules: 5, evaluatedRules: 0, unresolved: 0, blockers: 0, warnings: 0, requiredServices: 0, coveredRules: 0
      });
    });

    it('counts resolved rules', () => {
      assert.deepStrictEqual(clinicalRuleSummaryForUi([{ ...defaultEval, resolved: true }], 5), {
        activeRules: 5, evaluatedRules: 1, unresolved: 0, blockers: 0, warnings: 0, requiredServices: 0, coveredRules: 1
      });
    });
    ...
    it('counts unresolved blockers and warnings', () => {
      ...
      assert.deepStrictEqual(clinicalRuleSummaryForUi(evaluations, 10), {
        activeRules: 10, evaluatedRules: 4, unresolved: 4, blockers: 2, warnings: 1, requiredServices: 0, coveredRules: 0
      });
    });
  ...
  describe('completedActContractReferenceForUi', () => {
    ...
    it('returns just contractNumber if contractDate is not present', () => {
      assert.strictEqual(
        completedActContractReferenceForUi({
          title: 'Doc Title',
          chainSummary: { paidMedicalServicesContract: { contractNumber: 'C-123' } } as any
        }),
        'C-123'
      );
    });
    ...
    it('returns formatted string if both number and date are present', () => {
      assert.strictEqual(
        completedActContractReferenceForUi({
          title: 'Doc Title',
          chainSummary: { paidMedicalServicesContract: { contractNumber: 'C-123', contractDate: '2023-10-15' } } as any
        }),
        'C-123 от 2023-10-15'
      );
    });
