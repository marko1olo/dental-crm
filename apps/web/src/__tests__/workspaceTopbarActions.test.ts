...
test("необязательные кнопки действительно скрыты до 1140px", () => {
	/*
	 * ЭТО ПРЕМИССА ГАРАНТИИ ПОРЯДКА, А НЕ УКРАШЕНИЕ. Утверждение «до 1140px
	 * переносить нечего» верно ровно потому, что .compact-top-button скрыт в
	 * узком медиавыражении. Уберут правило — гарантия станет ложной молча,
	 * поэтому она проверяется здесь, а не остаётся в комментарии.
	 */
