...
const mockDashboard = {
	...
	appointments: [
		// Conflict at 14:00 today for doc_1
		{
			...
			endsAt: new Date(new Date().setHours(15, 0, 0, 0)).toISOString(),
			status: "scheduled",
		},
	],
} as any;
...
for (const tc of testCases) {
	...
	console.log(
		`- Starts At: ${result.startsAt ? new Date(result.startsAt).toLocaleString() : "Not found"}`,
	);
	console.log(
		`- Ends At:   ${result.endsAt ? new Date(result.endsAt).toLocaleString() : "Not found"}`,
	);
}
console.log("\n=== TESTS FINISHED ===");
