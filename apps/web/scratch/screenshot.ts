	...
	for (const view of views) {
		try {
			if (view.selector) {
				...
				await page.click(
					`text=${view.name === "schedule" ? "Расписание" : view.name === "patients" ? "Пациенты" : view.name === "visit" ? "Прием" : "Документы"}`,
				);
			}
			await page.waitForTimeout(1000); // Wait for render
			await page.screenshot({
				path: path.join(process.cwd(), `scratch/screens/${view.name}.png`),
				fullPage: true,
			});
