...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.includes('<article className="document-payload-card">')) {
    ...
    outputLines.push('  <details className="document-manual-override" style={{ background: "#f8fafc", padding: "12px 16px", borderRadius: "8px", border: "1px solid #e2e8f0", marginTop: "16px" }}>');
    outputLines.push('    <summary style={{ cursor: "pointer", fontWeight: 600, color: "var(--brand-700)", userSelect: "none" }}>✏️ Ручная корректировка полей (развернуть)</summary>');
    outputLines.push('    <div className="document-payload-collapsed-content" style={{ marginTop: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>');
    i = j;
