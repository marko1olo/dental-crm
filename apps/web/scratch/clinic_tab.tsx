			...
			<div className="clinic-config-head">
				...
				>
					...
					<button
						...
						type="button"
						onClick={() => {
							dashboard.clinicSettings.soloDoctorMode =
								!dashboard.clinicSettings.soloDoctorMode;
							alert(
								"Режим " +
									(dashboard.clinicSettings.soloDoctorMode
										? "ВКЛЮЧЕН: Ассистенты скрыты"
										: "ВЫКЛЮЧЕН: Ассистенты показаны") +
									". Требуется перезагрузка для применения.",
							);
							window.location.reload();
						}}
						style={{
							background: dashboard.clinicSettings.soloDoctorMode
								? "var(--teal)"
								: "var(--slate-200)",
							color: dashboard.clinicSettings.soloDoctorMode
								? "#fff"
								: "var(--slate-700)",
							border: "none",
							padding: "6px 12px",
					...
					>
						{dashboard.clinicSettings.soloDoctorMode
							? "✓ Работаю без ассистента"
							: "Работаю с ассистентом"}
					</button>
				</div>
			</div>
