...
function refactorFile(file) {
  ...
  code = code.replace(/<details open>/g, '<details className="smart-details" open>');
  ...
  let matches;
  const inputRegex = /<label>\s*([^{<]+?)\s*<(input|DigitsInput|textarea)([^>]*(?:\/|>[^<]*<\/\2))>\s*<\/label>/g;
  ...
  code = code.replace(inputRegex, (match, labelText, tag, attrs) => {
    ...
    let extraClass = "";
    if (newAttrs.includes('type="date"') || newAttrs.includes('type="time"') || newAttrs.includes('type="datetime-local"')) {
       extraClass = " no-float";
    }
