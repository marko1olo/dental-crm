...
const importIndex = lines.findIndex(line => line.includes('import { PriceDictationBar }'));
if (importIndex !== -1 && !lines.some(l => l.includes('SettingsClinicTab'))) {
    lines.splice(importIndex + 1, 0, importStr);
}

// 2. Replace the tab JSX
const start = lines.findIndex(line => line.includes('settingsTab === "clinic" ? ('));
const nextTabStart = lines.findIndex(line => line.includes('settingsTab === "access" ? ('));
