...
test("Documents Lifecycle Smoke Test: Login, navigate to patient, generate document", async ({
	page,
}) => {
	await page.goto(process.env.WEB_BASE_URL || "http://127.0.0.1:5173");

	// Navigate to Patients section
	const patientsTab = page
		.locator('text="Пациенты"')
		.or(page.locator('text="Patients"'));
