...
telegramStmts.forEach((stmt) => {
	if (stmt.getKind() === SyntaxKind.VariableStatement) {
		stmt.getDeclarations().forEach((d) => {
			if (
				d.getName().startsWith("is") ||
				d.getInitializer()?.getKind() === SyntaxKind.ArrowFunction ||
...
useAppLogicDecl.insertStatements(
	0,
	`const telegramLogic = useTelegramLogic(this_is_a_hack);`,
);
