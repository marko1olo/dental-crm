...
const DATA_DIR =
	"C:/Users/Admin/Downloads/Telegram Desktop/BARABASH_SVETLANA_VIKTOROVNA_09141256/BARABASH_SVETLANA_VIKTOROVNA_09141256/Data";
const PROOFS_DIR = path.join(process.cwd(), "docs/proofs/cbct");
const BRAIN_DIRS = [
	path.join(process.cwd(), "docs/screenshots"),
	process.env.BRAIN_DIR,
].filter(Boolean);

const ALL_ARTIFACT_DIRS = [PROOFS_DIR, ...BRAIN_DIRS];
	...
	for (const dir of ALL_ARTIFACT_DIRS) {
		if (fs.existsSync(dir)) {
			...
			for (const f of files) {
				if (
					f.endsWith(".png") &&
					(f.includes("cbct") || f.includes("studio") || f.includes("implant") || f.includes("v4"))
				) {
...
async function saveScreenshotProof(page, fileName) {
	...
	if (size < 30000) {
		console.error(`[FATAL] Screenshot ${fileName} size too small (${size} bytes < 30KB). Rendering failed!`);
		process.exit(1);
	}
	...
	console.log(`[PROOF CAPTURED]: ${fileName} (${(size / 1024).toFixed(1)} KB) replicated to proofs and brain artifact directories.`);
}
	...
	try {
		...
		for (const vp of viewports) {
			...
			if (vp.isMobile) {
				...
				if (await axialTabBtn.isVisible()) {
					await axialTabBtn.click();
			...
			if (await toggleObliqueBtn.isVisible()) {
			...
			if (await resetAngleBadge.isVisible()) {
			...
			if (await maxAxialBtn.isVisible()) {
				...
				if (await axialContainer.isVisible()) {
			...
			if (vp.isMobile) {
				...
				if (await plannerTabBtn.isVisible()) {
...
run().catch((err) => {
	console.error("[FATAL] CBCT Screenshot capture failed:", err);
	process.exit(1);
});
