...
const ARTIFACT_DIR_LOCAL = "C:/Users/Admin/.gemini/antigravity/brain/0c697a46-7008-4c8b-a346-c30a7e747db1";

for (const dir of [PROOFS_DIR, ARTIFACT_DIR_PARENT, ARTIFACT_DIR_LOCAL]) {
...
async function saveScreenshotProof(page, fileName) {
  ...
  const targetDirs = [PROOFS_DIR, ARTIFACT_DIR_PARENT, ARTIFACT_DIR_LOCAL];
  ...
  for (const dir of targetDirs) {
    ...
    // Also save with cache-busting _live suffix
...
async function run() {
  ...
  try {
    ...
    // 4-State Visual Proof Matrix (PC/Mobile x Dark/Light):
    const viewports = [
    ...
    for (const vp of viewports) {
      ...
      // Wait for volume processing to complete and render physically onto canvas
      ...
      // ──────────────────────────────────────────────────────────────────────────
      // SHOT 1: Clean Diagnostic Mode with Collapsed Right Panel (100% width 4 viewports)
      // ──────────────────────────────────────────────────────────────────────────
      ...
      // Ensure sidebar is closed in diagnostic mode
      ...
      // ──────────────────────────────────────────────────────────────────────────
      // SHOT 2: Oblique MPR with Rotated Slice Axes (Rotation Handles & Angle Indicator)
      // ──────────────────────────────────────────────────────────────────────────
      ...
      if (await toggleObliqueBtn.isVisible().catch(() => false)) {
        await toggleObliqueBtn.click();
      } else {
        // Fallback to programmatic angle setter
        await page.evaluate(() => {
      ...
      // Reset oblique angle back to 0 for subsequent clean shots
      ...
      // ──────────────────────────────────────────────────────────────────────────
      // SHOT 3: Maximized Axial Viewport (Single 100% Viewport with Restore Button)
      // ──────────────────────────────────────────────────────────────────────────
    ...
    await browser.close();
    console.log("\n=== ALL 4-STATE REAL PATIENT CBCT V4 SCREENSHOTS CAPTURED SUCCESSFULLY ===");
  } finally {
    if (viteProcess) {
      viteProcess.kill();
    }
