...
async function main() {
		...
		for (const varDecl of variableDeclarations) {
			...
			if (initializer && initializer.getKind() === SyntaxKind.CallExpression) {
				...
				if (
					...
					expression.getText() === "useAppLogicContext"
				) {
					...
					if (nameNode.getKind() === SyntaxKind.ObjectBindingPattern) {
						...
						for (let i = elements.length - 1; i >= 0; i--) {
							...
							if (DEAD_PROPERTIES.has(propertyName)) {
								console.log(
									`[${sourceFile.getFilePath()}] Removing dead property '${propertyName}'`,
								);
								element.remove();
								totalRemoved++;
								fileModified = true;
						...
						if (bindingPattern.getElements().length === 0) {
							...
							if (statement) {
								console.log(
									`[${sourceFile.getFilePath()}] Removing empty useAppLogicContext() statement`,
								);
								statement.remove();
								fileModified = true;
							}
	...
	console.log(
		`Note: Changes were NOT saved. Uncomment 'await sourceFile.save()' to apply changes.`,
	);
}
