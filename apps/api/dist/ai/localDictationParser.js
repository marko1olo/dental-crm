...
function extractTime(text) {
    ...
    if (m) {
        ...
        const hourMap = { "пер": 12, "вто": 13, "тре": 14, "чет": 15, "пят": 16, "шес": 17, "сед": 18, "вос": 19, "дев": 20, "дес": 21, "оди": 10, "две": 11 };
        if (hourMap[word])
...
function extractCost(text) {
    ...
    if (mWord) {
        ...
        if (sum > 0) {
            if (hasHalf)
                sum += 0.5;
            if (mWord[2].startsWith("тыс") || mWord[1].includes("тысяч") || mWord[1].includes("тыс")) {
                sum *= 1000;
            }
            return sum;
        }
...
function extractEmkSections(text, updates) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (isKeyword) {
            ...
            else if (lowerToken.startsWith("объектив") || lowerToken.startsWith("осмотр"))
                matchedSection = "objectiveStatus";
            else if (lowerToken.startsWith("диагноз"))
                matchedSection = "diagnosis";
            else if (lowerToken.startsWith("лечен") || lowerToken.startsWith("план") || lowerToken.startsWith("рекомендац") || lowerToken.startsWith("проведен"))
...
export function parseDictationLocally(transcript, context) {
    ...
    if (openCardMatch && (openCardMatch[1]?.length ?? 0) > 3) {
        return { action: "open_card", payload: { patientName: openCardMatch[1]?.trim() } };
    }
    const implantMatch = text.match(/(?:поставь|добавь)?\s*имплант(?:ат)?\s*(\d+[\.,]?\d*)\s*(?:на|x|х)\s*(\d+[\.,]?\d*)/i);
    if (implantMatch && !text.includes("зуб")) {
        return { action: "add_implant", payload: { diameter: parseFloat(implantMatch[1]?.replace(',', '.') || "0"), length: parseFloat(implantMatch[2]?.replace(',', '.') || "0") } };
    }
    if (context === "visit") {
