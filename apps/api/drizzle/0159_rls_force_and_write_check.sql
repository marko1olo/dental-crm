...
-- A NULL app.current_tenant makes the ::uuid cast yield NULL, so the comparison
-- is NULL (not true) and the write is rejected -> fail-closed.
-- =============================================================================

ALTER POLICY tenant_isolation ON "advance_deposit_taggings"
  USING (current_setting('app.superuser_bypass', true) = 'on' OR organization_id = current_setting('app.current_tenant', true)::uuid)
...
CREATE POLICY tenant_isolation ON "organizations"
  USING (current_setting('app.superuser_bypass', true) = 'on' OR id = current_setting('app.current_tenant', true)::uuid)
  WITH CHECK (current_setting('app.superuser_bypass', true) = 'on' OR id = current_setting('app.current_tenant', true)::uuid);
