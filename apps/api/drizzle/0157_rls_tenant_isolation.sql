...
-- When app.current_tenant is NOT set (missing_ok returns NULL), the policy passes
-- through to application-level organizationId guards (defence-in-depth, not sole guard).
...
CREATE POLICY tenant_isolation ON "clinics" USING (
  ...
  OR organization_id = current_setting('app.current_tenant', true)::uuid
);
