...
async function callOpenAiCompatibleVisitDraft(input: {
	...
	Partial<VisitNoteDraft> & { _rawToothStates?: Record<string, unknown> | null }
> {
...
3. Сохраняйте все номера зубов (FDI), дозировки анестетиков и названия материалов.`,
	...
	const content = (payload as OpenAiCompletionResponse).choices?.[0]?.message
		?.content;
	...
	return {
		...
		_rawToothStates:
			typeof parsed.toothStates === "object" && parsed.toothStates !== null
				? (parsed.toothStates as Record<string, unknown>)
				: null,
	};
}
