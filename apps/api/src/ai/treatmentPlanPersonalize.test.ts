...
test("personalizeTreatmentPlan tests", async (t) => {
	...
	await t.test(
		...
		async () => {
			...
			assert.match(
				...
				/\*\*Почему важно провести лечение:\*\*/,
			);
			assert.match(
				result.patientHygieneAdvice,
				/\*\*Базовые правила домашней гигиены от DENTE:\*\*/,
			);
		},
	);
					...
					choices: [
						{
							message: {
								content: JSON.stringify({
									patientFriendlyExplanation: "AI Expl",
									patientHygieneAdvice: "AI Adv",
								}),
							},
						},
					],
	...
	await t.test(
		"cascades to fallback providers on primary provider failure",
		async () => {
			process.env.DENTAL_AI_NEURAL_DRAFT = "true";
			process.env.DENTAL_SPEECH_POLISH_PROVIDER = "openai";
						...
						choices: [
							{
								message: {
									content: JSON.stringify({
										patientFriendlyExplanation: "Cascade Expl",
										patientHygieneAdvice: "Cascade Adv",
									}),
								},
							},
						],
	...
	await t.test(
		"returns rule-based fallback when all AI calls fail",
		async () => {
			process.env.DENTAL_AI_NEURAL_DRAFT = "true";
			process.env.DENTAL_SPEECH_POLISH_PROVIDER = "openai";
			process.env.DENTAL_SPEECH_POLISH_API_KEY = "fail-key";
