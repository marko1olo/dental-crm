...
describe("buildKnd1151156Xml", () => {
	...
	test("happy path: generates valid XML for self payer", () => {
		...
		assert.match(
			result.xml,
			/<Документ КНД="1184043"[\s\S]*?ДатаДок="15\.05\.2024"[\s\S]*?ГодУсл="2024"/,
		);
		assert.match(
			result.xml,
			/<СвОргЮЛ[\s\S]*?ИННЮЛ="1234567890"[\s\S]*?КПП="123456789"/,
		);
		assert.match(
			result.xml,
			/<Подписант ПрПодп="1">[\s\S]*?<ФИО Фамилия="Петров" Имя="Петр" Отчество="Петрович"\/>/,
		);
		assert.match(result.xml, /<СвПациент ПризнПац="1"\/>/);
		assert.match(result.xml, /<СведРасхУсл КодУслуг="1" СумОпл="1000\.00"\/>/);
		assert.match(
			result.xml,
			/<СвФЛ ИННФЛ="123456789012" ДатаРожд="01\.01\.1990">[\s\S]*?<ФИО Фамилия="Иванов" Имя="Иван" Отчество="Иванович"\/>/,
		);
	});
	...
	test("happy path: generates valid XML for other payer", () => {
		...
		assert.match(result.xml, /<СвПациент[\s\S]*?ПризнПац="4"/);
		assert.match(
			result.xml,
			/<СвФЛ[\s\S]*?<ФИО Фамилия="Иванова" Имя="Мария" Отчество="Ивановна"\/>/,
		);
		assert.match(
			result.xml,
			/<СвПациент[\s\S]*?<ФИО Фамилия="Иванов" Имя="Иван" Отчество="Иванович"\/>/,
		);
	});
