...
import {
	...
	taxPaymentsForIssueSnapshot,
	taxPaymentYear,
} from "./taxPaymentSnapshot.js";

describe("taxDocumentDuplicateSensitive", () => {
	test("returns true for tax_deduction_certificate", () => {
		assert.strictEqual(
			taxDocumentDuplicateSensitive("tax_deduction_certificate"),
			true,
		);
	});
	...
	test("returns true for legacy_tax_deduction_certificate", () => {
		assert.strictEqual(
			taxDocumentDuplicateSensitive("legacy_tax_deduction_certificate"),
			true,
		);
	});
	...
	test("returns false for other document kinds", () => {
		assert.strictEqual(
			taxDocumentDuplicateSensitive("completed_works_act"),
			false,
		);
		assert.strictEqual(
			taxDocumentDuplicateSensitive("paid_medical_services_contract"),
			false,
		);
		assert.strictEqual(
			taxDocumentDuplicateSensitive("informed_consent"),
			false,
		);
	});
});
...
describe("taxPaymentYear", () => {
	...
	test("uses fiscalReceiptIssuedAt when both are provided", () => {
		assert.strictEqual(
			taxPaymentYear({
				...basePayment,
				fiscalReceiptIssuedAt: "2023-05-01T12:00:00Z",
	...
	test("extracts the year from standard ISO dates starting with 4 digits", () => {
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "2021-02-28T10:00:00Z" }),
			2021,
		);
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "1999-12-31" }),
			1999,
		);
	});
	...
	test("parses valid date strings that do not start with a 4-digit year", () => {
		...
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "Dec 25, 2020" }),
			2020,
		);
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "03/15/2019" }),
			2019,
		);
	});
	...
	test("returns null for completely invalid strings", () => {
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "not a date" }),
			null,
		);
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, paidAt: "----" }),
			null,
		);
	});
	...
	test("returns null for empty strings (and falls back if one is empty)", () => {
		assert.strictEqual(
			taxPaymentYear({ ...basePayment, fiscalReceiptIssuedAt: "", paidAt: "" }),
			null,
		);
		assert.strictEqual(
			taxPaymentYear({
...
describe("taxPaymentsForIssueSnapshot", () => {
	...
	test("returns base selected payments when explicit payment IDs are selected", () => {
		...
		const result = taxPaymentsForIssueSnapshot(
			document,
			payments,
			existingDocuments,
		);
		...
		const payments: Payment[] = [
			{
				...basePayment,
				id: "payment-1",
				fiscalReceiptNumber: "rcpt-1",
			} as Payment,
			{
				...basePayment,
				id: "payment-2",
				fiscalReceiptNumber: "rcpt-2",
			} as Payment,
		];
