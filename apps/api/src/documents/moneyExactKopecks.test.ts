...
import {
	...
	normalizeMoneyValue,
} from "../migration/valueNormalize.js";
...
function snapshotOf(amounts: number[]): TaxPaymentSnapshot {
	const payments = amounts.map((amount, index) =>
		paymentWith(`payment-${index + 1}`, amount),
	);
	return {
...
describe("итог справки для налогового вычета", () => {
	...
	test("копейки итога не теряются и не появляются из ниоткуда", () => {
		assert.equal(
			taxPaymentSnapshotTotalRub(snapshotOf([100.1, 200.2, 300.3])),
			600.6,
		);
		assert.equal(taxPaymentSnapshotTotalRub(snapshotOf([5400.5])), 5400.5);
		assert.equal(
			taxPaymentSnapshotTotalRub(snapshotOf([0.01, 0.01, 0.01])),
			0.03,
		);
...
describe("суммы в выгрузке КНД 1184043 для ФНС", () => {
	...
	test("дрейфующий набор выгружается как 1111.00", () => {
		assert.match(
			xmlFor(Array.from({ length: 20 }, () => 55.55)),
			/СумОпл="1111\.00"/,
		);
	});

	test("копеечная сумма выгружается двумя знаками, а не одним", () => {
		assert.match(xmlFor([5400.5]), /СумОпл="5400\.50"/);
		assert.match(xmlFor([0.01]), /СумОпл="0\.01"/);
	});

	test("круглая сумма сохраняет прежний вид 1000.00", () => {
		assert.match(xmlFor([1000]), /СумОпл="1000\.00"/);
	});
});
...
describe("перенос истории из чужой системы", () => {
	...
	test("вся история из десяти платежей сходится до копейки", () => {
		...
		const rublesTotal = source.reduce(
			(total, text) => total + (normalizeMoneyRubles(text).value ?? 0),
			0,
		);
		const kopecksTotal = source.reduce(
			(total, text) => total + (normalizeMoneyValue(text).value ?? 0),
			0,
		);
