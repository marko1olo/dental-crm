...
describe("taxPaymentSelectionErrorForDocument", () => {
	...
	test("returns null if input kind is not tax paid document", () => {
		const error = taxPaymentSelectionErrorForDocument(
			{ ...baseInput, kind: "completed_works_act" },
			basePayments,
		);
		assert.strictEqual(error, null);
	});
		...
		const error = taxPaymentSelectionErrorForDocument(
			{
				...
				payload: {
					taxDeductionApplication: {
						...
						selectedPaymentIds: [],
					} as unknown as TaxDeductionApplicationPayload,
				},
			},
			basePayments,
		);
	...
	test("returns error if there are duplicates in selected ids", () => {
		const input = {
			...baseInput,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: ["payment-1", "payment-1"] },
			},
		};
		const error = taxPaymentSelectionErrorForDocument(input, basePayments);
		assert.strictEqual(
			error,
			"В выбранных чеках есть дубли. Оставьте каждый фискальный чек один раз.",
		);
	});
	...
	test("returns error if a selected payment is not found", () => {
		const input = {
			...baseInput,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: ["payment-1", "payment-3"] },
			},
		};
		const error = taxPaymentSelectionErrorForDocument(input, basePayments);
		assert.strictEqual(
			error,
			"Выбранный фискальный чек не найден. Обновите экран и выберите чек заново.",
		);
	});
	...
	test("returns error if selected payment belongs to another patient", () => {
		...
		assert.strictEqual(
			error,
			"Выбранный фискальный чек относится к другому пациенту.",
		);
	});
	...
	test("returns error if selected payment is not paid or amount is <= 0", () => {
		...
		assert.strictEqual(
			error,
			"В налоговый документ можно включать только проведенные положительные оплаты.",
		);
		...
		const input2 = {
			...baseInput,
			payload: {
				taxPaymentSelection: { selectedPaymentIds: ["payment-1", "payment-4"] },
			},
		};
	...
	test("returns error if selected payment does not match tax year", () => {
		const payments = [
			...basePayments,
			{
				...
				status: "paid",
				amountRub: 100,
				fiscalReceiptIssuedAt: "2022-01-01",
				payerInn: "123456789012",
			} as Payment,
		];
		const input = {
		...
		assert.strictEqual(
			error,
			"Выбранный фискальный чек не относится к выбранному налоговому году.",
		);
	});
	...
	test("returns error if selected payment does not match payer INN", () => {
		...
		assert.strictEqual(
			error,
			"Выбранный фискальный чек относится к другому ИНН плательщика.",
		);
	});
});
...
describe("paidAmountRubForDocument", () => {
	const basePayments: Payment[] = [
		...
		} as Payment,
		{
			...
			status: "paid",
			amountRub: 300,
			fiscalReceiptIssuedAt: "2023-03-01",
			payerInn: "987654321098",
			visitId: "visit-3",
		} as Payment,
		{
	...
	test("returns 0 if metadata requiresPaidRecord, group is not tax, and no visitId", () => {
		...
		const amount = paidAmountRubForDocument(
			"payment_receipt",
			input,
			basePayments,
		);
