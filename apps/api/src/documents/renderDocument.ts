...
function hasOutpatient025uClinicalRows(
	value: OutpatientMedicalCard025uPayload | null | undefined,
): boolean {
	return Boolean(
...
function clinicalToothSurfaceLabel(
...
function clinicLegalProfileMissingFields(
...
function documentPayloadBlockReason(
...
function issueSignatureModeLabel(
...
function releaseMaterialKindLabel(
...
function releaseDeliveryMethodLabel(
	method: NonNullable<
...
function releaseJournalEntryKindLabel(
...
const treatmentPlanItemStatusLabels: Record<
...
function documentTreatmentPlanItems(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function financialDocumentTreatmentItems(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function treatmentPlanTotalRub(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function financialServiceTable(
	document: GeneratedDocument,
	context: DocumentRenderContext,
	includeStatus = false,
) {
...
function paidTotalForDocument(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
	return paidPaymentsForDocument(document, context).reduce(
...
function installmentRows(
	...
	const totalRub =
		treatmentPlanTotalRub(document, context) ?? document.totalAmountRub ?? 0;
	const paidRub = paidTotalForDocument(document, context);
	const remainingRub = Math.max(0, totalRub - paidRub);
	const rows: string[] = [];
	if (paidRub > 0) {
		rows.push(
			`<tr><td>Оплачено по сохраненным платежам</td><td>${escapeHtml(rub(paidRub))}</td><td>оплачено</td></tr>`,
		);
	}
	if (remainingRub > 0) {
		...
		rows.push(
			`<tr><td>Следующий платеж до ближайшего визита</td><td>${escapeHtml(rub(firstPart))}</td><td>план</td></tr>`,
		);
		if (secondPart > 0) {
			rows.push(
				`<tr><td>Финальный платеж до выдачи акта</td><td>${escapeHtml(rub(secondPart))}</td><td>план</td></tr>`,
			);
		}
	}
	return rows.length
...
function paidMedicalServicesContract(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function namePartsForTax(fullName: string): {
	lastName: string;
	firstName: string;
	middleName: string;
} {
...
function identityDocumentNumberForTax(
...
function identityDocumentIssuedAtForTax(
	value: string | null | undefined,
): string | null {
...
function hasTaxPersonIdentifier(
...
function payerIdentityDocumentForTax(
...
const taxApplicationRelationshipLabels: Record<
...
const taxApplicationDeliveryChannelLabels: Record<
...
function normalizedTaxpayerRelationship(
	value: string | null | undefined,
...
function patientIdentityDocumentForTax(
	payment: Payment | null,
	patient: Patient,
) {
	return (
...
function paidPaymentsForDocument(
	document: GeneratedDocument,
	context: DocumentRenderContext,
): Payment[] {
...
function paymentReceiptSelectionBlockReason(
	document: GeneratedDocument,
	context: DocumentRenderContext,
): string | null {
	...
	const actualTotalRub = selectedPayments.reduce(
		(total, payment) => total + payment.amountRub,
		0,
	);
...
function completedWorksActFiscalReceiptBlockReason(
	document: GeneratedDocument,
	context: DocumentRenderContext,
): string | null {
...
function paymentRefundCorrectionFiscalReceiptBlockReason(
	document: GeneratedDocument,
	context: DocumentRenderContext,
): string | null {
...
function taxDeductionCertificate(
	document: GeneratedDocument,
	patient: Patient,
	context: DocumentRenderContext,
) {
...
function legacyTaxDeductionCertificate(
	document: GeneratedDocument,
	patient: Patient,
	context: DocumentRenderContext,
) {
...
function taxApplicationSelectedPaymentsSummary(
	payments: Payment[],
...
function taxDeductionApplication(
	document: GeneratedDocument,
	patient: Patient,
	context: DocumentRenderContext,
) {
...
function treatmentPlanAcceptanceVariantLabel(
...
function minorLegalRepresentativeConsent(
	document: GeneratedDocument,
	patient: Patient,
) {
...
function treatmentCostEstimate(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function paymentReceipt(
	...
	const paidRub = documentPayments.reduce(
		(total, payment) => total + payment.amountRub,
		0,
	);
	if (payload) {
...
function installmentPaymentSchedule(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function postVisitCareTopicLabel(
...
type Outpatient025uDiagnosisRow =
	OutpatientMedicalCard025uPayload["finalDiagnoses"][number];
type Outpatient025uTextRecord =
	OutpatientMedicalCard025uPayload["dynamicObservationRecords"][number];
type Outpatient025uEventRow =
	OutpatientMedicalCard025uPayload["hospitalizationRows"][number];
type Outpatient025uXrayDoseRow =
	OutpatientMedicalCard025uPayload["xrayDoseRows"][number];

function outpatient025uValue(value: string | null | undefined): string {
	return present(value) ?? "нет данных";
}

function outpatient025uCode(
	value: "1" | "2" | "unknown" | null | undefined,
	labels: Record<"1" | "2", string>,
): string {
	return value && value !== "unknown" ? labels[value] : "нет данных";
}

function outpatient025uDoctorLine(
	doctor: Outpatient025uTextRecord["doctor"] | Outpatient025uEventRow["doctor"],
): string {
	if (!doctor) return "нет данных";
	return compactParts([doctor.fullName, doctor.position, doctor.specialty]);
}

function outpatient025uDiagnosisDoctorLine(
	item: Outpatient025uDiagnosisRow,
): string {
	return compactParts([
		item.doctorFullName,
		item.doctorPosition,
...
function outpatient025uFirstOrRepeat(
	value: Outpatient025uDiagnosisRow["firstOrRepeat"],
): string {
...
function outpatient025uDiagnosisRowsTable(
	rows: readonly Outpatient025uDiagnosisRow[],
	emptyText: string,
): string {
...
function outpatient025uTextRecordsTable(
	rows: readonly Outpatient025uTextRecord[],
	emptyText: string,
): string {
	if (!rows.length) return `<p>${escapeHtml(emptyText)}</p>`;
	return `<table>
...
function outpatient025uEventRowsTable(
	rows: readonly Outpatient025uEventRow[],
	emptyText: string,
): string {
	if (!rows.length) return `<p>${escapeHtml(emptyText)}</p>`;
	return `<table>
...
function outpatient025uXrayDoseTable(
	rows: readonly Outpatient025uXrayDoseRow[],
	emptyText: string,
): string {
	if (!rows.length) return `<p>${escapeHtml(emptyText)}</p>`;
	return `<table>
...
function outpatientMedicalCard025u(
	...
	const payload = document.payload?.outpatientMedicalCard025u as
...
function render025uSpecialistVisitRecord(
	record: OutpatientMedicalCard025uSpecialistVisitRecord,
	index: number,
) {
	return `<section>
...
function render025uMedicalOrganization(
...
function medicalDocumentReleaseReceipt(
	document: GeneratedDocument,
	patient: Patient,
) {
...
function visitAttendanceCertificate(
	document: GeneratedDocument,
	patient: Patient,
) {
...
function paymentRefundCorrectionRequest(
	document: GeneratedDocument,
	context: DocumentRenderContext,
) {
...
function normalizedTaxpayerIdentityPart(
	value: string | null | undefined,
): string {
	return (present(value) ?? "").replace(/\s+/g, " ").toLocaleLowerCase("ru-RU");
}
...
function taxDocumentBlockReason(
	document: GeneratedDocument,
	patient: Patient,
	context: DocumentRenderContext,
): string | null {
...
function documentIssueBlockReasonRaw(
	...
	if (metadata.requiresPaidRecord && metadata.group !== "tax") {
		const paidPayments = paidPaymentsForDocument(document, context);
		const paidTotalRub = paidPayments.reduce(
			(total, payment) => total + payment.amountRub,
			0,
		);
