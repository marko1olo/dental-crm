...
describe("денежные гейты выдачи: квитанция и возврат", () => {
	...
	/**
	 * Контрактное ограничение, которое этим пакетом не закрывается: точную сумму
	 * 600,60 руб. нельзя заявить в квитанции, потому что
	 * paymentReceiptPayloadSchema.totalPaidRub объявлено как z.number().int().
	 * Пока это так, набор оплат с копейками в итоге выдать нельзя ни при какой
	 * арифметике гейта — это дефект контракта, а не гейта, и владелец контракта
	 * увидит здесь, что ограничение всё ещё живо.
	 */
	test("контракт всё ещё отвергает копеечный итог квитанции", () => {
		const payments = [paidPayment(100.1, 0), paidPayment(200.2, 1), paidPayment(300.3, 2)];
