 ...
 */
function isHallucinatedTranscript(text: string): {
...
function publicSpeechProviderFailure(
...
function localSpeechTranscribeUrl(
	providerId: SpeechGatewayProvider,
): string | null {
	const explicit =
...
function localSpeechBridgeProbeState(
	providerId: SpeechGatewayProvider,
): LocalSpeechBridgeProbeState {
...
function localSpeechBridgeProbeFresh(
...
async function runLocalSpeechBridgeProbe(
	providerId: SpeechGatewayProvider,
): Promise<void> {
...
function primeLocalSpeechBridgeProbe(
	providerId: SpeechGatewayProvider,
): LocalSpeechBridgeProbeState {
	const state = localSpeechBridgeProbeState(providerId);
...
function localSpeechBridgeProbeWarning(
	providerId: SpeechGatewayProvider,
): string | null {
	const state = primeLocalSpeechBridgeProbe(providerId);
...
function providerConfigMissingEnvVars(
	providerId: SpeechGatewayProvider,
): string[] {
...
function providerTranscriptionCurrentlyAvailable(
	providerId: SpeechGatewayProvider,
): boolean {
	if (isWiredServerProvider(providerId)) {
		return (
...
function anyProviderTranscriptionCurrentlyAvailable(
...
async function transcribeAssemblyAi(input: {
	...
	for (let attempt = 0; attempt < pollAttempts; attempt += 1) {
		...
		const pollResponse = await fetchWithProviderTimeout(
			`https://api.assemblyai.com/v2/transcript/${transcriptPayload.id}`,
			{
	...
	throw new Error(
		"AssemblyAI не успел обработать фрагмент. Укоротите запись или отправьте позже; локальный черновик сохранен.",
	);
}
