...
function buildChunkInput(
  overrides: Partial<SpeechChunkInput> & { recordingId: string; transcript: string }
): SpeechChunkInput {
  return {
    chunkIndex: 0,
    source: "visit",
 ...
 */
async function countRowsVisibleTo(recordingId: string, viewerOrganizationId: string): Promise<number> {
...
before(async () => {
  ...
  await withFixtureTenant(ORG_A, async () => {
    await db.insert(organizations).values({ id: ORG_A, name: "Клиника точки записи А" });
    await db
      .insert(patients)
      .values({ id: PATIENT_A, organizationId: ORG_A, fullName: "Зотова Лидия Андреевна", birthDate: "1981-09-08" });
  ...
  await withFixtureTenant(ORG_B, async () => {
    await db.insert(organizations).values({ id: ORG_B, name: "Клиника точки записи Б" });
    await db
      .insert(patients)
      .values({ id: PATIENT_B, organizationId: ORG_B, fullName: "Ершов Никита Валерьевич", birthDate: "1990-01-22" });
    await db.insert(visits).values({ id: VISIT_B, organizationId: ORG_B, patientId: PATIENT_B, status: "draft" });
  });
...
describe("гарантия тенант-контекста в точке записи диктовки", () => {
  ...
  it("фрагмент сохраняется, когда вызывающий дал обход, но не дал тенант-контекста", async () => {
    ...
    const chunk = await withSuperuserBypass(async () =>
      recordSpeechTranscriptionChunk(buildChunkInput({ recordingId, transcript: textA }))
    );

    assert.strictEqual(chunk.organizationId, ORG_A, "клиника фрагмента взята не из приема");
    ...
    assert.ok(row, "строки диктовки нет в ai_jobs: под обходом без арендатора WITH CHECK отверг запись");
  ...
  it("POST /api/speech/transcribe-chunk кладёт строку диктовки в ai_jobs", async () => {
    ...
    assert.strictEqual(response.statusCode, 201, `маршрут не принял фрагмент: ${response.body}`);
    const body = JSON.parse(response.body) as { chunk: SpeechTranscriptionChunk };
    assert.strictEqual(body.chunk.organizationId, ORG_A);
    assert.deepStrictEqual(
      body.chunk.warnings.filter((warning) => warning.startsWith(durableWriteFailureWarningPrefix)),
      [],
    ...
    assert.ok(row, `маршрут ответил 201, а строки диктовки в ai_jobs нет после ${attempt} проверок`);
    assert.strictEqual(row.organizationId, ORG_A);
  ...
  it("диктовки двух клиник ложатся каждая своей и чужой не видны", async () => {
    ...
    await recordSpeechTranscriptionChunk(buildChunkInput({ recordingId: recordingA, transcript: textA }));
    ...
    assert.strictEqual(await countRowsVisibleTo(recordingA, ORG_A), 1, "своя строка не видна своей клинике");
    assert.strictEqual(await countRowsVisibleTo(recordingA, ORG_B), 0, "строка клиники А видна клинике Б");
    assert.strictEqual(await countRowsVisibleTo(recordingB, ORG_B), 1, "своя строка не видна своей клинике");
    assert.strictEqual(await countRowsVisibleTo(recordingB, ORG_A), 0, "строка клиники Б видна клинике А");

    // Горячий кэш общий на процесс, поэтому выдача обязана сужаться по клинике.
    assert.strictEqual(listSpeechTranscriptionChunks(recordingA, { organizationId: ORG_A }).length, 1);
    assert.strictEqual(listSpeechTranscriptionChunks(recordingA, { organizationId: ORG_B }).length, 0);
    assert.strictEqual(listSpeechTranscriptionChunks(recordingB, { organizationId: ORG_A }).length, 0);
    assert.strictEqual(listSpeechTranscriptionChunks(recordingB, { organizationId: ORG_B }).length, 1);
  });
});
