...
function installFetchStub(script: StubScript): FetchStub {
  const state: FetchStub = { requests: [], uploadCount: 0, pollCount: 0, deleteCount: 0 };
  ...
  globalThis.fetch = (async (input: unknown, init?: { method?: string; headers?: Record<string, string> }) => {
    ...
    if (url === `${assemblyAiHost}/v2/transcript/${stubTranscriptId}` && method === "GET") {
      ...
      const failStatus = script.pollFailStatusAlways ?? script.pollFailStatusByAttempt?.[state.pollCount];
      if (failStatus) {
        return new Response(JSON.stringify({ error: "stub poll rejected" }), {
          status: failStatus,
          statusText: failStatus === 429 ? "Too Many Requests" : "Service Unavailable"
        });
      }
...
function captureConsole(): { logged: { errors: string[]; warnings: string[] }; restore: () => void } {
  const errors: string[] = [];
...
describe("AssemblyAI: бюджет ожидания задания и удаление аудио у провайдера", () => {
  ...
  it("один 429 посреди опроса больше не убивает живое задание: текст приходит, аудио не загружается второй раз", async () => {
    ...
    process.env.ASSEMBLYAI_POLL_INTERVAL_MS = "1";
    process.env.ASSEMBLYAI_POLL_MAX_INTERVAL_MS = "1";
    const stub = installFetchStub({ processingPolls: 4, pollFailStatusByAttempt: { 3: 429 } });
    const warnings: string[] = [];
    ...
    assert.strictEqual(result.text, "Осмотр зуба 36");
    assert.strictEqual(stub.pollCount, 5, "опрос должен был дожить до пятой попытки");
    assert.strictEqual(stub.uploadCount, 1, "повторной загрузки аудио пациента быть не должно");
    assert.strictEqual(stub.deleteCount, 1, "расшифровка удаляется ровно один раз, на готовом тексте");
    ...
    assert.match(capture.logged.warnings[0] ?? "", /опрос задания N 3 не прошёл/);
    assert.match(capture.logged.warnings[0] ?? "", /запас неудачных опросов 1\/3/);
  ...
  it("оборванный опрос терпится: задание доживает до готового текста", async () => {
    ...
    process.env.ASSEMBLYAI_POLL_INTERVAL_MS = "1";
    process.env.ASSEMBLYAI_POLL_MAX_INTERVAL_MS = "1";
    const stub = installFetchStub({ processingPolls: 3, pollAbortsOnAttempts: [2] });
    const warnings: string[] = [];
    const capture = captureConsole();
    ...
    assert.deepStrictEqual(warnings, []);
    assert.strictEqual(capture.logged.warnings.length, 1);
    assert.match(capture.logged.warnings[0] ?? "", /опрос задания N 2 не прошёл/);
  });
  ...
  it("исчерпанный запас неудачных опросов бросает задание, но не молча: предупреждение врачу и запись в журнал", async () => {
    ...
    assert.ok(error instanceof SpeechProviderRequestError, `ожидался SpeechProviderRequestError, получено ${String(error)}`);
    assert.strictEqual(error.statusCode, 429);
    // Запас 2: первые два отказа терпим, отказ наступает на третьем.
    assert.strictEqual(stub.pollCount, 3, `опросов должно быть 3, получено ${stub.pollCount}`);
    assert.strictEqual(stub.deleteCount, 1, "брошенное задание обязано быть удалено у источника");
    assert.strictEqual(warnings.length, 1, `ожидалось одно предупреждение врачу, получено ${JSON.stringify(warnings)}`);
    ...
    assert.ok(
      capture.logged.errors.some((line) => /\[SpeechGateway\]/.test(line) && /3 опроса задания подряд не прошли/.test(line)),
  ...
  it("бюджет ожидания, истёкший на серии неудачных опросов, называет причину, а не выдаёт медлительность источника", async () => {
    process.env.ASSEMBLYAI_POLL_TIMEOUT_MS = "60";
    ...
    const stub = installFetchStub({ processingPolls: 0, pollFailStatusAlways: 503 });
    const warnings: string[] = [];
    const capture = captureConsole();
    ...
    assert.ok(stub.pollCount >= 2, `опросов должно быть больше одного, получено ${stub.pollCount}`);
    assert.strictEqual(stub.deleteCount, 1);
    assert.strictEqual(warnings.length, 1, `ожидалось одно предупреждение врачу, получено ${JSON.stringify(warnings)}`);
    assert.match(warnings[0] ?? "", /бюджет ожидания истёк/);
    assert.match(warnings[0] ?? "", /у источника временный сбой/);
    assert.ok(
      capture.logged.errors.some((line) => /\[SpeechGateway\]/.test(line) && /бюджет ожидания истёк/.test(line)),
