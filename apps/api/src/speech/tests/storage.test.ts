...
const dictationText =
	"Жалобы на боль в зубе 26 при накусывании. Перкуссия слабоположительная.";
...
before(async () => {
	...
	await withFixtureTenant(ORG_MAIN, async () => {
		await db
			...
			.values({ id: ORG_MAIN, name: "Клиника хранения диктовки" });
		...
		await db.insert(patients).values({
			id: PATIENT_MAIN,
			organizationId: ORG_MAIN,
			fullName: "Тарасова Инна Петровна",
	...
	await withFixtureTenant(ORG_OTHER, async () => {
		await db
			.insert(organizations)
			.values({ id: ORG_OTHER, name: "Соседняя клиника хранения диктовки" });
		await db.insert(patients).values({
			id: PATIENT_OTHER,
			organizationId: ORG_OTHER,
			fullName: "Крылов Артём Игоревич",
...
describe("хранение расшифровок диктовки", () => {
	it("текст переживает перезапуск процесса и читается из PostgreSQL", async () => {
		...
		assert.deepStrictEqual(
			chunk.warnings.filter((warning) =>
				warning.includes("не сохранен в базу"),
			),
			[],
		...
		assert.strictEqual(
			assembleSpeechRecording(recordingId).transcript,
			dictationText,
		);
	});
	...
	it("одна запись диктовки даёт одну строку, а не строку на каждый фрагмент", async () => {
		...
		for (let chunkIndex = 0; chunkIndex < 4; chunkIndex += 1) {
			await recordSpeechTranscriptionChunk(
				buildChunkInput({
					recordingId,
					chunkIndex,
					transcript: `Фрагмент ${chunkIndex + 1}: осмотр продолжается.`,
				}),
			);
		}

		const rows = await withFixtureTenant(
			clinicalScope.organizationId,
			async () =>
				db
					.select({ id: aiJobs.id })
					.from(aiJobs)
		...
		assert.strictEqual(
			rows.length,
			1,
			"на запись диктовки должна приходиться ровно одна строка ai_jobs",
		);
		...
		await withEnv(
			...
			async () => {
				...
				for (const [chunkIndex, transcript] of lines.entries()) {
					await recordSpeechTranscriptionChunk(
						buildChunkInput({ recordingId, chunkIndex, transcript }),
					);
				}
				...
				const envelope = JSON.parse(row.inputText ?? "{}") as {
					chunks?: unknown[];
				};
				assert.strictEqual(
				...
				assert.strictEqual(
					listSpeechTranscriptionChunks(recordingId).length,
					3,
					"из базы восстановлены не все фрагменты",
				);
				assert.strictEqual(
					assembleSpeechRecording(recordingId).transcript,
					lines.join("\n"),
	...
	it("чужие строки voice_transcription не съедают лимит восстановления", async () => {
		...
		const [foreignJob] = await withFixtureTenant(
			scope.organizationId,
			async () =>
				db
					.insert(aiJobs)
					.values({
						...
						sourceLabel: "manual",
						inputText:
							"Распознавание, созданное через POST /api/ai/recognition-jobs.",
						resultText:
							"Распознавание, созданное через POST /api/ai/recognition-jobs.",
						confidence: 0.72,
						warnings: [],
		...
		assert.ok(
			foreignJob,
			"не удалось создать чужую строку voice_transcription",
		);
		...
		await withEnv({ DENTAL_SPEECH_CACHED_RECORDINGS: "1" }, async () => {
			...
			await recordSpeechTranscriptionChunk(
				buildChunkInput({
					...
					transcript: "Своя клиника: осмотр без особенностей.",
				}),
			);
			...
			assert.strictEqual(
				listSpeechTranscriptionChunks(ownRecordingId).length,
				1,
			);
		});
	});
		...
		await withEnv(
			...
			async () => {
				...
				assert.strictEqual(
					row,
					undefined,
					"строка ai_jobs не должна была появиться при отказе записи",
				);
			},
		);
	});
	...
	it("неизвестная уверенность распознавания не выдаётся молча за нулевую", async () => {
		...
		resetSpeechTranscriptionCacheForRestart();
		const base = buildChunkInput({
			recordingId,
			transcript: "Локальный текст браузера без оценки уверенности.",
		});
		await recordSpeechTranscriptionChunk({
			...base,
			confidence: null,
		...
		assert.strictEqual(
			row.confidence,
			0,
			"колонка NOT NULL DEFAULT 0 не умеет хранить «неизвестно»",
		);
		assert.ok(
