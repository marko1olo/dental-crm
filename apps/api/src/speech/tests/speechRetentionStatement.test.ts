...
const asyncDeletionFragment =
	"аудио и расшифровку сервер удаляет отдельным запросом";
...
async function visitDictationStepDetail(): Promise<string> {
	...
	try {
		...
		const response = await app.inject({
			method: "GET",
			url: "/api/system/local-bridges/use-plans",
		});
		assert.strictEqual(
			response.statusCode,
			200,
			`маршрут должен отдать 200, получено ${response.statusCode}`,
		);
		const payload = response.json() as {
			plans: Array<{
				scenario: string;
				steps: Array<{ order: number; detail: string }>;
			}>;
		};
		const dictationPlan = payload.plans.find(
			(plan) => plan.scenario === "visit_dictation",
		);
		...
		assert.ok(
			providerStep,
			"в плане диктовки должен быть шаг выбора источника распознавания",
		);
...
describe("Утверждение продукта о судьбе аудио пациента на серверном маршруте", () => {
	...
	it("смешанная цепочка источников получает оба утверждения, а не одно вместо другого", async () => {
		...
		assert.ok(
			detail.includes(asyncDeletionFragment),
			`нет утверждения об удалении у асинхронного источника: ${detail}`,
		);
		assert.ok(
			detail.includes(oneShotFragment),
			`нет оговорки об одноразовых источниках: ${detail}`,
		);
		assert.ok(
			!detail.includes(falseRetentionClaim),
			`в ответе снова живёт ложное утверждение: ${detail}`,
		);
	});
	...
	it("обещание видимости заменено настоящими адресами записи неудачного удаления", async () => {
		...
		assert.ok(
			!detail.includes(retractedVisibilityClaim),
			`обещание видимости вернулось в ответ: ${detail}`,
		);
	...
	it("цепочка без асинхронного источника не обещает удаления, которого не делает", async () => {
		...
		assert.ok(
			!detail.includes(asyncDeletionFragment),
			`обещание удаления там, где удалять нечем: ${detail}`,
		);
		assert.ok(
			!detail.includes(falseRetentionClaim),
			`в ответе снова живёт ложное утверждение: ${detail}`,
