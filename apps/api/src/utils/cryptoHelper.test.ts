...
describe("cryptoHelper", () => {
  describe("verifyToken", () => {
    ...
    test("returns null for token without a dot", () => {
      assert.strictEqual(verifyToken("invalidtoken", SECRET), null);
    });

    test("returns null for token with too many parts", () => {
      assert.strictEqual(verifyToken("part1.part2.part3", SECRET), null);
    });

    test("returns null for token with missing signature", () => {
      assert.strictEqual(verifyToken("part1.", SECRET), null);
    });

    test("returns null for token with invalid signature (different length)", () => {
      const token = signToken(PAYLOAD, SECRET);
      assert.strictEqual(verifyToken(token + "x", SECRET), null);
    });

    test("returns null for token with invalid signature (same length)", () => {
      const token = signToken(PAYLOAD, SECRET);
      const parts = token.split(".");
      // flip a character in signature to keep same length
      const sig = parts[1];
      const tamperedSig =
    ...
    test("returns null when using wrong secret", () => {
      const token = signToken(PAYLOAD, SECRET);
      assert.strictEqual(verifyToken(token, "wrong-secret"), null);
    });

    test("returns null for expired token", () => {
      // TTL is -1 second, meaning it's already expired
      const token = signToken(PAYLOAD, SECRET, -1);
      assert.strictEqual(verifyToken(token, SECRET), null);
    });

    test("returns null when data is not valid JSON", async () => {
      // import crypto for this manual step
      const { createHmac } = await import("node:crypto");
      const data = Buffer.from("not-json").toString("base64url");
      const signature = createHmac("sha256", SECRET)
        .update(data)
        .digest("base64url");
      assert.strictEqual(verifyToken(`${data}.${signature}`, SECRET), null);
    });
  });
  ...
  describe("verifyCredential", () => {
    test("returns true for legacy plain equality", async () => {
      assert.strictEqual(
        await verifyCredential("password123", "password123"),
        true,
      );
    });
    ...
    test("returns false for incorrect legacy plain equality", async () => {
      assert.strictEqual(
        await verifyCredential("password123", "wrongpassword"),
        false,
      );
    });
