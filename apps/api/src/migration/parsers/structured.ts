...
function flattenValue(
  ...
  if (Array.isArray(value)) {
    ...
    if (allPrimitive) {
      target.set(prefix || "value", value.map((item) => (item === null ? "" : String(item))).join("; "));
      return;
    }
  ...
  if (typeof value === "object") {
    ...
    for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
      flattenValue(nested, prefix ? `${prefix}.${key}` : key, target, depth + 1);
    }
    return;
  }
...
function findRecordArray(root: unknown): { records: unknown[]; path: string } {
  ...
  const visit = (value: unknown, path: string, depth: number): void => {
    ...
    if (Array.isArray(value)) {
      const objectItems = value.filter((item) => item !== null && typeof item === "object" && !Array.isArray(item));
...
function recordsToTable(records: unknown[]): { columns: string[]; rows: string[][]; warnings: string[] } {
  ...
  if (skippedPrimitives > 0) {
    warnings.push(`${skippedPrimitives} элемент(ов) выгрузки не являются объектами; их значение сохранено в колонке «value».`);
  }

  const rows = flattened.map((map) => columnOrder.map((column) => map.get(column) ?? ""));
...
export function parseJsonSource(text: string): StructuredParseResult {
  ...
  if (!trimmed) {
    return { columns: [], rows: [], recordPath: "", warnings: ["Источник JSON пуст."] };
  }

  // JSONL: каждая строка — самостоятельный объект. Признак — несколько строк,
  // каждая из которых начинается с «{».
  const lines = trimmed.split("\n").map((line) => line.trim()).filter(Boolean);
  const looksLikeJsonl = lines.length > 1 && lines.every((line) => line.startsWith("{") && line.endsWith("}"));
  ...
  if (looksLikeJsonl) {
    ...
    return { ...table, recordPath: "", warnings: [...warnings, ...table.warnings] };
  }
  ...
  if (found.records.length === 0) {
    ...
    const table = recordsToTable([parsed]);
    warnings.push("В JSON не найдено массива записей; файл обработан как одна запись.");
    return { ...table, recordPath: "", warnings: [...warnings, ...table.warnings] };
  }

  if (found.path) warnings.push(`Записи найдены по пути «${found.path}».`);
  const table = recordsToTable(found.records);
  return { ...table, recordPath: found.path, warnings: [...warnings, ...table.warnings] };
}
 ...
 */
function parseXmlTree(xml: string): { root: XmlNode | null; warnings: string[] } {
  const warnings: string[] = [];
  ...
  if (/<!DOCTYPE/i.test(working)) {
    warnings.push("Объявление DOCTYPE пропущено: внешние сущности XML не обрабатываются из соображений безопасности.");
  ...
  working = working.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, (_, content: string) =>
  ...
  while ((match = tagPattern.exec(working)) !== null) {
    ...
    for (const attributeMatch of (match[2] ?? "").matchAll(/([\w.:-]+)\s*=\s*(?:"([^"]*)"|'([^']*)')/g)) {
      attributes[attributeMatch[1]!] = decodeXmlEntities(attributeMatch[2] ?? attributeMatch[3] ?? "");
    }
...
function xmlNodeToRecord(node: XmlNode, prefix: string, target: Map<string, string>, depth = 0): void {
  ...
  for (const child of node.children) {
    ...
    xmlNodeToRecord(child, prefix ? `${prefix}.${suffix}` : suffix, target, depth + 1);
  }
}
...
export function parseXmlSource(text: string): StructuredParseResult {
  ...
  if (!root) {
    return { columns: [], rows: [], recordPath: "", warnings: [...warnings, "XML не содержит корневого элемента."] };
  }
  ...
  if (best.nodes.length === 0) {
    warnings.push("В XML не найдено повторяющихся элементов; файл обработан как одна запись.");
  } else {
    warnings.push(`Записи найдены по пути «${best.path}» (${best.nodes.length} шт.).`);
  }
