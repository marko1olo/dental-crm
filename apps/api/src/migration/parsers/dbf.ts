...
const LANGUAGE_DRIVER_ENCODINGS: Record<number, string> = {
	1: "ibm437", // US MS-DOS
	2: "ibm850", // International MS-DOS
	3: "windows-1252", // Windows ANSI
	8: "ibm865", // Danish OEM
	9: "ibm437",
	10: "ibm850",
	11: "ibm437",
	100: "ibm852", // Eastern European MS-DOS
	101: "ibm866", // Russian MS-DOS — типично для DOS-эпохи
	102: "ibm865",
	103: "ibm861",
	106: "ibm737",
	107: "ibm857",
	...
	125: "windows-1255",
	126: "windows-1256",
	200: "windows-1250", // Eastern European Windows
	201: "windows-1251", // Russian Windows — типично для 2000-х
	202: "windows-1254",
	203: "windows-1253",
	135: "ibm852",
	136: "windows-1250",
...
const DBF_VERSION_LABELS: Record<number, string> = {
	2: "FoxBASE",
	3: "dBASE III+ без memo",
	4: "dBASE IV без memo",
	5: "dBASE 5 без memo",
	48: "Visual FoxPro",
	49: "Visual FoxPro с автоинкрементом",
	50: "Visual FoxPro с полем Varchar",
	67: "dBASE IV SQL-таблица",
	123: "dBASE IV с memo",
...
export function looksLikeDbf(buffer: Buffer): boolean {
	...
	if (
		headerLength < HEADER_SIZE + FIELD_DESCRIPTOR_SIZE ||
...
export function parseDbf(
	buffer: Buffer,
	options: DbfParseOptions = {},
): DbfParseResult {
	const warnings: string[] = [];

	if (buffer.length < HEADER_SIZE) {
		throw new Error(
			"Файл короче заголовка DBF — вероятно, он повреждён или загружен не полностью.",
		);
	}
	...
	const versionLabel =
		...
		`неизвестная версия 0x${version.toString(16)}`;
	...
	if (recordLength === 0) {
		throw new Error(
			"Заголовок DBF объявляет нулевую длину записи — файл повреждён.",
		);
	}
	...
	// ------------------------------------------------------------------
	let encoding =
		options.forcedEncoding ?? LANGUAGE_DRIVER_ENCODINGS[languageDriver] ?? "";
	const encodingFromHeader =
		!options.forcedEncoding &&
		Boolean(LANGUAGE_DRIVER_ENCODINGS[languageDriver]);
	if (!encoding) {
	...
	try {
		...
		warnings.push(
			`Кодировка «${encoding}» не поддерживается средой; текст прочитан как windows-1251.`,
		);
		encoding = "windows-1251";
		decoder = new TextDecoder(encoding);
	}
	...
	while (offset + FIELD_DESCRIPTOR_SIZE <= headerLength) {
		...
		const name = asciiDecoder
			.decode(descriptor.subarray(0, 11))
			.replace(/\0.*$/, "")
			.trim();
	...
	const declaredWidth =
		fields.reduce((sum, field) => sum + field.length, 0) + 1;
	...
	const limit = Math.min(
		declaredRecordCount === 0
			? availableRecords
			: Math.min(declaredRecordCount, availableRecords),
		options.maxRows ?? Number.MAX_SAFE_INTEGER,
	);
	if (limit < Math.min(declaredRecordCount, availableRecords)) {
		warnings.push(
			`Прочитаны первые ${limit} записей из ${Math.min(declaredRecordCount, availableRecords)} — сработал предел разбора.`,
		);
	}
		...
		if (flag === RECORD_DELETED) {
			...
			if (
				!warnings.some((warning) =>
					warning.startsWith("Встречены записи с неизвестным"),
				)
			) {
				warnings.push(
					"Встречены записи с неизвестным флагом удаления; они прочитаны как действующие.",
				);
			}
		}
