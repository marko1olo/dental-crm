...
export function detectDelimiter(text: string): { delimiter: string; confidence: number } {
  ...
  return { delimiter: best.delimiter, confidence: Math.min(1, best.score / Math.log2(best.columns + 1)) };
}
 ...
 */
function looksLikeHeader(first: string[], second: string[] | undefined): boolean {
  const cells = first.map((cell) => cell.trim());
  if (cells.length === 0) return false;
  if (cells.every((cell) => cell === "")) return false;

  const numericCells = cells.filter((cell) => cell !== "" && /^[\d\s.,+-]+$/.test(cell)).length;
  // Больше трети числовых ячеек — это данные, а не подписи колонок.
  if (numericCells / cells.length > 0.34) return false;

  const looksLikeDate = cells.filter((cell) => /^\d{1,4}[./-]\d{1,2}[./-]\d{1,4}$/.test(cell)).length;
   ...
   * заголовок.
   */
  const secondNumeric = second.filter((cell) => cell.trim() !== "" && /^[\d\s.,+-]+$/.test(cell.trim())).length;
  const secondDates = second.filter((cell) => /^\d{1,4}[./-]\d{1,2}[./-]\d{1,4}/.test(cell.trim())).length;
  ...
  const averageLength = (values: string[]) =>
    values.reduce((sum, value) => sum + value.trim().length, 0) / Math.max(1, values.length);
  return averageLength(cells) < averageLength(second) * 0.8;
}

/** Приводит имена колонок к уникальным непустым значениям. */
function normalizeColumnNames(header: string[]): { columns: string[]; warnings: string[] } {
  ...
  const columns = header.map((raw, index) => {
    ...
    if (name === "") {
      ...
      warnings.push(`Колонка ${index + 1} без названия — использовано «${name}».`);
    }
    ...
    if (previous !== undefined) {
      ...
      warnings.push(`Колонка «${name}» встречается несколько раз; вторая переименована в «${unique}».`);
      return unique;
    }
...
export function parseDelimited(text: string, forcedDelimiter?: string): DelimitedParseResult {
  const warnings: string[] = [];
  ...
  if (hasHeader) {
    ...
    warnings.push("Строка заголовка не распознана; колонки пронумерованы, сопоставление полей выполнено по содержимому.");
    const width = Math.max(...allRows.map((row) => row.length));
    columns = Array.from({ length: width }, (_, index) => `Колонка ${index + 1}`);
    dataRows = allRows;
  }
  ...
  const oversized = rows.filter((row) => row.join("").length > MIGRATION_MAX_ROW_CHARS).length;
  if (oversized > 0) {
    warnings.push(`${oversized} строк(и) превышают безопасный размер и будут изолированы в карантин.`);
  }
  ...
  return { columns, rows, delimiter: detection.delimiter, hasHeader, warnings, raggedRowNumbers };
}
