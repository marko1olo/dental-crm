...
function looksLikeSqlDump(text: string): boolean {
	...
	return (
		/INSERT\s+INTO/.test(head) ||
		/CREATE\s+TABLE/.test(head) ||
		/^--\s/.test(head)
	);
}
...
function looksLikeFreeText(text: string): boolean {
	...
	const withSeparators = lines.filter(
		(line) => /[;\t|]/.test(line) || (line.match(/,/g) ?? []).length >= 2,
	).length;
 ...
 */
function readSqliteAsTable(
	buffer: Buffer,
	sourceName: string,
): { table: ParsedTable; warnings: string[] } {
	...
	try {
		...
		if (!chosen) {
			return {
				...
				warnings: [
					...inspection.warnings,
					"В базе SQLite нет таблиц с данными.",
				],
			};
		}

		const sample = readSqliteSample(
			temporaryPath,
			chosen.name,
...
function findSuspectRows(
	rows: string[][],
	suspectFromParser: number[],
): number[] {
...
export function parseSource(input: ParseSourceInput): ParsedSource {
	...
	if (input.content && input.content.length > 0) {
		...
		if (
			...
			(input.forcedKind === undefined && looksLikeDbf(buffer))
		) {
			...
			return {
				...
				delimiter: null,
				warnings: [
					`Прочитана таблица ${result.versionLabel}.`,
					...result.warnings,
				],
		...
		if (looksLikeZipContainer(buffer)) {
			...
			const isOds =
				extension === "ods" ||
				buffer.includes(Buffer.from("opendocument.spreadsheet"));
			...
			if (result.sheets.length > 0) {
				const tables = result.sheets.map((sheet) => {
					...
					warnings.push(
						...parsed.warnings.map(
							(warning) => `Лист «${sheet.name}»: ${warning}`,
						),
					);
					return {
						name: sheet.name,
						...
						rows: parsed.rows,
						suspectRowNumbers: findSuspectRows(
							parsed.rows,
							parsed.raggedRowNumbers,
						),
					};
				});
				return {
