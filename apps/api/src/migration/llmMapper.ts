...
import { type ColumnProfile, describeColumnForModel } from "./columnProfile.js";
...
const FIELD_CHECK: Partial<
	Record<MigrationTargetField, keyof ColumnProfile["parseRates"]>
> = {
...
function buildSystemPrompt(
	...
	allowedFields: MigrationTargetField[],
): string {
...
ОРИЕНТИРЫ КЛИНИЧЕСКИХ СТАНДАРТОВ РФ:
- "service.code": код услуги по Приказу Минздрава 804н (номенклатура медицинских услуг, маски "A99.99.999", "A99.999.999", "А99.99.999", "B99.999.999") либо артикул/шифр.
- "service.name": коммерческое или номенклатурное наименование услуги.
- "service.priceRub": стоимость/цена услуги (доля разбора money высокая).
- "visit.diagnosis": диагноз, включая коды МКБ-10 (стоматология: K02 кариес, K04 пульпит/периодонтит, K05 гингивит/пародонтит, K08 др., маски "A99.9", "A99", "А99.9").
...
function buildUserPrompt(profiles: ColumnProfile[]): string {
	const lines = profiles.map(
		(profile, index) => `${index + 1}. ${describeColumnForModel(profile)}`,
	);
...
export async function mapColumnsWithLlm(
	...
	const response = await requestLlmJson<LlmSuggestion[]>({
		...
		validate: (raw) => {
			...
			for (const item of mappings) {
				...
				const sourceColumn =
					typeof entry.sourceColumn === "string" ? entry.sourceColumn : null;
				const targetFieldRaw =
					typeof entry.targetField === "string" ? entry.targetField : null;
				if (!sourceColumn || !targetFieldRaw) continue;

				// Ограничение 3: замкнутый список полей.
				const parsedField =
					migrationTargetFieldSchema.safeParse(targetFieldRaw);
				...
				const rationale =
					typeof entry.rationale === "string"
						? entry.rationale.slice(0, 300)
						: "Предложено языковой моделью.";
				suggestions.push({
					sourceColumn,
					targetField: parsedField.data,
					confidence,
					rationale,
				});
			}
			...
			if (suggestions.length === 0) {
				return {
					ok: false,
					reason:
						"Ни одно предложение не прошло проверку списка полей и имён колонок.",
				};
			}
	...
	result.warnings.push(
		...response.failures.map((failure) => `Языковая модель: ${failure}`),
	);
	...
	const ordered = [...response.value].sort(
	...
	result.stillUnmapped = profiles
		.filter((profile) => !mapped.has(profile.name))
