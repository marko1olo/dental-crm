...
async function processRun(
	...
	try {
		...
		await updateRun(run.id, run.organizationId, {
			...
			heartbeatAt: null,
			finishedAt: new Date(),
			errorClass: isPhaseError
				? (error as MigrationPhaseError).code
			...
			console.error(
				`[migration worker] не удалось записать ошибку прогона ${run.id}:`,
				updateError,
			);
		});
	} finally {
...
export async function startMigrationWorker(): Promise<void> {
	...
	const released = await releaseOwnRuns().catch((error: unknown) => {
		console.error(
			"[migration worker] не удалось освободить свои прогоны:",
			error,
		);
		return 0;
	});
	if (released > 0) {
		console.info(
			`[migration worker] освобождено прогонов после перезапуска: ${released}`,
		);
	}
	...
	const cleanup = async (): Promise<void> => {
		...
		if (removed > 0)
			console.info(
				`[migration worker] удалено просроченных файлов выгрузок: ${removed}`,
			);
	};
	void cleanup();
