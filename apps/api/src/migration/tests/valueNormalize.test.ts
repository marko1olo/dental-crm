...
describe("признак пустого значения", () => {
	...
	test("значения ошибок Excel не переносятся как текст", () => {
		...
		for (const token of [
			...
			assert.equal(
				isNullToken(token, false),
				true,
				`«${token}» должно считаться пустым и для денег`,
			);
		}
	});
...
describe("даты", () => {
	...
	test("колонка со смешанным порядком помечается, а значения не выдумываются", () => {
		...
		assert.ok(
			...
			`уверенность должна быть низкой, получено ${parsed.confidence}`,
		);
	});
	...
	test("форматы старых систем читаются без настройки", () => {
		...
		assert.equal(
			normalizeDateValue("12.03.2019 14:30:00", hint).value,
			"2019-03-12",
		);
		assert.equal(
			normalizeDateValue("2019-03-12T14:30:00Z", hint).value,
			"2019-03-12",
		);
...
describe("перевод местного времени клиники в абсолютное", () => {
	...
	test("исторический переход на летнее время учитывается", () => {
		 ...
		 */
		const winter2010 = storedDateTimeToUtc(
			"2010-01-15T12:00:00",
			"Europe/Moscow",
		);
		const summer2010 = storedDateTimeToUtc(
			"2010-07-15T12:00:00",
			"Europe/Moscow",
		);
...
describe("телефоны", () => {
	test("российские записи приводятся к E.164", () => {
		for (const input of [
			...
			assert.equal(
				normalizePhoneValue(input).value?.e164,
				"+79001234567",
				`не разобрано: ${input}`,
			);
		}
...
describe("имена", () => {
	test("верхний регистр из DOS-систем приводится к обычному", () => {
		assert.equal(
			normalizeNameValue("ИВАНОВ ИВАН ИВАНОВИЧ").value?.fullName,
			"Иванов Иван Иванович",
		);
		assert.equal(
			normalizeNameValue("петрова мария сергеевна").value?.fullName,
	...
	test("смешанный регистр не ломается", () => {
		...
		assert.equal(
			normalizeNameValue("МакДональд Джон").value?.fullName,
			"МакДональд Джон",
		);
	});
	...
	test("двойная фамилия через дефис сохраняет обе заглавные", () => {
		assert.equal(
			normalizeNameValue("ИВАНОВА-ПЕТРОВА АННА").value?.fullName,
			"Иванова-Петрова Анна",
		);
	});

	test("частицы фамилий остаются со строчной", () => {
		assert.equal(
			normalizeNameValue("ВАН ДЕР БЕРГ ЯН").value?.fullName,
			"ван дер Берг Ян",
		);
	});
	...
	test("раздельные колонки фамилии, имени и отчества собираются", () => {
		...
		assert.equal(
			combineNameParts("Иванов", "Иван", null).value?.fullName,
			"Иванов Иван",
		);
	});
});
...
describe("перечисления, пол, флаги", () => {
	test("пол распознаётся во всех встречающихся написаниях", () => {
		for (const male of ["М", "м", "муж", "мужской", "M", "male", "1"]) {
			assert.equal(
				normalizeGenderValue(male).value,
				"male",
				`не распознано: ${male}`,
			);
		}
		for (const female of ["Ж", "жен", "женский", "F", "female", "2"]) {
			assert.equal(
				normalizeGenderValue(female).value,
				"female",
				`не распознано: ${female}`,
			);
		}
...
describe("зубная формула и почта", () => {
	...
	test("почта проверяется, из нескольких адресов берётся первый", () => {
		assert.equal(
			normalizeEmailValue("Ivanov@Example.COM").value,
			"ivanov@example.com",
		);
...
describe("текст", () => {
	test("лишние пробелы сжимаются, повреждение кодировки отклоняется", () => {
		assert.equal(
			normalizeText("  Жалобы   на    боль  ").value,
			"Жалобы на боль",
		);
