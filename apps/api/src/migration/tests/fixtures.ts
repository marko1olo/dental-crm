  ...
  const body = records.map((record, recordIndex) => {
    ...
    fields.forEach((field, fieldIndex) => {
      ...
      if (field.type === "N") {
        ...
        bytes.copy(buffer, offset + Math.max(0, field.length - bytes.length), 0, Math.min(field.length, bytes.length));
      } else {
 ...
 */
export function buildZip(files: Array<{ name: string; content: string }>): Buffer {
  const locals: Buffer[] = [];
 ...
 */
export function buildXlsxFile(sheetName: string, cells: XlsxFixtureCell[]): Buffer {
  ...
  const rowsXml = [...byRow.entries()]
    ...
    .map(([rowNumber, rowCells]) => {
      const cellsXml = rowCells
        .map((cell) => {
          if (cell.type === "n") return `<c r="${cell.ref}"><v>${cell.value}</v></c>`;
          // Стиль 1 объявлен в styles.xml как формат даты (numFmtId 14).
          if (cell.type === "date") return `<c r="${cell.ref}" s="1"><v>${cell.value}</v></c>`;
          const escaped = cell.value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  ...
  const escapedSheetName = sheetName.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/"/g, "&quot;");
  ...
  return buildZip([
    ...
    },
    {
      ...
      content:
        `<?xml version="1.0"?><workbook xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="${escapedSheetName}" sheetId="1" r:id="rId1"/></sheets></workbook>`
    },
    {
