...
function resolve(
	columns: string[],
	rows: string[][],
	options: Parameters<typeof resolveDeterministicMapping>[0] extends never
		? never
		: Partial<Parameters<typeof resolveDeterministicMapping>[0]> = {},
) {
...
function fieldFor(
	...
	return result.columns.find((mapping) => mapping.sourceColumn === column)
		?.targetField;
}
...
describe("опознание системы-источника", () => {
	...
	test("сущность определяется по ключевым колонкам, а не по общим", () => {
		...
		assert.equal(
			detectEntityKind(["Дата", "Жалобы", "Диагноз", "Лечение"], "выгрузка")
				.entityKind,
			"visit",
		);
		assert.equal(
			detectEntityKind(["Дата", "Сумма", "Способ оплаты"], "выгрузка")
				.entityKind,
			"payment",
		);
		assert.equal(
			detectEntityKind(["ФИО", "Дата рождения", "Телефон"], "выгрузка")
				.entityKind,
			"patient",
		);
		assert.equal(
			detectEntityKind(["Услуга", "Цена", "Код услуги"], "выгрузка").entityKind,
			"service",
		);
	});
	...
	test("имя таблицы весит, но одной общей колонки для решения не хватает", () => {
		...
		assert.equal(
			detectEntityKind(["Наименование", "Сумма"], "оплаты_2019.xlsx")
				.entityKind,
			"payment",
		);
	});
...
describe("алгоритмическое сопоставление колонок", () => {
	...
	test("правило профиля системы важнее обобщённого словаря", () => {
		...
		assert.equal(
			result.columns.find((mapping) => mapping.sourceColumn === "PatNum")
				?.decidedBy,
			"vendor_profile",
		);
	});
	...
	test("КОЛОНКА С ОБМАНЧИВЫМ ИМЕНЕМ отклоняется содержимым", () => {
		...
		assert.ok(
			result.warnings.some(
				(warning) =>
					warning.includes("Дата рождения") && warning.includes("отклонено"),
			),
	...
	test("ручная поправка оператора имеет приоритет и не оспаривается данными", () => {
		...
		assert.equal(
			result.columns.find((mapping) => mapping.sourceColumn === "Прочее")
				?.decidedBy,
			"manual",
		);
	});
	...
	test("нераспознанные непустые колонки предлагаются модели, пустые — нет", () => {
		...
		assert.ok(
			names.includes("Kod_L"),
			"непустая нераспознанная колонка должна уйти модели",
		);
		assert.ok(
			!names.includes("Пустая"),
			"пустую колонку модели показывать незачем",
		);
	...
	test("в карту соответствия попадают маски, а не значения пациентов", () => {
		...
		for (const mapping of result.columns) {
			assert.ok(
				...
				`у «${mapping.sourceColumn}» нет примеров формы`,
			);
			for (const sample of mapping.sampleValues) {
				for (const real of realValues) {
					assert.notEqual(
						sample,
						real,
						`в примерах «${mapping.sourceColumn}» осталось настоящее значение`,
					);
				}
				// Ни фрагментов ФИО, ни настоящих цифр телефона.
				assert.ok(
					!/[бвгдежзийклмнопрстуфхцчшщъыьэюя]/.test(sample.replace(/а/g, "")),
					`в примере «${sample}» осталась настоящая буква`,
				);
				assert.ok(
					!/[0-8]/.test(sample),
					`в примере «${sample}» осталась настоящая цифра`,
				);
			}
		}
	});
...
describe("обязательные поля", () => {
	test("ФИО можно собрать либо целиком, либо из частей", () => {
		assert.deepEqual(
			missingRequiredFields("patient", ["patient.fullName"]),
			[],
		);
		assert.deepEqual(
			missingRequiredFields("patient", [
				"patient.lastName",
				"patient.firstName",
			]),
			[],
		...
		assert.deepEqual(missingRequiredFields("patient", ["patient.lastName"]), [
			"patient.fullName",
		]);
	});

	test("платёж без суммы или без пациента загружать нельзя", () => {
		assert.deepEqual(missingRequiredFields("payment", ["payment.patientRef"]), [
			"payment.amountRub",
		]);
		assert.deepEqual(
			missingRequiredFields("payment", [
				"payment.patientRef",
				"payment.amountRub",
			]),
			[],
	...
	test("запись в расписании требует пациента и времени начала", () => {
		assert.deepEqual(
			missingRequiredFields("appointment", ["appointment.patientRef"]),
