...
export function normalizeNameForComparison(value: string | null | undefined): string {
  if (!value) return "";
  return value
    .toLowerCase()
    .replace(/ё/g, "е")
 ...
 */
export function scoreIdentity(incoming: IdentityCandidate, existing: IdentityCandidate): IdentityVerdict {
  ...
  const nameSimilarity = incomingName && existingName ? stringSimilarity(incomingName, existingName) : 0;
  score += nameSimilarity * 0.4;
  if (nameSimilarity >= 0.99) reasons.push("ФИО совпадает");
  else if (nameSimilarity >= 0.85) reasons.push(`ФИО близко (${Math.round(nameSimilarity * 100)}%)`);
  else reasons.push(`ФИО различается (совпадение ${Math.round(nameSimilarity * 100)}%)`);
  ...
  const action: IdentityAction =
    bounded >= SAME_THRESHOLD ? "same" : bounded >= REVIEW_THRESHOLD ? "needs_review" : "different";
...
export function blockingKeys(candidate: IdentityCandidate): string[] {
  ...
  const nameWords = normalizeNameForComparison(candidate.fullName).split(" ").filter(Boolean);
  if (nameWords.length > 0) {
    const nameStem = nameWords.map((word) => word.slice(0, 3)).sort().join("");
    if (candidate.birthDate) keys.push(`bd:${candidate.birthDate}:${nameStem.slice(0, 9)}`);
    if (nameWords.length >= 2) keys.push(`n2:${nameWords.slice(0, 2).join(" ")}`);
  }
...
export class IdentityIndex<T extends IdentityCandidate> {
  ...
  findBest(candidate: IdentityCandidate): { record: T; verdict: IdentityVerdict } | null {
...
export function naturalKeyFor(
  ...
  switch (entityKind) {
    ...
    case "payment": {
      ...
      if (!values.patientKey || values.amountRub === null || values.amountRub === undefined) return null;
