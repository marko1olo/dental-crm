...
function sampleValues(rows: string[][], columnIndex: number): string[] {
  ...
  const values: string[] = [];
  for (let index = 0; index < total && values.length < PROFILE_SAMPLE_LIMIT; index += step) {
...
export function profileColumn(name: string, index: number, rows: string[][]): ColumnProfile {
...
export function profileTable(columns: string[], rows: string[][]): ColumnProfile[] {
...
export function describeColumnForModel(profile: ColumnProfile): string {
  ...
  const fillRate = profile.totalRows === 0 ? 0 : Math.round((profile.nonEmptyCount / Math.min(profile.totalRows, PROFILE_SAMPLE_LIMIT)) * 100);
  ...
  if (profile.valueShapes.length > 0) parts.push(`маски: ${profile.valueShapes.slice(0, 3).join(" | ")}`);
