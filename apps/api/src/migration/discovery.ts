...
import { looksLikeDicom, mayBeDicomFileName, readDicomMetadata } from "./formats/dicom.js";
import { identifyFormat, type FormatSignature, type SourceFormatId } from "./formats/signatures.js";
...
const SKIP_EXTENSIONS = new Set([
  "exe", "dll", "sys", "msi", "cab", "ocx", "drv", "cpl", "scr",
  "png", "jpg", "jpeg", "gif", "bmp", "ico", "svg", "webp",
  "mp3", "mp4", "avi", "mov", "wav", "mkv",
  "ttf", "otf", "woff", "woff2",
  "lnk", "url", "ini", "log", "tmp", "bak~", "chm", "hlp"
]);
...
const CLINIC_DIRECTORY_HINTS = [
  "ident", "dental", "стомат", "stomat", "dentalpro", "infodent", "инфодент",
  "клиник", "clinic", "медицин", "medic", "1cv8", "1с", "base", "базы", "data",
  "backup", "выгрузк", "экспорт", "export", "пациент", "patient"
];
...
export interface DiscoveryResult {
  ...
  imagingFolders: Array<{ directory: string; fileCount: number; sample: string[] }>;
  filesScanned: number;
...
function relevanceOf(source: {
  ...
  if (CLINIC_DIRECTORY_HINTS.some((hint) => directoryLower.includes(hint))) score += 30;

  const nameLower = source.fileName.toLowerCase();
  if (/(pacient|patient|пациент|klient|client|kart|карт)/i.test(nameLower)) score += 40;
  if (/(visit|priem|приём|прием|lech|лечен)/i.test(nameLower)) score += 25;
  if (/(oplat|payment|плат|kassa|касса|schet|счет)/i.test(nameLower)) score += 25;
 ...
 */
async function collectDetails(filePath: string, format: FormatSignature, head: Buffer): Promise<string[]> {
  const details: string[] = [];

  try {
    if (format.id === "sqlite") {
      const inspection = inspectSqlite(filePath);
      const meaningful = inspection.tables.filter((table) => !table.system && table.rowCount > 0);
      if (meaningful.length === 0) {
        ...
        for (const table of meaningful.slice(0, 6)) {
          details.push(`  ${table.name}: ${table.rowCount} строк, колонок ${table.columns.length}`);
        }
        if (meaningful.length > 6) details.push(`  …и ещё ${meaningful.length - 6}`);
      }
    ...
    if (format.id === "dbf" && head.length > 32) {
      ...
      const codepage =
        languageDriver === 0x65 ? "cp866" : languageDriver === 0xc9 ? "cp1251" : languageDriver === 0x03 ? "cp1252" : null;
      details.push(`Записей: ${records}, полей: ${fieldCount}.`);
      details.push(codepage ? `Кодировка из заголовка: ${codepage}.` : "Кодовая страница в заголовке не указана.");
      return details;
    }

    if (format.id === "dicom") {
      const metadata = await readDicomMetadata(filePath);
      details.push(`Пациент: ${metadata.patientName ?? "не указан"}${metadata.patientId ? ` (ид. ${metadata.patientId})` : ""}`);
      details.push(`Модальность: ${metadata.modality ?? "не указана"}, дата: ${metadata.studyDate ?? "не указана"}`);
      if (metadata.manufacturer) details.push(`Аппарат: ${metadata.manufacturer}`);
      return details;
    }
  } catch (error) {
    details.push(`Файл опознан, но не открылся: ${error instanceof Error ? error.message : String(error)}`);
  }
 ...
 * а не вторые.
 */
export async function discoverLocalSources(options: DiscoveryOptions = {}): Promise<DiscoveryResult> {
  ...
  const imagingByDirectory = new Map<string, { count: number; sample: string[] }>();

  let filesScanned = 0;
  let directoriesScanned = 0;
  let truncated = false;

  const queue: Array<{ directory: string; depth: number }> = roots.map((directory) => ({ directory, depth: 0 }));
  const visited = new Set<string>();

  while (queue.length > 0) {
    if (Date.now() - started > timeBudget) {
      truncated = true;
      warnings.push(`Поиск остановлен по времени (${Math.round(timeBudget / 1000)} с). Показано найденное на этот момент.`);
      break;
    }
    if (filesScanned >= MAX_FILES_SCANNED || sources.length >= MAX_CANDIDATES) {
      truncated = true;
      warnings.push("Поиск остановлен по числу файлов. Сузьте область поиска, указав каталог старой системы.");
      break;
    }
    ...
    for (const entry of entries) {
      ...
      if (mayBeDicomFileName(entry.name)) {
        ...
        try {
          ...
          if (looksLikeDicom(head)) {
            imagingByDirectory.set(item.directory, { count: 1, sample: [entry.name] });
            continue;
          }
        } catch {
          continue;
      ...
      if ((format.id === "json" || format.id === "xml") && info.size < 4096) continue;
      ...
      sources.push({
        ...
        format,
        details,
        relevance: relevanceOf({ format, directory: item.directory, byteSize: info.size, fileName: entry.name })
      });
    }
  }
  ...
  const imagingFolders = [...imagingByDirectory.entries()]
    .map(([directory, group]) => ({ directory, fileCount: group.count, sample: group.sample }))
...
export function summarizeDiscovery(result: DiscoveryResult): {
  readable: number;
  needsExport: number;
  byFormat: Array<{ format: SourceFormatId; title: string; count: number; readable: boolean }>;
} {
  const byFormat = new Map<SourceFormatId, { title: string; count: number; readable: boolean }>();
  for (const source of result.sources) {
    const existing = byFormat.get(source.format.id);
    if (existing) existing.count += 1;
    else byFormat.set(source.format.id, { title: source.format.title, count: 1, readable: source.format.readable });
  }

  return {
    readable: result.sources.filter((source) => source.format.readable).length,
    needsExport: result.sources.filter((source) => !source.format.readable).length,
