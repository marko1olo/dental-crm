...
function ok<T>(value: T, transforms: string[], confidence = 1): NormalizedValue<T> {
...
const NULL_TOKENS = new Set([
  "", "-", "--", "---", "—", "–", "н/д", "нд", "н.д.", "нет", "нет данных",
  "не указан", "не указано", "не указана", "неизвестно", "не помнит",
  "отсутствует", "null", "nil", "none", "n/a", "na", "?", "??",
  "0", "00.00.0000", "00/00/0000", "0000-00-00", "01.01.1900", "1900-01-01",
  "1899-12-30", "30.12.1899", "empty", "пусто", "x", "xxx",
  /**
   ...
   */
  "#н/д", "#знач!", "#дел/0!", "#ссылка!", "#имя?", "#пусто!", "#число!",
 ...
 */
export function isNullToken(value: string | null | undefined, treatZeroAsNull = true): boolean {
...
export function normalizeText(raw: string | null | undefined, maxLength = MAX_TEXT_LENGTH): NormalizedValue<string> {
  if (isNullToken(raw, false)) return empty(["null-token"]);
  const transforms: string[] = [];
  let value = String(raw);

  if (hasEncodingDamage(value)) {
    return bad("Значение содержит нечитаемые символы — повреждена кодировка источника.", ["encoding-check"]);
  }
...
function arrangeParts(a: number, b: number, c: number, order: Exclude<DateOrder, "unknown">): DateParts {
...
function extractDateNumbers(raw: string): { numbers: number[]; hadTime: boolean } | null {
  ...
  const withoutTime = trimmed.replace(/[T\s]+\d{1,2}:\d{2}(:\d{2})?(\.\d+)?\s*(Z|[+-]\d{2}:?\d{2})?$/i, "");
  ...
  if (compact) {
    return { numbers: [Number(compact[1]), Number(compact[2]), Number(compact[3])], hadTime };
  }
 ...
 */
export function detectDateOrder(samples: readonly (string | null | undefined)[]): DateFormatHint {
  let dmyWitness = 0;
  let mdyWitness = 0;
  let ymdWitness = 0;
  let parsable = 0;
  ...
  if (ymdWitness > dmyWitness && ymdWitness > mdyWitness) {
    return { order: "ymd", rationale: "Первое число больше 31 — год стоит впереди.", coverage };
  }
  ...
  return {
    order: "dmy",
    rationale: "Однозначных признаков нет; принят российский порядок день-месяц-год.",
    coverage
  };
}
...
export function normalizeDateValue(
  ...
  if (/^\d{10}$/.test(text) || /^\d{13}$/.test(text)) {
    ...
    if (!Number.isNaN(date.getTime())) {
      const parts = { year: date.getUTCFullYear(), month: date.getUTCMonth() + 1, day: date.getUTCDate() };
      if (validateParts(parts)) {
        return ok(partsToIso(parts), [text.length === 10 ? "unix-seconds" : "unix-millis"], 0.85);
      }
    }
  }
  ...
  if (!extracted) {
    return bad(`Значение «${truncateForMessage(text)}» не разобрано как дата.`, transforms);
  }
  ...
  const orders: Exclude<DateOrder, "unknown">[] =
    a > 31 ? ["ymd"] : hint.order === "unknown" ? ["dmy", "mdy"] : [hint.order, hint.order === "dmy" ? "mdy" : "dmy", "ymd"];
  ...
  for (const [index, order] of orders.entries()) {
    ...
    if (parts.year < 100) {
      ...
      if (!transforms.includes("expand-2digit-year")) transforms.push("expand-2digit-year");
    }
...
export function normalizePhoneValue(raw: string | null | undefined): NormalizedValue<NormalizedPhone> {
  if (isNullToken(raw)) return empty(["null-token"]);
  const text = String(raw).trim();
  const transforms: string[] = [];
  ...
  const extensionMatch = /(?:доб\.?|добавочный|ext\.?|extension|x|#)\s*(\d{1,6})\s*$/i.exec(working);
  if (extensionMatch) {
   ...
   */
  const segments = working.split(/[,;/]|\sили\s|\sи\s/i).map((part) => part.trim()).filter(Boolean);
  if (segments.length > 1) transforms.push("multiple-numbers-first-taken");
  const candidate = segments[0] ?? working;

  const digits = candidate.replace(/\D/g, "");
  if (!digits) {
    return bad(`В значении «${truncateForMessage(text)}» нет ни одной цифры телефона.`, transforms);
  }
  ...
  if (national.length !== 10) {
    return bad(`Номер «${truncateForMessage(text)}» не приводится к российскому формату.`, transforms);
  }
  ...
  if (national.startsWith("0") || national.startsWith("1")) {
    return bad(`Номер «${truncateForMessage(text)}» имеет несуществующий код «${national.slice(0, 3)}».`, transforms);
  }
 ...
 */
const LOWERCASE_NAME_PARTICLES = new Set(["ван", "де", "дер", "ла", "ле", "фон", "да", "ди", "дель", "оглы", "кызы", "уулу"]);
 ...
 */
export function fixNameCase(value: string): { value: string; changed: boolean } {
 ...
 */
export function normalizeNameValue(raw: string | null | undefined): NormalizedValue<NormalizedName> {
  if (isNullToken(raw, false)) return empty(["null-token"]);
  const text = String(raw).trim();
  const transforms: string[] = [];

  if (hasEncodingDamage(text)) {
    return bad("ФИО содержит нечитаемые символы — повреждена кодировка источника.", ["encoding-check"]);
  }
  ...
  if (text.includes("^")) {
    ...
    parts = [surname ?? "", ...(rest ?? "").trim().split(/\s+/)].map((part) => part.trim()).filter(Boolean);
  ...
  const cased = parts.map((part) => {
    ...
    if (changed && !transforms.includes("fix-case")) transforms.push("fix-case");
    return value;
  });
  ...
  if (letterCount < 2) {
    return bad(`Значение «${truncateForMessage(text)}» не похоже на ФИО.`, transforms);
  }
   ...
   */
  const confidence = cased.length === 1 ? 0.65 : cased.length === 2 ? 0.9 : 0.98;

  return ok({ fullName, lastName, firstName, middleName }, transforms, confidence);
}
...
export function normalizeMoneyValue(raw: string | null | undefined): NormalizedValue<number> {
  ...
  if (lastSeparator === -1) {
    ...
    if (tail.length === 3 && (headHasSeparator || head.length <= 3)) {
      ...
      return bad(`Сумма «${truncateForMessage(text)}» имеет непонятный разделитель разрядов.`, transforms);
    }
  }
...
const MALE_TOKENS = new Set(["м", "муж", "мужской", "мужчина", "m", "male", "1", "м."]);
const FEMALE_TOKENS = new Set(["ж", "жен", "женский", "женщина", "f", "female", "ж.", "w", "2"]);

export function normalizeGenderValue(raw: string | null | undefined): NormalizedValue<Gender> {
  if (isNullToken(raw, false)) return empty(["null-token"]);
  ...
  return bad(`Значение пола «${truncateForMessage(token)}» не распознано.`, ["gender:token"]);
}

const TRUE_TOKENS = new Set(["1", "true", "да", "yes", "y", "истина", "t", "+", "есть"]);
const FALSE_TOKENS = new Set(["0", "false", "нет", "no", "n", "ложь", "f", "-", "отсутствует"]);

export function normalizeBooleanValue(raw: string | null | undefined): NormalizedValue<boolean> {
  if (raw === null || raw === undefined || String(raw).trim() === "") return empty(["null-token"]);
  const token = String(raw).trim().toLowerCase();
  if (TRUE_TOKENS.has(token)) return ok(true, ["boolean:token"]);
  if (FALSE_TOKENS.has(token)) return ok(false, ["boolean:token"]);
  return bad(`Значение «${truncateForMessage(token)}» не распознано как да/нет.`, ["boolean:token"]);
}
...
export function normalizeEnumValue<T extends string>(
  ...
  if (isNullToken(raw, false)) {
    return fallback ? ok(fallback, ["enum:default"], 0.5) : empty(["null-token"]);
  }
  const token = String(raw).trim().toLowerCase().replace(/[\s_-]+/g, " ");
  ...
  if (fallback) {
    return { value: fallback, transforms: ["enum:fallback"], confidence: 0.4, issue: null };
  }
  return bad(`Значение «${truncateForMessage(token)}» не сопоставлено ни одному известному состоянию.`, ["enum:miss"]);
}
 ...
 */
export function normalizeToothCode(raw: string | null | undefined): NormalizedValue<string> {
  if (isNullToken(raw)) return empty(["null-token"]);
  const text = String(raw).trim().replace(/\s+/g, "");

  const fdiDotted = /^([1-8])\.([1-8])$/.exec(text);
  if (fdiDotted) return ok(`${fdiDotted[1]}${fdiDotted[2]}`, ["tooth:fdi-dotted"], 0.97);
  ...
  if (digits.length === 2) {
    ...
    const deciduous = quadrant >= 5 && quadrant <= 8 && position >= 1 && position <= 5;
    if (permanent || deciduous) return ok(digits, ["tooth:fdi"], 0.98);
    return bad(`Номер зуба «${truncateForMessage(text)}» не существует в нотации FDI.`, ["tooth:fdi"]);
  }
  ...
  if (digits.length === 1 || (digits.length === 2 && Number(digits) <= 32)) {
    ...
    if (universal >= 1 && universal <= 32) {
      ...
      const fdi =
        quadrant === 1 ? `1${8 - indexInQuadrant}`
        : quadrant === 2 ? `2${indexInQuadrant + 1}`
        : quadrant === 3 ? `3${8 - indexInQuadrant}`
        : `4${indexInQuadrant + 1}`;
      return ok(fdi, ["tooth:universal-to-fdi"], 0.6);
    }
  }

  return bad(`Номер зуба «${truncateForMessage(text)}» не распознан.`, ["tooth:unknown"]);
}
...
export function normalizeEmailValue(raw: string | null | undefined): NormalizedValue<string> {
  if (isNullToken(raw, false)) return empty(["null-token"]);
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(first)) {
    return bad(`Адрес «${truncateForMessage(text)}» не похож на электронную почту.`, ["email:validate"]);
  }
  return ok(first, first === text ? ["email:validate"] : ["email:first-of-many"], 0.95);
}
