...
function birthDateIssues(birthDate: string): string[] {
	...
	return transformed.issues.filter((issue) => issue.fieldPath === "birthDate").map((issue) => issue.message);
}
...
test("максимальная дата на Земле опережает UTC в заданные мгновения", () => {
	const cases = [
		...
		{ instant: "2026-07-29T23:00:00.000Z", utcDay: "2026-07-29", earthDay: "2026-07-30" },
		// UTC 10:00 — самая ранняя граница: +14 даёт ровно полночь следующих суток.
		{ instant: "2026-07-29T10:00:00.000Z", utcDay: "2026-07-29", earthDay: "2026-07-30" },
		// Через границу месяца: 31 июля в UTC — уже 1 августа на востоке.
		{ instant: "2026-07-31T20:30:00.000Z", utcDay: "2026-07-31", earthDay: "2026-08-01" },
		// Через границу года.
		{ instant: "2026-12-31T14:00:00.000Z", utcDay: "2026-12-31", earthDay: "2027-01-01" },
		// UTC 09:00 — расхождения ещё нет, обе даты совпадают: проверка не должна
		// «улучшать» ответ там, где улучшать нечего.
		{ instant: "2026-07-29T09:00:00.000Z", utcDay: "2026-07-29", earthDay: "2026-07-29" }
	];
	...
	for (const { instant, utcDay, earthDay } of cases) {
		...
		assert.equal(oldUtcDay, utcDay, `контрольное значение UTC для ${instant} посчитано неверно`);
		assert.equal(
...
test("действительно будущая дата рождения по-прежнему называется будущей", () => {
	...
	const farFuture = calendarDateIn("Pacific/Kiritimati", new Date(now.getTime() + 2 * 86_400_000));
	...
	assert.match(issues[0] ?? "", /в будущем/, "предупреждение обязано называть причину словами оператора");
});
