...
export async function detectSourceShape(input: {
  ...
  if (input.forcedKind === "dbf" || (input.forcedKind === undefined && looksLikeDbf(head))) {
  ...
  const looksTree = headText.startsWith("{") || headText.startsWith("[") || headText.startsWith("<");

  if (looksZip || looksTree || (input.forcedKind !== undefined && input.forcedKind !== "delimited" && input.forcedKind !== "clipboard")) {
  ...
  return {
    ...
    sampleRows: headParsed.rows.slice(0, Math.max(0, Math.min(2000, headParsed.rows.length - 1))),
...
async function readDbfMeta(filePath: string): Promise<DbfMeta> {
  ...
  try {
    ...
    if (headerLength < 64 || recordLength === 0) {
      throw new Error("Заголовок DBF повреждён: не согласуются длины заголовка и записи.");
    }
    ...
    for (let offset = 0; offset + 32 <= descriptors.length; offset += 32) {
      ...
      const name = ascii.decode(descriptors.subarray(offset, offset + 11)).replace(/\0.*$/, "").trim();
      if (!name) continue;
      fields.push({
        name,
    ...
    if (fields.length === 0) throw new Error("В заголовке DBF не найдено ни одного описания поля.");
...
function decodeDbfRecord(record: Buffer, meta: DbfMeta, decoder: TextDecoder): string[] {
  const values: string[] = [];
  let offset = 1;
  for (const field of meta.fields) {
    ...
    switch (field.type) {
      ...
      case "L": {
        ...
        values.push(char === "T" || char === "Y" ? "1" : char === "F" || char === "N" ? "0" : "");
        break;
      }
      case "D": {
      ...
      case "Y": {
        ...
        values.push(`${negative ? "-" : ""}${absolute / 10000n}.${(absolute % 10000n).toString().padStart(4, "0")}`);
        break;
      }
      case "B":
      case "O":
      ...
      case "@": {
        ...
        values.push(Number.isNaN(date.getTime()) ? "" : date.toISOString().replace(".000Z", "Z"));
        break;
      }
      default:
...
async function collectDbfSample(filePath: string, meta: DbfMeta, limit: number): Promise<string[][]> {
  const rows: string[][] = [];
  for await (const batch of streamDbfRows(filePath, meta, Math.min(limit, STREAM_BATCH_ROWS))) {
...
async function* streamDbfRows(
  ...
  try {
    ...
    const availableRecords = Math.floor((fileSize - meta.headerLength) / meta.recordLength);
    const totalRecords =
      meta.declaredRecords === 0 ? availableRecords : Math.min(meta.declaredRecords, availableRecords);
    ...
    while (processed < totalRecords) {
      ...
      const { bytesRead } = await handle.read(chunkBuffer, 0, bytesToRead, position);
...
function quoteStateOf(fragment: string): boolean {
  ...
  for (let index = 0; index < fragment.length; index += 1) {
    ...
    fieldEmpty = char === "," || char === ";" || char === "\t" || char === "|" || char === "\n";
  }
  return inQuotes;
}
...
async function* streamDelimitedRows(
  ...
  if (tailText) {
    for (const row of accumulator.push(tailText.replace(/\r\n/g, "\n").replace(/\r/g, "\n"))) {
      if (!headerDropped) {
        headerDropped = true;
        continue;
      }
...
export async function* streamSourceRows(input: {
  ...
  const align = (row: string[]): string[] => {
    ...
    if (row.length < columnCount) return [...row, ...Array<string>(columnCount - row.length).fill("")];
    // Хвост склеивается в последнюю колонку — текст не теряется.
    const head = row.slice(0, columnCount - 1);
    return [...head, row.slice(columnCount - 1).join(input.shape.delimiter ?? " ")];
  };
