...
const FIELD_VALUE_KIND: Record<
	MigrationTargetField,
	keyof ColumnProfile["parseRates"] | "text" | "any"
> = {
		...
		for (const column of rule.columns) {
			...
			if (!best || candidate.length > best.length) {
				best = {
					...
					isVendor: ruleIndex < fromVendorCount,
				};
			}
		}
	}
...
function candidateFromContent(
	...
	if (rates.personName >= INFERENCE_MIN_RATE) {
		return {
			targetField:
				entityKind === "doctor" ? "doctor.fullName" : "patient.fullName",
			decidedBy: "inferred",
			confidence: 0.82,
 ...
 */
const OVERFLOW_FIELDS: Partial<
	Record<MigrationTargetField, MigrationTargetField>
> = {
...
export interface ResolveMappingInput {
	...
	overrides?:
		| Array<{ sourceColumn: string; targetField: MigrationTargetField }>
		| undefined;
}

export function resolveDeterministicMapping(
	input: ResolveMappingInput,
): ResolvedMapping {
	const warnings: string[] = [];

	const vendorMatch = matchVendorProfile(
		input.columns,
		input.tableName,
		input.requestedVendorProfile,
	);
	const detected = input.requestedEntityKind
		? {
				entityKind: input.requestedEntityKind,
				rationale: "Сущность указана оператором.",
			}
	...
	const overrideByColumn = new Map(
		(input.overrides ?? []).map((override) => [
			canonicalColumnName(override.sourceColumn),
			override.targetField,
		]),
	);
	...
	const slots: Slot[] = input.profiles.map((profile) => {
		...
		let candidate = candidateFromColumnName(
			profile,
			entityKind,
			vendorMatch.profile,
		);
		...
		if (!candidate) {
			...
			if (candidate)
				validation = validateAgainstData(candidate.targetField, profile);
		}
	...
	return {
		...
		vendorRationale: vendorMatch.profile
			? vendorMatch.rationale
			: detected.rationale,
		columns,
		unmappedColumns,
		warnings,
...
export function missingRequiredFields(
	...
	return required.filter((field) => {
		...
		if (field === "patient.fullName") {
			return !(
				present.has("patient.lastName") && present.has("patient.firstName")
			);
		}
		if (field === "doctor.fullName") {
			return !(
				present.has("doctor.fullName") || present.has("doctor.externalId")
			);
		}
		return true;
	});
