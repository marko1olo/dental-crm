...
export function transformRow(input: TransformRowInput): TransformedRow {
  ...
  const columnIndex = new Map(input.columns.map((column, index) => [column, index]));
  ...
  if (totalLength > MIGRATION_MAX_ROW_CHARS) {
    issues.push({
      ...
      suggestedFix: "Проверьте разделитель и кодировку источника, затем повторите разбор."
    });
    return { entityKind: input.entityKind, values, lineage, issues, confidence: 0 };
  }
  ...
  const apply = <T>(
    ...
    if (parsed.issue) {
      ...
      issues.push({
        reason: parsed.issue.includes("кодировк") ? "encoding_damage" : "unparsable_value",
        blocking,
        fieldPath: key,
    ...
    if (CONCATENATED_FIELDS.has(targetField) && typeof finalValue === "string" && typeof values[key] === "string") {
  ...
  for (const column of input.mapping) {
    ...
    switch (field) {
      ...
      case "doctor.phone":
        ...
        // Добавочный номер — тоже данные, и терять его нельзя.
        {
          ...
          if (parsedPhone.value?.extension) {
            const noteKey = input.entityKind === "doctor" ? "specialty" : "notes";
            const existing = typeof values[noteKey] === "string" ? `${values[noteKey] as string}\n` : "";
            values[noteKey] = `${existing}Добавочный номер: ${parsedPhone.value.extension}`;
          }
        }
        break;
      ...
      case "patient.email":
      case "doctor.email":
        apply(field, column.sourceColumn, rawValue, normalizeEmailValue(rawValue), column.decidedBy, column.confidence);
        break;
      ...
      case "patient.gender":
        apply(field, column.sourceColumn, rawValue, normalizeGenderValue(rawValue), column.decidedBy, column.confidence);
        break;
      ...
      case "toothState.toothCode":
        apply(field, column.sourceColumn, rawValue, normalizeToothCode(rawValue), column.decidedBy, column.confidence);
        break;
      ...
      case "toothState.patientRef": {
        ...
        if (column.confidence < minConfidence) minConfidence = column.confidence;
        break;
      }

      default:
        // Текстовые поля: имя, отчество, адрес, жалобы, диагноз, примечания.
        apply(field, column.sourceColumn, rawValue, normalizeText(rawValue), column.decidedBy, column.confidence);
        break;
    }
  }
  ...
  if (!values.fullName && (values.lastName || values.firstName)) {
    ...
    if (combined.value) {
      ...
      const nameField: MigrationTargetField = input.entityKind === "doctor" ? "doctor.fullName" : "patient.fullName";
      lineage.push({
        field: nameField,
  ...
  for (const field of requiredFieldsFor(input.entityKind)) {
    ...
    if (values[key] === undefined || values[key] === null || values[key] === "") {
      ...
      issues.push({
        ...
        suggestedFix: "Сопоставьте нужную колонку вручную либо заполните значение и перенесите строку из карантина."
      });
    }
  }
  ...
  // Порог уверенности.
  // ------------------------------------------------------------------
  const confidence = lineage.some((entry) => entry.confidence > 0) ? minConfidence : 0;
  if (confidence > 0 && confidence < input.confidenceThreshold && !issues.some((issue) => issue.blocking)) {
    const weakest = [...lineage].filter((entry) => entry.confidence > 0).sort((left, right) => left.confidence - right.confidence)[0];
    issues.push({
      ...
      suggestedFix: "Проверьте строку в предпросмотре; при верном разборе подтвердите её из карантина."
    });
  }
...
function requiredFieldsFor(entityKind: MigrationEntityKind): MigrationTargetField[] {
  switch (entityKind) {
    case "patient":
...
function isRequiredField(entityKind: MigrationEntityKind, field: MigrationTargetField): boolean {
...
function domainRuleIssues(entityKind: MigrationEntityKind, values: Record<string, unknown>): RowIssue[] {
  const issues: RowIssue[] = [];
  ...
  if (entityKind === "patient") {
    ...
    if (birthDate && birthDate > today) {
      issues.push({
        ...
        suggestedFix: "Проверьте значение в источнике; поле перенесено как есть и требует исправления."
      });
    }
  }
  ...
  if (entityKind === "appointment") {
    ...
    if (startsAt && endsAt && endsAt < startsAt) {
      issues.push({
        ...
        suggestedFix: "Окончание будет рассчитано по длительности приёма по умолчанию."
      });
    }
  }
...
export function describeRowForOperator(profile: ColumnProfile[], row: string[]): string {
  const parts = profile.slice(0, 3).map((column, index) => `${column.name}=${truncateForMessage(row[index] ?? "", 20)}`);
  return parts.join("; ");
}
