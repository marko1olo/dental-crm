...
import { maskValueShape, profileTable, type ColumnProfile } from "./columnProfile.js";
import { sourceFingerprint, naturalKeyFor } from "./identity.js";
import { mapColumnsWithLlm } from "./llmMapper.js";
import { loaderFor, recordQuarantine, stageRows, type StagedRow } from "./loader.js";
...
export interface EngineInput {
  ...
  dryRun: boolean;
  sourceSystem: string;
  mappingOverrides: Array<{ sourceColumn: string; targetField: MigrationTargetField }>;
}
...
function decodeContent(contentBase64: string | undefined): Buffer | undefined {
  ...
  const clean = contentBase64.includes(",") ? (contentBase64.split(",").pop() ?? "") : contentBase64;
...
async function prepareSource(input: EngineInput): Promise<{
  ...
  for (const table of parsed.tables) {
    if (table.columns.length === 0) {
      warnings.push(`В таблице «${table.name}» не определены колонки — она пропущена.`);
      continue;
    }
    ...
    if (input.allowLlm && deterministic.candidatesForLlm.length > 0 && deterministic.entityKind !== "unknown") {
      ...
      const acceptedColumns = new Set(llm.accepted.map((column) => column.sourceColumn));
    ...
    const dateHints = new Map<string, DateFormatHint>(profiles.map((profile) => [profile.name, profile.dateHint]));
...
function sourceProfileFor(parsed: ParsedSource, prepared: PreparedTable): MigrationSourceProfile {
  return {
 ...
 */
export async function analyzeSource(input: EngineInput): Promise<MigrationAnalyzeResponse> {
  ...
  const findingsByMessage = new Map<string, { severity: "info" | "warning" | "blocker"; rows: number }>();
  ...
  const qualityFindings = [
    ...
    ...warnings.map((message) => ({ severity: "info" as const, message, affectedRows: 0 }))
  ]
    ...
    .sort((left, right) => {
      ...
      return bySeverity !== 0 ? bySeverity : right.affectedRows - left.affectedRows;
    })
    .slice(0, 40);
  ...
  if (tables.length > 1) {
    qualityFindings.unshift({
      ...
      message: `В источнике ${tables.length} таблиц(ы): ${tables
        .map((item) => `${item.table.name} → ${migrationEntityKindTitles[item.entityKind]}`)
...
function summaryFrom(run: typeof migrationRuns.$inferSelect): MigrationRunSummary {
  return {
    runId: run.id,
 ...
 */
export async function runMigration(input: EngineInput): Promise<MigrationRunResponse> {
  const content = decodeContent(input.contentBase64);
  const fingerprint = sourceFingerprint(content ?? input.rawText ?? "");

  const { parsed, tables, warnings } = await prepareSource(input);
  if (tables.length === 0) {
    throw new Error(`Источник «${input.sourceName}» не содержит распознаваемых таблиц. ${warnings.join(" ")}`.trim());
  }

  const totalSourceRows = tables.reduce((sum, item) => sum + item.table.rows.length, 0);
  ...
  try {
    ...
    for (const prepared of tables) {
      ...
      const suspect = new Set(prepared.table.suspectRowNumbers.map((rowNumber) => rowNumber + 1));
      for (const row of staged.rows) {
        if (suspect.has(row.sourceRowNumber)) {
          row.issues.push({
            ...
            suggestedFix: "Проверьте строку в источнике; данные сохранены целиком."
          });
        }
      }
      ...
      for (const [stagingId, issue] of outcome.issuesByStagingId) {
        const row = staged.rows.find((candidate) => candidate.stagingId === stagingId);
    ...
    const quarantinePreview = await listQuarantine(input.organizationId, run.id, QUARANTINE_PREVIEW_LIMIT);
...
export async function listQuarantine(
  ...
  const rows = await withTenantCtx(organizationId, async (tx) =>
    ...
    .leftJoin(migrationStagingRecords, eq(migrationQuarantineRecords.stagingRecordId, migrationStagingRecords.id))
    .where(
      and(
 ...
 */
const ROLLBACK_ORDER: MigrationEntityKind[] = ["payment", "visit", "appointment", "tooth_state", "patient"];
...
export async function rollbackRun(input: {
  ...
  await withTenantCtx(input.organizationId, async (tx) => {
    for (const entityKind of ROLLBACK_ORDER) {
      ...
      if (entityKind === "payment") {
        const removed = await tx
          .delete(payments)
          .where(and(eq(payments.organizationId, input.organizationId), inArray(payments.id, ids)))
        ...
        const removed = await tx
          .delete(visits)
          .where(and(eq(visits.organizationId, input.organizationId), inArray(visits.id, ids)))
        ...
        const removed = await tx
          .delete(appointments)
          .where(and(eq(appointments.organizationId, input.organizationId), inArray(appointments.id, ids)))
        ...
        const referencedRows = await tx
          ...
          .from(appointments)
          .where(and(eq(appointments.organizationId, input.organizationId), inArray(appointments.patientId, ids)))
          .union(
            tx
              .select({ patientId: visits.patientId })
              .from(visits)
              .where(and(eq(visits.organizationId, input.organizationId), inArray(visits.patientId, ids)))
          )
          .union(
            tx
              .select({ patientId: payments.patientId })
              .from(payments)
              .where(and(eq(payments.organizationId, input.organizationId), inArray(payments.patientId, ids)))
          );

        const referenced = new Set(
          referencedRows.map((row) => row.patientId).filter((value): value is string => Boolean(value))
        );
        ...
        if (deletable.length > 0) {
          const removed = await tx
            .delete(patients)
            .where(and(eq(patients.organizationId, input.organizationId), inArray(patients.id, deletable)))
    ...
    await tx
      .update(migrationRuns)
      .set({ status: "rolled_back", updatedAt: new Date(), finishedAt: new Date() })
    ...
    await tx.insert(auditEvents).values({
      ...
      reason: `Откат переноса из «${run.sourceName}»: ${outcomes
        .map((outcome) => `${outcome.entityKind} удалено ${outcome.deleted}, сохранено ${outcome.retained}`)
        .join("; ")}.`
    });
  });

  const totalDeleted = outcomes.reduce((sum, outcome) => sum + outcome.deleted, 0);
  const totalRetained = outcomes.reduce((sum, outcome) => sum + outcome.retained, 0);
