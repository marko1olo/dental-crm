...
export function identifyFormat(head: Buffer, fileName = ""): FormatSignature {
  ...
  if (asciiAt(head, 128, 4) === "DICM") {
    return { id: "dicom", title: "Снимок DICOM", readable: true, version: "Part 10", guidance: null, confidence: 1 };
  }
  ...
  const mdfMarker =
    head.length > 0x1000 &&
    head.subarray(0, Math.min(head.length, 0x4000)).toString("latin1").includes("Microsoft SQL Server");
  if (mdfMarker || extension === "mdf" || extension === "ndf" || extension === "ldf") {
    return {
      id: "mssql_mdf",
  ...
  if (head.length > 2 && head[2] === 0x00 && (head[4] === 0x00 || head[4] === 0x01) && extension === "db") {
  ...
  if (head.length > 32) {
    ...
    if (label) {
      ...
      const day = head[3]!;
      const plausible =
        headerLength >= 64 && recordLength > 0 && month >= 1 && month <= 12 && day >= 1 && day <= 31;
      if (plausible) {
        // Старший бит версии означает наличие memo-файла рядом.
        const hasMemo = (version & 0x80) !== 0 || version === 0x30 || version === 0xf5 || version === 0x7b;
        return {
          id: "dbf",
          title: `Таблица ${label}`,
          readable: true,
  ...
  if (matches(head, 0, [0x50, 0x4b, 0x03, 0x04]) || matches(head, 0, [0x50, 0x4b, 0x05, 0x06])) {
    const asText = head.subarray(0, Math.min(head.length, 4096)).toString("latin1");
    const ooxml = asText.includes("[Content_Types].xml") || asText.includes("xl/") || asText.includes("word/");
    return {
  ...
  if (asciiAt(head, 0, 5) === "{\\rtf") {
    return { id: "rtf", title: "Документ RTF", readable: false, version: null, guidance: "Сохраните как CSV или XLSX.", confidence: 1 };
  }
  ...
  if (head.length >= 64) {
    ...
    if (oddZero / pairs > 0.7) {
      return { id: "utf16_text", title: "Текст в UTF-16", readable: true, version: null, guidance: null, confidence: 0.8 };
    }
  }
  ...
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    return { id: "json", title: "Выгрузка JSON", readable: true, version: null, guidance: null, confidence: 0.85 };
  }
  if (trimmed.startsWith("<?xml") || /^<[A-Za-z_]/.test(trimmed)) {
    return { id: "xml", title: "Выгрузка XML", readable: true, version: null, guidance: null, confidence: 0.85 };
  }
  if (/\b(INSERT\s+INTO|CREATE\s+TABLE|COPY\s+\w+\s+FROM\s+stdin)\b/i.test(trimmed)) {
    return {
      id: "sql_dump",
  ...
  if (lines.length >= 2) {
    for (const delimiter of [";", "\t", ",", "|"]) {
      ...
      if (first >= 2 && counts.filter((count) => count === first).length / counts.length >= 0.8) {
        return {
          id: "delimited",
...
export const READABLE_FORMATS: ReadonlySet<SourceFormatId> = new Set<SourceFormatId>([
  "dbf",
  "sqlite",
  "zip_ooxml",
  "json",
