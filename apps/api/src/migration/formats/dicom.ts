...
export function looksLikeDicom(head: Buffer): boolean {
	return (
		head.length > 132 && head.subarray(128, 132).toString("latin1") === "DICM"
	);
}
 ...
 */
export async function readDicomMetadata(
	filePath: string,
	...
	if (!looksLikeDicom(buffer)) {
		throw new Error(
			"Файл не является снимком DICOM: отсутствует магическая метка DICM после префикса в 128 байт.",
		);
	}
	...
	if (
		...
		metaStart = 144;
		if (
			declaredGroupLength > 0 &&
			metaStart + declaredGroupLength <= buffer.length
		) {
		...
		warnings.push(
			"В снимке нет элемента длины мета-группы (0002,0000); граница определена по содержимому.",
		);
	}
	...
	const transferSyntaxUid =
		meta.elements.get(TAGS.transferSyntaxUid)?.value ?? null;
	...
	if (!transferSyntaxUid) {
		warnings.push(
			"В снимке не объявлен синтаксис передачи; заголовок прочитан как явный с прямым порядком байт.",
		);
	}

	// Первый проход основного набора нужен, чтобы узнать объявленную кодировку.
	const probe = readElements(
		buffer,
		datasetStart,
		buffer.length,
		!implicitVr,
	...
	try {
		...
		warnings.push(
			`Кодировка «${declaredCharset}» не поддерживается средой; текст прочитан как latin1.`,
		);
		decoder = latin;
	}
	...
	const value = (tag: string): string | undefined =>
		data.elements.get(tag)?.value || undefined;
