...
export async function findMemoFile(dbfPath: string): Promise<string | null> {
  ...
  for (const extension of MEMO_EXTENSIONS) {
    for (const candidateBase of [base, base.toUpperCase(), base.toLowerCase()]) {
 ...
 * при выгрузке.
 */
export async function openMemoFile(filePath: string, encoding: string): Promise<MemoFile> {
  ...
  try {
    ...
    const looksLikeFpt = fptBlockSize >= 32 && fptBlockSize <= 32768 && (fptBlockSize & (fptBlockSize - 1)) === 0;
    ...
    return {
      ...
      async read(blockNumber: number): Promise<string> {
        ...
        return decoder.decode(Buffer.concat(chunks)).replace(/\0+$/g, "").trim();
      },
      async close(): Promise<void> {
        await handle.close();
      }
