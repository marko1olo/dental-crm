...
export type DetectedEncoding = (typeof CANDIDATE_ENCODINGS)[number] | "utf-16le" | "utf-16be";
 ...
 */
const IMPLAUSIBLE_RU_BIGRAMS = /[бвгджзйклмнпрстфхцчшщ]{5,}|[аеёиоуыэюя]{4,}|[ъь][ъь]/;
...
export function detectEncoding(buffer: Buffer): EncodingDetection {
  ...
  if (buffer.length === 0) {
    return { encoding: "utf-8", confidence: 1, explicit: false, warnings: ["Источник пуст."] };
  }
  ...
  if (utf16Probe >= 32) {
    ...
    if (oddZero / pairs > 0.6 && evenZero / pairs < 0.1) {
      ...
      return { encoding: "utf-16le", confidence: 0.85, explicit: false, warnings };
    }
    if (evenZero / pairs > 0.6 && oddZero / pairs < 0.1) {
      warnings.push("UTF-16BE без BOM определён по расположению нулевых байт.");
      return { encoding: "utf-16be", confidence: 0.85, explicit: false, warnings };
    }
  }
  ...
  if (isValidUtf8(buffer)) {
    ...
    if (looksDoubleEncoded(decoded)) {
      warnings.push("Текст похож на UTF-8, ранее прочитанный как windows-1251; выполнено восстановление.");
  ...
  if (!best) {
    warnings.push("Ни одна кодировка не распозналась; текст прочитан как UTF-8 с потерями.");
  ...
  const confidence = Math.max(0, Math.min(1, best.score * 0.6 + Math.min(margin * 4, 1) * 0.4));
...
function repairCp1251DoubleEncoding(text: string): string {
  try {
    ...
    return scoreRussianPlausibility(repaired) > scoreRussianPlausibility(text) ? repaired : text;
  } catch {
    return text;
  }
}
...
export function decodeSourceBuffer(buffer: Buffer): DecodedSource {
  ...
  if (detection.explicit) {
    ...
    else if (detection.encoding === "utf-16le" || detection.encoding === "utf-16be") body = buffer.subarray(2);
  }
