...
import { detectSourceShape, streamSourceRows, type SourceShape } from "./streamStage.js";
...
async function requireRunWithUpload(runId: string, organizationId: string) {
  ...
  if (!run) {
    throw new MigrationPhaseError("RunNotFound", "Прогон переноса не найден в этой организации.");
  }
...
export async function mapRunPhase(context: PhaseContext): Promise<MapPhaseResult> {
  ...
  if (context.allowLlm && deterministic.candidatesForLlm.length > 0 && deterministic.entityKind !== "unknown") {
  ...
  const findings = new Map<string, { severity: "info" | "warning" | "blocker"; rows: number }>();
  ...
  for (const row of shape.sampleRows) {
    ...
    if (transformed.issues.some((issue) => issue.blocking)) projectedQuarantine += 1;
    ...
    for (const issue of transformed.issues) {
      ...
      else findings.set(key, { severity: issue.blocking ? "blocker" : "warning", rows: 1 });
    }
  }
  ...
  const analyze: MigrationAnalyzeResponse = {
    ...
    qualityFindings: [...findings.entries()]
      ...
      .concat(mapping.warnings.map((message) => ({ severity: "warning" as const, message, affectedRows: 0 })))
      .sort((left, right) => {
 ...
 */
export async function stageRunPhase(context: PhaseContext): Promise<StagePhaseResult> {
  const run = await requireRunWithUpload(context.runId, context.organizationId);
  ...
  return { sourceRows, stagedRows: counts.total, quarantinedRows: counts.quarantined };
}
  ...
  for (const entityKind of kinds) {
    ...
    for (;;) {
      ...
      const issueRows = rows
        ...
        .map((row) => ({ ...row, issues: [outcome.issuesByStagingId.get(row.stagingId)!] }));
