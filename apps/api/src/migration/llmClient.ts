 ...
 */
const MIGRATION_MODEL_CASCADE: Array<{
...
async function callOnce<T>(
	...
	if (!config.baseUrl || !config.modelName) {
		return {
			ok: false,
			reason: "Языковая модель не настроена: не задан адрес или имя модели.",
		};
	}
	...
	const data = (await response
		.json()
		.catch(() => ({}))) as OpenAiCompletionResponse & OpenAiErrorResponse;
	if (!response.ok) {
...
export async function requestLlmJson<T>(
	...
	for (const fallback of MIGRATION_MODEL_CASCADE) {
		if (
			...
			fallback.model === primary.modelName
		)
			continue;
