...
function parseWindowMs(timeWindow: number | string): number {
  ...
  const match = /^(\d+)\s*(ms|milliseconds?|s|seconds?|m|minutes?|h|hours?)?$/i.exec(timeWindow.trim());
  if (!match) return 60_000;
  const amount = Number(match[1]);
  const unit = (match[2] ?? "ms").toLowerCase();
  if (unit.startsWith("h")) return amount * 3_600_000;
  if (unit.startsWith("m") && !unit.startsWith("ms") && unit !== "milliseconds") return amount * 60_000;
...
function clientKey(request: FastifyRequest): string {
  const routeId = (request as unknown as { routeOptions?: { url?: string } }).routeOptions?.url ?? request.url;
...
const DEFAULT_RULES: Array<{ test: RegExp; rule: RateLimitRule }> = [
  ...
  { test: /^\/api\/auth\/(clinic\/login|staff\/unlock|login)$/, rule: { max: 5, timeWindow: "1 minute" } },
  // Создание организаций и приглашений — защита от массовой регистрации.
  { test: /^\/api\/auth\/(register|setup\/init|invites\/accept)$/, rule: { max: 10, timeWindow: "1 minute" } },
  // Смена собственных учётных данных — защита от перебора старого пароля/PIN.
  { test: /^\/api\/auth\/user\/update-(password|pin)$/, rule: { max: 10, timeWindow: "1 minute" } },
  // Административная смена чужих учётных данных.
  { test: /^\/api\/auth\/(clinic\/set-password|staff\/set-pin)$/, rule: { max: 10, timeWindow: "1 minute" } },
...
function resolveRule(request: FastifyRequest): RateLimitRule | null {
  const routeConfig = (request as unknown as { routeOptions?: { config?: { rateLimit?: RateLimitRule | false } } })
  ...
  const url = (request as unknown as { routeOptions?: { url?: string } }).routeOptions?.url ?? request.url.split("?")[0] ?? "";
...
export function registerRateLimiting(app: FastifyInstance): void {
  app.addHook("onRequest", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!existing || existing.resetAt <= now) {
      ...
      reply.header("x-ratelimit-remaining", String(Math.max(0, rule.max - 1)));
      return;
    }
