...
import { inspectSqlite, rankTablesByRelevance } from "../migration/formats/sqlite.js";
import { runMigration } from "../migration/engine.js";
import { buildDbfFile, encodeSingleByte, type DbfFixtureField } from "../migration/tests/fixtures.js";
import { detectSourceShape, streamSourceRows } from "../migration/streamStage.js";
 ...
 */
function buildFptFile(texts: string[], encoding: string, blockSize = 64): { buffer: Buffer; pointers: number[] } {
 ...
 */
function buildDbtFile(texts: string[], encoding: string): { buffer: Buffer; pointers: number[] } {
  const blockSize = 512;
  const header = Buffer.alloc(blockSize);
  const blocks: Buffer[] = [];
  const pointers: number[] = [];
...
function buildDicomFile(values: {
  ...
  const element = (group: number, elementId: number, vr: string, value: Buffer): Buffer => {
    // Значения выравниваются до чётной длины: текст — пробелом, UID — нулём.
    const padded =
      value.length % 2 === 0 ? value : Buffer.concat([value, Buffer.from([vr === "UI" ? 0x00 : 0x20])]);
  ...
  const metaLength = element(0x0002, 0x0000, "UL", (() => {
  ...
  return Buffer.concat([Buffer.alloc(128), Buffer.from("DICM", "latin1"), metaLength, metaElements, dataset]);
}
...
const [org] = await db.insert(organizations).values({ name: `E2E-formats-${Date.now()}` }).returning();
const ORG = org!.id;
...
try {
  ...
  const foxproRows = [
    ["101", "Иванов Иван Иванович", "89001234567", "19800115", String(fpt.pointers[0])],
    ["102", "Петрова Мария Сергеевна", "89161112233", "19920320", String(fpt.pointers[1])],
    ["103", "Сидоров Алексей Николаевич", "84957778899", "19751122", String(fpt.pointers[2])]
  ];
  const dbfWithMemo = buildDbfFile(foxproRows.length > 0 ? foxproFields : foxproFields, foxproRows, {
    languageDriver: 0x65,
    encoding: "ibm866",
    version: 0xf5
  });
  ...
  const dbfFormat = identifyFormat(dbfWithMemo.subarray(0, 4096), "PACIENT.DBF");
  same("формат опознан", dbfFormat.id, "dbf");
  same("версия FoxPro с memo", dbfFormat.version, "FoxPro 2.x с memo");
  check("предупреждение о memo выдано", dbfFormat.guidance?.includes("memo") === true, dbfFormat.guidance ?? "нет");
  ...
  for await (const batch of streamSourceRows({ filePath: dbfPath, fileName: "PACIENT.DBF", shape: dbfShape })) {
  ...
  check("второй memo прочитан целиком", dbfBatches[1]?.[4] === memoTexts[1], (dbfBatches[1]?.[4] ?? "").slice(0, 50));
  check("третий memo прочитан", dbfBatches[2]?.[4] === memoTexts[2], (dbfBatches[2]?.[4] ?? "").slice(0, 50));

  // =====================================================================
  console.log("--- 2. DBF dBASE III с memo .DBT (терминатор 0x1A, длины нет)");
  const dbtTexts = ["Хронический пародонтит средней степени тяжести.", "Состояние после имплантации 36 зуба."];
  ...
  for await (const batch of streamSourceRows({ filePath: dbase3Path, fileName: "KARTA.DBF", shape: dbase3Shape })) {
    dbase3Rows.push(...batch.rows);
  }
  same("прочитано 2 записи dBASE III", dbase3Rows.length, 2);
  check("memo .DBT прочитан по терминатору", dbase3Rows[0]?.[2] === dbtTexts[0], (dbase3Rows[0]?.[2] ?? "").slice(0, 50));
  check("второй блок .DBT прочитан", dbase3Rows[1]?.[2] === dbtTexts[1], (dbase3Rows[1]?.[2] ?? "").slice(0, 50));
  ...
  const sqlitePath = path.join(workDir, "clinic.db");
  {
    ...
    const names = [
      ["Волкова Анна Петровна", "+79031234567", "1988-07-07", "Ортодонтия, брекеты"],
      ["Соколов Дмитрий Игоревич", "89052223344", "1970-11-30", "Импланты 36, 46"],
      ["Лебедева Ирина Олеговна", "89091112233", "1995-02-14", "Отбеливание"]
    ];
    names.forEach((row, index) => insertPatient.run(index + 1, row[0]!, row[1]!, row[2]!, row[3]!));
    const insertVisit = database.prepare("insert into visits (id, patient_id, visit_date, complaint) values (?,?,?,?)");
  ...
  same("SQLite опознан по сигнатуре", identifyFormat(sqliteHead, "clinic.db").id, "sqlite");

  const inspection = inspectSqlite(sqlitePath);
  const ranked = rankTablesByRelevance(inspection.tables);
  console.log(`       таблицы: ${inspection.tables.map((t) => `${t.name}(${t.rowCount})`).join(", ")}`);
  same("самой значимой признана таблица пациентов", ranked[0]?.name, "patients");
  check("служебная таблица settings исключена", !ranked.some((t) => t.name === "settings"), ranked.map((t) => t.name).join(","));
  ...
  check("перечислены все таблицы с данными", (sqliteShape.availableTables?.length ?? 0) >= 2,
    JSON.stringify(sqliteShape.availableTables));

  const sqliteRows: string[][] = [];
  for await (const batch of streamSourceRows({ filePath: sqlitePath, fileName: "clinic.db", shape: sqliteShape })) {
    sqliteRows.push(...batch.rows);
  }
  same("прочитано 3 пациента из SQLite", sqliteRows.length, 3);
  check("ФИО из SQLite целы", sqliteRows.some((row) => row.includes("Волкова Анна Петровна")), sqliteRows[0]?.join("|"));
  ...
  same("DICOM опознан", identifyFormat(dicomBuffer.subarray(0, 4096), "IM000001").id, "dicom");

  const dicom = await readDicomMetadata(dicomPath);
  console.log(`       пациент: ${dicom.patientName}, ДР ${dicom.patientBirthDate}, ${dicom.modality}, ${dicom.manufacturer}`);
  /**
   ...
   */
  same("ФИО прочитано и развёрнуто из формы с ^", dicom.patientName, "Иванов Иван Иванович");
  ...
  same("UID исследования", dicom.studyInstanceUid, "1.2.840.113619.2.55.3.2831178355.8");
  same("толщина среза", dicom.sliceThickness, "0.25");

  // =====================================================================
  console.log("--- 5. Закрытые форматы: опознание и инструкция вместо выдумки");
  const closedFormats: Array<{ name: string; buffer: Buffer; expectedId: string; expectHint: RegExp }> = [
    { name: "IDENT.FDB", buffer: buildFirebirdHeader(12), expectedId: "firebird", expectHint: /IDENT|Firebird|ISQL/i },
    { name: "clinic.mdb", buffer: buildAccessJet4Header(), expectedId: "access_jet4", expectHint: /Access|CSV/i },
    { name: "dentalpro.bak", buffer: buildMssqlBackupHeader(), expectedId: "mssql_backup", expectHint: /RESTORE|SQL Server/i },
    { name: "1Cv8.1CD", buffer: buildOneCHeader(), expectedId: "onec_1cd", expectHint: /1С|конфигуратор/i }
  ];
  ...
  for (const item of closedFormats) {
    ...
    same(`${item.name}: движок честно говорит, что не читает`, format.readable, false);
    check(
      `${item.name}: выдана конкретная инструкция`,
      typeof format.guidance === "string" && item.expectHint.test(format.guidance),
      (format.guidance ?? "нет").slice(0, 90)
    );
    if (format.version) console.log(`       ${item.name}: ${format.title}, ${format.version}`);
  }

  // =====================================================================
  console.log("--- 6. Поиск по каталогу: что нашлось и что с этим делать");
  const discovery = await discoverLocalSources({ roots: [workDir], maxDepth: 2, timeBudgetMs: 20_000 });
  const summary = summarizeDiscovery(discovery);
  console.log(`       просмотрено файлов: ${discovery.filesScanned}, найдено источников: ${discovery.sources.length}`);
  ...
  check("найдены читаемые источники", summary.readable >= 3, `читаемых ${summary.readable}`);
  check("найдены требующие выгрузки", summary.needsExport >= 4, `нечитаемых ${summary.needsExport}`);
  check("каталог снимков распознан отдельно", discovery.imagingFolders.length >= 1,
    JSON.stringify(discovery.imagingFolders.map((f) => `${f.directory}:${f.fileCount}`)));
  check(
    "у DBF показано число записей",
    discovery.sources.some((s) => s.format.id === "dbf" && s.details.some((d) => d.includes("Записей"))),
    discovery.sources.find((s) => s.format.id === "dbf")?.details.join("; ") ?? "нет"
  );
  check(
    "у SQLite показаны таблицы",
    discovery.sources.some((s) => s.format.id === "sqlite" && s.details.some((d) => d.includes("patients"))),
    discovery.sources.find((s) => s.format.id === "sqlite")?.details.join("; ") ?? "нет"
  );
  ...
  console.log(`       создано ${migration.run.loadedRows}, карантин ${migration.run.quarantinedRows}`);
  const loaded = await db
    .select({ fullName: patients.fullName, phone: patients.phone, notes: patients.notes })
    .from(patients)
    .where(eq(patients.organizationId, ORG));
  same("перенесено 3 пациента", loaded.length, 3);
  check("телефон нормализован", loaded.some((p) => p.phone === "+79001234567"), loaded.map((p) => p.phone).join(","));
  /**
  ...
  console.log(`       создано ${sqliteMigration.run.loadedRows}, карантин ${sqliteMigration.run.quarantinedRows}`);
  const afterSqlite = await db
    .select({ n: sql<string>`count(*)` })
    .from(patients)
    .where(eq(patients.organizationId, ORG));
  check("пациенты из SQLite добавились", Number(afterSqlite[0]!.n) >= 6, `всего ${afterSqlite[0]!.n}`);
  check("сверка сошлась", sqliteMigration.reconciliation.balanced,
    sqliteMigration.reconciliation.checks.filter((c) => !c.passed).map((c) => c.title).join("; ") || "ок");
} catch (error) {
  fail += 1;
  ...
  console.log("\n--- Уборка");
  await db.delete(patients).where(eq(patients.organizationId, ORG)).catch(() => undefined);
  ...
  for (const run of runs) {
    ...
    await db.delete(migrationReconciliations).where(eq(migrationReconciliations.runId, run.id));
  }
  ...
  if (pass === 0) console.error("Ни одна проверка не выполнена — считаем это провалом.");
  await pool.end();
