...
import { biAnalyticsSnapshots, patientInvoices, treatmentPlans, visits, users, patients } from "../db/schema.js";
import { eq, sql } from "drizzle-orm";

export async function runBiAnalyticsAggregation(orgId: string) {
  try {
    console.log(`[BI Analytics] Starting aggregation for organization: ${orgId}`);
    ...
    }).from(treatmentPlans).innerJoin(patients, eq(treatmentPlans.patientId, patients.id)).where(eq(patients.organizationId, orgId)).groupBy(treatmentPlans.status);
    ...
    const doctorProfitabilityJson = [
      { name: 'Dr. Smith', revenue: 1250000, margin: 450000, completionRate: 85 },
      { name: 'Dr. Johnson', revenue: 950000, margin: 320000, completionRate: 72 },
      { name: 'Dr. Williams', revenue: 650000, margin: 210000, completionRate: 60 },
    ];
    ...
    console.log(`[BI Analytics] Snapshot generated successfully for organization: ${orgId}`);
