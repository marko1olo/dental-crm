...
async function seedAuth() {
	...
	if (existingOrg) {
		await db
			...
			.where(eq(schema.organizations.id, DEMO_ORG_ID));
		console.log(
			`  ✅ Organization updated: ${DEMO_CLINIC_NAME} [${DEMO_ORG_ID}]`,
		);
	} else {
		...
		console.log(
			`  ✅ Organization created: ${DEMO_CLINIC_NAME} [${DEMO_ORG_ID}]`,
		);
	}
	...
	for (const staff of DEMO_STAFF) {
		...
		if (existingUser) {
			...
			console.log(
				`  ✅ User PIN updated: ${staff.fullName} (${staff.role}) [PIN: ${pin}]`,
			);
		} else {
	...
	for (const s of DEMO_STAFF) {
		console.log(
			`    ${s.fullName.padEnd(32)} PIN: ${s.isAdmin ? adminPin : staffPin}`,
		);
	}
}
