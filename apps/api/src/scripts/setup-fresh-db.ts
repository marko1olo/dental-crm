...
const existing = await (db as any).query(
...
if (!password) {
  console.error(
    "[SETUP] Error: CLINIC_PASSWORD environment variable must be provided.",
  );
  process.exit(1);
}
...
console.log(`  Clinic login: clinic@example.com / ${password}`);
console.log(`  Staff login:  owner@clinic.com / ${password}`);
