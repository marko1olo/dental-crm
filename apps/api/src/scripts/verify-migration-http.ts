...
const SURNAMES = [
  ...
  "Новиков", "Фёдоров", "Егоров", "Павлов", "Козлов", "Степанов", "Николаев", "Орлов",
  "Андреев", "Макаров", "Никитин", "Захаров"
];
const GIVEN_NAMES = [
  "Александр", "Мария", "Дмитрий", "Анна", "Сергей", "Ольга", "Андрей", "Елена",
  "Алексей", "Наталья", "Михаил", "Ирина", "Николай", "Татьяна", "Владимир", "Светлана"
];
const PATRONYMICS = [
  "Иванович", "Сергеевна", "Петрович", "Андреевна", "Николаевич", "Дмитриевна",
  "Алексеевич", "Владимировна"
];

console.log(`\n=== HTTP-конвейер переноса: ${ROWS} записей DBF через живой API ===\n`);

const [org] = await db.insert(organizations).values({ name: `E2E-http-${Date.now()}` }).returning();
const ORG = org!.id;
...
const baseUrl = typeof address === "object" && address !== null ? `http://127.0.0.1:${address.port}` : "";
...
const authHeaders: Record<string, string> = {
  ...
  [STAFF_TOKEN_HEADER]: signToken({ userId: null, role: "admin", organizationId: ORG }, secret, 3600)
};
...
async function api(method: string, path: string, body?: unknown, extraHeaders: Record<string, string> = {}): Promise<ApiResult> {
...
try {
  ...
  const dbf = buildDbfFile(
    ...
    Array.from({ length: ROWS }, (_, index) => {
      ...
      const patronymic =
        PATRONYMICS[Math.floor(index / (SURNAMES.length * GIVEN_NAMES.length)) % PATRONYMICS.length]!;
  ...
  console.log(`       размер файла: ${(dbf.byteLength / (1024 * 1024)).toFixed(2)} МБ`);
  console.log(`       память после сборки образца: heap ${heapUsedMb()} МБ (было ${memBeforeBuild}), rss ${rssMb()} МБ`);
  ...
  same("колонки прочитаны", (source?.columns as string[] | undefined)?.length, 6);
  console.log(`       заливка заняла ${uploadMs} мс, память heap ${heapUsedMb()} МБ, rss ${rssMb()} МБ`);

  // =====================================================================
  console.log("--- 3. POST /:runId/map — сопоставление колонок");
  const mapResult = await api("POST", `/api/migration/${runId}/map`, { allowLlm: false, entityKind: "patient" });
  same("map вернул 200", mapResult.status, 200);
  const mapping = mapResult.body.mapping as Record<string, unknown> | undefined;
  const mappedColumns = (mapping?.columns ?? []) as Array<{ sourceColumn: string; targetField: string }>;
  console.log(`       карта: ${mappedColumns.map((c) => `${c.sourceColumn}→${c.targetField}`).join(", ")}`);
  same("сущность определена как пациенты", mapping?.entityKind, "patient");
  check("ФИО сопоставлено", mappedColumns.some((c) => c.targetField === "patient.fullName"));
  check("телефон сопоставлен", mappedColumns.some((c) => c.targetField === "patient.phone"));
  check("дата рождения сопоставлена", mappedColumns.some((c) => c.targetField === "patient.birthDate"));

  // =====================================================================
  console.log("--- 4. POST /:runId/execute — обязан вернуть 202 немедленно");
  const executeStarted = Date.now();
  const executeResult = await api("POST", `/api/migration/${runId}/execute`, { dryRun: false, sourceSystem: "ident10k" });
  ...
  console.log(`       ответ за ${executeMs} мс: ${String(executeResult.body.message)}`);

  // Повторное нажатие не должно поставить в очередь второй раз.
  const doubleExecute = await api("POST", `/api/migration/${runId}/execute`, { dryRun: false });
  same("повторный execute отклонён 409", doubleExecute.status, 409);
  const doubleError = doubleExecute.body.error as Record<string, unknown> | undefined;
  same("код ошибки повторной постановки", doubleError?.code, "RunAlreadyQueued");
  check(
    "формат ошибки {error:{code,message,details}}",
    typeof doubleError?.code === "string" && typeof doubleError?.message === "string" && typeof doubleError?.details === "object",
  ...
  console.log(`       обработка ${ROWS} строк заняла ${(workMs / 1000).toFixed(1)} с`);
  ...
  console.log(`       статус ${String(run.status)}, фаза «${String(run.phase)}»`);
  console.log(
  ...
  check("прогон завершён", run.status === "completed" || run.status === "completed_with_quarantine", String(run.status));
  ...
  const given5000 = GIVEN_NAMES[Math.floor(index5000 / SURNAMES.length) % GIVEN_NAMES.length]!;
  const patronymic5000 =
    PATRONYMICS[Math.floor(index5000 / (SURNAMES.length * GIVEN_NAMES.length)) % PATRONYMICS.length]!;
  ...
  const [sampleRow] = await db
    ...
    .from(patients)
    .where(and(eq(patients.organizationId, ORG), eq(patients.fullName, expectedName5000)));
  check("кириллица из cp866 цела на 5000-й строке", sampleRow !== undefined, sampleRow?.fullName ?? "не найдено");
  check("телефон нормализован", sampleRow?.phone?.startsWith("+79") === true, sampleRow?.phone ?? "нет");
  check("дата рождения разобрана", /^\d{4}-\d{2}-\d{2}$/.test(sampleRow?.birthDate ?? ""), sampleRow?.birthDate ?? "нет");

  // =====================================================================
  console.log("--- 8. GET /:runId/reconciliation — акт сверки");
  const reconResult = await api("GET", `/api/migration/${runId}/reconciliation`);
  same("reconciliation вернул 200", reconResult.status, 200);
  const checks = reconResult.body.checks as Array<{ title: string; expected: number; actual: number; passed: boolean }>;
  for (const item of checks) {
    console.log(`       ${item.passed ? "[+]" : "[-]"} ${item.title}: ожидалось ${item.expected}, получено ${item.actual}`);
  }
  check("сверка сошлась", reconResult.body.balanced === true, checks.filter((c) => !c.passed).map((c) => c.title).join("; ") || "ок");

  const csvResult = await fetch(`${baseUrl}/api/migration/${runId}/reconciliation.csv`, { headers: authHeaders });
  /**
  ...
  check("CSV содержит итог сверки", csvText.includes("СОШЛОСЬ"), csvText.split("\r\n")[2] ?? "");
  ...
  const secondUpload = await fetch(`${baseUrl}/api/migration/upload`, {
    ...
    headers: {
      ...
      "content-type": "application/octet-stream",
      "x-migration-file-name": "PACIENT.DBF",
      "x-migration-source-name": encodeURIComponent("Выгрузка IDENT 10k (повтор)")
    },
    body: dbf
  });
  ...
  await api("POST", `/api/migration/${secondRunId}/map`, { allowLlm: false, entityKind: "patient" });
  await api("POST", `/api/migration/${secondRunId}/execute`, { dryRun: false, sourceSystem: "ident10k" });
  ...
  const [afterSecond] = await db
    ...
    .from(patients)
    .where(eq(patients.organizationId, ORG));
  same("повторный прогон не создал новых карточек", Number(afterSecond!.n), ROWS);

  const secondStatus = await api("GET", `/api/migration/${secondRunId}`);
  const secondCounters = (secondStatus.body.run as Record<string, unknown>).counters as Record<string, number>;
  console.log(
  ...
  const recognised = (secondCounters.updatedRows ?? 0) + (secondCounters.duplicateRows ?? 0);
  check("второй прогон узнал уже перенесённых", recognised === ROWS, `обновлено+дублей = ${recognised}`);
  ...
  const thirdRunId = String(((await thirdUpload.json()) as Record<string, unknown>).runId ?? "");
  await api("POST", `/api/migration/${thirdRunId}/map`, { allowLlm: false, entityKind: "patient" });
  // Укладываем строки отдельной фазой, но НЕ выполняем.
  const stageResult = await api("POST", `/api/migration/${thirdRunId}/stage`, {});
  ...
  check("осиротевший прогон подобран", resumed >= 1, `обработано прогонов: ${resumed}`);
  ...
  console.log(`       статус после возобновления: ${String(resumeRun.status)}, подборов ${String(resumeWorker.resumeCount)}`);
  console.log(`       создано ${resumeCounters.loadedRows} из 200`);
  check("прогон доведён до конца", resumeRun.status === "completed" || resumeRun.status === "completed_with_quarantine", String(resumeRun.status));
  check("зафиксирован факт возобновления", Number(resumeWorker.resumeCount) >= 1, String(resumeWorker.resumeCount));
  same("загружены все 200 строк", resumeCounters.loadedRows, 200);

  // =====================================================================
  console.log("--- 11. Формат ошибок на несуществующем прогоне");
  const missing = await api("GET", "/api/migration/00000000-0000-4000-8000-000000000000");
  same("несуществующий прогон — 404", missing.status, 404);
  const missingError = missing.body.error as Record<string, unknown> | undefined;
  same("код ошибки", missingError?.code, "RunNotFound");
  check("сообщение на русском", typeof missingError?.message === "string" && /[А-Яа-я]/.test(String(missingError.message)));
} catch (error) {
  /**
