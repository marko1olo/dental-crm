...
if (!organizationId) {
	console.error(
		"Укажите идентификатор организации: npx tsx src/scripts/inspectRecall.ts <organizationId>",
	);
	process.exit(1);
}
...
const query = db
	.select({
		...
		lastCompletedAt:
			sql<Date | null>`(SELECT max(a.starts_at) FROM ${appointments} a WHERE a.patient_id = ${patients}."id" AND a.status = 'completed')`.as(
				"last_completed_at",
			),
		...
		totalAppointments:
			sql<number>`(SELECT count(*) FROM ${appointments} a WHERE a.patient_id = ${patients}."id")`.as(
				"total_appointments",
			),
	})
	.from(patients)
...
const [totals] = await db
	.select({
		...
		completed:
			sql<number>`count(*) FILTER (WHERE ${appointments.status} = 'completed')::int`.as(
				"completed",
			),
	})
	.from(appointments)
...
console.log(
	`Приёмов в клинике всего: ${totals?.total ?? 0}, из них завершённых: ${totals?.completed ?? 0}`,
);
...
for (const row of rows) {
	const last = row.lastCompletedAt
		? new Date(row.lastCompletedAt).toISOString().slice(0, 10)
		: "нет";
	console.log(
