 ...
 * 404. ОДИН УЖЕ ЗАКРЫТ ИСПОЛНЕНИЕМ — DELETE /api/clinical/rules зарегистрирован, и
 * строку снял сам храповик ниже, а не ревизия глазом (история — в комментарии на
 * её месте). Остался один, и починка его — правка apps/api, то есть работа за
 * границей этого файла; поэтому здесь долг назван, а не спрятан. Список не должен
 * расти.
 */
...
describe("адреса, которые зовёт интерфейс", () => {
	...
	test("список известного долга не разрастается молча", () => {
		 ...
		 *
		 * Сейчас 1: DELETE /api/clinical/rules закрыт настоящим маршрутом, и число
		 * опущено вместе со снятой строкой — по правилу соседней проверки выше.
		 * Оставить здесь 2 значило бы держать свободную единицу, то есть разрешить
		 * добавить одну строку долга молча, а ради запрета ровно такого добавления
		 * эта проверка и написана. Появится второй случай — его придётся объявить
		 * вместе с этим числом.
		 */
		assert.ok(
			KNOWN_METHOD_MISMATCH.length <= 1,
