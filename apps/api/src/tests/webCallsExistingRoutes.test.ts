...
describe("адреса, которые зовёт интерфейс", () => {
	...
	test("починенные адреса удаляются из списка долга", () => {
		...
		/*
		 * Тот же сторож для долга по методу. Оба случая стоят в очереди на починку в
		 * apps/api, и когда DELETE /api/clinical/rules или PUT /api/xray/scans будут
		 * зарегистрированы, эта строка покраснеет — так и задумано: долг, который не
		 * убирают после починки, за месяц превращается в список, на который никто не
		 * смотрит. Именно на такой красноте из KNOWN_MISSING ушёл /api/billing/payouts.
		 */
