...
describe("splitSqlStatements — границы выражений", () => {
	...
	it("считает блочные комментарии вложенными, как требует PostgreSQL", () => {
		...
		assert.equal(
			splitSqlStatements("SELECT 1 /* a /* b; */ c; */ ;").length,
			1,
		);
	});
	...
	it("не режет тело $$…$$ и $tag$…$tag$", () => {
		assert.equal(
			splitSqlStatements("DO $$ BEGIN a; b; END $$; SELECT 1;").length,
			2,
		);
		assert.equal(
			splitSqlStatements(
	...
	it("допускает вложение долларовых литералов с РАЗНЫМИ тегами", () => {
		assert.equal(
			splitSqlStatements(
				"DO $outer$ x $inner$ a; b $inner$ y; $outer$; SELECT 1;",
			).length,
			2,
		);
	});
...
describe("carriesConcurrently — CONCURRENTLY в коде, а не в комментарии", () => {
	it("не считает упоминание в комментарии", () => {
		assert.equal(
			carriesConcurrently("-- CONCURRENTLY\nCREATE INDEX i ON t (c);"),
			false,
		);
		assert.equal(
			carriesConcurrently("/* CONCURRENTLY */ CREATE INDEX i ON t (c);"),
			false,
		);
	});
	...
	it("считает настоящий оператор", () => {
		assert.equal(
			carriesConcurrently("CREATE INDEX CONCURRENTLY i ON t (c);"),
			true,
		);
	});
});
...
describe("реальные миграции репозитория", () => {
	it("0134: DO-блок остаётся целым, за ним отдельный CREATE UNIQUE INDEX", () => {
		const statements = splitSqlStatements(
			migration("0134_ai_jobs_recording_path_index.sql"),
		);
	...
	it("0134 и 0141 объясняют в комментарии, почему НЕ используют CONCURRENTLY, и проверку не будят", () => {
		assert.equal(
			carriesConcurrently(migration("0134_ai_jobs_recording_path_index.sql")),
			false,
		);
		assert.equal(
			carriesConcurrently(migration("0141_hot_path_indexes.sql")),
			false,
		);
	});
	...
	it("0155 и 0156 действительно содержат CONCURRENTLY и помечены no-transaction", () => {
		for (const name of [
			"0155_paranoid_fk_indexes.sql",
			"0156_paranoid_gin_indexes.sql",
		]) {
			const sql = migration(name);
			assert.equal(carriesConcurrently(sql), true, name);
			assert.match(
				sql.split(/\r?\n/)[0] ?? "",
				/^\s*--\s*no-transaction\s*$/,
				name,
			);
		}
	});
