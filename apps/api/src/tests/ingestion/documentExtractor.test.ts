...
import { DocumentIngestionRequest } from '@dental/shared';
...
describe('documentExtractor', () => {
  describe('extractDocument', () => {
    test('extracts plain text provided via rawText', () => {
      ...
      assert.ok(result.routes.some(route => route.target === 'plain_text' && route.enabled));
    });
    ...
    test('handles PDF format', () => {
      ...
      assert.ok(result.warnings.includes('pdf_text_not_extracted_may_be_scanned'));
    ...
    test('handles legacy database format', () => {
      const base64Content = Buffer.from('SQLite format 3\u0000').toString('base64');
      ...
      assert.ok(result.extractedText.includes('Источник старой базы: база SQLite Источник миграции'));
      assert.ok(result.extractedText.includes('Тип источника: старая база'));
      assert.ok(result.warnings.includes('legacy_source_staging_manifest_only'));
    });
    ...
    test('decodes unknown format as text', () => {
      const base64Content = Buffer.from('some text in an unknown extension').toString('base64');
      ...
      assert.strictEqual(result.extractedText, 'some text in an unknown extension');
