...
describe("loadServerEnv", () => {
	...
	describe("getLoadedServerEnvFiles", () => {
		test("should return an array of strings without duplicates", () => {
			...
			assert.ok(
				Array.isArray(afterFiles),
				"getLoadedServerEnvFiles should return an array",
			);
			assert.deepStrictEqual(
				afterFiles,
				returnedFiles,
				"getLoadedServerEnvFiles should return the same array as loadAdditionalServerEnv",
			);
			...
			assert.strictEqual(
				afterFiles.length,
				uniqueFiles.size,
				"The returned array should not contain duplicate entries",
			);

			// Verify the expected files are in the array
			assert.ok(
				afterFiles.includes(path.resolve(testEnvPath)),
				"The explicit test environment file should be included in the loaded files list",
			);
			assert.ok(
				afterFiles.includes(path.resolve(anotherEnvPath)),
				"The extra environment file should be included in the loaded files list",
			);
			...
			assert.strictEqual(
				...
				"123",
				"The environment variable from the mock file should be loaded",
			);
			assert.strictEqual(
				process.env.ANOTHER_VAR,
				"456",
				"The environment variable from the another file should be loaded",
			);
		});
		...
		test("should handle read errors gracefully without throwing", () => {
			...
			assert.ok(
				!afterFiles.includes(path.resolve(errorEnvPath)),
				"The error file/directory should not be included in loaded files",
			);
