...
describe("taxPaymentSelectionErrorForDocument", () => {
	...
	test("detects unpaid or zero amount payments", () => {
		...
		let error = taxPaymentSelectionErrorForDocument(input, [
			createPayment({ status: "planned" }),
		]);
		assert.strictEqual(
			error,
		...
		error = taxPaymentSelectionErrorForDocument(input, [
			createPayment({ amountRub: 0 }),
		]);
		assert.strictEqual(
			error,
			"В налоговый документ можно включать только проведенные положительные оплаты.",
	...
	test("detects payment outside of tax year", () => {
		...
		const payment = createPayment({
			fiscalReceiptIssuedAt: "2023-12-31T23:59:59Z",
	...
	test("validates payments for tax_deduction_application if provided", () => {
		...
		const validError = taxPaymentSelectionErrorForDocument(input, [
			validPayment,
		]);
