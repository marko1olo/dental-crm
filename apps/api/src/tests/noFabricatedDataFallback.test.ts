 ...
 *      (объявленная подмена в `lostPatientsFiltersQuery.ts` перестала бы находиться и
 *      покраснела бы как `stale`) — но ЧАСТИЧНОЕ не поймал бы никто: одна
...
test("перепись слоя доступа не выродилась", () => {
	...
	assert.ok(
		census.databaseTryBlocks >= 11,
		`Перепись нашла ${census.databaseTryBlocks} блоков try с обращением к базе, а на момент установки ` +
			"порога их было 11. Обращение к базе распознаётся по получателю (db, tx, trx, pool) и по " +
