...
describe("привязка Telegram переживает перезапуск сервера", () => {
	...
	test("привязка чата доходит до базы, а не только до массива процесса", async () => {
		...
		assert.equal(
			linkCodeResponse.statusCode,
			200,
			`код привязки не выпущен: ${linkCodeResponse.body}`,
		);
		...
		assert.equal(
			...
			200,
			`веб-хук отказал: ${webhookResponse.body}`,
		);
		assert.equal(
		...
		assert.equal(
			row.subjectId,
			PATIENT_SUBJECT,
			"связка записана не тому пациенту",
		);
		...
		assert.equal(
			row.clinicId,
			null,
			"филиал записан идентификатором организации — внешний ключ должен был это отвергнуть",
		);
	});
	...
	test("после перезапуска (пустой массив процесса) список связок всё равно показывает привязку", async () => {
		...
		assert.equal(
			...
			200,
			chatLinksBeforeRestart.body,
		);
		assert.equal(
			chatLinksBeforeRestart.json().activeCount,
			1,
			chatLinksBeforeRestart.body,
		);
		...
		assert.equal(
			inMemoryChatLinks.length,
			0,
			"массив процесса не очищен — перезапуск не смоделирован",
		);
		...
		const list = afterRestart.json() as {
			...
			chatLinks: {
				subjectId: string;
				status: string;
				chatTransportRef?: unknown;
			}[];
		};
		assert.equal(
			list.totalCount,
			1,
			`после перезапуска связок ${list.totalCount}, ожидалась одна`,
		);
		assert.equal(
			list.activeCount,
			1,
	...
	test("отзыв привязки доходит до базы, а не только до памяти", async () => {
		...
		assert.equal(
			...
			404,
			`повторный отзыв ответил ${revokedTwice.statusCode}`,
		);
	});
	...
	test("отбор по состоянию и страницы считаются базой, а не после выборки", async () => {
		...
		assert.equal(
			firstPageList.cursor,
			null,
			"у первой страницы не должно быть входного курсора",
		);
		assert.equal(
			firstPageList.nextCursor,
			"1",
			`следующая страница не предложена: ${firstPage.body}`,
		);
		...
		assert.equal(
			secondPageList.nextCursor,
			null,
			"после второй страницы предложена третья",
		);
		...
		assert.equal(
			...
			2,
			`страницы пересеклись: ${pagedIds.join(", ")}`,
		);

		await withFixtureTenant(runtime.organizationId, async () => {
			await db
				.delete(denteTelegramChatLinks)
				.where(eq(denteTelegramChatLinks.id, second.id));
		});
	});
	...
	test("чужая клиника не видит связку и не может её отозвать", async () => {
		...
		if (foreignList.statusCode === 200) {
			const list = foreignList.json() as {
				totalCount: number;
				chatLinks: unknown[];
			};
			assert.equal(
				list.totalCount,
				0,
				`чужая клиника увидела связки: ${foreignList.body}`,
			);
		...
		assert.ok(
			!foreignList.body.includes(PATIENT_SUBJECT),
			"в ответе чужой клинике утёк пациент",
		);
		assert.ok(
			!foreignList.body.includes(linkId),
			"в ответе чужой клинике утёк идентификатор связки",
		);
	});
	...
	test("отбор по арендатору стоит в самом запросе, а не в разборе маршрута", async () => {
		...
		const mine = await buildDenteTelegramChatLinkList({
			organizationId: runtime.organizationId,
		});
		const theirs = await buildDenteTelegramChatLinkList({
			organizationId: FOREIGN_ORG,
		});
		...
		assert.equal(
			mine.totalCount,
			1,
			`своя клиника видит ${mine.totalCount} связок вместо одной`,
		);
		assert.equal(
			theirs.totalCount,
			1,
			`чужая клиника видит ${theirs.totalCount} связок вместо одной`,
		);
		assert.deepEqual(
		...
		assert.equal(
			await countActiveDenteTelegramChatLinks({ organizationId: FOREIGN_ORG }),
			1,
		);
		assert.equal(
			await countActiveDenteTelegramChatLinks({
				organizationId: runtime.organizationId,
			}),
			0,
		);
		...
		assert.equal(
			await revokeDenteTelegramChatLink(
				{ organizationId: runtime.organizationId },
				foreignLink.id,
			),
			null,
