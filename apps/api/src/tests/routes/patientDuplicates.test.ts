...
describe("поиск и слияние дублей", () => {
	...
	test("слияние переносит все записи и ничего не теряет", async (context) => {
		...
		// Приём, визит и оплата теперь у оставшейся карточки.
		const movedAppointments = await db
		...
		const leftoverAppointments = await db
			.select({ id: appointments.id })
			.from(appointments)
			.where(eq(appointments.patientId, DUP_SECOND));
		assert.equal(leftoverAppointments.length, 0);
		const leftoverPayments = await db.select({ id: payments.id }).from(payments).where(eq(payments.patientId, DUP_SECOND));
