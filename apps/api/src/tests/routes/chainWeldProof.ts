...
async function main(): Promise<void> {
	...
	try {
		...
		console.log(
			"РАЗРЫВ: смета лежит в treatment_plans/treatment_plan_items_new, а все деньги читают treatment_items — там 0 строк.",
		);
		...
		console.log(
			`позиций лечения у подписанного приёма: ${itemsAfterAccept?.n} — подписание карты приёма позиции не создаёт и не закрывает ` +
				"(visitsQuery.ts: acceptVisitDraftInDb трогает только visits)",
		);
	} finally {
