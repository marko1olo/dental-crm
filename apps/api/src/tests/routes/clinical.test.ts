...
describe('clinical routes integration', () => {
  ...
  test('PATCH /api/clinical/rules/:ruleId succeeds', async () => {
    ...
    mock.method(db, 'select', () => ({
        from: () => ({
            where: () => ({ limit: async () => [{ id: 'rule1', organizationId: ORG_ID, title: 'old', category: 'consultation', specialty: 'therapist', action: 'show_warning', severity: 'warning', ownerRole: 'doctor', triggerServiceIdsJson: '["s1"]', requiredServiceIdsJson: '[]', requiresCompletedServiceIdsJson: '[]', blockedServiceIdsJson: '[]', condition: null, warningText: 'warning', patientText: 'patient', isActive: true }] })
        })
    }));
