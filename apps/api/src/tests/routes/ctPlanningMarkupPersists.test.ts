 ...
 *
 * ИЗМЕРЕННЫЙ ДЕФЕКТ РАЗБОРА КОЛОНКИ. `db/schema.ts:1901-1903` объявляет
 * `spline_points_json`, `nerve_points_json`, `implants_json` типом `jsonb`, а
 * миграция, создавшая таблицу, объявила их `text DEFAULT '[]' NOT NULL`
 * (`drizzle/0000_freezing_randall_flagg.sql:828-830`), и в живой базе они
 * действительно `text` — миграция 0118 их не переписывала. Расходится именно
 * `schema.ts`. Цена расхождения: у drizzle `jsonb` при записи делает
 * `JSON.stringify` (`drizzle-orm/pg-core/columns/jsonb.js:22`), то есть строка
 * `[{"x":1}]` попадала в текстовую колонку как `"[{\"x\":1}]"` — с кавычками и
 * экранированием. При чтении тот же класс делает `JSON.parse`, поэтому круг
 * сходился, но в самой базе лежал двойной кодировкой текст, а строка, созданная
 * значением колонки по умолчанию (`'[]'`), возвращалась МАССИВОМ, тогда как
 * строка, записанная маршрутом, — СТРОКОЙ. Одна колонка, два разных типа на
 * выходе. Маршрут обходит сбитый разбор явными приведениями к тексту; `schema.ts`
 * занят другой волной и здесь не тронут.
 *
