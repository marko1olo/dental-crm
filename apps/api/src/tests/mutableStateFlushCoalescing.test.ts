...
describe("слияние записей снимка состояния", () => {
	...
	test("непрерывный поток изменений не откладывает запись бесконечно", async () => {
		process.env.DENTAL_STATE_FLUSH_DELAY_MS = "50";
		resetWriteCounters();
		...
		// Окно фиксированное, а не продлеваемое при каждом изменении: за 250 мс
		// потока обязано пройти несколько записей. Продлеваемое окно (классический
		// debounce) не записало бы файл ни разу до конца потока.
		assert.ok(
			stateWriteCount >= 2,
			`поток изменений обязан записываться по ходу дела, а записей ${stateWriteCount}`,
		);
		assert.ok(
			stateWriteCount * 2 < actionCount,
			`записей должно быть кратно меньше числа действий, а их ${stateWriteCount} на ${actionCount}`,
		);
		console.log(
