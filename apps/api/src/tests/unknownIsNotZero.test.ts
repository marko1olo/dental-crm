...
import type { GeneratedDocument, MigrationReconciliationReport, Patient } from "@dental/shared";
 ...
 * отсутствие.
 *
 * Здесь закреплены три места, где подставленный ноль печатался как факт. Каждое
 * утверждение ниже КРАСНЕЛО на коде до правки — это проверено прогоном, а не
 * заявлено: ни одно из них не проходит на подставленном нуле по построению.
 */
...
describe("сумма платежей источника: нечитаемое значение не становится нулём", () => {
  ...
  test("одно нечитаемое значение делает сумму источника неопределимой", () => {
    ...
    assert.equal(measured.unreadableCells, 1, "Нечитаемые значения не посчитаны — оператору нечего показать.");
    assert.equal(measured.parsedCells, 2, "Разобранные значения посчитаны неверно.");
    ...
    assert.equal(substitutedZeroTotal, 350050, "Прежнее выражение изменилось — перепроверьте измерение.");
    assert.notEqual(
      measured.totalKopecks,
  ...
  test("все значения читаемы — сумма точна до копейки", () => {
    ...
    assert.equal(measured.totalKopecks, 350050, "Сумма читаемых значений посчитана неточно.");
  ...
  test("колонка суммы не заполнена нигде — это тоже «не определяется», а не ноль", () => {
    ...
    assert.equal(measured.unreadableCells, 0, "Пустая клетка — не нечитаемое значение: источник прямо говорит, что суммы нет.");
  });
});
...
describe("акт сверки переноса печатает «не определяется», а не молчит", () => {
  ...
  test("неопределимая сумма источника не уносит из акта измеренное «Загружено»", () => {
    ...
    const explanation = moneyLine(csv, "Почему сумма в источнике не определяется");
    assert.ok(
      explanation && explanation.includes("НЕ ВЫПОЛНЯЛАСЬ"),
  ...
  test("все три суммы известны — печатаются как раньше", () => {
    ...
    assert.equal(moneyLine(csv, "Сумма в источнике"), `Сумма в источнике;${expectedMoneyCell(24901.5)}`);
    assert.equal(
  ...
  test("платежей в переносе не было — денежного раздела нет вовсе", () => {
    ...
    assert.ok(!csv.includes("Деньги;Значение"), "Денежный раздел напечатан там, где ни одной суммы нет.");
  });
});
...
describe("график рассрочки не объявляет план оплаченным, когда сумма не определяется", () => {
  ...
  test("неизвестная сумма плана не печатается ни нулём, ни «План полностью оплачен»", () => {
    const html = normalized(renderDocumentHtml(scheduleDocument(null), patient, {}));
  ...
  test("известная сумма плана печатается и делится на платежи как раньше", () => {
    const html = normalized(renderDocumentHtml(scheduleDocument(10000), patient, {}));
