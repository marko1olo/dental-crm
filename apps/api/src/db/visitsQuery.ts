  ...
  return await db.transaction(async (tx) => {
    ...
    if (patientRecord?.insuranceContractId) {
      const [contract] = await tx
        ...
        .where(
          eq(schema.insuranceContracts.id, patientRecord.insuranceContractId),
        );
    ...
    await deductVisitInventoryIdempotent(
      tx,
      organizationId,
      visit.id,
      userId || "SYSTEM",
...
export async function getVisitsByIdsInDb(
  organizationId: string,
  ids: readonly string[],
) {
...
export async function deductVisitInventoryIdempotent(
  tx: any,
  organizationId: string,
  visitId: string,
  userId: string,
  ...
  const [existingDeduction] = await tx
    ...
    .where(
      and(
        eq(schema.inventoryTransactions.visitId, visitId),
        eq(schema.inventoryTransactions.transactionType, "visit_deduction"),
      ),
    )
    .limit(1);
  ...
  const tItems = await tx
    ...
    .where(
      and(
        eq(schema.treatmentItems.visitId, visitId),
        eq(schema.treatmentItems.status, "completed"),
      ),
    );
