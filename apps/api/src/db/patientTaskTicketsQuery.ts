 ...
 */
function toTicket(
	row: typeof schema.patientTaskTickets.$inferSelect,
): PatientTaskTicket {
	return {
		id: row.id,
...
export async function createPatientTaskTicketInDb(
	...
	if (!ownedPatient) {
		throw new Error(
			"patient_task_tickets: patient does not belong to organization",
		);
	}
		...
		const [ownedStaff] = await db
			...
			.where(
				and(
					...
					eq(schema.users.id, input.assignedToId),
				),
			)
			.limit(1);
		if (!ownedStaff) {
			throw new Error(
				"patient_task_tickets: assignee does not belong to organization",
			);
		}
	}
	const [row] = await db
	...
	if (!row) {
		throw new Error(
			"patient_task_tickets: вставка не вернула созданную строку",
		);
	}
	return toTicket(row);
}
