...
// ─────────────────────────────────────────────────────────────
// PHASE 25: ORTHODONTIC CEPHALOMETRICS & TRG ANALYSIS
// ─────────────────────────────────────────────────────────────

export const orthodonticCephalometricStudies = pgTable(
	"orthodontic_cephalometric_studies",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
			.notNull()
		...
		studyDate: timestamp("study_date", { withTimezone: true })
			.notNull()
			.defaultNow(),
		xrayScanId: uuid("xray_scan_id"),
		imageWidthPx: integer("image_width_px").notNull(),
		imageHeightPx: integer("image_height_px").notNull(),
		scaleMmPerPixel: numeric("scale_mm_per_pixel", {
			precision: 8,
			scale: 6,
		}).notNull(),
		calibrationRulerJson: jsonb("calibration_ruler_json").notNull(),
		landmarksJson: jsonb("landmarks_json").notNull(),
		snaAngleDeg: numeric("sna_angle_deg", { precision: 5, scale: 2 }).notNull(),
		snbAngleDeg: numeric("snb_angle_deg", { precision: 5, scale: 2 }).notNull(),
		anbAngleDeg: numeric("anb_angle_deg", { precision: 5, scale: 2 }).notNull(),
		snGoGnAngleDeg: numeric("sn_go_gn_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		u1NaAngleDeg: numeric("u1_na_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		u1NaDistanceMm: numeric("u1_na_distance_mm", {
			precision: 5,
			scale: 2,
		}).notNull(),
		l1NbAngleDeg: numeric("l1_nb_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		l1NbDistanceMm: numeric("l1_nb_distance_mm", {
			precision: 5,
			scale: 2,
		}).notNull(),
		interincisalAngleDeg: numeric("interincisal_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		steinerUpperLipDistanceMm: numeric("steiner_upper_lip_dist_mm", {
			precision: 5,
			scale: 2,
		}).notNull(),
		steinerLowerLipDistanceMm: numeric("steiner_lower_lip_dist_mm", {
			precision: 5,
			scale: 2,
		}).notNull(),
		fmaAngleDeg: numeric("fma_angle_deg", { precision: 5, scale: 2 }).notNull(),
		impaAngleDeg: numeric("impa_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		fmiaAngleDeg: numeric("fmia_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		witsAoBoMm: numeric("wits_ao_bo_mm", { precision: 5, scale: 2 }).notNull(),
		skeletalClass: text("skeletal_class").notNull(),
		facialBiotype: text("facial_biotype").notNull(),
		growthVector: varchar("growth_vector", { length: 32 }).notNull(),
		clinicalInterpretationReport: text("clinical_interpretation_report").notNull(),
		isApprovedByOrthodontist: boolean("is_approved_by_orthodontist")
			.notNull()
			.default(false),
		approvedAt: timestamp("approved_at", { withTimezone: true }),
		createdAt: timestamp("created_at", { withTimezone: true })
	...
	(t) => ({
		orgPatientIdx: index("ortho_ceph_org_patient_idx").on(
			t.organizationId,
			t.patientId,
		),
		studyDateIdx: index("ortho_ceph_study_date_idx").on(
			t.patientId,
			t.studyDate,
		),
	}),
...
export const orthodonticBiomechanicalPlans = pgTable(
	"orthodontic_biomechanical_plans",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
			.notNull()
		...
		patientId: uuid("patient_id")
			.notNull()
			.references(() => patients.id, { onDelete: "cascade" }),
		cephalometricStudyId: uuid("cephalometric_study_id").references(
			() => orthodonticCephalometricStudies.id,
			{ onDelete: "set null" },
		),
		planTitle: varchar("plan_title", { length: 255 }).notNull(),
		applianceType: text("appliance_type").notNull(),
		extractionProtocol: text("extraction_protocol").notNull(),
		incisorRetractionGoalMm: numeric("incisor_retraction_goal_mm", {
			precision: 4,
			scale: 2,
		}).default("0.00"),
		incisorIntrusionGoalMm: numeric("incisor_intrusion_goal_mm", {
			precision: 4,
			scale: 2,
		}).default("0.00"),
		molarDistalizationGoalMm: numeric("molar_distalization_goal_mm", {
			precision: 4,
			scale: 2,
		}).default("0.00"),
		archExpansionGoalMm: numeric("arch_expansion_goal_mm", {
			precision: 4,
			scale: 2,
		}).default("0.00"),
		anchorageRequirement: varchar("anchorage_requirement", {
			length: 32,
		})
			.notNull()
			.default("moderate"),
		tadsCoordinatesJson: jsonb("tads_coordinates_json")
			.notNull()
			.default(sql`'[]'::jsonb`),
		archwireSequenceJson: jsonb("archwire_sequence_json").notNull(),
	...
	(t) => ({
		orgPlanIdx: index("ortho_bio_org_plan_idx").on(
			t.organizationId,
			t.patientId,
		),
	}),
...
// ─────────────────────────────────────────────────────────────
// PHASE 27: ENDODONTICS & NITI ROTARY FILE FATIGUE LIFECYCLE
// ─────────────────────────────────────────────────────────────

export const endoTreatmentSessions = pgTable(
	"endo_treatment_sessions",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
			.notNull()
		...
		toothNumberFdi: integer("tooth_number_fdi").notNull(),
		pulpalDiagnosis: varchar("pulpal_diagnosis", { length: 120 }).notNull(),
		periapicalDiagnosis: varchar("periapical_diagnosis", {
			length: 120,
		}).notNull(),
		rubberDamUsed: boolean("rubber_dam_used").notNull().default(true),
		rubberDamClampNumber: varchar("rubber_dam_clamp_number", { length: 40 }),
		magnificationType: varchar("magnification_type", { length: 60 })
			.notNull()
			.default("microscope"),
		magnificationFactor: numeric("magnification_factor", {
			precision: 4,
			scale: 1,
		}),
		irrigationProtocol: jsonb("irrigation_protocol")
			.notNull()
			.default({
				naoclPct: 3.0,
				edtaPct: 17.0,
				chxPct: 2.0,
				ultrasonicActivation: true,
			}),
		canalCount: integer("canal_count").notNull().default(1),
		isthmusObserved: boolean("isthmus_observed").notNull().default(false),
		isthmusClassification: varchar("isthmus_classification", { length: 40 }),
		cShapedCategory: varchar("c_shaped_category", { length: 40 }),
		temporaryMedication: varchar("temporary_medication", { length: 120 }),
		sealerBrand: varchar("sealer_brand", { length: 120 }),
		obturationTechnique: varchar("obturation_technique", { length: 120 }),
		treatmentPhase: varchar("treatment_phase", { length: 60 })
			.notNull()
	...
	(table) => ({
		orgIdx: index("endo_sessions_org_idx").on(table.organizationId),
		patientIdx: index("endo_sessions_patient_idx").on(table.patientId),
		toothIdx: index("endo_sessions_tooth_idx").on(
			table.organizationId,
			table.patientId,
			table.toothNumberFdi,
		),
	}),
);

export const endoCanalMeasurements = pgTable(
	"endo_canal_measurements",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		sessionId: uuid("session_id")
			.notNull()
			.references(() => endoTreatmentSessions.id, { onDelete: "cascade" }),
		organizationId: uuid("organization_id")
			.notNull()
			.references(() => organizations.id, { onDelete: "cascade" }),
		canalName: varchar("canal_name", { length: 60 }).notNull(),
		coronalLandmarkRef: varchar("coronal_landmark_ref", {
			length: 100,
		}).notNull(),
		ealReadingApexDistanceMm: numeric("eal_reading_apex_distance_mm", {
			precision: 4,
			scale: 2,
		})
			.notNull()
			.default("0.00"),
		ealMachineBrand: varchar("eal_machine_brand", { length: 100 }),
		workingLengthMm: numeric("working_length_mm", {
			precision: 4,
			scale: 2,
		}).notNull(),
		referenceLengthMm: numeric("reference_length_mm", {
			precision: 4,
			scale: 2,
		}).notNull(),
		patencyConfirmed: boolean("patency_confirmed").notNull().default(true),
		patencyFileIsoSize: integer("patency_file_iso_size").notNull().default(10),
		masterApicalFileIso: integer("master_apical_file_iso").notNull(),
		masterApicalTaper: numeric("master_apical_taper", {
			precision: 4,
			scale: 2,
		}).notNull(),
		masterGuttaPerchaPointIso: integer("master_gp_point_iso"),
		schneiderAngleDeg: numeric("schneider_angle_deg", {
			precision: 5,
			scale: 2,
		})
			.notNull()
			.default("0.00"),
		schneiderRadiusMm: numeric("schneider_radius_mm", {
			precision: 5,
			scale: 2,
		})
			.notNull()
			.default("10.00"),
		isDoubleCurvatureS: boolean("is_double_curvature_s")
			.notNull()
			.default(false),
		canalStatus: varchar("canal_status", { length: 60 })
			.notNull()
			.default("obturated"),
		createdAt: timestamp("created_at", { withTimezone: true })
			.notNull()
	...
	(table) => ({
		sessionIdx: index("endo_canals_session_idx").on(table.sessionId),
		orgIdx: index("endo_canals_org_idx").on(table.organizationId),
	}),
);

export const nitiFileCatalogItems = pgTable(
	"niti_file_catalog_items",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
			.notNull()
		...
		isoTipSize: integer("iso_tip_size").notNull(),
		taper: numeric("taper", { precision: 4, scale: 2 }).notNull(),
		lengthMm: integer("length_mm").notNull().default(25),
		kinematics: varchar("kinematics", { length: 60 }).notNull(),
		recommendedRpm: integer("recommended_rpm").notNull().default(300),
		recommendedTorqueNcm: numeric("recommended_torque_ncm", {
			precision: 4,
			scale: 2,
		})
			.notNull()
			.default("2.50"),
		heatTreatmentAlloy: varchar("heat_treatment_alloy", { length: 80 })
			.notNull()
			.default("gold_wire"),
		fatigueCapSeconds: integer("fatigue_cap_seconds").notNull().default(600),
		maxCanalUses: integer("max_canal_uses").notNull().default(6),
	...
	(table) => ({
		orgIdx: index("niti_catalog_org_idx").on(table.organizationId),
	}),
);

export const nitiFileInstances = pgTable(
	"niti_file_instances",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
		...
		catalogItemId: uuid("catalog_item_id")
			.notNull()
			.references(() => nitiFileCatalogItems.id, { onDelete: "restrict" }),
		serialOrBarcode: varchar("serial_or_barcode", { length: 120 }).notNull(),
		lotNumber: varchar("lot_number", { length: 80 }),
		autoclaveCyclesCount: integer("autoclave_cycles_count").notNull().default(0),
		cumulativeUsageSeconds: integer("cumulative_usage_seconds")
			.notNull()
			.default(0),
		cumulativeCanalsCount: integer("cumulative_canals_count")
			.notNull()
			.default(0),
		accumulatedFatiguePhi: numeric("accumulated_fatigue_phi", {
			precision: 6,
			scale: 4,
		})
			.notNull()
	...
	(table) => ({
		orgIdx: index("niti_instances_org_idx").on(table.organizationId),
		barcodeIdx: uniqueIndex("niti_instances_barcode_idx").on(
			table.organizationId,
			table.serialOrBarcode,
		),
	}),
);

export const nitiFileUsageLogs = pgTable(
	"niti_file_usage_logs",
	{
		id: uuid("id").primaryKey().default(sql`uuidv7()`),
		organizationId: uuid("organization_id")
			.notNull()
			.references(() => organizations.id, { onDelete: "cascade" }),
		fileInstanceId: uuid("file_instance_id")
			.notNull()
			.references(() => nitiFileInstances.id, { onDelete: "cascade" }),
		sessionId: uuid("session_id")
			.notNull()
			.references(() => endoTreatmentSessions.id, { onDelete: "cascade" }),
		canalMeasurementId: uuid("canal_measurement_id").references(
			() => endoCanalMeasurements.id,
			{ onDelete: "set null" },
		),
		activeDurationSeconds: integer("active_duration_seconds").notNull(),
		measuredSchneiderAngleDeg: numeric("measured_schneider_angle_deg", {
			precision: 5,
			scale: 2,
		}).notNull(),
		measuredSchneiderRadiusMm: numeric("measured_schneider_radius_mm", {
			precision: 5,
			scale: 2,
		}).notNull(),
		peakTorqueNcm: numeric("peak_torque_ncm", { precision: 4, scale: 2 }),
		incrementalFatigueDeltaPhi: numeric("incremental_fatigue_delta_phi", {
			precision: 6,
			scale: 4,
		}).notNull(),
		resultingFatiguePhi: numeric("resulting_fatigue_phi", {
			precision: 6,
			scale: 4,
		}).notNull(),
		isMotorStallOccurred: boolean("is_motor_stall_occurred")
			.notNull()
			.default(false),
		isDefectsObserved: boolean("is_defects_observed").notNull().default(false),
	...
	(table) => ({
		instanceIdx: index("niti_usage_instance_idx").on(table.fileInstanceId),
		sessionIdx: index("niti_usage_session_idx").on(table.sessionId),
		orgIdx: index("niti_usage_org_idx").on(table.organizationId),
	}),
);
