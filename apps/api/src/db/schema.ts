...
export const appointmentChannelInheritances = pgTable("appointment_channel_inheritances", {
	...
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	patientName: text("patient_name").notNull(),
	chatChannelType: text("chat_channel_type").default("telegram").notNull(),
	inheritedNotificationChannel: text("inherited_notification_channel").default("telegram").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});
...
export const externalScheduleActionLogs = pgTable("external_schedule_action_logs", {
	id: uuid("id").primaryKey().defaultRandom(),
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	externalSourceName: text("external_source_name").notNull(),
...
export const patientDuplicateMergeQueues = pgTable("patient_duplicate_merge_queues", {
	...
	similarityScorePercent: integer("similarity_score_percent").default(95).notNull(),
...
// #4 — пациенты::хронологическая_история_коммуникаций
export const patientCommunicationTimelines = pgTable("patient_communication_timelines", {
	id: uuid("id").primaryKey().defaultRandom(),
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	patientName: text("patient_name").notNull(),
	eventType: text("event_type").default("call").notNull(),
	statusColor: text("status_color").default("green").notNull(),
	audioRecordingUrl: text("audio_recording_url"),
	comment: text("comment").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

// #8 — интеграции::егисз_гибкое_управление_бланками
export const egiszBlankPermissions = pgTable("egisz_blank_permissions", {
	id: uuid("id").primaryKey().defaultRandom(),
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	formCode: text("form_code").notNull(),
	fieldName: text("field_name").notNull(),
	isExportAllowed: boolean("is_export_allowed").default(true).notNull(),
	patientOptOutRespect: boolean("patient_opt_out_respect").default(true).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
});

// #10 — коммуникации::просмотр_истории_предыдущих_диалогов_в_чате
export const previousChatDialogHistories = pgTable("previous_chat_dialog_histories", {
	id: uuid("id").primaryKey().defaultRandom(),
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	dialogSessionId: text("dialog_session_id").notNull(),
	patientName: text("patient_name").notNull(),
...
// #12 — коммуникации::завершение_и_согласованная_обработка_чатов
export const collaborativeChatProcessingStates = pgTable("collaborative_chat_processing_states", {
	...
	hasAgentReplied: boolean("has_agent_replied").default(false).notNull(),
...
export const chatMessageDispatchStatuses = pgTable("chat_message_dispatch_statuses", {
	...
	canRetry: boolean("can_retry").default(false).notNull(),
	dispatchTimestamp: timestamp("dispatch_timestamp", { withTimezone: true }).notNull().defaultNow(),
});

// =====================================================
// WAVE 16 — COMPETITOR PARITY SCHEMA TABLES
// =====================================================
...
export const visitExaminationPhotoLinks = pgTable("visit_examination_photo_links", {
	...
	examinationFormId: text("examination_form_id").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

// #16 — пациенты::массовые_операции_с_изображениями
export const bulkImageOperationLogs = pgTable("bulk_image_operation_logs", {
	id: uuid("id").primaryKey().defaultRandom(),
	organizationId: uuid("organization_id").notNull().references(() => organizations.id),
	patientName: text("patient_name").notNull(),
	selectedImagesCount: integer("selected_images_count").default(1).notNull(),
	assignedToothNumber: integer("assigned_tooth_number"),
	operationType: text("operation_type").default("batch_link").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

// #20 — пациенты::причины_списания_в_архив_и_запрет_записи
export const patientArchiveReasonsAndBlacklists = pgTable("patient_archive_reasons_and_blacklists", {
	...
	isBookingBlocked: boolean("is_booking_blocked").default(true).notNull(),
	warningBadge: text("warning_badge").default("Черный список").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});

// #24 — коммуникации::речевая_аналитика_и_расшифровка_звонков_uis
export const uisCallSpeechTranscripts = pgTable("uis_call_speech_transcripts", {
	...
	keyTimestampsJson: text("key_timestamps_json").notNull(),
	sentimentScore: text("sentiment_score").default("positive").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});
...
export const familyRecommendationSources = pgTable("family_recommendation_sources", {
	...
	assignedMarketingSource: text("assigned_marketing_source").default("Рекомендация семьи").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
});
