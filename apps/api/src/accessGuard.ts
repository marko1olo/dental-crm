...
export function requireAuthTokenSecret(): string {
	...
	if (!secret)
		throw new Error(
			"AUTH_TOKEN_SECRET is required for authentication tokens in production.",
		);
	return secret;
}
...
function requestOrganizationHint(request: FastifyRequest): string | null {
	const headerHint =
		headerValue(request, "x-dente-organization-id") ??
...
async function resolveAdminSecretOrganizationId(
...
async function resolveDevelopmentDefaultOrganizationId(): Promise<
	string | null
> {
...
export async function resolveExplicitOrganizationId(
	request: FastifyRequest,
): Promise<string | null> {
	const organizationId = requestOrganizationHint(request);
	if (!organizationId) return null;
...
export async function resolveAuthenticatedOrganizationId(
	request: FastifyRequest,
): Promise<string | null> {
	const clinicHeader = request.headers["x-dente-clinic-token"];
	const clinicToken = Array.isArray(clinicHeader)
...
export async function requireResolvedOrganizationId(
	...
	reply.code(401).send({
		error: "AuthRequired",
		message:
			"Нужна действующая сессия клиники или сотрудника; при доступе по секрету администратора передайте x-dente-organization-id.",
		protectedArea,
	});
	return null;
}

export async function resolveStaffOrAdminOrganizationId(
	request: FastifyRequest,
): Promise<string | null> {
	const staffHeader = request.headers["x-dente-staff-token"];
	const staffToken = Array.isArray(staffHeader) ? staffHeader[0] : staffHeader;
...
export async function requireResolvedStaffOrAdminOrganizationId(
	...
	reply.code(403).send({
		...
		message:
			"Для изменения защищенных данных нужна действующая сессия сотрудника; при доступе по секрету администратора передайте x-dente-organization-id.",
		protectedArea,
	});
	return null;
}
