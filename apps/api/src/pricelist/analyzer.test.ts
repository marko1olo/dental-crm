...
test("safeParseJsonObject", async (t) => {
	...
	await t.test(
		"parses JSON object with text before and after (fallback logic)",
		() => {
			const textWithJson =
	...
	await t.test(
		"returns empty object if fallback regex finds no object match",
		() => {
	...
	await t.test("throws when fallback extracts invalid JSON", () => {
		const textWithInvalidJson =
			'Prefix text { "key": unquoted_value } Suffix text';
		assert.throws(() => safeParseJsonObject(textWithInvalidJson), SyntaxError);
	});
});
