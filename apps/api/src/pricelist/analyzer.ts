 ...
 * альтернативой.
 */
const amountPattern = "\\d{1,3}(?:[\\s.]\\d{3})+(?:[.,]\\d{1,2})?|\\d{3,7}(?:[.,]\\d{1,2})?";
...
function blankNotMoney(line: string): string {
  return notMoneyPatterns.reduce(
    (text, pattern) => text.replace(pattern, (fragment) => " ".repeat(fragment.length)),
    line
  );
}
...
function collectPriceCandidates(line: string): PriceCandidate[] {
  ...
  for (const match of scanText.matchAll(priceRegex)) {
     ...
     */
    if (groups.separator === "/" && groups.high && !hasCurrency && looksLikeYear(groups.high)) continue;
    const high = groups.high ? parseMoney(groups.high) : null;
    if (high === null) {
      /*
       * Верхняя граница не деньги — значит диапазона нет, и отрезок обрывается
       * на нижней границе вместе с её знаком рубля. «Седация 5000/120 мин»
       * теряет только «5000»: 120 не прошло нижнюю границу цены в 300 ₽, минуты
       * остаются в названии, а косая черта остаётся читаться как «за единицу».
       */
       ...
       *
       * Отрезок до `lowEnd` — ровно та часть, которую мы оставляем ценой (та же
       * граница используется ниже для `end`), поэтому вопрос «есть ли знак рубля»
       * задаётся именно ей.
       */
...
function extractPrice(line: string): { priceRub: number | null; priceMaxRub: number | null; pricedSpan: PriceSpan | null } {
   ...
   */
  const ambiguous =
    !explicit.length && new Set(pool.map((candidate) => `${candidate.priceRub}/${candidate.priceMaxRub ?? "-"}`)).size > 1;
 ...
 */
function stripPriceFromTitle(line: string, pricedSpan: PriceSpan | null): string {
  const withoutPrice = pricedSpan
    ? `${line.slice(0, pricedSpan.start)} ${line.slice(pricedSpan.end)}`
    : line;
