...
function collectPriceCandidates(line: string): PriceCandidate[] {
  ...
  for (const match of scanText.matchAll(priceRegex)) {
    ...
    if (high === null) {
       ...
       *
       * Признак берётся по СТРОКЕ, а не по кандидату, и цену это не портит: если
       * знак рубля подписывает ДРУГОЕ число, то у того числа стоит explicit, и
       * extractPrice выбирает из явных кандидатов, а не из года.
       */
      if (!hasExplicitPriceMark(line) && looksLikeEditionYear(line, lowGroupText)) continue;
      /*
