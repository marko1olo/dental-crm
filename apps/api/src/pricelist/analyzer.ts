...
function extractPrice(line: string): { priceRub: number | null; priceMaxRub: number | null } {
   ...
   *
   * РАЗДЕЛИТЕЛИ ДИАПАЗОНА (`-`, `/`, «до») ЗАВЯЗАНЫ НА stripPriceFromTitle И
   * ОБЯЗАНЫ СОВПАДАТЬ С НИМ СИМВОЛ В СИМВОЛ. Добавишь разделитель только здесь
   * — цена разберётся, но её текст останется в названии услуги оборванным
   * хвостом («Отбеливание 12000/»), которое врач увидит в каталоге, а пациент
   * в подписываемом документе. Добавишь только там — нижняя граница исчезнет
   * из прайса совсем: из названия вырезана, в цену не записана. Косая черта
   * стоила ровно этого: пара «12000/18000» читалась как одна цена 18 000 ₽ с
   * priceMaxRub: null, и минимальная цена клиники пропадала молча.
   */
  const priceRegex =
...
function stripPriceFromTitle(line: string): string {
  return normalizeText(
    line
       ...
       *
       * КОСАЯ ЧЕРТА добавлена к тире и «до» тем же правилом и по той же
       * причине: «Отбеливание 12000/18000 руб» оставляло название «Отбеливание
       * 12000/». Разделитель диапазона обязан совпадать со списком в priceRegex
       * ровно, символ в символ. Разойдись они — и название потеряет нижнюю
       * границу, которую разбор в цену не записал: 12 000 ₽ исчезнет из прайса
       * целиком, а не просто останется висеть в названии. Внутри литерала
       * регулярки косая черта экранируется (`\/`).
       */
      .replace(
