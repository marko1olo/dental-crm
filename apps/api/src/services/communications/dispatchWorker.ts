...
} from "./appointmentReminders.js";
import {
	completeFinishedCampaigns,
	launchScheduledCampaigns,
} from "./campaigns.js";
import { type DispatchReport, dispatchDueMessages } from "./dispatcher.js";
 ...
 */
export function describeAutomaticSending(
	env: NodeJS.ProcessEnv = process.env,
): AutomaticSendingState {
	...
	const intervalMs = parseInteger(
		env.DENTE_COMMUNICATION_WORKER_INTERVAL_MS,
		30_000,
		5_000,
		15 * 60_000,
	);
	return {
		...
		batchSize: parseInteger(
			env.DENTE_COMMUNICATION_WORKER_BATCH_SIZE,
			25,
			1,
			200,
		),
...
export function startCommunicationDispatchWorker(
	...
	if (!parseBoolean(env.DENTE_COMMUNICATION_WORKER_ENABLED)) {
		return {
			...
			stop: () => undefined,
			runOnce: async () => null,
			scheduleRemindersOnce: async () => null,
		};
	}
	...
	const stuckLockMinutes = parseInteger(
		env.DENTE_COMMUNICATION_WORKER_STUCK_LOCK_MINUTES,
		10,
		1,
		240,
	);
	...
	const reminderEveryTicks = parseInteger(
		env.DENTE_COMMUNICATION_REMINDER_EVERY_TICKS,
		10,
		1,
		200,
	);
	...
	const runOnce = async (): Promise<DispatchReport | null> => {
		...
		inFlight = true;
		try {
			return await dispatchDueMessages({
				batchSize,
				workerId,
				stuckLockMinutes,
			});
	...
	const scheduleRemindersOnce =
		async (): Promise<ReminderScheduleReport | null> => {
	...
	const tick = async () => {
		...
		if (inFlight) {
			logger?.warn(
				{},
				"Разбор очереди сообщений: предыдущий тик ещё идёт, пропуск",
			);
			schedule(intervalMs);
			return;
		}
		try {
			...
			for (const problem of inbound.problems)
				logger?.warn({ problem }, "Входящее сообщение не разобрано");
		} catch (error) {
		...
		try {
			...
			if (ticksSinceReminders >= reminderEveryTicks) {
				...
				if (campaigns.launched > 0) {
					logger?.info(
						{ launched: campaigns.launched },
						"Отложенные рассылки запущены",
					);
				}
				for (const problem of campaigns.problems)
					logger?.warn({ problem }, "Отложенная рассылка не запущена");
				...
				const purged = await purgeExpiredActionCodes(
					new Date(Date.now() - 7 * 24 * 60 * 60 * 1000),
				);
				if (purged > 0)
					logger?.info({ purged }, "Просроченные ссылки подтверждения убраны");
					...
					logger?.info(
						{
							...
							examined: reminders.examined,
							problems: reminders.problems.length,
						},
						"Напоминания о приёме поставлены в очередь",
					);
				}
				for (const problem of reminders.problems)
					logger?.warn({ problem }, "Напоминания о приёме: помеха");
			} else {
		...
		try {
			...
			if (report && report.releasedStuck > 0) {
				logger?.warn(
					{ releasedStuck: report.releasedStuck },
					"Возвращены зависшие отправки",
				);
			}
		} catch (error) {
