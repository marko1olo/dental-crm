...
import { communicationOutbox, communicationTemplates } from "../../db/schema.js";
import {
	describeCriteria,
...
export function parseAudienceCriteria(raw: string): AudienceCriteria {
	try {
		...
		return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? (parsed as AudienceCriteria) : {};
	} catch {
		return {};
	}
}
 ...
 */
const CAMPAIGN_SAFE_VARIABLES = new Set(["patient", "patientFullName", "clinic", "clinicPhone", "clinicAddress", "link", "reviewLink"]);

export function campaignTemplateProblems(body: string, variables: string[]): string[] {
	const unsupported = variables.filter((key) => !CAMPAIGN_SAFE_VARIABLES.has(key));
...
export type CreateCampaignResult = { readonly ok: true; readonly campaign: CampaignRow } | { readonly ok: false; readonly reason: string };
...
export async function createCampaign(input: CreateCampaignInput): Promise<CreateCampaignResult> {
	const [template] = await db
		...
		.where(
			and(eq(communicationTemplates.id, input.templateId), eq(communicationTemplates.organizationId, input.organizationId))
		)
		.limit(1);
	if (!template) return { ok: false, reason: "Шаблон не найден в этой клинике." };
	if (!template.isActive) return { ok: false, reason: "Шаблон отключён и не может использоваться для рассылки." };
...
export async function previewCampaign(
	...
	const [campaign] = await db
		...
		.where(and(eq(communicationCampaigns.id, campaignId), eq(communicationCampaigns.organizationId, organizationId)))
		.limit(1);
	...
	if (campaign.templateId) {
		...
		if (!template) {
			...
			problems.push(...campaignTemplateProblems(template.body, parseVariables(template.variablesJson)));
			const rendered = renderTemplate(template.body, {}, { allowPhi: true, allowEmptyValues: true });
	...
	if (audience.deliverable === 0) {
		problems.push("Получателей нет: проверьте условия отбора, контакты пациентов и согласия.");
	}
	...
	return {
		...
		audience: {
			...
			candidates: audience.candidates.slice(0, Math.max(1, Math.min(50, options.sampleLimit ?? 10)))
		},
...
export async function launchCampaign(input: {
	...
	const [campaign] = await db
		...
		.from(communicationCampaigns)
		.where(
			and(eq(communicationCampaigns.id, input.campaignId), eq(communicationCampaigns.organizationId, input.organizationId))
		)
		.limit(1);
	if (!campaign) return { ok: false, reason: "Рассылка не найдена в этой клинике." };
	if (campaign.status === "cancelled") return { ok: false, reason: "Отменённую рассылку запустить нельзя." };
	if (campaign.status === "completed") return { ok: false, reason: "Рассылка уже завершена." };
	/*
	...
	if (campaign.status === "running") {
		return { ok: false, reason: "Рассылка уже выполняется. Дождитесь окончания или отмените её." };
	}
	if (!campaign.templateId) return { ok: false, reason: "У рассылки не выбран шаблон." };
	...
	if (!template.isActive) return { ok: false, reason: "Шаблон рассылки отключён." };

	const channel = campaign.channel as CommunicationChannelCode;
	const scope = campaign.scope as CommunicationConsentScope;
	const criteria = parseAudienceCriteria(campaign.audienceJson);
	const audience = await resolveAudience({ organizationId: input.organizationId, channel, scope, criteria, now });

	if (audience.deliverable === 0) {
		return { ok: false, reason: "Получателей нет: проверьте условия отбора, контакты пациентов и согласия." };
	}
	...
	return { ok: true, queued, alreadyQueued, skipped, matched: audience.matched };
}
...
export async function campaignProgress(organizationId: string, campaignId: string): Promise<CampaignProgress | null> {
	const [campaign] = await db
		.select()
		.from(communicationCampaigns)
		.where(and(eq(communicationCampaigns.id, campaignId), eq(communicationCampaigns.organizationId, organizationId)))
	...
	const rows = await db
		...
		.where(
			and(eq(communicationOutbox.organizationId, organizationId), eq(communicationOutbox.campaignId, campaignId))
		)
 ...
 */
export async function completeFinishedCampaigns(organizationId?: string | null, now = new Date()): Promise<number> {
	const scope = [eq(communicationCampaigns.status, "running" as const)];
	if (organizationId) scope.push(eq(communicationCampaigns.organizationId, organizationId));
	...
	for (const campaign of running) {
		const [pending] = await db
			...
			.where(
				and(
					eq(communicationOutbox.campaignId, campaign.id),
					inArray(communicationOutbox.status, ["queued", "sending"])
				)
			);
 ...
 */
export async function launchScheduledCampaigns(now = new Date()): Promise<{ launched: number; problems: string[] }> {
	const due = await db
		.select({ id: communicationCampaigns.id, organizationId: communicationCampaigns.organizationId })
		.from(communicationCampaigns)
		.where(and(eq(communicationCampaigns.status, "scheduled"), sql`${communicationCampaigns.scheduledAt} <= ${now}`));
