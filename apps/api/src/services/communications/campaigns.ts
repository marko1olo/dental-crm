...
export async function launchScheduledCampaigns(
  ...
  for (const [index, result] of results.entries()) {
    const campaign = due[index]!;
    if (result.status === "fulfilled") {
      if (result.value.ok) launched += 1;
      else problems.push(`Рассылка ${campaign.id}: ${result.value.reason}`);
    } else {
      problems.push(
        `Рассылка ${campaign.id}: Внутренняя ошибка (${String(result.reason)})`,
      );
      console.error(`Ошибка запуска рассылки ${campaign.id}:`, result.reason);
    }
  }
