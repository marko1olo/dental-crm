...
export const communicationTemplateVariables: readonly TemplateVariableDefinition[] = [
	{ key: "patient", label: "Имя пациента", example: "Марина Петровна", phi: false },
	{ key: "patientFullName", label: "Пациент полностью", example: "Орлова Марина Петровна", phi: false },
	{ key: "clinic", label: "Название клиники", example: "Клиника на Ленина", phi: false },
	{ key: "clinicPhone", label: "Телефон клиники", example: "+7 495 000-00-00", phi: false },
	{ key: "clinicAddress", label: "Адрес клиники", example: "Москва, ул. Ленина, 1", phi: false },
	...
	{ key: "planTitle", label: "План лечения", example: "Терапия верхней челюсти", phi: true },
	{ key: "documentTitle", label: "Документ", example: "Акт выполненных работ", phi: true }
];

const variableByKey = new Map(communicationTemplateVariables.map((variable) => [variable.key, variable]));
...
export function validateTemplateBody(body: string, options: ValidateTemplateBodyOptions = {}): TemplateValidationResult {
	const variables = extractTemplateVariables(body);
	const unknownVariables = variables.filter((key) => !variableByKey.has(key));
	const phiVariables = variables.filter((key) => variableByKey.get(key)?.phi === true);
...
export function describeSmsPayload(text: string): SmsPayloadDescription {
	...
	if (requiresUcs2) {
		...
		const units = [...text].reduce((total, character) => total + ((character.codePointAt(0) ?? 0) > 0xffff ? 2 : 1), 0);
		const single = 70;
		const multipart = 67;
		const segments = units <= single ? Math.max(1, Math.ceil(units / single)) : Math.ceil(units / multipart);
	...
	const single = 160;
	const multipart = 153;
	const segments = characters <= single ? Math.max(1, Math.ceil(characters / single)) : Math.ceil(characters / multipart);
	const capacity = segments <= 1 ? single : multipart * segments;
	return {
		encoding: "gsm7",
		characters,
...
export function checkChannelFit(channel: string, text: string, options: CheckChannelFitOptions = {}): ChannelFitResult {
	const limit = channelBodyLimits[channel] ?? 4096;
	const problems: string[] = [];
	const length = [...text].length;

	if (length > limit) {
		problems.push(`Текст длиннее предела канала: ${length} из ${limit} символов.`);
	}
