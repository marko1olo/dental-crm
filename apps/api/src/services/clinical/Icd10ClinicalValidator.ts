 ...
 * 1. Проверка принадлежности кода МКБ-10 стоматологическому разделу (K00–K14, Z01.2, Z00.0, Z13.84, Z46.3/4, Z96.5).
...
export const DENTAL_ICD10_RUBRICS: Readonly<Record<string, DentalIcd10Category>> = {
	...
	// Z00–Z96: Профилактические осмотры, диспансеризация, скрининг, примерка устройств и имплантаты
	Z00: {
		code: "Z00",
		titleRu: "Общий осмотр и обследование лиц, не имеющих жалоб или установленного диагноза (профилактический осмотр)",
		requiresTooth: false,
	},
	Z01: {
		code: "Z01",
		titleRu: "Другие специальные осмотры и обследования (стоматологическое обследование, санация, гигиена полости рта)",
		requiresTooth: false,
	},
	Z13: {
		code: "Z13",
		titleRu: "Специальное скрининговое обследование для выявления других болезней и нарушений (стоматологический скрининг)",
		requiresTooth: false,
	},
	Z46: {
		code: "Z46",
		titleRu: "Примерка и подгонка других устройств (зубных протезных и ортодонтических аппаратов)",
		requiresTooth: false,
	},
	Z96: {
		code: "Z96",
		titleRu: "Наличие других функциональных имплантатов (зубных и нижнечелюстных имплантатов)",
		requiresTooth: false,
	},
};
...
export const DETAILED_DENTAL_ICD10_TITLES: Readonly<Record<string, string>> = {
	...
	"K14.6": "Глоссодиния",
	// Z00–Z96: Профилактические осмотры, скрининг, протезирование и имплантаты
	"Z00.0": "Общий медицинский осмотр (профилактический)",
	"Z01.2": "Стоматологическое обследование (профилактический осмотр, гигиена полости рта, санация)",
	"Z13.84": "Специальное скрининговое обследование для выявления стоматологических нарушений",
	"Z46.3": "Примерка и подгонка зубного протезного устройства",
	"Z46.4": "Примерка и подгонка ортодонтического устройства",
	"Z96.5": "Наличие зубных и нижнечелюстных имплантатов",
};
...
export class Icd10ClinicalValidator {
	...
	public static normalizeCode(raw: unknown): string {
		...
		// Если код вида K021, Z012 или K0402 -> вставляем точку после первых 3 символов
		if (/^[KZ]\d{3,4}$/i.test(str)) {
	...
	/**
	 * Проверяет, принадлежит ли код стоматологическому разделу МКБ-10 (K00–K14, Z01.2, Z00.0, Z13.84, Z46.3/4, Z96.5).
	 */
	public static isDentalIcd10(code: unknown): boolean {
		const normalized = Icd10ClinicalValidator.normalizeCode(code);
		if (!normalized) return false;
		return /^(K(0[0-9]|1[0-4])|Z01\.2|Z00\.0|Z13\.84|Z46\.[34]|Z96\.5)(\.\d{1,4})?$/i.test(normalized);
	}
	...
	public static validate(
		...
		// 2. Нормализация и проверка стоматологического раздела K00-K14 и Z-кодов
		...
		if (!Icd10ClinicalValidator.isDentalIcd10(normalizedCode)) {
			return {
				...
				errorMessage: `Код диагноза «${rawCodeStr}» не входит в стоматологический раздел МКБ-10 (K00–K14, Z01.2, Z00.0, Z13.84, Z46.3/4, Z96.5) или имеет неверный формат. Укажите корректный код (например, Z01.2 для стоматологического осмотра, K02.1 для кариеса дентина, K04.0 для пульпита, K05.1 для гингивита, K08.1 для адентии).`,
				normalizedCode,
				rawCode: rawCodeStr,
			};
		}
