...
export const CLINICAL_PHASE_CODES = ["PHASE_1_THERAPY", "PHASE_2_SURGERY"] as const;
export type ClinicalPhaseCode = (typeof CLINICAL_PHASE_CODES)[number];

export function isClinicalPhaseCode(value: unknown): value is ClinicalPhaseCode {
  return typeof value === "string" && (CLINICAL_PHASE_CODES as readonly string[]).includes(value);
}
 ...
 */
function buildHandoffDescription(spec: PhaseHandoffSpec, toothCodes: readonly string[], notes: string): string {
...
export class ClinicalRouter {
  ...
  public async listTasks(organizationId: string, patientId?: string): Promise<ClinicalTaskRecord[]> {
