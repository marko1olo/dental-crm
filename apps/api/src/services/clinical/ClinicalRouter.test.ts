...
async function findFixture(): Promise<TestFixture | null> {
  ...
  const ownRow = (own.rows ?? [])[0] as { id: string; organization_id: string } | undefined;
...
describe("ClinicalRouter — передача между клиническими этапами", () => {
  test("задача передачи записывается в clinical_tasks и читается обратно", async (t) => {
    if (!fixture) {
      t.skip("В базе нет ни одного пациента — фикстуру для записи задачи взять неоткуда.");
      return;
    }
    ...
    const raw = await db.execute(sql`SELECT * FROM clinical_tasks WHERE id = ${task.id}::uuid`);
    const rawRow = (raw.rows ?? [])[0] as Record<string, unknown> | undefined;
    assert.ok(rawRow, "строка обязана существовать в clinical_tasks после вызова");
    ...
    const seenByNextDoctor = await new ClinicalRouter().listTasks(fixture.organizationId, fixture.patientId);
  ...
  test("повторная отправка того же завершения не плодит вторую открытую задачу", async (t) => {
    ...
    const first = await router.handlePhaseCompletion(fixture.organizationId, input);
    assert.ok(first);
    createdTaskIds.push(first.id);

    const second = await router.handlePhaseCompletion(fixture.organizationId, input);
    assert.ok(second);
    assert.strictEqual(second.id, first.id, "второе нажатие обязано вернуть ту же задачу, а не создать новую");
  ...
  test("описание не содержит пустых разделов, когда зубы и комментарий не указаны", async (t) => {
    ...
    const task = await new ClinicalRouter().handlePhaseCompletion(fixture.organizationId, {
      patientId: fixture.patientId,
      completedPhaseCode: "PHASE_1_THERAPY",
      notes: "   ",
      toothCodes: [" ", ""],
  ...
  test("неизвестный код этапа возвращает null и ничего не пишет", async (t) => {
    ...
    const countBefore = await db.execute(sql`SELECT count(*)::int AS n FROM clinical_tasks`);
    const result = await new ClinicalRouter().handlePhaseCompletion(fixture.organizationId, {
      patientId: fixture.patientId,
    ...
    const countAfter = await db.execute(sql`SELECT count(*)::int AS n FROM clinical_tasks`);
    assert.strictEqual(Number((countAfter.rows ?? [])[0]?.n), Number((countBefore.rows ?? [])[0]?.n));
  });
  ...
  test("пациент чужой клиники отклоняется, а не записывается под своей организацией", async (t) => {
    ...
    if (!fixture.foreignPatientId) {
      t.skip("В базе одна организация — межарендную проверку подтвердить нечем.");
      return;
    }
    await assert.rejects(
      ...
      (error: unknown) => error instanceof ClinicalTaskOwnershipError && error.field === "patientId",
    );
