...
const ENCRYPTION_KEY = process.env.CLINIC_ENCRYPTION_KEY || "0123456789abcdef0123456789abcdef"; // 32 bytes
const IV_LENGTH = 16;
 ...
 */
export async function createEncryptedBackup(): Promise<{ success: boolean; filePath?: string; error?: string }> {
  return new Promise((resolve) => {
    try {
      ...
      const portablePgDumpPath = path.resolve(process.cwd(), "../../.postgres/bin/pg_dump.exe");
      const pgDumpCommand = fs.existsSync(portablePgDumpPath) ? portablePgDumpPath : "pg_dump";
      
      const pgDump = spawn(pgDumpCommand, ["-U", "dental", "-d", "dental_crm", "--clean", "--if-exists"]);
      ...
      pgDump.on("close", (code) => {
        if (code === 0) {
          console.log(`[BackupWorker] Successfully created encrypted backup: ${filePath}`);
          resolve({ success: true, filePath });
        } else {
          console.error(`[BackupWorker] pg_dump exited with code ${code}`);
          resolve({ success: false, error: `pg_dump exited with code ${code}` });
        }
      });
    } catch (err: any) {
