...
describe("IdentityResolutionEngine - levenshteinDistance", () => {
	...
	it("should calculate correct distance for substitutions", () => {
		...
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("sitten", "sittin"),
			1,
		); // 1 substitution
		assert.equal(
	...
	it("should calculate correct distance for additions/insertions", () => {
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("hello", "helloo"),
			1,
		);
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("sittin", "sitting"),
			1,
		);
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("kitten", "sitting"),
			3,
		); // 2 subs, 1 addition
	});
	...
	it("should calculate correct distance for deletions", () => {
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("hello", "hell"),
			1,
		);
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("Иванов", "Ивано"),
			1,
		);
	});
	...
	it("should be case sensitive", () => {
		...
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("Иванов", "иванов"),
			1,
		);
	});
	...
	it("should calculate correct distance for completely different strings", () => {
		...
		assert.equal(
			IdentityResolutionEngine.levenshteinDistance("flitten", "dog"),
			7,
		);
	});
});
