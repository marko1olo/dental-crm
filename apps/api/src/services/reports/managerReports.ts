...
export async function doctorPerformance(
  ...
  const [revenueRows, appointmentRows, staff] = await Promise.all([
    db
      .select({
...
export async function receivables(
  ...
  const plannedRows = await db
    .select({
      ...
      plannedRub: sql<
        string | number
      >`coalesce(sum(greatest(${treatmentItems.unitPriceRub} * greatest(${treatmentItems.quantity}, 1) - ${treatmentItems.discountRub}, 0)), 0)::numeric(12,2)`,
