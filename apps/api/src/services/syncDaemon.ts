...
async function mockCloudVaultExchange(localPayload: any) {
  ...
  try {
    ...
    if (unpaid.length > 0) {
      ...
      if (targetInvoice) {
        console.log(`[SyncDaemon Mock] Simulating online payment for invoice ${targetInvoice.id} in Patient Portal`);
    ...
    console.warn("[SyncDaemon Mock] Failed to generate mock cloud changes:", err.message);
  }
...
export async function runSyncCycle(): Promise<SyncReport> {
  ...
  try {
    ...
    const unsyncedDiaries = await db.select().from(visitDiaries).where(eq(visitDiaries.isSynced, false));
    const unsyncedToothStates = await db.select().from(toothStates).where(eq(toothStates.isSynced, false));
    const unsyncedPlans = await db.select().from(treatmentPlans).where(eq(treatmentPlans.isSynced, false));
    const unsyncedInvoices = await db.select().from(patientInvoices).where(eq(patientInvoices.isSynced, false));
    ...
    if (totalUnsynced > 0) {
      console.log(`[SyncDaemon] Outbound sync: pushing ${totalUnsynced} unsynced records to Cloud Vault`);
    }
    ...
    if (response.success) {
      ...
      if (unsyncedPatients.length > 0) {
        await db.update(patients).set({ isSynced: true }).where(or(...unsyncedPatients.map(p => eq(patients.id, p.id))));
      }
      if (unsyncedDiaries.length > 0) {
        await db.update(visitDiaries).set({ isSynced: true }).where(or(...unsyncedDiaries.map(d => eq(visitDiaries.id, d.id))));
      }
      if (unsyncedToothStates.length > 0) {
        await db.update(toothStates).set({ isSynced: true }).where(or(...unsyncedToothStates.map(s => eq(toothStates.id, s.id))));
      }
      if (unsyncedPlans.length > 0) {
        await db.update(treatmentPlans).set({ isSynced: true }).where(or(...unsyncedPlans.map(p => eq(treatmentPlans.id, p.id))));
      }
      if (unsyncedInvoices.length > 0) {
        await db.update(patientInvoices).set({ isSynced: true }).where(or(...unsyncedInvoices.map(i => eq(patientInvoices.id, i.id))));
      }
      ...
      const mergeTable = async (
        ...
        for (const record of cloudRecords) {
          const localMatch = await db.select().from(table).where(eq(table.id, record.id)).limit(1);
          ...
          if (!local) {
            ...
            const cloudDate = record.updatedAt ? new Date(record.updatedAt).getTime() : 0;

            if (cloudDate > localDate) {
              // Cloud is newer -> Update locally
              await db.update(table).set({ ...record, isSynced: true }).where(eq(table.id, record.id));
              report.downloadedCount++;
              report.details[detailsKey].downloaded++;

              // Side-effects: if patient paid invoice in cloud, insert payment in local ledger
              if (table === patientInvoices && record.status === "paid" && local.status !== "paid") {
                console.log(`[SyncDaemon] Invoice ${record.id} was paid online. Logging transaction to cash_ledger`);
              ...
              await db.update(table).set({ isSynced: false }).where(eq(table.id, record.id));
            }
          }
        }
      };
      ...
      await mergeTable(patientInvoices, cloud.patientInvoices, "patientInvoices");
    }
  } catch (err: any) {
