...
function buildPatientInsights(): PatientInsight[] {
  ...
  for (const document of documents) {
    let list = documentsByPatient.get(document.patientId);
    if (!list) {
      list = [];
      documentsByPatient.set(document.patientId, list);
    }
    list.push(document);
  }
  ...
  for (const task of communicationTasks) {
    if (isOpenCommunicationTask(task)) {
      let list = tasksByPatient.get(task.patientId);
      if (!list) {
        list = [];
        tasksByPatient.set(task.patientId, list);
      }
      list.push(task);
    }
  }
  ...
  for (const study of imagingStudies) {
    let list = imagesByPatient.get(study.patientId);
    if (!list) {
      list = [];
      imagesByPatient.set(study.patientId, list);
    }
    list.push(study);
  }
  ...
  for (const payment of payments) {
    if (payment.status === "paid") {
      let list = paymentsByPatient.get(payment.patientId);
      if (!list) {
        list = [];
  ...
  for (const item of treatmentPlanItems) {
    let list = planItemsByPatient.get(item.patientId);
    if (!list) {
      list = [];
      planItemsByPatient.set(item.patientId, list);
    }
    list.push(item);
  }
  ...
  for (const appointment of appointments) {
    let list = appointmentsByPatient.get(appointment.patientId as string);
    if (!list) {
      list = [];
      appointmentsByPatient.set(appointment.patientId as string, list);
    }
...
function buildAppointmentReadiness(
  ...
  return appointments.map((appointment) => {
    ...
    const doctor = staffMembers.find(
      (member) => member.id === appointment.doctorUserId && member.active,
    );
    const assistant = appointment.assistantUserId
      ? staffMembers.find(
          (member) =>
            member.id === appointment.assistantUserId &&
            member.role === "assistant" &&
            member.active,
        )
      : null;
    const chair = chairs.find(
      (item) => item.id === appointment.chairId && item.active,
    );
    const patientDocuments = documents.filter(
      (document) =>
        document.patientId === appointment.patientId &&
        document.status !== "voided",
    );
    const patientImages = imagingStudies.filter(
      (study) => study.patientId === appointment.patientId,
    );
    const insight = patientInsights.find(
      (item) => item.patientId === appointment.patientId,
    );
    const appointmentTasks = communicationTasks.filter(
      (task) =>
        task.appointmentId === appointment.id && isOpenCommunicationTask(task),
    );
