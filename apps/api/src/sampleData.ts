...
export function buildPatientInsights(): PatientInsight[] {
...
export function buildAppointmentReadiness(
  ...
  const staffMembersById = new Map<string, StaffMember>();
  for (const staff of staffMembers) {
    if (staff.active) {
      staffMembersById.set(staff.id, staff);
    }
  }

  const chairsById = new Map<string, Chair>();
  ...
  const patientDocumentsByPatientId = new Map<string, typeof documents>();
  for (const doc of documents) {
    if (doc.status !== "voided") {
      const arr = patientDocumentsByPatientId.get(doc.patientId) || [];
      arr.push(doc);
      patientDocumentsByPatientId.set(doc.patientId, arr);
    }
  }

  const patientImagesByPatientId = new Map<string, typeof imagingStudies>();
  for (const img of imagingStudies) {
    const arr = patientImagesByPatientId.get(img.patientId) || [];
    arr.push(img);
    patientImagesByPatientId.set(img.patientId, arr);
  }

  const insightsByPatientId = new Map<string, (typeof patientInsights)[0]>();
  for (const insight of patientInsights) {
    insightsByPatientId.set(insight.patientId, insight);
  }

  const communicationTasksByAppointmentId = new Map<
    string,
    typeof communicationTasks
  >();
  for (const task of communicationTasks) {
    if (isOpenCommunicationTask(task) && task.appointmentId) {
      const arr =
        communicationTasksByAppointmentId.get(task.appointmentId) || [];
      arr.push(task);
      communicationTasksByAppointmentId.set(task.appointmentId, arr);
    }
  }
  ...
  return appointments.map((appointment) => {
    ...
    const doctor = appointment.doctorUserId
      ? staffMembersById.get(appointment.doctorUserId)
      : undefined;
    const possibleAssistant = appointment.assistantUserId
      ? staffMembersById.get(appointment.assistantUserId)
      : undefined;
    const assistant =
      possibleAssistant && possibleAssistant.role === "assistant"
        ? possibleAssistant
        : null;
    const chair = appointment.chairId
      ? chairsById.get(appointment.chairId)
      : undefined;
    const patientDocuments = appointment.patientId
      ? patientDocumentsByPatientId.get(appointment.patientId) || []
      : [];
    const patientImages = appointment.patientId
      ? patientImagesByPatientId.get(appointment.patientId) || []
      : [];
    const insight = appointment.patientId
      ? insightsByPatientId.get(appointment.patientId)
      : undefined;
    const appointmentTasks =
      communicationTasksByAppointmentId.get(appointment.id) || [];
...
function normalizeExistingDenteTelegramVisualCardUrls(
  input: unknown,
): DenteTelegramVisualCardUrls {
  try {
  ...
  const staff = input.staffId
    ? (staffMembers.find(
        (member) =>
          ...
          member.organizationId === organizationScope &&
          member.active,
      ) ?? null)
    : null;
...
export function renderDenteTelegramMessagePreview(
  ...
  const previews: Record<
    DenteTelegramMessagePreviewRequest["templateKind"],
    Omit<DenteTelegramMessagePreview, "replyMarkup" | "photoUrl">
  > = {
    appointment_reminder: {
      templateKind: "appointment_reminder",
    ...
    staff_daily_digest: buildStaffDailyDigestTelegramPreview(
      input,
      baseWarning,
...
function buildDenteTelegramRecallItems(
  ...
  return treatmentPlanItems.flatMap((item) => {
    ...
    const patient = patients.find(
      (candidate) =>
        candidate.id === item.patientId && candidate.status === "active",
    );
    if (!patient) return [];
...
export function findDenteTelegramOutboxDeliveryReceipt(
  ...
  return (
    denteTelegramOutboxDeliveryReceipts.find(
      (receipt) =>
        receipt.outboxItemId === outboxItemId &&
...
function buildDenteTelegramReviewRequestItems(
  ...
  for (const payment of paidMilestones) {
    const patient =
      patients.find(
        (candidate) =>
          candidate.id === payment.patientId && candidate.status === "active",
      ) ?? null;
...
export function recordDenteTelegramOutboxDelivery(input: {
  ...
  if (clientMutationId) {
    ...
    const existingIndex = denteTelegramOutboxDeliveryReceipts.findIndex(
      (candidate) =>
        candidate.outboxItemId === receipt.outboxItemId &&
        candidate.clientMutationId === receipt.clientMutationId,
    );
    if (existingIndex >= 0) {
...
export function getOrCreateImagingViewerSession(
  ...
  const existing = imagingViewerSessions.find(
    (session) => session.studyId === studyId,
  );
  if (existing) return existing;
