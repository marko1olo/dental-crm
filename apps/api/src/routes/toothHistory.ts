...
import { patients, visitDiaries, treatmentPlans, treatmentPlanItemsNew, toothStates } from "../db/schema.js";
...
export default async function registerToothHistoryRoutes(app: FastifyInstance) {
  app.get("/api/odontogram/tooth-history/:patientId/:toothId", async (req, reply) => {
    const orgId = await requireResolvedOrganizationId(req, reply, "read tooth history");
    if (!orgId) return;
    ...
    const diaries = await db.select().from(visitDiaries).where(and(eq(visitDiaries.patientId, patientId), eq(visitDiaries.diagnosisTooth, toothId), eq(visitDiaries.organizationId, orgId)));
    diaries.forEach(d => {
      events.push({
        type: "diary",
        date: d.createdAt,
    ...
    const states = await db.select().from(toothStates).where(and(eq(toothStates.patientId, patientId), eq(toothStates.toothNumber, toothNum))).orderBy(desc(toothStates.updatedAt));
    states.forEach(s => {
      events.push({
        type: "state_change",
        date: s.updatedAt,
