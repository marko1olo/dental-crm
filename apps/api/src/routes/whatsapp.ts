            ...
            await withTenantCtx(inboundOrganizationId, async (tx) => {
              ...
              if (msgIds.length > 0) {
                const existing = await tx
                  .select({ externalId: messengerInboundEvents.externalId })
              ...
              for (const p of parsedMessages) {
                if (p.msgId && existingIds.has(p.msgId)) {
                  request.log.info(
                    { msgId: p.msgId, inboundOrganizationId },
                ...
                valuesToInsert.push({
                  ...
                  channel: "whatsapp",
                  externalId: p.msgId,
                  externalChatId: p.fromId,
                  messageText: p.textBody,
                  eventKind: "message",
                  rawPayload: p.m as Record<string, unknown>,
                });
              }

              if (valuesToInsert.length > 0) {
                await tx.insert(messengerInboundEvents).values(valuesToInsert);
              }
            });
          }
        }
