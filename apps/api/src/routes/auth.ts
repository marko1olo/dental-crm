...
export async function requireClinicToken(
...
export async function registerAuthRoutes(app: FastifyInstance) {
	...
	app.post(
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			const { token, fullName, password, pinCode } =
				(request.body as any) ?? {};
		...
		async (request: FastifyRequest, reply: FastifyReply) => {
			...
			if (!user) {
				return reply.code(500).send({
					error: "NoUsers",
					message:
						"No seed users found in the database. Please run seed script first.",
				});
			}
