...
import { dismissDuplicatePair, mergePatients } from "../services/patients/patientMerge.js";
...
function validationError(reply: FastifyReply, message: string) {
	return reply.code(400).send({ error: "PatientDuplicateValidationError", message });
}
...
export async function registerPatientDuplicateRoutes(app: FastifyInstance) {
	...
	app.get("/api/patients/duplicates", async (request, reply) => {
		const context = await requireClinicalReadContext(request, reply, "patient duplicates");
		if (!context) return;
		if (!enforcePermissionWhenStaffKnown(request, reply, "patients.read")) return;

		const parsed = listQuerySchema.safeParse(request.query);
		if (!parsed.success) return validationError(reply, "Проверьте параметры: порог уверенности и предел списка.");

		const options: { minConfidence?: number; limit?: number } = {};
		if (parsed.data.minConfidence !== undefined) options.minConfidence = parsed.data.minConfidence;
	...
	app.get("/api/patients/:patientId/duplicates", async (request, reply) => {
		const context = await requireClinicalReadContext(request, reply, "patient duplicates by patient");
		if (!context) return;
		if (!enforcePermissionWhenStaffKnown(request, reply, "patients.read")) return;

		const patientId = (request.params as { patientId?: string }).patientId;
		if (!patientId) return validationError(reply, "Не указан пациент.");
		if (!(await patientBelongsToOrganization(context.organizationId, patientId))) {
			return reply.code(404).send({ error: "PatientNotFound", message: "Пациент не найден в этой клинике." });
		}

		const candidates = await duplicatesForPatient(context.organizationId, patientId);
	...
	app.post("/api/patients/duplicates/merge", async (request, reply) => {
		const context = await requireClinicalMutationContext(request, reply, "patient duplicate merge");
		if (!context) return;
		if (!enforcePermissionWhenStaffKnown(request, reply, "patients.write")) return;

		const parsed = mergeSchema.safeParse(request.body);
		if (!parsed.success) {
			return validationError(reply, "Укажите две разные карточки: какая останется и какая объединяется в неё.");
		}
		...
		if (!result.ok) return reply.code(409).send({ error: "PatientMergeRejected", message: result.reason });

		const movedTotal = Object.values(result.movedRows).reduce((total, count) => total + count, 0);
		return {
			...result,
			...
			summary:
				...
				(result.filledFields.length > 0 ? `. Дозаполнено в основной карточке: ${result.filledFields.join(", ")}` : "") +
	...
	app.post("/api/patients/duplicates/dismiss", async (request, reply) => {
		const context = await requireClinicalMutationContext(request, reply, "patient duplicate dismiss");
		if (!context) return;
		if (!enforcePermissionWhenStaffKnown(request, reply, "patients.write")) return;

		const parsed = dismissSchema.safeParse(request.body);
		if (!parsed.success) return validationError(reply, "Укажите две разные карточки.");
		...
		if (!result.ok) return validationError(reply, result.reason ?? "Пара не сохранена.");
		return { ok: true, message: "Пара помечена как разные люди и больше не предлагается." };
	});
}
