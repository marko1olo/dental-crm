	...
	app.post("/api/prescriptions", async (request, reply) => {
		...
		if (
			...
			return reply.code(422).send({
				...
				message:
					"Отказ по Постановлению Правительства РФ №659 от 30.05.2026: выписка льготных рецептов и привязка полиса ОМС для анонимных пациентов категорически запрещена.",
			});
		}
