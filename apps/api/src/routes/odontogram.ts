...
export async function registerOdontogramRoutes(app: FastifyInstance) {
  ...
  app.post("/api/patients/:id/tooth-states", async (request, reply) => {
    try {
      const { id: patientId } = request.params as { id: string };
      const { toothNumber, state } = request.body as { toothNumber: number; state: any };
  ...
  app.post("/api/patients/:id/treatment-plans", async (request, reply) => {
    try {
      const { id: patientId } = request.params as { id: string };
      const { id: planId, name, items } = request.body as any;
      ...
      if (items && Array.isArray(items)) {
        ...
        if (items.length > 0) {
          const insertData = items.map((i: any) => ({
            planId: finalPlanId,
      ...
      const total = savedItems.reduce((acc, item) => acc + (parseFloat(item.price as string) * item.quantity - parseFloat(item.discount as string)), 0);
