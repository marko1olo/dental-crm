...
const toothStateValues = ["Caries", "Pulpitis", "Missing", "Crown", "Implant", "Filled", "Healthy", "Planned_Implant"] as const;
...
async function ensurePatientInOrganization(patientId: string, organizationId: string) {
...
function serializeTreatmentPlan(plan: TreatmentPlanRow, items: TreatmentPlanItemRow[]) {
  return {
    id: plan.id,
...
export async function registerOdontogramRoutes(app: FastifyInstance) {
  ...
  app.post("/api/patients/:patientId/tooth-states/batch", async (request, reply) => {
    ...
    if (toothNumbers.length === 0) return reply.send({ success: true, states: [] });
