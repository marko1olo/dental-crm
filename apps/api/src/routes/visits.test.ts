...
describe("visits routes - охрана каждого маршрута", () => {
  ...
  const routes = [
    { method: "POST", url: `/api/appointments/${zero}/visit`, mutating: true, error: "ClinicalAdminSecretRequired" },
    { method: "GET", url: `/api/visits/${zero}/draft/autosave`, mutating: false, error: "ClinicalReadSecretRequired" },
    { method: "PUT", url: `/api/visits/${zero}/draft/autosave`, mutating: true, error: "ClinicalAdminSecretRequired" },
    { method: "POST", url: `/api/visits/${zero}/draft/accept`, mutating: true, error: "ClinicalAdminSecretRequired" }
  ] as const;
  ...
  for (const route of routes) {
    ...
    test(`${label} — токен кабинета без секрета администратора не пропускается`, async () => {
      ...
      const response = await app.inject({
        method: route.method,
        url: route.url,
        headers: { "x-dente-clinic-token": signToken({ organizationId: org }, TOKEN_SECRET()) }
      });
      assert.strictEqual(
        response.statusCode,
        403,
  ...
  test("оба фактора вместе открывают маршрут — охрана не кирпичная стена", async () => {
    ...
    const headers = {
      "x-dente-clinic-token": signToken({ organizationId: org }, TOKEN_SECRET()),
    ...
    assert.notStrictEqual(rejected.statusCode, 401, "гейт пройден, а ответ 401");
    assert.notStrictEqual(rejected.statusCode, 403, "гейт пройден, а ответ 403");
  });
});
