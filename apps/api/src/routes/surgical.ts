...
import {
  drillProtocols, doctorCommissions, schedulerReservations,
  treatmentPlans, patients, appointments, users
} from '../db/schema.js';
import { eq, and, isNull, lte, sql } from 'drizzle-orm';

// --- Types matching frontend boneQualityEngine ---
interface HUZoneProfile { corticalHU: number; cancellousHU: number; apicalHU: number; }
interface DrillStep { step: number; drillType: string; diameterMm: number; depthMm: number; rpmRange: string; torqueNcm: string; irrigation: boolean; note?: string; }

type MischClass = 'D1' | 'D2' | 'D3' | 'D4';
type ImplantSystem = 'osstem' | 'straumann' | 'nobel' | 'bredent' | 'mdi' | 'other';

function classifyMisch(avgHU: number): MischClass {
...
function extractHUZones(samples: number[]): HUZoneProfile {
  if (samples.length === 0) return { corticalHU: 0, cancellousHU: 0, apicalHU: 0 };
...
export default async function registerSurgicalRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/surgical/drill-protocol/save', async (req, reply) => {
    ...
    const steps: DrillStep[] = buildDrillSteps(mischClass, body.implantSystem, body.implantDiameterMm, body.implantLengthMm);
  ...
  fastify.get('/api/surgical/drill-protocols', async (req, reply) => {
    const { patientId, toothFdi } = req.query as { patientId?: string; toothFdi?: string };
    let rows;
  ...
  fastify.post('/api/commissions', async (req, reply) => {
    const body = req.body as {
      ...
      materialCostDeductionPct?: number;
    };
    ...
    const [row] = await db.insert(doctorCommissions).values({
      organizationId: body.organizationId,
      userId: body.userId,
      specialty: body.specialty as any,
      serviceCategory: body.serviceCategory as any,
  ...
  fastify.post('/api/commissions/calculate', async (req, reply) => {
    ...
    const [config] = await db.select().from(doctorCommissions)
      .where(and(
        ...
        eq(doctorCommissions.serviceCategory, body.serviceCategory as any),
        eq(doctorCommissions.isActive, true),
      )).limit(1);

    if (!config) return reply.code(404).send({ error: 'Commission config not found for this doctor+category' });
  ...
  fastify.post('/api/scheduler/reserve', async (req, reply) => {
    ...
    let recallDueAt: Date | null = null;
    if (body.phase === 2) {
  ...
  fastify.get('/api/scheduler/reservations', async (req, reply) => {
    const { organizationId, phase } = req.query as { organizationId?: string; phase?: string };
    if (!organizationId) return reply.code(400).send({ error: 'organizationId required' });
    const conditions = [eq(schedulerReservations.organizationId, organizationId)];
    if (phase) conditions.push(eq(schedulerReservations.phase, parseInt(phase)));
  ...
  fastify.get('/api/scheduler/recall-due', async (req, reply) => {
    ...
    const now = new Date();
    const overdue = await db.select().from(schedulerReservations).where(
      and(
