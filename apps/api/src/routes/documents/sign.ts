...
export async function register(app: FastifyInstance) {
	app.post("/api/documents/:id/sign", async (request, reply) => {
		...
		if (!signatureSvg.startsWith("<svg") || !signatureSvg.endsWith("</svg>")) {
			return reply.code(400).send({
				error: "ValidationError",
				message: "Invalid signature format: must be an SVG element.",
			});
		}

		if (
			/(<script|<\/?iframe|<\/?object|<\/?embed|on\w+\s*=)/i.test(signatureSvg)
		) {
			return reply.code(400).send({
				error: "ValidationError",
				message: "Unsafe signature content detected.",
			});
		}
		...
		try {
			...
			if (replayed) {
				return reply.code(409).send({
					...
					message:
						"Эта подпись уже использована для другого документа. Повторное использование запрещено.",
				});
			}
			...
			if (doc.status !== "draft") {
				return reply.code(409).send({
					error: "Conflict",
					message:
						"Подпись невозможна: документ уже финализирован или аннулирован.",
				});
			}
