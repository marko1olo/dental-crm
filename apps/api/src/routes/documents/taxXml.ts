...
function checkDocumentPreconditions(document: DocumentType, reply: FastifyReply) {
...
function handleDuplicateTaxCertificate(document: DocumentType, reply: FastifyReply) {
...
function getBlockReasonForDocument(xmlDocument: any, xmlPatient: any, xmlRenderContext: any) {
  ...
  if (chainBlockReason) return chainBlockReason;

  return null;
}

async function handleTaxXmlRequest(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
  ...
  const preconditionError = checkDocumentPreconditions(document, reply);
  ...
  if (taxDocumentUsesPaymentSnapshot(document.kind) && !taxPaymentSnapshot) {
    const duplicateError = handleDuplicateTaxCertificate(document, reply);
  ...
  const blockReason = getBlockReasonForDocument(xmlDocument, xmlPatient, xmlRenderContext);
  if (blockReason) {
    return reply.code(409).send(apiError(blockReason));
  }

  const duplicateTaxCertificateError = handleDuplicateTaxCertificate(xmlDocument, reply);
  if (duplicateTaxCertificateError) return duplicateTaxCertificateError;
...
export async function register(app: FastifyInstance) {
  app.get<{ Params: { id: string } }>("/api/documents/:id/tax-xml", handleTaxXmlRequest);
}
