...
function telegramMessagePreviewRejection(error: unknown): {
...
const telegramTransportFailureLabels: Record<
...
function telegramRetryAfterSeconds(
...
function telegramTransportFailureText(
	result: TelegramTransportFailure,
	scope: string,
): string {
...
function telegramPhotoFallbackWarning(
	result: TelegramTransportFailure,
): string {
...
function telegramPhotoCaptionSplitTextWarning(
	result: TelegramTransportFailure,
): string {
	return telegramTransportFailureText(
		result,
...
function telegramOutboxTransportFailureWarning(
	result: TelegramTransportFailure,
): string {
	return telegramTransportFailureText(result, "Telegram не принял сообщение");
}

function telegramCallbackTransportFailureWarning(
	result: TelegramTransportFailure,
): string {
	return telegramTransportFailureText(
		result,
...
function telegramWebhookReplyFailureWarning(
	result: TelegramTransportFailure,
): string {
	return telegramTransportFailureText(result, "Ответ Telegram не отправлен");
}

function outboxDeliveryClaimKey(
	outboxItemId: string,
...
function parseTelegramQueryPositiveInt(
	value: unknown,
	fallback: number,
	max: number,
): number {
...
function parseTelegramOutboxStatusQuery(
	value: unknown,
): DenteTelegramOutboxStatusFilter {
	const raw = firstTelegramQueryValue(value);
...
function parseTelegramOutboxTemplateQuery(
	value: unknown,
...
function parseTelegramOutboxQuery(
	query: unknown,
): BuildDenteTelegramOutboxOptions {
	const source =
...
function parseTelegramOutboxRuntimeScopeQuery(query: unknown): {
	organizationId: string | null;
	botConfigId: string | null;
} {
	const source =
...
function parseTelegramSubjectTypeQuery(
	value: unknown,
): "patient" | "staff" | "all" {
	const raw = firstTelegramQueryValue(value);
	if (!raw || raw === "all") return "all";
...
function parseTelegramLinkCodeStatusQuery(
	value: unknown,
): DenteTelegramLinkCodeListStatusFilter {
	const raw = firstTelegramQueryValue(value);
	if (!raw || raw === "all") return "all";
...
function parseTelegramChatLinkStatusQuery(
	value: unknown,
): DenteTelegramChatLinkListStatusFilter {
	const raw = firstTelegramQueryValue(value);
	if (!raw || raw === "all") return "all";
...
function parseTelegramLinkCodeListQuery(
	query: unknown,
): BuildDenteTelegramLinkCodeListOptions {
	const source =
		query && typeof query === "object" ? (query as UnknownRecord) : {};
...
function parseTelegramChatLinkListQuery(
	query: unknown,
): BuildDenteTelegramChatLinkListOptions {
	const source =
		query && typeof query === "object" ? (query as UnknownRecord) : {};
...
function parseTelegramOutboxSendDueInput(
	body: unknown,
...
function dueOutboxClientMutationId(
	outboxItemId: string,
	scheduledAt: string,
): string {
...
function isDenteTelegramOutboxItemDue(
	...
	const transport = await (async () => {
		...
		if (photoUrl) {
			const shouldSplitPhotoCaption =
				deliveryText.length > telegramPhotoCaptionMaxLength;
			...
			if (photoTransport.ok) {
				...
				deliveryWarnings.push(
					telegramPhotoCaptionSplitTextWarning(textTransport),
				);
...
function parseTelegramWorkerInt(
...
function normalizedTelegramBotUsername(
...
function visualCardUrlsFromEnvConfig(
...
function postVisitCheckupDelayHoursFromEnvConfig(
	record: UnknownRecord,
...
function reviewRequestDelayHoursFromEnvConfig(
	record: UnknownRecord,
): number | null {
	const parsed =
...
function clinicBotEnvConfigFor(
	settings: DenteTelegramBotSettings,
): TelegramClinicBotEnvConfig | null {
...
function configuredSharedBotUsername(
	settings: DenteTelegramBotSettings,
): string | null {
...
function configuredClinicOwnedBotUsername(
	settings: DenteTelegramBotSettings,
): string | null {
	return normalizedTelegramBotUsername(
...
function configuredClinicOwnedBotToken(
	settings: DenteTelegramBotSettings,
): string | null {
	return (
...
function denteTelegramOutboxRuntimeScope(
...
function denteTelegramResolvedOutboxRuntime(
	runtime: TelegramRuntimeContext,
...
function resolveTelegramOutboxRuntimeScopeFromQuery(
	query: unknown,
):
...
async function requireTelegramControlPlaneAccess(
...
function extractSafeCallbackAction(
...
function safeHttpsTelegramButton(
...
function telegramInlineKeyboardRows(
...
const telegramCareCallbackTopicByAction: Partial<
...
function careTopicFromFreeText(
	text: string,
...
function replyMarkupWithNextActions(
...
function telegramFeatureEnabled(
	settings: DenteTelegramBotSettings,
...
function featureDisabledReplyFor(
	settings: DenteTelegramBotSettings,
	title: string,
): TelegramWebhookReplyPackage {
	return {
...
function unsafeTelegramAttachmentReplyFor(
	settings: DenteTelegramBotSettings,
	...
	return denteTelegramBotStatusSchema.parse(
		readableTelegramPayload({
			...
			activeChatLinkCount: isPrimaryRuntime
				? listDenteTelegramChatLinks(100).filter(
...
function registerTelegramStatusRoutes(
	...
	app.get("/api/telegram/status", telegramControlPlaneRouteOptions, async () =>
		buildStatus(),
	);
...
function registerTelegramSettingsRoutes(
	app: FastifyInstance,
	telegramControlPlaneRouteOptions: {
		preHandler: (request: FastifyRequest, reply: FastifyReply) => Promise<void>;
	},
...
function registerTelegramOutboxRoutes(
	app: FastifyInstance,
	telegramControlPlaneRouteOptions: {
		preHandler: (request: FastifyRequest, reply: FastifyReply) => Promise<void>;
	},
...
function registerTelegramLinkRoutes(
	app: FastifyInstance,
	telegramControlPlaneRouteOptions: {
		preHandler: (request: FastifyRequest, reply: FastifyReply) => Promise<void>;
	},
...
function registerTelegramPreviewRoutes(
	app: FastifyInstance,
	telegramControlPlaneRouteOptions: {
		preHandler: (request: FastifyRequest, reply: FastifyReply) => Promise<void>;
	},
