...
import { familyGroups, payments, patients } from "../db/schema.js";
...
async function familyGroupForOrganization(familyGroupId: string, organizationId: string) {
  const [family] = await db
    .select()
    .from(familyGroups)
    .where(and(eq(familyGroups.id, familyGroupId), or(eq(familyGroups.organizationId, organizationId), isNull(familyGroups.organizationId))))
    .limit(1);
  if (!family) return null;
  if (!family.organizationId) {
    await db.update(familyGroups).set({ organizationId }).where(eq(familyGroups.id, family.id));
    return { ...family, organizationId };
  }
  return family;
}

async function familyMembersForOrganization(familyGroupId: string, organizationId: string) {
  return db.select({
    id: patients.id,
    ...
    .from(patients)
    .where(and(eq(patients.familyGroupId, familyGroupId), eq(patients.organizationId, organizationId)));
}
...
export async function registerFamilyFinanceRoutes(app: FastifyInstance) {
  ...
  app.get("/api/finance/family/:familyGroupId", async (req, reply) => {
    const organizationId = await requireResolvedOrganizationId(req, reply, "family finance read");
    if (!organizationId) return;

    const { familyGroupId } = req.params as { familyGroupId: string };
    const family = await familyGroupForOrganization(familyGroupId, organizationId);
    if (!family) return reply.code(404).send({ error: "Family group not found" });

    const members = await familyMembersForOrganization(familyGroupId, organizationId);
    if (members.length === 0) return reply.code(404).send({ error: "Family group not found" });
  ...
  app.get("/api/finance/family/patient/:patientId", async (req, reply) => {
    ...
    const family = await familyGroupForOrganization(patient.familyGroupId, organizationId);
    if (!family) return reply.code(404).send({ error: "Family group not found" });

    const members = await familyMembersForOrganization(patient.familyGroupId, organizationId);
  ...
  app.post("/api/finance/family", async (req, reply) => {
    const organizationId = await requireResolvedStaffOrAdminOrganizationId(req, reply, "family finance write");
    if (!organizationId) return;
    ...
    wsBroker.broadcastToOrganization(organizationId, { type: "FAMILY_GROUP_CREATED", payload: family });
    return family;
  });
  ...
  app.put("/api/finance/family/:id", async (req, reply) => {
    ...
    const [family] = await db.update(familyGroups)
      .set(data)
      .where(and(eq(familyGroups.id, id), eq(familyGroups.organizationId, organizationId)))
      .returning();

    if (!family) return reply.code(404).send({ error: "Family group not found" });
    wsBroker.broadcastToOrganization(organizationId, { type: "FAMILY_GROUP_UPDATED", payload: family });
    return family;
  });
  ...
  app.delete("/api/finance/family/:id", async (req, reply) => {
    ...
    if (members.length > 0) {
      return reply.code(400).send({ error: "Cannot delete family group with members" });
    }

    const result = await db.delete(familyGroups)
      .where(and(eq(familyGroups.id, id), eq(familyGroups.organizationId, organizationId)))
      .returning() as any;
    const family = result[0];

    if (!family) return reply.code(404).send({ error: "Family group not found" });
    wsBroker.broadcastToOrganization(organizationId, { type: "FAMILY_GROUP_DELETED", payload: { id } });
    return { success: true };
  });

  // POST /api/finance/family/pay — deduct balance in transaction
  app.post("/api/finance/family/pay", async (req, reply) => {
    const organizationId = await requireResolvedStaffOrAdminOrganizationId(req, reply, "family finance payment");
    if (!organizationId) return;
    ...
    try {
      const result = await db.transaction(async (tx) => {
        const [patient] = await tx
          ...
          .from(patients)
          .where(and(eq(patients.id, payload.patientId), eq(patients.organizationId, organizationId)))
          .limit(1);
        ...
        const [family] = await tx
          .select()
          .from(familyGroups)
          .where(and(eq(familyGroups.id, payload.familyGroupId), or(eq(familyGroups.organizationId, organizationId), isNull(familyGroups.organizationId))))
          .limit(1)
          .for("update");
        if (!family) {
      ...
      wsBroker.broadcastToOrganization(organizationId, { type: "FAMILY_BALANCE_UPDATED", payload: { organizationId, familyGroupId: payload.familyGroupId, balance: result.newBalance } });
      wsBroker.broadcastToOrganization(organizationId, { type: "PAYMENT_CREATED", payload: result.payment });
