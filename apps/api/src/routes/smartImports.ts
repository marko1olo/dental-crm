...
function buildClinicProfileSuggestion(
...
function publicLookupLabeledDigits(
...
function buildClinicSuggestionFromFields(
	fields: UpdateClinicProfileInput,
): SmartImportClinicProfileSuggestion | null {
...
function buildManualClinicPublicLookupSuggestion(
	fields: UpdateClinicProfileInput,
...
function uniqueClinicPublicLookupSuggestions(
...
function mapDadataPartySuggestion(
	item: unknown,
): ClinicPublicLookupSuggestion | null {
...
async function fetchDadataClinicSuggestions(
...
function registerMigrationSourceRoute(
	sourceRef: string,
	sourceKind: SmartImportLegacySource["kind"],
	safeDisplayName: string,
) {
...
function migrationDbfFolderSourceRequired(
	folderPath: string,
	firstMatchPath: string,
) {
...
function migrationProfileSafeAlias(
	profileLabel: string,
	kind: SmartImportLegacySource["kind"],
	sourceRef: string,
) {
...
function migrationWorkstationSignalChannelTitle(
...
async function readWindowsMigrationWorkstationSignalValues(
	warnings: Set<string>,
) {
...
async function collectMigrationWorkstationSignals(
...
function migrationProfileRelativeDataRoots(
...
function migrationRootsFromWorkstationProfiles(
	profiles: (typeof migrationWorkstationProfiles)[number][],
) {
	const roots: string[] = [];
...
function migrationRootsFromWorkstationSignals(
...
function migrationCandidateFromWorkstationSignal(
...
async function discoverLocalMigrationSources(
	input: MigrationLocalSourceDiscoveryRequest,
) {
...
function migrationWorkupExtractableEntities(
	kind: SmartImportLegacySource["kind"],
) {
...
function migrationReadinessItem(
...
function migrationBridgeAction(
	input: MigrationBridgeKitAction,
): MigrationBridgeKitAction {
	return input;
}

function migrationBridgeOutputManifest(
	kind: SmartImportLegacySource["kind"],
	endpoint: string,
...
function buildMigrationLocalSourceWorkup(
	input: MigrationLocalSourceWorkupRequest,
) {
	if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
		throw new Error(
			"Чтение локальной файловой системы сервера отключено в облачном режиме. Доступ запрещен.",
		);
	}
...
const migrationProbeArtifactKindTitles: Record<
...
function migrationProbeArtifactKind(
	filePath: string,
): MigrationProbeArtifactKind {
...
async function readMigrationProbeHeader(
	filePath: string,
	maxBytes: number,
): Promise<Buffer> {
...
function migrationProbeFormatSignals(
	filePath: string,
	header: Buffer,
...
function migrationProbeSafeArtifact(
	filePath: string,
	kind: MigrationProbeArtifactKind,
	depth: number,
	signals: string[],
...
function incrementMigrationProbeCount(
	counts: ReturnType<typeof emptyMigrationProbeCounts>,
	kind: MigrationProbeArtifactKind,
) {
...
async function buildMigrationLocalSourceProbe(
	input: MigrationLocalSourceProbeRequest,
) {
	if (process.env.DENTE_ENABLE_LOCAL_MIGRATION_SCAN !== "true") {
		throw new Error(
...
function migrationAutopilotPriority(
	score: number,
...
type MigrationAutopilotPacketStatus =
	MigrationAutopilotOperatorPacket["overallStatus"];

function migrationPacketStatusFromSources(
	sources: MigrationAutopilotSource[],
...
function migrationPacketLaneStatusFromSubset(
	sources: MigrationAutopilotSource[],
...
function migrationPacketStatusFromBridgeStatus(
...
function migrationHandoffPhaseForBridge(
...
function migrationSourceCanStartDryRunPreview(
...
function migrationSmartPreviewSourceRef(
...
function migrationCandidateFromSmartLegacySource(
	source: SmartImportLegacySource,
	index: number,
...
function clinicLookupInputFromSmartImport(
...
async function buildMigrationAutopilot(
	orgId: string,
...
function detectLegacySourceKind(
	value: string,
...
function safeLegacySourceAlias(
	kind: SmartImportLegacySource["kind"],
	sourceRef: string | null,
) {
...
async function buildSmartImportPreview(
	orgId: string,
...
type SmartImportPreviewType = Awaited<
	ReturnType<typeof buildSmartImportPreview>
>;

function getLineClassificationRows(
...
function getPatientPreviewRows(
	preview: SmartImportPreviewType,
): SmartImportCsvRow[] {
...
function getImagingPreviewRows(
	preview: SmartImportPreviewType,
): SmartImportCsvRow[] {
...
function getClinicSuggestionRows(
	preview: SmartImportPreviewType,
): SmartImportCsvRow[] {
...
function getSafeHandoffSummaryRow(
...
function getSafeHandoffMigrationStepRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
...
function getSafeHandoffPatientRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	return preview.patientPreview.rows.map((row) => [
		"patient_row",
...
function getSafeHandoffImagingRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	return preview.imagingPreview.rows.map((row) => {
...
function getSafeHandoffClinicSuggestionRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	if (!preview.clinicSuggestion) return [];
	return Object.entries(preview.clinicSuggestion.fields).map(
...
function getSafeHandoffPublicLookupRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	return preview.publicLookupTargets.map((target, index) => [
		"public_lookup",
...
function getSafeHandoffLegacySourceRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	return preview.legacySources.map((source, index) => [
		"legacy_source",
...
function getSafeHandoffParserNoteRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
	return preview.parserNotes.map((note, index) => [
		"parser_note",
...
function getSafeHandoffPrivacyWarningRows(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
): SafeHandoffRow[] {
...
function buildSmartImportSafeHandoffReportCsv(
	preview: Awaited<ReturnType<typeof buildSmartImportPreview>>,
) {
...
function buildMigrationAutopilotReportCsv(
...
function parseSmartImportPayload<T>(
