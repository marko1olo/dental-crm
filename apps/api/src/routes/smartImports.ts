...
function classifyLine(
  ...
  if (
    /клиник|стоматолог|dental|dent|clinic/i.test(text) &&
    hasLikelyName(text)
  ) {
    clinicScore += 0.08;
  ...
  if (hasLegacySourceKeyword) {
    ...
    reasons.push(
      "найдены признаки старой МИС, базы, архива снимков или выгрузки",
    );
  }
  if (hasLegacyMisName) {
...
function uniqueClinicPublicLookupSuggestions(
  ...
  for (const suggestion of suggestions) {
    const key = JSON.stringify(
      Object.entries(suggestion.fields).sort(([left], [right]) =>
...
async function discoverLocalMigrationSources(
  ...
  const roots = [
    ...
    .filter(
      (root, index, all) =>
        migrationRootExists(root) && all.indexOf(root) === index,
    );
  ...
  while (queue.length && scannedFolders < input.maxFolders) {
    ...
    const detectedSourceKind = migrationSourceKindFromCounts({
      ...
      firstMatchPath:
        firstMatchPath || firstProfileEvidencePath || rawSourceRef,
      databaseFiles,
      dumpFiles,
      tableFiles,
      archiveFiles,
...
async function buildMigrationAutopilot(input: MigrationAutopilotRequest) {
  ...
  const smartImportPreview = input.smartImport
    ? buildSmartImportPreview(input.smartImport)
    : null;
...
function buildSmartImportPreview(input: {
  sourceName: string;
  rawText: string;
  mode: SmartImportMode;
}) {
...
function buildSmartImportReportCsv(
  preview: ReturnType<typeof buildSmartImportPreview>,
) {
  ...
  preview.imagingPreview.rows.forEach((row) => {
    rows.push([
      ...
      row.status === "ready"
        ? "ready_for_commit"
        : "needs_mapping_or_source_fix",
      "",
    ]);
  });

  if (preview.clinicSuggestion) {
    Object.entries(preview.clinicSuggestion.fields).forEach(
      ([field, value]) => {
        rows.push([
