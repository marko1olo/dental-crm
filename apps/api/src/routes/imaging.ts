...
function buildDicomSeriesGroups(rows: DicomSeriesPreviewRow[]) {
  ...
  return Array.from(buckets.values()).map(
    (seriesRows, index): DicomSeriesPreviewGroup => {
      ...
      let kind: ImagingStudyKind | null = null;
      ...
      for (const row of seriesRows) {
        ...
        if (!modality && row.modality) modality = row.modality;
        if (!patientId && row.patientId) patientId = row.patientId;
        if (!patientName && row.patientName) patientName = row.patientName;
        if (!studyInstanceUid && row.studyInstanceUid)
          studyInstanceUid = row.studyInstanceUid;
        if (!seriesInstanceUid && row.seriesInstanceUid)
          seriesInstanceUid = row.seriesInstanceUid;
        if (!studyDescription && row.studyDescription)
          studyDescription = row.studyDescription;
        if (!seriesDescription && row.seriesDescription)
          seriesDescription = row.seriesDescription;
        if (!capturedAt && row.capturedAt) capturedAt = row.capturedAt;
        if (!firstFilePath && row.filePath) firstFilePath = row.filePath;
        if (!imageRows && row.imageRows) imageRows = row.imageRows;
        if (!imageColumns && row.imageColumns) imageColumns = row.imageColumns;
        if (!bitsAllocated && row.bitsAllocated)
          bitsAllocated = row.bitsAllocated;
        if (!samplesPerPixel && row.samplesPerPixel)
