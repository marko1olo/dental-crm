...
export function parseImagingManifest(input: {
  sourceName: string;
  sourceKind: ImagingSourceKind;
  rawText: string;
}) {
  ...
  const rows: ImagingImportPreviewRow[] = (
    ...
    if (isZipArchivePath(filePath)) {
      ...
      try {
        ...
        for (const entry of dicomEntries.slice(0, dicomZipMetadataEntryLimit)) {
          ...
          filesParsed += 1;
          warnings.push(
            ...metadata.warnings.map((warning) => `${virtualPath}: ${warning}`),
          );
          rows.push(
...
function parseDicomFirstFramePixel(
  ...
  if (
    pixelDataLength < expectedBytes ||
    pixelDataOffset + expectedBytes > buffer.length
  ) {
    return {
      status: "unsupported",
      transferSyntaxUid,
  ...
  if (scale < 1)
    warnings.push(
      `Предпросмотр уменьшен с ${columns}x${rows} до ${width}x${height}.`,
    );
...
function readDicomFirstFramePreviewBuffer(
  filePath: string,
  maxFileBytes: number,
): { buffer: Buffer | null; warnings: string[] } {
  const warnings: string[] = [];
  const stats = statSync(filePath);
...
async function buildDicomFirstFramePreview(
  ...
  for (const index of candidateIndexes) {
    ...
    try {
      const previewBuffer = readDicomFirstFramePreviewBuffer(
        filePath,
        input.maxFileBytes,
      );
...
function readZipCentralDirectoryDetailed(
  filePath: string,
): ZipCentralDirectoryDetailedResult {
  const warnings: string[] = [];
  ...
  const centralDirectory = readExactFileRange(
    descriptor,
...
function readZipCentralDirectory(filePath: string): {
  entries: string[];
  warnings: string[];
} {
...
function expandDicomArchiveManifestLines(lines: string[]): {
  lines: string[];
  notes: string[];
} {
...
function buildDicomSeriesGroups(rows: DicomSeriesPreviewRow[]) {
  ...
  return Array.from(buckets.values()).map(
    (seriesRows, index): DicomSeriesPreviewGroup => {
      ...
      const patientId =
        seriesRows.find((row) => row.patientId)?.patientId ?? null;
      const patientName =
        seriesRows.find((row) => row.patientName)?.patientName ?? null;
      const studyInstanceUid =
        seriesRows.find((row) => row.studyInstanceUid)?.studyInstanceUid ??
        null;
      const seriesInstanceUid =
        seriesRows.find((row) => row.seriesInstanceUid)?.seriesInstanceUid ??
        null;
      const studyDescription =
        seriesRows.find((row) => row.studyDescription)?.studyDescription ??
        null;
      const seriesDescription =
        seriesRows.find((row) => row.seriesDescription)?.seriesDescription ??
        null;
      const capturedAt =
        seriesRows.find((row) => row.capturedAt)?.capturedAt ?? null;
      const firstFilePath =
        seriesRows.find((row) => row.filePath)?.filePath ?? null;
      const sourceKind = first?.sourceKind ?? "dicom_file";
      const sourceName = first?.sourceName ?? "dicom_series";
      const imageRows =
        seriesRows.find((row) => row.imageRows)?.imageRows ?? null;
      const imageColumns =
        seriesRows.find((row) => row.imageColumns)?.imageColumns ?? null;
      const bitsAllocated =
        seriesRows.find((row) => row.bitsAllocated)?.bitsAllocated ?? null;
      const samplesPerPixel =
        seriesRows.find((row) => row.samplesPerPixel)?.samplesPerPixel ?? null;
      const rowPixelBytes = seriesRows.reduce(
        (total, row) => total + (row.estimatedPixelBytes ?? 0),
        0,
      );
      ...
      if (kind === "cbct" && seriesRows.length < 8)
        warnings.add(
...
function parseDicomManifestLine(
  line: string,
  rowNumber: number,
  sourceKind: ImagingSourceKind,
  sourceName: string,
...
export function parseDicomSeriesManifest(input: {
  sourceName: string;
  sourceKind: ImagingSourceKind;
  rawText: string;
}) {
  ...
  const hasHeader = headers.some(Boolean);
  const rows: DicomSeriesPreviewRow[] = (
    hasHeader ? lines.slice(1) : lines
  ).map((line, index) => {
    if (!hasHeader)
      return parseDicomManifestLine(
        line,
        index + 1,
        input.sourceKind,
        input.sourceName,
    ...
    const lineFallback = parseDicomManifestLine(
      line,
      index + 2,
      input.sourceKind,
      input.sourceName,
...
function buildDicomViewerLaunchManifest(
...
function targetToolForAnnotation(
...
function buildDicomViewerViewports(
  input: DicomViewerToolStateBundleRequest,
...
function buildDicomViewerToolStateBundle(
  input: DicomViewerToolStateBundleRequest,
) {
  const series = input.series;
  const warnings = new Set<string>(series.warnings);
...
function describeDicomExecutionLaneForOperator(
...
function registerImagingImportRoutes(app: FastifyInstance) {
...
function registerImagingDicomRoutes(app: FastifyInstance) {
  ...
  app.get("/api/imaging/dicom/workbench-bundles", async (request, reply) => {
    ...
    const bundles = listDicomWorkbenchBundles(
...
function registerImagingScannerRoutes(app: FastifyInstance) {
  app.post(
...
function registerImagingStudyRoutes(app: FastifyInstance) {
  app.get("/api/imaging/studies", async (request, reply) => {
    ...
    const studies = patientId
      ? imagingStudies.filter((study) => study.patientId === patientId)
      : imagingStudies;
...
export async function registerImagingRoutes(app: FastifyInstance) {
  registerImagingImportRoutes(app);
  registerImagingDicomRoutes(app);
  registerImagingScannerRoutes(app);
  registerImagingStudyRoutes(app);
}

export function commitImagingImport(input: {
  sourceName: string;
  sourceKind: ImagingSourceKind;
  rawText: string;
}) {
