...
function buildCtSurfaceModelManifest(input: {
  ...
  warnings.push("CRM хранит только маршрут КТ-поверхности и состояние проверки; геометрия сетки остается в локальном 3D-модуле или внешнем просмотрщике моделей.");
  if (archiveOrUnknown) {
    warnings.push("Архив или неизвестный формат поверхности остается только метаданными, пока локальный модуль не проверит сетку.");
  }
  return {
    ...
    nextAction:
      readiness === "pending_local_bridge"
        ? "Передайте эту КТ-поверхность в локальный 3D-модуль для регистрации, статистики сетки и клинической проверки; CRM не хранит payload сетки."
        : readiness === "ready_external"
          ? "Откройте эту поверхность во внешнем просмотрщике моделей; CRM остается слоем пациента, привязки к КТ и заметок."
          : "Оставьте эту поверхность как метаданные, пока локальный модуль не проверит архив, формат и регистрацию КТ."
  };
}
