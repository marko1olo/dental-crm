...
function buildVisitDictationPlan(
	readiness: LocalBridgeReadinessResponse,
): LocalBridgeUsePlan {
		...
		steps: [
			...
			planStep(
				...
				dicom
					...
					: ohif ? "Открыть внешний просмотр" : "Остаться в режиме метаданных",
				"system",
				primaryPath,
				true,
				false,
...
function buildImagingImportPlan(
	readiness: LocalBridgeReadinessResponse,
): LocalBridgeUsePlan {
	const dicom = readyBridge(readiness, "dicom_cbct");
	const ocr = readyBridge(readiness, "ocr_vision");
	...
	app.post("/api/system/sync/push", async (request, reply) => {
		...
		try {
			...
			const existing = await db
				...
				.where(
					and(
						eq(dbTable.id, payload.id),
						eq(dbTable.organizationId, organizationId),
					),
				)
				.limit(1);
			...
			if (existing.length > 0) {
				...
				if (
					...
					if (localTime > remoteTime) {
						console.warn(
							`[Sync Push] Conflict detected on table ${table}, id ${payload.id}. Server version is newer.`,
						);
