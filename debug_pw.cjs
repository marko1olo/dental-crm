...
(async () => {
  ...
  await p.route('**/api/imaging/**', r => r.fulfill({status: 200, body: '{"items":[],"bundles":[],"ok":true}'}));
  ...
  try {
      const staffBtn = p.locator('button:has-text("Доктор"), button:has-text("Сисадмин"), button:has-text("Иванов")').first();
