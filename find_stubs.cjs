...
for (const sourceFile of project.getSourceFiles()) {
	const objLiterals = sourceFile.getDescendantsOfKind(
			...
			if (prop.getKind() === SyntaxKind.SpreadAssignment) {
				...
				if (
					...
					if (
						...
						if (
							body.getKind() === SyntaxKind.UndefinedKeyword ||
							body.getKind() === SyntaxKind.NullKeyword
						) {
							isEmptyStub = true;
						}
					}
				...
				if (isEmptyStub) {
					console.log(
						`Found stub in ${sourceFile.getFilePath()} at line ${prop.getStartLineNumber()}: ${prop.getName()}`,
					);
