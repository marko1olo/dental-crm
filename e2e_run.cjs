...
(async () => {
	...
	page.on("console", (msg) => {
		if (msg.type() === "error")
			errors.push(`CONSOLE ERROR [${msg.location().url}]: ${msg.text()}`);
	});
	...
	const loginInput = page
		.locator(
			'input[type="text"], input[type="email"], input[name="login"], input[name="email"]',
		)
		.first();
	...
	if (loginExists) {
		...
		if (await passInput.isVisible().catch(() => false)) {
			...
			const submitBtn = page
				.locator(
					'button[type="submit"], button:has-text("Войти"), button:has-text("Login")',
				)
				.first();
			if (await submitBtn.isVisible().catch(() => false)) {
				...
				const workspaceVisible = await page
					.locator('[class*="workspace"], [class*="shell"], nav, aside')
					.first()
					.isVisible()
					.catch(() => false);
