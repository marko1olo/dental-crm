...
## 10. Оптимизация памяти и диска (Lightweight Runtime)
Для работы на слабых компьютерах в DENTE CRM внедрены автоматические механизмы оптимизации:
- **Zero-Leak Runtime**: Тяжёлые модули (3D-зубная формула, Планировщик лечения) загружаются в память *только при их открытии*, а при закрытии — выгружаются (очистка DOM-дерева), освобождая оперативную память.
...
## 11. Шифрование и Резервное Копирование (Cloud Vault)
...
## 16. Клинический SOAP Дневник и Судебно-Медицинская Защита (Forensic Lock)

### 16.1 Стандарт SOAP (Progress Notes)
Все медицинские записи ведутся по международному стандарту SOAP, что гарантирует полноту диагностических данных и юридическую силу карты:
- **S (Subjective):** Жалобы пациента и анамнез со слов пациента.
- **O (Objective):** Объективный статус (Status Localis), результаты осмотра полости рта (пальпация, перкуссия, зондирование), данные КЛКТ.
- **A (Assessment):** Диагноз. Полностью интегрирован встроенный справочник стоматологических кодов **МКБ-10** (K02, K04, K05, K07 и др.). Диагноз привязывается к конкретному зубу по системе FDI (например, зуб 36).
- **P (Plan):** Описание проведенного лечения и выданные рекомендации.

### 16.2 Автоматизация (Clinical Templates)
Для сокращения времени на заполнение медицинских карт до 10 секунд, система предлагает более **30 предустановленных стоматологических шаблонов**:
- При открытии дневника выберите подходящий шаблон (например, "Лечение пульпита (Экстирпация)").
- Поля S, O и P автоматически заполнятся заготовленным эталонным текстом, а диагноз МКБ-10 проставится автоматически.
- Вы можете свободно редактировать подставленный текст под клиническую картину конкретного пациента. Автосохранение черновика работает каждую 30 секунд.

### 16.3 Forensic Lock (Судебно-медицинская блокировка)
С целью минимизации судебных и страховых рисков клиники (подделка карт, изменения задним числом), DENTE реализует механизм **Forensic Lock**:
1. После того как врач заполнил дневник приема, он нажимает **"Завершить и подписать прием"**. Требуется подтверждение личным PIN-кодом.
2. Система вычисляет уникальный криптографический **SHA-256 хэш** дневника и навсегда блокирует поля от редактирования (`isLocked = true`). Хэш выступает аналогом ЭЦП (Электронной Цифровой Подписи).
3. **Ревизии и Аудит (Court Trail):** Если после блокировки требуется внести исправления (например, по запросу страховой компании), система **запрещает тихое удаление или изменение**. Администратор может инициировать ревизию: система создаст новую запись в специальной таблице ревизий (`visit_diary_revisions`), сохранив старый текст, дату изменения и ФИО автора правок. Индикатор количества ревизий (например, `⚠ 1 ревиз.`) навсегда останется в шапке дневника.

### 16.4 Печать медицинской карты (Форма 043/у)
- Заблокированный дневник можно отправить на печать в виде стандартизированного превью Формы № 043/у (согласно приказу МЗ РФ).
- В печатной форме в специальном зеленом блоке распечатывается SHA-256 хэш подписи и отметка о времени блокировки (Time-stamp), что придает документу дополнительный вес при внутренних проверках качества или в суде.


## 17. Executive BI Analytics (Аналитика для Руководителя)
...
1. **Когортный анализ LTV (Lifetime Value)**:
   - Позволяет отследить удержанный доход. Пациенты группируются по месяцу первого обращения.
   - Вы можете увидеть, сколько выручки принесла каждая когорта через 1, 3, 6 и 12 месяцев.
2. **Воронка планов лечения**:
   - Наглядно показывает конверсию планов от этапа "Черновик" (Draft) к "Согласовано" (Approved) и "Завершено" (Completed). Помогает кураторам искать "застрявшие" сметы.
3. **Утилизация кресел (Chair Utilization)**:
   - Рассчитывается как отношение забронированного времени к общим рабочим часам клиники. Помогает понять, какие кабинеты простаивают.
4. **Чистая маржинальность докторов**:
   - Лидерборд с учетом валовой выручки, комиссий врача и себестоимости списанных материалов по каждому визиту.

### Производительность:
- Все отчеты рассчитываются сервером в фоновом режиме (каждую ночь), что гарантирует моментальную загрузку дашборда на любых устройствах.
- Графики оптимизированы и не вызывают перегрузок памяти браузера на слабых устройствах клиники (OOM Safety).
