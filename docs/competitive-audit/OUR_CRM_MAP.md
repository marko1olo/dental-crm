...
### 2.10. Пятьдесят четыре киллер-фичи снижения трения, ликвидации симуляторов и автономии врача (Мандаты 8e, 8k, 8n)
...
- **Бэкенд**:
  - `apps/api/src/routes/sterilization.ts` (коммит `6c41ee42d`).
...
- **Фронтенд / Shared**:
  - `apps/web/src/components/orthodontics/OrthodonticVisitProtocolWidget.tsx`, `OrthodonticStudioModal.tsx`, `packages/shared/src/orthodontics/orthoEngine.ts`, `types.ts` (коммит `6c41ee42d`).
- **Тесты**:
