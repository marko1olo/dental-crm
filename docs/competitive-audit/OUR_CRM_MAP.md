...
### 2.10.258. Волна 177: Математика CPR-панорамы (Catmull-Rom сплайн и нормали), классификация плотности кости по Misch (D1..D5 HU), клиренс нижнечелюстного нерва IAN (1.5 мм), ликвидация 5 параллельных перспектив по Мандату 8s и десктопная эргономика расписания 1440x900 (Мандаты 8c, 8d, 8e, 8i, 8k, 8n, 8p, 8s, 8t)
- **Идея & Бизнес-эффект**: Интеграция математического ядра CPR (Curved Planar Reformation) сплайн-интерполяции и нормалей зубной дуги для построения ортогональных кросс-секционных срезов КТ; внедрение анатомической классификации плотности костного ложа по Мишу (D1..D5 HU) с генерацией протокола сверления и торка; аналитический расчет и строгая валидация безопасного клиренса до нижнечелюстного нерва (IAN safety buffer clearance >=1.5 мм) без блокировки врача (Мандат 8e); консолидация 5 параллельных перспектив в пользу канонических экранов по Мандату 8s; устранение дефекта обрезания текста тулбара расписания на десктопе 1440x900.
- **Архитектурные механизмы**:
  1. *Математическое ядро CPR (Curved Planar Reformation) и нормали зубной дуги*:
  ...
  2. *Классификация плотности костной ткани по Misch (D1..D5 HU) с клиническими рекомендациями*:
  ...
  3. *Валидация безопасного клиренса до нижнечелюстного нерва (IAN safety clearance >=1.5 мм)*:
  ...
  4. *Ликвидация 5 параллельных перспектив по Закону Единого Авторитета (Мандат 8s)*:
     - В `apps/web/src/App.tsx`, `workspaceShell.tsx` и `store/perspectiveStore.ts` устранено дублирование параллельных экранов (`ChairsiderPerspectiveView`, `FrontdeskPerspectiveView`, `CasePresentationView`, `OrthodonticPerspectiveView`, `PediatricPerspectiveView`);
  ...
  5. *Эргономика тулбара расписания на десктопе 1440x900*:
...
- **Файлы**: `packages/shared/src/radiology/cprMath.ts`, `packages/shared/src/radiology/cprPanoramicEngine.ts`, `apps/web/src/components/dicom/panoramicMprMath.ts`, `apps/web/src/components/dicom/panoramicArch.ts`, `packages/shared/src/radiology/boneQualityEngine.ts`, `packages/shared/src/radiology/cbctCropBox.ts`, `apps/web/src/components/dicom/BoneQualityPanel.tsx`, `packages/shared/src/radiology/cbctSafetyEngine.ts`, `packages/shared/src/radiology/implantSafetyClearance.ts`, `apps/web/src/components/dicom/ctPlanningPersistence.ts`, `apps/web/src/App.tsx`, `apps/web/src/workspaceShell.tsx`, `apps/web/src/store/perspectiveStore.ts`, `apps/web/src/components/schedule/ScheduleFilterStrip.tsx`, `apps/web/src/ScheduleView.tsx`, `docs/competitive-audit/FEATURES_REGISTRY.md`, `docs/competitive-audit/BACKLOG.md`, `docs/competitive-audit/OUR_CRM_MAP.md`.
- **Тесты**: `check:encoding` 0 ошибок (UTF-8), Single-Compiler Gate защищен (Мандат 8t), `cbctSafetyAndMisch.test.ts` PASS, `panoramicReconstruction.test.ts` PASS, 325/325 фичей со статусом [ДА] (100% паритет).
