...
- **Возможности**:
  ...
  - **Унификация параллельного клона зубной формулы в канонический OdontogramModule (Мандат 8s / Волна 185, коммит `4f500b5e2`)**: компоненты StomX-дефектов и позиционных аномалий в `apps/web/src/components/formula/` (`stomxFormulaAdapter.ts`, `StomxDefectsPalette.tsx`, `StomxToothFormulaView.tsx`) переведены на прозрачный адаптер к каноническому `OdontogramModule.tsx` с полной двусторонней трансляцией состояний FDI (11..48 adult, 51..85 pediatric) без дублирования состояния формулы.
