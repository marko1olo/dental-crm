...
- **Возможности**:
  ...
  - **Унификация складского домена и утилизации карпул в канонический модуль `components/inventory/` с фасадами в `components/warehouse/` (Мандат 8s / Волна 186)**: списание клинических расходников, нарядов и утилизация карпул консолидированы в канонических компонентах `apps/web/src/components/inventory/` (`ClinicalWriteoffModal.tsx`, `ProcedureMaterialDeductionModal.tsx`, `mdlp/MdlpDisposalQueueModal.tsx`, `mdlp/SeniorNurseDisposalActModal.tsx`), а папка `components/warehouse/` выступает прозрачным фасадом-делегатом (`NurseCarpuleDisposalModal.tsx`, `WarehousePackageWriteOffBar.tsx`, `warehousePackageWriteOffEngine.ts`); обеспечена 100% автономия медсестры — 1-кликовое списание пустых карпул анестетиков без комиссий и мягкий овердрафт склада без блокировки операций по Мандатам 8e п. 10, 8n п. 2.
...
### 2.10.269. Волна 188: Гидратация Telegram в PostgreSQL, мобильная эргономика расписания и кассы 54-ФЗ (390x844) и ликвидация фасадных импортов (Мандаты 8c, 8d, 8e, 8h, 8n, 8s, 8t)
- **Идея & Бизнес-эффект**: Полная гидратация состояния Telegram из базы данных PostgreSQL с автоматическим сохранением подтверждений приёмов в таблицу `appointments` со статусом `confirmed`, ликвидация фасадных импортов в тестах Telegram по Мандату 8s, устранение разрыва мобильной эргономики на экранах 390x844 (тач-таргеты $\ge 44\text{px}$ на всех кнопках и полях ввода `FastCheckoutModal.tsx`, `CashRegisterModal.tsx` и `QuickBookingDrawer.tsx`, адаптивный липкий футер `flex-col-reverse` без горизонтального скролла), мобильная адаптация расписания `ScheduleGrid.tsx` и `ScheduleFilterStrip.tsx` (коммит `938bc4ebc`): устранение жесткого хардкода `min-w-[700px]`, адаптивная сетка соло-врача на 100% ширины экрана 390px, тач-таргеты $\ge 40\text{--}44\text{px}$ на всех 22 кнопках меню опций по стандартам Apple HIG и Мандатам 8c, 8d, 8e, 8n.
- **Архитектурные механизмы**:
  1. *Полная гидратация Telegram и прямая синхронизация подтверждений в PostgreSQL (Мандаты 8b, 8e, 8n)*:
     - В `apps/api/src/routes/telegram.ts` и `services/telegram/telegramLegacyMemoryStore.ts` внедрена сквозная передача гидратированного из БД PostgreSQL состояния `DomainState` (`hydrateTelegramDomainState`) во все функции конвейера: `prepareDenteTelegramOutboxDelivery`, `buildDenteTelegramOutbox`, `executeTelegramOutboxSend`, `executeDenteTelegramOutboxDueBatch`, `visibleTelegramScheduleAppointments` и `handleDenteTelegramAppointmentCallback`;
  ...
  2. *Мобильная адаптация расписания соло-врача (Мандаты 8c, 8d, 8e, 8n, коммит `938bc4ebc`)*:
  ...
  3. *Мобильная эргономика кассы 54-ФЗ и дровера записи (Apple HIG, Мандаты 8c, 8d, 8e, 8n)*:
...
- **Файлы**: `apps/api/src/routes/telegram.ts`, `apps/api/src/services/telegram/telegramLegacyMemoryStore.ts`, `apps/api/src/tests/telegramBotSettings.test.ts`, `apps/api/src/tests/telegramMessageRender.test.ts`, `apps/web/src/components/finance/CashRegisterModal.tsx`, `apps/web/src/components/finance/FastCheckoutModal.tsx`, `apps/web/src/components/schedule/QuickBookingDrawer.tsx`, `apps/web/src/components/schedule/ScheduleFilterStrip.tsx`, `apps/web/src/components/schedule/ScheduleGrid.tsx`.
- **Тесты**: `scripts/check-encoding.mjs`: 0 ошибок (UTF-8, 5187 файлов проверено), Single-Compiler Gate защищен (Мандат 8t), 326/326 фичей со статусом [ДА] (100% паритет).
