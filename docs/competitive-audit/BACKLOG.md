...
## 116. `идс_фолбэки::1_клик_подтверждение_на_бумаге_в_панелях_планшета_и_sms_otp` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8k, 8n, ФИЧА #126)
- **Идея**: Избавление врачей и пациентов от ступора при сбоях сенсорного экрана планшета, разрядке стилуса или задержках SMS-шлюза:
  1. *Инлайн fallback в панели стилуса*: в `consent-canvas-tools` добавлена кнопка `btn-stylus-paper-fallback` («На бумаге (1 клик)»), позволяющая мгновенно подписать согласие на бумажном носителе при пустом холсте без ручного переключения вкладок.
  2. *Инлайн fallback в карточке SMS OTP*: в `consent-otp-box` добавлена кнопка `btn-sms-paper-fallback` («На бумаге при сбое SMS (1 клик)»), предотвращающая задержку приёма в подвальных помещениях клиники без мобильной связи.
  3. *Пакетная обработка*: fallback корректно подписывает все документы активного пакета ИДС методом `paper_physical`.
- **Статус**:
  - Фронтенд: `apps/web/src/components/consents/InformedConsentModal.tsx` (коммит `75377d8ef`).
  - Тесты: `apps/web/src/components/consents/__tests__/informedConsentFallbackAutonomy.test.tsx` (5/5 passing).

---

## 117. `онбординг_автономия::свободное_сокрытие_и_черновик_мастера_настройки_без_блоков` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8n, ФИЧА #127)
...
## 118. `коммуникации_омниканал::постоянно_активное_поле_ввода_сообщений_независимо_от_sms_квоты` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8n, ФИЧА #128)
...
Все 63 канонические фичи из [`FEATURES_REGISTRY.md`](file:///C:/Clinic_MVP/dental-crm/docs/competitive-audit/FEATURES_REGISTRY.md) (IDENT, DentalPRO, iStom), а также 65 дополнительных системных аддендум-фич клинической автономии (Wave 15..26, фичи 64..128) имеют статус **`[РЕАЛИЗОВАНО]`**:
