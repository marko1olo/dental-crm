...
>
> ⚠️ **СТАТУС (2026-09-07 / WAVE 25): ВСЕ 63 ФИЧИ, 9 КИЛЛЕР-МОДУЛЕЙ И 116 КИЛЛЕР-ФИЧ АВТОНОМИИ ВРАЧА, КЛИНИЧЕСКИХ ПРЕСЕТОВ 1-КЛИКА И СНИЖЕНИЯ ТРЕНИЯ ПОЛНОСТЬЮ РЕАЛИЗОВАНЫ (ВСЕГО 150 ФИЧ: 63 КАНОНИЧЕСКИЕ + 87 АДДЕНДУМ).**  
...
## 138. `документы_валидация::автономия_врача_043у_без_зубных_строк` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8i, ФИЧА #148)
...
- **Статус**:
  ...
  - Тесты: `apps/web/src/tests/documentNavigationWorkflow.test.ts` (15 тестов, 100% pass).

---

## 139. `ортодонтия::дедупликация_фотопротокола_и_44px_тач_таргеты` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8c, 8d, ФИЧА #149)
...
- **Статус**:
  ...
  - Тесты: `apps/web/src/tests/OrthodonticPhotoProtocolModal.test.tsx` (7 тестов, 100% pass).

---

## 140. `эмк_043у::искоренение_soap_и_регламентные_разделы_минздрава` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8i, ФИЧА #150)
...
- **Статус**:
  ...
  - Тесты: `apps/web/src/components/emr/__tests__/emr043Export.test.ts` (13 тестов, 100% pass), `apps/web/src/components/emr/templates/__tests__/clinicalDiaryTemplatesEngine.test.ts` (18 тестов, 100% pass).
...
## 145. `смены_врача_смс::кокпит_десктоп_неблокирующая_валидация_смс_и_сессионный_пэп` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e & 8n, ФИЧА #147)
...
## 146. `перевод_пациента::активная_валидация_трансфера_без_мертвых_кнопок_и_тач_таргеты` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e & 8n, ФИЧА #148)
...
## 147. `касса_54фз::быстрый_чек_1_клик_автобалансировка_расхождений_и_снятие_блокировки` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8k, 8n, ФИЧА #149)
...
## 148. `онлайн_запись::активная_валидация_шагов_автовыбор_первого_слота_и_zero_friction` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8k, 8n, ФИЧА #150)
...
## 149. `документы_аннулирование::безопасные_дефолты_и_неблокирующая_кнопка_автономии` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e, 8k, 8n, ФИЧА #151)
...
## 153. `расписание_касса_54фз::суверенитет_соло_врача_и_1_клик_пресеты_без_сдачи_100_карта` [РЕАЛИЗОВАНО] -> KILLER (МАНДАТЫ 8e п. 8, п. 9, 8n, 8k)
...
- **Статус**:
  ...
  - Тесты: `apps/web/src/components/finance/__tests__/cashierAutonomy54Fz.test.ts` (18 тестов, 100% pass), `apps/web/src/components/schedule/__tests__/soloDoctorScheduleAutonomy.test.tsx` (3 теста), `QuickBookingDrawer.test.ts` (10 тестов), `AppointmentCard.test.tsx` (6 тестов).
