...
>
> ⚠️ **СТАТУС (2026-09-12 / WAVES 156–158 / PURGE OF DUPLICATES & CBCT ADAPTATION): ВСЕ 63 КАНОНИЧЕСКИЕ ФИЧИ, 9 КИЛЛЕР-МОДУЛЕЙ И 252 АДДЕНДУМ-ФИЧИ АВТОНОМИИ ВРАЧА, КЛИНИЧЕСКИХ ПРЕСЕТОВ 1-КЛИКА, КЛКТ И СНИЖЕНИЯ ТРЕНИЯ ПОЛНОСТЬЮ РЕАЛИЗОВАНЫ (ВСЕГО 315 ФИЧ: 63 КАНОНИЧЕСКИЕ + 252 АДДЕНДУМ).**  
> Проведена тотальная дедупликация и адаптация открытых математических движков: адаптирован модуль DenCT 3D CBCT (CPR панорама, трилинейное сэмплирование, Catmull-Rom автодетект дуги в 1 клик — коммит `124a5800d`), ликвидировано 7218 строк мертвого кода DICOM в API (`ab0bd1400`), канонизирован SSOT генерации справок ФНС КНД 1151156 (`7a5431010`), консолидирован EMR протокол и геометрия 3D имплантатов (`ed0f29012`), валидаторы СЭМД CDA Минздрава РФ сведены к `@dental/shared` (`c572020bf`), утилиты дат в web сведены к единому SSOT `dateTimeUtils.ts` (`00bff2892`), пропись сумм перенесена в SSOT `@dental/shared/src/moneyWordsRu.ts` (`474e3b901`), дублирующие движки автодетекции дуги объединены в `archDetectEngine.ts` (`49611adbe`), жесткая регистрация КТ со сканами Horn/ICP консолидирована в `scanRegistrationEngine.ts` (`bfaba5fa0`), папка `dmsManager/` консолидирована в `insurance/` (`2a98675aa`), устранен 1-й грех UI в тулбаре Cornerstone3D (`f9b792b8a`), декомпозирован монолит `CommonHelpers.ts` под 600 строк (`4a1330287`), дублирующие модалки кассы и профосмотров схлопнуты в канонические фасады (`ce982eb8a`), папка `components/patient/` объединена с `components/patients/` (`21088a3a5`), паразитные тесты-обертки ликвидированы (`1a765e82a`). Все системы покрыты автоматическими тестами, работают в production и строго соответствуют Высшей Конституции THE HAMMER и Мандатам 8a–8s. Повторная разработка запрещена (Мандаты 8g, 8h).
