...
- Optional Groq vision/text JSON extraction for price-list photos/OCR, with deterministic fallback and schema validation before anything reaches the catalog.
...
- `/api/system/local-bridges/readiness` also reports optional DICOM/CBCT worker, OCR worker, and OHIF viewer availability with redacted URLs and localhost/private-LAN default probing, so Settings can prepare heavy pixel/OCR handoff without cluttering the doctor's Visit screen;
- `/api/system/local-bridges/use-plans` maps CBCT/MPR, imaging import, document OCR, and price-photo OCR to local worker, Groq/server path, metadata-only preview, or manual review depending on the current workstation. It does not write imports or open diagnostic pixels by itself.
...
- Local discovery now uses workstation profiles and a priority queue for suspicious folders, so a noisy desktop/server disk can surface 1C/1Cv8, Инфоклиника, Cliniccards, Dental4Windows, Dental Pro, IDENT/StomX, Firebird/InterBase, Access, SQL Server, SQLite, Sidexis/Sirona, Romexis/Planmeca, Vatech/EzDent, Carestream/Kodak, OnDemand3D, Invivo, Cliniview, DBSWIN/VistaSoft, Digora/Soredex, Trophy/Visiodent, DTX Studio, 3Shape/Medit/exocad, DICOM/PACS, and RVG/OPG/XRay folders even before an admin manually browses into them;
- Workstation discovery also scans Start Menu/Desktop/common shortcut roots and can emit safe `workstation-profile:<fingerprint>` hints from `.lnk`/app-name evidence. It can also collect bounded workstation signals from configured hints and, on Windows, filtered process names, service names, and uninstall-registry DisplayName values matching known dental DB/imaging profiles; those become `workstation-signal:<fingerprint>` candidates. Both hint types are explicitly labeled as installed/system traces, not data folders: workup/probe can plan the right bridge/export path without pretending that a shortcut, process, service, or installed app record is a patient database;
- Workup/probe preserves safe vendor labels for profile hints and adds vendor-specific migration guidance: Romexis/Planmeca, Sidexis/Sirona, Vatech/EzDent, Carestream/Kodak, 1C/1Cv8, Firebird/InterBase, and common legacy MIS profiles get preferred export formats, data-folder cautions, bridge requirements, and the next action an administrator can execute without browsing blindly;
...
- `POST /api/imports/smart/clinic-public-lookup` builds a clinic-only public lookup from INN/ОГРН/КПП/name/address/license, optionally calls DaData when `DENTAL_DADATA_API_KEY` or `DADATA_API_KEY` is configured, and never accepts patient names, patient phones, birth dates, DICOM paths, DB paths, or pixels as public-search input;
...
- legacy DB/dump uploads (`.fdb`, `.gdb`, `.mdb`, `.accdb`, `.sqlite`, `.db`, `.dbf`, `.bak`, `.dump`, `.sql`, `.backup`) are not decoded as text; ingestion returns a redacted staging manifest for smart import and requires a local read-only bridge before patient/visit/payment/media extraction;
- returns safe route suggestions for smart import, patient import, imaging manifest, price-list analyzer, or plain-text review;
- returns extraction quality, confidence, suggested target, detected signals, extracted file list, and next action so admins know whether to open preview, review text manually, or run OCR/Groq vision first;
...
- If the selected route is `pricelist` and the file is an image, the web client preserves the compressed image for the price-list OCR/Groq vision path instead of replacing the working text with an empty placeholder.
