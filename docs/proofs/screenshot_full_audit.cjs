...
const VIEWS = [
	...
	// Onboarding preview
	{
		url: `${BASE}/#onboarding-preview`,
		name: "Onboarding_Wizard",
		states: ["PC_LIGHT", "PC_DARK", "MOBILE_LIGHT", "MOBILE_DARK"],
	},
];
	...
	for (const view of VIEWS) {
		for (const stateName of view.states) {
			...
			await page.setViewport({
				width: dim.w,
				height: dim.h,
				deviceScaleFactor: dim.scale,
			});

			await page
				.goto(view.url, { waitUntil: "networkidle2", timeout: 30000 })
				.catch(() => {});
			await wait(3000);
			...
			if (view.name.includes("Onboarding")) {
				await page.evaluate(() => {
					const c =
						document.getElementById("preset-card-solo_therapist") ||
